...
void StVecMesonHistoManger::InitSys(Int_t X_flag, Int_t mode) // 0 for Same Event, 1 for Mixed Event | mode is particle (phi-meson = 0) | study --> 0: pT/cent, 1: y, 2: |p|
{
  // for spectra 
  ...
  if(mStudy != 2) 
  {
    for(Int_t i_pt = 0; i_pt < vmsa::pt_total; i_pt++) // pt bin 
    {
      for(Int_t i_cent = vmsa::Cent_start; i_cent < vmsa::Cent_stop; i_cent++) // centrality bin
      {
        for(Int_t i_theta = 0; i_theta < 9; i_theta++) // cos(theta*) bins
        {
          for(Int_t i_phi = 0; i_phi < 12; i_phi++) // cos(theta*) bins
          {
            ...
            for(Int_t i_dca = vmsa::Dca_start; i_dca < vmsa::Dca_stop; i_dca++)
            {
              for(Int_t i_sig = vmsa::nSigKaon_start; i_sig < vmsa::nSigKaon_stop; i_sig++)
              {
                ...
                if(mStudy == 1) 
                {
                  ...
                  {
                    KEY_Mass2 = Form("pt_%d_eta_%d_Centrality_%d_CosThetaStar_%d_PhiPrime_%d_%s_Dca_%d_Sig_%d_%s_%s",i_pt,i_eta,i_cent,i_theta,i_phi,EP_string.c_str(),i_dca,i_sig,vmsa::mPID[mode].c_str(),Mode[X_flag].Data());
                ...
                if(mStudy == 3) 
                {
                  ...
                  {
                    KEY_Mass2 = Form("pt_%d_phipsi_%d_Centrality_%d_CosThetaStar_%d_PhiPrime_%d_%s_Dca_%d_Sig_%d_%s_%s",i_pt,i_phipsi,i_cent,i_theta,i_phi,EP_string.c_str(),i_dca,i_sig,vmsa::mPID[mode].c_str(),Mode[X_flag].Data());
                    //cout << KEY_Mass2 << endl;
                    h_mMass2[KEY_Mass2] = new TH1F(KEY_Mass2.Data(),KEY_Mass2.Data(),50,vmsa::InvMass_low[mode],vmsa::InvMass_high[mode]);
                    h_mMass2[KEY_Mass2]->Sumw2();
                  }
          ...
          for(Int_t i_phi = 0; i_phi < 12; i_phi++) // cos(theta*) bins
          {
            for(Int_t i_dca = vmsa::Dca_start; i_dca < vmsa::Dca_stop; i_dca++)
            {
              for(Int_t i_sig = vmsa::nSigKaon_start; i_sig < vmsa::nSigKaon_stop; i_sig++)
              {
                ...
                TString KEY_Mass2 = Form("p_%d_Centrality_%d_CosThetaStar_%d_PhiPrime_%d_%s_Dca_%d_Sig_%d_%s_%s",i_p,i_cent,i_theta,i_phi,EP_string.c_str(),i_dca,i_sig,vmsa::mPID[mode].c_str(),Mode[X_flag].Data());
                h_mMass2[KEY_Mass2] = new TH1F(KEY_Mass2.Data(),KEY_Mass2.Data(),100,vmsa::InvMass_low[mode],vmsa::InvMass_high[mode]);
                h_mMass2[KEY_Mass2]->Sumw2();
              }
            }
...
//-------------------------------------------------------------
void StVecMesonHistoManger::FillSys(Float_t pt, Float_t eta, Float_t phi, Float_t p, Int_t cent9, Float_t CosThetaStar, Float_t PhiPsi, Float_t Cos2PhiStarPhi, Float_t phiprime, Int_t dcaSys, Int_t nSigSys, Float_t Res2, Float_t InvMass, Double_t reweight, Int_t X_flag, Int_t mode)
{
  TString Mode[2] = {"SE","ME"};
  //if(Res2 > 0.0)
  //{
  ...
  if(Res2 > 0.0)
  {
    if(mStudy != 2) 
    {
      if(mStudy == 0) 
      {
        for(Int_t i_pt = 0; i_pt < vmsa::pt_total; i_pt++) // pt_bin
        {
          if(pt >= vmsa::ptRawStart[i_pt] && pt < vmsa::ptRawStop[i_pt])
          {
            for(Int_t i_theta = 0; i_theta < 9; i_theta++) // phi-psi2 bin
            {
              if(CosThetaStar >= (float(i_theta)-4.5)/4.5 && CosThetaStar < (float(i_theta)-3.5)/4.5)
              {
                for(Int_t i_phi = 0; i_phi < 12; i_phi++) // phi-psi2 bin
                {
                  if(phiprime >= TMath::Pi()*2.*float(i_phi)/12. && phiprime < TMath::Pi()*2.*float(i_phi+1.)/12.)
                  {
                    // spin alignment
      ...
      {
        if(pt >= vmsa::ptyRawStart[i_pt] && pt < vmsa::ptyRawStop[i_pt])
        {
          for(Int_t i_theta = 0; i_theta < 9; i_theta++) // phi-psi2 bin
          {
            if(CosThetaStar >= (float(i_theta)-4.5)/4.5 && CosThetaStar < (float(i_theta)-3.5)/4.5)
                ...
                if(phiprime >= TMath::Pi()*2.*float(i_phi)/12. && phiprime < TMath::Pi()*2.*float(i_phi+1.)/12.)
                {
                  ...
                  if(mStudy == 1) 
                  {
                    for(Int_t i_eta = 0; i_eta < vmsa::eta_total; i_eta++)
                    {
                      ...
                      {
                        KEY_Mass2 = Form("pt_%d_eta_%d_Centrality_%d_CosThetaStar_%d_PhiPrime_%d_%s_Dca_%d_Sig_%d_%s_%s",i_pt,i_eta,cent9,i_theta,i_phi,EP_string.c_str(),dcaSys,nSigSys,vmsa::mPID[mode].c_str(),Mode[X_flag].Data());
                        h_mMass2[KEY_Mass2]->Fill(InvMass,reweight);
                        if(cent9 >= vmsa::cent_low[0] && cent9 <= vmsa::cent_up[0]) // 20%-60%
                        {
                          TString KEY_Mass2Sys = Form("pt_%d_eta_%d_Centrality_9_CosThetaStar_%d_PhiPrime_%d_%s_Dca_%d_Sig_%d_%s_%s",i_pt,i_eta,i_theta,i_phi,EP_string.c_str(),dcaSys,nSigSys,vmsa::mPID[mode].c_str(),Mode[X_flag].Data());
                          h_mMass2[KEY_Mass2Sys]->Fill(InvMass,reweight);
                        }
                        break;
                      }
                  ...
                  if(mStudy == 3) 
                  {
                    for(Int_t i_phipsi = 0; i_phipsi < 12; i_phipsi++)
                    {
                      ...
                      {
                        KEY_Mass2 = Form("pt_%d_phipsi_%d_Centrality_%d_CosThetaStar_%d_PhiPrime_%d_%s_Dca_%d_Sig_%d_%s_%s",i_pt,i_phipsi,cent9,i_theta,i_phi,EP_string.c_str(),dcaSys,nSigSys,vmsa::mPID[mode].c_str(),Mode[X_flag].Data());
                        h_mMass2[KEY_Mass2]->Fill(InvMass,reweight);
                        if(cent9 >= vmsa::cent_low[0] && cent9 <= vmsa::cent_up[0]) // 20%-60%
                        {
                          TString KEY_Mass2Sys = Form("pt_%d_phipsi_%d_Centrality_9_CosThetaStar_%d_PhiPrime_%d_%s_Dca_%d_Sig_%d_%s_%s",i_pt,i_phipsi,i_theta,i_phi,EP_string.c_str(),dcaSys,nSigSys,vmsa::mPID[mode].c_str(),Mode[X_flag].Data());
                          h_mMass2[KEY_Mass2Sys]->Fill(InvMass,reweight);
                        }
                        break;
                      }
    ...
    if(mStudy == 2)
    {
      ...
      {
        if(pt >= vmsa::pRawStart[i_p] && pt < vmsa::pRawStop[i_p])
        {
          for(Int_t i_theta = 0; i_theta < 9; i_theta++) // phi-psi2 bin
          {
            if(CosThetaStar >= (float(i_theta)-4.5)/4.5 && CosThetaStar < (float(i_theta)-3.5)/4.5)
            {
              for(Int_t i_phi = 0; i_phi < 12; i_phi++) // phi-psi2 bin
              {
                ...
                {
                  // spin alignment
                  TString KEY_Mass2 = Form("p_%d_Centrality_%d_CosThetaStar_%d_PhiPrime_%d_%s_Dca_%d_Sig_%d_%s_%s",i_p,cent9,i_theta,i_phi,EP_string.c_str(),dcaSys,nSigSys,vmsa::mPID[mode].c_str(),Mode[X_flag].Data());
                  h_mMass2[KEY_Mass2]->Fill(InvMass,reweight);
                  if(cent9 >= vmsa::cent_low[0] && cent9 <= vmsa::cent_up[0]) // 20%-60%
                  {
                    TString KEY_Mass2Sys = Form("p_%d_Centrality_9_CosThetaStar_%d_PhiPrime_%d_%s_Dca_%d_Sig_%d_%s_%s",i_p,i_theta,i_phi,EP_string.c_str(),dcaSys,nSigSys,vmsa::mPID[mode].c_str(),Mode[X_flag].Data());
                    h_mMass2[KEY_Mass2Sys]->Fill(InvMass,reweight);
                  }
                  break;
                }
...
void StVecMesonHistoManger::WriteSys(Int_t X_flag, Int_t mode)
{
  ...
  if(mStudy != 2)
  {
    for(Int_t i_pt = 0; i_pt < vmsa::pt_total; i_pt++) // pt bin
    {
      for(Int_t i_cent = vmsa::Cent_start; i_cent < vmsa::Cent_stop; i_cent++) // centrality bin
      {
        for(Int_t i_theta = 0; i_theta < 9; i_theta++) // cos(theta*) bin
        {
          for(Int_t i_phi = 0; i_phi < 12; i_phi++) // cos(theta*) bin
          {
            for(Int_t i_dca = vmsa::Dca_start; i_dca < vmsa::Dca_stop; i_dca++)
            {
              for(Int_t i_sig = vmsa::nSigKaon_start; i_sig < vmsa::nSigKaon_stop; i_sig++)
              {
                if( i_dca != 0 && i_sig != 0) continue;
                TString KEY_Mass2 = Form("pt_%d_Centrality_%d_CosThetaStar_%d_PhiPrime_%d_%s_Dca_%d_Sig_%d_%s_%s",i_pt,i_cent,i_theta,i_phi,EP_string.c_str(),i_dca,i_sig,vmsa::mPID[mode].c_str(),Mode[X_flag].Data());
                if(mStudy == 0) h_mMass2[KEY_Mass2]->Write();
