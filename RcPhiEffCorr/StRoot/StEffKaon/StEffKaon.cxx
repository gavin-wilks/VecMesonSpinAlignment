...
StEffKaon::StEffKaon(int Energy, long StartEvent, long StopEvent, int PID, int year, int cut)
{
  ...
  string InPutList = Form("/star/data01/pwg/sunxuhit/AuAu%s/SpinAlignment/Embedding/%s/List/%s/%s_StMcEvent.list",vmsa::mBeamEnergy[mEnergy].c_str(),vmsa::mParType[mPID].c_str(),vmsa::mYear[year].c_str(),vmsa::mParType[mPID].c_str());
  ...
  string OutPutFile = Form("/star/data01/pwg/sunxuhit/AuAu%s/SpinAlignment/Embedding/%s/Efficiency/Eff_%s_StMcEvent_%s_%s.root",vmsa::mBeamEnergy[mEnergy].c_str(),vmsa::mParType[mPID].c_str(),vmsa::mBeamEnergy[mEnergy].c_str(),vmsa::mYear[year].c_str(),vmsa::mCuts[cut].c_str());
