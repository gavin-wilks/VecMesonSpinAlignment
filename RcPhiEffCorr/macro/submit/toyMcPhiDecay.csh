...
if (  "$SLURM_JOB_PARTITION" =~ *"chos"  ) then  
    ...
    echo  starting phi-meson spin alighment analysis with execName=$EXEC_NAME, workerName=`hostname -f`, startDate=`date`
    ...
    cd ${WRK_DIR}/VecMesonSpinAlignment/RcPhiEffCorr/macro
    echo `pwd`
    /usr/bin/time -v  $EXEC_NAME -b -q toyMcPhiDecay.C\($energy,$pid,$year,$cut,$NMax,${SLURM_ARRAY_TASK_ID}\) >& $LOG_PATH/McPhi${Energy}_${SLURM_ARRAY_JOB_ID}_${SLURM_ARRAY_TASK_ID}.log
