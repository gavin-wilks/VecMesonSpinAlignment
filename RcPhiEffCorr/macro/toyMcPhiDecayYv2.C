...
double pt_low[2]  = {1.0,2.0};
double pt_high[2] = {2.0,5.0};
...
std::string etastring[4] = {"eta0p4","eta0p6","eta0p8","eta1_eta1"};

TF1 *f_pDel;
TF1 *f_res;

TH1D *v2y = new TH1D("v2y","v2y",10,-1,1);

void toyMcPhiDecayYv2(const int energy = 4, int ptbin = 0, const int centrality = 3, const int pid = 0, const int year = 0, const int cut = 0, const int NMax = 100, int mode = 1, int fitmode = 0, const char* jobID = "20221209") 
{
  // mode = 0 --> pT depedence (20-60%), mode = 1 --> centrality depedence (1.0 < pT < 5.0) GeV/c
  // fitmode = 0 --> fit to plateau, fitmode = 1 --> fit to eta
  ...
  int centFile[9] = {0,0,0,0,1,1,1,2,2};
  ...
  int etaindex = 0;
  // if(mEtaMode == 0) etaindex = 3;
  // if(mEtaMode == 3) etaindex = 0;
  // if(mEtaMode == 4) etaindex = 1;
  // if(mEtaMode == 5) etaindex = 2;

  if((energy == 4 || energy == 3 || energy == 2 || energy == 0) && mMode == 1) InPutV2 = Form("/gpfs01/star/pwg/gwilks3/VectorMesonSpinAlignment/Data/Phi/v2/V2_Rapidity_SysErrors_eta1_eta1.root");
  TFile *File_v2 = TFile::Open(InPutV2.c_str());
  std::cout << "v2 file: " << InPutV2 << endl;

  TGraphAsymmErrors *gv2y; 
  if((energy == 4 || energy == 3 || energy == 2 || energy == 0) && mMode == 1) 
  {
    gv2y = (TGraphAsymmErrors*) File_v2->Get(Form("g_v2_19GeV_Phi_pt%d_cent%d_StatError",mPtBin,centFile[centrality]));
    cout << "Loading " << Form("g_v2_19GeV_Phi_pt%d_cent%d_StatError",mPtBin,centFile[centrality]) << endl;
    gv2y->Print();
    for(int i = 0+2; i < 14-2; i++)
    {
      double y = 0.0, v2 = 0.0;
      gv2y->GetPoint(i,y,v2);
      cout << "i = " << i << " y = " << y << " v2 = " << v2 << endl;
      v2y->SetBinContent(i-1,v2);
      cout << v2y->GetBinContent(i-1) << endl;;
    }
  }
...
void getKinematics(TLorentzVector& lPhi, double const mass)
{
 ...
 // cout << "Before getv2()" << endl;
  f_flow->SetParameter(0,getv2(y));
  //cout << "y = " << y << "  v2 = " << getv2(y) <<  endl;
//  cout << "After getv2()" << endl;
  double const phi = f_flow->GetRandom(); //+ mPsi2; // sample with measured v2
...
bool tpcReconstructed(int iParticleIndex, int cent, float Psi2, TLorentzVector const& lKaon)
{
   ...
   if(fabs(lKaon.Eta()) <= 1.0) findHist_ToF(lKaon,iParticleIndex,EtaBin_ToF,PhiBin_ToF);
...
double  getv2(double y){
  //double bin = v2y->FindBin(y);
  //double v2 = v2y->GetBinContent(bin);
  //cout << bin << "     v2 = " << v2 << endl;
  return v2y->GetBinContent(v2y->FindBin(y)); 
}
