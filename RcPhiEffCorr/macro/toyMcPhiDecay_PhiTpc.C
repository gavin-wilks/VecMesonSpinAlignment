...
bool tpcReconstructed(int iParticleIndex, int cent, float Psi2, TLorentzVector* lPhi);
bool tofReconstructed(int iParticleIndex, int cent, float Psi2, TLorentzVector const& lKaon);
void findHist(TLorentzVector* lPhi, int iParticleIndex, float Psi2, int& EtaBin, int& PhiBin); // iParticleIndex = 0 => K+
...
double pt_low[6]  = {1.0,2.0,3.0,2.0,1.0,1.2};
double pt_high[6] = {3.0,4.0,5.0,5.0,5.0,4.2};
...
void toyMcPhiDecay_PhiTpc(const int energy = 4, int ptbin = 5, const int centrality = 3, const int pid = 0, const int NMax = 10000, int mode = 1, int fitmode = 0, const char* jobID = "20240625") 
{
...
void getKinematics(TLorentzVector& lPhi, double const mass, int energy, int centrality)
{
  ...
  //if (mMode == 1 ) double const pt = gRandom->Uniform(pt_low[mPtBin], pt_high[mPtBin]); // sample with measured spectra
...
void decayAndFill(int centrality, int const kf, TLorentzVector* lPhi, TClonesArray& daughters)
{
  ...
  bool kplusflag = false;
  bool kminusflag = false;
...
void fill(int centrality, int const kf, TLorentzVector* lPhi, TLorentzVector const& lKplus, TLorentzVector const& lKminus)
{
  ...
  arr[iArr++] = lRcPhi.Phi();
  arr[iArr++] = lRcPhi.M();
  arr[iArr++] = tpcReconstructed(0,centrality,mPsi2,lPhi);
...
bool tpcReconstructed(int iParticleIndex, int cent, float Psi2, TLorentzVector* lPhi)
{
   //if(fabs(lPhi.Eta()) >= vmsa::mEtaMax) return false;

   TH1D *h_TPC = NULL;
   int EtaBin_TPC = -1;
   int PhiBin_TPC = -1;
   findHist(lPhi,iParticleIndex,Psi2,EtaBin_TPC,PhiBin_TPC);
...
void findHist(TLorentzVector* lPhi, int iParticleIndex, float Psi2, int& EtaBin, int& PhiBin)
{
  float eta = lPhi->Eta();
...
void readEfficiency(int energy, int year, int cut, const char *jobID)
{
  ...
  string inputPhi = Form("/star/data01/pwg/gwilks3/VectorMesonSpinAlignment/Data/Phi/Efficiency/TPC/Eff_Phi_%s.root",vmsa::mBeamEnergy[energy].c_str());
  ...
  h_FrameEta[0] = (TH1D*)File_Phi->Get("h_FrameEta");
  h_FramePhi[0] = (TH1D*)File_Phi->Get("h_FramePhi");
  ...
  string outputfile = Form("./Eff_%s_Phi_cent%d_pt%d_%s.root",vmsa::mBeamEnergy[energy].c_str(),mCentrality,mPtBin,jobID);
  ...
  const char* varlist = "Centrality:PsiRP:Psi1:Psi2:McPt:McP:McEta:McY:McPhi:McInvMass:McPid:" // MC phi 
                        ...
                        "KpRcPt:KpRcEta:KpRcY:KpRcPhi:KpRcM:KpRcTof:" // RC K+
                        "KmMcPt:KmMcEta:KmMcY:KmMcPhi:KmMcM:KmMcPid:" // MC K-
                        "KmRcPt:KmRcEta:KmRcY:KmRcPhi:KmRcM:KmRcTof:" // RC K-
                        "RcPt:RcP:RcEta:RcY:RcPhi:RcInvMass:RcTpc"; // reconstructed phi
...
void readTofEff(int energy)
{
  ...
  for(int i_cent = 0; i_cent < 10; ++i_cent)
  {
    for(int i_eta = 0; i_eta < h_FrameEta_ToF[0]->GetNbinsX()/*vmsa::BinEta*/; ++i_eta)
    {
      string KEY;
      ...
      for(int i_phi = 0; i_phi < h_FramePhi_ToF[0]->GetNbinsX()/*vmsa::BinPhi*/; ++i_phi)
      {
	// string KEY;
...
void readTofEffFit(int energy, int i_cent)
{
  ...
  //for(int i_cent = 0; i_cent < 10; ++i_cent)
  //{
  for(int i_eta = 0; i_eta < h_FrameEta_ToF[0]->GetNbinsX()/*vmsa::BinEta*/; ++i_eta) // eta
  {
    TH1D *h_Kplus = NULL;
    string KEY;
  ...
  for(int i_eta = 0; i_eta < h_FrameEta_ToF[0]->GetNbinsX() /*vmsa::BinEta*/; ++i_eta) // eta & phi
  {
    for(int i_phi = 0; i_phi < h_FramePhi_ToF[0]->GetNbinsX() /*vmsa::BinPhi*/; ++i_phi)
    {
      TH1D *h_Kplus = NULL;
...
TF1* readv2(int energy, int pid, int centrality){
  ...
  TF1 *f_v2temp = new TF1("f_v2",v2_pT_FitFunc,vmsa::ptMin,vmsa::ptMax,5);
  f_v2temp->FixParameter(0,2);
  f_v2temp->SetParameter(1,0.1);
  f_v2temp->SetParameter(2,0.1);
  f_v2temp->SetParameter(3,0.1);
  f_v2temp->SetParameter(4,0.1);
  f_v2temp->SetLineColor(2);
  f_v2temp->SetLineWidth(2);
  f_v2temp->SetLineStyle(2);
  cout << "Fitting v2" << endl;
  g_v2->Fit(f_v2temp,"N");
...
TF1* readspec(int energy, int pid, int centrality)
{
  ...
  if(energy != 3)
  {
    ...
    TF1 *f_spectemp = new TF1("f_spec",pTLevy,pt_low[mPtBin], pt_high[mPtBin], 3);
    f_spectemp->SetParameter(0,f_Levy->GetParameter(0));
    f_spectemp->SetParameter(1,f_Levy->GetParameter(1));
    f_spectemp->SetParameter(2,f_Levy->GetParameter(2));
    f_spectemp->SetLineStyle(2);
    f_spectemp->SetLineColor(2);
    f_spectemp->SetLineWidth(2);
...
*/  
  ...
  if(energy == 3)
  {
    f_spectemp = new TF1("f_specExp",pTspecExp, pt_low[mPtBin], pt_high[mPtBin], 2);
    f_spectemp->SetParameter(0,dNdy14[centrality]);
    f_spectemp->SetParameter(1,Texp14[centrality]);
