...
void toyMcPhiDecay(const int energy = 0, int ptbin = 4, const int centrality = 5, const int pid = 0, const int NMax = 10000, int mode = 1, int fitmode = 0, const char* jobID = "20221209") 
{
  ...
  //mRes = TMath::Sqrt(0.25);
  mRes = 0.7;//TMath::Sqrt(0.25);
  cout << "Resolution2 = " << mRes << endl;
  f_res = new TF1("resolution",EventPlaneResolution,0,80,0);
  mChi = f_res->GetX(mRes);

  cout << "mChi = " << mChi << endl;
  //f_pDel = new TF1("deltaPsi",EventPlaneDist,-3.*TMath::Pi()/2.0,3.*TMath::Pi()/2.0,2);
  f_pDel = new TF1("deltaPsi",EventPlaneDist,-TMath::Pi()/2.0,TMath::Pi()/2.0,2);
  //f_pDel = new TF1("deltaPsi",EventPlaneDist,-TMath::Pi(),TMath::Pi(),2);
  ...
  cout << "cos(2delta) = " << cos2delta/sum << endl;
  cout << "cos(delta)  = " << cosdelta/sum << endl;
  cout << "sin(2delta) = " << sin2delta/sum << endl;
  cout << "sin(delta)  = " << sindelta/sum << endl;
...
void getKinematics(TLorentzVector& lPhi, double const mass, int energy, int centrality)
{
  ...
  //mPsi2 = (delta-Psi)/2.; // EP, the RP is smeared by delta
  //mPsi2RP = Psi2;       // RP
  ...
  cos2delta += TMath::Cos(2.0*(Psi-mPsi2));
  cosdelta += TMath::Cos((Psi-mPsi2));
  sin2delta += TMath::Sin(2.0*(Psi-mPsi2));
  sindelta += TMath::Sin((Psi-mPsi2));
  sum += 1;
  //cos2delta1 = 0;
}
