...
void pTinterpolation()
{
  ...
  {
    std::string outputname = Form("figures/ptLevyFits_en%d.pdf",ien);
    ...
    { 
      ...
      for(int ipar = 0; ipar < 3; ipar++)
      {
        ...
        g_par[icent][ipar]->SetPoint(ien, snn[ien], f_Levy->GetParameter(ipar));
        g_par[icent][ipar]->SetPointError(ien, 0.0, 0.0, f_Levy->GetParError(ipar), f_Levy->GetParError(ipar));
      }
  ...
  for(int ipar = 0; ipar < 3; ipar++)
  {
    std::string outputname = Form("figures/pt_par%d.pdf",ipar);
    std::string output_start = Form("%s[",outputname.c_str());
