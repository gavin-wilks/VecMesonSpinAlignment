...
if (  "$SLURM_JOB_PARTITION" =~ *"chos"  ) then  
    ...
    echo  starting ${pid} embedding analysis with PATH_XRD=$PATH_XRD, daqN=$daqN, output=$output, execName=$EXEC_NAME, workerName=`hostname -f`, startDate=`date`
    ...
    cd ${WRK_DIR}/VecMesonSpinAlignment/Embedding
    echo `pwd`
    /usr/bin/time -v  $EXEC_NAME -b -q run_StMcAnalysisMaker.C\(\"$daqN\",\"$output\"\) >& $LOG_PATH/${pid}${Energy}_${SLURM_ARRAY_JOB_ID}_${SLURM_ARRAY_TASK_ID}.log
