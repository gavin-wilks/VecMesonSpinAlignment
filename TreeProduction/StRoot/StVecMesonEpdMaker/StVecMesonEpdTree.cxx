...
ClassImp(StVecMesonEpdTree)

//------------------------------------------------------------------------------------------------------------------
StVecMesonEpdTree::StVecMesonEpdTree(Int_t energy)
{
  mEnergy = energy;
}

StVecMesonEpdTree::~StVecMesonEpdTree()
{
  /* */
}
...
void StVecMesonEpdTree::InitPhi()
{
  ...
  for(Int_t cent = 0; cent < vmsa::Bin_Centrality; cent++)
  {
    ...
    {
      for(Int_t phi_psi = 0; phi_psi < vmsa::Bin_Phi_Psi1; phi_psi++)
      {
...
void StVecMesonEpdTree::InitRho()
{
  mVecMesonCut = new StVecMesonEpdCut(mEnergy);
  TString HistName = "Mass2_pt";
...
void StVecMesonEpdTree::InitKStar()
{
  mVecMesonCut = new StVecMesonEpdCut(mEnergy);
  TString HistName = "Mass2_pt";
...
void StVecMesonEpdTree::WriteMass2Phi()
{
  h_Mass2->Write();
  h_KdEdxRig->Write();
  h_KM2Rig->Write();
...
void StVecMesonEpdTree::WriteMass2Rho()
{
  h_Mass2->Write();
  h_PidEdxRig->Write();
  h_PiM2Rig->Write();
...
void StVecMesonEpdTree::WriteMass2KStar()
{
  h_Mass2->Write();
  h_KdEdxRig->Write();
  h_KM2Rig->Write();
...
void StVecMesonEpdTree::clear_phi(Int_t cent9, Int_t Bin_vz, Int_t Bin_Psi1)
{
  mPrimaryvertex[cent9][Bin_vz][Bin_Psi1].clear();
  mRefMult[cent9][Bin_vz][Bin_Psi1].clear();
  mCentrality[cent9][Bin_vz][Bin_Psi1].clear();
  mRunId[cent9][Bin_vz][Bin_Psi1].clear();
  mEventId[cent9][Bin_vz][Bin_Psi1].clear();
  mN_prim[cent9][Bin_vz][Bin_Psi1].clear();
  mN_non_prim[cent9][Bin_vz][Bin_Psi1].clear();
  mN_Tof_match[cent9][Bin_vz][Bin_Psi1].clear();
  mZDCx[cent9][Bin_vz][Bin_Psi1].clear();
  mBBCx[cent9][Bin_vz][Bin_Psi1].clear();
  ...
  mNumTrackFullEast[cent9][Bin_vz][Bin_Psi1].clear();
  mNumTrackFullWest[cent9][Bin_vz][Bin_Psi1].clear();
  ...
  mEventCounter2[cent9][Bin_vz][Bin_Psi1] = 0;
}

void StVecMesonEpdTree::clear_rho(Int_t cent9, Int_t Bin_vz, Int_t Bin_Psi1)
{
  mPrimaryvertex[cent9][Bin_vz][Bin_Psi1].clear();
  mRefMult[cent9][Bin_vz][Bin_Psi1].clear();
  mCentrality[cent9][Bin_vz][Bin_Psi1].clear();
...
void StVecMesonEpdTree::clear_kstar(Int_t cent9, Int_t Bin_vz, Int_t Bin_Psi1)
{
  mPrimaryvertex[cent9][Bin_vz][Bin_Psi1].clear();
  mRefMult[cent9][Bin_vz][Bin_Psi1].clear();
  mCentrality[cent9][Bin_vz][Bin_Psi1].clear();
  ...
  for(Int_t Bin_Event = 0; Bin_Event < vmsa::Buffer_depth; Bin_Event++)
  {
    ...
    {
      MEKey key = MEKey(cent9,Bin_vz,Bin_Psi1,Bin_Event,id);
      mHelix[key].clear();
      mMomentum[key].clear();
      mMass2[key].clear();
      mDca[key].clear();
...
void StVecMesonEpdTree::size_phi(Int_t cent9, Int_t Bin_vz, Int_t Bin_Psi1)
{
  LOG_INFO << "Event Buffer: Centrality = " << cent9 << ", VertexZ = " << Bin_vz << ", Psi1 = " << Bin_Psi1 << endm;
  LOG_INFO << "Buffer_depth = " << mEventCounter2[cent9][Bin_vz][Bin_Psi1] << endm;

  LOG_INFO << "Size of primaryVertex = " << mPrimaryvertex[cent9][Bin_vz][Bin_Psi1].size() << endm;;
  LOG_INFO << "Size of refMult       = " << mRefMult[cent9][Bin_vz][Bin_Psi1].size() << endm;;
  LOG_INFO << "---------------------------------------------------------------------------" << endm;

  for(Int_t Bin_Event = 0; Bin_Event < mEventCounter2[cent9][Bin_vz][Bin_Psi1]; Bin_Event++)
  {
    MEKey key = MEKey(cent9,Bin_vz,Bin_Psi1,Bin_Event,0);
    ...
    key = MEKey(cent9,Bin_vz,Bin_Psi1,Bin_Event,1);
 ...
 void StVecMesonEpdTree::size_rho(Int_t cent9, Int_t Bin_vz, Int_t Bin_Psi1)
{
  LOG_INFO << "Event Buffer: Centrality = " << cent9 << ", VertexZ = " << Bin_vz << ", Psi1 = " << Bin_Psi1 << endm;
  LOG_INFO << "Buffer_depth = " << mEventCounter2[cent9][Bin_vz][Bin_Psi1] << endm;
...
void StVecMesonEpdTree::size_kstar(Int_t cent9, Int_t Bin_vz, Int_t Bin_Psi1)
{
  LOG_INFO << "Event Buffer: Centrality = " << cent9 << ", VertexZ = " << Bin_vz << ", Psi1 = " << Bin_Psi1 << endm;
  LOG_INFO << "Buffer_depth = " << mEventCounter2[cent9][Bin_vz][Bin_Psi1] << endm;
  ...
  for(Int_t Bin_Event = 0; Bin_Event < mEventCounter2[cent9][Bin_vz][Bin_Psi1]; Bin_Event++)
  {
    ...
    key = MEKey(cent9,Bin_vz,Bin_Psi1,Bin_Event,2);
    LOG_INFO << "Event Number " << Bin_Event << ":" << endm; 
    LOG_INFO << "Positive Pion:" << endm;
    LOG_INFO << "  Size of Helix        = " << mHelix[key].size() << endm;;
    LOG_INFO << "  Size of Momentum     = " << mMomentum[key].size() << endm;
    ...
    key = MEKey(cent9,Bin_vz,Bin_Psi1,Bin_Event,3);
    LOG_INFO << "Negative Pion:" << endm;
    LOG_INFO << "  Size of Helix        = " << mHelix[key].size() << endm;
    LOG_INFO << "  Size of Momentum     = " << mMomentum[key].size() << endm;
    LOG_INFO << "  Size of Mass2        = " << mMass2[key].size() << endm;
...
void StVecMesonEpdTree::doPhi(Int_t Flag_ME, Int_t cent9, Int_t Bin_vz, Int_t Bin_Psi1) // 0: Same Event, 1: Mix Event
{
  if(Flag_ME == 0) // same event
  {
    for(Int_t Bin_Event = 0; Bin_Event < mEventCounter2[cent9][Bin_vz][Bin_Psi1]; Bin_Event++)
    {
      ...
      mMesonEvent->setPrimaryVertex(mPrimaryvertex[cent9][Bin_vz][Bin_Psi1][Bin_Event]);
      mMesonEvent->setRunId(mRunId[cent9][Bin_vz][Bin_Psi1][Bin_Event]);
      mMesonEvent->setEventId(mEventId[cent9][Bin_vz][Bin_Psi1][Bin_Event]);
      mMesonEvent->setRefMult(mRefMult[cent9][Bin_vz][Bin_Psi1][Bin_Event]);
      mMesonEvent->setCentrality(mCentrality[cent9][Bin_vz][Bin_Psi1][Bin_Event]);
      mMesonEvent->setN_prim(mN_prim[cent9][Bin_vz][Bin_Psi1][Bin_Event]);
      mMesonEvent->setN_non_prim(mN_non_prim[cent9][Bin_vz][Bin_Psi1][Bin_Event]);
      mMesonEvent->setN_Tof_match(mN_Tof_match[cent9][Bin_vz][Bin_Psi1][Bin_Event]);
      // QVector
      ...
      mMesonEvent->setNumTrackWest(mNumTrackWest[cent9][Bin_vz][Bin_Psi1][Bin_Event]);
      mMesonEvent->setNumTrackFull(mNumTrackFull[cent9][Bin_vz][Bin_Psi1][Bin_Event]);
      mMesonEvent->setNumTrackFullEast(mNumTrackFullEast[cent9][Bin_vz][Bin_Psi1][Bin_Event]);
      mMesonEvent->setNumTrackFullWest(mNumTrackFullWest[cent9][Bin_vz][Bin_Psi1][Bin_Event]);

      mMesonEvent->setZDCx(mZDCx[cent9][Bin_vz][Bin_Psi1][Bin_Event]);
      mMesonEvent->setBBCx(mBBCx[cent9][Bin_vz][Bin_Psi1][Bin_Event]);
      mMesonEvent->setVzVpd(mVzVpd[cent9][Bin_vz][Bin_Psi1][Bin_Event]);
   
      // start to select phi candidate in a event
      MEKey key_plus  = MEKey(cent9,Bin_vz,Bin_Psi1,Bin_Event,0);
      MEKey key_minus = MEKey(cent9,Bin_vz,Bin_Psi1,Bin_Event,1);
      ...
      {
	StThreeVectorF p_vecA = mHelix[key_plus][n_kplus].cat(mHelix[key_plus][n_kplus].pathLength(mPrimaryvertex[cent9][Bin_vz][Bin_Psi1][Bin_Event]));  // primary momentum
	...
	{
	  StThreeVectorF p_vecB = mHelix[key_minus][n_kminus].cat(mHelix[key_minus][n_kminus].pathLength(mPrimaryvertex[cent9][Bin_vz][Bin_Psi1][Bin_Event]));  // primary momentum
  ...
  if(Flag_ME == 1) // mixed event
  {
    ...
    for(Int_t Bin_Event_A = 0; Bin_Event_A < mEventCounter2[cent9][Bin_vz][Bin_Psi1]-1; Bin_Event_A++)
    {
      MEKey key_A_plus  = MEKey(cent9,Bin_vz,Bin_Psi1,Bin_Event_A,0);
      MEKey key_A_minus = MEKey(cent9,Bin_vz,Bin_Psi1,Bin_Event_A,1);
      for(Int_t Bin_Event_B = Bin_Event_A+1; Bin_Event_B < mEventCounter2[cent9][Bin_vz][Bin_Psi1]; Bin_Event_B++)
      {
	MEKey key_B_plus  = MEKey(cent9,Bin_vz,Bin_Psi1,Bin_Event_B,0);
	MEKey key_B_minus = MEKey(cent9,Bin_vz,Bin_Psi1,Bin_Event_B,1);
	...
	{
	  StThreeVectorF p_vecA(mHelix[key_A_plus][n_kplus].cat(mHelix[key_A_plus][n_kplus].pathLength(mPrimaryvertex[cent9][Bin_vz][Bin_Psi1][Bin_Event_A])));  // primary momentum
	  ...
	  {
	    StThreeVectorF p_vecB(mHelix[key_B_minus][n_kminus].cat(mHelix[key_B_minus][n_kminus].pathLength(mPrimaryvertex[cent9][Bin_vz][Bin_Psi1][Bin_Event_B])));  // primary momentum
	...
	{
	  StThreeVectorF p_vecA(mHelix[key_A_minus][n_kminus].cat(mHelix[key_A_minus][n_kminus].pathLength(mPrimaryvertex[cent9][Bin_vz][Bin_Psi1][Bin_Event_A])));  // primary momentum
	  ...
	  {
	    StThreeVectorF p_vecB(mHelix[key_B_plus][n_kplus].cat(mHelix[key_B_plus][n_kplus].pathLength(mPrimaryvertex[cent9][Bin_vz][Bin_Psi1][Bin_Event_B])));  // primary momentum
...
void StVecMesonEpdTree::doRho(Int_t Flag_ME, Int_t cent9, Int_t Bin_vz, Int_t Bin_Psi1) // 0: Same Event, 1: Mix Event
{
  if(Flag_ME == 0) // same event
  {
    for(Int_t Bin_Event = 0; Bin_Event < mEventCounter2[cent9][Bin_vz][Bin_Psi1]; Bin_Event++)
    {
      ...
      {
	StThreeVectorF p_vecA = mHelix[key_plus][n_piplus].cat(mHelix[key_plus][n_piplus].pathLength(mPrimaryvertex[cent9][Bin_vz][Bin_Psi1][Bin_Event]));  // primary momentum
	...
	{
	  StThreeVectorF p_vecB = mHelix[key_minus][n_piminus].cat(mHelix[key_minus][n_piminus].pathLength(mPrimaryvertex[cent9][Bin_vz][Bin_Psi1][Bin_Event]));  // primary momentum
  ...
  if(Flag_ME == 1) // mixed event
  {
	...
	{
	  StThreeVectorF p_vecA(mHelix[key_A_plus][n_piplus].cat(mHelix[key_A_plus][n_piplus].pathLength(mPrimaryvertex[cent9][Bin_vz][Bin_Psi1][Bin_Event_A])));  // primary momentum
	  ...
	  {
	    StThreeVectorF p_vecB(mHelix[key_B_minus][n_piminus].cat(mHelix[key_B_minus][n_piminus].pathLength(mPrimaryvertex[cent9][Bin_vz][Bin_Psi1][Bin_Event_B])));  // primary momentum
	...
	{
	  StThreeVectorF p_vecA(mHelix[key_A_minus][n_piminus].cat(mHelix[key_A_minus][n_piminus].pathLength(mPrimaryvertex[cent9][Bin_vz][Bin_Psi1][Bin_Event_A])));  // primary momentum
	  ...
	  {
	    StThreeVectorF p_vecB(mHelix[key_B_plus][n_piplus].cat(mHelix[key_B_plus][n_piplus].pathLength(mPrimaryvertex[cent9][Bin_vz][Bin_Psi1][Bin_Event_B])));  // primary momentum
...
void StVecMesonEpdTree::doKStar(Int_t Flag_ME, Int_t cent9, Int_t Bin_vz, Int_t Bin_Psi1) // 0: Same Event, 1: Mix Event
{
  if(Flag_ME == 0) // same event
  {
    for(Int_t Bin_Event = 0; Bin_Event < mEventCounter2[cent9][Bin_vz][Bin_Psi1]; Bin_Event++)
    {
      ...
      // start to select phi candidate in a event
      MEKey key_kplus   = MEKey(cent9,Bin_vz,Bin_Psi1,Bin_Event,0); //K+
      MEKey key_kminus  = MEKey(cent9,Bin_vz,Bin_Psi1,Bin_Event,1); //K-
      MEKey key_piplus  = MEKey(cent9,Bin_vz,Bin_Psi1,Bin_Event,2); //pi+
      MEKey key_piminus = MEKey(cent9,Bin_vz,Bin_Psi1,Bin_Event,3); //pi-

      TLorentzVector ltrackA, ltrackB;
      for(Int_t n_kplus = 0; n_kplus < mHelix[key_kplus].size(); n_kplus++) // first track loop over K+ candidates
      {
	StThreeVectorF p_vecA = mHelix[key_kplus][n_kplus].cat(mHelix[key_kplus][n_kplus].pathLength(mPrimaryvertex[cent9][Bin_vz][Bin_Psi1][Bin_Event]));  // primary momentum
	...
	{
	  StThreeVectorF p_vecB = mHelix[key_piminus][n_piminus].cat(mHelix[key_piminus][n_piminus].pathLength(mPrimaryvertex[cent9][Bin_vz][Bin_Psi1][Bin_Event]));  // primary momentum
      ...
      {
	StThreeVectorF p_vecA = mHelix[key_piplus][n_piplus].cat(mHelix[key_piplus][n_piplus].pathLength(mPrimaryvertex[cent9][Bin_vz][Bin_Psi1][Bin_Event]));  // primary momentum
	...
	{
	  StThreeVectorF p_vecB = mHelix[key_kminus][n_kminus].cat(mHelix[key_kminus][n_kminus].pathLength(mPrimaryvertex[cent9][Bin_vz][Bin_Psi1][Bin_Event]));  // primary momentum
  ...
  if(Flag_ME == 1) // mixed event
  {
    for(Int_t Bin_Event_A = 0; Bin_Event_A < mEventCounter2[cent9][Bin_vz][Bin_Psi1]-1; Bin_Event_A++)
    {
      MEKey key_A_kplus   = MEKey(cent9,Bin_vz,Bin_Psi1,Bin_Event_A,0); //K+
      MEKey key_A_kminus  = MEKey(cent9,Bin_vz,Bin_Psi1,Bin_Event_A,1); //K-
      MEKey key_A_piplus  = MEKey(cent9,Bin_vz,Bin_Psi1,Bin_Event_A,2); //pi+
      MEKey key_A_piminus = MEKey(cent9,Bin_vz,Bin_Psi1,Bin_Event_A,3); //pi-
      for(Int_t Bin_Event_B = Bin_Event_A+1; Bin_Event_B < mEventCounter2[cent9][Bin_vz][Bin_Psi1]; Bin_Event_B++)
      {
        MEKey key_B_kplus   = MEKey(cent9,Bin_vz,Bin_Psi1,Bin_Event_B,0); //K+
        MEKey key_B_kminus  = MEKey(cent9,Bin_vz,Bin_Psi1,Bin_Event_B,1); //K-
        MEKey key_B_piplus  = MEKey(cent9,Bin_vz,Bin_Psi1,Bin_Event_B,2); //pi+
        MEKey key_B_piminus = MEKey(cent9,Bin_vz,Bin_Psi1,Bin_Event_B,3); //pi-
	...
	{
	  StThreeVectorF p_vecA(mHelix[key_A_kplus][n_kplus].cat(mHelix[key_A_kplus][n_kplus].pathLength(mPrimaryvertex[cent9][Bin_vz][Bin_Psi1][Bin_Event_A])));  // primary momentum
	  ...
	  {
	    StThreeVectorF p_vecB(mHelix[key_B_piminus][n_piminus].cat(mHelix[key_B_piminus][n_piminus].pathLength(mPrimaryvertex[cent9][Bin_vz][Bin_Psi1][Bin_Event_B])));  // primary momentum
	...
	{
	  StThreeVectorF p_vecA(mHelix[key_A_piplus][n_piplus].cat(mHelix[key_A_piplus][n_piplus].pathLength(mPrimaryvertex[cent9][Bin_vz][Bin_Psi1][Bin_Event_A])));  // primary momentum
	  ...
	  {
	    StThreeVectorF p_vecB(mHelix[key_B_kminus][n_kminus].cat(mHelix[key_B_kminus][n_kminus].pathLength(mPrimaryvertex[cent9][Bin_vz][Bin_Psi1][Bin_Event_B])));  // primary momentum
	...
	{
	  StThreeVectorF p_vecA(mHelix[key_A_kminus][n_kminus].cat(mHelix[key_A_kminus][n_kminus].pathLength(mPrimaryvertex[cent9][Bin_vz][Bin_Psi1][Bin_Event_A])));  // primary momentum
	  ...
	  {
	    StThreeVectorF p_vecB(mHelix[key_B_piplus][n_piplus].cat(mHelix[key_B_piplus][n_piplus].pathLength(mPrimaryvertex[cent9][Bin_vz][Bin_Psi1][Bin_Event_B])));  // primary momentum
	...
	{
	  StThreeVectorF p_vecA(mHelix[key_A_piminus][n_piminus].cat(mHelix[key_A_piminus][n_piminus].pathLength(mPrimaryvertex[cent9][Bin_vz][Bin_Psi1][Bin_Event_A])));  // primary momentum
	  ...
	  {
	    StThreeVectorF p_vecB(mHelix[key_B_kplus][n_kplus].cat(mHelix[key_B_kplus][n_kplus].pathLength(mPrimaryvertex[cent9][Bin_vz][Bin_Psi1][Bin_Event_B])));  // primary momentum
...
void StVecMesonEpdTree::MixEvent_Phi(Int_t Flag_ME, StPicoDst *pico, Int_t cent9, Float_t vz, Float_t Psi1)
{
  StPicoEvent *event = (StPicoEvent*)pico->event();

  Int_t Bin_vz, Bin_Psi1;

  Float_t vz_start = vmsa::mVzMaxMap[mEnergy];
  Float_t vz_bin = 2*vz_start/vmsa::Bin_VertexZ;

  Float_t psi1_start = TMath::Pi();
  Float_t psi1_bin = 2*psi1_start/vmsa::Bin_Phi_Psi1;
  ...
  mPrimaryvertex[cent9][Bin_vz][Bin_Psi1].push_back(static_cast<StThreeVectorF>(primVer));
  mRefMult[cent9][Bin_vz][Bin_Psi1].push_back(static_cast<Int_t>(event->refMult()));
  mCentrality[cent9][Bin_vz][Bin_Psi1].push_back(static_cast<Int_t>(cent9));
  mRunId[cent9][Bin_vz][Bin_Psi1].push_back(static_cast<Int_t>(event->runId()));
  mEventId[cent9][Bin_vz][Bin_Psi1].push_back(static_cast<Int_t>(event->eventId()));
  mN_prim[cent9][Bin_vz][Bin_Psi1].push_back(static_cast<Int_t>(mNumber_prim));
  mN_non_prim[cent9][Bin_vz][Bin_Psi1].push_back(static_cast<Int_t>(mNumber_non_prim));
  mN_Tof_match[cent9][Bin_vz][Bin_Psi1].push_back(static_cast<Int_t>(mNumber_Tof_match));
  mZDCx[cent9][Bin_vz][Bin_Psi1].push_back(static_cast<Float_t>(event->ZDCx()));
  mBBCx[cent9][Bin_vz][Bin_Psi1].push_back(static_cast<Float_t>(event->BBCx()));
  mVzVpd[cent9][Bin_vz][Bin_Psi1].push_back(static_cast<Float_t>(event->vzVpd()));
  ...
  mNumTrackFull[cent9][Bin_vz][Bin_Psi1].push_back(static_cast<Int_t>(mTrackFull));
  ...
  {
    if(mEventCounter2[cent9][Bin_vz][Bin_Psi1] == vmsa::Buffer_depth)
    {
      doPhi(Flag_ME,cent9,Bin_vz,Bin_Psi1);
      clear_phi(cent9,Bin_vz,Bin_Psi1);
    }
...
void StVecMesonEpdTree::MixEvent_Rho(Int_t Flag_ME, StPicoDst *pico, Int_t cent9, Float_t vz, Float_t Psi1)
{
  StPicoEvent *event = (StPicoEvent*)pico->event();
  ...
  Float_t psi1_start = TMath::Pi();
  Float_t psi1_bin = 2*psi1_start/vmsa::Bin_Rho_Psi;
  ...
  if(Flag_ME == 0) // same event
  {
    doRho(Flag_ME,cent9,Bin_vz,Bin_Psi1);
    clear_rho(cent9,Bin_vz,Bin_Psi1);
  }
...
void StVecMesonEpdTree::MixEvent_KStar(Int_t Flag_ME, StPicoDst *pico, Int_t cent9, Float_t vz, Float_t Psi1)
{
  StPicoEvent *event = (StPicoEvent*)pico->event();
  ...
  Float_t psi1_start = TMath::Pi();
  Float_t psi1_bin = 2*psi1_start/vmsa::Bin_KStar_Psi;
  ...
  if(Flag_ME == 0) // same event
  {
    doKStar(Flag_ME,cent9,Bin_vz,Bin_Psi1);
    clear_kstar(cent9,Bin_vz,Bin_Psi1);
  }

  if(Flag_ME == 1) // mix event
  {
    if(mEventCounter2[cent9][Bin_vz][Bin_Psi1] == vmsa::Buffer_depth_KStar)
    {
      doKStar(Flag_ME,cent9,Bin_vz,Bin_Psi1);
      clear_kstar(cent9,Bin_vz,Bin_Psi1);
    }
...
void StVecMesonEpdTree::clearEvent()
{
  mNumber_prim = 0;
  mNumber_non_prim = 0;
  mNumber_Tof_match = 0;

  mQVector1East.Set(-999.9,-999.9);
  mQVector1West.Set(-999.9,-999.9);
...
void StVecMesonEpdTree::passEvent(Int_t N_prim, Int_t N_non_prim, Int_t N_Tof_match)
{
  mNumber_prim = N_prim;
...
void StVecMesonEpdTree::passNumTrack(Int_t NumTrackEast, Int_t NumTrackWest, Int_t NumTrackFull, Int_t NumTrackFullEast, Int_t NumTrackFullWest)
{
