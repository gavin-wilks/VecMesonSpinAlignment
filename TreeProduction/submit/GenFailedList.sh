...
then
  ...
  sed -i 's/GeV_\(.*\)_/GeV_Phi_ME_/g' $TempROOT # mode = 3 => ME
  ...
  # ls -d $OriginForest/Resolution/*Resolution_*.root | sort > $OriginROOT #mode = 2 => resolution
  ls -d $OriginForest/Phi/Forest/*${SM}*.root | sort > $OriginROOT #mode = 3 => SE/ME
