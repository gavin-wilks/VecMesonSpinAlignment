...
void azimuthalCalTpcTrackEfficiency(int energy = 4, int mpid = 0, int pid = 1, int year = 0, int EP = 2)
{
  ...
  string inputfile_Kplus = Form("data/%s/%s_%s%s.root",vmsa::mBeamEnergy[energy].c_str(),vmsa::mPID[mpid].c_str(),vmsa::mParType[0].c_str(),eventplane.c_str());
  string inputfile_Kminus = Form("data/%s/%s_%s%s.root",vmsa::mBeamEnergy[energy].c_str(),vmsa::mPID[mpid].c_str(),vmsa::mParType[1].c_str(),eventplane.c_str());
  TFile *File_InPut_Kplus = TFile::Open(inputfile_Kplus.c_str());
  TFile *File_InPut_Kminus = TFile::Open(inputfile_Kminus.c_str());
  
  TH1D *h_FrameEta = (TH1D*)File_InPut_Kplus->Get("h_FrameEta");
  TH1D *h_FramePhi = (TH1D*)File_InPut_Kplus->Get("h_FramePhi");
  ...
  for(int i_cent = 0; i_cent < 9; ++i_cent)
  {
    ...
    h_mMcTracks[i_cent] = (TH3D*)File_InPut_Kplus->Get(HistName.c_str())->Clone();
    h_mMcTracks[i_cent]->Sumw2();
    temp = (TH3D*)File_InPut_Kminus->Get(HistName.c_str())->Clone();
    temp->Sumw2();
    h_mMcTracks[i_cent]->Add(temp,1.0);

    HistName = Form("h_mRcTracks_%d",i_cent);
    h_mRcTracks[i_cent] = (TH3D*)File_InPut_Kplus->Get(HistName.c_str())->Clone();
    h_mRcTracks[i_cent]->Sumw2();
    temp = (TH3D*)File_InPut_Kminus->Get(HistName.c_str())->Clone();
    temp->Sumw2();
    h_mRcTracks[i_cent]->Add(temp,1.0);
  }
  ...
  if(EP == 0 || EP == 2) h_mEffPhi[9]->GetXaxis()->SetTitle("#phi");
  if(EP == 1) h_mEffPhi[9]->GetXaxis()->SetTitle("#phi-#Psi_{2}");
  h_mEffPhi[9]->GetYaxis()->SetTitle("RC/MC");
  h_mEffPhi[9]->Draw("pE");
  c1->SaveAs(Form("phieff%s_cent9.pdf",eventplane.c_str()));
  ...
  string outputfile = Form("/gpfs01/star/pwg/gwilks3/VectorMesonSpinAlignment/Data/%s/Efficiency/TPC/Eff_%s%s.root",vmsa::mPID[mpid].c_str(),vmsa::mBeamEnergy[energy].c_str(),eventplane.c_str());
  ...
  File_InPut_Kplus->cd();
  File_InPut_Kplus->Close();
  File_InPut_Kminus->cd();
  File_InPut_Kminus->Close();
}
