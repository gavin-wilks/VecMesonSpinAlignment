...
if (  "$SLURM_JOB_PARTITION" =~ *"chos"  ) then  
    ...
    echo  start to fill phi-meson spin alignment ZDCSMD with execName=$EXEC_NAME, workerName=`hostname -f`, startDate=`date`
    ...
    echo `pwd`
    /usr/bin/time -v  $EXEC_NAME -b -q FillSpinAlignmentZdcSmd.C\($energy,$flag_ME,${SLURM_ARRAY_TASK_ID},$startEvent,$stopEvent,$mode\) >& $LOG_PATH/phi${SM}${Energy}_${SLURM_ARRAY_JOB_ID}_${SLURM_ARRAY_TASK_ID}.log
