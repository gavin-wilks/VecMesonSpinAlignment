...
void plotResolution(int beamEnergy = 4)
{
  ...
  TGraphAsymmErrors *g_mTpcSubRes2[6];
  TFile *File_InPut[6];
  for(int i = 4; i < 5; i++)
  {
  ...
  */
  ...
  double mTpcSubRes2Val[6][9];
  double mTpcSubRes2Err[6][9];
  //double mTpcSubRes2ValXu[9];
  //double mTpcSubRes2ErrXu[9];
  double mTpcFullRes2Val[6][9];
  double mTpcFullRes2Err[6][9];
 ...
 // TGraphAsymmErrors *g_mTpcFullRes2 = new TGraphAsymmErrors();
  ...
  cout << "TPC Event Plane Resolution:" << endl;
  cout << "Sub Event Plane:" << endl;
  TProfile *p_mTpcSubRes2[6];
  TProfile *p_mTpcRanRes2[6];
  for(int i = 4; i < 5; i++)
  {
  ...
  for(int i = 4; i < 5; i++)
  {
    ...
    p_mTpcRanRes2[i] = (TProfile*)File_InPut[i]->Get("p_mRes2_Ran");
    for(int i_cent = 0; i_cent < 9; ++i_cent)
    {
      const double resRaw = p_mTpcRanRes2[i]->GetBinContent(p_mTpcRanRes2[i]->FindBin(i_cent));
      const double errRaw = p_mTpcRanRes2[i]->GetBinError(p_mTpcRanRes2[i]->FindBin(i_cent));
      if(resRaw > 0)
      {
        const double resSub = TMath::Sqrt(resRaw);
        const double errSub = errRaw/(2.0*TMath::Sqrt(resRaw));
        ...
        const double errChiSub = errSub/f_res->Derivative(chiSub);
        const double chiFull = chiSub*TMath::Sqrt(2.0);
        mTpcFullRes2Val[i][i_cent] = f_res->Eval(chiFull);
        mTpcFullRes2Err[i][i_cent] = f_res->Derivative(chiFull)*errChiSub*TMath::Sqrt(2.0);
      }
      cout << "i_cent = " << i_cent << ", resRaw = " << resRaw << ", resFull = " << mTpcFullRes2Val[i][i_cent] << " +/- " << mTpcFullRes2Err[i][i_cent] << endl;
...
*/
  ...
  g_mTpcSubRes2[0]->SetMarkerColor(kBlue);
  ...
  leg->AddEntry(g_mTpcSubRes2[0],"7.7 GeV #eta_{sub} TPC","p");
