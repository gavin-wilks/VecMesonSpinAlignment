...
double Poly1Voigt(double *x, double *par)
{
  double xp = x[0]-par[0];
  double sigma = par[1];
  double gamma = par[2];
...
double Voigt(double *x, double *par)
{
  ...
  return norm * TMath::Voigt(xp,sigma,gamma);
}

// ---------------------------------------------------------------------
// Convolution of a Gaussian (centered at mKK, width sigma)
// with a Lorentzian (centered at M0, width Gamma0).
// The integrand is:
//    exp[-(mKK - m')^2 / (2 sigma^2)] / [ (m'-M0)^2 + (Gamma0^2 / 4) ]
// times the prefactor A * Gamma0 / ((2 pi)^(3/2) * sigma).
//
// par[0] = sigma   (Gaussian resolution)
// par[1] = Gamma0  (resonance width)
// par[2] = M0      (resonance mass)
// par[3] = A       (overall amplitude)
// ---------------------------------------------------------------------
Double_t VoigtFunc(Double_t *x, Double_t *par)
{
   ...
   // Prefactor from the formula:
   Double_t prefactor = A * Gamma0 / ( TMath::Power((2*TMath::Pi()), 1.5) * sigma );

   // We'll do a 1D numerical integration over m':
   // Decide on integration limits. Because the Gaussian
   // is centered at mKK with width sigma, ± ~5sigma is often enough.
   // But you may want to extend further if the Lorentzian is wide.
   const Int_t    nSteps = 2000;        // number of steps for trapezoid rule
   const Double_t range  = 5.0 * sigma; // half-width for integration
   Double_t lower = mKK - range;
   Double_t upper = mKK + range;
   Double_t step  = (upper - lower) / nSteps;

   // Trapezoidal integration:
   Double_t sum = 0.0;
   for (Int_t i = 0; i <= nSteps; ++i) {
      // current integration point m':
      Double_t mp = lower + i * step;

      // Gaussian part (centered at mKK, width sigma):
      Double_t gauss = TMath::Exp( - ( (mKK - mp)*(mKK - mp) ) / (2.0 * sigma*sigma ) );
      //gauss /= ( sigma * TMath::Sqrt(2.0 * TMath::Pi()) );  // normalize

      // Lorentzian part (centered at M0, width Gamma0):
      Double_t lorentz = 1.0 / ( (mp - M0)*(mp - M0) + 0.25 * Gamma0 * Gamma0 );

      // product of Gaussian and Lorentzian:
      Double_t integrand = gauss * lorentz;

      // Trapezoidal rule weighting:
      if (i == 0 || i == nSteps) {
         sum += 0.5 * integrand;
      } else {
         sum += integrand;
   ...
   // Multiply by step size and prefactor:
   Double_t val = prefactor * sum * step;
   return val;
}

Double_t Poly1VoigtFunc(Double_t *x, Double_t *par)
{
   // Unpack parameters:
   Double_t sigma  = par[0];
   Double_t Gamma0 = par[1];
   ...
   // Multiply by step size and prefactor:
   Double_t val = prefactor * sum * step;
   Double_t Poly = par[4] + par[5]*x[0];
   return val+Poly;
}
Double_t Poly2VoigtFunc(Double_t *x, Double_t *par)
{
   // Unpack parameters:
   Double_t sigma  = par[0];
   Double_t Gamma0 = par[1];
   ...
   // Multiply by step size and prefactor:
   Double_t val = prefactor * sum * step;
   Double_t Poly = par[4] + par[5]*x[0] + par[6]*x[0]*x[0];
   return val+Poly;
}
Double_t Poly3VoigtFunc(Double_t *x, Double_t *par)
{
   // Unpack parameters:
   Double_t sigma  = par[0];
   Double_t Gamma0 = par[1];
   ...
   // Multiply by step size and prefactor:
   Double_t val = prefactor * sum * step;
   Double_t Poly = par[4] + par[5]*x[0] + par[6]*x[0]*x[0] + par[7]*x[0]*x[0]*x[0];
   return val+Poly;
}
...
double SpinDensity2Dcos(double *x, double *p)
{
  ...
  //return p[5]*((1.-p[0])+(3.*p[0] -1.)*x[0]*x[0]
  //             -sqrt(2.)*p[1]*2*x[0]*TMath::Sqrt(s2)*cos(x[1])
  //             +sqrt(2.)*p[2]*2*x[0]*TMath::Sqrt(s2)*sin(x[1])
  //             -2.*p[3]*(s2)*cos(2.*x[1])
  //             +2.*p[4]*(s2)*sin(2.*x[1]));
  return p[5]*(2./3.+(p[0]-1./3.)*(3*x[0]*x[0]-1.)
               -sqrt(2.)*p[1]*2*x[0]*TMath::Sqrt(s2)*cos(x[1])
               +sqrt(2.)*p[2]*2*x[0]*TMath::Sqrt(s2)*sin(x[1])
               -2.*p[3]*(s2)*cos(2.*x[1])
               +2.*p[4]*(s2)*sin(2.*x[1]));
...
double m2_PID_power(double *x_val, double *par)
{
  double pT = x_val[0];
  double a  = par[0];
  double b  = par[1];
  ...
  return a/(1.+pow((std::max(pT-b,0.0)/c),d));
}
...
double EventPlaneResolutionK2(double *var, double *par)
{
  ...
  double BesselIhalf  = std::sqrt(2.0 / TMath::Pi() / arg) * std::sinh(arg);
  double BesselI3half = std::sqrt(2.0 / TMath::Pi() / arg) * (-std::sinh(arg)/arg + std::cosh(arg));
  //double BesselI3half = std::sqrt(2.0 / TMath::Pi() / arg) * (std::sinh(arg)/arg - std::cosh(arg));

  double y = norm*chi*TMath::Exp(-1.0*arg)*(BesselIhalf+BesselI3half);
...
double LevyScaled(double *var, double *par)
{
  
  double ptval[9] = {1.2,1.4,1.6,1.8,2.0,2.2,2.4,3.0,4.2};
  double scale[8] = {0.950447, 0.999705, 1.02101, 1.04496, 1.06284, 1.08563, 1.10833, 1.22269};

  double const m0 = 1.01946; // phi-meson mass
  double pT   = var[0];
  ...
  {
    if(pT >= ptval[ipt] && pT < ptval[ipt+1])
    {
      scalefactor = scale[ipt];
    }
  }

 
  double y = scalefactor*numer*power/denom;
...
double mTLevy(double *var, double *par)
{
  //double const m0 = 1.01940; // phi-meson mass
  double const m0 = 1.01946; // phi-meson mass
  //double pT   = var[0];
  double mTm0   = var[0];//sqrt(pT*pT+m0*m0);
  double dNdy = par[0];
  double n    = par[1];
  double T    = par[2];
  ...
  double numer = dNdy*(n-1)*(n-2);
  double denom = n*T*(n*T+m0*(n-2));
  double power = pow(1+(mTm0)/(n*T),-1.0*n);
...
double ExpBreitWigner(double *x_val, double *par)
{
  double x = x_val[0];
  double m0 = par[0];
  double Gamma = par[1];
  ...
  double Poly = par[3]*exp(par[4]*x);

  double y = BW + Poly;

  return y;
}
double Exp(double *x_val, double *par)
{
  return par[0]*exp(par[1]*x_val[0]);
}
...
double Poly1MBreitWigner(double *x_val, double *par)
{
  double x = x_val[0];
  double m0 = par[0];
  double Gamma = par[1];
...
double Poly2MBreitWigner(double *x_val, double *par)
{
  ...
  double Poly = par[3] + par[4]*(x-m0) + par[5]*(x-m0)*(x-m0);
...
double Poly2M(double *x_val, double *par)
{
  double x = x_val[0];
  double m0 = par[0];
  double y = par[1] + par[2]*(x-m0) + par[3]*(x-m0)*(x-m0);

  return y;
}
double Poly1M(double *x_val, double *par)
{
  double x = x_val[0];
  double m0 = par[0];
  double y = par[1] + par[2]*(x-m0);
