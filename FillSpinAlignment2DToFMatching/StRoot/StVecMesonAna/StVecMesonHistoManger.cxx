...
void StVecMesonHistoManger::InitSys(Int_t X_flag, Int_t mode) // 0 for Same Event, 1 for Mixed Event
{
  ...
  TString Tag[2] = {"TagKplus","TagKminus"};
  // for spectra 
        ...
        for(Int_t i_cent = vmsa::Cent_start; i_cent < vmsa::Cent_stop; i_cent++) // centrality bin
        {
          for(Int_t i_dca = vmsa::Dca_start; i_dca < 1/*vmsa::Dca_stop*/; i_dca++)
          {
            for(Int_t i_sig = vmsa::nSigKaon_start; i_sig < 1/*vmsa::nSigKaon_stop*/; i_sig++)
            {
              ...
              for(Int_t i_tag = 0; i_tag < 2; i_tag++)
              {               
                ...
                TString KEY_Mass2 = Form("pt_%d_eta_%d_Centrality_%d_%s_Dca_%d_Sig_%d_%s_%s_%s",i_pt,i_eta,i_cent,EP_string.c_str(),i_dca,i_sig,vmsa::mPID[mode].c_str(),Mode[X_flag].Data(),Tag[i_tag].Data());
                h_mMass2[KEY_Mass2] = new TH1F(KEY_Mass2.Data(),KEY_Mass2.Data(),100,vmsa::InvMass_low[mode],vmsa::InvMass_high[mode]);
                h_mMass2[KEY_Mass2]->Sumw2();
              }
            }
...
//-------------------------------------------------------------
void StVecMesonHistoManger::FillSys(Float_t pt, Float_t eta, Float_t phi, Int_t cent9, Float_t CosThetaStar, Float_t PhiPsi, Float_t Cos2PhiStarPhi, Float_t phiprime, Int_t dcaSys, Int_t nSigSys, Float_t Res2, Float_t InvMass, Double_t reweight, Int_t X_flag, Int_t mode, Bool_t tag)
{
  TString Mode[2] = {"SE","ME"};
  TString Tag[2] = {"TagKplus","TagKminus"};
  int tagint = 0;
        ...
        for(Int_t i_eta = 0; i_eta < 16; i_eta++)
        {
          if(eta >= float(i_eta-8.)/8. && eta < float(i_eta-7.)/8.)
          {
            ...
            //  {
                ...
                //}
                TString KEY_Mass2 = Form("pt_%d_eta_%d_Centrality_%d_%s_Dca_%d_Sig_%d_%s_%s_%s",i_pt,i_eta,cent9,EP_string.c_str(),dcaSys,nSigSys,vmsa::mPID[mode].c_str(),Mode[X_flag].Data(),Tag[tagint].Data());
                h_mMass2[KEY_Mass2]->Fill(InvMass,reweight);
                if(cent9 >= vmsa::cent_low[0] && cent9 <= vmsa::cent_up[0]) // 20%-60%
                {
                  TString KEY_Mass2Sys = Form("pt_%d_eta_%d_Centrality_9_%s_Dca_%d_Sig_%d_%s_%s_%s",i_pt,i_eta,EP_string.c_str(),dcaSys,nSigSys,vmsa::mPID[mode].c_str(),Mode[X_flag].Data(),Tag[tagint].Data());
                  h_mMass2[KEY_Mass2Sys]->Fill(InvMass,reweight);
                }
                break;
            //  }
