...
void toyMcPhiDecay_FinerPhiBins(const int energy = 4, int ptbin = 5, const int centrality = 5, const int pid = 0, const int NMax = 5000, int mode = 1, int fitmode = 0, const char* jobID = "20240515") 
{
  mNMax = NMax;
  ...
  if(energy == 3) InPutFile_Res1 = Form("Utility/EpdResolution/Resolution_file_%s_EpdCorrections_6.root",vmsa::mBeamEnergy[energy].c_str());
...
void readEfficiency(int energy, int year, int cut, const char *jobID)
{
  ...
  string inputKplus = Form("/star/data01/pwg/gwilks3/VectorMesonSpinAlignment/Data/Phi/Efficiency/TPC/Eff_%s_%s_FinerPhiBins.root",vmsa::mParType[0].c_str(),vmsa::mBeamEnergy[energy].c_str());
  ...
  string inputKminus = Form("/star/data01/pwg/gwilks3/VectorMesonSpinAlignment/Data/Phi/Efficiency/TPC/Eff_%s_%s_FinerPhiBins.root",vmsa::mParType[1].c_str(),vmsa::mBeamEnergy[energy].c_str());
...
void readTofEff(int energy)
{
  // string inputfile = Form("/project/projectdirs/starprod/rnc/xusun/OutPut/AuAu%s/SpinAlignment/ToFMatch/Eff_%s_ToFMatch_FinerPhiBins.root",vmsa::mBeamEnergy[energy].c_str(),vmsa::mBeamEnergy[energy].c_str());
  string inputfile = Form("/gpfs01/star/pwg/gwilks3/VectorMesonSpinAlignment/Data/Phi/Efficiency/ToF/Eff_%s_ToFMatch_FinerPhiBins.root",vmsa::mBeamEnergy[energy].c_str());
...
void readTofEffFit(int energy, int i_cent)
{
  ...
  string inputKplus = Form("/gpfs01/star/pwg/gwilks3/VectorMesonSpinAlignment/Data/Phi/Efficiency/ToF/FitPar_AuAu%s_Kplus_cent%d_FinerPhiBins.root",vmsa::mBeamEnergy[energy].c_str(),i_cent);
  TFile *File_Kplus = TFile::Open(inputKplus.c_str());
  ...
  string inputKminus = Form("/gpfs01/star/pwg/gwilks3/VectorMesonSpinAlignment/Data/Phi/Efficiency/ToF/FitPar_AuAu%s_Kminus_cent%d_FinerPhiBins.root",vmsa::mBeamEnergy[energy].c_str(),i_cent);
  TFile *File_Kminus = TFile::Open(inputKminus.c_str());
...
TF1* readspec(int energy, int pid, int centrality)
{
  ...
  if(energy != 3)
  {
    ...
    if(centrality >= 0 && centrality <= 1) f_Levy->SetParameter(0,0.01);
