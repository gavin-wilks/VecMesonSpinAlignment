...
double SpinDensity2D(double *x, double *p)
{
  return 3./(8.*TMath::Pi())*sin(x[0])*((1.-p[0])+(3.*p[0] -1.)*cos(x[0])*cos(x[0])
                                        -sqrt(2)*p[1]*sin(2.*x[0])*cos(x[1])
                                        +sqrt(2)*p[2]*sin(2.*x[0])*sin(x[1])
                                        -2.*p[3]*sin(x[0])*sin(x[0])*cos(2.*x[1])
                                        +2.*p[4]*sin(x[0])*sin(x[0])*sin(2.*x[1]));
}

double SpinDensity2Dcos(double *x, double *p)
{
  return 3./(8.*TMath::Pi())*((1.-p[0])+(3.*p[0] -1.)*x[0]*x[0]
...
double SpinDensity2Dcosneg(double *x, double *p)
{
  return -3./(8.*TMath::Pi())*((1.-p[0])+(3.*p[0] -1.)*x[0]*x[0]
                                        -sqrt(2)*p[1]*2*x[0]*TMath::Sqrt(1-x[0]*x[0])*cos(x[1])
                                        +sqrt(2)*p[2]*2*x[0]*TMath::Sqrt(1-x[0]*x[0])*sin(x[1])
...
double SpinDensity2Dneg(double *x, double *p)
{
  return -3./(8.*TMath::Pi())*sin(x[0])*((1.-p[0])+(3.*p[0] -1.)*cos(x[0])*cos(x[0])
                                        -sqrt(2)*p[1]*sin(2.*x[0])*cos(x[1])
                                        +sqrt(2)*p[2]*sin(2.*x[0])*sin(x[1])
...
double flowSampleNorm(double *x_val, double *par) 
{
  double x, y, v2;
  x = x_val[0];
  v2 = par[0];
  y = (1./(2.*TMath::Pi()))*(1.0 + 2.0*v2*cos(2.0*x));
...
double v2_pT_FitFunc_Poly3(double *x_val, double *par)
{
  // Fit function for v2 vs. pT
  // From arXiv:nucl-th/0403030v5: Resonance decay effects on anisotrotpy parameters
  double v2, pT, a, b, c, d, e, f, g, h, n;
  pT = x_val[0];
  n  = par[0]; // number-of-constituent quarks
  a  = par[1];
  b  = par[2];
...
double pTLevyNormalized(double *var, double *par)
{
  double const m0 = 1.01946; // phi-meson mass
  double pT   = var[0];
  double mT   = sqrt(pT*pT+m0*m0);
