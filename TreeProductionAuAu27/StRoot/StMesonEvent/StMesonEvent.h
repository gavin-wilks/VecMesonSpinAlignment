...
class StMesonEvent : public TObject
{
  ...
  public:
    StMesonEvent() :
      mPrimaryvertex(-1.0,-1.0,-1.0),mRunId(-1),mEventId(-1),mRefMult(-1),mGRefMult(-1),mCentrality(-1),mN_prim(-1),mN_non_prim(-1),mN_Tof_match(-1),mZDCx(-1),mBBCx(-1),mVzVpd(-1),fNumTracks(0)
  {
    ...
    void       setGRefMult(Int_t r)                    { mGRefMult = r;             }
    Int_t      getGRefMult() const                     { return mGRefMult;          }
