  ...
  : StMaker(name)
{
  ...
  if(mMode == 4)
  {
    mOutPut_PID = Form("EP_%s_%s_%s_%s.txt",vmsa::mPID[mFlag_PID].c_str(),vmsa::MixEvent[mFlag_ME].Data(),vmsa::mBeamEnergy[energy].c_str(),jobCounter);
  }
}
...
Int_t StVecMesonMaker::Init() 
{
  ...
  mRefMultCorr = CentralityMaker::instance()->getgRefMultCorr_Run14_AuAu200_VpdMB5_P16id();
  //}
  ...
  if(mMode == 4)
  {
    ...
    mVecMesonCorrection->InitResolutionCorr();
    cout << "InitResolutionCorr" << endl;
    mFile_OutPut.open(mOutPut_PID.Data());
    cout << "INIT EP FILE" << endl;
  }
...
//  cout << mRunIdEventsDb->getTotalNrRunIds() << endl;
  ...
  //mRefMultCorr->initEvent(refMult,vz,zdcX); 
  mRefMultCorr->initEvent(grefMult,vz,zdcX); 
  bool isPileUpEvent = false;
  // IMPORTANT: vertex position is needed for Au+Au 19.6 GeV 2019
  //if (mRefMultCorr->isPileUpEvent( grefMult, nBTofMatched, vz ) ) isPileUpEvent = true;
  if(mRefMultCorr->passnTofMatchRefmultCut(1.*grefMult, 1.*nBTofMatched)) isPileUpEvent = true; // reject pileup of StRefMultCorr
  ...
  if(!isPileUpEvent && mVecMesonCut->passEventCut(mPicoDst) && cent9 > -0.5) // event cut
  {
    ...
    if(vz > -70.0 && vz <= 0.0) vz_sign = 0;
    //if(vz > -30.0 && vz <= 0.0  ) vz_sign = 1;
    //if(vz > 0.0   && vz <= +30.0) vz_sign = 2;
    if(vz < +70.0 && vz >  0.0) vz_sign = 1;
	...
	if(fabs(eta) < vmsa::mEtaMax && track->gDCA(vx,vy,vz) < 3.0)
	{
      ...
      {
	if(mMode == 0 || mMode == 2) // fill re-center parameter
	{
	...
	if(mMode != 0) // calculate Q Vector after recentering for full event and eta sub event
	{
	  if(mVecMesonCorrection->passTrackFull(track))
	  {
    ...
    if(mMode == 4 && cent9 >= 2 && cent9 <= 5)
    {
      if(mVecMesonCorrection->passTrackFullNumCut())
      {
	Float_t Psi2_full = mVecMesonCorrection->calShiftAngle2Full_EP(runIndex,cent9,vz_sign);
	Float_t Psi2_east = mVecMesonCorrection->calShiftAngle2East_EP(runIndex,cent9,vz_sign);
	Float_t Psi2_west = mVecMesonCorrection->calShiftAngle2West_EP(runIndex,cent9,vz_sign);
