...
void acceptanceQA_4th_Res_v2test_v2vary_lessv2(const int energy = 4, const int pid = 0, std::string res = "0p4", double resval = 0.4, int cut = 1, int method = 0, int v2 = 1, int fit2d = 0, int EP = 1, int etamode = 0, int ieta = 1) {
 ...
 int  rhoorder[3]={1,0,2};
  ...
  for(int ir = 0; ir < 3; ir++)
  {
    ...
    for(int iem = 0; iem < 6; iem++)
    {
      ...
      if(cut == 1 && method == 1){
        if(fit2d){FvalB[iem][ieta][irho] = Func_beta->GetParameter(1);
        FerrB[iem][ieta][irho] = Func_beta->GetParError(1);
        GvalB[iem][ieta][irho] = Func_beta->GetParameter(2);
        GerrB[iem][ieta][irho] = Func_beta->GetParError(2);
        TCanvas *c1 = new TCanvas();
