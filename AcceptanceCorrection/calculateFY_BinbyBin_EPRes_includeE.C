...
void calculateFY_BinbyBin_EPRes_includeE(const int energy = 4, const int pid = 0, bool doall = false, bool isBesI = false, bool random3D = false, int mode = 1, int etamode = 3, int ypadding = 5, int ipt = 1, int icent = 1, std::string ep = "sub") {
  ...
  for(int iy = 0; iy < vmsa::y_total; iy++)
  {
    ...
    Eval[iy] = Func_A->GetParameter(0);
    Eerr[iy] = Func_A->GetParError(0);
    Fval[iy] = Func_A->GetParameter(1);
    Ferr[iy] = Func_A->GetParError(1);
    Gval[iy] = Func_A->GetParameter(2);
    Gerr[iy] = Func_A->GetParError(2);
  ...
  if(doall)
  {
    ...
    string outputname = Form("output/%s/%s/AccResRhoEtaSys_%s_cent_%d_ipt_%d_%s%s_2ndSubEPRes_includeE.root",vmsa::mPID[pid].c_str(),vmsa::mBeamEnergy[energy].c_str(),etastring.c_str(),icent,ipt,ep.c_str(),xtraopt.c_str());
        ...
        for(int i_norm = vmsa::Norm_start; i_norm < vmsa::Norm_stop; ++i_norm)
        {
          for(int i_sigma = vmsa::Sig_start; i_sigma < vmsa::Sig_stop; ++i_sigma)
          {
            for(int i_method = vmsa::Method_start; i_method < vmsa::Method_stop; ++i_method)
            {
              ...
              Title = new TString(Form("fit/yield_pt_%d_cent_%d_2nd_Dca_%d_Sig_%d_%s_Norm_%d_Sigma_%d_%s_EtaMode%d_Divided.pdf",ipt,icent,i_dca,i_sig,vmsa::mPID[pid].c_str(),i_norm,i_sigma,vmsa::mInteMethod[i_method].c_str(),etamode));
              ...
              for(int iy = 0+ypadding; iy < vmsa::y_total-ypadding; iy++) 
              {
                ...
                Func_rho[iy] = new TF1(Form("Func_rho_%d",iy),FuncAEFG,0,1,6);
                Func_rho[iy]->SetParameter(0, PtCos[iy]->GetBinContent(5));
                Func_rho[iy]->SetParameter(1, 0.333);
                Func_rho[iy]->SetParLimits(1, 0.0, 1.0);
                Func_rho[iy]->FixParameter(2, Eval[iy]);
                Func_rho[iy]->FixParameter(3, Fval[iy]);
                Func_rho[iy]->FixParameter(4, Gval[iy]);
                Func_rho[iy]->FixParameter(5, Res_12[icent]);
...
double Func4th(double *x_val, double *par) {
  ...
  double result = E + (4.*F+3.*G)/8. - (2.*F+3.*G)/4.*CosTheta*CosTheta + 3.*G/8.*CosTheta*CosTheta*CosTheta*CosTheta;
...
double FuncAEFG(double *x_val, double *par) {
  ...
  double order0 = 2.*E + F - Bs*F/2. + 3.*G/4. - Bs*G/2.;    
  double order2 = (2.*As*E - F + As*F + Bs*F - 3.*G/2. + 3.*As*G/4. + 3.*Bs*G/2.)*CosTheta*CosTheta;
