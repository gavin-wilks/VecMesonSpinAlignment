...
double d_frho_in(double *x, double *par) {
  double m_pi = 0.13957;
  double gamma = pow(((x[0]*x[0] - 4.*m_pi*m_pi)/(par[1]*par[1] - 4.*m_pi*m_pi)),3./2.)*par[1]*par[2]/x[0];

  double fitVal;
  fitVal =
    exp(-sqrt(x[0]*x[0]+par[3]*par[3])/0.12)*x[0]/sqrt(x[0]*x[0]+par[3]*par[3])* //Phase Space
    par[0]*par[1]*x[0]*gamma/(pow((x[0]*x[0]-par[1]*par[1]),2.) + par[1]*par[1]*gamma*gamma); //UPC 
  return fitVal;
}
...
double Expo(double *var, double *par) {

  double pT   = var[0];

  double y = par[0]*exp(-par[1]*pT);
...
{
  double const m0 = 0.7755; // rho-meson mass
  double pT   = var[0];
