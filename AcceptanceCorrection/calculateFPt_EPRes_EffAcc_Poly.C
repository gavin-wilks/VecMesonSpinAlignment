...
void calculateFPt_EPRes_EffAcc_Poly(const int energy = 0, const int pid = 0, bool doall = false, bool isBesI = false, bool random3D = false, int etamode = 0, int order = 2, int yspectra = 2, int EP = 0, int v2 = 1, int EPCorr = 1) {
...
//  Double_t pt_set[8] = {0.6, 1.4, 2.2, 3.0, 3.8, 4.6, 5.4, 7.2};
  ...
  */
  ...
  if(order == 2) outputname = Form("output/%s/%s/AccRes%sPtSys_%s_Poly%s%s%s.root",vmsa::mPID[pid].c_str(),vmsa::mBeamEnergy[energy].c_str(),vmsa::mPID[pid].c_str(),etastring.c_str(),spectra.c_str(),eptext.c_str(),v2text.c_str());
  if(!EPCorr)
  {
              ...
              for(int i_poly = 0; i_poly < 3; i_poly++)
              {
                for(int i_F = 0; i_F < 1; i_F++)
                {
                  for(int i_eff = 0; i_eff < 1; i_eff++)
                  {
                    ...
                    for(Int_t PtBin=3; PtBin<=5/*6*/; PtBin++) 
                    {
