void fitcos_test(int eff_mode=0, int dca_mode=0, int sigma_mode=1, int norm_mode=0, int inte_mode=-1,int plane_mode=2, int run_mode=1) {
...
//  Double_t pt_set[8] = {0.6, 1.4, 2.2, 3.0, 3.8, 4.6, 5.4, 7.2};
  ...
  double eta_D[6] = {0.0262611, 0.0181768, 0.0107771, 0.00359328, 0.00353325, 0.00185303}; //eta<1.0
//  double eta_D[6] = {0.182266, 0.102203, 0.0640437, 0.0493561, 0.0337873, 0.0222808}; //eta<0.6
//  double eta_D[6] = {0}; //no eta
...
//  TFile event("run11_event_output.root");
  ...
  for(Int_t cent=0; cent<9; cent++) {
    ...
    Double_t CosMean = -1.*DeltaPsi1->GetBinContent(cent+1);
    ...
    cout<<CosMean<<" "<<CosError<<" "<<ZDCSMD_resSub<<" "<<ZDCSMD_resSub<<endl;
    cout<<"Cent"<<cent_set[cent+1]<<"_"<<cent_set[cent]<<": "<<ResZ<<" +/- "<<ResErrZ<<endl;
  ...
  for(Int_t cent=2; cent<=5; cent++) {
    ...
    *Title += cent_set[cent];
    ...
    for(Int_t PtBin=1; PtBin<=6; PtBin++) {
      ...
      Double_t Res_1 = resolution_1->GetBinContent(cent+1);
      Double_t Res_12 = resolution_12->GetBinContent(cent+1);
...
cout<<Func_rho->GetChisquare()<<"/"<<Func_rho->GetNDF()<<endl;
  ...
  TFile *output = new TFile("rho00_200GeV.root","UPDATE");
