...
void McEtaF_Pt(double Nrho=3333, int SetPt=3, int energy = 4, int pid = 0, int cent = 5, int const NMax = 500, double rapidity = 1.0, int mode = 0, int etamode = 0, double Res = 0.4, bool fullEP = 0, bool v2on = 0, float v2input = 0.0, const char* jobID = "1")
{
  v2flag = v2on;
  mNEvents = NMax;
  mEtaMode = etamode;
  ...
  //  for(int i = 2; i <= 5; i++)
  //  {
  //    avg += TMath::Sqrt(p_res2->GetBinContent(p_res2->FindBin(i)));
  //    cout << TMath::Sqrt(p_res2->GetBinContent(p_res2->FindBin(i))) << endl;
  //  }
  //  Res = avg/4.;
  //}
  ...
  for(int ie; ie < 6; ie++) //index corresponds to eta cut, 0 = none, 1 = 1.0, 2 = 0.8, 3 = 0.6, 4 = 0.4, 5 = 0.2
  {
    ...
    histname = Form("p_cos2betacos4beta_%d",ie);
    p_cos2betacos4beta[ie]    = new TProfile(histname.c_str(),histname.c_str(), 7, 0.0, 1.0);
    ...
    histname = Form("p_cos2betacos4betaP_%d",ie);
    p_cos2betacos4betaP[ie]    = new TProfile(histname.c_str(),histname.c_str(), 7, 0.0, 1.0);
...
TF1* readv2(int energy, int pid, int centrality)
{
  ...
  if((energy == 4 || energy == 3 || energy == 2 || energy == 0) && (mMode == 0 || mMode == 1 || mMode == 3)) InPutV2 = "/gpfs01/star/pwg/gwilks3/VectorMesonSpinAlignment/Data/Phi/v2/HEPData-ins1395151-v2-root.root";
  ...
  if((energy == 4 || energy == 3 || energy == 2 || energy == 0) && (mMode == 0 || mMode == 1 || mMode == 3)) 
  {
...
TF1* readspec(int energy, int pid, int centrality)
{
  ...
  if((energy == 4 || energy == 2 || energy == 0) && (mMode == 0 || mMode == 1 || mMode == 3)) InPutSpec = "/gpfs01/star/pwg/gwilks3/VectorMesonSpinAlignment/Data/Phi/pTspectra/HEPData-ins1378002-v1-root.root";
  ...
  if((energy == 4 || energy == 2 || energy == 0) && (mMode == 0 || mMode == 1 || mMode == 3)) 
  {
...
//  double const pt = f_spec->GetRandom(vmsa::ptMin, vmsa::ptMax);
  ...
  if(mMode == 0) pt = gRandom->Uniform(pt_set[pt_bin-1], pt_set[pt_bin]);
...
void fill(TLorentzVector* lPhi, TLorentzVector const& lKplus, TLorentzVector const& lKminus, int centrality)
{
  ...
  if(KplusPt < 0.1 || KminusPt < 0.1) return;
  ...
  p_cos2betacos4beta[0]->Fill(TMath::Abs(CosThetaStarRP),TMath::Cos(2.0*Beta)*TMath::Cos(4.0*Beta));
  p_cos2betacos4betaP[0]->Fill(TMath::Abs(CosThetaStarEP),TMath::Cos(2.0*BetaP)*TMath::Cos(4.0*BetaP));
  ...
  if(mMode != 3)
  {
    for(int ie = 1; ie < 6; ie++)
    {
      if(TMath::Abs(KplusEta)<=eta_gap[ie-1] && TMath::Abs(KminusEta)<=eta_gap[ie-1])// && KplusPt > 0.1 && KminusPt > 0.1 && KminusPt < 10.0 && KplusPt < 10.0)
      {
        ...
        p_cos2betacos4beta[ie]->Fill(TMath::Abs(CosThetaStarRP),TMath::Cos(2.0*Beta)*TMath::Cos(4.0*Beta));
        p_cos2betacos4betaP[ie]->Fill(TMath::Abs(CosThetaStarEP),TMath::Cos(2.0*BetaP)*TMath::Cos(4.0*BetaP));
...
void write(int energy, int Nrho)
{
  ...
  if(mMode != 3)
  {
    for(int ie = 0; ie < 6; ie++)
    {
      ...
      p_cos2beta[ie]->Write();  
      p_cos4beta[ie]->Write();  
      p_cos2betacos4beta[ie]->Write();  
      p_cosbetaP[ie]->Write(); 
      p_cos2betaP[ie]->Write(); 
      p_cos4betaP[ie]->Write(); 
      p_cos2betacos4betaP[ie]->Write(); 
