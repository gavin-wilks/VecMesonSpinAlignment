...
void calculateFCent_EPRes_EffAcc(const int energy = 4, const int pid = 0, int ptbin = 0, bool doall = false, bool isBesI = false, bool random3D = false, int mode = 0, int etamode = 0, std::string ep = "sub", int order = 1) {
...
/*
  ...
  */  
  ...
  if(doall)
  {
    ...
    if(energy == 3 && order == 1) eff_file = new TFile(Form("/gpfs01/star/pwg/gwilks3/VectorMesonSpinAlignment/AuAu14GeV_2019/OutPut/CosEfficiencyProccessTuple_EPRes_EffAcc_Ycut_Order1/Eff_14GeV_SingleParticle_noToF_Mode1_EtaMode%d.root",etamode),"READ");
    if(energy == 3 && order == 2) eff_file = new TFile(Form("/gpfs01/star/pwg/gwilks3/VectorMesonSpinAlignment/AuAu14GeV_2019/OutPut/CosEfficiencyProccessTuple_EPRes_EffAcc_Ycut_Order2/Eff_14GeV_SingleParticle_noToF_Mode1_EtaMode%d.root",etamode),"READ");
    //if(order == 1) eff_file = new TFile(Form("/gpfs01/star/pwg/gwilks3/VectorMesonSpinAlignment/AuAu19GeV_2019/OutPut/CosEfficiencyProccessTuple_EPRes_EffAcc_Ycut_Order1/Eff_19GeV_SingleParticle_noToF_Mode1_EtaMode%d.root",etamode),"READ");
    //if(order == 2) eff_file = new TFile(Form("/gpfs01/star/pwg/gwilks3/VectorMesonSpinAlignment/AuAu19GeV_2019/OutPut/CosEfficiencyProccessTuple_EPRes_EffAcc_Ycut_Order2_FixedRes/Eff_19GeV_SingleParticle_noToF_Mode1_EtaMode%d.root",etamode),"READ");
    eff_file->Print();
    ...
    TFile *input;
    if(order == 1) input = new TFile(Form("rho00/%s/%s/RawRhoCentSys_%s_FirstOrder.root",vmsa::mPID[pid].c_str(),vmsa::mBeamEnergy[energy].c_str(),etastring.c_str()),"READ");
    if(order == 2) input = new TFile(Form("rho00/%s/%s/fixed_ptbin/RawRhoCentSys_%s.root",vmsa::mPID[pid].c_str(),vmsa::mBeamEnergy[energy].c_str(),etastring.c_str()),"READ");
    if(energy == 3 && order == 2) input = new TFile(Form("rho00/%s/%s/RawRhoCentSys_%s.root",vmsa::mPID[pid].c_str(),vmsa::mBeamEnergy[energy].c_str(),etastring.c_str()),"READ");
    ...
    string outputname;
    if(order == 1) outputname = Form("output/%s/%s/AccResRhoCentSys_%s_FirstOrder_pt_%d.root",vmsa::mPID[pid].c_str(),vmsa::mBeamEnergy[energy].c_str(),etastring.c_str(),ptbin);
    if(order == 2) outputname = Form("output/%s/%s/AccResRhoCentSys_%s_pt_%d.root",vmsa::mPID[pid].c_str(),vmsa::mBeamEnergy[energy].c_str(),etastring.c_str(),ptbin);
          ...
          for(int i_norm = vmsa::Norm_start; i_norm < vmsa::Norm_stop; ++i_norm)
          {
            for(int i_sigma = vmsa::Sig_start; i_sigma < vmsa::Sig_stop; ++i_sigma)
            {
              for(int i_method = vmsa::Method_start; i_method < vmsa::Method_stop; ++i_method)
              {
                ...
                Title = new TString(Form("fit/Phi/%s/Order%d/centrality/yield_pt_%d_%s_Dca_%d_Sig_%d_%s_Norm_%d_Sigma_%d_%s_EtaMode%d_Divided.pdf",vmsa::mBeamEnergy[energy].c_str(),order,ptbin,ordertext[order-1].c_str(),i_dca,i_sig,vmsa::mPID[pid].c_str(),i_norm,i_sigma,vmsa::mInteMethod[i_method].c_str(),etamode));


                for(int icent = 0; icent < 9; ++icent) // Centrality loop
                {
                  string key = Form("pt_%d_Centrality_%d_%s_Dca_%d_Sig_%d_%s_Norm_%d_Sigma_%d_%s",ptbin,icent,ordertext[order-1].c_str(),i_dca,i_sig,vmsa::mPID[pid].c_str(),i_norm,i_sigma,vmsa::mInteMethod[i_method].c_str());
                  cout << key << endl;
               ...
               // }
                ...
                Title = new TString(Form("rhoRaw_pt_%d_%s_Dca_%d_Sig_%d_%s_Norm_%d_Sigma_%d_%s",ptbin,ordertext[order-1].c_str(),i_dca,i_sig,vmsa::mPID[pid].c_str(),i_norm,i_sigma,vmsa::mInteMethod[i_method].c_str()));                             
                ...
                Title = new TString(Form("rhoFinalWeighted_pt_%d_%s_Dca_%d_Sig_%d_%s_Norm_%d_Sigma_%d_%s",ptbin,ordertext[order-1].c_str(),i_dca,i_sig,vmsa::mPID[pid].c_str(),i_norm,i_sigma,vmsa::mInteMethod[i_method].c_str()));                             
