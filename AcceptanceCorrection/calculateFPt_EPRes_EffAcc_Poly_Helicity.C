...
void calculateFPt_EPRes_EffAcc_Poly_Helicity(const int energy = 4, const int pid = 0, bool doall = true, bool isBesI = false, bool random3D = false, int etamode = 0, int order = 2, int yspectra = -10, int EP = 0, int v2 = 1, int EPCorr = 0) {
...
//  Double_t pt_set[8] = {0.6, 1.4, 2.2, 3.0, 3.8, 4.6, 5.4, 7.2};
  ...
  if(doall)
  {
  double eff_plateau[10][7][14];
  double eff_plateau_error[10][7][14];
  double eff_eta[10][7][14];
  double eff_eta_error[10][7][14];
  //if(eff_mode==0)
  ...
  if(energy == 3 || energy == 4)
  {
    ...
    if(order == 2) eff_file_plateau = new TFile(Form("../RcPhiEffCorr_Cos2PhiStarPhi/Eff_%s_SingleParticle_noToF_Mode0_EtaMode%d_PtBins2_5.root",vmsa::mBeamEnergy[energy].c_str(),etamode),"READ");
  ...
  if(energy == 0)
  {
    if(order == 1) eff_file_plateau = new TFile(Form("../RcPhiEffCorr/order1/Eff_%s_SingleParticle_noToF_Mode0_EtaMode%d.root",vmsa::mBeamEnergy[energy].c_str(),etamode),"READ");
    if(order == 2) eff_file_plateau = new TFile(Form("../RcPhiEffCorr/order2/Eff_%s_SingleParticle_noToF_Mode0_EtaMode%d.root",vmsa::mBeamEnergy[energy].c_str(),etamode),"READ");
  //TFile *eff_file_eta = new TFile(Form("/gpfs01/star/pwg/gwilks3/VectorMesonSpinAlignment/Data/%s/Efficiency/Cos/Eff_%s_SingleParticle_2060_fittoeta.root",vmsa::mPID[pid].c_str(),vmsa::mBeamEnergy[energy].c_str()),"READ");
  }
     ...
     for(int k=1; k<=14; k++)
      {
        ...
        Title = new TString("h_mEffCosH_Cent_");
        *Title += 9;
        *Title += "_Pt_";
        *Title += (j-1);
        //Title->Print();
  ...
  */
  ...
  if(order == 2) input = new TFile(Form("rho00/%s/%s/Poly/Raw%sPtSys_%s_PolySys_Helicity.root",vmsa::mPID[pid].c_str(),vmsa::mBeamEnergy[energy].c_str(),vmsa::mPID[pid].c_str(),etastring.c_str()),"READ");
  ...
  TH1D *Big_yield = new TH1D("Big_yield","Big_yield", 14, -1, 1);
  ...
  string outputname;
  if(order == 1) outputname = Form("output/%s/%s/AccRes%sPtSys_%s_Poly_FirstOrder%s%s%s.root",vmsa::mPID[pid].c_str(),vmsa::mBeamEnergy[energy].c_str(),vmsa::mPID[pid].c_str(),etastring.c_str(),spectra.c_str(),eptext.c_str(),v2text.c_str());
  if(order == 2) outputname = Form("output/%s/%s/AccRes%sPtSys_%s_PolySys_Helicity.root",vmsa::mPID[pid].c_str(),vmsa::mBeamEnergy[energy].c_str(),vmsa::mPID[pid].c_str(),etastring.c_str());
  if(!EPCorr)
  {
    if(order == 1) outputname = Form("output/%s/%s/EffAcc%sPtSys_%s_Poly_FirstOrder%s%s%s.root",vmsa::mPID[pid].c_str(),vmsa::mBeamEnergy[energy].c_str(),vmsa::mPID[pid].c_str(),etastring.c_str(),spectra.c_str(),eptext.c_str(),v2text.c_str());
    if(order == 2) outputname = Form("output/%s/%s/EffAcc%sPtSys_%s_Poly_Helicity.root",vmsa::mPID[pid].c_str(),vmsa::mBeamEnergy[energy].c_str(),vmsa::mPID[pid].c_str(),etastring.c_str());
  }
                ...
                for(int i_F = 0; i_F < 1; i_F++)
                {
                  for(int i_eff = 0; i_eff < 1; i_eff++)
                  {
                    ...
                    for(Int_t PtBin=3; PtBin<=6; PtBin++) 
                    {
                      ...
                      TH1F *PtCos = new TH1F(key.c_str(),key.c_str(), 14, -1, 1);
                      //delete Title;
                      for(int i=1; i<=14; i++) {
                      ...
                      TF1 *Func_obs = new TF1("Func_obs",SpinDensity,-1,1,2);
                      Func_obs->SetParameter(0,0.33);
                      Func_obs->SetParameter(1,PtCos->GetMaximum());

                      PtCos->Fit(Func_obs,"NMI"); // fit corrected distribution for observerved rho00
                      double rho_obs = Func_obs->GetParameter(0);
                      double rho_obs_err = Func_obs->GetParError(0);
                      cout << "rho00 observed = " << rho_obs << " +/- " << rho_obs_err << endl;
                      ...
                      Float_t weight = PtCos->Integral(1,14);
