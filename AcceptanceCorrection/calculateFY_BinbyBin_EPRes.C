...
void calculateFY_BinbyBin_EPRes(const int energy = 4, const int pid = 0, bool doall = false, bool isBesI = false, bool random3D = false, int mode = 1, int etamode = 3, int ypadding = 5, int ipt = 1, int icent = 1, std::string ep = "sub") {
  ...
  for(int iy = 0; iy < vmsa::y_total; iy++)
  {
    ...
    Func_A->SetParameter(1,0);
    h_theta_star_ratio[ipt][icent][iy]->Fit(Func_A,"NMIER");
  ...
  if(doall)
  {
    ...
    TFile *input = new TFile(Form("rho00/%s/%s/fixed_ptbin/RawRhoEtaSys_%s.root",vmsa::mPID[pid].c_str(),vmsa::mBeamEnergy[energy].c_str(),etastring.c_str()),"READ");
    
    string outputname = Form("output/%s/%s/AccResRhoEtaSys_%s_cent_%d_ipt_%d_%s%s.root",vmsa::mPID[pid].c_str(),vmsa::mBeamEnergy[energy].c_str(),etastring.c_str(),icent,ipt,ep.c_str(),xtraopt.c_str());
          ...
          for(int i_sigma = vmsa::Sig_start; i_sigma < vmsa::Sig_stop; ++i_sigma)
          {
            for(int i_method = vmsa::Method_start; i_method < vmsa::Method_stop; ++i_method)
            {
              ...
              for(int iy = 0+ypadding; iy < vmsa::y_total-ypadding; iy++) 
              {
                ...
                PtCos[iy]->Fit(Func_rho[iy], "NMIER");
