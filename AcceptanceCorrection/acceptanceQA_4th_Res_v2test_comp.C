...
void acceptanceQA_4th_Res_v2test_comp(const int energy = 4, const int pid = 0, bool doall = true, std::string res = "0p4", double resval = 0.4, int cut = 1, int method = 0, int v2 = 1) {
...
*/  
  ...
  std::string FigureName = Form("./figures/AcceptanceQA/AcceptanceQAInput_%s_RecoObs_%s%s_method%d.pdf",ytextfile.c_str(),vmsa::mBeamEnergy[energy].c_str(),cutoption.c_str(),method);
  ...
  FigureName = Form("./figures/AcceptanceQA/AcceptanceQADifferenceFromIdeal_%s_%s%s_method%d.pdf",ytextfile.c_str(),vmsa::mBeamEnergy[energy].c_str(),cutoption.c_str(),method);
