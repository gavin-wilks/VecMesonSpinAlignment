...
void fitcos_2nd_effsys_4thOrder(const int energy = 4, const int pid = 0, bool doall = true, bool isBesI = false, bool random3D = false, int etamode = 0) {
...
//  Double_t pt_set[8] = {0.6, 1.4, 2.2, 3.0, 3.8, 4.6, 5.4, 7.2};
  ...
  for(int ipt = 2; ipt < 6; ipt++)
  {
    ...
    TF1 *Func_A = new TF1("Func4th",Func4th,0,1,3);
    ...
    h_theta_clone->Fit(Func_Theta,"ERQ");
    ...
    Func_A->Draw("same");
    c_play->SaveAs(Form("FValues/pt%d_thetaStar_EtaMode%d.pdf",ipt,etamode));
  ...
  //if(eff_mode==0)
  TFile *eff_file_plateau = new TFile(Form("/gpfs01/star/pwg/gwilks3/VectorMesonSpinAlignment/Data/%s/Efficiency/Cos/Eff_%s_SingleParticle_2060_fittoplateau.root",vmsa::mPID[pid].c_str(),vmsa::mBeamEnergy[energy].c_str()),"READ");
  ...
  TFile *input = new TFile(Form("rho00/%s/%s/Raw%sPtSys.root",vmsa::mPID[pid].c_str(),vmsa::mBeamEnergy[energy].c_str(),vmsa::mPID[pid].c_str()),"READ");
  ...
  string outputname = Form("output/%s/%s/AccRes%sPtSys_4th.root",vmsa::mPID[pid].c_str(),vmsa::mBeamEnergy[energy].c_str(),vmsa::mPID[pid].c_str());
              ...
              for(Int_t PtBin=3; PtBin<=6; PtBin++) 
              {
                ...
                for(int i=1; i<=7; i++) {
                  ...
                  if(i_eff == 0)
                  {
                    ...
                    PtCos->SetBinError(i, inte_mean/eff_plateau[i_cent][PtBin-1][i-1]*TMath::Sqrt(inte_mean_error*inte_mean_error/inte_mean/inte_mean+eff_plateau_error[i][PtBin-1][i-1]*eff_plateau_error[i][PtBin-1][i-1]));
                  }
                  if(i_eff == 1)
                  {
