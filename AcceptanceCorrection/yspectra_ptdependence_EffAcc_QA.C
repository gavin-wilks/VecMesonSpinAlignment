...
void yspectra_ptdependence_EffAcc_QA(const int energy = 4, const int pid = 0, int mode = 1, int etamode = 0, int ypadding = 2, int order = 2) {
  ...
  TH1D *h_theta_star_ratio[10];
  TH1D *h_theta_ratio[10];
  ...
  TString *Title;
  {
    ...
    TH1D *eff_ratio[vmsa::pt_rebin];

    TFile *eff_file;
    if(order == 1) eff_file = new TFile(Form("/gpfs01/star/pwg/gwilks3/VectorMesonSpinAlignment/AuAu%s_2019/OutPut/CosEfficiencyProccessTuple_EPRes_EffAcc_Ycut_Order1_NoRapiditySpectra/Eff_%s_SingleParticle_noToF_Mode0_EtaMode%d.root",vmsa::mBeamEnergy[energy].c_str(),vmsa::mBeamEnergy[energy].c_str(),etamode),"READ");
    if(order == 2) eff_file = new TFile(Form("/gpfs01/star/pwg/gwilks3/VectorMesonSpinAlignment/AuAu%s_2019/OutPut/CosEfficiencyProccessTuple_EPRes_EffAcc_Ycut_Order2_NoRapiditySpectra/Eff_%s_SingleParticle_noToF_Mode0_EtaMode%d.root",vmsa::mBeamEnergy[energy].c_str(),vmsa::mBeamEnergy[energy].c_str(),etamode),"READ");
    eff_file->Print();
    TFile *eff_file_y;
    if(order == 1) eff_file_y = new TFile(Form("/gpfs01/star/pwg/gwilks3/VectorMesonSpinAlignment/AuAu%s_2019/OutPut/CosEfficiencyProccessTuple_EPRes_EffAcc_Ycut_Order1_WithRapiditySpectra/Eff_%s_SingleParticle_noToF_Mode0_EtaMode%d.root",vmsa::mBeamEnergy[energy].c_str(),vmsa::mBeamEnergy[energy].c_str(),etamode),"READ");
    if(order == 2) eff_file_y = new TFile(Form("/gpfs01/star/pwg/gwilks3/VectorMesonSpinAlignment/AuAu%s_2019/OutPut/CosEfficiencyProccessTuple_EPRes_EffAcc_Ycut_Order2_WithRapiditySpectra/Eff_%s_SingleParticle_noToF_Mode0_EtaMode%d.root",vmsa::mBeamEnergy[energy].c_str(),vmsa::mBeamEnergy[energy].c_str(),etamode),"READ");
    eff_file_y->Print();

    for(int ipt = 2; ipt < 6; ipt++)
    {
      Title = new TString(Form("h_mEffCos_Cent_%d_Pt_%d",9,ipt));
      ...
      eff_hist->Print();
      eff_hist_y->Print();
      eff_ratio[ipt] = (TH1D*) eff_hist_y->Clone();
      eff_ratio[ipt]->Divide(eff_hist);
      eff_ratio[ipt]->Print();
    ...
    TCanvas *c_y = new TCanvas("c_y","c_y",10,10,600,600);
    c_y->Divide(2,2);

    string outputname = Form("output/%s/%s/yspectra_ptdpendenceQA_%s_Order%d.pdf",vmsa::mPID[pid].c_str(),vmsa::mBeamEnergy[energy].c_str(),etastring.c_str(),order);
    ...
    {
      c_y->cd(ipt-1);
      c_y->cd(ipt-1)->SetLeftMargin(0.15);
      c_y->cd(ipt-1)->SetBottomMargin(0.15);
      c_y->cd(ipt-1)->SetTicks(1,1);
      c_y->cd(ipt-1)->SetGrid(0,0); 

      cout << vmsa::pt_low[energy][ipt] << endl;
      cout << vmsa::pt_up[energy][ipt] << endl;
      string efftitle = Form("Cent %d-%d, %1.1f<p_{T}<%1.1f GeV/c", 20, 60, vmsa::pt_low[energy][ipt], vmsa::pt_up[energy][ipt]);
      cout << efftitle << endl;
      eff_ratio[ipt]->Print();
      eff_ratio[ipt]->SetTitle(efftitle.c_str());
      eff_ratio[ipt]->GetXaxis()->SetTitle("|cos#theta*|");
      eff_ratio[ipt]->GetYaxis()->SetTitle("(w/ y-spectra) / (w/o)");
      eff_ratio[ipt]->Draw("pE");
      for(int i = 0; i < 7; i++)
      {
        cout << eff_ratio[ipt]->GetBinContent(i+1) << endl;
      }  
    }
    c_y->Update();
