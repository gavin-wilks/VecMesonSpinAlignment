...
void McEtaF_Resolution_Beta_VaryV2_1stOrder(double Nrho=3333, int SetPt=4, int energy = 4, int pid = 0, int cent = 5, int const NMax = 30000, double rapidity = 1.0, int mode = 1, int etamode = 0, double Res = 0.449167, bool fullEP = 0, bool v2on = 0, float v2input = 0.0, const char* jobID = "1")
{
  v2flag = v2on;
  mNEvents = NMax;
  mEtaMode = etamode;
  ...
  const char* varlist = "Centrality:McPt:McP:McEta:McY:McPhi:McCos:McCosTheta:McKpEta:McKmEta:McCosRP:McBeta:McBetaP"; // reconstructed phi
  ...
  cout << "Sub event plane resolution = " << Res << endl;
  ...
  //}
  mChi*=TMath::Sqrt(2.0); // to get full chi
  cout << "mChi = " << mChi << endl;
  f_pDel = new TF1("deltaPsi",EventPlaneDist1st,-TMath::Pi(),TMath::Pi(),2);
...
//  double const y = gRandom->Uniform(-vmsa::acceptanceRapidity, vmsa::acceptanceRapidity);
  ...
  mPsi = Psi + delta; // EP, the RP is smeared by delta
  mPsiRP = Psi;       // RP
  ...
  if(v2flag)
  {
    ...
    phi += mPsiRP; //Therefore we must add RP
  }  
  else 
  {
  ...
  cos2r2  += TMath::Cos(2.0*(mPsi-mPsiRP));
  cos2del += TMath::Cos(2.0*delta);
  cosdel  += TMath::Cos(delta);
  sin2del += TMath::Sin(2.0*delta);
  sindel  += TMath::Sin(delta);

  //cout << "mRP = " << mRP << ",     mPsi = " << mPsi << ",    delta = " << delta << endl;

  //mPsiRP = Psi;
  //cout << "Original Psi EP = " << mPsi << endl;


  //cout << "Wrapped Psi EP = " << mPsi << endl;

  //cout << "y = " << y << ", Psi = " << Psi << ", delta = " << delta << ", phi = " << defaultphi << ", final phi = " << phi << endl;
...
void fill(TLorentzVector* lPhi, TLorentzVector const& lKplus, TLorentzVector const& lKminus, int centrality)
{
  ...
  TVector3 nQ(TMath::Sin(mPsi),-1.0*TMath::Cos(mPsi),0.0); // direction of angular momentum with un-smeared EP
  TVector3 nQRP(TMath::Sin(mPsiRP),-1.0*TMath::Cos(mPsiRP),0.0); // direction of angular momentum with un-smeared EP
  ...
  TLorentzVector lPsiRP, lPsiEP, lZ;

  lPsiRP.SetXYZT(nQRP.X(), nQRP.Y(), nQRP.Z(), 0.0);
  lPsiEP.SetXYZT(nQ.X(), nQ.Y(), nQ.Z(), 0.0);
  lZ.SetXYZT(zQ.X(), zQ.Y(), zQ.Z(), 0.0);

  lPsiRP.Boost(vMcBeta); // boost normal to RP to phi-meson rest frame
  lPsiEP.Boost(vMcBeta); // boost normal to EP to phi-meson rest frame
  lZ.Boost(vMcBeta); // boost normal to EP to phi-meson rest frame

  TVector3 vNRP = lPsiRP.Vect().Unit(); // momentum direction of Kplus in phi-meson rest frame
  TVector3 vNEP = lPsiEP.Vect().Unit(); // momentum direction of Kplus in phi-meson rest frame
  ...
  //cout << "CosThetaStarEP = " << CosThetaStarEP << endl;
  //cout << "CosThetaStarRP = " << CosThetaStarRP << endl;
  ...
  //cout << "cos(theta*)  = " << CosThetaStarRP << ",    theta*  = " << TMath::ACos(CosThetaStarRP) << endl;
  //cout << "cos(theta*') = " << CosThetaStarEP << ",    theta*' = " << TMath::ACos(CosThetaStarEP) << endl;
  ...
  TVector3 xVecRP(TMath::Cos(mPsiRP),TMath::Sin(mPsiRP),0.0); // direction of x axis in RP frame
  //TVector3 xVecRP(TMath::Cos(mRP),TMath::Sin(mRP),0.0); // direction of x axis in RP frame
  TVector3 xVecEP(TMath::Cos(mPsi),TMath::Sin(mPsi),0.0); // direction of x axis in EP frame

  TLorentzVector xPsiRP, xPsiEP;

  xPsiRP.SetXYZT(xVecRP.X(), xVecRP.Y(), xVecRP.Z(), 0.0);
  xPsiEP.SetXYZT(xVecEP.X(), xVecEP.Y(), xVecEP.Z(), 0.0);

  xPsiRP.Boost(vMcBeta); // boost normal to RP to phi-meson rest frame
  xPsiEP.Boost(vMcBeta); // boost normal to EP to phi-meson rest frame

  TVector3 xNRP = xPsiRP.Vect().Unit(); // momentum direction of Kplus in phi-meson rest frame
  TVector3 xNEP = xPsiEP.Vect().Unit(); // momentum direction of Kplus in phi-meson rest frame
  ...
  //RotIntoRPFrame(0,0) = TMath::Cos(mPsiRP);          RotIntoRPFrame(0,1) = 0.0;              RotIntoRPFrame(0,2) = -TMath::Sin(mPsiRP);
  //RotIntoRPFrame(1,0) = 0.0;                          RotIntoRPFrame(1,1) = 1.0;              RotIntoRPFrame(1,2) = 0.0;
  //RotIntoRPFrame(2,0) = TMath::Sin(mPsiRP);          RotIntoRPFrame(2,1) = 0.0;              RotIntoRPFrame(2,2) = TMath::Cos(mPsiRP);
  //  
  //TMatrixD RotIntoEPFrame(3,3);
  //RotIntoEPFrame(0,0) = TMath::Cos(mPsi);          RotIntoEPFrame(0,1) = 0.0;              RotIntoEPFrame(0,2) = -TMath::Sin(mPsi);
  //RotIntoEPFrame(1,0) = 0.0;                        RotIntoEPFrame(1,1) = 1.0;              RotIntoEPFrame(1,2) = 0.0;
  //RotIntoEPFrame(2,0) = TMath::Sin(mPsi);          RotIntoEPFrame(2,1) = 0.0;              RotIntoEPFrame(2,2) = TMath::Cos(mPsi);
  ...
  //cout << "CosThetaStarRP (from dot product) = " << CosThetaStarRP << "   cos(theta*) from sin(theta)sin(Psi-phi) = " << SinThetaStarZP*TMath::Sin(mRP - KplusPhi) << "   cos(theta*) from sin(theta)sin(phi-Psi) = " << SinThetaStarZP*TMath::Sin(KplusPhi-mRP) << endl;
  //cout << "CosThetaStarEP (from dot product) = " << CosThetaStarEP << "   cos(theta*') from sin(theta)sin(Psi-phi) = " << SinThetaStarZP*TMath::Sin(mPsi - KplusPhi) << "   cos(theta*') from sin(theta)sin(phi-Psi) = " << SinThetaStarZP*TMath::Sin(KplusPhi-mPsi) << endl;
