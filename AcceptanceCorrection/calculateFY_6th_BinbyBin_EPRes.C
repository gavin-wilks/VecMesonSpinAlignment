...
void calculateFY_6th_BinbyBin_EPRes(const int energy = 4, const int pid = 0, bool doall = false, bool isBesI = false, bool random3D = false, int mode = 1, int etamode = 3, int ypadding = 5, int ipt = 1, int icent = 1, std::string ep = "sub") {
  ...
  double Hval[15] = {0.0};//{-0.0104367, -0.0104367, -0.0112319, -0.00879225, -0.00634735, -0.0053006};//{0.0}; //0.08555382,0.0660837,0.0363711,-0.00175735,0.0126171,-0.0257385};
  double Herr[15] = {0.0};//{-0.0104367, -0.0104367, -0.0112319, -0.00879225, -0.00634735, -0.0053006};//{0.0}; //0.08555382,0.0660837,0.0363711,-0.00175735,0.0126171,-0.0257385};
  ...
  for(int iy = 0; iy < vmsa::y_total; iy++)
  {
    ...
    Gval[iy] = Func_A->GetParameter(2);
    Gerr[iy] = Func_A->GetParError(2);
    Hval[iy] = Func_A->GetParameter(3);
    Herr[iy] = Func_A->GetParError(3);
          ...
          for(int i_sigma = vmsa::Sig_start; i_sigma < vmsa::Sig_stop; ++i_sigma)
          {
            for(int i_method = vmsa::Method_start; i_method < vmsa::Method_stop; ++i_method)
            {
              ...
              for(int iy = 0+ypadding; iy < vmsa::y_total-ypadding; iy++) 
              {
                ...
                Func_rho[iy] = new TF1(Form("Func_rho_%d",iy),FuncAFGH,0,1,6);
                ...
                Func_rho[iy]->FixParameter(4, Hval[iy]);
