...
void samplesizeQA(const int energy = 4, const int pid = 0, bool doall = false) {
...
//  Double_t pt_set[8] = {0.6, 1.4, 2.2, 3.0, 3.8, 4.6, 5.4, 7.2};
  ...
  TFile *MCFiles[6]; 
  double rho00in[6] = {0.2667,0.30,0.3333,0.3667,0.40};
  TGraphAsymmErrors *g_rho00outBefore = new TGraphAsymmErrors();
  TGraphAsymmErrors *g_rho00outAfter  = new TGraphAsymmErrors();
  double Fval[6] = {0.0}; //0.08555382,0.0660837,0.0363711,-0.00175735,0.0126171,-0.0257385};
  double Ferr[6] = {0.0};
  double Fpval[6] = {0.0}; //0.08555382,0.0660837,0.0363711,-0.00175735,0.0126171,-0.0257385};
  double Fperr[6] = {0.0};
  double FfromFp[6] = {0.0};
  double FfromFperr[6] = {0.0};

  int samplesize[6] = {5,100,200,300,400,500};
  std::string files[6] = {"","_40004999","_30004999","_20004999","_10004999","_00004999"};

  for(int irho = 0; irho < 6; irho++)
  {
    MCFiles[irho] = new TFile(Form("/gpfs01/star/pwg/gwilks3/VectorMesonSpinAlignment/Data/%s/Acceptance/McAcceptanceOutput_pt3_energy%d_pid%d_cent9%s.root",vmsa::mPID[pid].c_str(),energy,pid,files[irho].c_str()),"READ");
    ...
    Func_A->SetParameter(0,h_theta[irho]->GetBinContent(1));
    ...
    c_play->SetTicks(1,1);
    c_play->cd();
    h_theta[irho]->Draw("pE");
    Func_A->Draw("same");
    c_play->SaveAs(Form("FValues/samplesizeQA/Fstar_file%d.pdf",irho));
    delete c_play;
    ...
    FfromFperr[irho] = TMath::Abs((Fpval[irho]/(2.-Fpval[irho])/(2.-Fpval[irho])-1./(2.-Fpval[irho]))*Fperr[irho]);
    ...
    Func_A->Draw("same");
    c_play->SaveAs(Form("FValues/samplesizeQA/file%d.pdf",irho));
    delete c_play;
  ...
  for(int irho = 0; irho < 6; irho++)
  {
    cout << std::fixed << std::setprecision(5) << "Sample size: "; (irho == 0)? cout << "  " : ; cout << samplesize[irho] << "M   " << "F = " << Fval[irho] << " +/- " << Ferr[irho] << "     F* = " << Fpval[irho] << " +/- " << Fperr[irho] << "     F from F* = " << FfromFp[irho] << " +/- " << FfromFperr[irho] << endl;
  }
  ...
  for(int irho = 0; irho < 5; irho++) 
  {
    ...
    Func_rho->FixParameter(2, -Fpval[irho]/(2+Fpval[irho]));
