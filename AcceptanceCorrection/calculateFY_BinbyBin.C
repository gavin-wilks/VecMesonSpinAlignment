...
void calculateFY_BinbyBin(const int energy = 4, const int pid = 0, bool doall = false, bool isBesI = false, bool random3D = false, int mode = 1, int etamode = 3, int ypadding = 5, int ipt = 1, int icent = 1) {
...
//  Double_t pt_set[8] = {0.6, 1.4, 2.2, 3.0, 3.8, 4.6, 5.4, 7.2};
 ...
 // c_play->Print(output_start.c_str());
  ...
  std::string outputname = Form("figures/Fplots_%s_%s_Mode%d_EtaMode%d_rapidity_theta_cent%d_pt%d.pdf",vmsa::mPID[pid].c_str(),vmsa::mBeamEnergy[energy].c_str(),mode,etamode,icent,ipt);
  ...
  outputname = Form("figures/Fplots_%s_%s_Mode%d_EtaMode%d_rapidity_thetastar_cent%d_pt%d.pdf",vmsa::mPID[pid].c_str(),vmsa::mBeamEnergy[energy].c_str(),mode,etamode,icent,ipt);
