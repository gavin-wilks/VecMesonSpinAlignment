...
void acceptanceQA_4th(const int energy = 4, const int pid = 0, bool doall = true, std::string res = "0p6", double resval = 0.6) {
...
//  Double_t pt_set[8] = {0.6, 1.4, 2.2, 3.0, 3.8, 4.6, 5.4, 7.2};
  ...
  for(int irho = 0; irho < 5; irho++)
  {
    ...
    MCFiles[irho] = new TFile(Form("/gpfs01/star/pwg/gwilks3/VectorMesonSpinAlignment/Data/%s/Acceptance/Resolution_0p8y1_%s/McAcceptanceOutput_pt1_energy%d_pid%d_cent5_Res%s_rho%d.root",vmsa::mPID[pid].c_str(),res.c_str(),energy,pid,res.c_str(),irho),"READ");
    ...
    legend->Draw("same");
    c1->SaveAs(Form("FValues/0p8y1/pt1_%d.pdf",irho));
    delete c1;
  ...
  for(int irho = 0; irho < 5; irho++) 
  {
    ...
    //Func_rho->Draw("same");
    //delete Func_rdl;
    TString *Title = new TString(Form("fit/acceptanceQA/yield_pt_3_cent_9_rho%d.pdf",irho));
