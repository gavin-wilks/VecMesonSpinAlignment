...
void calculateFPt_EPRes_EffAcc(const int energy = 4, const int pid = 0, bool doall = false, bool isBesI = false, bool random3D = false, int etamode = 0, int order = 1) {
...
//  Double_t pt_set[8] = {0.6, 1.4, 2.2, 3.0, 3.8, 4.6, 5.4, 7.2};
  ...
  TFile *eff_file_plateau;
    if(order == 1) eff_file_plateau = new TFile(Form("/gpfs01/star/pwg/gwilks3/VectorMesonSpinAlignment/AuAu19GeV_2019/OutPut/CosEfficiencyProccessTuple_EPRes_EffAcc_Ycut_Order1/Eff_19GeV_SingleParticle_noToF_Mode0_EtaMode%d.root",etamode),"READ");
    if(order == 2) eff_file_plateau = new TFile(Form("/gpfs01/star/pwg/gwilks3/VectorMesonSpinAlignment/AuAu19GeV_2019/OutPut/CosEfficiencyProccessTuple_EPRes_EffAcc_Ycut_Order2_FixedRes/Eff_19GeV_SingleParticle_noToF_Mode0_EtaMode%d.root",etamode),"READ");
    if(energy == 3 && order == 1) eff_file_plateau = new TFile(Form("/gpfs01/star/pwg/gwilks3/VectorMesonSpinAlignment/AuAu14GeV_2019/OutPut/CosEfficiencyProccessTuple_EPRes_EffAcc_Ycut_Order1/Eff_14GeV_SingleParticle_noToF_Mode0_EtaMode%d.root",etamode),"READ");
    if(energy == 3 && order == 2) eff_file_plateau = new TFile(Form("/gpfs01/star/pwg/gwilks3/VectorMesonSpinAlignment/AuAu14GeV_2019/OutPut/CosEfficiencyProccessTuple_EPRes_EffAcc_Ycut_Order2/Eff_14GeV_SingleParticle_noToF_Mode0_EtaMode%d.root",etamode),"READ");
  ...
  */
  TFile *input;
  if(order == 1) input = new TFile(Form("rho00/%s/%s/Raw%sPtSys_%s_FirstOrder.root",vmsa::mPID[pid].c_str(),vmsa::mBeamEnergy[energy].c_str(),vmsa::mPID[pid].c_str(),etastring.c_str()),"READ");
  if(order == 2) input = new TFile(Form("rho00/%s/%s/Raw%sPtSys_%s.root",vmsa::mPID[pid].c_str(),vmsa::mBeamEnergy[energy].c_str(),vmsa::mPID[pid].c_str(),etastring.c_str()),"READ");
  ...
  string outputname;
  if(order == 1) outputname = Form("output/%s/%s/AccRes%sPtSys_%s_FirstOrder.root",vmsa::mPID[pid].c_str(),vmsa::mBeamEnergy[energy].c_str(),vmsa::mPID[pid].c_str(),etastring.c_str());
  if(order == 2) outputname = Form("output/%s/%s/AccRes%sPtSys_%s.root",vmsa::mPID[pid].c_str(),vmsa::mBeamEnergy[energy].c_str(),vmsa::mPID[pid].c_str(),etastring.c_str());
        ...
        for(int i_norm = vmsa::Norm_start; i_norm < vmsa::Norm_stop; ++i_norm)
        {
          for(int i_sigma = vmsa::Sig_start; i_sigma < vmsa::Sig_stop; ++i_sigma)
          {
            ...
            for(int i_eff = 0; i_eff < 1; i_eff++)
            {
              ...
              Title = new TString(Form("fit/Phi/%s/Order%d/pTstudy/yield_%s_Dca_%d_Sig_%d_%s_Norm_%d_Sigma_%d_%s_EtaMode%d_Divided.pdf",vmsa::mBeamEnergy[energy].c_str(),order,ordertext[order-1].c_str(),i_dca,i_sig,vmsa::mPID[pid].c_str(),i_norm,i_sigma,vmsa::mInteMethod[i_method].c_str(),etamode));

              for(Int_t PtBin=3; PtBin<=6; PtBin++) 
              {
                string key = Form("pt_%d_Centrality_%d_%s_Dca_%d_Sig_%d_%s_Norm_%d_Sigma_%d_%s",PtBin-1,i_cent,ordertext[order-1].c_str(),i_dca,i_sig,vmsa::mPID[pid].c_str(),i_norm,i_sigma,vmsa::mInteMethod[i_method].c_str());
              ...
              Title = new TString(Form("rhoRaw_Centrality_%d_%s_Dca_%d_Sig_%d_%s_Norm_%d_Sigma_%d_%s_F_%d_Eff_%d",i_cent,ordertext[order-1].c_str(),i_dca,i_sig,vmsa::mPID[pid].c_str(),i_norm,i_sigma,vmsa::mInteMethod[i_method].c_str(),i_F,i_eff));                             
              ...
              Title = new TString(Form("rhoFinalWeighted_Centrality_%d_%s_Dca_%d_Sig_%d_%s_Norm_%d_Sigma_%d_%s_F_%d_Eff_%d",i_cent,ordertext[order-1].c_str(),i_dca,i_sig,vmsa::mPID[pid].c_str(),i_norm,i_sigma,vmsa::mInteMethod[i_method].c_str(),i_F,i_eff));                             
