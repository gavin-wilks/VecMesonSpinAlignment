...
TProfile *resolution;
TH1F *h_theta[vmsa::eta_total], *h_theta_before[vmsa::eta_total], *h_theta_star[vmsa::eta_total], *h_theta_star_before[vmsa::eta_total], *h_out1, *h_out2;
TProfile *cos2phi;
TF1 *cut_pt = new TF1("cut_pt","1",0,1);

Double_t pt_set[2] = {1.0,5.0};
...
void McEtaF_PhiEta(double Nrho=3333, int energy = 4, int pid = 0, int cent = 10, int const NMax = 10000, double rapidity = 1.0, const char* jobID = "1")
{
  string outputfile = Form("McAcceptanceOutput_EtaDependence_energy%d_pid%d_cent%d_%s.root",energy,pid,cent,jobID);
  ...
  cout << "yinput = " << yinput << endl;
  cout << "pt range = [" << pt_set[0] << "," << pt_set[1] << "] GeV/c" << endl;  
  ...
  for(int i = 0; i < vmsa::eta_total; i++){
    h_theta[i] = new TH1F(Form("h_theta_%d",i),Form("h_theta_%d",i), 10, 0, 1);
    h_theta_before[i] = new TH1F(Form("h_theta_before_%d",i),Form("h_theta_before_%d",i), 10, 0, 1);
    h_theta_star[i] = new TH1F(Form("h_theta_star_%d",i),Form("h_theta_star_%d",i), 7, 0, 1);
    h_theta_star_before[i] = new TH1F(Form("h_theta_star_before_%d",i),Form("h_theta_star_before_%d",i), 7, 0, 1);
...
TF1* readspec(int energy, int pid, int centrality)
{
  ...
  string InPutSpec = "/gpfs01/star/pwg/gwilks3/VectorMesonSpinAlignment/Data/Phi/pTspectra/HEPData-ins1378002-v1-root.root";
  TFile *File_Spec = TFile::Open(InPutSpec.c_str());
  cout << "Input spectra" << InPutSpec << endl;
 
  //g_spec = (TGraphAsymmErrors*)File_Spec->Get("g_spec");
  double ptweighted[11] = {0.0};
  double yieldval[11] = {0.0};
  double yielderr[11] = {0.0};
 
  for(int i_cent = 0; i_cent < 9; ++i_cent)
  {
    if(i_cent == 1 || i_cent == 3 || i_cent == 7) continue;

    TDirectory *dir = (TDirectory*) File_Spec->Get(Form("Table %d",tableNum[energy][i_cent]));
    dir->cd(); 
    TGraphAsymmErrors *g_spectemp = (TGraphAsymmErrors*)dir->Get("Graph1D_y1");
    
    for(int i = 0; i < g_spectemp->GetN(); i++)
    {
      double pt,yield;
      g_spectemp->GetPoint(i,pt,yield);
      double err = g_spectemp->GetErrorYlow(i);
      ptweighted[i] += (yield * ptvalues[energy][i_cent][i]);
      yieldval[i] += yield;
      yielderr[i] += (err * err);
  ...
  TGraphAsymmErrors *g_spec = new TGraphAsymmErrors();
  for(int i = 0; i < 11; ++i)
  {
    ptweighted[i] /= yieldval[i];
    yielderr[i] = TMath::Sqrt(yielderr[i]);
    g_spec->SetPoint(i,ptweighted[i],yieldval[i]);
    g_spec->SetPointError(i,fabs(ptweighted[i]-ptlow[energy][i]),fabs(ptweighted[i]-pthigh[energy][i]),yielderr[i],yielderr[i]);
  }
...
//  TF1 *f_spec = new TF1("f_spec",pTLevy,vmsa::ptMin,vmsa::ptMax,3);
  TF1 *f_spec = new TF1("f_spec",pTLevy,pt_set[0], pt_set[1], 3);
...
//  double const pt = f_spec->GetRandom(vmsa::ptMin, vmsa::ptMax);
  double const pt = f_spec->GetRandom(pt_set[0], pt_set[1]);
...
void fill(TLorentzVector* lPhi, TLorentzVector const& lKplus, TLorentzVector const& lKminus)
{
  ...
  for(int i = 0; i < vmsa::eta_total; i++){
    if(PhiEta > vmsa::eta_raw[i] && PhiEta < vmsa::eta_raw[i+1]){
      h_theta_before[i]->Fill(TMath::Abs(CosThetaStarZP));
      h_theta_star_before[i]->Fill(TMath::Abs(CosThetaStarEP));
...
//    if(TMath::Abs(KplusEta)<=eta_gap && TMath::Abs(KminusEta)<=eta_gap)
   ...
   // cout << "AFTER:  K+ eta = " << KplusEta << "     K- eta = " << KminusEta << "     phi rapidity = " << PhiRapidity << endl;
        h_theta[i]->Fill(TMath::Abs(CosThetaStarZP));
        h_theta_star[i]->Fill(TMath::Abs(CosThetaStarEP));
      }
      continue;
    }
  }
...
*/
  ...
  for(int i = 0; i < vmsa::eta_total; ++i)
  {
    h_theta_star_before[i]->Write();
    h_theta[i]->Write();
    h_theta_before[i]->Write();
    h_theta_star[i]->Write();
  }
  //h_out1->Write();
  //h_out2->Write();
  File_OutPut->Close();
