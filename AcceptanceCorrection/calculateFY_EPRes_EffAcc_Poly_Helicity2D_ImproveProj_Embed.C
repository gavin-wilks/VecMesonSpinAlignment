...
void calculateFY_EPRes_EffAcc_Poly_Helicity2D_ImproveProj_Embed(const int energy = 4, const int pid = 0, bool doall = true, bool isBesI = false, bool random3D = false, int etamode = 0, int order = 2, int yspectra = 2, int EP = 0, int v2 = 1) {
  ...
  if(doall)
  {
    ...
    TFile *eff_file_plateau;
      ...
      if(order == 2) eff_file_plateau = new TFile(Form("../anapicodst_phiembed/HelicityCorrections/Embedding_%s_20240820_Fixed.root",vmsa::mBeamEnergy[energy].c_str()),"READ");
    ...
    for(int i=9; i<10;i++)
    {
      for(int j=1; j<=10; j++)
      {
        string histnamemc = Form("h_mRc0EffCosHPhiPrime_Cent_9_Y_%d",j-1);
        string histnamerc = Form("h_mRc8EffCosHPhiPrime_Cent_9_Y_%d",j-1);
            ...
            for(int i_sigma = vmsa::Sig_start; i_sigma < 1/*vmsa::Sig_stop*/; ++i_sigma)
            {
              for(int i_method = 1/*vmsa::Method_start*/; i_method < vmsa::Method_stop; ++i_method)
              {
                for(int i_poly = 0; i_poly < 1/*3*/; i_poly++)
                {
                  ...
                  //////////////////////   2D Efficiency ///////////////////////////
                  c->SaveAs(Form("figures/%s/%s/RapidityEmbed2DHelicityEfficiency.pdf",vmsa::mPID[pid].c_str(),vmsa::mBeamEnergy[energy].c_str()));
                  for(int i = 0; i < 10; i++)
                  {   
                    c->cd(i+1);
                  ...
                  c->SaveAs(Form("figures/%s/%s/RapidityEmbed2DHelicityEfficiencyCorrectedYields.pdf",vmsa::mPID[pid].c_str(),vmsa::mBeamEnergy[energy].c_str()));
                  //////////////////////   2D Efficiency ///////////////////////////
                  //////////////////////   2D Corrected Yields ///////////////////////////
                  for(int i = 0; i < 10; i++)
                  {   
                  ...
                  c->SaveAs(Form("figures/%s/%s/RapidityEmbed2DHelicity2DFunc.pdf",vmsa::mPID[pid].c_str(),vmsa::mBeamEnergy[energy].c_str()));
                  //////////////////////   2D Corrected Yields ///////////////////////////
                  //////////////////////   2D Raw Yields ///////////////////////////
                  for(int i = 0; i < 10; i++)
                  {   
                  ...
                  c->SaveAs(Form("figures/%s/%s/RapidityEmbed2DHelicityRawYields.pdf",vmsa::mPID[pid].c_str(),vmsa::mBeamEnergy[energy].c_str()));
                  //////////////////////   2D Raw Yields ///////////////////////////
                  //////////////////////   1D Cos Efficiency ///////////////////////////
                  for(int i = 0; i < 10; i++)
                  {   
                  ...
                  c->SaveAs(Form("figures/%s/%s/RapidityEmbed1DCosHelicityEfficiency.pdf",vmsa::mPID[pid].c_str(),vmsa::mBeamEnergy[energy].c_str()));
                  //////////////////////   1D Cos Efficiency ///////////////////////////
                  //////////////////////   1D Cos Corrected Yields Small Bins ///////////////////////////
  
                  string outputpdf = Form("figures/%s/%s/RapidityEmbed1DCosHelicityEfficiencyCorrectedYields_SmallBins.pdf",vmsa::mPID[pid].c_str(),vmsa::mBeamEnergy[energy].c_str());
                  ...
                  c->SaveAs(Form("figures/%s/%s/RapidityEmbed1DCosHelicityEfficiencyCorrectedYields.pdf",vmsa::mPID[pid].c_str(),vmsa::mBeamEnergy[energy].c_str()));
                  //////////////////////   1D Cos Corrected Yields ///////////////////////////
                  //////////////////////   1D Cos Raw Yields ///////////////////////////
                  for(int i = 0; i < 10; i++)
                  {   
                  ...
                  c->SaveAs(Form("figures/%s/%s/RapidityEmbed1DCosHelicityRawYields.pdf",vmsa::mPID[pid].c_str(),vmsa::mBeamEnergy[energy].c_str()));
                  //////////////////////   1D Cos Raw Yields ///////////////////////////
                  //////////////////////   1D Phi Efficiency  ///////////////////////////
                  for(int i = 0; i < 10; i++)
                  {   
                  ...
                  c->SaveAs(Form("figures/%s/%s/RapidityEmbed1DPhiHelicityEfficiency.pdf",vmsa::mPID[pid].c_str(),vmsa::mBeamEnergy[energy].c_str()));
                  //////////////////////   1D Phi Efficiency  ///////////////////////////
                  ...
                  c->SaveAs(Form("figures/%s/%s/RapidityEmbed1DPhiHelicityEfficiencyCorrectedYields.pdf",vmsa::mPID[pid].c_str(),vmsa::mBeamEnergy[energy].c_str()));
                  //////////////////////   1D Phi Corrected Yields  ///////////////////////////
                  //////////////////////   1D Phi Raw Yields  ///////////////////////////
                  ...
                  c->SaveAs(Form("figures/%s/%s/RapidityEmbed1DPhiHelicityRawYields.pdf",vmsa::mPID[pid].c_str(),vmsa::mBeamEnergy[energy].c_str()));
                  //////////////////////   1D Phi Raw Yields  ///////////////////////////
                  //////////////////////   1D Phi Raw Yields Small Bins ///////////////////////////
    ...
    if(order == 2) outputname = Form("output/%s/%s/AccPhiPtSys_%s_PolySys_Helicity_2D_OffDiag_Embed_Rapidity.root",vmsa::mPID[pid].c_str(),vmsa::mBeamEnergy[energy].c_str(),etastring.c_str(),spectra.c_str());
