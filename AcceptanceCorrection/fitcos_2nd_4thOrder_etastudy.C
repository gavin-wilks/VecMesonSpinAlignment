...
void fitcos_2nd_4thOrder_etastudy(const int energy = 4, const int pid = 0, bool doall = true, bool isBesI = false, bool random3D = false, int etamode = 0) {
...
//  Double_t pt_set[8] = {0.6, 1.4, 2.2, 3.0, 3.8, 4.6, 5.4, 7.2};
  ...
  for(int ipt = 2; ipt < 6; ipt++)
  {
    ...
    MCFiles[ipt] = new TFile(Form("/gpfs01/star/pwg/gwilks3/VectorMesonSpinAlignment/Data/%s/Acceptance/PtAcceptance_V2Off_spectraOff/McAcceptanceOutput_pt%d_energy%d_pid%d_cent9.root",vmsa::mPID[pid].c_str(),ipt+1,energy,pid),"READ");
