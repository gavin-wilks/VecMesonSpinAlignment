...
void calculateFPt_EPRes_EffAcc_Poly_Global2D(const int energy = 4, const int pid = 0, bool doall = true, bool isBesI = false, bool random3D = false, int etamode = 0, int order = 2, int yspectra = 2, int EP = 0, int v2 = 1) {
  ...
  if(doall)
  {
    double eff_plateau[10][10][10][10];
    double eff_plateau_error[10][10][10][10];
    //if(eff_mode==0)
    ...
    TFile *eff_file_plateau;
      ...
      //if(order == 2) eff_file_plateau = new TFile(Form("../RcPhiEffCorr_Cos2PhiStarPhi_PhiEmbed/GlobalCorrectionsMcDataRcWeight/Eff_%s_SingleParticle_noToF_Mode4_EtaMode%d_PtBins2_5.root",vmsa::mBeamEnergy[energy].c_str(),etamode),"READ");
      if(order == 2) eff_file_plateau = new TFile(Form("../RcPhiEffCorr_Cos2PhiStarPhi_PhiEmbed/GlobalCorrections/Eff_%s_SingleParticle_noToF_Mode0_EtaMode%d_PtBins2_5.root",vmsa::mBeamEnergy[energy].c_str(),etamode),"READ");
    ...
    for(int i=9; i<10;i++)
    {
      for(int j=3; j<=6; j++)
      {
        string histnamemc = Form("h_mMcEffCosPhiPrime_Cent_9_Pt_%d",j-1);
        string histnamerc = Form("h_mRc5EffCosPhiPrime_Cent_9_Pt_%d",j-1);
    ...
    if(order == 2) input = new TFile(Form("rho00/%s/%s/Poly/PhiPrime_RawPhiPtSys_%s_PolySys.root",vmsa::mPID[pid].c_str(),vmsa::mBeamEnergy[energy].c_str(),etastring.c_str()),"READ");
              ...
              for(int i_method = 1/*vmsa::Method_start*/; i_method < vmsa::Method_stop; ++i_method)
              {
                for(int i_poly = 0; i_poly < 1/*3*/; i_poly++)
                {
                  ...
                  for(Int_t PtBin=3; PtBin<=6; PtBin++) 
                  {
                    ...
                    yield_hist_plot[PtBin-1] = new TH2D(Form("correta%d",PtBin-1),Form("correta%d",PtBin-1), 10, -1, 1, 10, 0.0, TMath::Pi()*2.0);
                    for(int i=1; i<=10; i++) 
                    {
                      for(int j=1; j<=10; j++) 
                      {
                  ...
                  c->SaveAs(Form("figures/%s/%s/Pt2DGlobalEfficiency.pdf",vmsa::mPID[pid].c_str(),vmsa::mBeamEnergy[energy].c_str()));
                  for(int i = 0; i < 4; i++)
                  {   
                    c->cd(i+1);
                  ...
                  c->SaveAs(Form("figures/%s/%s/Pt2DGlobalEfficiencyCorrectedYields.pdf",vmsa::mPID[pid].c_str(),vmsa::mBeamEnergy[energy].c_str()));
                  for(int i = 0; i < 4; i++)
                  {   
                    c->cd(i+1);
                  ...
                  c->SaveAs(Form("figures/%s/%s/Pt2DGlobal2DFunc.pdf",vmsa::mPID[pid].c_str(),vmsa::mBeamEnergy[energy].c_str()));
                  for(int i = 0; i < 4; i++)
                  {   
                    c->cd(i+1);
                  ...
                  c->SaveAs(Form("figures/%s/%s/Pt2DGlobalRawYields.pdf",vmsa::mPID[pid].c_str(),vmsa::mBeamEnergy[energy].c_str()));
                  for(int i = 0; i < 4; i++)
                  { 
                    c->cd(i+1);           
                  ...
                  c->SaveAs(Form("figures/%s/%s/Pt1DCosGlobalEfficiency.pdf",vmsa::mPID[pid].c_str(),vmsa::mBeamEnergy[energy].c_str()));
                  for(int i = 0; i < 4; i++)
                  {   
                    c->cd(i+1);
                  ...
                  c->SaveAs(Form("figures/%s/%s/Pt1DCosGlobalEfficiencyCorrectedYields.pdf",vmsa::mPID[pid].c_str(),vmsa::mBeamEnergy[energy].c_str()));
                  for(int i = 0; i < 4; i++)
                  {   
                    c->cd(i+1);
                  ...
                  c->SaveAs(Form("figures/%s/%s/Pt1DCosGlobalRawYields.pdf",vmsa::mPID[pid].c_str(),vmsa::mBeamEnergy[energy].c_str()));
                  for(int i = 0; i < 4; i++)
                  {   
                    c->cd(i+1);           
                  ...
                  c->SaveAs(Form("figures/%s/%s/Pt1DPhiGlobalEfficiency.pdf",vmsa::mPID[pid].c_str(),vmsa::mBeamEnergy[energy].c_str()));
                  for(int i = 0; i < 4; i++)
                  {   
                    c->cd(i+1);
                  ...
                  c->SaveAs(Form("figures/%s/%s/Pt1DPhiGlobalEfficiencyCorrectedYields.pdf",vmsa::mPID[pid].c_str(),vmsa::mBeamEnergy[energy].c_str()));
                  for(int i = 0; i < 4; i++)
                  {   
                    c->cd(i+1);
                  ...
                  c->SaveAs(Form("figures/%s/%s/Pt1DPhiGlobalRawYields.pdf",vmsa::mPID[pid].c_str(),vmsa::mBeamEnergy[energy].c_str()));
                }
              }
            }
          }
    ...
    if(order == 2) outputname = Form("output/%s/%s/AccPhiPtSys_%s_PolySys_Global_2D_OffDiag.root",vmsa::mPID[pid].c_str(),vmsa::mBeamEnergy[energy].c_str(),etastring.c_str(),spectra.c_str());
