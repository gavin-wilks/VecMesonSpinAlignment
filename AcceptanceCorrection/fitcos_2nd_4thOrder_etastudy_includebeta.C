...
void fitcos_2nd_4thOrder_etastudy_includebeta(const int energy = 4, const int pid = 0, bool doall = true, bool isBesI = false, bool random3D = false, int etamode = 3) {
...
//  Double_t pt_set[8] = {0.6, 1.4, 2.2, 3.0, 3.8, 4.6, 5.4, 7.2};
  ...
  TProfile *p_cos2betacos4beta[6];
  TProfile *p_cos2betacos4betaP[6];
  ...
  int etaplot = 0;
  if(etamode == 0) etaplot = 1;
  if(etamode == 3) etaplot = 4;
  if(etamode == 4) etaplot = 3;
  if(etamode == 5) etaplot = 2;
  ...
  for(int ipt = 2; ipt < 6; ipt++)
  {
    ...
    //MCFiles[ipt] = new TFile(Form("/gpfs01/star/pwg/gwilks3/VectorMesonSpinAlignment/Data/%s/Acceptance/PtDependence_v2On/McAcceptanceOutput_pt%d_energy%d_pid%d_cent9_EtaMode_0.root",vmsa::mPID[pid].c_str(),ipt+1,energy,pid),"READ");
    MCFiles[ipt] = new TFile(Form("/gpfs01/star/pwg/gwilks3/VectorMesonSpinAlignment/Data/%s/Acceptance/PtAcceptance_V2On/cos2bcos4b/McAcceptanceOutput_pt%d_energy%d_pid%d_cent9_EtaMode_0.root",vmsa::mPID[pid].c_str(),ipt+1,energy,pid),"READ");
    ...
    //h_theta_before[ipt] = (TH1D*) MCFiles[ipt]->Get(Form("h_theta_before_%d",etaplot));
    h_theta_star_before[ipt] = (TH1D*) MCFiles[ipt]->Get(Form("h_theta_star_%d",0));
    //h_theta[ipt] = (TH1D*) MCFiles[ipt]->Get(Form("h_theta_%d",etaplot));
    h_theta_star[ipt] = (TH1D*) MCFiles[ipt]->Get(Form("h_theta_star_%d",etaplot));

    p_cos2beta[ipt]  = (TProfile*) MCFiles[ipt]->Get(Form("p_cos2beta_%d",etaplot));
    p_cos2betaP[ipt] = (TProfile*) MCFiles[ipt]->Get(Form("p_cos2betaP_%d",etaplot));
    p_cos4beta[ipt]  = (TProfile*) MCFiles[ipt]->Get(Form("p_cos4beta_%d",etaplot));
    p_cos4betaP[ipt] = (TProfile*) MCFiles[ipt]->Get(Form("p_cos4betaP_%d",etaplot));
    p_cos2betacos4beta[ipt] = (TProfile*) MCFiles[ipt]->Get(Form("p_cos2betacos4beta_%d",etaplot));
    p_cos2betacos4betaP[ipt] = (TProfile*) MCFiles[ipt]->Get(Form("p_cos2betacos4betaP_%d",etaplot));

    TF1 *cos2b = new TF1("cos2b","[0]+[1]*x+[2]*x*x",0,1);
    TF1 *cos4b = new TF1("cos4b","[0]+[1]*x+[2]*x*x",0,1);
    TF1 *cos2bcos4b = new TF1("cos4b","[0]+[1]*x+[2]*x*x",0,1);

    p_cos2betaP[ipt]->Fit(cos2b,"NER");
    p_cos4betaP[ipt]->Fit(cos4b,"NER");
    p_cos2betacos4betaP[ipt]->Fit(cos2bcos4b,"NER");

    b2a[ipt] = cos2b->GetParameter(0);
    b2b[ipt] = cos2b->GetParameter(1);
    b2c[ipt] = cos2b->GetParameter(2);
    b4a[ipt] = cos4b->GetParameter(0);
    b4b[ipt] = cos4b->GetParameter(1);
    b4c[ipt] = cos4b->GetParameter(2);
    b2b4a[ipt] = cos2bcos4b->GetParameter(0);
    b2b4b[ipt] = cos2bcos4b->GetParameter(1);
    b2b4c[ipt] = cos2bcos4b->GetParameter(2);


    TCanvas *c_cos2beta = new TCanvas("c_cos2beta","c_cos2beta",10,10,800,800);
    c_cos2beta->cd();
    p_cos2betaP[ipt]->SetMarkerColor(kBlack);
    p_cos2betaP[ipt]->SetLineColor(kBlack);
    p_cos2betaP[ipt]->GetXaxis()->SetTitle("cos(#theta*)");
    p_cos2betaP[ipt]->GetYaxis()->SetTitle("<cos(2#beta)");
    p_cos2betaP[ipt]->Draw("pE");
    cos2b->SetLineColor(kRed);
    cos2b->Draw("l same");
    c_cos2beta->SaveAs(Form("figures/cos2beta_etamode%d_pt%d.pdf",etamode,ipt));

    TCanvas *c_cos4beta = new TCanvas("c_cos4beta","c_cos4beta",10,10,800,800);
    c_cos4beta->cd();
    p_cos4betaP[ipt]->SetMarkerColor(kBlack);
    p_cos4betaP[ipt]->SetLineColor(kBlack);
    p_cos4betaP[ipt]->GetXaxis()->SetTitle("cos(#theta*)");
    p_cos4betaP[ipt]->GetYaxis()->SetTitle("<cos(4#beta)");
    p_cos4betaP[ipt]->Draw("pE");
    cos4b->SetLineColor(kRed);
    cos4b->Draw("l same");
    c_cos4beta->SaveAs(Form("figures/cos4beta_etamode%d_pt%d.pdf",etamode,ipt));
    ...
    Func_A[ipt] = new TF1("Func4th",Func4th,0,1,9);
    ...
    Func_A[ipt]->SetParameter(1,0.0);
    Func_A[ipt]->FixParameter(3,b2a[ipt]);
    Func_A[ipt]->FixParameter(4,b2b[ipt]);
    Func_A[ipt]->FixParameter(5,b2c[ipt]);
    Func_A[ipt]->FixParameter(6,b4a[ipt]);
    Func_A[ipt]->FixParameter(7,b4b[ipt]);
    Func_A[ipt]->FixParameter(8,b4c[ipt]);
    h_theta_star_clone[ipt]->Fit(Func_A[ipt],"NER");
  ...
  //for(int cent=2; cent<=5; cent++) {
  //  Res_12 += resolution_2sub->GetBinContent(cent+1)/resolution_2sub->GetBinError(cent+1)/resolution_2sub->GetBinError(cent+1);
  //  Res_12_weight += 1./resolution_2sub->GetBinError(cent+1)/resolution_2sub->GetBinError(cent+1);
  //}
  ...
  if(doall)
  {
  double eff_plateau[10][7][7];
  double eff_plateau_error[10][7][7];
  double eff_eta[10][7][7];
  double eff_eta_error[10][7][7];
  //if(eff_mode==0)
  ...
  TFile *eff_file_plateau = new TFile(Form("../RcPhiEffCorr/Rebinned/Eff_%s_SingleParticle_noToF_Mode0_EtaMode%d.root",vmsa::mBeamEnergy[energy].c_str(),etamode),"READ");
     ...
     for(int k=1; k<=7; k++)
      {
        ...
        *Title += "_Pt_";
        *Title += (j-1);
        TH1D *eff_hist = (TH1D*)eff_file_plateau->Get(Title->Data());
        eff_plateau[i][j-1][k-1] = eff_hist->GetBinContent(k);
        eff_plateau_error[i][j-1][k-1] = eff_hist->GetBinError(k)/eff_plateau[i][j-1][k-1];
  ...
  eff_file_eta->Close();
  */
  TFile *input = new TFile(Form("rho00/%s/%s/Raw%sPtSys_%s.root",vmsa::mPID[pid].c_str(),vmsa::mBeamEnergy[energy].c_str(),vmsa::mPID[pid].c_str(),etastring.c_str()),"READ");
  ...
  TF1 *Func_rho = new TF1("Func_rho",FuncAFG,0,1,15);
  ...
  string outputname = Form("output/%s/%s/AccRes%sPtSys_%s.root",vmsa::mPID[pid].c_str(),vmsa::mBeamEnergy[energy].c_str(),vmsa::mPID[pid].c_str(),etastring.c_str());
            ...
            for(int i_eff = 0; i_eff < 1; i_eff++)
            {
              ...
              for(Int_t PtBin=3; PtBin<=6; PtBin++) 
              {
                ...
                for(int i=1; i<=7; i++) {
                  ...
                  if(i_eff == 0)
                  {
                    PtCos->SetBinContent(i, inte_mean/eff_plateau[i_cent][PtBin-1][i-1]);
                    PtCos->SetBinError(i, inte_mean/eff_plateau[i_cent][PtBin-1][i-1]*TMath::Sqrt(inte_mean_error*inte_mean_error/inte_mean/inte_mean+eff_plateau_error[i_cent][PtBin-1][i-1]*eff_plateau_error[i_cent][PtBin-1][i-1]));
                  }
                  if(i_eff == 1)
                  {
                    PtCos->SetBinContent(i, inte_mean/eff_eta[i_cent][PtBin-1][i-1]);
                    PtCos->SetBinError(i, inte_mean/eff_eta[i_cent][PtBin-1][i-1]*TMath::Sqrt(inte_mean_error*inte_mean_error/inte_mean/inte_mean+eff_eta_error[i_cent][PtBin-1][i-1]*eff_eta_error[i_cent][PtBin-1][i-1]));
                  }
                }
                ...
                Func_rho->FixParameter(2, Fval[PtBin-1]);
                Func_rho->FixParameter(3, Gval[PtBin-1]);
                ...
                Func_rho->FixParameter(5, b2a[PtBin-1]);
                Func_rho->FixParameter(6, b2b[PtBin-1]);
                Func_rho->FixParameter(7, b2c[PtBin-1]);
                Func_rho->FixParameter(8, b4a[PtBin-1]);
                Func_rho->FixParameter(9, b4b[PtBin-1]);
                Func_rho->FixParameter(10, b4c[PtBin-1]);
                Func_rho->FixParameter(11, b2b4a[PtBin-1]);
                Func_rho->FixParameter(12, b2b4b[PtBin-1]);
                Func_rho->FixParameter(13, b2b4c[PtBin-1]);
                ...
                PtCos->Fit(Func_rho, "NER");
                while(fabs(Func_rho->GetParameter(14)) > 1./2.*(1.-Func_rho->GetParameter(1)))
                { 
                  cout << "THIS PARAMETER IS INVALID" << endl;
                  Func_rho->SetParameter(1,  0.5);
                  Func_rho->SetParameter(14, 0.0); 
                  PtCos->Fit(Func_rho, "NER");
                }
                Func_rho->Draw("same");
              ...
              Title = new TString(Form("rhoRaw_Centrality_%d_2nd_Dca_%d_Sig_%d_%s_Norm_%d_Sigma_%d_%s_F_%d_Eff_%d",i_cent,i_dca,i_sig,vmsa::mPID[pid].c_str(),i_norm,i_sigma,vmsa::mInteMethod[i_method].c_str(),i_F,i_eff));                             
              ...
              Title = new TString(Form("rhoFinalWeighted_Centrality_%d_2nd_Dca_%d_Sig_%d_%s_Norm_%d_Sigma_%d_%s_F_%d_Eff_%d",i_cent,i_dca,i_sig,vmsa::mPID[pid].c_str(),i_norm,i_sigma,vmsa::mInteMethod[i_method].c_str(),i_F,i_eff));                             
