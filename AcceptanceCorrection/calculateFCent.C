...
void calculateFCent(const int energy = 4, const int pid = 0, int ptbin = 0, bool doall = true, bool isBesI = false, bool random3D = false, int mode = 0, int etamode = 0) {
...
//  Double_t pt_set[8] = {0.6, 1.4, 2.2, 3.0, 3.8, 4.6, 5.4, 7.2};
 ...
 // c_play->Print(output_start.c_str());
  ...
  std::string outputname = Form("figures/Fplots_%s_%s_Mode%d_EtaMode%d_cent_thetastar.pdf",vmsa::mPID[pid].c_str(),vmsa::mBeamEnergy[energy].c_str(),mode,etamode);
  ...
  for(int ipt = vmsa::pt_rebin_first_cent[energy]; ipt <= vmsa::pt_rebin_last_cent[energy]; ipt++)
  {
    ...
    for(int icent = 0; icent < 9; icent++)
    {
      ...
      //h_theta_star_before[ipt] = (TH1D*) MCFiles->Get("h_theta_star_before");
      h_theta_ratio[ipt][icent] = (TH1D*) MCFile->Get(Form("h_mEffCosS_Cent_%d_Pt_%d",icent,ipt))->Clone();
      //h_theta_star_ratio[ipt][icent]->GetYaxis()->SetTitle(Form("Cent %d  %1.1f<pT<%1.1f",icent,vmsa::pt_low_cent[energy][ipt],vmsa::pt_up_cent[energy][ipt]));
      cout << Form("Cent %d  %1.1f<pT<%1.1f",icent,vmsa::pt_low_cent[energy][ipt],vmsa::pt_up_cent[energy][ipt]) << endl;
      h_theta_ratio[ipt][icent]->GetXaxis()->SetTitle(Form("Cent %d    %1.1f<pT<%1.1f    cos#theta*",icent,vmsa::pt_low_cent[energy][ipt],vmsa::pt_up_cent[energy][ipt]));
      //h_theta[ipt] = (TH1D*) MCFiles->Get("h_theta");
      //h_out1[ipt] = (TH1D*) MCFiles->Get("h_out1");
      //h_out2[ipt] = (TH1D*) MCFiles->Get("h_out2");
      ...
      //h_theta_star_before[ipt]->Sumw2();
      //h_theta_star_clone->Divide(h_theta_star_before[ipt]); 
      Func_A->SetParameter(0,h_theta_ratio[ipt][icent]->GetBinContent(1));
      Func_A->SetParameter(1,0);
      h_theta_ratio[ipt][icent]->Fit(Func_A,"ER");
      ...
      h_theta_ratio[ipt][icent]->Draw("pE");
      Func_A->SetLineColor(kRed);
      Func_A->SetLineWidth(1);
      Func_A->Draw("same");
      //c_play->SaveAs(Form("FValues/pt%d.pdf",ipt));
  ...
  string outputname = Form("figures/Fplots_%s_%s_Mode%d_EtaMode%d_centrality_Fvalues.pdf",vmsa::mPID[pid].c_str(),vmsa::mBeamEnergy[energy].c_str(),mode,etamode);
  string output_start = Form("%s[",outputname.c_str());
  ...
  if(doall)
  {
    ...
    TFile *eff_file = new TFile(Form("../RcPhiEffCorr/Rebinned/Eff_19GeV_SingleParticle_noToF_Mode1_EtaMode%d.root",etamode),"READ");
    eff_file->Print();
    ...
    TFile *input = new TFile(Form("rho00/%s/%s/RawRhoCentSys_%s.root",vmsa::mPID[pid].c_str(),vmsa::mBeamEnergy[energy].c_str(),etastring.c_str()),"READ");
    ...
    string outputname = Form("output/%s/%s/AccResRhoCentSys_%s_pt_%d.root",vmsa::mPID[pid].c_str(),vmsa::mBeamEnergy[energy].c_str(),etastring.c_str(),ptbin);
          ...
          for(int i_norm = vmsa::Norm_start; i_norm < vmsa::Norm_stop; ++i_norm)
          {
            for(int i_sigma = vmsa::Sig_start; i_sigma < vmsa::Sig_stop; ++i_sigma)
            {
              for(int i_method = vmsa::Method_start; i_method < vmsa::Method_stop; ++i_method)
              {
                ...
                {
                  string key = Form("pt_%d_Centrality_%d_2nd_Dca_%d_Sig_%d_%s_Norm_%d_Sigma_%d_%s",ptbin,icent,i_dca,i_sig,vmsa::mPID[pid].c_str(),i_norm,i_sigma,vmsa::mInteMethod[i_method].c_str());
                  cout << key << endl;
                  ...
                  Func_rho->FixParameter(2, Fval[ptbin][icent]);
                  Func_rho->FixParameter(3, Gval[ptbin][icent]);
               ...
               // }
                ...
                Title = new TString(Form("rhoRaw_pt_%d_2nd_Dca_%d_Sig_%d_%s_Norm_%d_Sigma_%d_%s",ptbin,i_dca,i_sig,vmsa::mPID[pid].c_str(),i_norm,i_sigma,vmsa::mInteMethod[i_method].c_str()));                             
                ...
                Title = new TString(Form("rhoFinalWeighted_pt_%d_2nd_Dca_%d_Sig_%d_%s_Norm_%d_Sigma_%d_%s",ptbin,i_dca,i_sig,vmsa::mPID[pid].c_str(),i_norm,i_sigma,vmsa::mInteMethod[i_method].c_str()));                             
