...
void calculateFY_BinbyBin_EPRes_EffAcc_19GeV(const int energy = 4, const int pid = 0, bool doall = false, bool isBesI = false, bool random3D = false, int mode = 1, int etamode = 0, int ypadding = 2, int ipt = 1, int icent = 1, std::string ep = "sub", int order = 1) {
  ...
  if(doall)
  {
    ...
    TFile *eff_file;
    if(order == 1) eff_file = new TFile(Form("/gpfs01/star/pwg/gwilks3/VectorMesonSpinAlignment/AuAu19GeV_2019/OutPut/CosEfficiencyProccessTuple_EPRes_EffAcc_Ycut_Order1/Eff_19GeV_SingleParticle_noToF_Mode2_EtaMode%d.root",etamode),"READ");
    if(order == 2) eff_file = new TFile(Form("/gpfs01/star/pwg/gwilks3/VectorMesonSpinAlignment/AuAu19GeV_2019/OutPut/CosEfficiencyProccessTuple_EPRes_EffAcc_Ycut_Order2_FixedRes/Eff_19GeV_SingleParticle_noToF_Mode2_EtaMode%d.root",etamode),"READ");
    if(energy == 3 && order == 1) eff_file = new TFile(Form("/gpfs01/star/pwg/gwilks3/VectorMesonSpinAlignment/AuAu14GeV_2019/OutPut/CosEfficiencyProccessTuple_EPRes_EffAcc_Ycut_Order1/Eff_14GeV_SingleParticle_noToF_Mode2_EtaMode%d.root",etamode),"READ");
    if(energy == 3 && order == 2) eff_file = new TFile(Form("/gpfs01/star/pwg/gwilks3/VectorMesonSpinAlignment/AuAu14GeV_2019/OutPut/CosEfficiencyProccessTuple_EPRes_EffAcc_Ycut_Order2/Eff_14GeV_SingleParticle_noToF_Mode2_EtaMode%d.root",etamode),"READ");
    ...
    TFile *input;
    if(order == 1) input = new TFile(Form("rho00/%s/%s/RawRhoEtaSys_%s_FirstOrder.root",vmsa::mPID[pid].c_str(),vmsa::mBeamEnergy[energy].c_str(),etastring.c_str()),"READ");
    if(order == 2) input = new TFile(Form("rho00/%s/%s/fixed_ptbin/RawRhoEtaSys_%s.root",vmsa::mPID[pid].c_str(),vmsa::mBeamEnergy[energy].c_str(),etastring.c_str()),"READ");
    if(energy == 3 && order == 2) input = new TFile(Form("rho00/%s/%s/RawRhoEtaSys_%s.root",vmsa::mPID[pid].c_str(),vmsa::mBeamEnergy[energy].c_str(),etastring.c_str()),"READ");
    ...
    string outputname;
    if(order == 1) outputname = Form("output/%s/%s/AccResRhoEtaSys_%s_FirstOrder_cent_%d_ipt_%d.root",vmsa::mPID[pid].c_str(),vmsa::mBeamEnergy[energy].c_str(),etastring.c_str(),icent,ipt);
    if(order == 2) outputname = Form("output/%s/%s/AccResRhoEtaSys_%s_cent_%d_ipt_%d.root",vmsa::mPID[pid].c_str(),vmsa::mBeamEnergy[energy].c_str(),etastring.c_str(),icent,ipt);
    ...
    //    {
    //      for(int i_sigma = vmsa::Sig_start; i_sigma < 1/*msa::Sig_stop*/; ++i_sigma)
    //      {
    ...
    //        {
              ...
              Title = new TString(Form("fit/Phi/%s/Order%d/yield_pt_%d_cent_%d_%s_Dca_%d_Sig_%d_%s_Norm_%d_Sigma_%d_%s_EtaMode%d_Divided.pdf",vmsa::mBeamEnergy[energy].c_str(),order,ipt,icent,ordertext[order-1].c_str(),i_dca,i_sig,vmsa::mPID[pid].c_str(),i_norm,i_sigma,vmsa::mInteMethod[i_method].c_str(),etamode));
