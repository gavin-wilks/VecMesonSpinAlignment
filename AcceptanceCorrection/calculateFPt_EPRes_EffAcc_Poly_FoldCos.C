...
void calculateFPt_EPRes_EffAcc_Poly_FoldCos(const int energy = 4, const int pid = 0, bool doall = true, bool isBesI = false, bool random3D = false, int etamode = 0, int order = 2, int yspectra = 2, int EP = 0, int v2 = 1, int EPCorr = 1) {
...
//  Double_t pt_set[8] = {0.6, 1.4, 2.2, 3.0, 3.8, 4.6, 5.4, 7.2};
  ...
  if(energy == 3 || energy == 4)
  {
    ...
    if(order == 2) eff_file_plateau = new TFile(Form("../RcPhiEffCorr_Cos2PhiStarPhi_PhiEmbed/Eff_%s_SingleParticle_noToF_Mode0_EtaMode%d_PtBins2_5.root",vmsa::mBeamEnergy[energy].c_str(),etamode),"READ");
    //if(order == 2) eff_file_plateau = new TFile(Form("/gpfs01/star/pwg/gwilks3/VectorMesonSpinAlignment/AuAu%s_2019/OutPut/CosEfficiencyProccessTuple_EPRes_EffAcc_Ycut_Order2%s%s%s/Eff_%s_SingleParticle_noToF_Mode0_EtaMode%d.root",vmsa::mBeamEnergy[energy].c_str(),spectra.c_str(),eptext.c_str(),v2text.c_str(),vmsa::mBeamEnergy[energy].c_str(),etamode),"READ");
     ...
     for(int k=1; k<=7; k++)
      {
        ...
        Title = new TString("h_m5EffCos_Cent_");
        *Title += 9;
        *Title += "_Pt_";
        *Title += (j-1);
        //Title->Print();
        TH1D *eff_hist = (TH1D*)eff_file_plateau->Get(Title->Data());
        //double val1 = eff_hist->GetBinContent(k);
        //double val2 = eff_hist->GetBinContent(15-k);
        //double err1 = eff_hist->GetBinError(k);
        //double err2 = eff_hist->GetBinError(15-k);
  ...
  */
  ...
  if(order == 2) outputname = Form("output/%s/%s/AccRes%sPtSys_%s_Poly%s%s%s_FoldCos.root",vmsa::mPID[pid].c_str(),vmsa::mBeamEnergy[energy].c_str(),vmsa::mPID[pid].c_str(),etastring.c_str(),spectra.c_str(),eptext.c_str(),v2text.c_str());
  if(!EPCorr)
  {
    ...
    if(order == 2) outputname = Form("output/%s/%s/EffAcc%sPtSys_%s_Poly%s%s%s.root",vmsa::mPID[pid].c_str(),vmsa::mBeamEnergy[energy].c_str(),vmsa::mPID[pid].c_str(),etastring.c_str(),spectra.c_str(),eptext.c_str(),v2text.c_str());
  }
              ...
              for(int i_poly = 0; i_poly < 3; i_poly++)
              {
                for(int i_F = 0; i_F < 1; i_F++)
                {
                  for(int i_eff = 0; i_eff < 1; i_eff++)
                  {
                    ...
                    for(Int_t PtBin=3; PtBin<=6/*6*/; PtBin++) 
                    {
