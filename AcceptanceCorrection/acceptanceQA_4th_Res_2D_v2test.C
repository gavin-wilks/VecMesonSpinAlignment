...
void acceptanceQA_4th_Res_2D_v2test(const int energy = 4, const int pid = 0, bool doall = true, std::string res = "0p1", double resval = 0.1, int cut = 1, int method = 1 ) {
...
//  Double_t pt_set[8] = {0.6, 1.4, 2.2, 3.0, 3.8, 4.6, 5.4, 7.2};
  ...
  TH1D *h_theta_star_RP_before[5];

  TH2D *h2_theta_star_RP[5];
  TH2D *h2_theta_star_RP_before[5];
  TH2D *h2_theta_star[5];
  TH2D *h2_theta_star_before[5];


  TFile *MCFiles[5]; 
  double rho00in[5] = {0.25,0.33,0.40};
  int rho00val[5] = {2500,3300,4000};
  ...
  for(int irho = 0; irho < 3; irho++)
  {
    ...
    //MCFiles[irho] = new TFile(Form("/gpfs01/star/pwg/gwilks3/VectorMesonSpinAlignment/Data/%s/Acceptance/ResolutionTesting/yabs1_0p1_noEtaCut_fullRP/McAcceptanceOutput_pt1_energy%d_pid%d_cent5_EtaMode_0_nrho%d.root",vmsa::mPID[pid].c_str(),energy,pid,rho00val[irho]),"READ");
    MCFiles[irho] = new TFile(Form("/gpfs01/star/pwg/gwilks3/VectorMesonSpinAlignment/Data/%s/Acceptance/ResolutionTesting/ResolutionTest_0p1_EtaCut0_yabs1_v20/McAcceptanceOutput_pt1_energy%d_pid%d_cent4_EtaMode_0_nrho%d.root",vmsa::mPID[pid].c_str(),energy,pid,rho00val[irho]),"READ");
    ...
    h_theta_star_RP_before[irho] = (TH1D*)((TH1D*)MCFiles[irho]->Get("h_theta_star_before_RP"))->Clone(Form("h_theta_star_before_RP_%d",irho));


    h2_theta_star_before[irho] = (TH2D*)((TH2D*)MCFiles[irho]->Get("h2_theta_star_before"))->Clone(Form("h2_theta_star_before_%d",irho));
    h2_theta_star[irho] = (TH2D*)((TH2D*)MCFiles[irho]->Get("h2_theta_star"))->Clone(Form("h2_theta_star_%d",irho));
    h2_theta_star_RP[irho] = (TH2D*)((TH2D*)MCFiles[irho]->Get("h2_theta_star_RP"))->Clone(Form("h2_theta_star_RP_%d",irho));
    h2_theta_star_RP_before[irho] = (TH2D*)((TH2D*)MCFiles[irho]->Get("h2_theta_star_RP_before"))->Clone(Form("h2_theta_star_RP_before_%d",irho));

   
    TF1 *Func_rho = new TF1("Func_rho","[0]*(1.-[1]+(3.*[1]-1)*(x*x))",0,1);
    TF1 *Func_A = new TF1("Func_A",Func4th,0,1,3);
    Func_A->SetLineColor(kBlue);
    TF1 *Func_Theta = new TF1("Func_Theta","[0]*(1 + [2]*x*x*x*x + [3]*x*x*x*x*x*x)",0,1);
    ...
    if(cut == 0)
    {
      ...
      g_rho00outBefore->SetPoint(irho,rhoobs[irho],rhoreal[irho]);
      g_rho00outBefore->SetPointError(irho,0.0,0.0,Func_rho->GetParError(1),Func_rho->GetParError(1));
      g_rho00outBeforeIn->SetPoint(irho,rho00in[irho],rhoreal[irho]);
      g_rho00outBeforeIn->SetPointError(irho,0.0,0.0,Func_rho->GetParError(1),Func_rho->GetParError(1));
    }
    ...
    Fpval[irho] = Func_Theta->GetParameter(1);
    Fperr[irho] = Func_Theta->GetParError(1);
    Gpval[irho] = Func_Theta->GetParameter(2);
    Gperr[irho] = Func_Theta->GetParError(2);
    ...
    legend0->AddEntry(Func_Theta,"6th Order Fit","l");
    //legend->AddEntry(Func_Orig,"2nd Order Fit","l");
    legend0->Draw("same");
    c0->SaveAs(Form("FValues/0p8y1%s/pt1_%d%s_method%d_FROMTHETA.pdf",cutoption.c_str(),irho,cutoption.c_str(),method));
    delete c0;
    ...
    TH2D *h2_theta_star_clone = (TH2D*)h2_theta_star[irho]->Clone("h2_theta_star_clone");
    h2_theta_star_clone->Sumw2();
    h2_theta_star_before[irho]->Sumw2();
    h2_theta_star_clone->Divide(h2_theta_star_before[irho]); 

    TH1D *h_theta_star_integrated = new TH1D("h_theta_star_integrated","h_theta_star_integrated",7,0,1);
    for(int i = 0; i < h2_theta_star_clone->GetNbinsX(); i++)
    {
      double integral, error;
      integral = h2_theta_star_clone->IntegralAndError(i+1,i+1,1,h2_theta_star_clone->GetNbinsY(),error);
      h_theta_star_integrated->SetBinContent(i+1,integral);
      h_theta_star_integrated->SetBinError(i+1,error);
    }
    h_theta_star_integrated->Scale(1./float(h2_theta_star_clone->GetNbinsY()));

    cout << "Comparing Ratios from EP cos(theta*')" << endl;
    for(int i = 0; i < h2_theta_star_clone->GetNbinsX(); i++)
    {
      cout << "Original Method = " << h_theta_star_clone->GetBinContent(i+1) << " +/- " << h_theta_star_clone->GetBinError(i+1)
           << "           Integrate Beta = " << h_theta_star_integrated->GetBinContent(i+1) << " +/- " << h_theta_star_integrated->GetBinError(i+1) << endl;
    }
    ...
    h2_theta_star_clone->GetXaxis()->SetTitle("|cos(#theta*')|");
    h2_theta_star_clone->GetYaxis()->SetTitle("#beta");
    h2_theta_star_clone->Draw("colz");
    c2d->SaveAs(Form("FValues/yabs1/RatioPlot2D_EP_pt1_%d%s_method%d.pdf",irho,cutoption.c_str(),method));


    Func_A->SetParameter(0,h_theta_star_integrated->GetBinContent(1));
    ...
    if(cut == 1 && method == 0){
      h_theta_star_integrated->Fit(Func_A,"ER");
      ...
      h_theta_star_integrated->Draw("pE");
      Func_A->Draw("same"); 
    ...
    TH1D *h_theta_star_RP_clone = (TH1D*)h_theta_star_RP[irho]->Clone("h_theta_star_RP_clone");
    h_theta_star_RP_clone->Sumw2();
    h_theta_star_RP_before[irho]->Sumw2();
    h_theta_star_RP_clone->Divide(h_theta_star_RP_before[irho]); 

    TH2D *h2_theta_star_RP_clone = (TH2D*)h2_theta_star_RP[irho]->Clone("h2_theta_star_RP_clone");
    h2_theta_star_RP_clone->Sumw2();
    h2_theta_star_RP_before[irho]->Sumw2();
    h2_theta_star_RP_clone->Divide(h2_theta_star_RP_before[irho]); 

    TH1D *h_theta_star_RP_integrated = new TH1D("h_theta_star_RP_integrated","h_theta_star_RP_integrated",7,0,1);
    for(int i = 0; i < h2_theta_star_RP_clone->GetNbinsX(); i++)
    {
      double integral, error;
      integral = h2_theta_star_RP_clone->IntegralAndError(i+1,i+1,1,h2_theta_star_RP_clone->GetNbinsY(),error);
      h_theta_star_RP_integrated->SetBinContent(i+1,integral);
      h_theta_star_RP_integrated->SetBinError(i+1,error);
    }
    h_theta_star_RP_integrated->Scale(1./float(h2_theta_star_RP_clone->GetNbinsY()));

    cout << "Comparing Ratios from RP cos(theta*)" << endl;
    for(int i = 0; i < h2_theta_star_RP_clone->GetNbinsX(); i++)
    {
      cout << "Original Method = " << h_theta_star_RP_clone->GetBinContent(i+1) << " +/- " << h_theta_star_RP_clone->GetBinError(i+1)
           << "           Integrate Beta = " << h_theta_star_RP_integrated->GetBinContent(i+1) << " +/- " << h_theta_star_RP_integrated->GetBinError(i+1) << endl;
    }
    
    h2_theta_star_RP_clone->GetXaxis()->SetTitle("|cos(#theta*')|");
    h2_theta_star_RP_clone->GetYaxis()->SetTitle("#beta");
    h2_theta_star_RP_clone->Draw("colz");
    c2d->SaveAs(Form("FValues/yabs1/RatioPlot2D_RP_pt1_%d%s_method%d.pdf",irho,cutoption.c_str(),method));
    delete c2d;

    Func_A->SetParameter(0,h_theta_star_RP_integrated->GetBinContent(1));
    Func_A->SetParameter(1,0);
    Func_A->SetParameter(2,0.0);
    //Func_Orig->SetParameter(0,h_theta_star_clone_RP->GetBinContent(1));
    //Func_Orig->SetParameter(1,0);
    //h_theta_star_clone_RP->Fit(Func_Orig,"ER");
    h_theta_star_RP_integrated->Fit(Func_A,"ER");
    //h_theta_star_RP_clone->Fit(Func_A,"ER");
    ...
    if(cut == 1 && method == 1){
      ...
      c1->SetBottomMargin(0.15);
      c1->SetLeftMargin(0.15);
      h_theta_star_RP_integrated->Draw("pE");
      Func_A->Draw("same"); 
      auto legend = new TLegend(0.1,0.7,0.48,0.9); 
      legend->AddEntry(Func_A,"4th Order Fit","l");
      //legend->AddEntry(Func_Orig,"2nd Order Fit","l");
    ...
    if(cut == 1)
    {
      ...
      g_rho00outBefore->SetPoint(irho,rhoobs[irho],rhoreal[irho]);
      g_rho00outBefore->SetPointError(irho,0.0,0.0,Func_AFG->GetParError(1),Func_AFG->GetParError(1));
      g_rho00outBeforeIn->SetPoint(irho,rho00in[irho],rhoreal[irho]);
      g_rho00outBeforeIn->SetPointError(irho,0.0,0.0,Func_AFG->GetParError(1),Func_AFG->GetParError(1));
    }
...
*/
  ...
  for(int irho = 0; irho < 3; irho++)
  {
    ...
    cout << "F = " << Fval[irho] << " +/- " << Ferr[irho] << "     F* = " << Fpval[irho] << " +/- " << Fperr[irho] << endl;
    cout << "G = " << Gval[irho] << " +/- " << Gerr[irho] << "     G* = " << Gpval[irho] << " +/- " << Gperr[irho] << endl;
  }
  ...
  for(int irho = 0; irho < 3; irho++) 
  {
    ...
    Func_rho->FixParameter(2, Fval[1]);
    Func_rho->FixParameter(3, Gval[1]);
