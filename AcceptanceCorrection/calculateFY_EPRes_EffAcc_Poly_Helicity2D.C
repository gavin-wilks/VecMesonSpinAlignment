...
void calculateFY_EPRes_EffAcc_Poly_Helicity2D(const int energy = 4, const int pid = 0, bool doall = true, bool isBesI = false, bool random3D = false, int etamode = 0, int order = 2, int yspectra = 2, int EP = 0, int v2 = 1) {
              ...
              for(int i_method = 1/*vmsa::Method_start*/; i_method < vmsa::Method_stop; ++i_method)
              {
                for(int i_poly = 0; i_poly < 1/*3*/; i_poly++)
                {
                  ...
                  for(Int_t PtBin=1; PtBin<=10; PtBin++) 
                  {
                    ...
                    cout << "Normalization = " << Func_obs[PtBin-1]->GetParameter(5) << endl;
                    ...
                    //  cout << "Before scaling bin = " << i << ", cos = " << cos << ", integral = " << cosintegral << endl;
                    //  cosintegral /= coswidth;
                    //  cosintegral /= (2.0*TMath::Pi());
                    //  //cosintegral *= double(nFineBinsCos)/9;
                    //  Func_obs_cos[PtBin-1]->SetBinContent(i, cosintegral);
                    //  cout << "bin = " << i << ", cos = " << cos << ", integral = " << cosintegral << endl;
                    //  cout << "bin = " << i << ", cos - 0.5 * coswidth = " << cos - 0.5 * coswidth << endl;
                    //  cout << "bin = " << i << ", cos + 0.5 * coswidth = " << cos + 0.5 * coswidth << endl;
                    //     
                    //}
                    ...
                    //  double phi = Func_obs_phi[PtBin-1]->GetBinCenter(i);
                    //  double phiwidth = Func_obs_phi[PtBin-1]->GetBinWidth(i);
                    //  double phiintegral = Func_obs[PtBin-1]->Integral(-1.0, 1.0, 
                    //                                                   phi - 0.5 * phiwidth, 
                    //                                                   phi + 0.5 * phiwidth);
                    //  cout << "Before scaling bin = " << i << ", phi = " << phi << ", integral = " << phiintegral << endl;
                    //  phiintegral /= phiwidth;
                    //  phiintegral /= 2.0;
                    //  //phiintegral *= double(nFineBinsPhi)/12;
                    //  Func_obs_phi[PtBin-1]->SetBinContent(i, phiintegral);
                    //  cout << "bin = " << i << ", phi = " << phi << ", integral = " << phiintegral << endl;
                    //  cout << "bin = " << i << ", phi - 0.5 * phiwidth = " << phi - 0.5 * phiwidth << endl;
                    //  cout << "bin = " << i << ", phi + 0.5 * phiwidth = " << phi + 0.5 * phiwidth << endl;
                    //}
                    // 1D projection in cos(theta*) for 2D fit
                  ...
                  {   
                    ...
                    float max = -99999999;
                    float min = 99999999;
                    float maxyield = yield_hist_plot_cos[i]->GetMaximum();
                    float minyield = yield_hist_plot_cos[i]->GetMinimum();
                    float maxfit = Func_obs_cos[i]->GetMaximum(); 
                    float minfit = Func_obs_cos[i]->GetMinimum();

                    if(maxyield > maxfit) max = maxyield;
                    if(maxyield < maxfit) max = maxfit;
                    if(minyield < minfit) min = minyield;
                    if(minyield > minfit) min = minfit;

                    yield_hist_plot_cos[i]->GetYaxis()->SetRangeUser(0.9*min,1.1*max);
                       
                    yield_hist_plot_cos[i]->Draw("pE");
                    Func_obs_cos[i]->SetLineColor(kRed);
                  ...
                  {   
                    ...
                    float max = -99999999;
                    float min = 99999999;
                    float maxyield = yield_hist_plot_phi[i]->GetMaximum();
                    float minyield = yield_hist_plot_phi[i]->GetMinimum();
                    float maxfit = Func_obs_phi[i]->GetMaximum(); 
                    float minfit = Func_obs_phi[i]->GetMinimum();
                    ...
                    yield_hist_plot_phi[i]->GetYaxis()->SetRangeUser(0.9*min,1.1*max);
                    yield_hist_plot_phi[i]->Draw("pE");
                    Func_obs_phi[i]->SetLineColor(kRed);
