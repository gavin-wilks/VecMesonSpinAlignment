...
void acceptanceQA_4th_Res_v2test_betafit(const int energy = 4, const int pid = 0, bool doall = true, std::string res = "0p4", double resval = 0.4, int cut = 1, int method = 1, int etamode = 3) {
...
//  Double_t pt_set[8] = {0.6, 1.4, 2.2, 3.0, 3.8, 4.6, 5.4, 7.2};
  ...
  TGraphAsymmErrors *g_rho00RecoObs  = new TGraphAsymmErrors();
  double Fval[5] = {0.0}; //0.08555382,0.0660837,0.0363711,-0.00175735,0.0126171,-0.0257385};
  double Ferr[5] = {0.0};
  double FvalB[5] = {0.0}; //0.08555382,0.0660837,0.0363711,-0.00175735,0.0126171,-0.0257385};
  double FerrB[5] = {0.0};
  double Gval[5] = {0.0}; //0.08555382,0.0660837,0.0363711,-0.00175735,0.0126171,-0.0257385};
  double Gerr[5] = {0.0};
  double GvalB[5] = {0.0}; //0.08555382,0.0660837,0.0363711,-0.00175735,0.0126171,-0.0257385};
  double GerrB[5] = {0.0};
  ...
  for(int irho = 0; irho < 3; irho++)
  {
    ...
    MCFiles[irho] = new TFile(Form("/gpfs01/star/pwg/gwilks3/VectorMesonSpinAlignment/Data/%s/Acceptance/ResolutionTesting/ResolutionTest_0p4_EtaCut%d_0p2y0p4_v21_scale1p5/McAcceptanceOutput_pt1_energy%d_pid%d_cent4_EtaMode_0_nrho%d.root",vmsa::mPID[pid].c_str(),etamode,energy,pid,rho00val[irho]),"READ");
    ...
    rhoobsErr[irho] = Func_rho->GetParError(1);
...
*/
  ...
  for(int irho = 0; irho < 3; irho++) 
  {
    ...
    g_rho00RecoObs->SetPoint(irho,rho00in[irho],rhoobs[irho]);
    g_rho00RecoObs->SetPointError(irho,0.0,0.0,rhoobsErr[irho],rhoobsErr[irho]);
  ...
  g_rho00RecoObs->SetMarkerStyle(24);
  g_rho00RecoObs->SetMarkerColor(kRed);
  g_rho00RecoObs->SetMarkerSize(3.0);
  g_rho00RecoObs->SetLineWidth(3);
  g_rho00RecoObs->Draw("pE Same");
  ...
  leg2->SetFillColor(10);
  leg2->SetBorderSize(0);
  leg2->AddEntry(g_rho00RecoObs,"Before Correction","p");
  leg2->AddEntry(g_rho00outAfterIn,"After Correction","p");
  leg2->AddEntry(linear,"reco = input","l");
  leg2->Draw("same");

  FigureName = Form("./figures/AcceptanceQA/AcceptanceQAInput_RecoObs_%s%s_method%d.pdf",vmsa::mBeamEnergy[energy].c_str(),cutoption.c_str(),method);
  c_play2->SaveAs(FigureName.c_str());
