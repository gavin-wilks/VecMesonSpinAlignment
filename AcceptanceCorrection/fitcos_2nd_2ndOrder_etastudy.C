...
void fitcos_2nd_2ndOrder_etastudy(const int energy = 4, const int pid = 0, bool doall = true, bool isBesI = false, bool random3D = false, int etamode = 0) {
...
//  Double_t pt_set[8] = {0.6, 1.4, 2.2, 3.0, 3.8, 4.6, 5.4, 7.2};
  ...
  for(int ipt = 2; ipt < 6; ipt++)
  {
    ...
    MCFiles[ipt] = new TFile(Form("/gpfs01/star/pwg/gwilks3/VectorMesonSpinAlignment/Data/%s/Acceptance/PtAcceptance_V2On/McAcceptanceOutput_pt%d_energy%d_pid%d_cent9_EtaMode_0.root",vmsa::mPID[pid].c_str(),ipt+1,energy,pid),"READ");
    ...
    h_theta_star_before[ipt] = (TH1D*) MCFiles[ipt]->Get(Form("h_theta_star_RP_%d",0));
    //h_theta[ipt] = (TH1D*) MCFiles[ipt]->Get(Form("h_theta_%d",etaplot));
    h_theta_star[ipt] = (TH1D*) MCFiles[ipt]->Get(Form("h_theta_star_RP_%d",etaplot));
    ...
    Func_A[ipt] = new TF1("Func_A","[0]*(1.+[1]*(x*x))",0,1);
  ...
  TFile *eff_file_plateau = new TFile(Form("/gpfs01/star/pwg/gwilks3/VectorMesonSpinAlignment/AuAu19GeV_2019/OutPut/CosEfficiencyProccessTuple_EPResPt_JustEfficiency/Eff_%s_SingleParticle_noToF_Mode0_EtaMode%d.root",vmsa::mBeamEnergy[energy].c_str(),etamode),"READ");
