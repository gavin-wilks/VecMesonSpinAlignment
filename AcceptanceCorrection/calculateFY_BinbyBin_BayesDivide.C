...
void calculateFY_BinbyBin_BayesDivide(const int energy = 4, const int pid = 0, bool doall = true, bool isBesI = false, bool random3D = false, int mode = 1, int etamode = 0, int ypadding = 2, int ipt = 0, int icent = 1, std::string etastring = "eta1_eta1") {
...
//  Double_t pt_set[8] = {0.6, 1.4, 2.2, 3.0, 3.8, 4.6, 5.4, 7.2};
  ...
  TGraphAsymmErrors *h_theta_star_ratio[10][10][15];
  TGraphAsymmErrors *h_theta_ratio[10][10][15];
  ...
  MCFile = new TFile(Form("./Rebinned/BayesDivide/Acceptance_%s_19GeV_Mode%d_EtaMode%d.root",vmsa::mPID[pid].c_str(),mode,etamode),"READ");
 ...
 // c_play->Print(output_start.c_str());
  ...
  std::string outputname = Form("figures/BayesDivide/Fplots_%s_%s_Mode%d_EtaMode%d_rapidity_thetastar_cent%d_pt%d.pdf",vmsa::mPID[pid].c_str(),vmsa::mBeamEnergy[energy].c_str(),mode,etamode,icent,ipt);
  std::string output_start = Form("%s[",outputname.c_str());
  c_play->Print(output_start.c_str());
  cout << "c_play->Print()" << endl;
  
  TF1* Func_A[vmsa::y_total];  
  ...
  for(int iy = 0; iy < vmsa::y_total; iy++)
  {
    ...
    //h_theta_star_before[ipt] = (TH1D*) MCFiles->Get("h_theta_star_before");
    h_theta_ratio[ipt][icent][iy] = (TGraphAsymmErrors*) MCFile->Get(Form("g_mEffCosS_Cent_%d_Pt_%d_Y_%d",icent,ipt,iy))->Clone();
    //h_theta_star_ratio[ipt][icent]->GetYaxis()->SetTitle(Form("Cent %d  %1.1f<pT<%1.1f",icent,vmsa::pt_low_cent[energy][ipt],vmsa::pt_up_cent[energy][ipt]));
    cout << Form("Cent %d  %1.1f<pT<%1.1f",icent,vmsa::pt_low_y[energy][ipt],vmsa::pt_up_y[energy][ipt]) << endl;
    h_theta_ratio[ipt][icent][iy]->GetHistogram()->GetXaxis()->SetTitle(Form("Cent %d  %1.1f <y<%1.1f  %1.1f<pT<%1.1f    cos#theta*",icent,vmsa::y_bin[iy],vmsa::y_bin[iy+1],vmsa::pt_low_y[energy][ipt],vmsa::pt_up_y[energy][ipt]));
    //h_theta[ipt] = (TH1D*) MCFiles->Get("h_theta");
    //h_out1[ipt] = (TH1D*) MCFiles->Get("h_out1");
    //h_out2[ipt] = (TH1D*) MCFiles->Get("h_out2");
    ...
    //TF1 *Func_rho = new TF1("Func_rho","[0]*(1.-[1]+(3.*[1]-1)*(x*x))",0,1);
    //TF1 *Func_A = new TF1("Func_A","[0]*(1.+[1]*(x*x))",0,1);
    Func_A[iy] = new TF1(Form("Func_A_%d",iy),Func4th,0,1,3);

    for(int itheta = 0; itheta < 7; itheta++)
    {
      double x, y;
      double xerr, yerrh, yerrl;
      yerrh = h_theta_ratio[ipt][icent][iy]->GetErrorYhigh(itheta);
      yerrl = h_theta_ratio[ipt][icent][iy]->GetErrorYlow(itheta);
      h_theta_ratio[ipt][icent][iy]->SetPointError(itheta,0.0,0.0,yerrl,yerrh);
    }
    ...
    //h_theta_star_before[ipt]->Sumw2();
    //h_theta_star_clone->Divide(h_theta_star_before[ipt]); 
    Func_A[iy]->SetParameter(0,h_theta_ratio[ipt][icent][iy]->GetMaximum());
    Func_A[iy]->SetParameter(1,0);
    h_theta_ratio[ipt][icent][iy]->Fit(Func_A[iy],"ER");

    Fval[ipt][icent][iy] = Func_A[iy]->GetParameter(1);
    Ferr[ipt][icent][iy] = Func_A[iy]->GetParError(1);
    Gval[ipt][icent][iy] = Func_A[iy]->GetParameter(2);
    Gerr[ipt][icent][iy] = Func_A[iy]->GetParError(2);
    ...
    h_theta_ratio[ipt][icent][iy]->Draw("pEA");
    Func_A[iy]->SetLineColor(kRed);
    Func_A[iy]->SetLineWidth(1);
    Func_A[iy]->Draw("same");
    //c_play->SaveAs(Form("FValues/pt%d.pdf",ipt));
  ...
  string outputname = Form("figures/BayesDivide/Fplots_%s_%s_Mode%d_EtaMode%d_rapidity_Fvalues_pt%d_cent%d.pdf",vmsa::mPID[pid].c_str(),vmsa::mBeamEnergy[energy].c_str(),mode,etamode,ipt,icent);
  string output_start = Form("%s[",outputname.c_str());
  ...
  if(doall)
  {
    ...
    string outputname = Form("output/%s/%s/AccResRhoEtaSys_%s_cent_%d_ipt_%d.root",vmsa::mPID[pid].c_str(),vmsa::mBeamEnergy[energy].c_str(),etastring.c_str(),icent,ipt);
            ...
            for(int i_method = vmsa::Method_start; i_method < vmsa::Method_stop; ++i_method)
            {
              ...
              for(int iy = 0+ypadding; iy < vmsa::y_total-ypadding; iy++) 
              {
                ...
                for(int itheta = 0; itheta < 7; itheta++) 
                {
                  ...
                  PtCos->SetBinContent(itheta+1, inte_mean/eff[iy][itheta]);
                  PtCos->SetBinError(itheta+1, inte_mean/eff[iy][itheta]*TMath::Sqrt(inte_mean_error*inte_mean_error/inte_mean/inte_mean+eff_error[iy][itheta]*eff_error[iy][itheta]));
                }
                PtCos->Write();
                ...
                Func_rho->FixParameter(2, Fval[icent][ipt][iy]);
                Func_rho->FixParameter(3, Gval[icent][ipt][iy]);
