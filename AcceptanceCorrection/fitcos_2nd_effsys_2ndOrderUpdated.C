...
void fitcos_2nd_effsys_2ndOrderUpdated(const int energy = 4, const int pid = 0, bool doall = true, bool isBesI = false, bool random3D = false) {
...
//  Double_t pt_set[8] = {0.6, 1.4, 2.2, 3.0, 3.8, 4.6, 5.4, 7.2};
  ...
  string outputname = Form("output/%s/%s/AccRes%sPtSys_2ndUpdated.root",vmsa::mPID[pid].c_str(),vmsa::mBeamEnergy[energy].c_str(),vmsa::mPID[pid].c_str());
