...
void calculateFCent_EPRes_EffAcc_14GeV(const int energy = 4, const int pid = 0, int ptbin = 0, bool doall = true, bool isBesI = false, bool random3D = false, int mode = 0, int etamode = 0, std::string ep = "sub", int order = 1) {
...
/*
  ...
  */  
  ...
  if(doall)
  {
    ...
    TFile *eff_file;
    if(order == 1) eff_file = new TFile(Form("/gpfs01/star/pwg/gwilks3/VectorMesonSpinAlignment/AuAu14GeV_2019/OutPut/CosEfficiencyProccessTuple_EPRes_EffAcc_Ycut_Order1/Eff_14GeV_SingleParticle_noToF_Mode1_EtaMode%d.root",etamode),"READ");
    if(order == 2) eff_file = new TFile(Form("/gpfs01/star/pwg/gwilks3/VectorMesonSpinAlignment/AuAu14GeV_2019/OutPut/CosEfficiencyProccessTuple_EPRes_EffAcc_Ycut_Order2/Eff_14GeV_SingleParticle_noToF_Mode1_EtaMode%d.root",etamode),"READ");
    eff_file->Print();
    ...
    if(order == 2) input = new TFile(Form("rho00/%s/%s/RawRhoCentSys_%s.root",vmsa::mPID[pid].c_str(),vmsa::mBeamEnergy[energy].c_str(),etastring.c_str()),"READ");
