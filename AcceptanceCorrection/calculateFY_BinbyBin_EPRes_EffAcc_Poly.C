...
void calculateFY_BinbyBin_EPRes_EffAcc_Poly(const int energy = 4, const int pid = 0, bool doall = false, bool isBesI = false, bool random3D = false, int mode = 1, int etamode = 0, int ypadding = 2, int ipt = 1, int icent = 1, int i_poly = 0, std::string ep = "sub", int order = 1, int yspectra = 0, int EP = 1, int v2 = 0) {
  ...
  if(doall)
  {
    ...
    if(energy == 0)
    {
      if(order == 1) eff_file = new TFile(Form("../RcPhiEffCorr/order1/Eff_%s_SingleParticle_noToF_Mode2_EtaMode%d.root",vmsa::mBeamEnergy[energy].c_str(),etamode),"READ");
      if(order == 2) eff_file = new TFile(Form("../RcPhiEffCorr/order2/Eff_%s_SingleParticle_noToF_Mode2_EtaMode%d.root",vmsa::mBeamEnergy[energy].c_str(),etamode),"READ");
    }
    ...
    string outputname;
    if(order == 1) outputname = Form("output/%s/%s/AccResRhoEtaSys_%s_Poly_FirstOrder_cent_%d_ipt_%d_poly%d%s%s%s.root",vmsa::mPID[pid].c_str(),vmsa::mBeamEnergy[energy].c_str(),etastring.c_str(),icent,ipt,i_poly+1,spectra.c_str(),eptext.c_str(),v2text.c_str());
    if(order == 2) outputname = Form("output/%s/%s/AccResRhoEtaSys_%s_Poly_cent_%d_ipt_%d_poly%d%s%s%s.root",vmsa::mPID[pid].c_str(),vmsa::mBeamEnergy[energy].c_str(),etastring.c_str(),icent,ipt,i_poly+1,spectra.c_str(),eptext.c_str(),v2text.c_str());
