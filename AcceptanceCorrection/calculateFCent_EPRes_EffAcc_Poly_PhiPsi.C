...
void calculateFCent_EPRes_EffAcc_Poly_PhiPsi(const int energy = 4, const int pid = 0, int ptbin = 0, bool doall = false, bool isBesI = false, bool random3D = false, int mode = 0, int etamode = 0, std::string ep = "sub", int order = 1, int yspectra = 0, int EP = 1, int v2 = 0) {
...
/*
  ...
  */  
  ...
  if(doall)
  {
    double eff[10][10][7][7];
    double eff_error[10][10][7][7];
    ...
    for(int icent = 0; icent < 9; icent++)
    {
      ...
      {  
        for(int itheta = 0; itheta < 7; itheta++)
        {
          ...
          {
            TString *Title = new TString(Form("h_mEffCosEP_Cent_%d_Pt_%d",icent,ipt));
            TH2D *eff_hist = (TH2D*)eff_file->Get(Title->Data());
            eff[icent][ipt][itheta][iphipsi] = eff_hist->GetBinContent(itheta+1,iphipsi+1);
            eff_error[icent][ipt][itheta][iphipsi] = eff_hist->GetBinError(itheta+1,iphipsi+1)/eff[icent][ipt][itheta][iphipsi];
            cout << "icent = " << icent << " ipt = " << ipt << " itheta = " << itheta << " iphipsi = " << iphipsi << " eff = " << eff[icent][ipt][itheta][iphipsi] << " +/- " << eff_error[icent][ipt][itheta][iphipsi] << endl;
            delete Title;
            delete eff_hist;
          }
        }      
    ...
    TFile *input;
    if(order == 1) input = new TFile(Form("rho00/%s/%s/Poly/PhiPsi_RawRhoCentSys_%s_Poly_FirstOrder.root",vmsa::mPID[pid].c_str(),vmsa::mBeamEnergy[energy].c_str(),etastring.c_str()),"READ");
    if(order == 2) input = new TFile(Form("rho00/%s/%s/Poly/PhiPsi_RawRhoCentSys_%s_Poly.root",vmsa::mPID[pid].c_str(),vmsa::mBeamEnergy[energy].c_str(),etastring.c_str()),"READ");
    ...
    string outputname;
    if(order == 1) outputname = Form("output/%s/%s/PhiPsi_AccResRhoCentSys_%s_Poly_FirstOrder_pt_%d%s%s%s.root",vmsa::mPID[pid].c_str(),vmsa::mBeamEnergy[energy].c_str(),etastring.c_str(),ptbin,spectra.c_str(),eptext.c_str(),v2text.c_str());
    if(order == 2) outputname = Form("output/%s/%s/PhiPsi_AccResRhoCentSys_%s_Poly_pt_%d%s%s%s.root",vmsa::mPID[pid].c_str(),vmsa::mBeamEnergy[energy].c_str(),etastring.c_str(),ptbin,spectra.c_str(),eptext.c_str(),v2text.c_str());
                ...
                for(int icent = 0; icent < 9; ++icent) // Centrality loop
                {
                  ...
                  for(int itheta = 0; itheta < 7; itheta++) 
                  {
                    for(int iphipsi = 0; iphipsi < 7; iphipsi++) 
                    {
                      ...
                      PtCos_2D->SetBinContent(itheta+1, iphipsi+1, inte_mean/eff[icent][ptbin][itheta][iphipsi]);
                      PtCos_2D->SetBinError(itheta+1, iphipsi+1, inte_mean/eff[icent][ptbin][itheta][iphipsi]*TMath::Sqrt(inte_mean_error*inte_mean_error/inte_mean/inte_mean+eff_error[icent][ptbin][itheta][iphipsi]*eff_error[icent][ptbin][itheta][iphipsi]));
                    }
                  }
