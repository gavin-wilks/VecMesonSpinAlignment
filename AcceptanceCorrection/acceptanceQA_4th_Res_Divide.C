...
void acceptanceQA_4th_Res_Divide(const int energy = 4, const int pid = 0, bool doall = true, std::string res = "0p1", double resval = 0.1, int cut = 1, int method = 1) {
...
//  Double_t pt_set[8] = {0.6, 1.4, 2.2, 3.0, 3.8, 4.6, 5.4, 7.2};
  ...
  TH1D *h_theta_star_corrected[5];
  TH1D *h_theta_star_RP_corrected[5];
  TH1D *h_out1[5];
  TH1D *h_out2[5];
  ...
  for(int irho = 0; irho < 5; irho++)
  {
    ...
    MCFiles[irho] = new TFile(Form("/gpfs01/star/pwg/gwilks3/VectorMesonSpinAlignment/Data/%s/Acceptance/ResolutionTesting/yabs1_%s%s_noDelta/McAcceptanceOutput_pt1_energy%d_pid%d_cent5_EtaMode_0_nrho%d.root",vmsa::mPID[pid].c_str(),res.c_str(),cutoption.c_str(),energy,pid,rho00val[irho]),"READ");
    ...
    if(cut == 1 && method == 0){
      h_theta_star_corrected[irho] = (TH1D*) h_theta_star[irho]->Clone();
      //h_theta_star[irho]->Sumw2(); 
      h_theta_star_corrected[irho]->Sumw2(); 
      h_theta_star_corrected[irho]->Divide(h_theta_star_clone); 
      //h_theta_star_corrected[irho]->Divide(h_theta_star[irho],h_theta_star_clone,1,1,"B");
    }
    ...
    if(cut == 1 && method == 1){
      ...
      //h_theta_star[irho]->Sumw2(); 
      h_theta_star_corrected[irho]->Sumw2(); 
      h_theta_star_corrected[irho]->Divide(h_theta_star_clone_RP); 
      //h_theta_star_corrected[irho]->Divide(h_theta_star[irho],h_theta_star_clone_RP,1,1,"B");
    }
    h_theta_star_RP_corrected[irho] = (TH1D*) h_theta_star_RP[irho]->Clone();
    //h_theta_star_RP[irho]->Sumw2(); 
    h_theta_star_RP_corrected[irho]->Sumw2(); 
    h_theta_star_RP_corrected[irho]->Divide(h_theta_star_clone_RP); 
    //h_theta_star_RP_corrected[irho]->Divide(h_theta_star_RP[irho],h_theta_star_clone_RP,1,1,"B");

    TF1 *Func_AFG = new TF1("Func_AFG",FuncAFG,0,1,5);
    Func_AFG->SetParameter(0,h_theta_star_RP_corrected[irho]->GetBinContent(1));
    Func_AFG->SetParameter(1,1./3.);
    Func_AFG->FixParameter(2, 0.0);//Func_A->GetParameter(1));//Fval[irho]);
    Func_AFG->FixParameter(3, 0.0);//Func_A->GetParameter(2));//Gval[irho]);
    Func_AFG->FixParameter(4, 1.0);//Res_12);
    h_theta_star_RP_corrected[irho]->Fit(Func_AFG,"NERQ");
...
*/
  ...
  for(int irho = 0; irho < 5; irho++) 
  {
    ...
    h_theta_star_corrected[irho]->GetXaxis()->SetTitleOffset(1.2);
    h_theta_star_corrected[irho]->GetXaxis()->SetTitle("cos#theta*");
    h_theta_star_corrected[irho]->GetYaxis()->SetTitle("yield");
    h_theta_star_corrected[irho]->GetYaxis()->SetTitleOffset(1.0);
    h_theta_star_corrected[irho]->SetMarkerColor(2);
    h_theta_star_corrected[irho]->SetMarkerSize(1.8);
    h_theta_star_corrected[irho]->SetMarkerStyle(21);
    //h_theta_star[irho]->Draw();
    Func_rho->SetParameter(0, h_theta_star_corrected[irho]->GetBinContent(5));
    ...
    Func_rho->FixParameter(2, 0.0);
    Func_rho->FixParameter(3, 0.0);
    Func_rho->FixParameter(4, resval);//Res_12);

    //Func_rho->FixParameter(3, 1.0);//Res_12);

    Func_rhoOrig->SetParameter(0, h_theta_star_corrected[irho]->GetBinContent(5));
    ...
    //Func_rho->Draw("same");
    //delete Func_rdl;
    TString *Title = new TString(Form("fit/acceptanceQA/yield_pt_1_cent_5_rho%d%s_method%d_Divide.pdf",irho,cutoption.c_str(),method));
    ...
    c1->SetBottomMargin(0.15);
    c1->SetLeftMargin(0.15);
    h_theta_star_corrected[irho]->Draw("pE");
    //h_theta_star_corrected[irho]->Fit(Func_rhoOrig, "NMI");
   // Func_rhoOrig->Draw("same");
    h_theta_star_corrected[irho]->Fit(Func_rho, "NMI");
    Func_rho->Draw("same");
  ...
  string FigureName = Form("./figures/AcceptanceQA/AcceptanceQA_%s%s_method%d_Divide.pdf",vmsa::mBeamEnergy[energy].c_str(),cutoption.c_str(),method);
  ...
  FigureName = Form("./figures/AcceptanceQA/AcceptanceQAInput_%s%s_method%d_Divide.pdf",vmsa::mBeamEnergy[energy].c_str(),cutoption.c_str(),method);
  ...
  FigureName = Form("./figures/AcceptanceQA/AcceptanceQADifference_%s%s_method%d_Divide.pdf",vmsa::mBeamEnergy[energy].c_str(),cutoption.c_str(),method);
