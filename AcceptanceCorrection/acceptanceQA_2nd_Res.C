...
void acceptanceQA_2nd_Res(const int energy = 4, const int pid = 0, bool doall = true, std::string res = "0p1", double resval = 0.1, int cut = 1, int method = 1) {
...
//  Double_t pt_set[8] = {0.6, 1.4, 2.2, 3.0, 3.8, 4.6, 5.4, 7.2};
  ...
  for(int irho = 0; irho < 5; irho++)
  {
    ...
    TF1 *Func_Theta = new TF1("Func_Theta","[0]*(1 + [1]*x*x)",0,1);
    ...
    legend0->AddEntry(Func_Theta,"2nd Order Fit","l");
    legend0->Draw("same");
   ...
   // Func_A->SetParameter(2,0.0);
    ...
    if(cut == 1 && method == 0){
      ...
      legend->AddEntry(Func_A,"2nd Order Fit","l");
      legend->Draw("same");
      c1->SaveAs(Form("FValues/y1/pt1_%d%s_method%d.pdf",irho,cutoption.c_str(),method));
      delete c1;
    }
    ...
    TF1 *Func_AFG = new TF1("Func_AFG",FuncAF_Updated,0,1,4);
    ...
    Func_AFG->FixParameter(3, 1.0);//Res_12);
  ...
  for(int irho = 0; irho < 5; irho++) 
  {
    ...
    Func_rho->FixParameter(3, resval);//Res_12);
    ...
    auto legend = new TLegend(0.1,0.7,0.48,0.9); 
    legend->AddEntry(Func_rho,"2nd Order Fit","l");
    legend->Draw("same");
...
double FuncAF_Updated(double *x_val, double *par) {
  ...
  double result = (1. + Bs*F/2.) + (As + F - Bs*F)*CosTheta*CosTheta + (As*F + Bs*F/2.)*CosTheta*CosTheta*CosTheta*CosTheta;
...
double Func4th(double *x_val, double *par) {
  ...
  double result = 2. + 2.*F + 3.*G/4. - F*CosTheta*CosTheta - 3.*G/2.*CosTheta*CosTheta + 3.*G/4.*CosTheta*CosTheta*CosTheta*CosTheta;
