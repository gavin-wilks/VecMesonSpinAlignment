...
void McEtaF_Resolution_Beta_VaryV2(double Nrho=3333, int SetPt=4, int energy = 4, int pid = 0, int cent = 5, int const NMax = 5000, double rapidity = 1.0, int mode = 3, int etamode = 0, double Res = 0.4, bool fullEP = 0, bool v2on = 0, float v2input = 0.0, const char* jobID = "1")
{
  v2flag = v2on;
  mNEvents = NMax;
  mEtaMode = etamode;
  ...
  cout << "Resolution = " << Res << ",   <cos2(psi_e-psi_w)> = " << Res*Res << endl;
  f_res = new TF1("resolution",EventPlaneResolution,0,80,0);
  mChi = f_res->GetX(Res*Res);
