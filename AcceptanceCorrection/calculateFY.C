...
void calculateFY(const int energy = 4, const int pid = 0, bool doall = true, bool isBesI = false, bool random3D = false, int mode = 1, int etamode = 0, int ypadding = 2) {
...
//  Double_t pt_set[8] = {0.6, 1.4, 2.2, 3.0, 3.8, 4.6, 5.4, 7.2};
 ...
 // c_play->Print(output_start.c_str());
  ...
  std::string outputname = Form("figures/Fplots_%s_%s_Mode%d_EtaMode%d_rapidity_theta.pdf",vmsa::mPID[pid].c_str(),vmsa::mBeamEnergy[energy].c_str(),mode,etamode);
  ...
  TCanvas *c_play2 = new TCanvas("c_play2","c_play2",10,10,900,1200);
  c_play2->Divide(3,4);
  for(int i = 0; i < 12; i++)
  {
  ...
  string outputname = Form("figures/Fplots_%s_%s_Mode%d_EtaMode%d_rapidity_Fvalues.pdf",vmsa::mPID[pid].c_str(),vmsa::mBeamEnergy[energy].c_str(),mode,etamode);
  string output_start = Form("%s[",outputname.c_str());
  ...
  for(int iF = 0; iF < 2; iF++)
  {
    for(int ipt = vmsa::pt_rebin_first_y[energy]; ipt <= vmsa::pt_rebin_last_y[energy]; ipt++)
    {
      ...
      for(int icent = 0; icent < vmsa::cent_rebin_total; icent++)
      {
        c_play2->cd(icent*vmsa::pt_rebin_y+ipt+1);
  ...
  {
    for(int icent = vmsa::cent_rebin[icent_rebin]; icent < vmsa::cent_rebin[icent_rebin+1]; icent++) {
      Res_12[icent_rebin] += resolution_12->GetBinContent(icent+1)/resolution_12->GetBinError(icent+1)/resolution_12->GetBinError(icent+1);
      Res_12_weight[icent_rebin] += 1./resolution_12->GetBinError(icent+1)/resolution_12->GetBinError(icent+1);
    }
  ...
  if(doall)
  {
    double eff[vmsa::cent_rebin_total][vmsa::pt_rebin_y][vmsa::y_total][7];
    double eff_error[vmsa::cent_rebin_total][vmsa::pt_rebin_y][vmsa::y_total][7];
    ...
    for(int icent = 0; icent < vmsa::cent_rebin_total; icent++)
    {
      for(int ipt = 0; ipt < vmsa::pt_rebin_y-1; ipt++)
      { 
        for(int iy = 0+ypadding; iy < vmsa::y_total-ypadding; iy++)
        { 
          for(int itheta = 0; itheta < 7; itheta++)
          {
            ...
            eff[icent][ipt][iy][itheta] = eff_hist->GetBinContent(itheta+1);
            eff_error[icent][ipt][iy][itheta] = eff_hist->GetBinError(itheta+1)/eff[icent][ipt][iy][itheta];
            cout << "icent = " << icent << " ipt = " << ipt << " iy = " << iy << " itheta = " << itheta << " eff = " << eff[icent][ipt][iy][itheta] << " +/- " << eff_error[icent][ipt][iy][itheta] << endl;
            delete Title;
            delete eff_hist;
          }
        }
    ...
    TFile *input = new TFile(Form("rho00/%s/%s/RawRhoEtaSys_eta1_eta1.root",vmsa::mPID[pid].c_str(),vmsa::mBeamEnergy[energy].c_str()),"READ");
    ...
    string outputname = Form("output/%s/%s/AccResRhoEtaSys_eta1_eta1.root",vmsa::mPID[pid].c_str(),vmsa::mBeamEnergy[energy].c_str());
    TFile *output = new TFile(outputname.c_str(),"RECREATE");
    output->cd();

    for(int icent = 0; icent < vmsa::cent_rebin_total; ++icent) // Centrality loop
    {
      for(int ipt = 0; ipt < vmsa::pt_rebin_y-1; ipt++)
      {
                ...
                for(int i_method = vmsa::Method_start; i_method < vmsa::Method_stop; ++i_method)
                {
                  ...
                  for(int iy = 0+ypadding; iy < vmsa::y_total-ypadding; iy++) 
                  {
                    ...
                    for(int itheta = 0; itheta < 7; itheta++) 
                    {
                      ...
                      PtCos->SetBinContent(itheta+1, inte_mean/eff[icent][ipt][iy][itheta]);
                      PtCos->SetBinError(itheta+1, inte_mean/eff[icent][ipt][iy][itheta]*TMath::Sqrt(inte_mean_error*inte_mean_error/inte_mean/inte_mean+eff_error[icent][ipt][iy][itheta]*eff_error[icent][ipt][iy][itheta]));
                    }
                    PtCos->Write();
