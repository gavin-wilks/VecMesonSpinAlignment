...
void plotQA_EPDependentEfficiency_pT(const int energy = 4, const int pid = 0, int etamode = 0, int ypadding = 2, int order = 2, int yspectra = 0) {
  ...
  TFile *eff_file;
  if(order == 1) eff_file = new TFile(Form("/gpfs01/star/pwg/gwilks3/VectorMesonSpinAlignment/AuAu19GeV_2019/OutPut/CosEfficiencyProccessTuple_EPRes_EffAcc_Ycut_Order1%s_EP/Eff_19GeV_SingleParticle_noToF_Mode0_EtaMode%d.root",spectra.c_str(),etamode),"READ");
  if(order == 2) eff_file = new TFile(Form("/gpfs01/star/pwg/gwilks3/VectorMesonSpinAlignment/AuAu19GeV_2019/OutPut/CosEfficiencyProccessTuple_EPRes_EffAcc_Ycut_Order2%s_EP/Eff_19GeV_SingleParticle_noToF_Mode0_EtaMode%d.root",spectra.c_str(),etamode),"READ");
  if(energy == 3 && order == 1) eff_file = new TFile(Form("/gpfs01/star/pwg/gwilks3/VectorMesonSpinAlignment/AuAu14GeV_2019/OutPut/CosEfficiencyProccessTuple_EPRes_EffAcc_Ycut_Order1%s_EP/Eff_14GeV_SingleParticle_noToF_Mode0_EtaMode%d.root",spectra.c_str(),etamode),"READ");
  if(energy == 3 && order == 2) eff_file = new TFile(Form("/gpfs01/star/pwg/gwilks3/VectorMesonSpinAlignment/AuAu14GeV_2019/OutPut/CosEfficiencyProccessTuple_EPRes_EffAcc_Ycut_Order2%s_EP/Eff_14GeV_SingleParticle_noToF_Mode0_EtaMode%d.root",spectra.c_str(),etamode),"READ");
  eff_file->Print();
  ...
  std::string outputname = Form("figures/%s/%s/pTstudy_EPefficiency_%s.pdf",vmsa::mPID[pid].c_str(),vmsa::mBeamEnergy[energy].c_str(),vmsa::mBeamEnergy[energy].c_str(),order);
  ...
  for(int ipt = 2; ipt < 6; ipt++)
  {
    cout << "In this loop 1" << endl;
    c_fit->cd(ipt-1);
    //eff[ipt][9]->CenterTitle();
    cout << "In this loop 2" << endl;
    eff[ipt][9]->SetTitle(Form("%1.1f<p_{T}<%1.1f, Cent 20-60",vmsa::pt_low[energy][ipt],vmsa::pt_up[energy][ipt]));
    cout << "In this loop 3" << endl;
    cout << Form("%1.1f<p_{T}<%1.1f, Cent 20-60",vmsa::pt_low[energy][ipt],vmsa::pt_up[energy][ipt]) << endl;
    cout << "In this loop 4" << endl;
    eff[ipt][9]->GetXaxis()->SetTitle("|cos#theta*|");
    eff[ipt][9]->GetXaxis()->CenterTitle();
    eff[ipt][9]->GetYaxis()->SetTitle("#phi-#Psi_{2}");
    eff[ipt][9]->GetYaxis()->CenterTitle();
    eff[ipt][9]->Draw("colz");
  }
  c_fit->Update();
