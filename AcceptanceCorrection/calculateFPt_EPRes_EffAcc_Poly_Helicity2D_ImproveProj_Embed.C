...
void calculateFPt_EPRes_EffAcc_Poly_Helicity2D_ImproveProj_Embed(const int energy = 4, const int pid = 0, bool doall = true, bool isBesI = false, bool random3D = false, int etamode = 0, int order = 2, int yspectra = 2, int EP = 0, int v2 = 1) {
  ...
  if(doall)
  {
    ...
    for(int i=9; i<10;i++)
    {
      for(int j=3; j<=6; j++)
      {
        string histnamemc = Form("h_mRc0EffCosHPhiPrime_Cent_9_Pt_%d",j-1);
        string histnamerc = Form("h_mRc8EffCosHPhiPrime_Cent_9_Pt_%d",j-1);
            ...
            for(int i_sigma = vmsa::Sig_start; i_sigma < 1/*vmsa::Sig_stop*/; ++i_sigma)
            {
              for(int i_method = 1/*vmsa::Method_start*/; i_method < vmsa::Method_stop; ++i_method)
              {
                for(int i_poly = 0; i_poly < 1/*3*/; i_poly++)
                {
                  ...
                  //////////////////////   2D Efficiency ///////////////////////////
                  c->SaveAs(Form("figures/%s/%s/PtEmbed2DHelicityEfficiency.pdf",vmsa::mPID[pid].c_str(),vmsa::mBeamEnergy[energy].c_str()));
                  for(int i = 0; i < 4; i++)
                  {   
                    c->cd(i+1);
                  ...
                  c->SaveAs(Form("figures/%s/%s/PtEmbed2DHelicityEfficiencyCorrectedYields.pdf",vmsa::mPID[pid].c_str(),vmsa::mBeamEnergy[energy].c_str()));
                  //////////////////////   2D Efficiency ///////////////////////////
                  //////////////////////   2D Corrected Yields ///////////////////////////
                  for(int i = 0; i < 4; i++)
                  {   
                  ...
                  c->SaveAs(Form("figures/%s/%s/PtEmbed2DHelicity2DFunc.pdf",vmsa::mPID[pid].c_str(),vmsa::mBeamEnergy[energy].c_str()));
                  //////////////////////   2D Corrected Yields ///////////////////////////
                  //////////////////////   2D Raw Yields ///////////////////////////
                  for(int i = 0; i < 4; i++)
                  {   
                  ...
                  c->SaveAs(Form("figures/%s/%s/PtEmbed2DHelicityRawYields.pdf",vmsa::mPID[pid].c_str(),vmsa::mBeamEnergy[energy].c_str()));
                  //////////////////////   2D Raw Yields ///////////////////////////
                  //////////////////////   1D Cos Efficiency ///////////////////////////
                  for(int i = 0; i < 4; i++)
                  {   
                  ...
                  c->SaveAs(Form("figures/%s/%s/PtEmbed1DCosHelicityEfficiency.pdf",vmsa::mPID[pid].c_str(),vmsa::mBeamEnergy[energy].c_str()));
                  //////////////////////   1D Cos Efficiency ///////////////////////////
                  //////////////////////   1D Cos Corrected Yields Small Bins ///////////////////////////
  
                  string outputpdf = Form("figures/%s/%s/PtEmbed1DCosHelicityEfficiencyCorrectedYields_SmallBins.pdf",vmsa::mPID[pid].c_str(),vmsa::mBeamEnergy[energy].c_str());
                  ...
                  for(int iphi = 0; iphi < 12; iphi++)
                  {
                    ...
                    {   
                      ...
                      yield_hist_plot_cos_bin[iphi][i+2]->SetTitle(Form("Corrected Yields %1.1f<p_{T}<%1.1f, %d#pi/6<#phi'<%d#pi/6",ptbin_rho00[i+2],ptbin_rho00[i+3],iphi,iphi+1));
                      yield_hist_plot_cos_bin[iphi][i+2]->GetXaxis()->SetTitle("cos(#theta*)");
                      yield_hist_plot_cos_bin[iphi][i+2]->GetYaxis()->SetTitle("count");
                      yield_hist_plot_cos_bin[iphi][i+2]->Draw("pE");

                      Func_obs_cos_bin[iphi][i+2]->SetLineColor(color[0]);
                      Func_obs_cos_bin[iphi][i+2]->Draw("C same");
                    }
                    c->Update();
                  ...
                  outputpdf = Form("figures/%s/%s/PtEmbed1DCosHelicityRawYields_SmallBins.pdf",vmsa::mPID[pid].c_str(),vmsa::mBeamEnergy[energy].c_str());
                  ...
                  for(int iphi = 0; iphi < 12; iphi++)
                  {
                    ...
                    {   
                      ...
                      rawyield_hist_plot_cos_bin[iphi][i+2]->SetTitle(Form("RawYields %1.1f<p_{T}<%1.1f, %d#pi/6<#phi'<%d#pi/6",ptbin_rho00[i+2],ptbin_rho00[i+3],iphi,iphi+1));
                      rawyield_hist_plot_cos_bin[iphi][i+2]->GetXaxis()->SetTitle("cos(#theta*)");
                      rawyield_hist_plot_cos_bin[iphi][i+2]->GetYaxis()->SetTitle("count");
                      rawyield_hist_plot_cos_bin[iphi][i+2]->Draw("pE");
                    }
                    c->Update();
                    c->Print(outputpdf.c_str());
                  }
                  ...
                  outputpdf = Form("figures/%s/%s/PtEmbed1DCosHelicityEfficiency_SmallBins.pdf",vmsa::mPID[pid].c_str(),vmsa::mBeamEnergy[energy].c_str());
                  outputstartpdf = Form("%s[",outputpdf.c_str());
                  outputstoppdf = Form("%s]",outputpdf.c_str());
                  ...
                  for(int iphi = 0; iphi < 12; iphi++)
                  {
                    ...
                    {   
                      ...
                      eff_hist_plot_cos_bin[iphi][i+2]->SetTitle(Form("Efficiency %1.1f<p_{T}<%1.1f, %d#pi/6<#phi'<%d#pi/6",ptbin_rho00[i+2],ptbin_rho00[i+3],iphi,iphi+1));
                      eff_hist_plot_cos_bin[iphi][i+2]->GetXaxis()->SetTitle("cos(#theta*)");
                      eff_hist_plot_cos_bin[iphi][i+2]->GetYaxis()->SetTitle("count");
                      eff_hist_plot_cos_bin[iphi][i+2]->Draw("pE");
                    }
                    c->Update();
                    c->Print(outputpdf.c_str());
                  }
                  ...
                  {   
                    ...
                    Func_obs_cos[i+2]->SetLineColor(color[0]);
                    Func_obs_cos[i+2]->Draw("C same");
                  }
                  c->SaveAs(Form("figures/%s/%s/PtEmbed1DCosHelicityEfficiencyCorrectedYields.pdf",vmsa::mPID[pid].c_str(),vmsa::mBeamEnergy[energy].c_str()));
                  //////////////////////   1D Cos Corrected Yields ///////////////////////////
                  //////////////////////   1D Cos Raw Yields ///////////////////////////
                  for(int i = 0; i < 4; i++)
                  {   
                  ...
                  c->SaveAs(Form("figures/%s/%s/PtEmbed1DCosHelicityRawYields.pdf",vmsa::mPID[pid].c_str(),vmsa::mBeamEnergy[energy].c_str()));
                  //////////////////////   1D Cos Raw Yields ///////////////////////////
                  //////////////////////   1D Phi Efficiency  ///////////////////////////
                  for(int i = 0; i < 4; i++)
                  {   
                  ...
                  c->SaveAs(Form("figures/%s/%s/PtEmbed1DPhiHelicityEfficiency.pdf",vmsa::mPID[pid].c_str(),vmsa::mBeamEnergy[energy].c_str()));
                  //////////////////////   1D Phi Efficiency  ///////////////////////////
                  //////////////////////   1D Phi Raw Yields Small Bins  ///////////////////////////

                  outputpdf = Form("figures/%s/%s/PtEmbed1DPhiHelicityRawYields_SmallBins.pdf",vmsa::mPID[pid].c_str(),vmsa::mBeamEnergy[energy].c_str());
                  outputstartpdf = Form("%s[",outputpdf.c_str());
                  outputstoppdf = Form("%s]",outputpdf.c_str());
                  ...
                  for(int icos = 0; icos < 9; icos++)
                  {
                    ...
                    {   
                      ...
                      rawyield_hist_plot_phi_bin[icos][i+2]->SetTitle(Form("Raw Yields %1.1f<p_{T}<%1.1f, %1.1f/4.5<cos(#theta*)<%1.1f/4.5",ptbin_rho00[i+2],ptbin_rho00[i+3],-4.5+icos,-3.5+icos));
                      rawyield_hist_plot_phi_bin[icos][i+2]->GetXaxis()->SetTitle("cos(#theta*)");
                      rawyield_hist_plot_phi_bin[icos][i+2]->GetYaxis()->SetTitle("count");
                      rawyield_hist_plot_phi_bin[icos][i+2]->Draw("pE");
                    }
                    c->Update();
                    c->Print(outputpdf.c_str());
                  }
                  ...
                  outputpdf = Form("figures/%s/%s/PtEmbed1DPhiHelicityEfficiency_SmallBins.pdf",vmsa::mPID[pid].c_str(),vmsa::mBeamEnergy[energy].c_str());
                  outputstartpdf = Form("%s[",outputpdf.c_str());
                  outputstoppdf = Form("%s]",outputpdf.c_str());
                  ...
                  for(int icos = 0; icos< 9; icos++)
                  {
                    ...
                    {   
                      ...
                      eff_hist_plot_phi_bin[icos][i+2]->SetTitle(Form("Efficiency %1.1f<p_{T}<%1.1f, %1.1f/4.5<cos(#theta*)<%1.1f/4.5",ptbin_rho00[i+2],ptbin_rho00[i+3],-4.5+icos,-3.5+icos));
                      eff_hist_plot_phi_bin[icos][i+2]->GetXaxis()->SetTitle("cos(#theta*)");
                      eff_hist_plot_phi_bin[icos][i+2]->GetYaxis()->SetTitle("count");
                      eff_hist_plot_phi_bin[icos][i+2]->Draw("pE");
                    }
                    c->Update();
                    c->Print(outputpdf.c_str());
                  }
                  ...
                  outputpdf = Form("figures/%s/%s/PtEmbed1DPhiHelicityEfficiencyCorrectedYields_SmallBins.pdf",vmsa::mPID[pid].c_str(),vmsa::mBeamEnergy[energy].c_str());
                  outputstartpdf = Form("%s[",outputpdf.c_str());
                  outputstoppdf = Form("%s]",outputpdf.c_str());
                  ...
                  for(int icos = 0; icos < 9; icos++)
                  {
                    ...
                    {   
                      ...
                      yield_hist_plot_phi_bin[icos][i+2]->SetTitle(Form("Corrected Yields %1.1f<p_{T}<%1.1f, %1.1f/4.5<cos(#theta*)<%1.1f/4.5",ptbin_rho00[i+2],ptbin_rho00[i+3],-4.5+icos,-3.5+icos));
                      yield_hist_plot_phi_bin[icos][i+2]->GetXaxis()->SetTitle("phi'");
                      yield_hist_plot_phi_bin[icos][i+2]->GetYaxis()->SetTitle("count");
                      yield_hist_plot_phi_bin[icos][i+2]->Draw("pE");

                      Func_obs_phi_bin[icos][i+2]->SetLineColor(color[0]);
                      Func_obs_phi_bin[icos][i+2]->Draw("C same");
                    }
                    c->Update();
                    c->Print(outputpdf.c_str());
                  }
                  ...
                  {   
                    ...
                    Func_obs_phi[i+2]->SetLineColor(color[0]);
                    Func_obs_phi[i+2]->Draw("C same");
                  }
                  c->SaveAs(Form("figures/%s/%s/PtEmbed1DPhiHelicityEfficiencyCorrectedYields.pdf",vmsa::mPID[pid].c_str(),vmsa::mBeamEnergy[energy].c_str()));
                  //////////////////////   1D Phi Corrected Yields  ///////////////////////////
                  //////////////////////   1D Phi Raw Yields  ///////////////////////////
                  ...
                  c->SaveAs(Form("figures/%s/%s/PtEmbed1DPhiHelicityRawYields.pdf",vmsa::mPID[pid].c_str(),vmsa::mBeamEnergy[energy].c_str()));
                  //////////////////////   1D Phi Raw Yields  ///////////////////////////
                  //////////////////////   1D Phi Raw Yields Small Bins ///////////////////////////

                  outputpdf = Form("figures/%s/%s/PtEmbed1DPhiHelicitRawYields_SmallBins.pdf",vmsa::mPID[pid].c_str(),vmsa::mBeamEnergy[energy].c_str());
                  outputstartpdf = Form("%s[",outputpdf.c_str());
                  outputstoppdf = Form("%s]",outputpdf.c_str());
                  ...
                  for(int icos = 0; icos < 9; icos++)
                  {
                    ...
                    {   
                      ...
                      rawyield_hist_plot_phi_bin[icos][i+2]->SetTitle(Form("Raw Yields %1.1f<p_{T}<%1.1f, %1.1f/4.5<cos(#theta*)<%1.1f/4.5",ptbin_rho00[i+2],ptbin_rho00[i+3],-4.5+icos,-3.5+icos));
                      rawyield_hist_plot_phi_bin[icos][i+2]->GetXaxis()->SetTitle("phi'");
                      rawyield_hist_plot_phi_bin[icos][i+2]->GetYaxis()->SetTitle("count");
                      rawyield_hist_plot_phi_bin[icos][i+2]->Draw("pE");
                    }
                    c->Update();
    ...
    if(order == 2) outputname = Form("output/%s/%s/AccPhiPtSys_%s_PolySys_Helicity_2D_OffDiag_Embed.root",vmsa::mPID[pid].c_str(),vmsa::mBeamEnergy[energy].c_str(),etastring.c_str(),spectra.c_str());
