...
void calculateFY_BinbyBin_EPRes_EffAcc_14GeV(const int energy = 3, const int pid = 0, bool doall = false, bool isBesI = false, bool random3D = false, int mode = 1, int etamode = 3, int ypadding = 5, int ipt = 1, int icent = 1, std::string ep = "sub", int order = 1) {
  ...
  if(doall)
  {
    ...
    TFile *eff_file;
    if(order == 1) eff_file = new TFile(Form("/gpfs01/star/pwg/gwilks3/VectorMesonSpinAlignment/AuAu14GeV_2019/OutPut/CosEfficiencyProccessTuple_EPRes_EffAcc_Ycut_Order1/Eff_14GeV_SingleParticle_noToF_Mode2_EtaMode%d.root",etamode),"READ");
    if(order == 2) eff_file = new TFile(Form("/gpfs01/star/pwg/gwilks3/VectorMesonSpinAlignment/AuAu14GeV_2019/OutPut/CosEfficiencyProccessTuple_EPRes_EffAcc_Ycut_Order2/Eff_14GeV_SingleParticle_noToF_Mode2_EtaMode%d.root",etamode),"READ");
    eff_file->Print();
        ...
        for(int i_norm = vmsa::Norm_start; i_norm < vmsa::Norm_stop; ++i_norm)
        {
          for(int i_sigma = vmsa::Sig_start; i_sigma < vmsa::Sig_stop; ++i_sigma)
          {
            for(int i_method = vmsa::Method_start; i_method < vmsa::Method_stop; ++i_method)
            {
              ...
              Title = new TString(Form("fit/yield_pt_%d_cent_%d_%s_Dca_%d_Sig_%d_%s_Norm_%d_Sigma_%d_%s_EtaMode%d_Divided.pdf",ipt,icent,ordertext[order-1].c_str(),i_dca,i_sig,vmsa::mPID[pid].c_str(),i_norm,i_sigma,vmsa::mInteMethod[i_method].c_str(),etamode));
              ...
              for(int iy = 0+ypadding; iy < vmsa::y_total-ypadding; iy++) 
              {
                ...
                //PtCos[iy]->Divide(h_theta_star_ratio[ipt][icent][iy]);
                PtCos[iy]->Write();
                ...
                PtCos[iy]->Fit(Func_obs,"NMIQ"); // fit corrected distribution for observerved rho00
                ...
                float weight = PtCos[iy]->Integral(1,7); //This is essentially the corrected yield with pt spectra known and rapidity spectra not known 
