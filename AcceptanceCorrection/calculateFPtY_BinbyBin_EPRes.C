...
void calculateFPtY_BinbyBin_EPRes(const int energy = 4, const int pid = 0, bool doall = true, bool isBesI = false, bool random3D = false, int mode = 2, int etamode = 0, int iy = 3, std::string ep = "sub") {
...
//  Double_t pt_set[8] = {0.6, 1.4, 2.2, 3.0, 3.8, 4.6, 5.4, 7.2};
  ...
  MCFile = new TFile(Form("/gpfs01/star/pwg/gwilks3/VectorMesonSpinAlignment/AuAu19GeV_2019/OutPut/AcceptanceProccessTupleEPRes_%s_noDelta/Acceptance_%s_19GeV_Mode%d_EtaMode%d.root",ep.c_str(),vmsa::mPID[pid].c_str(),mode,etamode),"READ");
  MCFile->Print();
 ...
 // c_play->Print(output_start.c_str());
  ...
  std::string outputname = Form("figures/Fplots_%s_%s_Mode%d_EtaMode%d_pty_theta_cent%d_y%d_%s.pdf",vmsa::mPID[pid].c_str(),vmsa::mBeamEnergy[energy].c_str(),mode,etamode,9,iy,ep.c_str());
  ...
  c_play->Print(output_stop.c_str()); // close pdf file
*/
  outputname = Form("figures/Fplots_%s_%s_Mode%d_EtaMode%d_pty_thetastar_cent%d_y%d_%s.pdf",vmsa::mPID[pid].c_str(),vmsa::mBeamEnergy[energy].c_str(),mode,etamode,9,iy,ep.c_str());
  ...
  string outputname = Form("figures/Fplots_%s_%s_Mode%d_EtaMode%d_pty_Fvalues_cent%d_y%d_%s.pdf",vmsa::mPID[pid].c_str(),vmsa::mBeamEnergy[energy].c_str(),mode,etamode,9,iy,ep.c_str());
  string output_start = Form("%s[",outputname.c_str());
 ...
 // }
  for(int i_cent = 2; i_cent <= 5; i_cent++) {
    Res_12 += resolution_2sub->GetBinContent(i_cent+1)/resolution_2sub->GetBinError(i_cent+1)/resolution_2sub->GetBinError(i_cent+1);
    Res_12_weight += 1./resolution_2sub->GetBinError(i_cent+1)/resolution_2sub->GetBinError(i_cent+1);
  }
  ...
  if(doall)
  {
    ...
    string outputname = Form("output/%s/%s/AccResRhoPtEtaSys_%s_iy_%d_%s_noDelta_2ndSubEPRes.root",vmsa::mPID[pid].c_str(),vmsa::mBeamEnergy[energy].c_str(),etastring.c_str(),iy,ep.c_str());
          ...
          for(int i_sigma = vmsa::Sig_start; i_sigma < vmsa::Sig_stop; ++i_sigma)
          {
            for(int i_method = vmsa::Method_start; i_method < vmsa::Method_stop; ++i_method)
            {
              ...
              for(int ipt = 2; ipt < 6; ipt++)
              {
                ...
                PtCos[ipt]->Divide(h_theta_star_ratio[ipt][9][iy]);
                PtCos[ipt]->Write();
                ...
                Func_rho[ipt]->FixParameter(2, 0.0);//Fval[ipt]);
                Func_rho[ipt]->FixParameter(3, 0.0);//Gval[ipt]);
