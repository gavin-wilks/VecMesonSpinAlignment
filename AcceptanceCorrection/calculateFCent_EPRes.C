...
void calculateFCent_EPRes(const int energy = 4, const int pid = 0, int ptbin = 0, bool doall = true, bool isBesI = false, bool random3D = false, int mode = 0, int etamode = 0, std::string ep = "sub") {
...
//  Double_t pt_set[8] = {0.6, 1.4, 2.2, 3.0, 3.8, 4.6, 5.4, 7.2};
  ...
  //MCFile = new TFile(Form("/gpfs01/star/pwg/gwilks3/VectorMesonSpinAlignment/AuAu19GeV_2019/OutPut/AcceptanceProccessTupleEPRes_%s%s_yabs1/Acceptance_%s_19GeV_Mode%d_EtaMode%d.root",ep.c_str(),xtraopt.c_str(),vmsa::mPID[pid].c_str(),mode,etamode),"READ");
  MCFile = new TFile(Form("./Acceptance_WithBeta/Acceptance_%s_19GeV_Mode%d_EtaMode%d.root",vmsa::mPID[pid].c_str(),mode,etamode),"READ");
 ...
 // c_play->Print(output_start.c_str());
  ...
  if(doall)
  {
    ...
    TFile *input = new TFile(Form("rho00/%s/%s/fixed_ptbin/RawRhoCentSys_%s.root",vmsa::mPID[pid].c_str(),vmsa::mBeamEnergy[energy].c_str(),etastring.c_str()),"READ");
    ...
    string outputname = Form("output/%s/%s/AccResRhoCentSys_%s_pt_%d_%s%s_2ndSubEPRes.root",vmsa::mPID[pid].c_str(),vmsa::mBeamEnergy[energy].c_str(),etastring.c_str(),ptbin,ep.c_str(),xtraopt.c_str());
