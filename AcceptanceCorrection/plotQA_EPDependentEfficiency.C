...
void calculateFY_BinbyBin_EPRes_EffAcc_Poly(const int energy = 4, const int pid = 0, bool doall = false, bool isBesI = false, bool random3D = false, int mode = 1, int etamode = 0, int ypadding = 2, int ipt = 1, int icent = 1, int i_poly = 0, std::string ep = "sub", int order = 1, int yspectra = 0) {
  ...
  if(doall)
  {
    ...
    TFile *eff_file;
    if(order == 1) eff_file = new TFile(Form("/gpfs01/star/pwg/gwilks3/VectorMesonSpinAlignment/AuAu19GeV_2019/OutPut/CosEfficiencyProccessTuple_EPRes_EffAcc_Ycut_Order1%s/Eff_19GeV_SingleParticle_noToF_Mode2_EtaMode%d.root",spectra.c_str(),etamode),"READ");
    if(order == 2) eff_file = new TFile(Form("/gpfs01/star/pwg/gwilks3/VectorMesonSpinAlignment/AuAu19GeV_2019/OutPut/CosEfficiencyProccessTuple_EPRes_EffAcc_Ycut_Order2%s/Eff_19GeV_SingleParticle_noToF_Mode2_EtaMode%d.root",spectra.c_str(),etamode),"READ");
    if(energy == 3 && order == 1) eff_file = new TFile(Form("/gpfs01/star/pwg/gwilks3/VectorMesonSpinAlignment/AuAu14GeV_2019/OutPut/CosEfficiencyProccessTuple_EPRes_EffAcc_Ycut_Order1%s/Eff_14GeV_SingleParticle_noToF_Mode2_EtaMode%d.root",spectra.c_str(),etamode),"READ");
    if(energy == 3 && order == 2) eff_file = new TFile(Form("/gpfs01/star/pwg/gwilks3/VectorMesonSpinAlignment/AuAu14GeV_2019/OutPut/CosEfficiencyProccessTuple_EPRes_EffAcc_Ycut_Order2%s/Eff_14GeV_SingleParticle_noToF_Mode2_EtaMode%d.root",spectra.c_str(),etamode),"READ");
    ...
    TFile *input;
    if(order == 1) input = new TFile(Form("rho00/%s/%s/Poly/RawRhoEtaSys_%s_Poly_FirstOrder.root",vmsa::mPID[pid].c_str(),vmsa::mBeamEnergy[energy].c_str(),etastring.c_str()),"READ");
    if(order == 2) input = new TFile(Form("rho00/%s/%s/Poly/RawRhoEtaSys_%s_Poly.root",vmsa::mPID[pid].c_str(),vmsa::mBeamEnergy[energy].c_str(),etastring.c_str()),"READ");
    ...
    string outputname;
    if(order == 1) outputname = Form("output/%s/%s/AccResRhoEtaSys_%s_Poly_FirstOrder_cent_%d_ipt_%d_poly%d%s.root",vmsa::mPID[pid].c_str(),vmsa::mBeamEnergy[energy].c_str(),etastring.c_str(),icent,ipt,i_poly+1,spectra.c_str());
    if(order == 2) outputname = Form("output/%s/%s/AccResRhoEtaSys_%s_Poly_cent_%d_ipt_%d_poly%d%s.root",vmsa::mPID[pid].c_str(),vmsa::mBeamEnergy[energy].c_str(),etastring.c_str(),icent,ipt,i_poly+1,spectra.c_str());
