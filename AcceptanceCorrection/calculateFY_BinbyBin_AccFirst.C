...
void calculateFY_BinbyBin_AccFirst(const int energy = 4, const int pid = 0, bool doall = true, bool isBesI = false, bool random3D = false, int mode = 1, int etamode = 0, int ypadding = 2, int ipt = 0, int icent = 1, std::string etastring = "eta1_eta1") {
...
//  Double_t pt_set[8] = {0.6, 1.4, 2.2, 3.0, 3.8, 4.6, 5.4, 7.2};
 ...
 // c_play->Print(output_start.c_str());
  ...
  std::string outputname = Form("figures/Fplots_%s_%s_Mode%d_EtaMode%d_rapidity_thetastar_cent%d_pt%d.pdf",vmsa::mPID[pid].c_str(),vmsa::mBeamEnergy[energy].c_str(),mode,etamode,icent,ipt);
  ...
  string outputname = Form("figures/Fplots_%s_%s_Mode%d_EtaMode%d_rapidity_Fvalues_pt%d_cent%d.pdf",vmsa::mPID[pid].c_str(),vmsa::mBeamEnergy[energy].c_str(),mode,etamode,ipt,icent);
  string output_start = Form("%s[",outputname.c_str());
          ...
          for(int i_sigma = vmsa::Sig_start; i_sigma < vmsa::Sig_stop; ++i_sigma)
          {
            for(int i_method = vmsa::Method_start; i_method < vmsa::Method_stop; ++i_method)
            {
              ...
              for(int iy = 0+ypadding; iy < vmsa::y_total-ypadding; iy++) 
              {
                ...
                Func_rho[iy] = new TF1(Form("Func_rho_%d",iy),FuncAFG_noRes,0,1,4);
...
double FuncAFG_noRes(double *x_val, double *par) {
  ...
  double As = (3.*rho-1.)/(1.-rho);
  //double As = A*(1.+3.*R)/(4.+A*(1.-R));
  //double Bs = A*(1.-R)/(4.+A*(1.-R));
  //double As = (A*(1.+3.*R) + B*(3.-3.*R))/denom;
  double Bs = 0.0;//(A*(1.-R)    + B*(3.+R)   )/denom;
...
double CalcResCorr(double obsrho) {
 
  return (4./(1. + 3.*R))*(obsrho - 1./3.) + 1./3.;
