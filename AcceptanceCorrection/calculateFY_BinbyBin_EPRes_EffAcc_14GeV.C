...
void calculateFY_BinbyBin_EPRes_EffAcc_14GeV(const int energy = 4, const int pid = 0, bool doall = false, bool isBesI = false, bool random3D = false, int mode = 1, int etamode = 0, int ypadding = 2, int ipt = 0, int icent = 0, std::string ep = "sub", int order = 2) {
  ...
  if(doall)
  {
    ...
    if(order == 2) input = new TFile(Form("rho00/%s/%s/RawRhoEtaSys_%s.root",vmsa::mPID[pid].c_str(),vmsa::mBeamEnergy[energy].c_str(),etastring.c_str()),"READ");
          ...
          for(int i_sigma = vmsa::Sig_start; i_sigma < vmsa::Sig_stop; ++i_sigma)
          {
            for(int i_method = vmsa::Method_start; i_method < vmsa::Method_stop; ++i_method)
            {
              ...
              for(int iy = 0+ypadding; iy < vmsa::y_total-ypadding; iy++) 
              {
                ...
                PtCos[iy]->Fit(Func_obs,"NMI"); // fit corrected distribution for observerved rho00
