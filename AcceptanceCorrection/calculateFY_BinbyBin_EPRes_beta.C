...
void calculateFY_BinbyBin_EPRes_beta(const int energy = 4, const int pid = 0, bool doall = false, bool isBesI = false, bool random3D = false, int mode = 1, int etamode = 3, int ypadding = 5, int ipt = 1, int icent = 1, std::string ep = "sub") {
  ...
  for(int iy = 0; iy < vmsa::y_total; iy++)
  {
    ...
    Func_A->SetParameter(1,0);
    h_theta_star_ratio[ipt][icent][iy]->Fit(Func_A,"WNMIER");
          ...
          for(int i_sigma = vmsa::Sig_start; i_sigma < vmsa::Sig_stop; ++i_sigma)
          {
            for(int i_method = vmsa::Method_start; i_method < vmsa::Method_stop; ++i_method)
            {
              ...
              for(int iy = 0+ypadding; iy < vmsa::y_total-ypadding; iy++) 
              {
                ...
                PtCos[iy]->Fit(Func_rho[iy], "WNMIER");
