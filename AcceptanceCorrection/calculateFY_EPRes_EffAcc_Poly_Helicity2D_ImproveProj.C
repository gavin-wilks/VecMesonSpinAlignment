...
void calculateFY_EPRes_EffAcc_Poly_Helicity2D_ImproveProj(const int energy = 4, const int pid = 0, bool doall = true, bool isBesI = false, bool random3D = false, int etamode = 0, int order = 2, int yspectra = 2, int EP = 0, int v2 = 1) {
  ...
  if(doall)
  {
    ...
    TFile *eff_file_plateau;
      ...
      if(order == 2) eff_file_plateau = new TFile(Form("../RcPhiEffCorr_Cos2PhiStarPhi_PhiEmbed/HelicityCorrectionsFixedBins/Eff_%s_SingleParticle_noToF_Mode4_EtaMode%d_PtBins2_5.root",vmsa::mBeamEnergy[energy].c_str(),etamode),"READ");
    ...
    TH1D *eff_hist_plot_cos_bin[12][10];
    TH1D *histmc_cos_bin[12][10];
    TH1D *histrc_cos_bin[12][10];

    TH1D *eff_hist_plot_phi_bin[9][10];
    TH1D *histmc_phi_bin[9][10];
    TH1D *histrc_phi_bin[9][10];

    TH2D *yield_hist_plot[10];
    TH1D *yield_hist_plot_cos[10];
    TH1D *yield_hist_plot_phi[10];
    TH1D *yield_hist_plot_cos_bin[12][10];
    TH1D *yield_hist_plot_phi_bin[9][10];

    TH2F *rawyield_hist_plot[10];
    TH1F *rawyield_hist_plot_cos[10];
    TH1F *rawyield_hist_plot_phi[10];
    TH1F *rawyield_hist_plot_cos_bin[12][10];
    TH1F *rawyield_hist_plot_phi_bin[9][10];
    
    for(int i=9; i<10;i++)
    {
      for(int j=1; j<=10; j++)
      {
        string histnamemc = Form("h_mMcEffCosHPhiPrime_Cent_9_Y_%d",j-1);
        string histnamerc = Form("h_mRc5EffCosHPhiPrime_Cent_9_Y_%d",j-1);
        ...
        for(int k=1; k<=9; k++)
        {
          histmc_phi_bin[k-1][j-1] = (TH1D*) histmc[j-1]->ProjectionY(Form("mcphi%d_%d",j-1,k-1),k,k,"e");
          histrc_phi_bin[k-1][j-1] = (TH1D*) histrc[j-1]->ProjectionY(Form("rcphi%d_%d",j-1,k-1),k,k,"e");

          eff_hist_plot_phi_bin[k-1][j-1] = (TH1D*) histrc_phi_bin[k-1][j-1]->Clone(Form("effphi%d_%d",j-1,k-1));
          eff_hist_plot_phi_bin[k-1][j-1]->Divide(histrc_phi_bin[k-1][j-1],histmc_phi_bin[k-1][j-1],1,1,"B");
        }
        for(int l=1; l<=12; l++)
        {
          histmc_cos_bin[l-1][j-1] = (TH1D*) histmc[j-1]->ProjectionX(Form("mccos%d_%d",j-1,l-1),l,l,"e");
          histrc_cos_bin[l-1][j-1] = (TH1D*) histrc[j-1]->ProjectionX(Form("rccos%d_%d",j-1,l-1),l,l,"e");

          eff_hist_plot_cos_bin[l-1][j-1] = (TH1D*) histrc_cos_bin[l-1][j-1]->Clone(Form("effcos%d_%d",j-1,l-1));
          eff_hist_plot_cos_bin[l-1][j-1]->Divide(histrc_cos_bin[l-1][j-1],histmc_cos_bin[l-1][j-1],1,1,"B");
        }
    ...
    double imrhoerr[10];
    TF2 *Func_obs[10];
    TH1D *Func_obs_cos[10];
    TH1D *Func_obs_phi[10];
    TH1D *Func_obs_cos_bin[12][10];
    TH1D *Func_obs_phi_bin[9][10];
            ...
            for(int i_sigma = vmsa::Sig_start; i_sigma < 1/*vmsa::Sig_stop*/; ++i_sigma)
            {
              for(int i_method = 1/*vmsa::Method_start*/; i_method < vmsa::Method_stop; ++i_method)
              {
                for(int i_poly = 0; i_poly < 1/*3*/; i_poly++)
                {
                  ...
                  for(Int_t PtBin=1; PtBin<=10; PtBin++) 
                  {
                    //eff_hist_plot[PtBin-1]->Print();
                    string key = Form("eta_%d_Centrality_%d_%s_Dca_%d_Sig_%d_%s_Norm_%d_Sigma_%d_%s_Poly%d",PtBin-1,i_cent,ordertext[order-1].c_str(),i_dca,i_sig,vmsa::mPID[pid].c_str(),i_norm,i_sigma,vmsa::mInteMethod[i_method].c_str(),i_poly+1);
                    cout << key << endl;
                    ...
                    rawyield_hist_plot[PtBin-1]->Print();
                    rawyield_hist_plot_cos[PtBin-1] = (TH1F*) rawyield_hist_plot[PtBin-1]->ProjectionX(Form("rawyieldcos_%d",PtBin-1),1,12,"e");
                    rawyield_hist_plot_phi[PtBin-1] = (TH1F*) rawyield_hist_plot[PtBin-1]->ProjectionY(Form("rawyieldphi_%d",PtBin-1),1,9,"e");
                    yield_hist_plot_cos[PtBin-1] = (TH1D*) yield_hist_plot[PtBin-1]->ProjectionX(Form("yieldcos_%d",PtBin-1),1,12,"e");
                    yield_hist_plot_phi[PtBin-1] = (TH1D*) yield_hist_plot[PtBin-1]->ProjectionY(Form("yieldphi_%d",PtBin-1),1,9,"e");

                    for(int iphi = 0; iphi < 12; iphi++)
                    {
                      yield_hist_plot_cos_bin[iphi][PtBin-1] = (TH1D*) yield_hist_plot[PtBin-1]->ProjectionX(Form("yieldcos_%d_%d",PtBin-1,iphi),iphi+1,iphi+1,"e");
                      rawyield_hist_plot_cos_bin[iphi][PtBin-1] = (TH1F*) rawyield_hist_plot[PtBin-1]->ProjectionX(Form("rawyieldcos_%d_%d",PtBin-1,iphi),iphi+1,iphi+1,"e");
                    }
                    for(int icos = 0; icos < 9; icos++)
                    {
                      yield_hist_plot_phi_bin[icos][PtBin-1] = (TH1D*) yield_hist_plot[PtBin-1]->ProjectionY(Form("yieldphi_%d_%d",PtBin-1,icos),icos+1,icos+1,"e");
                      rawyield_hist_plot_phi_bin[icos][PtBin-1] = (TH1F*) rawyield_hist_plot[PtBin-1]->ProjectionY(Form("rawyieldphi_%d_%d",PtBin-1,icos),icos+1,icos+1,"e");
                    }
                    ...
                    for(int icos = 0; icos < 9; icos++)
                    {
                      Func_obs_phi_bin[icos][PtBin-1] = new TH1D(Form("Func_obs_phi%d_%d",PtBin-1,icos),Form("Func_obs_phi%d_%d",PtBin-1,icos),nFineBinsPhi,0.0,2.0*TMath::Pi());
                    }
                    for(int iphi = 0; iphi < 12; iphi++)
                    {
                      Func_obs_cos_bin[iphi][PtBin-1] = new TH1D(Form("Func_obs_cos%d_%d",PtBin-1,iphi),Form("Func_obs_cos%d_%d",PtBin-1,iphi),nFineBinsCos,-1.0,1.0);
                    }
                    for(int i = 1; i <= nFineBinsCos; ++i)
                    {
                      for(int j = 1; j <= nFineBinsPhi; ++j)
                      {
                        double phi      = Func_obs_phi_bin[i-1][PtBin-1]->GetBinCenter(j);
                        double phiwidth = Func_obs_phi_bin[i-1][PtBin-1]->GetBinWidth(j);
                        double cos      = Func_obs_cos_bin[j-1][PtBin-1]->GetBinCenter(i);
                        double coswidth = Func_obs_cos_bin[j-1][PtBin-1]->GetBinWidth(i);

                        double integral = Func_obs[PtBin-1]->Integral(cos - 0.5 * coswidth, 
                                                                      cos + 0.5 * coswidth,
                                                                      phi - 0.5 * phiwidth,
                                                                      phi + 0.5 * phiwidth);
                        integral /= coswidth;
                        ...
                        Func_obs_cos_bin[j-1][PtBin-1]->SetBinContent(i, integral);
                        Func_obs_phi_bin[i-1][PtBin-1]->SetBinContent(j, integral);
                      }
                    }


                    Func_obs_cos[PtBin-1] = new TH1D(Form("Func_obs_cos%d",PtBin-1),Form("Func_obs_cos%d",PtBin-1),nFineBinsCos,-1.0,1.0);
                    Func_obs_phi[PtBin-1] = new TH1D(Form("Func_obs_phi%d",PtBin-1),Form("Func_obs_phi%d",PtBin-1),nFineBinsPhi,0.0,2.0*TMath::Pi());
                    for(int i = 1; i <= nFineBinsCos; ++i)
                    {
                      double cos = Func_obs_cos[PtBin-1]->GetBinCenter(i);
                      double coswidth = Func_obs_cos[PtBin-1]->GetBinWidth(i);
                      double totalvalue = 0.0;
                      for(int j = 1; j <= nFineBinsPhi; ++j)
                      {
                        double phi = Func_obs_phi[PtBin-1]->GetBinCenter(j);
                        double phiwidth = Func_obs_phi[PtBin-1]->GetBinWidth(j);
                        double cosintegral = Func_obs[PtBin-1]->Integral(cos - 0.5 * coswidth, 
                                                                         cos + 0.5 * coswidth,
                                                                         phi - 0.5 * phiwidth,
                                                                         phi + 0.5 * phiwidth);
                      ...
                      Func_obs_cos[PtBin-1]->SetBinContent(i, totalvalue);
                      cout << "bin = " << i << ", cos = " << cos << ", integral = " << totalvalue << endl;
                      cout << "bin = " << i << ", cos - 0.5 * coswidth = " << cos - 0.5 * coswidth << endl;
                      cout << "bin = " << i << ", cos + 0.5 * coswidth = " << cos + 0.5 * coswidth << endl;
                         

                      //cout << "Before scaling bin = " << i << ", cos = " << cos << ", integral = " << cosintegral << endl;
                    }
                    for(int i = 1; i <= nFineBinsPhi; ++i)
                    {
                      double phi = Func_obs_phi[PtBin-1]->GetBinCenter(i);
                      double phiwidth = Func_obs_phi[PtBin-1]->GetBinWidth(i);
                      double totalvalue = 0.0;
                      for(int j = 1; j <= nFineBinsCos; ++j)
                      {
                        double cos = Func_obs_cos[PtBin-1]->GetBinCenter(j);
                        double coswidth = Func_obs_cos[PtBin-1]->GetBinWidth(j);
                        double phiintegral = Func_obs[PtBin-1]->Integral(cos - 0.5 * coswidth,
                                                                         cos + 0.5 * coswidth, 
                                                                         phi - 0.5 * phiwidth,
                                                                         phi + 0.5 * phiwidth);
                      ...
                      Func_obs_phi[PtBin-1]->SetBinContent(i, totalvalue);
                      cout << "bin = " << i << ", phi = " << phi << ", integral = " << totalvalue << endl;
                      cout << "bin = " << i << ", phi - 0.5 * phiwidth = " << phi - 0.5 * phiwidth << endl;
                      cout << "bin = " << i << ", phi + 0.5 * phiwidth = " << phi + 0.5 * phiwidth << endl;
                    }
                  ...
                  TCanvas *c = new TCanvas("c","c",10,10,2000,800);
                  c->Divide(5,2);
                  for(int i = 0; i < 10; i++)
                  {   
                    c->cd(i+1);
                    ...
                    eff_hist_plot[i]->SetTitle(Form("Efficiency %d/5<y<%d/5",i-5,i-4));
                    eff_hist_plot[i]->GetXaxis()->SetTitle("cos(#theta*)");
                    eff_hist_plot[i]->GetYaxis()->SetTitle("#phi'");
                    eff_hist_plot[i]->Draw("Colz");
                  }
                  //////////////////////   2D Efficiency ///////////////////////////
                  c->SaveAs(Form("figures/%s/%s/Rapidity2DHelicityEfficiency.pdf",vmsa::mPID[pid].c_str(),vmsa::mBeamEnergy[energy].c_str()));
                  for(int i = 0; i < 10; i++)
                  {   
                    c->cd(i+1);
                      
                    yield_hist_plot[i]->SetTitle(Form("Corrected Yields %d/5<y<%d/5",i-5,i-4));
                    yield_hist_plot[i]->GetXaxis()->SetTitle("cos(#theta*)");
                    yield_hist_plot[i]->GetYaxis()->SetTitle("#phi'");
                    yield_hist_plot[i]->Draw("Colz");
                    //Func_obs[i]->Draw("same");
                  }
                  c->SaveAs(Form("figures/%s/%s/Rapidity2DHelicityEfficiencyCorrectedYields.pdf",vmsa::mPID[pid].c_str(),vmsa::mBeamEnergy[energy].c_str()));
                  //////////////////////   2D Efficiency ///////////////////////////
                  //////////////////////   2D Corrected Yields ///////////////////////////
                  for(int i = 0; i < 10; i++)
                  {   
                    ...
                    yield_hist_plot[i]->GetYaxis()->SetTitle("#phi'");
                    yield_hist_plot[i]->Draw("Colz");
                    Func_obs[i]->Draw("Colz");
                  }
                  c->SaveAs(Form("figures/%s/%s/Rapidity2DHelicity2DFunc.pdf",vmsa::mPID[pid].c_str(),vmsa::mBeamEnergy[energy].c_str()));
                  //////////////////////   2D Corrected Yields ///////////////////////////
                  //////////////////////   2D Raw Yields ///////////////////////////
                  for(int i = 0; i < 10; i++)
                  {   
                    ...
                    rawyield_hist_plot[i]->Print();
                    rawyield_hist_plot[i]->SetTitle(Form("Raw Yields %d/5<y<%d/5",i-5,i-4));
                    rawyield_hist_plot[i]->GetXaxis()->SetTitle("cos(#theta*)");
                    rawyield_hist_plot[i]->GetYaxis()->SetTitle("#phi'");
                    rawyield_hist_plot[i]->Draw("Colz");
                  }
                  c->SaveAs(Form("figures/%s/%s/Rapidity2DHelicityRawYields.pdf",vmsa::mPID[pid].c_str(),vmsa::mBeamEnergy[energy].c_str()));
                  //////////////////////   2D Raw Yields ///////////////////////////
                  //////////////////////   1D Cos Efficiency ///////////////////////////
                  for(int i = 0; i < 10; i++)
                  {   
                    c->cd(i+1);           
                    eff_hist_plot_cos[i]->SetTitle(Form("Efficiency %d/5<y<%d/5",i-5,i-4));
                  ...
                  c->SaveAs(Form("figures/%s/%s/Rapidity1DCosHelicityEfficiency.pdf",vmsa::mPID[pid].c_str(),vmsa::mBeamEnergy[energy].c_str()));
                  //////////////////////   1D Cos Efficiency ///////////////////////////
                  //////////////////////   1D Cos Corrected Yields Small Bins ///////////////////////////
  
                  string outputpdf = Form("figures/%s/%s/Rapidity1DCosHelicityEfficiencyCorrectedYields_SmallBins.pdf",vmsa::mPID[pid].c_str(),vmsa::mBeamEnergy[energy].c_str());
                  ...
                  for(int iphi = 0; iphi < 12; iphi++)
                  {
                    ...
                    {   
                      ...
                      yield_hist_plot_cos_bin[iphi][i]->SetTitle(Form("Corrected Yields %d/5<y<%d/5, %d#pi/6<#phi'<%d#pi/6",i-5,i-4,iphi,iphi+1));
                      yield_hist_plot_cos_bin[iphi][i]->GetXaxis()->SetTitle("cos(#theta*)");
                      yield_hist_plot_cos_bin[iphi][i]->GetYaxis()->SetTitle("count");
                      yield_hist_plot_cos_bin[iphi][i]->Draw("pE");

                      Func_obs_cos_bin[iphi][i]->SetLineColor(color[0]);
                      Func_obs_cos_bin[iphi][i]->Draw("C same");
                    }
                    c->Update();
                  ...
                  outputpdf = Form("figures/%s/%s/Rapidity1DCosHelicityRawYields_SmallBins.pdf",vmsa::mPID[pid].c_str(),vmsa::mBeamEnergy[energy].c_str());
                  ...
                  for(int iphi = 0; iphi < 12; iphi++)
                  {
                    ...
                    {   
                      ...
                      rawyield_hist_plot_cos_bin[iphi][i]->SetTitle(Form("RawYields %d/5<y<%d/5, %d#pi/6<#phi'<%d#pi/6",i-5,i-4,iphi,iphi+1));
                      rawyield_hist_plot_cos_bin[iphi][i]->GetXaxis()->SetTitle("cos(#theta*)");
                      rawyield_hist_plot_cos_bin[iphi][i]->GetYaxis()->SetTitle("count");
                      rawyield_hist_plot_cos_bin[iphi][i]->Draw("pE");
                    }
                    c->Update();
                    c->Print(outputpdf.c_str());
                  }
                  ...
                  outputpdf = Form("figures/%s/%s/Rapidity1DCosHelicityEfficiency_SmallBins.pdf",vmsa::mPID[pid].c_str(),vmsa::mBeamEnergy[energy].c_str());
                  outputstartpdf = Form("%s[",outputpdf.c_str());
                  outputstoppdf = Form("%s]",outputpdf.c_str());
                  ...
                  for(int iphi = 0; iphi < 12; iphi++)
                  {
                    ...
                    {   
                      ...
                      eff_hist_plot_cos_bin[iphi][i]->SetTitle(Form("Efficiency %d/5<y<%d/5, %d#pi/6<#phi'<%d#pi/6",i-5,i-4,iphi,iphi+1));
                      eff_hist_plot_cos_bin[iphi][i]->GetXaxis()->SetTitle("cos(#theta*)");
                      eff_hist_plot_cos_bin[iphi][i]->GetYaxis()->SetTitle("count");
                      eff_hist_plot_cos_bin[iphi][i]->Draw("pE");
                    }
                    c->Update();
                    c->Print(outputpdf.c_str());
                  }
                  ...
                  {   
                    ...
                    yield_hist_plot_cos[i]->SetTitle(Form("Corrected Yields %d/5<y<%d/5",i-5,i-4));
                    yield_hist_plot_cos[i]->GetXaxis()->SetTitle("cos(#theta*)");
                    yield_hist_plot_cos[i]->GetYaxis()->SetTitle("count");
                    yield_hist_plot_cos[i]->Draw("pE");

                    Func_obs_cos[i]->SetLineColor(color[0]);
                    Func_obs_cos[i]->Draw("C same");
                  }
                  c->SaveAs(Form("figures/%s/%s/Rapidity1DCosHelicityEfficiencyCorrectedYields.pdf",vmsa::mPID[pid].c_str(),vmsa::mBeamEnergy[energy].c_str()));
                  //////////////////////   1D Cos Corrected Yields ///////////////////////////
                  //////////////////////   1D Cos Raw Yields ///////////////////////////
                  for(int i = 0; i < 10; i++)
                  {   
                    ...
                    rawyield_hist_plot_cos[i]->SetTitle(Form("Raw Yields %d/5<y<%d/5",i-5,i-4));
                    rawyield_hist_plot_cos[i]->GetXaxis()->SetTitle("cos(#theta*)");
                    rawyield_hist_plot_cos[i]->GetYaxis()->SetTitle("count");
                    rawyield_hist_plot_cos[i]->Draw("pE");
                  }
                  c->SaveAs(Form("figures/%s/%s/Rapidity1DCosHelicityRawYields.pdf",vmsa::mPID[pid].c_str(),vmsa::mBeamEnergy[energy].c_str()));
                  //////////////////////   1D Cos Raw Yields ///////////////////////////
                  //////////////////////   1D Phi Efficiency  ///////////////////////////
                  for(int i = 0; i < 10; i++)
                  {   
                    c->cd(i+1);           
                    eff_hist_plot_phi[i]->SetTitle(Form("Efficiency %d/5<y<%d/5",i-5,i-4));
                    eff_hist_plot_phi[i]->GetXaxis()->SetTitle("phi'");
                    eff_hist_plot_phi[i]->GetYaxis()->SetTitle("Efficiency");
                    eff_hist_plot_phi[i]->Draw("pE");
                  }
                  c->SaveAs(Form("figures/%s/%s/Rapidity1DPhiHelicityEfficiency.pdf",vmsa::mPID[pid].c_str(),vmsa::mBeamEnergy[energy].c_str()));
                  //////////////////////   1D Phi Efficiency  ///////////////////////////
                  //////////////////////   1D Phi Raw Yields Small Bins  ///////////////////////////

                  outputpdf = Form("figures/%s/%s/Rapidity1DPhiHelicityRawYields_SmallBins.pdf",vmsa::mPID[pid].c_str(),vmsa::mBeamEnergy[energy].c_str());
                  outputstartpdf = Form("%s[",outputpdf.c_str());
                  outputstoppdf = Form("%s]",outputpdf.c_str());
                  ...
                  for(int icos = 0; icos < 9; icos++)
                  {
                    ...
                    {   
                      ...
                      rawyield_hist_plot_phi_bin[icos][i]->SetTitle(Form("Raw Yields %d/5<y<%d/5, %1.1f/4.5<cos(#theta*)<%1.1f/4.5",i-5,i-4,-4.5+icos,-3.5+icos));
                      rawyield_hist_plot_phi_bin[icos][i]->GetXaxis()->SetTitle("cos(#theta*)");
                      rawyield_hist_plot_phi_bin[icos][i]->GetYaxis()->SetTitle("count");
                      rawyield_hist_plot_phi_bin[icos][i]->Draw("pE");
                    }
                    c->Update();
                    c->Print(outputpdf.c_str());
                  }
                  ...
                  outputpdf = Form("figures/%s/%s/Rapidity1DPhiHelicityEfficiency_SmallBins.pdf",vmsa::mPID[pid].c_str(),vmsa::mBeamEnergy[energy].c_str());
                  outputstartpdf = Form("%s[",outputpdf.c_str());
                  outputstoppdf = Form("%s]",outputpdf.c_str());
                  ...
                  for(int icos = 0; icos< 9; icos++)
                  {
                    ...
                    {   
                      ...
                      eff_hist_plot_phi_bin[icos][i]->SetTitle(Form("Efficiency %d/5<y<%d/5, %1.1f/4.5<cos(#theta*)<%1.1f/4.5",i-5,i-4,-4.5+icos,-3.5+icos));
                      eff_hist_plot_phi_bin[icos][i]->GetXaxis()->SetTitle("cos(#theta*)");
                      eff_hist_plot_phi_bin[icos][i]->GetYaxis()->SetTitle("count");
                      eff_hist_plot_phi_bin[icos][i]->Draw("pE");
                    }
                    c->Update();
                    c->Print(outputpdf.c_str());
                  }
                  ...
                  outputpdf = Form("figures/%s/%s/Rapidity1DPhiHelicityEfficiencyCorrectedYields_SmallBins.pdf",vmsa::mPID[pid].c_str(),vmsa::mBeamEnergy[energy].c_str());
                  outputstartpdf = Form("%s[",outputpdf.c_str());
                  outputstoppdf = Form("%s]",outputpdf.c_str());
                  ...
                  for(int icos = 0; icos < 9; icos++)
                  {
                    ...
                    {   
                      ...
                      yield_hist_plot_phi_bin[icos][i]->SetTitle(Form("Corrected Yields %d/5<y<%d/5, %1.1f/4.5<cos(#theta*)<%1.1f/4.5",i-5,i-4,-4.5+icos,-3.5+icos));
                      yield_hist_plot_phi_bin[icos][i]->GetXaxis()->SetTitle("phi'");
                      yield_hist_plot_phi_bin[icos][i]->GetYaxis()->SetTitle("count");
                      yield_hist_plot_phi_bin[icos][i]->Draw("pE");

                      Func_obs_phi_bin[icos][i]->SetLineColor(color[0]);
                      Func_obs_phi_bin[icos][i]->Draw("C same");
                    }
                    c->Update();
                    c->Print(outputpdf.c_str());
                  }
                  ...
                  {   
                    ...
                    yield_hist_plot_phi[i]->SetTitle(Form("Corrected Yields %d/5<y<%d/5",i-5,i-4));
                    yield_hist_plot_phi[i]->GetXaxis()->SetTitle("phi'");
                    yield_hist_plot_phi[i]->GetYaxis()->SetTitle("count");
                    yield_hist_plot_phi[i]->Draw("pE");

                    Func_obs_phi[i]->SetLineColor(color[0]);
                    Func_obs_phi[i]->Draw("C same");
                  }
                  c->SaveAs(Form("figures/%s/%s/Rapidity1DPhiHelicityEfficiencyCorrectedYields.pdf",vmsa::mPID[pid].c_str(),vmsa::mBeamEnergy[energy].c_str()));
                  //////////////////////   1D Phi Corrected Yields  ///////////////////////////
                  //////////////////////   1D Phi Raw Yields  ///////////////////////////
                  ...
                  {   
                    ...
                    rawyield_hist_plot_phi[i]->SetTitle(Form("Raw Yields %d/5<y<%d/5",i-5,i-4));
                    rawyield_hist_plot_phi[i]->GetXaxis()->SetTitle("phi'");
                    rawyield_hist_plot_phi[i]->GetYaxis()->SetTitle("count");
                    rawyield_hist_plot_phi[i]->Draw("pE");
                  }
                  c->SaveAs(Form("figures/%s/%s/Rapidity1DPhiHelicityRawYields.pdf",vmsa::mPID[pid].c_str(),vmsa::mBeamEnergy[energy].c_str()));
                  //////////////////////   1D Phi Raw Yields  ///////////////////////////
                  //////////////////////   1D Phi Raw Yields Small Bins ///////////////////////////

                  outputpdf = Form("figures/%s/%s/Rapidity1DPhiHelicitRawYields_SmallBins.pdf",vmsa::mPID[pid].c_str(),vmsa::mBeamEnergy[energy].c_str());
                  outputstartpdf = Form("%s[",outputpdf.c_str());
                  outputstoppdf = Form("%s]",outputpdf.c_str());
                  ...
                  for(int icos = 0; icos < 9; icos++)
                  {
                    ...
                    {   
                      ...
                      rawyield_hist_plot_phi_bin[icos][i]->SetTitle(Form("Raw Yields %d/5<y<%d/5, %1.1f/4.5<cos(#theta*)<%1.1f/4.5",i-5,i-4,-4.5+icos,-3.5+icos));
                      rawyield_hist_plot_phi_bin[icos][i]->GetXaxis()->SetTitle("phi'");
                      rawyield_hist_plot_phi_bin[icos][i]->GetYaxis()->SetTitle("count");
                      rawyield_hist_plot_phi_bin[icos][i]->Draw("pE");
                    }
                    c->Update();
    ...
    if(order == 2) outputname = Form("output/%s/%s/AccPhiPtSys_%s_PolySys_Helicity_2D_OffDiag_Rapidity.root",vmsa::mPID[pid].c_str(),vmsa::mBeamEnergy[energy].c_str(),etastring.c_str(),spectra.c_str());
              ...
              for(int i_method = 1/*vmsa::Method_start*/; i_method < vmsa::Method_stop; ++i_method)
              {
                for(int i_poly = 0; i_poly < 1/*3*/; i_poly++)
                {
                  ...
                  for(Int_t PtBin=1; PtBin<=10; PtBin++) {
                    ...
                    g_rho00->SetPoint(PtBin-1,ptmean,rho00[PtBin-1]);
                    g_rho00->SetPointError(PtBin-1,0.0,0.0,rho00err[PtBin-1],rho00err[PtBin-1]);
                  }
                  g_rho00->Write();
                  delete Title;
                  ...
                  for(Int_t PtBin=1; PtBin<=10; PtBin++) {
                    ...
                    g_real->SetPoint(PtBin-1,ptmean,real[PtBin-1]);
                    g_real->SetPointError(PtBin-1,0.0,0.0,realerr[PtBin-1],realerr[PtBin-1]);
                  }
                  g_real->Write();
                  delete Title;
                  ...
                  for(Int_t PtBin=1; PtBin<=10; PtBin++) {
                    //double ptmean = (ptbin_rho00[PtBin]+ptbin_rho00[PtBin-1])/2.0;
                    double ptmean = (float(PtBin)-5.5)/5.; //(ptbin_rho00[PtBin]+ptbin_rho00[PtBin-1])/2.0;
                    g_imag->SetPoint(PtBin-1,ptmean,imag[PtBin-1]);
                    g_imag->SetPointError(PtBin-1,0.0,0.0,imagerr[PtBin-1],imagerr[PtBin-1]);
                  }
                  g_imag->Write();
                  delete Title;
                  ...
                  for(Int_t PtBin=1; PtBin<=10; PtBin++) {
                    //double ptmean = (ptbin_rho00[PtBin]+ptbin_rho00[PtBin-1])/2.0;
                    double ptmean = (float(PtBin)-5.5)/5.; //(ptbin_rho00[PtBin]+ptbin_rho00[PtBin-1])/2.0;
                    g_rerho->SetPoint(PtBin-1,ptmean,rerho[PtBin-1]);
                    g_rerho->SetPointError(PtBin-1,0.0,0.0,rerhoerr[PtBin-1],rerhoerr[PtBin-1]);
                  }
                  g_rerho->Write();
                  delete Title;
                  ...
                  for(Int_t PtBin=1; PtBin<=10; PtBin++) {
                    //double ptmean = (ptbin_rho00[PtBin]+ptbin_rho00[PtBin-1])/2.0;
                    double ptmean = (float(PtBin)-5.5)/5.; //(ptbin_rho00[PtBin]+ptbin_rho00[PtBin-1])/2.0;
                    g_imrho->SetPoint(PtBin-1,ptmean,imrho[PtBin-1]);
                    g_imrho->SetPointError(PtBin-1,0.0,0.0,imrhoerr[PtBin-1],imrhoerr[PtBin-1]);
                  }
                  g_imrho->Write();
                  delete Title;
