...
void plotQA_EPDependentEfficiency_Y(const int energy = 4, const int pid = 0, int etamode = 0, int ypadding = 2, int order = 2, int yspectra = 0) {
  ...
  TFile *eff_file;
  if(order == 1) eff_file = new TFile(Form("/gpfs01/star/pwg/gwilks3/VectorMesonSpinAlignment/AuAu19GeV_2019/OutPut/CosEfficiencyProccessTuple_EPRes_EffAcc_Ycut_Order1%s_EP/Eff_19GeV_SingleParticle_noToF_Mode2_EtaMode%d.root",spectra.c_str(),etamode),"READ");
  if(order == 2) eff_file = new TFile(Form("/gpfs01/star/pwg/gwilks3/VectorMesonSpinAlignment/AuAu19GeV_2019/OutPut/CosEfficiencyProccessTuple_EPRes_EffAcc_Ycut_Order2%s_EP/Eff_19GeV_SingleParticle_noToF_Mode2_EtaMode%d.root",spectra.c_str(),etamode),"READ");
  if(energy == 3 && order == 1) eff_file = new TFile(Form("/gpfs01/star/pwg/gwilks3/VectorMesonSpinAlignment/AuAu14GeV_2019/OutPut/CosEfficiencyProccessTuple_EPRes_EffAcc_Ycut_Order1%s_EP/Eff_14GeV_SingleParticle_noToF_Mode2_EtaMode%d.root",spectra.c_str(),etamode),"READ");
  if(energy == 3 && order == 2) eff_file = new TFile(Form("/gpfs01/star/pwg/gwilks3/VectorMesonSpinAlignment/AuAu14GeV_2019/OutPut/CosEfficiencyProccessTuple_EPRes_EffAcc_Ycut_Order2%s_EP/Eff_14GeV_SingleParticle_noToF_Mode2_EtaMode%d.root",spectra.c_str(),etamode),"READ");
  eff_file->Print();
  ...
  for(int ipt = 0; ipt < vmsa::pt_rebin_y; ipt++)
  {
    for(int icent = 0; icent < vmsa::cent_rebin_total; icent++)
    {
      ...
      { 
        std::string HistName;
        HistName = Form("h_mEffCosEP_Cent_%d_Pt_%d_Y_%d",icent,ipt,iy);
        cout << HistName << endl;
        eff[ipt][icent][iy] = (TH2D*)eff_file->Get(HistName.c_str());
        eff[ipt][icent][iy]->Print();
      }
    }
  }
  ...
  std::string outputname = Form("figures/%s/%s/ystudy_EPefficiency_%s.pdf",vmsa::mPID[pid].c_str(),vmsa::mBeamEnergy[energy].c_str(),vmsa::mBeamEnergy[energy].c_str(),order);
  ...
  for(int ipt = 0; ipt < vmsa::pt_rebin_y; ipt++)
  {
    for(int icent = 0; icent < vmsa::cent_rebin_total; icent++)
    {
      ...
      { 
        c_fit->cd(iy-1);
        //eff[ipt][icent][iy]->CenterTitle();
        eff[ipt][icent][iy]->SetTitle(Form("%1.1f<p_{T}<%1.1f, Cent %d-%d, %1.1f<y<%1.1f",vmsa::pt_low_y[energy][ipt],vmsa::pt_up_y[energy][ipt],cent_set[vmsa::cent_rebin[icent+1]],cent_set[vmsa::cent_rebin[icent]],vmsa::y_bin[iy],vmsa::y_bin[iy+1]));
        cout << Form("%1.1f<p_{T}<%1.1f, Cent %d-%d, %1.1f<y<%1.1f",vmsa::pt_low_y[energy][ipt],vmsa::pt_up_y[energy][ipt],cent_set[vmsa::cent_rebin[icent+1]],cent_set[vmsa::cent_rebin[icent]],vmsa::y_bin[iy],vmsa::y_bin[iy+1]) << endl;
        eff[ipt][icent][iy]->GetXaxis()->SetTitle("|cos#theta*|");
        eff[ipt][icent][iy]->GetXaxis()->CenterTitle();
        eff[ipt][icent][iy]->GetYaxis()->SetTitle("#phi-#Psi_{2}");
        eff[ipt][icent][iy]->GetYaxis()->CenterTitle();
        eff[ipt][icent][iy]->Draw("colz");
      }
      c_fit->Update();
