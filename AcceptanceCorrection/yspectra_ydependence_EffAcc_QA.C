...
void yspectra_ydependence_EffAcc_QA(const int energy = 4, const int pid = 0, int mode = 1, int etamode = 0, int ypadding = 2, int order = 2) {
  ...
  TString *Title;
  {
    ...
    TH1D *eff_ratio[vmsa::pt_rebin_y][vmsa::cent_rebin_total][vmsa::y_total];

    TFile *eff_file;
    if(order == 1) eff_file = new TFile(Form("/gpfs01/star/pwg/gwilks3/VectorMesonSpinAlignment/AuAu%s_2019/OutPut/CosEfficiencyProccessTuple_EPRes_EffAcc_Ycut_Order1_NoRapiditySpectra/Eff_%s_SingleParticle_noToF_Mode2_EtaMode%d.root",vmsa::mBeamEnergy[energy].c_str(),vmsa::mBeamEnergy[energy].c_str(),etamode),"READ");
    if(order == 2) eff_file = new TFile(Form("/gpfs01/star/pwg/gwilks3/VectorMesonSpinAlignment/AuAu%s_2019/OutPut/CosEfficiencyProccessTuple_EPRes_EffAcc_Ycut_Order2_NoRapiditySpectra/Eff_%s_SingleParticle_noToF_Mode2_EtaMode%d.root",vmsa::mBeamEnergy[energy].c_str(),vmsa::mBeamEnergy[energy].c_str(),etamode),"READ");
    eff_file->Print();
    TFile *eff_file_y;
    if(order == 1) eff_file_y = new TFile(Form("/gpfs01/star/pwg/gwilks3/VectorMesonSpinAlignment/AuAu%s_2019/OutPut/CosEfficiencyProccessTuple_EPRes_EffAcc_Ycut_Order1_WithRapiditySpectra/Eff_%s_SingleParticle_noToF_Mode2_EtaMode%d.root",vmsa::mBeamEnergy[energy].c_str(),vmsa::mBeamEnergy[energy].c_str(),etamode),"READ");
    if(order == 2) eff_file_y = new TFile(Form("/gpfs01/star/pwg/gwilks3/VectorMesonSpinAlignment/AuAu%s_2019/OutPut/CosEfficiencyProccessTuple_EPRes_EffAcc_Ycut_Order2_WithRapiditySpectra/Eff_%s_SingleParticle_noToF_Mode2_EtaMode%d.root",vmsa::mBeamEnergy[energy].c_str(),vmsa::mBeamEnergy[energy].c_str(),etamode),"READ");
    eff_file_y->Print();
    ...
    for(int ipt = 0; ipt < vmsa::pt_rebin_y; ipt++)
    {
      for(int icent = 0; icent < vmsa::cent_rebin_total; icent++)
      {
        ...
        { 
          ...
          eff_hist->Print();
          eff_hist_y->Print();
          eff_ratio[ipt][icent][iy] = (TH1D*) eff_hist_y->Clone();
          eff_ratio[ipt][icent][iy]->Divide(eff_hist);
          eff_ratio[ipt][icent][iy]->Print();
        }
      }
    }
    //eff_file->Close();
    ...
    TCanvas *c_y = new TCanvas("c_y","c_y",10,10,1500,600);
    c_y->Divide(5,2);

    string outputname = Form("output/%s/%s/yspectra_ydpendenceQA_%s_Order%d.pdf",vmsa::mPID[pid].c_str(),vmsa::mBeamEnergy[energy].c_str(),etastring.c_str(),order);
    ...
    for(int ipt = 0; ipt < vmsa::pt_rebin_y; ipt++)
    {
      for(int icent = 0; icent < vmsa::cent_rebin_total; icent++)
      {
        for(int iy = 0+ypadding; iy < vmsa::y_total-ypadding; iy++)
        { 
          cout << "iy-ypadding+1 = " << iy-ypadding+1 << endl;
          c_y->cd(iy-ypadding+1);
          c_y->cd(iy-ypadding+1)->SetLeftMargin(0.15);
          c_y->cd(iy-ypadding+1)->SetBottomMargin(0.15);
          c_y->cd(iy-ypadding+1)->SetTicks(1,1);
          c_y->cd(iy-ypadding+1)->SetGrid(0,0); 

          cout << centval[vmsa::cent_rebin[icent]] << endl;
          cout << centval[vmsa::cent_rebin[icent+1]] << endl;
          cout << vmsa::pt_low_y[energy][ipt] << endl;
	  cout << vmsa::pt_up_y[energy][ipt] << endl;
          cout << vmsa::y_bin[iy] << endl;
          cout << vmsa::y_bin[iy+1] << endl;
          string efftitle = Form("Cent %d-%d, %1.1f<p_{T}<%1.1f GeV/c, %1.1f<y<%1.1f", centval[vmsa::cent_rebin[icent+1]], centval[vmsa::cent_rebin[icent]], vmsa::pt_low_y[energy][ipt], vmsa::pt_up_y[energy][ipt], vmsa::y_bin[iy], vmsa::y_bin[iy+1]);
          cout << efftitle << endl;
          eff_ratio[ipt][icent][iy]->Print();
          eff_ratio[ipt][icent][iy]->SetTitle(efftitle.c_str());
          eff_ratio[ipt][icent][iy]->GetXaxis()->SetTitle("|cos#theta*|");
          eff_ratio[ipt][icent][iy]->GetYaxis()->SetTitle("(w/ y-spectra) / (w/o)");
          eff_ratio[ipt][icent][iy]->Draw("pE");
          for(int i = 0; i < 7; i++)
          {
            cout << eff_ratio[ipt][icent][iy]->GetBinContent(i+1) << endl;
          }
        }
        c_y->Update();
