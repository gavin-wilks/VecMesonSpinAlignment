...
TH2F *h2_theta_star, *h2_theta_star_before, *h2_theta_star_RP, *h2_theta_star_RP_before;
TProfile *cos2phi;
...
void McEtaF_Resolution(double Nrho=3333, int SetPt=1, int energy = 4, int pid = 0, int cent = 4, int const NMax = 5000, double rapidity = 1.0, int mode = 1, int etamode = 3, double Res = 0.4, bool fullEP = 0, bool v2on = 0, const char* jobID = "1")
{
  ...
  h2_theta_star           = new TH2F("h2_theta_star"          ,"h2_theta_star"          , 7, 0, 1, 30, 0.0, 2*TMath::Pi());
  h2_theta_star_before    = new TH2F("h2_theta_star_before"   ,"h2_theta_star_before"   , 7, 0, 1, 30, 0.0, 2*TMath::Pi());
  h2_theta_star_RP        = new TH2F("h2_theta_star_RP"       ,"h2_theta_star_RP"       , 7, 0, 1, 30, 0.0, 2*TMath::Pi());
  h2_theta_star_RP_before = new TH2F("h2_theta_star_RP_before","h2_theta_star_RP_before", 7, 0, 1, 30, 0.0, 2*TMath::Pi());
...
//  double const y = gRandom->Uniform(-vmsa::acceptanceRapidity, vmsa::acceptanceRapidity);
  ...
  if(mMode == 1) y = gRandom->Uniform(-1.0,1.0);
  //if(mMode == 1)
  //{
...
void fill(TLorentzVector* lPhi, TLorentzVector const& lKplus, TLorentzVector const& lKminus, int centrality)
{
  ...
  if(mMode != 3)
  {
    ...
    h2_theta_star_before->Fill(TMath::Abs(CosThetaStarEP),BetaP);
    h2_theta_star_RP_before->Fill(TMath::Abs(CosThetaStarRP),Beta);
  }
