...
void McEtaF_RapidityTest(double Nrho=3333, int SetPt=1, int energy = 4, int pid = 0, int cent = 5, int const NMax = 1000, double rapidity = 1.5, int mode = 1, double EP_res = 0.4, const char* jobID = "1")
{
  ...
  if(mMode == 1) cout << "pt range = [" << pt_centset[pt_bin-1] << "," << pt_centset[pt_bin] << "] GeV/c" << endl;  
...
//  TF1 *f_spec = new TF1("f_spec",pTLevy,vmsa::ptMin,vmsa::ptMax,3);
  ...
  if(mMode == 1) f_spec = new TF1("f_spec",pTLevy,pt_centset[pt_bin-1], pt_centset[pt_bin], 3);
...
//  double const pt = f_spec->GetRandom(vmsa::ptMin, vmsa::ptMax);
  ...
  if(mMode == 1) pt = f_spec->GetRandom(pt_centset[pt_bin-1], pt_centset[pt_bin]);
...
//  double const y = f_y->GetRandom(-2,2);
  ...
  double Psi2 = 0.0;
  if(mMode == 1) Psi2 = f_gaus->GetRandom(-TMath::Pi()/2.,TMath::Pi()/2.);
  f_flow->SetParameter(0,f_v2->Eval(pt));
  double const phi = f_flow->GetRandom() + Psi2;
