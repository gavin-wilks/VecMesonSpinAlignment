...
void acceptanceQA_4th_Res_v2test_comp_edge(const int energy = 4, const int pid = 0, bool doall = true, std::string res = "0p4", double resval = 0.4, int cut = 1, int method = 1, int v2 = 0) {
...
//  Double_t pt_set[8] = {0.6, 1.4, 2.2, 3.0, 3.8, 4.6, 5.4, 7.2};
  ...
  std::string etatext[4] = {"0.4","0.6","0.8","1.0"};
  std::string ytextfile[4] = {"0p2y0p4","0p4y0p6","0p6y0p8","0p8y1"};
  std::string ytext[4] = {"0.2<y<0.4","0.4<y<0.6","0.6<y<0.8","0.8<y<1"};
  ...
  for(int ieta = 0; ieta < 4; ieta++)
  {
    ...
    for(int irho = 0; irho < 3; irho++)
    {
      ...
      MCFiles[ieta][irho] = new TFile(Form("/gpfs01/star/pwg/gwilks3/VectorMesonSpinAlignment/Data/%s/Acceptance/ResolutionTesting/ResolutionTest_0p4_EtaCut%d_%s_v2%d/McAcceptanceOutput_pt1_energy%d_pid%d_cent4_EtaMode_0_nrho%d.root",vmsa::mPID[pid].c_str(),etamode[ieta],ytextfile[ieta].c_str(),v2,energy,pid,rho00val[irho]),"READ");
...
*/  
  ...
  double Fvalnov2[4] = {-1.50747,-0.692396,1.08766,2.87368};
  double Gvalnov2[4] = {0.589424,-0.351237,-2.26152,-3.94098};
  ...
  for(int ieta = 0; ieta < 4; ieta++)
  {
    ...
    for(int irho = 0; irho < 3; irho++) 
    {
      ...
      //Func_rho->FixParameter(2, Fvalnov2[ieta]);
      //Func_rho->FixParameter(3, Gvalnov2[ieta]);
  ...
  for(int ieta = 0; ieta < 4; ieta++)
  {
    ...
    leg2->AddEntry(g_rho00RecoObs[ieta],Form("|#eta|<%s, %s, Before Correction",etatext[ieta].c_str(),ytext[ieta].c_str()),"p");
    leg2->AddEntry(g_rho00outAfterIn[ieta],Form("|#eta|<%s, %s, After Correction",etatext[ieta].c_str(),ytext[ieta].c_str()),"p");
  }
  leg2->AddEntry(linear,"reco = input","l");
  leg2->Draw("same");

  std::string FigureName = Form("./figures/AcceptanceQA/EDGE_v2%d_AcceptanceQAInput_RecoObs_%s%s_method%d.pdf",v2,vmsa::mBeamEnergy[energy].c_str(),cutoption.c_str(),method);
  ...
  for(int ieta = 0; ieta < 4; ieta++)
  {
    ...
    leg3->AddEntry(g_diffI[ieta],Form("|#eta|<%s, %s",etatext[ieta].c_str(),ytext[ieta].c_str()),"p");
  }
  //leg2->AddEntry(linear,"","l");
 
  if(v2 == 1) h_play->GetYaxis()->SetRangeUser(-0.05,0.025);
  if(v2 == 0) h_play->GetYaxis()->SetRangeUser(-0.01,0.025);
  h_play->DrawCopy("pE");
  for(int ieta = 0; ieta < 4; ieta++)
  {  
  ...
  FigureName = Form("./figures/AcceptanceQA/EDGE_v2%d_AcceptanceQADifferenceFromIdeal_%s%s_method%d.pdf",v2,vmsa::mBeamEnergy[energy].c_str(),cutoption.c_str(),method);
