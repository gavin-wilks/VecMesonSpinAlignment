...
void acceptanceQA_6th_Res_v2test_v2vary_lessv2(const int energy = 4, const int pid = 0, std::string res = "0p4", double resval = 0.4, int cut = 1, int method = 0, int v2 = 1, int fit2d = 0, int EP = 1, int etamode = 0, int ieta = 1) {
  ...
  std::string v2text[11] = {"0.0","0.050","0.10","0.15","0.2","0.25"};
  double      v2val[11] = {0.0,0.050,0.10,0.15,0.2,0.25};
 ...
 int  rhoorder[3]={1,0,2};
  ...
  for(int ir = 0; ir < 3; ir++)
  {
    ...
    for(int iem = 0; iem < 6; iem++)
    {
      ...
      MCFiles[iem][irho] = new TFile(Form("/gpfs01/star/pwg/gwilks3/VectorMesonSpinAlignment/Data/%s/Acceptance/ResolutionTesting/AcceptanceQA_0.4_EtaCut%d_v2study_%s_v2val%s/McAcceptanceOutput_pt1_energy%d_pid%d_cent4_EtaMode_0_nrho%d.root",vmsa::mPID[pid].c_str(),etamode,ytextfile.c_str(),v2text[iem].c_str(),energy,pid,rho00val[irho]),"READ");
      ...
      TF1 *Func_1D = new TF1("Func_1D",FuncAFGH,0,1,6);
      ...
      if(EP == 0 && fit2d == 0)
      {
        ...
        Func_1D->FixParameter(4,0.0);
        Func_1D->FixParameter(5,1.0);
      ...
      TF1 *Func_A = new TF1("Func_A",Func6th,0,1,4);
      ...
      if(cut == 1 && method == 0){
        ...
        h_theta_star_clone->Fit(Func_A,"WNMIER");
        ...
        Hval[iem][ieta][irho] = Func_A->GetParameter(3);
        Herr[iem][ieta][irho] = Func_A->GetParError(3);
        c1->SetFillColor(0);
        c1->SetGrid(0,0);
        c1->SetTitle(0);
        c1->SetBottomMargin(0.15);
        ...
        //legend->AddEntry(Func_Orig,"2nd Order Fit","l");
        legend->Draw("same");
        c1->SaveAs(Form("FValues/yabs1/pt1_rho%d_iem%d_eta%d%s_method%d_EPSmear%d.pdf",irho,iem,ieta,cutoption.c_str(),method,EP));
        delete c1;
      }
      ...
      HvalRP[iem][ieta][irho] = Func_A->GetParameter(3);
      HerrRP[iem][ieta][irho] = Func_A->GetParError(3);
      ...
      if(fit2d == 0)
      {
        ...
        Func_AFG->FixParameter(4,HvalRP[iem][ieta][1]);//Gval[irho]);
        ...
        Func_AFG->FixParameter(5, 1.0);//Res_12);
        h_theta_star_RP[iem][ieta][irho]->Fit(Func_AFG,"WNMIER");
...
*/  
  ...
  for(int iem = 0; iem < 6; iem++)
  {
    ...
    //cout << "F from theta = " << Fpval[irho] << " +/- " << Fperr[ierr] << "      G from theta = " << Gpval[irho] << " +/- " << Gperr[irho] << endl;
      ...
      cout << "F = " << Fval[iem][ieta][irho] << " +/- " << Ferr[iem][ieta][irho] << endl;
      cout << "G = " << Gval[iem][ieta][irho] << " +/- " << Gerr[iem][ieta][irho] << endl;
      cout << "H = " << Hval[iem][ieta][irho] << " +/- " << Herr[iem][ieta][irho] << endl;
    } 
  }
  ...
  for(int irho = 0; irho < 3; irho++) 
  {
    ...
    for(int iem = 0; iem < 6; iem++)
    {
      ...
      Func_rho->FixParameter(4, Hval[iem][ieta][irho]);
      ...
      Func_rho->FixParameter(5, resval);//Res_12);
      ...
      h_theta_star[iem][ieta][irho]->Fit(Func_rho, "WNMIER");
