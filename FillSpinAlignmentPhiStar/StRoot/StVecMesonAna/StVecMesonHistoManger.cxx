...
void StVecMesonHistoManger::InitSys(Int_t X_flag, Int_t mode) // 0 for Same Event, 1 for Mixed Event
{
  ...
  {
    for(Int_t i_cent = 9/*vmsa::Cent_start*/; i_cent < 10 /*vmsa::Cent_stop*/; i_cent++) // centrality bin
    {
      ...
      //      {
      //        KEY_Mass2 = Form("pt_%d_Centrality_%d_CosThetaStar_%d_PhiPsi_%d_%s_Dca_%d_Sig_%d_%s_%s",i_pt,i_cent,i_theta,i_phipsi,EP_string.c_str(),i_dca,i_sig,vmsa::mPID[mode].c_str(),Mode[X_flag].Data());
      //        h_mMass2[KEY_Mass2] = new TH1F(KEY_Mass2.Data(),KEY_Mass2.Data(),100,vmsa::InvMass_low[mode],vmsa::InvMass_high[mode]);
      //        h_mMass2[KEY_Mass2]->Sumw2();
      //      }
      ...
      //        for(int i_phipsi = 0; i_phipsi < 7; i_phipsi++)
      //        {
      //          KEY_Mass2 = Form("eta_%d_pt_%d_Centrality_%d_CosThetaStar_%d_PhiPsi_%d_%s_Dca_%d_Sig_%d_%s_%s",i_eta,i_pt,i_cent,i_theta,i_phipsi,EP_string.c_str(),i_dca,i_sig,vmsa::mPID[mode].c_str(),Mode[X_flag].Data());
      //          h_mMass2[KEY_Mass2] = new TH1F(KEY_Mass2.Data(),KEY_Mass2.Data(),100,vmsa::InvMass_low[mode],vmsa::InvMass_high[mode]);
      //          h_mMass2[KEY_Mass2]->Sumw2();
      //        }
      //      }
      ...
      //  }
      //}
      for(Int_t i_phi = 0; i_phi < 10; i_phi++) // cos2(phistar-phi) bin
      {
        for(Int_t i_dca = vmsa::Dca_start; i_dca < 1/*vmsa::Dca_stop*/; i_dca++)
        {
          for(Int_t i_sig = vmsa::nSigKaon_start; i_sig < 1/*vmsa::nSigKaon_stop*/; i_sig++)
          {
            if( i_dca != 0 && i_sig != 0) continue;
            TString Mode[2] = {"SE","ME"};
            TString KEY_Mass2 = Form("pt_%d_Centrality_%d_Cos2PhiStarPhi_%d_%s_Dca_%d_Sig_%d_%s_%s",i_pt,i_cent,i_phi,EP_string.c_str(),i_dca,i_sig,vmsa::mPID[mode].c_str(),Mode[X_flag].Data());
            h_mMass2[KEY_Mass2] = new TH1F(KEY_Mass2.Data(),KEY_Mass2.Data(),100,vmsa::InvMass_low[mode],vmsa::InvMass_high[mode]);
            h_mMass2[KEY_Mass2]->Sumw2();

            //for(Int_t i_eta = 0; i_eta < vmsa::eta_total; i_eta++)
            //{
            //  KEY_Mass2 = Form("eta_%d_pt_%d_Centrality_%d_Cos2PhiStarPhi_%d_%s_Dca_%d_Sig_%d_%s_%s",i_eta,i_pt,i_cent,i_phi,EP_string.c_str(),i_dca,i_sig,vmsa::mPID[mode].c_str(),Mode[X_flag].Data());
            //  h_mMass2[KEY_Mass2] = new TH1F(KEY_Mass2.Data(),KEY_Mass2.Data(),100,vmsa::InvMass_low[mode],vmsa::InvMass_high[mode]);
            //  h_mMass2[KEY_Mass2]->Sumw2();
            //}
          }
	...
	//  {
        //    if( i_dca != 0 && i_sig != 0) continue;
	//    TString Mode[2] = {"SE","ME"};
	//    TString KEY_Mass2 = Form("profile_pt_%d_Centrality_%d_Cos2PhiStarPhi_%s_Dca_%d_Sig_%d_%s_%s",i_pt,i_cent,EP_string.c_str(),i_dca,i_sig,vmsa::mPID[mode].c_str(),Mode[X_flag].Data());
	//    p_mMass2[KEY_Mass2] = new TProfile(KEY_Mass2.Data(),KEY_Mass2.Data(),100,vmsa::InvMass_low[mode],vmsa::InvMass_high[mode]);
	//    p_mMass2[KEY_Mass2]->Sumw2();

        //    for(Int_t i_eta = 0; i_eta < vmsa::eta_total; i_eta++)
        //    {
        //      KEY_Mass2 = Form("profile_eta_%d_pt_%d_Centrality_%d_Cos2PhiStarPhi_%s_Dca_%d_Sig_%d_%s_%s",i_eta,i_pt,i_cent,EP_string.c_str(),i_dca,i_sig,vmsa::mPID[mode].c_str(),Mode[X_flag].Data());
	//      p_mMass2[KEY_Mass2] = new TProfile(KEY_Mass2.Data(),KEY_Mass2.Data(),100,vmsa::InvMass_low[mode],vmsa::InvMass_high[mode]);
	//      p_mMass2[KEY_Mass2]->Sumw2();
        //    }
	//  }
...
//-------------------------------------------------------------
void StVecMesonHistoManger::FillSys(Float_t pt, Float_t eta, Int_t cent9, Float_t CosThetaStar, Float_t PhiPsi, Float_t Cos2PhiStarPhi, Int_t dcaSys, Int_t nSigSys, Float_t Res2, Float_t InvMass, Double_t reweight, Int_t X_flag, Int_t mode)
{
  TString Mode[2] = {"SE","ME"};
  int final_phi_pt_bin = -1;
  if(Res2 > 0.0)
  {
    for(Int_t i_pt = 5; i_pt <= 16; i_pt++) // pt_bin
    {
      if(pt >= vmsa::ptRawStart[i_pt] && pt < vmsa::ptRawStop[i_pt])
      {
        final_phi_pt_bin = data_constants::phi_final_pt_bins[i_pt];
        ...
        for(Int_t i_phi = 0; i_phi < 10; i_phi++) // cos2(phistar-phi) bin
        {
          if(Cos2PhiStarPhi >= (float(i_phi)-5.)/5. && Cos2PhiStarPhi < (float(i_phi+1)-5.)/5.)
          {
            // spin alignment
            TString KEY_Mass2 = Form("pt_%d_Centrality_%d_Cos2PhiStarPhi_%d_%s_Dca_%d_Sig_%d_%s_%s",i_pt,cent9,i_phi,EP_string.c_str(),dcaSys,nSigSys,vmsa::mPID[mode].c_str(),Mode[X_flag].Data());
            //h_mMass2[KEY_Mass2]->Fill(InvMass,reweight);
            if(cent9 >= vmsa::cent_low[0] && cent9 <= vmsa::cent_up[0]) // 20%-60%
            {
              TString KEY_Mass2Sys = Form("pt_%d_Centrality_9_Cos2PhiStarPhi_%d_%s_Dca_%d_Sig_%d_%s_%s",i_pt,i_phi,EP_string.c_str(),dcaSys,nSigSys,vmsa::mPID[mode].c_str(),Mode[X_flag].Data());
              if(X_flag == 0) h_mMass2[KEY_Mass2Sys]->Fill(InvMass,reweight);
              if(X_flag == 1) h_mMass2[KEY_Mass2Sys]->Fill(InvMass,reweight*data_constants::phi_norm[i_pt-5][i_phi]);
            }
            //for(Int_t i_eta = 0; i_eta < vmsa::eta_total; i_eta++)
            //{
            //  if(eta > vmsa::eta_raw[i_eta] && eta < vmsa::eta_raw[i_eta+1])
            //  {
            //    TString KEY_Mass2Sys = Form("eta_%d_pt_%d_Centrality_%d_Cos2PhiStarPhi_%d_%s_Dca_%d_Sig_%d_%s_%s",i_eta,i_pt,cent9,i_phi,EP_string.c_str(),dcaSys,nSigSys,vmsa::mPID[mode].c_str(),Mode[X_flag].Data());
            //    h_mMass2[KEY_Mass2Sys]->Fill(InvMass,reweight);
            //    if(cent9 >= vmsa::cent_low[0] && cent9 <= vmsa::cent_up[0]) // 20%-60%
            //    {
            //      TString KEY_Mass2Sys9 = Form("eta_%d_pt_%d_Centrality_9_Cos2PhiStarPhi_%d_%s_Dca_%d_Sig_%d_%s_%s",i_eta,i_pt,i_phi,EP_string.c_str(),dcaSys,nSigSys,vmsa::mPID[mode].c_str(),Mode[X_flag].Data());
        ...
        //{
          ...
          //{
            // spin alignment
            //TString KEY_Mass2 = Form("profile_pt_%d_Centrality_%d_Cos2PhiStarPhi_%s_Dca_%d_Sig_%d_%s_%s",i_pt,cent9,EP_string.c_str(),dcaSys,nSigSys,vmsa::mPID[mode].c_str(),Mode[X_flag].Data());
            //p_mMass2[KEY_Mass2]->Fill(InvMass,Cos2PhiStarPhi,reweight);
            //if(cent9 >= vmsa::cent_low[0] && cent9 <= vmsa::cent_up[0]) // 20%-60%
            //{
            //  TString KEY_Mass2Sys = Form("profile_pt_%d_Centrality_9_Cos2PhiStarPhi_%s_Dca_%d_Sig_%d_%s_%s",i_pt,EP_string.c_str(),dcaSys,nSigSys,vmsa::mPID[mode].c_str(),Mode[X_flag].Data());
            //  p_mMass2[KEY_Mass2Sys]->Fill(InvMass,Cos2PhiStarPhi,reweight);
            //}
            //for(Int_t i_eta = 0; i_eta < vmsa::eta_total; i_eta++)
            //{
            //  if(eta > vmsa::eta_raw[i_eta] && eta < vmsa::eta_raw[i_eta+1])
            //  {
            //    TString KEY_Mass2Sys = Form("profile_eta_%d_pt_%d_Centrality_%d_Cos2PhiStarPhi_%s_Dca_%d_Sig_%d_%s_%s",i_eta,i_pt,cent9,EP_string.c_str(),dcaSys,nSigSys,vmsa::mPID[mode].c_str(),Mode[X_flag].Data());
            //    p_mMass2[KEY_Mass2Sys]->Fill(InvMass,Cos2PhiStarPhi,reweight);
            //    if(cent9 >= vmsa::cent_low[0] && cent9 <= vmsa::cent_up[0]) // 20%-60%
            //    {
            //      TString KEY_Mass2Sys9 = Form("profile_eta_%d_pt_%d_Centrality_9_Cos2PhiStarPhi_%s_Dca_%d_Sig_%d_%s_%s",i_eta,i_pt,EP_string.c_str(),dcaSys,nSigSys,vmsa::mPID[mode].c_str(),Mode[X_flag].Data());
            //      p_mMass2[KEY_Mass2Sys9]->Fill(InvMass,Cos2PhiStarPhi,reweight);
            //    }
            //  }
            //}  
          //}
	...
	//  if(TMath::Abs(CosThetaStar) >= vmsa::CTS_low[i_theta] && TMath::Abs(CosThetaStar) < vmsa::CTS_up[i_theta])
        ...
        //    for(int i_phipsi = 0; i_phipsi < 7; i_phipsi++)
        //    {
	//      //cout << (float(i_phipsi)*2.-7.)*TMath::Pi()/14. << " to " << ((float(i_phipsi)+1.)*2.-7.)*TMath::Pi()/14. << endl;
	//      if(PhiPsi >= (float(i_phipsi)*2.-7.)*TMath::Pi()/14. && PhiPsi < ((float(i_phipsi)+1.)*2.-7.)*TMath::Pi()/14.)
        //      {
        //        TString KEY_Mass2 = Form("pt_%d_Centrality_%d_CosThetaStar_%d_PhiPsi_%d_%s_Dca_%d_Sig_%d_%s_%s",i_pt,cent9,i_theta,i_phipsi,EP_string.c_str(),dcaSys,nSigSys,vmsa::mPID[mode].c_str(),Mode[X_flag].Data());
        //        h_mMass2[KEY_Mass2]->Fill(InvMass,reweight);
        //        if(cent9 >= vmsa::cent_low[0] && cent9 <= vmsa::cent_up[0]) // 20%-60%
        //        {
        //          TString KEY_Mass2Sys = Form("pt_%d_Centrality_9_CosThetaStar_%d_PhiPsi_%d_%s_Dca_%d_Sig_%d_%s_%s",i_pt,i_theta,i_phipsi,EP_string.c_str(),dcaSys,nSigSys,vmsa::mPID[mode].c_str(),Mode[X_flag].Data());
        //          h_mMass2[KEY_Mass2Sys]->Fill(InvMass,reweight);
        //        }
        //      }
        //    }
	...
	//          if(PhiPsi >= (float(i_phipsi)*2.-7.)*TMath::Pi()/14. && PhiPsi < ((float(i_phipsi)+1.)*2.-7.)*TMath::Pi()/14.)
        //          {
        //            TString KEY_Mass2Sys = Form("eta_%d_pt_%d_Centrality_%d_CosThetaStar_%d_PhiPsi_%d_%s_Dca_%d_Sig_%d_%s_%s",i_eta,i_pt,cent9,i_theta,i_phipsi,EP_string.c_str(),dcaSys,nSigSys,vmsa::mPID[mode].c_str(),Mode[X_flag].Data());
        //            h_mMass2[KEY_Mass2Sys]->Fill(InvMass,reweight);
        //            if(cent9 >= vmsa::cent_low[0] && cent9 <= vmsa::cent_up[0]) // 20%-60%
        //            {
        //              TString KEY_Mass2Sys9 = Form("eta_%d_pt_%d_Centrality_9_CosThetaStar_%d_PhiPsi_%d_%s_Dca_%d_Sig_%d_%s_%s",i_eta,i_pt,i_theta,i_phipsi,EP_string.c_str(),dcaSys,nSigSys,vmsa::mPID[mode].c_str(),Mode[X_flag].Data());
        //              h_mMass2[KEY_Mass2Sys9]->Fill(InvMass,reweight);
        //            }    
        //          }
        //        }
