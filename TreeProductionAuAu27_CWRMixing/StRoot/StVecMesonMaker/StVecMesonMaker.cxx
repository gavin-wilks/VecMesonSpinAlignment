...
Int_t StVecMesonMaker::Make() 
{
  ...
  //bool isMinBias = true;
  //if(mEnergy == 5 && vmsa::mBeamYear[mEnergy] == mPicoEvent->year() && !( mPicoEvent->isTrigger(610001) || mPicoEvent->isTrigger(610011) || mPicoEvent->isTrigger(610021) || mPicoEvent->isTrigger(610031) || mPicoEvent->isTrigger(610041) || mPicoEvent->isTrigger(610051) )) isMinBias = false; // 19.6GeV_2019
  //cout << "RunId : " << runId << endl;
  //cout << "IsMinBias? : " << isMinBias << endl;
...
//  cout << mRunIdEventsDb->getTotalNrRunIds() << endl;
  ...
  {
    //cout << "We have passed the event cut" << endl;
    int vz_sign = 0;
    ...
    if(mMode == 3)
    { // phi meson
      //cout << "We are in mMode = 3" << endl;
      if(mVecMesonCorrection->passTrackFullNumCut())
      {
        //cout << "We passed passTrackFullNumCut()" << endl;
	...
	mVecMesonTree->passNumTrack(NumTrackEast,NumTrackWest,NumTrackFull,NumTrackFullEast,NumTrackFullWest);
        if(mFlag_PID == 0)
        {
          //cout << "We are about to Mix the event " << endl;
