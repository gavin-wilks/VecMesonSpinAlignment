...
void plotMcPhiEff_Rapidity(int energy = 4, int pid = 0, int etamode = 0, int order = 1)
{
  gStyle->SetOptDate(0);
  ...
  string outputname = Form("figures/AnalysisNote/Efficiency/c_effAuAu%s_com_%s_rapidity_etamode%d_order%d.pdf",vmsa::mBeamEnergy[energy].c_str(),vmsa::mPID[pid].c_str(),etamode,order);
  ...
  for(int i_cent = 0; i_cent < vmsa::cent_rebin_total; i_cent++)
  {
    for(int i_y = 2; i_y < 12; i_y++)
    {
      ...
      for(int i_pt = 0/*vmsa::pt_rebin_first_y[energy]*/; i_pt <=1/* vmsa::pt_rebin_last_y[energy]*/; ++i_pt)
      {
        h_mEff[i_cent][i_pt][i_y]->SetMarkerStyle(vmsa::Style[i_pt]);
        h_mEff[i_cent][i_pt][i_y]->SetMarkerColor(vmsa::Color[i_pt]);
        h_mEff[i_cent][i_pt][i_y]->SetMarkerSize(1.4);
        //h_mEff[i_cent][i_pt][i_y]->SetTitle(Form("Cent %d Rapidity %d",i_cent,i_y));
        h_mEff[i_cent][i_pt][i_y]->Draw("pEX0 same");
