...
void plotQA_rhoSig_2nd(int energy = 6, int ptBin = 3)
{
  ...
  */
  ...
  plotTopLegend((char*)"2^{nd} EP",0.3,0.68,0.07,1,0.0,42,1,1);
  plotTopLegend((char*)beamenergy.c_str(),0.2,0.78,0.07,1,0.0,42,1,1);
  // TLegend *leg_energy = new TLegend(0.12,0.75,0.7,0.85);
  // leg_energy->SetBorderSize(0);
  // leg_energy->SetFillColor(10);
  // leg_energy->AddEntry((TObject*)0,beamenergy.c_str(),"");
  ...
  string FigName = Form("/Users/xusun/WorkSpace/STAR/figures/SpinAlignment/AnalysisNote/RhoSig/c_rhoSig_2nd_AuAu%s_Pt%d.eps",vmsa::mBeamEnergy[energy].c_str(),ptBin);
