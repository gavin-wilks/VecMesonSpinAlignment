...
void plotRho00Pt_single()
{
  ...
  for(int i_row = 0; i_row < 3; ++i_row)
  {
    ...
    h_frame_1st[i_row] = new TH1F(HistName.c_str(),HistName.c_str(),1000,-0.5,6);
  ...
  for(int i_row = 0; i_row < 3; i_row++)
  {
    ...
    h_frame_1st[i_row]->GetXaxis()->SetTitle("p_{T} (GeV/c)");
    ...
    h_frame_1st[i_row]->GetYaxis()->SetRangeUser(rho00_low[i_row*2],rho00_high[i_row*2]);
    ...
    Draw_TGAE_new_Symbol((TGraphAsymmErrors*)g_rho_1st_stat[i_row*2],plot_style_1st,plot_color_1st,plot_size_1st);
    plotSysErrors(g_rho_1st_sys[i_row*2],plot_color_1st);
    Draw_TGAE_new_Symbol((TGraphAsymmErrors*)g_rho_2nd_stat[i_row*2],plot_style_2nd,plot_color_2nd,plot_size_2nd);
    plotSysErrors(g_rho_2nd_sys[i_row*2],plot_color_2nd);
    plotTopLegend((char*)mBeanEnergy[i_row*2].c_str(),3.6,0.28,font_size*scaling_factor,1,0.0,42,0);
  ...
  c_rho00_1st->SaveAs("/Users/xusun/WorkSpace/STAR/figures/SpinAlignment/PaperDraft/c_rhoSys_pt_1st.eps");
