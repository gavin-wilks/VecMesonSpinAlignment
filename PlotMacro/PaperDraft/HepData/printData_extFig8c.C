...
void printData_extFig8c()
{
  string inputfile = "/Users/xusun/WorkSpace/STAR/Data/SpinAlignment/PaperDraft/Nature/Phi/rhoCent_62GeV_LXaxis.root";
  TFile *File_InPut = TFile::Open(inputfile.c_str());
  TGraphAsymmErrors *g_rhoPhiCent_1st_stat = (TGraphAsymmErrors*)File_InPut->Get("g_rhoCent_62GeV_1st_stat");
  TGraphAsymmErrors *g_rhoPhiCent_1st_sys  = (TGraphAsymmErrors*)File_InPut->Get("g_rhoCent_62GeV_1st_sys");
  TGraphAsymmErrors *g_rhoPhiCent_2nd_stat = (TGraphAsymmErrors*)File_InPut->Get("g_rhoCent_62GeV_2nd_stat");
  TGraphAsymmErrors *g_rhoPhiCent_2nd_sys  = (TGraphAsymmErrors*)File_InPut->Get("g_rhoCent_62GeV_2nd_sys");

  const int centStart[9] = {0,5,10,20,30,40,50,60,70};
  const int centStop[9]  = {5,10,20,30,40,50,60,70,80};

  string outputfile = "/Users/xusun/WorkSpace/STAR/Data/SpinAlignment/PaperDraft/Nature/HepData/dataExtFig8c.txt";
  ...
  // 2nd
  for(int iCent = g_rhoPhiCent_2nd_stat->GetN()-1; iCent > -1; --iCent)
  {
    double centrality, rho;
    ...
    if(errStat > 0)
    {
      cout << centStart[8-iCent] << '\t' << centStop[8-iCent] << '\t' << rho << '\t' << errStat << '\t' << errSyst << endl;
      file_output << centStart[8-iCent] << '\t' << centStop[8-iCent] << '\t' << rho << '\t' << errStat << '\t' << errSyst << endl;
    }
  }
  file_output << "***" << endl;
