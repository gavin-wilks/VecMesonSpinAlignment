...
void plotSysCent_Normalization(int energy = 0, int order = 1, string correction = "AccRes", string etamode = "eta1_eta1")
{
  ...
  for(int i_dca = 0; i_dca < 3; ++i_dca)
  {
    ...
    for(int i_mode = 0; i_mode < 4; ++i_mode)
    {
      ...
      string HistName = Form("rhoRaw_%s_Dca_0_Sig_0_Phi_Norm_%d_%s_Poly1",EP[order-1].c_str(),i_dca,mMode[i_mode].c_str());
  ...
  string FigName = Form("c_SysCentNormalization_AuAu%dGeV_order%d.pdf",mEnergy[energy],order);
