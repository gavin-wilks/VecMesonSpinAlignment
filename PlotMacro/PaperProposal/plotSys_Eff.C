...
void plotSys_Eff(int energy = 2)
{
  ...
  TH1F *h_rhoSys_Eff[3][4]; // 0 for different Eff | 1 for different yields extraction
  TGraphAsymmErrors *g_rhoSys_Eff[3][4];
  for(int i_eff = 0; i_eff < 2; ++i_eff)
  {
    if(i_eff == eff_default) continue;
    for(int i_mode = 0; i_mode < 4; ++i_mode)
    {
      string HistName = Form("EP_2_eff_%d_Dca_0_Sig_1_Phi_Norm_0_%s",i_eff,mMode[i_mode].c_str());
	...
	g_rhoSys_Eff[i_eff][i_mode]->SetPointError(i_point,0.0,0.0,err,err);
      ...
      g_rhoSys_Eff[i_eff][i_mode]->RemovePoint(0);
    }
  }
  ...
  string title_cuts = Form("Efficiency Systematics @ AuAu %s",mBeamEnergy[energy].c_str());
  ...
  for(int i_eff = 0; i_eff < 2; ++i_eff)
  {
    ...
    for(int i_mode = 0; i_mode < 4; ++i_mode)
    {
      // h_rhoSys_Eff[i_eff][i_mode]->Draw("pE same");
  ...
  leg_eff->AddEntry(g_rho_default,"default (ToF Matching 1)","P");
  for(int i_eff = 0; i_eff < 2; ++i_eff)
  {
    if(i_eff == eff_default) continue;
    leg_eff->AddEntry(g_rhoSys_Eff[i_eff][0],mLeg_eff[i_eff].c_str(),"P");
  }
  leg_eff->AddEntry((TObject*)0," ","");
  leg_eff->Draw("same");
  ...
  for(int i_mode = 0; i_mode< 4; ++i_mode)
  {
    leg_mode->AddEntry(h_rhoSys_Eff[1][i_mode],mLeg_mode[i_mode].c_str(),"P");
  }
  leg_mode->Draw("same");

  string FigName = Form("/Users/xusun/WorkSpace/STAR/figures/SpinAlignment/PaperProposal/NewF_JHChen/c_SysEff_AuAu%dGeV.eps",mEnergy[energy]);
