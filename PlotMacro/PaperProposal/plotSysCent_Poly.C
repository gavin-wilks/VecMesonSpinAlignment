...
void plotSysCent_Poly(int energy = 1, int order = 2, string correction = "AccRes", string etamode = "eta1_eta1")
{
  ...
  const float pt_shift[3] = {1.25,2.5,3.75};
  ...
  for(int i_dca = 0; i_dca < 2; ++i_dca)
  {
    ...
    for(int i_mode = 0; i_mode < 4; ++i_mode)
    {
      ...
      string HistName = Form("rhoRaw_%s_Dca_0_Sig_0_Phi_Norm_0_%s_Poly%d",EP[order-1].c_str(),mMode[i_mode].c_str(),i_dca+1);
  ...
  string FigName = Form("c_SysCentPoly_AuAu%dGeV_order%d.pdf",mEnergy[energy],order);
