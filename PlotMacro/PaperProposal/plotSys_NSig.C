...
void plotSys_NSig(int energy = 1, int order = 2, string correction = "AccRes", string etamode = "eta1_eta1")
{
  ...
  for(int i_dca = 0; i_dca < 3; ++i_dca)
  {
    ...
    for(int i_mode = 0; i_mode < 4; ++i_mode)
    {
      ...
      string HistName = Form("rhoRaw_Centrality_9_%s_Dca_0_Sig_%d_Phi_Norm_0_%s_Poly1_F_0_Eff_0",EP[order-1].c_str(),i_dca,mMode[i_mode].c_str());
  ...
  string FigName = Form("c_SysPtNSig_AuAu%dGeV_order%d.pdf",mEnergy[energy],order);
