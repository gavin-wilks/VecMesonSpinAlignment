...
void plotExtFig6_rho00YPhi()
{
  ...
  TFile *File_Input = TFile::Open("/Users/gavinwilks/Workspace/VectorMesonSpinAlignment/VecMesonSpinAlignment/output/RapidityDependentPaper/rho00y_all_Poly.root");
  for(int i_energy = 0; i_energy < 2; ++i_energy)
  {
    ...
    for(int i_pt = 0; i_pt < g_rho_1st_stat_temp[i_energy]->GetN(); ++i_pt) // shift pT of 1st EP results by 0.03
    {
      ...
      g_rho_1st_stat[i_energy]->SetPoint(i_pt,pt+0.03,rho);
    ...
    for(int i_pt = 0; i_pt < g_rho_1st_sys_temp[i_energy]->GetN(); ++i_pt) // shift pT of 1st EP results by 0.03
    {
      ...
      g_rho_1st_sys[i_energy]->SetPoint(i_pt,pt+0.03,rho);
