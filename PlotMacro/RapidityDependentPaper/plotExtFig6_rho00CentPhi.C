...
void plotExtFig6_rho00CentPhi()
{
  ...
  TFile *File_Input = TFile::Open("/Users/gavinwilks/Workspace/VectorMesonSpinAlignment/VecMesonSpinAlignment/output/RapidityDependentPaper/rho00cent_all_Poly.root");
  for(int i_energy = 0; i_energy < 2; ++i_energy)
  {
    ...
    for(int i_pt = 0; i_pt < g_rho_1st_stat_temp[i_energy]->GetN(); ++i_pt) // shift pT of 1st EP results by 0.03
    {
      ...
      g_rho_1st_stat[i_energy]->SetPoint(i_pt,pt+1.5,rho);
    ...
    for(int i_pt = 0; i_pt < g_rho_1st_sys_temp[i_energy]->GetN(); ++i_pt) // shift pT of 1st EP results by 0.03
    {
      ...
      g_rho_1st_sys[i_energy]->SetPoint(i_pt,pt+1.5,rho);
  ...
  for(int i_pad = 0; i_pad < N_total_pads; i_pad++)
  {
    ...
    h_frame[i_pad] = new TH1F(HistName.c_str(),HistName.c_str(),1000,-10,90);
    for(int bin_x = 1; bin_x < h_frame[i_pad]->GetNbinsX(); bin_x++)
    {
      h_frame[i_pad]->SetBinContent(bin_x,-10.0);
    }
    h_frame[i_pad]->GetXaxis()->SetRangeUser(-2.5,82.5);
    h_frame[i_pad]->GetYaxis()->SetRangeUser(0.22,0.41);
    h_frame[i_pad]->SetStats(0);
    h_frame[i_pad]->SetTitle("");
    h_frame[i_pad]->GetXaxis()->SetNdivisions(405,'N');
    h_frame[i_pad]->GetYaxis()->SetNdivisions(502,'N');
  ...
  for(int y_pads = 0; y_pads < N_y_pads; y_pads++)
  {
	...
	c_rho00_double->cd(total_pad)->SetTopMargin(Margin_t/(yUp_array[y_pads]-yDown_array[y_pads]));
      ...
      if(x_pads == 0 && y_pads != N_y_pads-1)
      {
	plotTopLegend((char*)mBeanEnergy[total_pad-1].c_str(),5.0,0.385,font_size*scaling_factor,1,0.0,42,0);
      }
      if(x_pads == 0 && y_pads == N_y_pads-1)
      {
	plotTopLegend((char*)mBeanEnergy[total_pad-1].c_str(),5.0,0.385,font_size*scaling_factor,1,0.0,42,0);
	//h_frame[total_pad-1]->SetTickLength(0.03);
      ...
      {
	plotTopLegend((char*)"Au+Au",5.0,0.29,leg_size*scaling_factor,1,0.0,42,0);
	plotTopLegend((char*)"|y| < 1.0 & 1.0 < p_{T} < 5.0 GeV/c",5.0,0.2775,leg_size*scaling_factor,1,0.0,42,0);
	...
	Draw_TGAE_Point_new_Symbol(7.5,0.26,0.0,0.0,0.0,0.0,style_phi_1st,color_phi_1st,size_marker-0.2);
	plotTopLegend((char*)"#phi (1^{st}-order EP)",11,0.2575,leg_size*scaling_factor,1,0.0,42,0);

	Draw_TGAE_Point_new_Symbol(7.5,0.24,0.0,0.0,0.0,0.0,style_phi_2nd,color_phi_2nd,size_marker+0.2);
	plotTopLegend((char*)"#phi (2^{nd}-order EP)",11,0.2375,leg_size*scaling_factor,1,0.0,42,0);
      }
      ...
      if(x_pads == 0 && y_pads == N_y_pads-1) plotTopLegend((char*)"Centrality (%)",0.43,0.05,0.075,1,0.0,42,1);
      //if(x_pads == 1 && y_pads == N_y_pads-1) plotTopLegend((char*)"(GeV/c)",0.0,0.09,0.09,1,0.0,42,1);
    }
  }

  c_rho00_double->SaveAs("figures/c_rhoSys_cent_BESII.pdf");
  c_rho00_double->SaveAs("figures/c_rhoSys_cent_BESII.png");
}
...
void plotSysErrorsBox(TGraphAsymmErrors *g_rho, int plot_color)
{
  ...
  for(int i_pt = 0; i_pt < g_rho->GetN(); ++i_pt) // plot sys errors
  {
    ...
    bSys[i_pt] = new TBox(pt-2.5,rho-err,pt+2.5,rho+err);
