...
void calTpcTrackEfficiency_Mother(int energy = 4, int mpid = 0, int pid = 0, int year = 0, int EP = 0)
{
  ...
  string inputfile = Form("data/%s/%s_WiderEta.root",vmsa::mBeamEnergy[energy].c_str(),vmsa::mPID[mpid].c_str());
  ...
  string outputfile = Form("/gpfs01/star/pwg/gwilks3/VectorMesonSpinAlignment/Data/%s/Efficiency/TPC/Eff_%s_%s.root",vmsa::mPID[mpid].c_str(),vmsa::mPID[mpid].c_str(),vmsa::mBeamEnergy[energy].c_str());
