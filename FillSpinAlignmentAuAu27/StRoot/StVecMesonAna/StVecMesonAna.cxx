...
void StVecMesonAna::MakePhi()
{
  ...
  for(int i_event = 0; i_event < numOfEvents; ++i_event)
  {
    ...
    GRefMult         = mMeson_event->getGRefMult();
    ...
    if(mEnergy != 6 ) mRefMultCorr->initEvent(RefMult,PrimaryVertex.z(),ZDCx);
    if(mEnergy == 6 ) mRefMultCorr->initEvent(GRefMult,PrimaryVertex.z(),ZDCx);
    ...
    if(PrimaryVertex.z() > -70.0 && PrimaryVertex.z() <= 0.0) vz_sign = 0;
    //if(vz > -30.0 && vz <= 0.0  ) vz_sign = 1;
    //if(vz > 0.0   && vz <= +30.0) vz_sign = 2;
    if(PrimaryVertex.z() < +70.0 && PrimaryVertex.z() >  0.0) vz_sign = 1;
    // Centrality
    ...
    mVecMesonHistoManger->FillEventCount(cent9,runIndex);
      ...
      for(UShort_t nTracks = 0; nTracks < NumTrackUsed; nTracks++) // loop over all tracks of the actual event
      {
          ...
          )
        {
          ...
          for(Int_t i_dca = vmsa::Dca_start; i_dca < 1/* vmsa::Dca_stop*/; i_dca++) // systematic loop for dca
          {
        	...
        	//if(flagB <= 1 && mVecMesonCut->passTrackEP(lTrackB,dcaB) && mVecMesonCut->passTrackEtaWest(lTrackB)) // trackB
        	{
                  if(NumTrackWest <= 1) continue;
        	...
        	//if(flagB <= 1 && mVecMesonCut->passTrackEP(lTrackB,dcaB) && mVecMesonCut->passTrackEtaEast(lTrackB)) // trackB
        	{
                  if(NumTrackEast <= 1) continue;
        	  Float_t  w = mVecMesonCorr->getWeight(lTrackB);
        	  TVector2 q2VectorB = mVecMesonCorr->calq2Vector(lTrackB);
