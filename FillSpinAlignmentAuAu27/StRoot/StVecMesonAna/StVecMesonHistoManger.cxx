...
void StVecMesonHistoManger::InitSys(Int_t X_flag, Int_t mode) // 0 for Same Event, 1 for Mixed Event
{
  ...
  { 
    string CountName = Form("EventCountPerRunId_Cent%d",icent);
    h_mEventCount[icent] = new TH1F(CountName.c_str(),CountName.c_str(),4000,-0.5,3999.5);
  }
...
//-------------------------------------------------------------
void StVecMesonHistoManger::FillEventCount(int cent9, int runidx)
{
  h_mEventCount[cent9]->Fill(runidx);
  if(cent9 >= vmsa::cent_low[0] && cent9 <= vmsa::cent_up[0]) // 20%-60%
  {
    h_mEventCount[9]->Fill(runidx);
  }
}
