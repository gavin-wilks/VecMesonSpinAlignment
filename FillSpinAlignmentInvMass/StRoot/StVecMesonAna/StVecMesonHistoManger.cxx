...
void StVecMesonHistoManger::InitSys(Int_t X_flag, Int_t mode) // 0 for Same Event, 1 for Mixed Event
{
    ...
    {	
      for(Int_t i_dca = vmsa::Dca_start; i_dca < vmsa::Dca_stop; i_dca++)
      {
        for(Int_t i_sig = vmsa::nSigKaon_start; i_sig < vmsa::nSigKaon_stop; i_sig++)
        {
          ...
          TString KEY_Mass2 = Form("pt_%d_Centrality_%d_%s_Dca_%d_Sig_%d_%s_%s",i_pt,i_cent,EP_string.c_str(),i_dca,i_sig,vmsa::mPID[mode].c_str(),Mode[X_flag].Data());
          cout << KEY_Mass2.Data() << endl;
          p_mMass2[KEY_Mass2] = new TProfile(KEY_Mass2.Data(),KEY_Mass2.Data(),200,vmsa::InvMass_low[mode],vmsa::InvMass_high[mode]);
          p_mMass2[KEY_Mass2]->Sumw2();
      
          //for(Int_t i_eta = 0; i_eta < vmsa::eta_total; i_eta++)
          //{
          //  KEY_Mass2 = Form("eta_%d_pt_%d_Centrality_%d_%s_Dca_%d_Sig_%d_%s_%s",i_eta,i_pt,i_cent,EP_string.c_str(),i_dca,i_sig,vmsa::mPID[mode].c_str(),Mode[X_flag].Data());
          //  p_mMass2[KEY_Mass2] = new TH1F(KEY_Mass2.Data(),KEY_Mass2.Data(),200,vmsa::InvMass_low[mode],vmsa::InvMass_high[mode]);
          //  p_mMass2[KEY_Mass2]->Sumw2()
	  //}
        }
      }
  ...
  for(Int_t i_pt = 0; i_pt < vmsa::pt_total; i_pt++) // pt bin
  {
    for(Int_t i_cent = 0; i_cent < vmsa::cent_rebin_total; i_cent++) // centrality bin
    {
      for(Int_t i_dca = vmsa::Dca_start; i_dca < vmsa::Dca_stop; i_dca++)
      {
	for(Int_t i_sig = vmsa::nSigKaon_start; i_sig < vmsa::nSigKaon_stop; i_sig++)
	{
          if( i_dca != 0 && i_sig != 0) continue;
	  TString Mode[2] = {"SE","ME"};
          for(Int_t i_eta = 0; i_eta < 20; i_eta++) // we will define 20 rapidity bins from |y| = [0,1]
          {
            TString KEY_Mass2_Spec = Form("Spec_eta_%d_pt_%d_Centrality_%d_Dca_%d_Sig_%d_%s_%s",i_eta,i_pt,i_cent,i_dca,i_sig,vmsa::mPID[mode].c_str(),Mode[X_flag].Data()); 
    ...
    for(Int_t i_pt = 0; i_pt < vmsa::pt_total; i_pt++) // pt_bin
    {
      if(pt >= vmsa::ptRawStart[i_pt] && pt < vmsa::ptRawStop[i_pt])
      {
        ...
        //{
          ...
          //{
            // spin alignment
            TString KEY_Mass2 = Form("pt_%d_Centrality_%d_%s_Dca_%d_Sig_%d_%s_%s",i_pt,cent9,EP_string.c_str(),dcaSys,nSigSys,vmsa::mPID[mode].c_str(),Mode[X_flag].Data());
            p_mMass2[KEY_Mass2]->Fill(InvMass,TMath::Power(CosThetaStar,2.),reweight);
            if(cent9 >= vmsa::cent_low[0] && cent9 <= vmsa::cent_up[0]) // 20%-60%
            {
              TString KEY_Mass2Sys = Form("pt_%d_Centrality_9_%s_Dca_%d_Sig_%d_%s_%s",i_pt,EP_string.c_str(),dcaSys,nSigSys,vmsa::mPID[mode].c_str(),Mode[X_flag].Data());
              p_mMass2[KEY_Mass2Sys]->Fill(InvMass,TMath::Power(CosThetaStar,2.),reweight);
            }
        ...
        // raw pt spectra
        for(Int_t i_eta = 0; i_eta < 20; i_eta++)
        {
          if(fabs(eta) >= float(i_eta)*1./20. && fabs(eta) < float(i_eta+1)*1./20.)
          {
            for(int i_cent = 0; i_cent < vmsa::cent_rebin_total; i_cent++)
            {
              if(cent9 < vmsa::cent_rebin[i_cent] || cent9 > vmsa::cent_rebin[i_cent+1]-1) continue;
              TString KEY_Mass2_Spec = Form("Spec_eta_%d_pt_%d_Centrality_%d_Dca_%d_Sig_%d_%s_%s",i_eta,i_pt,i_cent,dcaSys,nSigSys,vmsa::mPID[mode].c_str(),Mode[X_flag].Data());
