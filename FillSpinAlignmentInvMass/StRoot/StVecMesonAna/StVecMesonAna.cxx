...
void StVecMesonAna::MakePhi()
{
  ...
  for(int i_event = 0; i_event < numOfEvents; ++i_event)
  {
	...
	if(
	    ...
	    //( (m2A > 0.16 && m2A < 0.36) && (m2B > 0.16 && m2B < 0.36) && (TMath::Abs(eta_TrackA) <= 1.0 && TMath::Abs(eta_TrackB) <= 1.0) ) ||
            //( (TMath::Abs(eta_TrackA) > 1.0 && TMath::Abs(eta_TrackB) > 1.0 && TMath::Abs(eta_TrackA) <= 1.5 && TMath::Abs(eta_TrackB) <= 1.5 && nsA < 3.0 && nsB < 3.0) )
	    ((m2A > 0.16 && m2A < 0.36) && (m2B > 0.16 && m2B < 0.36)) //||
	    //((m2A > 0.16 && m2A < 0.36) && fabs(eta_TrackA) <= 1.0 && fabs(eta_TrackB) > 1.0) ||
	    //((m2B > 0.16 && m2B < 0.36) && fabs(eta_TrackB) <= 1.0 && fabs(eta_TrackA) > 1.0)// || 
	    //(fabs(eta_TrackA) > 1.0 && fabs(eta_TrackB) > 1.0) 
          )
	{
	  ...
	  for(Int_t i_dca = vmsa::Dca_start; i_dca < vmsa::Dca_stop; i_dca++) // systematic loop for dca
	  {
		...
		mVecMesonHistoManger->FillSys(pt_lTrack,rapidity_lTrack,cent9,CosThetaStar,i_dca,i_sig,Res2,InvMass_lTrack,reweight,mX_flag,mMode);
