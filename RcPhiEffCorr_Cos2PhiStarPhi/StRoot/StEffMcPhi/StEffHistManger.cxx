...
void StEffHistManger::InitHist()
{
  ...
  for(int i_cent = 0; i_cent < 10; ++i_cent)
  {
    ...
    for(int i_pt = mpt_first; i_pt <= mpt_last; ++i_pt) // use rebinned pt
    {
      ...
      h_mRcEffCos[i_cent][i_pt] = new TH1D(HistName.c_str(),HistName.c_str(),vmsa::BinCos*2.,-1.0,1.0);
      ...
      HistName = Form("h_mRcEffCosH_Cent_%d_Pt_%d",i_cent,i_pt);
      h_mRcEffCosH[i_cent][i_pt] = new TH1D(HistName.c_str(),HistName.c_str(),vmsa::BinCos*2.,-1.0,1.0);
      h_mRcEffCosH[i_cent][i_pt]->Sumw2();
      ...
      HistName = Form("h_mRcEffCosCosH_Cent_%d_Pt_%d",i_cent,i_pt);
      h_mRcEffCosCosH[i_cent][i_pt] = new TH2D(HistName.c_str(),HistName.c_str(),9/*vmsa::BinCos*2.*/,-1.0,1.0,9/*vmsa::BinCos*2.*/,-1.0,1.0);
      h_mRcEffCosCosH[i_cent][i_pt]->Sumw2();
...
void StEffHistManger::FillHistMc(int cent, float pt, float y, float phi, float cos, float Psi, float phistar, float phiprime, float kpt, float ky, float cosH)
{
  double  ratio_weight = 1.0;//h_mSpectraRatio->GetBinContent(h_mSpectraRatio->FindBin(pt,y));
...
void StEffHistManger::FillHistRc(int cent, float pt, float y, float phi, float cos, float Psi, float phistar, float phiprime, float kpt, float ky, float cosH)
{
  double  ratio_weight = 1.0;//h_mSpectraRatio->GetBinContent(h_mSpectraRatio->FindBin(pt,y));
    ...
    for(int i_cent = 0; i_cent < 10; ++i_cent)
    {
      ...
      if(flag_eff_Cos > 0.5)
      {
        ...
        for(int i_pt = mpt_first; i_pt <= mpt_last; ++i_pt)
        {
          ...
          h_mRcEffCosH[i_cent][i_pt]->Write();
            h_mEffCosH[i_cent][i_pt]->Write();
          h_mMcEffCosCosH[i_cent][i_pt]->Write();
          h_mRcEffCosCosH[i_cent][i_pt]->Write();
            h_mEffCosCosH[i_cent][i_pt]->Write();
          h_mMcEffPhiS[i_cent][i_pt]->Write();
          h_mRcEffPhiS[i_cent][i_pt]->Write();
            h_mEffPhiS[i_cent][i_pt]->Write();
          h_mMcEffCosEP[i_cent][i_pt]->Write();
