...
void StVecMesonTree::doPhi(Int_t Flag_ME, Int_t cent9, Int_t Bin_vz, Int_t Bin_Psi2) // 0: Same Event, 1: Mix Event
{
  if(Flag_ME == 0) // same event
  {
    for(Int_t Bin_Event = 0; Bin_Event < mEventCounter2[cent9][Bin_vz][Bin_Psi2]; Bin_Event++)
    {
      ...
      for(Int_t n_kplus1 = 0; n_kplus1 < mHelix[key_plus].size(); n_kplus1++) // first track loop over K+ candidates
      {
	StThreeVectorF p_vecA = mHelix[key_plus][n_kplus1].cat(mHelix[key_plus][n_kplus1].pathLength(mPrimaryvertex[cent9][Bin_vz][Bin_Psi2][Bin_Event]));  // primary momentum
        //cout << "p_vecA = (" << p_vecA.x() << ", " << p_vecA.y() << ", " << p_vecA.z() << ")" << endl;
	p_vecA *= mMomentum[key_plus][n_kplus1];
	ltrackA.SetXYZM(p_vecA.x(),p_vecA.y(),p_vecA.z(),vmsa::mMassKaon);

	for(Int_t n_kplus2 = n_kplus1 + 1; n_kplus2 < mHelix[key_plus].size(); n_kplus2++) // second track loop over K- candidates
	{
	  StThreeVectorF p_vecB = mHelix[key_plus][n_kplus2].cat(mHelix[key_plus][n_kplus2].pathLength(mPrimaryvertex[cent9][Bin_vz][Bin_Psi2][Bin_Event]));  // primary momentum
	  p_vecB *= mMomentum[key_plus][n_kplus2];
	  ...
	  if(InvMassAB > vmsa::mMassKaon*2 && InvMassAB < 1.05) 
	  {
	    ...
	    mMesonTrack->setMass2A(mMass2[key_plus][n_kplus1]); // K+
	    mMesonTrack->setMass2B(mMass2[key_plus][n_kplus2]); // K-
	    mMesonTrack->setNSigA(mNSigma[key_plus][n_kplus1]); // K+
	    mMesonTrack->setNSigB(mNSigma[key_plus][n_kplus2]); // K-
	    mMesonTrack->setDcaA(mDca[key_plus][n_kplus1]); // K+
	    mMesonTrack->setDcaB(mDca[key_plus][n_kplus2]); // K-
	    mMesonTrack->setNHitsFitA(mNHitsFit[key_plus][n_kplus1]); // K+
	    mMesonTrack->setNHitsFitB(mNHitsFit[key_plus][n_kplus2]); // K-
	    mMesonTrack->setNHitsMaxA(mNHitsMax[key_plus][n_kplus1]); // K+
	    mMesonTrack->setNHitsMaxB(mNHitsMax[key_plus][n_kplus2]); // K-
	    mMesonTrack->setDEdxA(mDEdx[key_plus][n_kplus1]); // K+
	    mMesonTrack->setDEdxB(mDEdx[key_plus][n_kplus2]); // K-
	    ...
	    mMesonTrack->setWeightA(mWeight[key_plus][n_kplus1]); // K+
	    mMesonTrack->setWeightB(mWeight[key_plus][n_kplus2]); // K-
	    h_Mass2->Fill(pt,InvMassAB);
	  }
	}
      }
      for(Int_t n_kminus1 = 0; n_kminus1 < mHelix[key_minus].size(); n_kminus1++) // first track loop over K+ candidates
      {
	StThreeVectorF p_vecA = mHelix[key_minus][n_kminus1].cat(mHelix[key_minus][n_kminus1].pathLength(mPrimaryvertex[cent9][Bin_vz][Bin_Psi2][Bin_Event]));  // primary momentum
        //cout << "p_vecA = (" << p_vecA.x() << ", " << p_vecA.y() << ", " << p_vecA.z() << ")" << endl;
	p_vecA *= mMomentum[key_minus][n_kminus1];
	ltrackA.SetXYZM(p_vecA.x(),p_vecA.y(),p_vecA.z(),vmsa::mMassKaon);

	for(Int_t n_kminus2 = n_kminus1 + 1; n_kminus2 < mHelix[key_minus].size(); n_kminus2++) // second track loop over K- candidates
	{
	  StThreeVectorF p_vecB = mHelix[key_minus][n_kminus2].cat(mHelix[key_minus][n_kminus2].pathLength(mPrimaryvertex[cent9][Bin_vz][Bin_Psi2][Bin_Event]));  // primary momentum
	  p_vecB *= mMomentum[key_minus][n_kminus2];
	  ltrackB.SetXYZM(p_vecB.x(),p_vecB.y(),p_vecB.z(),vmsa::mMassKaon);
	  TLorentzVector trackAB      = ltrackA+ltrackB;
	  Double_t InvMassAB          = trackAB.M();
	  Double_t pt = trackAB.Perp();
	  ...
	  {
	    mMesonTrack = mMesonEvent->createTrack();
	    mMesonTrack->setMass2A(mMass2[key_minus][n_kminus1]); // K+
	    mMesonTrack->setMass2B(mMass2[key_minus][n_kminus2]); // K-
	    mMesonTrack->setNSigA(mNSigma[key_minus][n_kminus1]); // K+
	    mMesonTrack->setNSigB(mNSigma[key_minus][n_kminus2]); // K-
	    mMesonTrack->setDcaA(mDca[key_minus][n_kminus1]); // K+
	    mMesonTrack->setDcaB(mDca[key_minus][n_kminus2]); // K-
	    mMesonTrack->setNHitsFitA(mNHitsFit[key_minus][n_kminus1]); // K+
	    mMesonTrack->setNHitsFitB(mNHitsFit[key_minus][n_kminus2]); // K-
	    mMesonTrack->setNHitsMaxA(mNHitsMax[key_minus][n_kminus1]); // K+
	    mMesonTrack->setNHitsMaxB(mNHitsMax[key_minus][n_kminus2]); // K-
	    mMesonTrack->setDEdxA(mDEdx[key_minus][n_kminus1]); // K+
	    mMesonTrack->setDEdxB(mDEdx[key_minus][n_kminus2]); // K-
	    mMesonTrack->setTrackA(ltrackA); // K+
	    mMesonTrack->setTrackB(ltrackB); // K-
	    mMesonTrack->setFlagA(Bin_Event); // K+
	    mMesonTrack->setFlagB(Bin_Event); // K-
	    mMesonTrack->setWeightA(mWeight[key_minus][n_kminus1]); // K+
	    mMesonTrack->setWeightB(mWeight[key_minus][n_kminus2]); // K-
	    h_Mass2->Fill(pt,InvMassAB);
	  }
	}
      }
