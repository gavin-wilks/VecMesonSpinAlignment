...
class StVecMesonTree
{
  public:
    ...
    void clearEvent();
    void passEvent(Int_t,Int_t,Int_t,Float_t); // N_prim,N_non_prim,N_Tof_match,mReweightFactor
  ...
  private:
    ...
    vectorFloatMap mNHitsMax;
    vectorFloatMap mDEdx;
    vectorFloatMap mNSigma;
    vectorIntMap mCharge;
    vectorFloatMap mWeight;
    ...
    std::vector<Float_t> mReweight[vmsa::Bin_Centrality][vmsa::Bin_VertexZ][vmsa::Bin_Phi_Psi];

    // passing variable
    Int_t mNumber_prim, mNumber_non_prim, mNumber_Tof_match;
    Float_t mReweightFactor;
