...
void subBackGround(int energy = 4, int pid = 0, int year = 0)
{
  ...
  string InPutFile_SE = "../data/FlowYields_Phi_SE_19GeV_20220411.root";
  TFile *File_SE = TFile::Open(InPutFile_SE.c_str());
  
  string InPutFile_ME = "../data/FlowYields_Phi_ME_19GeV_20220411.root";
  ...
  Float_t yields_total_gaus[4][vmsa::Dca_stop][vmsa::nSigKaon_stop][vmsa::Norm_stop] = {0.0};
  Float_t yields_total_bw[4][vmsa::Dca_stop][vmsa::nSigKaon_stop][vmsa::Norm_stop]   = {0.0};
    ...
    for(Int_t i_dca = vmsa::Dca_start; i_dca < vmsa::Dca_stop; i_dca++)
    {
      for(Int_t i_sig = vmsa::nSigKaon_start; i_sig < vmsa::nSigKaon_stop; i_sig++)
      {
        ...
        for(int i_norm = vmsa::Norm_start; i_norm < vmsa::Norm_stop; ++i_norm)
        {
          ...
          {
            yields_total_gaus[0][i_dca][i_sig][i_norm] += yields_Gaus[KEY_Yield][0];
            yields_total_bw[0][i_dca][i_sig][i_norm]   += yields_BW[KEY_Yield][0];
          }
          if(i_cent == 8 || i_cent == 7) // calculate resolution and total yields in 0-10% centrality bin
          {
            yields_total_gaus[1][i_dca][i_sig][i_norm] += yields_Gaus[KEY_Yield][0];
            yields_total_bw[1][i_dca][i_sig][i_norm]   += yields_BW[KEY_Yield][0];
          }
          if(i_cent <= 6 && i_cent >= 4) // calculate resolution and total yields in 10-40% centrality bin
          {
            yields_total_gaus[2][i_dca][i_sig][i_norm] += yields_Gaus[KEY_Yield][0];
            yields_total_bw[2][i_dca][i_sig][i_norm]   += yields_BW[KEY_Yield][0];
          }
          if(i_cent <= 3 && i_cent >= 0) // calculate resolution and total yields in 40-80% centrality bin
          {
            yields_total_gaus[3][i_dca][i_sig][i_norm] += yields_Gaus[KEY_Yield][0];
            yields_total_bw[3][i_dca][i_sig][i_norm]   += yields_BW[KEY_Yield][0];
          }
        }
      }
    }
  ...
  {   
    for(Int_t i_dca = vmsa::Dca_start; i_dca < vmsa::Dca_stop; i_dca++)
    {
      for(Int_t i_sig = vmsa::nSigKaon_start; i_sig < vmsa::nSigKaon_stop; i_sig++)
      {
        ...
        for(int i_norm = vmsa::Norm_start; i_norm < vmsa::Norm_stop; ++i_norm)
        {
          string KEY_WeightGaus = Form("ResWeight_Gauss_Centrality_%d_Dca_%d_Sig_%d_%s_Norm_%d_SM",i_cent+9,i_dca,i_sig,vmsa::mPID[pid].c_str(),i_norm);
          string KEY_WeightBW   = Form("ResWeight_BW_Centrality_%d_Dca_%d_Sig_%d_%s_Norm_%d_SM",i_cent+9,i_dca,i_sig,vmsa::mPID[pid].c_str(),i_norm);
    ...
    for(Int_t i_dca = vmsa::Dca_start; i_dca < vmsa::Dca_stop; i_dca++)
    {
      for(Int_t i_sig = vmsa::nSigKaon_start; i_sig < vmsa::nSigKaon_stop; i_sig++)
      {
        ...
        for(int i_norm = vmsa::Norm_start; i_norm < vmsa::Norm_stop; ++i_norm)
        {
          ...
          if(i_cent >= 0 && i_cent <= 8) // calculate resolution and total yields in 0-80% centrality bin
          {
            string KEY_WeightGaus = Form("ResWeight_Gauss_Centrality_9_Dca_%d_Sig_%d_%s_Norm_%d_SM",i_dca,i_sig,vmsa::mPID[pid].c_str(),i_norm);
            string KEY_WeightBW   = Form("ResWeight_BW_Centrality_9_Dca_%d_Sig_%d_%s_Norm_%d_SM",i_dca,i_sig,vmsa::mPID[pid].c_str(),i_norm);
            mean_res_gaus[KEY_WeightGaus][0] += yields_Gaus[KEY_Yield][0]/(mTpcSubRes2Val[i_cent]*yields_total_gaus[0][i_dca][i_sig][i_norm]);
            mean_res_bw[KEY_WeightBW][0]     += yields_BW[KEY_Yield][0]  /(mTpcSubRes2Val[i_cent]*yields_total_bw[0][i_dca][i_sig][i_norm]);
          }
          if(i_cent == 8 || i_cent == 7) // calculate resolution and total yields in 0-10% centrality bin
          {
            string KEY_WeightGaus = Form("ResWeight_Gauss_Centrality_10_Dca_%d_Sig_%d_%s_Norm_%d_SM",i_dca,i_sig,vmsa::mPID[pid].c_str(),i_norm);
            string KEY_WeightBW   = Form("ResWeight_BW_Centrality_10_Dca_%d_Sig_%d_%s_Norm_%d_SM",i_dca,i_sig,vmsa::mPID[pid].c_str(),i_norm);
            mean_res_gaus[KEY_WeightGaus][0] += yields_Gaus[KEY_Yield][0]/(mTpcSubRes2Val[i_cent]*yields_total_gaus[1][i_dca][i_sig][i_norm]);
            mean_res_bw[KEY_WeightBW][0]     += yields_BW[KEY_Yield][0]  /(mTpcSubRes2Val[i_cent]*yields_total_bw[1][i_dca][i_sig][i_norm]);
          }
          if(i_cent <= 6 && i_cent >= 4) // calculate resolution and total yields in 10-40% centrality bin
          {
            string KEY_WeightGaus = Form("ResWeight_Gauss_Centrality_11_Dca_%d_Sig_%d_%s_Norm_%d_SM",i_dca,i_sig,vmsa::mPID[pid].c_str(),i_norm);
            string KEY_WeightBW   = Form("ResWeight_BW_Centrality_11_Dca_%d_Sig_%d_%s_Norm_%d_SM",i_dca,i_sig,vmsa::mPID[pid].c_str(),i_norm);
            mean_res_gaus[KEY_WeightGaus][0] += yields_Gaus[KEY_Yield][0]/(mTpcSubRes2Val[i_cent]*yields_total_gaus[2][i_dca][i_sig][i_norm]);
            mean_res_bw[KEY_WeightBW][0]     += yields_BW[KEY_Yield][0]  /(mTpcSubRes2Val[i_cent]*yields_total_bw[2][i_dca][i_sig][i_norm]);
          }
          if(i_cent <= 3 && i_cent >= 0) // calculate resolution and total yields in 40-80% centrality bin
          {
            string KEY_WeightGaus = Form("ResWeight_Gauss_Centrality_12_Dca_%d_Sig_%d_%s_Norm_%d_SM",i_dca,i_sig,vmsa::mPID[pid].c_str(),i_norm);
            string KEY_WeightBW   = Form("ResWeight_BW_Centrality_12_Dca_%d_Sig_%d_%s_Norm_%d_SM",i_dca,i_sig,vmsa::mPID[pid].c_str(),i_norm);
            mean_res_gaus[KEY_WeightGaus][0] += yields_Gaus[KEY_Yield][0]/(mTpcSubRes2Val[i_cent]*yields_total_gaus[3][i_dca][i_sig][i_norm]);
            mean_res_bw[KEY_WeightBW][0]     += yields_BW[KEY_Yield][0]  /(mTpcSubRes2Val[i_cent]*yields_total_bw[3][i_dca][i_sig][i_norm]);
          }
        }
      }
    }
