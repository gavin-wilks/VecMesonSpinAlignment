...
void subBackGroundKStar(int energy = 4, int pid = 2, int year = 0)
{
  ...
  string InPutFile_SE = "../data/Yields_KStar_SE_19GeV_20220411.root";
  TFile *File_SE = TFile::Open(InPutFile_SE.c_str());
  
  string InPutFile_ME = "../data/Yields_KStar_ME_19GeV_20220411.root";
  ...
  string KEY_SE_QA = Form("pt_%d_Centrality_%d_PhiPsi_%d_2nd_Dca_%d_Sig_%d_NHit_%d_%s_Norm_%d_SE",vmsa::pt_RawQA[energy],vmsa::Cent_start,vmsa::PhiPsi_start,vmsa::Dca_start,vmsa::nSigKaon_start,0,vmsa::mPID[pid].c_str(),vmsa::Norm_QA);
  ...
  string KEY_ME_QA = Form("pt_%d_Centrality_%d_PhiPsi_%d_2nd_Dca_%d_Sig_%d_NHit_%d_%s_Norm_%d_ME",vmsa::pt_RawQA[energy],vmsa::Cent_start,vmsa::PhiPsi_start,vmsa::Dca_start,vmsa::nSigKaon_start,0,vmsa::mPID[pid].c_str(),vmsa::Norm_QA);
  ...
  string KEY_SM_QA = Form("pt_%d_Centrality_%d_PhiPsi_%d_2nd_Dca_%d_Sig_%d_NHit_%d_%s_Norm_%d_SM",vmsa::pt_RawQA[energy],vmsa::Cent_start,vmsa::PhiPsi_start,vmsa::Dca_start,vmsa::nSigKaon_start,0,vmsa::mPID[pid].c_str(),vmsa::Norm_QA);
  ...
  for(Int_t i_cent = 0; i_cent < 9; i_cent++) // Centrality loop
  {
	...
	for(Int_t i_nhit = vmsa::mNHit_start; i_nhit < vmsa::mNHit_stop; i_nhit++)
	{
          ...
          for(int i_norm = vmsa::Norm_start; i_norm < vmsa::Norm_stop; ++i_norm)
          {
            ...
            f_yields_bw->SetParLimits(0,vmsa::InvMass[pid]-1.5*vmsa::Width[pid],vmsa::InvMass[pid]+1.5*vmsa::Width[pid]);
