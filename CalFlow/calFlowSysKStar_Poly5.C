...
void calFlowSysKStar_Poly5(int energy = 4, int pid = 2, int year = 0, int ptQA = 5, int centQA = 6, int dcaQA = 0, int nsigQA = 0, int nhitQA = 0)
{
	...
	for(Int_t i_sig = vmsa::nSigKaon_start; i_sig < vmsa::nSigKaon_stop; i_sig++)
	{
	  ...
	  for(Int_t i_nhit = vmsa::mNHit_start; i_nhit < vmsa::mNHit_stop; i_nhit++)
	  {
	    ...
	    for(int i_norm = vmsa::Norm_start; i_norm < 1; ++i_norm)
	    {
	      ...
	      TF1 *f_bw = new TF1("f_bw",Poly5BreitWigner,vmsa::BW_Start[pid],vmsa::BW_Stop[pid],9);
...
*/
	    ...
	    for(int i_sigma = vmsa::Sig_start; i_sigma < vmsa::Sig_stop; ++i_sigma)
	    {
	      for(int i_method = vmsa::Method_start; i_method < vmsa::Method_stop; ++i_method)
	      {
	  	  ...
	  	  for(int i_theta = vmsa::PhiPsi_start; i_theta < vmsa::PhiPsi_stop; i_theta++) // cos(theta*) loop
	  	  {
	  	    ...
	  	    f_bw->SetParameter(7,Par_InteTheta[KEY_InteTheta][7]);
                    f_bw->SetParameter(8,Par_InteTheta[KEY_InteTheta][8]);
	  	    f_bw->SetRange(vmsa::BW_Start[pid],vmsa::BW_Stop[pid]);
	  	    TFitResultPtr result = h_mMass[KEY]->Fit(f_bw,"MQNRS");
	  	    float bin_center = (vmsa::PhiPsi_low[i_theta] + vmsa::PhiPsi_up[i_theta])/2.0;
                    
                    TF1 *f_bg = new TF1("f_bg",Poly5,vmsa::BW_Start[pid],vmsa::BW_Stop[pid],6);
                    ...
                    f_bg->SetParameter(4,f_bw->GetParameter(7));
                    f_bg->SetParameter(5,f_bw->GetParameter(8));
                    ...
                    f_bg->SetParError(4,f_bw->GetParError(7));
                    f_bg->SetParError(5,f_bw->GetParError(8));
                   
                    double params[6] = {result->GetParams()[3],result->GetParams()[4],result->GetParams()[5],result->GetParams()[6],result->GetParams()[7],result->GetParams()[8]};
                    TMatrixDSym covArr(6);
                    ...
                    covArr(1,4) = result->GetCovarianceMatrix()(4,7);
                    covArr(1,5) = result->GetCovarianceMatrix()(4,8);
                    ...
                    covArr(2,4) = result->GetCovarianceMatrix()(5,7);
                    covArr(2,5) = result->GetCovarianceMatrix()(5,8);
                    ...
                    covArr(3,4) = result->GetCovarianceMatrix()(6,7);
                    covArr(3,5) = result->GetCovarianceMatrix()(6,8);
                    covArr(4,0) = result->GetCovarianceMatrix()(7,3);
                    covArr(4,1) = result->GetCovarianceMatrix()(7,4);
                    covArr(4,2) = result->GetCovarianceMatrix()(7,5);
                    covArr(4,3) = result->GetCovarianceMatrix()(7,6);
                    covArr(4,4) = result->GetCovarianceMatrix()(7,7);
                    covArr(4,5) = result->GetCovarianceMatrix()(7,8);
                    covArr(5,0) = result->GetCovarianceMatrix()(8,3);
                    covArr(5,1) = result->GetCovarianceMatrix()(8,4);
                    covArr(5,2) = result->GetCovarianceMatrix()(8,5);
                    covArr(5,3) = result->GetCovarianceMatrix()(8,6);
                    covArr(5,4) = result->GetCovarianceMatrix()(8,7);
                    covArr(5,5) = result->GetCovarianceMatrix()(8,8);
  ...
  for(int i_cent = vmsa::Cent_start; i_cent < 13; ++i_cent) // Centrality loop
  {
    string outputname = Form("./figures/Phi/19GeV/allThetaYieldsPoly5_cent%d.pdf",i_cent);
    ...
    for(int i_pt = vmsa::pt_rebin_firstKSv2[energy]; i_pt < vmsa::pt_rebin_lastKSv2[energy]; ++i_pt) // pt loop
    {
      ...
      for(int i_theta = 0; i_theta < 12; i_theta++)
      {
        ...
        if(i_theta == vmsa::PhiPsi_stop)
        {
          ...
          f_bg->SetParameter(2,Par_InteTheta[KEY_InteTheta_QA][5]);
          f_bg->SetParameter(3,Par_InteTheta[KEY_InteTheta_QA][6]);
          f_bg->SetParameter(4,Par_InteTheta[KEY_InteTheta_QA][7]);
          f_bg->SetParameter(5,Par_InteTheta[KEY_InteTheta_QA][8]);
          f_bg->SetLineColor(4);
          f_bg->SetLineStyle(2);
          f_bg->SetLineWidth(2);
          f_bg->Draw("l same");
  ...
  string outputname = Form("./figures/Phi/19GeV/v2poly5.pdf");
  string output_start = Form("%s[",outputname.c_str());
 
  c_v2->Print(output_start.c_str());
  ...
  for(int i_cent = vmsa::Cent_start; i_cent < 13; ++i_cent) // Centrality loop
  {
    ...
    c_v2->Update();
    c_v2->Print(outputname.c_str());
  }

  string output_stop = Form("%s]",outputname.c_str());
  c_v2->Print(output_stop.c_str()); // close pdf file
