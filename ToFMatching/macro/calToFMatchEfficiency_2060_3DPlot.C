...
void calToFMatchEfficiency_2060_3DPlot(int energy = 4, int pid = 0, int EP = 5, int rebinint = 1)
{
  int nphibins = 240/rebinint;
  ...
  for(int i_pid = pid_start; i_pid < pid_stop; ++i_pid)
  {
	...
	h_mTracks_TPC[i_charge][i_pid][i_cent] = (TH3D*)File_InPut->Get(HistName_Trakcs_TPC.c_str());
        //h_mTracks_TPC[i_charge][i_pid][i_cent]->RebinY(2);
        h_mTracks_TPC[i_charge][i_pid][i_cent]->RebinZ(rebinint);
	...
	h_mTracks_ToF[i_charge][i_pid][i_cent] = (TH3D*)File_InPut->Get(HistName_Trakcs_ToF.c_str());
        //h_mTracks_ToF[i_charge][i_pid][i_cent]->RebinY(2);
        h_mTracks_ToF[i_charge][i_pid][i_cent]->RebinZ(rebinint);
      }
  ...
  TCanvas *c2 = new TCanvas("phieff","phieff",10,10,400,400);
  c2->cd(1)->SetLeftMargin(0.15);
  c2->cd(1)->SetBottomMargin(0.15);
  c2->cd(1)->SetGrid(0,0);
  c2->cd(1)->SetTicks(1,1);
  ...
  h_mEffPhi[9]->Draw("pE");
  c2->SaveAs(Form("phiToFMatchingeff%s_cent9.pdf",eventplane.c_str()));
  ...
  for(int i_pid = pid_start; i_pid < pid_stop; ++i_pid)
  {
    for(int i_charge = 0; i_charge < 2; ++i_charge)
    {
      for(int i_cent = 0; i_cent < 10; ++i_cent)
      {
        for(int i_eta = 0; i_eta < neta; ++i_eta)
        {
          for(int i_phi = 0; i_phi < nphibins; ++i_phi)
          {
  ...
  // calculate efficiencya vs. eta
  TH2D *h_mCounts_TPC_pteta[2][10]; // efficiency vs. cent & eta
  TH2D *h_mCounts_ToF_pteta[2][10];
  TH2D *h_mEfficiency_pteta[2][10];
  TH2D *h_mCounts_TPC_ptphi[2][10]; // efficiency vs. cent & eta
  TH2D *h_mCounts_ToF_ptphi[2][10];
  TH2D *h_mEfficiency_ptphi[2][10];
  TH2D *h_mCounts_TPC_etaphi[2][10]; // efficiency vs. cent & eta
  TH2D *h_mCounts_ToF_etaphi[2][10];
  TH2D *h_mEfficiency_etaphi[2][10];

  TH1D *h_mCounts_TPC_pt1D[2][10]; // efficiency vs. cent & eta
  TH1D *h_mCounts_ToF_pt1D[2][10];
  TH1D *h_mEfficiency_pt1D[2][10];
  TH1D *h_mCounts_TPC_eta1D[2][10]; // efficiency vs. cent & eta
  TH1D *h_mCounts_ToF_eta1D[2][10];
  TH1D *h_mEfficiency_eta1D[2][10];
  TH1D *h_mCounts_TPC_phi1D[2][10]; // efficiency vs. cent & eta
  TH1D *h_mCounts_ToF_phi1D[2][10];
  TH1D *h_mEfficiency_phi1D[2][10];
  ...
  for(int i_pid = pid_start; i_pid < pid_stop; ++i_pid)
  {
	...
	h_mCounts_TPC_pt1D[i_charge][i_cent] = (TH1D*)h_mTracks_TPC[i_charge][i_pid][i_cent]->ProjectionX(HistName_TPC.c_str(),0,-1,0,-1)->Clone();

	string HistName_ToF = Form("h_mCounts_ToF_%s%s_Cent_%d_pt",tof::mPID_ToF[i_pid].c_str(),tof::mCharge[i_charge].c_str(),i_cent);
	h_mCounts_ToF_pt1D[i_charge][i_cent] = (TH1D*)h_mTracks_ToF[i_charge][i_pid][i_cent]->ProjectionX(HistName_ToF.c_str(),0,-1,0,-1)->Clone();

	string HistName = Form("h_mEfficiency_%s%s_Cent_%d_pt",tof::mPID_ToF[i_pid].c_str(),tof::mCharge[i_charge].c_str(),i_cent);
	h_mEfficiency_pt1D[i_charge][i_cent] = (TH1D*)h_mCounts_ToF_pt1D[i_charge][i_cent]->Clone(HistName.c_str());
	h_mEfficiency_pt1D[i_charge][i_cent]->SetTitle(HistName.c_str());
	h_mEfficiency_pt1D[i_charge][i_cent]->Reset();
	h_mEfficiency_pt1D[i_charge][i_cent]->Divide(h_mCounts_ToF_pt1D[i_charge][i_cent],h_mCounts_TPC_pt1D[i_charge][i_cent],1,1,"B");

        //Eta
	HistName_TPC = Form("h_mCounts_TPC_%s%s_Cent_%d_eta",tof::mPID_ToF[i_pid].c_str(),tof::mCharge[i_charge].c_str(),i_cent);
	h_mCounts_TPC_eta1D[i_charge][i_cent] = (TH1D*)h_mTracks_TPC[i_charge][i_pid][i_cent]->ProjectionY(HistName_TPC.c_str(),0,-1,0,-1)->Clone();

	HistName_ToF = Form("h_mCounts_ToF_%s%s_Cent_%d_eta",tof::mPID_ToF[i_pid].c_str(),tof::mCharge[i_charge].c_str(),i_cent);
	h_mCounts_ToF_eta1D[i_charge][i_cent] = (TH1D*)h_mTracks_ToF[i_charge][i_pid][i_cent]->ProjectionY(HistName_ToF.c_str(),0,-1,0,-1)->Clone();

	HistName = Form("h_mEfficiency_%s%s_Cent_%d_eta",tof::mPID_ToF[i_pid].c_str(),tof::mCharge[i_charge].c_str(),i_cent);
	h_mEfficiency_eta1D[i_charge][i_cent] = (TH1D*)h_mCounts_ToF_eta1D[i_charge][i_cent]->Clone(HistName.c_str());
	h_mEfficiency_eta1D[i_charge][i_cent]->SetTitle(HistName.c_str());
	h_mEfficiency_eta1D[i_charge][i_cent]->Reset();
	h_mEfficiency_eta1D[i_charge][i_cent]->Divide(h_mCounts_ToF_eta1D[i_charge][i_cent],h_mCounts_TPC_eta1D[i_charge][i_cent],1,1,"B");
      
        //Phi
	HistName_TPC = Form("h_mCounts_TPC_%s%s_Cent_%d_phi",tof::mPID_ToF[i_pid].c_str(),tof::mCharge[i_charge].c_str(),i_cent);
	h_mCounts_TPC_phi1D[i_charge][i_cent] = (TH1D*)h_mTracks_TPC[i_charge][i_pid][i_cent]->ProjectionZ(HistName_TPC.c_str(),0,-1,0,-1)->Clone();

	HistName_ToF = Form("h_mCounts_ToF_%s%s_Cent_%d_phi",tof::mPID_ToF[i_pid].c_str(),tof::mCharge[i_charge].c_str(),i_cent);
	h_mCounts_ToF_phi1D[i_charge][i_cent] = (TH1D*)h_mTracks_ToF[i_charge][i_pid][i_cent]->ProjectionZ(HistName_ToF.c_str(),0,-1,0,-1)->Clone();

	HistName = Form("h_mEfficiency_%s%s_Cent_%d_phi",tof::mPID_ToF[i_pid].c_str(),tof::mCharge[i_charge].c_str(),i_cent);
	h_mEfficiency_phi1D[i_charge][i_cent] = (TH1D*)h_mCounts_ToF_phi1D[i_charge][i_cent]->Clone(HistName.c_str());
	h_mEfficiency_phi1D[i_charge][i_cent]->SetTitle(HistName.c_str());
	h_mEfficiency_phi1D[i_charge][i_cent]->Reset();
	h_mEfficiency_phi1D[i_charge][i_cent]->Divide(h_mCounts_ToF_phi1D[i_charge][i_cent],h_mCounts_TPC_phi1D[i_charge][i_cent],1,1,"B");

        //PT Eta
	HistName_TPC = Form("h_mCounts_TPC_%s%s_Cent_%d_pteta",tof::mPID_ToF[i_pid].c_str(),tof::mCharge[i_charge].c_str(),i_cent);
	h_mCounts_TPC_pteta[i_charge][i_cent] = (TH2D*)h_mTracks_TPC[i_charge][i_pid][i_cent]->Project3D("yx")->Clone();

	HistName_ToF = Form("h_mCounts_ToF_%s%s_Cent_%d_pteta",tof::mPID_ToF[i_pid].c_str(),tof::mCharge[i_charge].c_str(),i_cent);
	h_mCounts_ToF_pteta[i_charge][i_cent] = (TH2D*)h_mTracks_ToF[i_charge][i_pid][i_cent]->Project3D("yx")->Clone();

	HistName = Form("h_mEfficiency_%s%s_Cent_%d_pteta",tof::mPID_ToF[i_pid].c_str(),tof::mCharge[i_charge].c_str(),i_cent);
	h_mEfficiency_pteta[i_charge][i_cent] = (TH2D*)h_mCounts_ToF_pteta[i_charge][i_cent]->Clone(HistName.c_str());
	h_mEfficiency_pteta[i_charge][i_cent]->SetTitle(HistName.c_str());
	h_mEfficiency_pteta[i_charge][i_cent]->Reset();
	h_mEfficiency_pteta[i_charge][i_cent]->Divide(h_mCounts_ToF_pteta[i_charge][i_cent],h_mCounts_TPC_pteta[i_charge][i_cent],1,1,"B");
	
        //PT Phi
	HistName_TPC = Form("h_mCounts_TPC_%s%s_Cent_%d_ptphi",tof::mPID_ToF[i_pid].c_str(),tof::mCharge[i_charge].c_str(),i_cent);
	h_mCounts_TPC_ptphi[i_charge][i_cent] = (TH2D*)h_mTracks_TPC[i_charge][i_pid][i_cent]->Project3D("zx")->Clone();

	HistName_ToF = Form("h_mCounts_ToF_%s%s_Cent_%d_ptphi",tof::mPID_ToF[i_pid].c_str(),tof::mCharge[i_charge].c_str(),i_cent);
	h_mCounts_ToF_ptphi[i_charge][i_cent] = (TH2D*)h_mTracks_ToF[i_charge][i_pid][i_cent]->Project3D("zx")->Clone();

	HistName = Form("h_mEfficiency_%s%s_Cent_%d_pthpi",tof::mPID_ToF[i_pid].c_str(),tof::mCharge[i_charge].c_str(),i_cent);
	h_mEfficiency_ptphi[i_charge][i_cent] = (TH2D*)h_mCounts_ToF_ptphi[i_charge][i_cent]->Clone(HistName.c_str());
	h_mEfficiency_ptphi[i_charge][i_cent]->SetTitle(HistName.c_str());
	h_mEfficiency_ptphi[i_charge][i_cent]->Reset();
	h_mEfficiency_ptphi[i_charge][i_cent]->Divide(h_mCounts_ToF_ptphi[i_charge][i_cent],h_mCounts_TPC_ptphi[i_charge][i_cent],1,1,"B");

        //Eta Phi
	HistName_TPC = Form("h_mCounts_TPC_%s%s_Cent_%d_etaphi",tof::mPID_ToF[i_pid].c_str(),tof::mCharge[i_charge].c_str(),i_cent);
	h_mCounts_TPC_etaphi[i_charge][i_cent] = (TH2D*)h_mTracks_TPC[i_charge][i_pid][i_cent]->Project3D("zy")->Clone();

	HistName_ToF = Form("h_mCounts_ToF_%s%s_Cent_%d_etaphi",tof::mPID_ToF[i_pid].c_str(),tof::mCharge[i_charge].c_str(),i_cent);
	h_mCounts_ToF_etaphi[i_charge][i_cent] = (TH2D*)h_mTracks_ToF[i_charge][i_pid][i_cent]->Project3D("zy")->Clone();

	HistName = Form("h_mEfficiency_%s%s_Cent_%d_pthpi",tof::mPID_ToF[i_pid].c_str(),tof::mCharge[i_charge].c_str(),i_cent);
	h_mEfficiency_etaphi[i_charge][i_cent] = (TH2D*)h_mCounts_ToF_etaphi[i_charge][i_cent]->Clone(HistName.c_str());
	h_mEfficiency_etaphi[i_charge][i_cent]->SetTitle(HistName.c_str());
	h_mEfficiency_etaphi[i_charge][i_cent]->Reset();
	h_mEfficiency_etaphi[i_charge][i_cent]->Divide(h_mCounts_ToF_etaphi[i_charge][i_cent],h_mCounts_TPC_etaphi[i_charge][i_cent],1,1,"B");
  ...
  string outputname = Form("figures/AllToFMatchingProjections_%s_%s_rebinto%dphibins_pteta.pdf",vmsa::mBeamEnergy[energy].c_str(),eventplane.c_str(),nphibins);  
  ...
  for(int icent = 0; icent < 10; icent++)
  {
    ...
    {
      c1->cd(1+3*ic);
      h_mCounts_TPC_pteta[ic][icent]->GetXaxis()->SetTitle("p_{T} (GeV/c)");
      h_mCounts_TPC_pteta[ic][icent]->GetYaxis()->SetTitle("#eta");
      h_mCounts_TPC_pteta[ic][icent]->Draw("Colz");

      c1->cd(2+3*ic);
      h_mCounts_ToF_pteta[ic][icent]->GetXaxis()->SetTitle("p_{T} (GeV/c)");
      h_mCounts_ToF_pteta[ic][icent]->GetYaxis()->SetTitle("#eta");
      h_mCounts_ToF_pteta[ic][icent]->Draw("Colz");

      c1->cd(3+3*ic);
      h_mEfficiency_pteta[ic][icent]->GetXaxis()->SetTitle("p_{T} (GeV/c)");
      h_mEfficiency_pteta[ic][icent]->GetYaxis()->SetTitle("#eta");
      h_mEfficiency_pteta[ic][icent]->Draw("Colz");   
    }
    c1->Update();
  ...
  outputname = Form("figures/AllToFMatchingProjections_%s_%s_rebinto%dphibins_ptphi.pdf",vmsa::mBeamEnergy[energy].c_str(),eventplane.c_str(),nphibins);  
  ...
  for(int icent = 0; icent < 10; icent++)
  {
    ...
    {
      c1->cd(1+3*ic);
      h_mCounts_TPC_ptphi[ic][icent]->GetXaxis()->SetTitle("p_{T} (GeV/c)");
      h_mCounts_TPC_ptphi[ic][icent]->GetYaxis()->SetTitle("#phi");
      h_mCounts_TPC_ptphi[ic][icent]->Draw("Colz");

      c1->cd(2+3*ic);
      h_mCounts_ToF_ptphi[ic][icent]->GetXaxis()->SetTitle("p_{T} (GeV/c)");
      h_mCounts_ToF_ptphi[ic][icent]->GetYaxis()->SetTitle("#phi");
      h_mCounts_ToF_ptphi[ic][icent]->Draw("Colz");

      c1->cd(3+3*ic);
      h_mEfficiency_ptphi[ic][icent]->GetXaxis()->SetTitle("p_{T} (GeV/c)");
      h_mEfficiency_ptphi[ic][icent]->GetYaxis()->SetTitle("#phi");
      h_mEfficiency_ptphi[ic][icent]->Draw("Colz");   
    }
    c1->Update();
    c1->Print(outputname.c_str());
  }
  ...
  outputname = Form("figures/AllToFMatchingProjections_%s_%s_rebinto%dphibins_etaphi.pdf",vmsa::mBeamEnergy[energy].c_str(),eventplane.c_str(),nphibins);  
  outputstart = Form("%s[",outputname.c_str());
  outputstop = Form("%s]",outputname.c_str());
  ...
  for(int icent = 0; icent < 10; icent++)
  {
    ...
    {
      c1->cd(1+3*ic);
      h_mCounts_TPC_etaphi[ic][icent]->GetXaxis()->SetTitle("#eta");
      h_mCounts_TPC_etaphi[ic][icent]->GetYaxis()->SetTitle("#phi");
      h_mCounts_TPC_etaphi[ic][icent]->Draw("Colz");

      c1->cd(2+3*ic);
      h_mCounts_ToF_etaphi[ic][icent]->GetXaxis()->SetTitle("#eta");
      h_mCounts_ToF_etaphi[ic][icent]->GetYaxis()->SetTitle("#phi");
      h_mCounts_ToF_etaphi[ic][icent]->Draw("Colz");

      c1->cd(3+3*ic);
      h_mEfficiency_etaphi[ic][icent]->GetXaxis()->SetTitle("#eta");
      h_mEfficiency_etaphi[ic][icent]->GetYaxis()->SetTitle("#phi");
      h_mEfficiency_etaphi[ic][icent]->Draw("Colz");   
    }
    c1->Update();
    c1->Print(outputname.c_str());
  }
  ...
  outputname = Form("figures/AllToFMatchingProjections_%s_%s_rebinto%dphibins_pt1D.pdf",vmsa::mBeamEnergy[energy].c_str(),eventplane.c_str(),nphibins);  
  outputstart = Form("%s[",outputname.c_str());
  outputstop = Form("%s]",outputname.c_str());
  ...
  for(int icent = 0; icent < 10; icent++)
  {
    ...
    {
      c1->cd(1+3*ic);
      h_mCounts_TPC_pt1D[ic][icent]->GetXaxis()->SetTitle("p_{T} (GeV/c)");
      h_mCounts_TPC_pt1D[ic][icent]->GetYaxis()->SetTitle("TPC Counts");
      h_mCounts_TPC_pt1D[ic][icent]->Draw("pE");

      c1->cd(2+3*ic);
      h_mCounts_ToF_pt1D[ic][icent]->GetXaxis()->SetTitle("p_{T} (GeV/c)");
      h_mCounts_ToF_pt1D[ic][icent]->GetYaxis()->SetTitle("TOF Matched Counts");
      h_mCounts_ToF_pt1D[ic][icent]->Draw("pE");

      c1->cd(3+3*ic);
      h_mEfficiency_pt1D[ic][icent]->GetXaxis()->SetTitle("p_{T} (GeV/c)");
      h_mEfficiency_pt1D[ic][icent]->GetYaxis()->SetTitle("Matching Efficiency");
      h_mEfficiency_pt1D[ic][icent]->Draw("pE");   
    }
    c1->Update();
    c1->Print(outputname.c_str());
  }
  ...
  outputname = Form("figures/AllToFMatchingProjections_%s_%s_rebinto%dphibins_eta1D.pdf",vmsa::mBeamEnergy[energy].c_str(),eventplane.c_str(),nphibins);  
  outputstart = Form("%s[",outputname.c_str());
  outputstop = Form("%s]",outputname.c_str());
  ...
  for(int icent = 0; icent < 10; icent++)
  {
    ...
    {
      c1->cd(1+3*ic);
      h_mCounts_TPC_eta1D[ic][icent]->GetXaxis()->SetTitle("#eta");
      h_mCounts_TPC_eta1D[ic][icent]->GetYaxis()->SetTitle("TPC Counts");
      h_mCounts_TPC_eta1D[ic][icent]->Draw("pE");

      c1->cd(2+3*ic);
      h_mCounts_ToF_eta1D[ic][icent]->GetXaxis()->SetTitle("#eta");
      h_mCounts_ToF_eta1D[ic][icent]->GetYaxis()->SetTitle("TOF Matched Counts");
      h_mCounts_ToF_eta1D[ic][icent]->Draw("pE");

      c1->cd(3+3*ic);
      h_mEfficiency_eta1D[ic][icent]->GetXaxis()->SetTitle("#eta");
      h_mEfficiency_eta1D[ic][icent]->GetYaxis()->SetTitle("Matching Efficiency");
      h_mEfficiency_eta1D[ic][icent]->Draw("pE");   
    }
    c1->Update();
    c1->Print(outputname.c_str());
  }
  ...
  outputname = Form("figures/AllToFMatchingProjections_%s_%s_rebinto%dphibins_phi1D.pdf",vmsa::mBeamEnergy[energy].c_str(),eventplane.c_str(),nphibins);  
  outputstart = Form("%s[",outputname.c_str());
  outputstop = Form("%s]",outputname.c_str());
  ...
  for(int icent = 0; icent < 10; icent++)
  {
    ...
    {
      c1->cd(1+3*ic);
      h_mCounts_TPC_phi1D[ic][icent]->GetXaxis()->SetTitle("#phi");
      h_mCounts_TPC_phi1D[ic][icent]->GetYaxis()->SetTitle("TPC Counts");
      h_mCounts_TPC_phi1D[ic][icent]->Draw("pE");

      c1->cd(2+3*ic);
      h_mCounts_ToF_phi1D[ic][icent]->GetXaxis()->SetTitle("#phi");
      h_mCounts_ToF_phi1D[ic][icent]->GetYaxis()->SetTitle("TOF Matched Counts");
      h_mCounts_ToF_phi1D[ic][icent]->Draw("pE");

      c1->cd(3+3*ic);
      h_mEfficiency_phi1D[ic][icent]->GetXaxis()->SetTitle("#phi");
      h_mEfficiency_phi1D[ic][icent]->GetYaxis()->SetTitle("Matching Efficiency");
      h_mEfficiency_phi1D[ic][icent]->Draw("pE");   
    }
    c1->Update();
    c1->Print(outputname.c_str());
  }
  ...
  string outputfile = Form("../../output/%s/Eff_%s_ToFMatch%s_rebinto%dphibins.root",vmsa::mPID[pid].c_str(),vmsa::mBeamEnergy[energy].c_str(),eventplane.c_str(),nphibins);
