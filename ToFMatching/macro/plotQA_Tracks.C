...
void plotQA_Tracks(int energy = 6)
{
  ...
  TH1D *h_QA_TPC = (TH1D*)h_mTracks_TPC[chargeQA][pidQA][centQA]->ProjectionX(HistQA_Trakcs_TPC.c_str(),etaQA+1,etaQA+1,phiQA+1,phiQA+1);
  ...
  TH1D *h_QA_ToF = (TH1D*)h_mTracks_ToF[chargeQA][pidQA][centQA]->ProjectionX(HistQA_Trakcs_ToF.c_str(),etaQA+1,etaQA+1,phiQA+1,phiQA+1);
  ...
  TH1D *h_mEfficiency = h_mCounts_ToF[HistQA_ToF];
  h_mEfficiency->Divide(h_mCounts_ToF[HistQA_ToF],h_mCounts_TPC[HistQA_TPC],1.0,1.0,"B");
  h_mEfficiency->SetMarkerStyle(24);
  h_mEfficiency->SetMarkerSize(1.2);
  h_mEfficiency->SetMarkerColor(2);
  h_mEfficiency->Draw("pE same");
  // TGraphAsymmErrors *g_mEfficiency = new TGraphAsymmErrors();
  // g_mEfficiency->BayesDivide(h_mCounts_ToF[HistQA_ToF],h_mCounts_TPC[HistQA_TPC]);
