...
void plotQA_ToFMatchEff_Cent(const int energy = 4, const int cent = 0, const int dpid = 0, const int charge = 0, const int pid = 0, const int mKpCent = 3, const int mKmCent = 4, const int EP = 2)
{
  ...
  for(int i_eta = 0; i_eta < neta; ++i_eta) // ToF efficiency at different eta and phi bin at cent 9
  {
    ...
    f_kaon_eta[i_eta] = new TF1(FuncName.c_str(),tof_Kaon,0.1,10,7);
  ...
  for(int i_eta = 0; i_eta < neta; ++i_eta) // ToF efficiency at different eta and phi bin at cent 9
  {
    ...
    f_kaon_eta[i_eta]->SetRange(0.1,8.0);
