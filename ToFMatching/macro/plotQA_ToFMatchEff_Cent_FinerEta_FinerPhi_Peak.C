...
void plotQA_ToFMatchEff_Cent_FinerEta_FinerPhi_Peak(const int energy = 4, const int cent = 0, const int dpid = 0, const int charge = 0, const int pid = 0, const int mKpCent = 3, const int mKmCent = 4, const int EP = 5)
{
  ...
  int const mColor[48] = {11,11,2,2,4,4,7,7,8,8,38,38,11,11,2,2,4,4,7,7,8,8,38,38,11,11,2,2,4,4,7,7,8,8,38,38,11,11,2,2,4,4,7,7,8,8,38,38};
  int const mStyle[48] = {30,24,30,24,30,24,30,24,30,24,30,24,30,24,30,24,30,24,30,24,30,24,30,24,30,24,30,24,30,24,30,24,30,24,30,24,30,24,30,24,30,24,30,24,30,24,30,24};
  int const mLineStyle[48] = {3,2,3,2,3,2,3,2,3,2,3,2,3,2,3,2,3,2,3,2,3,2,3,2,3,2,3,2,3,2,3,2,3,2,3,2,3,2,3,2,3,2,3,2,3,2,3,2};
  ROOT::Math::MinimizerOptions::SetDefaultMaxFunctionCalls(50000);

  std::string inputfile = Form("../../output/%s/Eff_%s_ToFMatch%s_rebinto48phibins.root",vmsa::mPID[pid].c_str(),vmsa::mBeamEnergy[energy].c_str(),eventplane.c_str());
  ...
  const int nphi = 48;//h_FramePhi_ToF->GetNbinsX();
  ...
  for(int i_eta = 0; i_eta < neta; ++i_eta) // ToF efficiency at different eta and phi bin at cent 9
  {
    ...
    for(int i_bin = 0; i_bin < h_mEffEta[HistName]->GetNbinsX(); ++i_bin) 
    {
      ...
      else if(charge == 1 && cent == 9 && i_eta == 23)
      {
        if ((pt > 0.5 && pt < 0.75) || (pt > 0.95 && pt < 1.6) || (pt > 3.8)) h_mEffEta[HistName]->SetBinError(i_bin+1,0.0); 
        if ((pt > 0.5 && pt < 0.75) || (pt > 0.95 && pt < 1.6) || (pt > 3.8)) h_mEffEta[HistName]->SetBinContent(i_bin+1,0.0);
      } 
      else if(charge == 0 && cent == 9 && (i_eta == 0 || i_eta == 23))
      {
        if ((pt > 0.5 && pt < 0.75) || (pt > 0.95 && pt < 2.0)) h_mEffEta[HistName]->SetBinError(i_bin+1,0.0); 
        if ((pt > 0.5 && pt < 0.75) || (pt > 0.95 && pt < 2.0)) h_mEffEta[HistName]->SetBinContent(i_bin+1,0.0);
      }
      else
      {
    ...
    for(int i_par = 0; i_par < 7; ++i_par)
    {
      ...
      if(charge == 1 && cent == 9 && i_eta == 23) f_kaon_eta[i_eta]->SetParameter(i_par,f_kaon_eta[0]->GetParameter(i_par));
	...
	f_kaon_phi[i_eta][i_phi]->FixParameter(i_par,par_fite[i_par]);
      ...
      double binMin = h_mEfficiency[HistName]->FindBin(0.2);
      double binMax = h_mEfficiency[HistName]->FindBin(1.2);
      double maxContent = -1.0;
      int maxBin = binMin;
      for(int bin = binMin; bin <= binMax; ++bin){
        double content = h_mEfficiency[HistName]->GetBinContent(bin);
        if(content > maxContent) {
          maxContent = content;
          maxBin = bin;
        }
      }
      double maxX = h_mEfficiency[HistName]->GetBinCenter(maxBin);  
      double xLow = maxX - 0.025;
      double xHigh = maxX + 0.025;      

      //f_kaon_phi[i_eta][i_phi]->SetParameter(0,maxContent);
      cout << "iphi = " << i_phi << ", ieta = " << i_eta << ", maxContent = " << maxContent << endl;
      f_kaon_phi[i_eta][i_phi]->SetRange(xLow,xHigh);
      ...
      h_mEfficiency[HistName]->Fit(f_kaon_phi[i_eta][i_phi], "NR","",xLow,xHigh);
      HistName_FitPar = Form("h_mFitParameters_%s%s_Cent_%d_Eta_%d_Phi_%d",tof::mPID_ToF[dpid].c_str(),tof::mCharge[charge].c_str(),cent,i_eta,i_phi);
      for(int i_bin = 0; i_bin < 7; ++i_bin)
      {
  ...
  for(int i_eta = 0; i_eta < neta; ++i_eta) // ToF efficiency at different eta and phi bin at cent 9
  {
    ...
    for(int i_phi = 0; i_phi < nphi; ++i_phi) 
    {
      ...
      HistName = Form("h_mEfficiency_%s%s_Cent_%d_Eta_%d_Phi_%d",tof::mPID_ToF[dpid].c_str(),tof::mCharge[charge].c_str(),cent,i_eta,i_phi);
      // h_mEfficiency[HistName]->Draw("pE same");
      h_mEfficiency_plot[HistName]->SetTitle(Form("%d#pi/6<#phi<%d#pi/6",-6+i_phi, -5+i_phi));
