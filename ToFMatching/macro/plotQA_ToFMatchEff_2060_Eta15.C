...
void plotQA_ToFMatchEff_2060(const int energy = 4, const int charge = 1)
{
  ...
  std::string inputfile = Form("../../output/Eff_%s_ToFMatch.root",vmsa::mBeamEnergy[energy].c_str());
  ...
  for(int i_bin = 0; i_bin < h_mEffCent[HistName]->GetNbinsX(); ++i_bin) 
  {
    ...
    if(charge == 1)
    {
      if ((pt > dip_1st[energy] && pt < dip_2nd[energy]+0.1) || (pt > dip_3rd[energy]+0.1 && pt < dip_4th[energy])) h_mEffCent[HistName]->SetBinError(i_bin+1,0.0); 
      if ((pt > dip_1st[energy] && pt < dip_2nd[energy]+0.1) || (pt > dip_3rd[energy]+0.1 && pt < dip_4th[energy])) h_mEffCent[HistName]->SetBinContent(i_bin+1,0.0); 
    }
  }
  ...
  for(int i_eta = 0; i_eta < tof::BinEta; ++i_eta) // ToF efficiency at different eta and phi bin at cent 9
  {
    ...
    for(int i_bin = 0; i_bin < h_mEffEta[HistName]->GetNbinsX(); ++i_bin) 
    {
      ...
      if(i_eta == 1 || i_eta == 13)
      {
        if ((pt > dip_1st[energy] && pt < dip_2nd[energy]-0.2) || (pt > dip_3rd[energy]-0.3 && pt < dip_4th[energy]-0.7)) h_mEffEta[HistName]->SetBinError(i_bin+1,0.0); 
        if ((pt > dip_1st[energy] && pt < dip_2nd[energy]-0.2) || (pt > dip_3rd[energy]-0.3 && pt < dip_4th[energy]-0.7)) h_mEffEta[HistName]->SetBinContent(i_bin+1,0.0); 
      } 
	...
	//{
        if(i_eta == 1 || i_eta == 13)
        {
          if ((pt > dip_1st[energy] && pt < dip_2nd[energy]-0.2) || (pt > dip_3rd[energy]-0.3 && pt < dip_4th[energy]-0.7)) h_mEfficiency[HistName]->SetBinError(i_bin+1,0.0); 
          if ((pt > dip_1st[energy] && pt < dip_2nd[energy]-0.2) || (pt > dip_3rd[energy]-0.3 && pt < dip_4th[energy]-0.7)) h_mEfficiency[HistName]->SetBinContent(i_bin+1,0.0);  
        }
        else if(i_eta == 2 || i_eta == 12)
        { 
	...
	f_kaon_phi[i_eta][i_phi]->FixParameter(i_par,par_fite[i_par]);
      ...
      if(i_eta == 1 || i_eta == 13) f_kaon_phi[i_eta][i_phi]->SetRange(dip_4th[energy]-0.7,range[energy]);
  ...
  string output_start = Form("figures/Efficiency/ToF/QAs/EffFit_AuAu%s_K%s_QA_2060.pdf[",vmsa::mBeamEnergy[energy].c_str(),tof::mCharge[charge].c_str());
  c_Efficiency->Print(output_start.c_str());

  string outputname = Form("figures/Efficiency/ToF/QAs/EffFit_AuAu%s_K%s_QA_2060.pdf",vmsa::mBeamEnergy[energy].c_str(),tof::mCharge[charge].c_str());
  ...
  string output_stop = Form("figures/Efficiency/ToF/QAs/EffFit_AuAu%s_K%s_QA_2060.pdf]",vmsa::mBeamEnergy[energy].c_str(),tof::mCharge[charge].c_str());
  c_Efficiency->Print(output_stop.c_str());

#endif

  string outputfile = Form("../../output/FitPar_AuAu%s_K%s_2060.root",vmsa::mBeamEnergy[energy].c_str(),tof::mCharge[charge].c_str());
