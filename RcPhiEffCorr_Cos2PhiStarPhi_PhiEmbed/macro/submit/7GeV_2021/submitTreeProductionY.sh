...
outDir=${particle}_EPRes_RapidityDependentV2
...
##########Test Production##########
star-submit-template -template testProductionRapidity.xml -entities fitmode=$fitmode,pt=$pt,cent=$cent,mode=$mode,particle=$particle,pid=$pid,energy=$energy,nJobs=$nJobs,nEvents=$nEvents,library=$library,codePath=$codePath,outPath=$outPath,listPath=$listPath,outDir=$outDir
