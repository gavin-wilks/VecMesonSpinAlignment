...
void calSpinAlignmentSysPhiCent_PolySys_Signed(int energy = 3, int pid = 0, int year = 0, bool random3D = false, int centQA = 4, int ptQA = 0, int dcaQA = 0, int nsigQA = 0, int order = 2, std::string etamode = "eta1_eta1")
{
  std::string EP[2] = {"",""};
  ...
  string inputfile = Form("../output/AuAu%s/%s/InvMassSubBgCent_%s_Signed.root",vmsa::mBeamEnergy[energy].c_str(),vmsa::mPID[pid].c_str(),etamode.c_str());
  if(order == 1) inputfile = Form("../output/AuAu%s/%s/InvMassSubBgCent_%s_Signed_FirstOrder.root",vmsa::mBeamEnergy[energy].c_str(),vmsa::mPID[pid].c_str(),etamode.c_str());
...
#if _PlotQA_
  string outputname = Form("./figures/%s/%s/centralitystudy/allThetaYields%s_%s_Order%d_%s_CentDependence_signedcosthetastar.pdf",vmsa::mPID[pid].c_str(),vmsa::mBeamEnergy[energy].c_str(),vmsa::mPID[pid].c_str(),vmsa::mBeamEnergy[energy].c_str(),order,etamode.c_str());
    ...
    for(int i_pt = vmsa::pt_rebin_first_cent[energy]; i_pt <= vmsa::pt_rebin_last_cent[energy]; ++i_pt) // pt loop
    {
      ...
      for(int i_theta = 0; i_theta < 16; ++i_theta)
      {
        ...
        if(i_theta < 14)
        {
          ...
          h_mMass[KEY_QA]->GetXaxis()->SetLabelSize(0.03);
          h_mMass[KEY_QA]->GetXaxis()->SetTitle("M(K^{+},K^{-})");
          h_mMass[KEY_QA]->SetTitle(Form("%.2f<p_{T}<%.2f, %d/7<|cos(#theta*)|<%d/7, cent%d",vmsa::pt_low_cent[energy][i_pt],vmsa::pt_up_cent[energy][i_pt],i_theta-7,i_theta-6,i_cent));
          h_mMass[KEY_QA]->GetXaxis()->SetTitleSize(0.05);
          h_mMass[KEY_QA]->GetXaxis()->SetTitleOffset(1.2);
          h_mMass[KEY_QA]->GetXaxis()->CenterTitle();
        ...
        if(i_theta == 14)
        {
          ...
          string KEY_InteTheta_QA_Poly = Form("pt_%d_Centrality_%d_%s_Dca_%d_Sig_%d_%s_Norm_%d_Poly3",i_pt,i_cent,EP[order-1].c_str(),vmsa::Dca_start,vmsa::nSigKaon_start,vmsa::mPID[pid].c_str(),vmsa::Norm_QA);
          h_mMass_InteTheta[KEY_InteTheta_QA]->SetTitle("");
          h_mMass_InteTheta[KEY_InteTheta_QA]->SetTitle(Form("%.2f<p_{T}<%.2f, -1<|cos(#theta*)|<1, cent%d",vmsa::pt_low_cent[energy][i_pt],vmsa::pt_up_cent[energy][i_pt],i_cent));
          h_mMass_InteTheta[KEY_InteTheta_QA]->GetXaxis()->SetNdivisions(505,'N');
          h_mMass_InteTheta[KEY_InteTheta_QA]->GetXaxis()->SetLabelSize(0.03);
          h_mMass_InteTheta[KEY_InteTheta_QA]->GetXaxis()->SetTitle("M(K^{+},K^{-})");
          h_mMass_InteTheta[KEY_InteTheta_QA]->GetXaxis()->SetTitleSize(0.05);
      ...
      for(int i_theta = vmsa::CTS_start; i_theta < 14; ++i_theta)
      {
        c_diff->cd(i_theta+1);
        string KEY_QA = Form("pt_%d_Centrality_%d_CosThetaStar_%d_%s_Dca_%d_Sig_%d_%s_Norm_%d",i_pt,i_cent,i_theta,EP[order-1].c_str(),vmsa::Dca_start,vmsa::nSigKaon_start,vmsa::mPID[pid].c_str(),vmsa::Norm_QA);
        string KEY_QA_Poly = Form("pt_%d_Centrality_%d_CosThetaStar_%d_%s_Dca_%d_Sig_%d_%s_Norm_%d_Poly3",i_pt,i_cent,i_theta,EP[order-1].c_str(),vmsa::Dca_start,vmsa::nSigKaon_start,vmsa::mPID[pid].c_str(),vmsa::Norm_QA);
        TF1 *f_bw = new TF1("f_bw",Poly3BreitWigner,vmsa::BW_Start[pid],vmsa::BW_Stop[pid],7);
      ...
      for(int i_sigma = vmsa::Sig_start; i_sigma < /*vmsa::Sig_stop*/1; ++i_sigma)
      {
        ...
        {
          string KEY_counts_QA = Form("pt_%d_Centrality_%d_%s_Dca_%d_Sig_%d_%s_Norm_%d_Sigma_%d_%s_Poly3",i_pt,i_cent,EP[order-1].c_str(),vmsa::Dca_start,vmsa::nSigKaon_start,vmsa::mPID[pid].c_str(),vmsa::Norm_QA,i_sigma,vmsa::mInteMethod[i_method].c_str());
...
//#endif
  ...
  string outputfile = Form("../output/AuAu%s/%s/RawRhoCentSys_%s_Poly_Signed.root",vmsa::mBeamEnergy[energy].c_str(),vmsa::mPID[pid].c_str(),etamode.c_str());
  if(order == 1) outputfile = Form("../output/AuAu%s/%s/RawRhoCentSys_%s_Poly_Signed_FirstOrder.root",vmsa::mBeamEnergy[energy].c_str(),vmsa::mPID[pid].c_str(),etamode.c_str());
