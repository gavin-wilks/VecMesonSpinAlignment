...
void calSpinAlignmentSysPhiP_Helicity_2D_OffDiag(int energy = 4, int pid = 0, int year = 0, bool random3D = false, int order = 2, string etamode = "eta1_eta1")
{
  ...
  string inputfile = Form("../output/AuAu%s/%s/Helicity2DP_InvMassSubBg_%s.root",vmsa::mBeamEnergy[energy].c_str(),vmsa::mPID[pid].c_str(),etamode.c_str());
  if(order == 1) inputfile = Form("../output/AuAu%s/%s/Helicity2DP_InvMassSubBg_%s_FirstOrder.root",vmsa::mBeamEnergy[energy].c_str(),vmsa::mPID[pid].c_str(),etamode.c_str());
	  ...
	  for(int i_norm = vmsa::Norm_start; i_norm < vmsa::Norm_stop; ++i_norm)
	  {
	    ...
	    for(int i_phi = vmsa::CTS_start; i_phi < 12/*2*vmsa::CTS_stop*/; i_phi++) // cos(theta*) loop
	    {
	      ...
	      string KEY_Poly = Form("Centrality_%d_%s_Dca_%d_Sig_%d_%s_Norm_%d_Poly%d",i_cent,EP[order-1].c_str(),i_dca,i_sig,vmsa::mPID[pid].c_str(),i_norm,i_poly+1);
              //if(i_pt == vmsa::p_rebin_first[energy]) g_mChiNDF[KEY_Poly] = new TGraphAsymmErrors();             
              //if(i_pt == vmsa::p_rebin_first[energy]) g_mPValue[KEY_Poly] = new TGraphAsymmErrors();             
	      ...
	      //float p_mean = (vmsa::p_low[energy][i_pt]+vmsa::p_up[energy][i_pt])/2.0;
...
//    {
//      string KEY_QA = Form("p_%d_Centrality_%d_CosThetaStar_%d_2nd_Dca_%d_Sig_%d_%s_Norm_%d",vmsa::p_QA[energy],9,i_theta,vmsa::Dca_start,vmsa::nSigKaon_start,vmsa::mPID[pid].c_str(),vmsa::Norm_QA);
...
//#endif
  ...
  for(int i_pt = vmsa::p_rebin_first[energy]; i_pt < vmsa::p_rebin_last[energy]; i_pt++) // pt loop
  {
	...
	for(int i_norm = vmsa::Norm_start; i_norm < vmsa::Norm_stop; ++i_norm)
	{
	  for(int i_func = vmsa::Func_start; i_func < vmsa::Func_stop; ++i_func)
	  {
	    ...
	    {
	      string KEY = Form("p_%d_Centrality_%d_EtaGap_%d_CosThetaStar_%d_2nd_%s_Norm_%d_Func_%d",i_pt,i_cent,i_eta,i_theta,vmsa::mPID[pid].c_str(),i_norm,i_func);
	    }
	  }
	}
      }
  ...
  */
	    ...
	    for(int i_method = 1/*vmsa::Method_start*/; i_method < vmsa::Method_stop; ++i_method)
	    {
	        ...
	        for(int i_pt = vmsa::p_rebin_first[energy]; i_pt < vmsa::p_rebin_last[energy]; ++i_pt) // pt loop
	        {
	          ...
	          for(int i_phi = vmsa::CTS_start; i_phi < 12/*2*vmsa::CTS_stop*/; ++i_phi) // cos(theta*) loop
	          {
	            ...
	            {
	              string KEY = Form("p_%d_Centrality_%d_CosThetaStar_%d_Phi_%d_%s_Dca_%d_Sig_%d_%s_Norm_%d",i_pt,i_cent,i_thetah,i_phi,EP[order-1].c_str(),i_dca,i_sig,vmsa::mPID[pid].c_str(),i_norm);
	              string KEY_Poly = Form("p_%d_Centrality_%d_CosThetaStar_%d_Phi_%d_%s_Dca_%d_Sig_%d_%s_Norm_%d_Poly%d",i_pt,i_cent,i_thetah,i_phi,EP[order-1].c_str(),i_dca,i_sig,vmsa::mPID[pid].c_str(),i_norm,i_poly+1);
	              TF1 *f_bw;
                      if(i_poly == 0) f_bw = new TF1("f_bw",PolyBreitWigner, vmsa::BW_Start[pid],vmsa::BW_Stop[pid],5);
                      if(i_poly == 1) f_bw = new TF1("f_bw",Poly2BreitWigner,vmsa::BW_Start[pid],vmsa::BW_Stop[pid],6);
                      if(i_poly == 2) f_bw = new TF1("f_bw",Poly3BreitWigner,vmsa::BW_Start[pid],vmsa::BW_Stop[pid],7);
	          ...
	          g_mRho[KEY_rho]->SetPoint(i_pt-vmsa::p_rebin_first[energy],p_mean,f_rho->GetParameter(0));
                  cout << "for rho00 calculation, p_mean = " << p_mean << ", rho = " << f_rho->GetParameter(0) << " +/- " << f_rho->GetParError(0) << endl;
	          g_mRho[KEY_rho]->SetPointError(i_pt-vmsa::p_rebin_first[energy],0.0,0.0,f_rho->GetParError(0),f_rho->GetParError(0));
	          g_mReal[KEY_real]->SetPoint(i_pt-vmsa::p_rebin_first[energy],p_mean,f_rho->GetParameter(1));
	          g_mReal[KEY_real]->SetPointError(i_pt-vmsa::p_rebin_first[energy],0.0,0.0,f_rho->GetParError(1),f_rho->GetParError(1));
	          g_mImag[KEY_imag]->SetPoint(i_pt-vmsa::p_rebin_first[energy],p_mean,f_rho->GetParameter(2));
	          g_mImag[KEY_imag]->SetPointError(i_pt-vmsa::p_rebin_first[energy],0.0,0.0,f_rho->GetParError(2),f_rho->GetParError(2));
	          g_mReRho1n1[KEY_rerho1n1]->SetPoint(i_pt-vmsa::p_rebin_first[energy],p_mean,f_rho->GetParameter(3));
	          g_mReRho1n1[KEY_rerho1n1]->SetPointError(i_pt-vmsa::p_rebin_first[energy],0.0,0.0,f_rho->GetParError(3),f_rho->GetParError(3));
	          g_mImRho1n1[KEY_imrho1n1]->SetPoint(i_pt-vmsa::p_rebin_first[energy],p_mean,f_rho->GetParameter(4));
	          g_mImRho1n1[KEY_imrho1n1]->SetPointError(i_pt-vmsa::p_rebin_first[energy],0.0,0.0,f_rho->GetParError(4),f_rho->GetParError(4));
  ...
  for(int i_poly = 0; i_poly < 1/*3*/; i_poly++)
  {
    for(int i_norm = 0; i_norm < 1/*3*/; i_norm++)
    {
      ...
      {
        string outputname = Form("./figures/%s/%s/pstudy/Helicity2D_allThetaYields%s_%s_Order%d_cent%d_%s_pTdependence_Norm%d_Poly%d.pdf",vmsa::mPID[pid].c_str(),vmsa::mBeamEnergy[energy].c_str(),vmsa::mPID[pid].c_str(),vmsa::mBeamEnergy[energy].c_str(),order,i_cent,etamode.c_str(),i_norm,i_poly+1);
        ...
        for(int i_pt = vmsa::p_rebin_first[energy]; i_pt < vmsa::p_rebin_last[energy]; ++i_pt) // pt loop
        {
          ...
          { 
            ...
            for(int i_thetah = 0; i_thetah < 12; ++i_thetah)
            {
              ...
              if(i_thetah < 9)
              {
                string KEY_QA = Form("p_%d_Centrality_%d_CosThetaStar_%d_Phi_%d_%s_Dca_%d_Sig_%d_%s_Norm_%d",i_pt,i_cent,i_thetah,i_phi,EP[order-1].c_str(),vmsa::Dca_start,0/*vmsa::nSigKaon_start*/,vmsa::mPID[pid].c_str(),i_norm);
                h_mMass[KEY_QA]->SetTitle("");
                h_mMass[KEY_QA]->GetXaxis()->SetNdivisions(505,'N');
                h_mMass[KEY_QA]->GetXaxis()->SetLabelSize(0.03);
                h_mMass[KEY_QA]->GetXaxis()->SetTitle("M(K^{+},K^{-})");
                h_mMass[KEY_QA]->SetTitle(Form("%.2f<p_{T}<%.2f phiH=%d, CTH=%d",vmsa::p_low[energy][i_pt],vmsa::p_up[energy][i_pt],i_phi,i_thetah));
                h_mMass[KEY_QA]->GetXaxis()->SetTitleSize(0.05);
                h_mMass[KEY_QA]->GetXaxis()->SetTitleOffset(1.2);
                h_mMass[KEY_QA]->GetXaxis()->CenterTitle();
              ...
              if(i_thetah == 9)
              {
                ...
                h_mMass_InteTheta[KEY_InteTheta_QA]->SetTitle("");
                h_mMass_InteTheta[KEY_InteTheta_QA]->SetTitle(Form("%.2f<p_{T}<%.2f, Full phiH, Full CTG",vmsa::p_low[energy][i_pt],vmsa::p_up[energy][i_pt]));
                h_mMass_InteTheta[KEY_InteTheta_QA]->GetXaxis()->SetNdivisions(505,'N');
                h_mMass_InteTheta[KEY_InteTheta_QA]->GetXaxis()->SetLabelSize(0.03);
                h_mMass_InteTheta[KEY_InteTheta_QA]->GetXaxis()->SetTitle("M(K^{+},K^{-})");
                h_mMass_InteTheta[KEY_InteTheta_QA]->GetXaxis()->SetTitleSize(0.05);
            ...
            for(int i_thetah = vmsa::CTS_start; i_thetah < 9/*&2*vmsa::CTS_stop*/; ++i_thetah)
            {
              c_diff->cd(i_thetah+1);
              string KEY_QA = Form("p_%d_Centrality_%d_CosThetaStar_%d_Phi_%d_%s_Dca_%d_Sig_%d_%s_Norm_%d",i_pt,i_cent,i_thetah,i_phi,EP[order-1].c_str(),vmsa::Dca_start,0/*vmsa::nSigKaon_start*/,vmsa::mPID[pid].c_str(),i_norm);
              string KEY_QA_Poly = Form("p_%d_Centrality_%d_CosThetaStar_%d_Phi_%d_%s_Dca_%d_Sig_%d_%s_Norm_%d_Poly%d",i_pt,i_cent,i_thetah,i_phi,EP[order-1].c_str(),vmsa::Dca_start,0/*vmsa::nSigKaon_start*/,vmsa::mPID[pid].c_str(),i_norm,i_poly+1);
              TF1 *f_bw;
              if(i_poly == 0) f_bw = new TF1("f_bw",PolyBreitWigner,vmsa::BW_Start[pid],vmsa::BW_Stop[pid],5);
              if(i_poly == 1) f_bw = new TF1("f_bw",Poly2BreitWigner,vmsa::BW_Start[pid],vmsa::BW_Stop[pid],6);
              if(i_poly == 2) f_bw = new TF1("f_bw",Poly3BreitWigner,vmsa::BW_Start[pid],vmsa::BW_Stop[pid],7);
          ...
          for(int i_sigma = 0/*vmsa::Sig_start*/; i_sigma < 1/*vmsa::Sig_stop*/; ++i_sigma)
          {
            for(int i_method = 1/*vmsa::Method_start*/; i_method < 2/*vmsa::Method_stop*/; ++i_method)
            {
              ...
              h_mCounts[KEY_counts_QA]->SetTitle(Form("Raw Yields, %.2f<p_{T}<%.2f GeV/c",vmsa::p_low[energy][i_pt],vmsa::p_up[energy][i_pt]));
 ...
 // {
  ...
  //        for(int i_pt = 2; i_pt < vmsa::p_rebin_last[energy] ; ++i_pt) // pt loop
  //        {
  //          c_chi->cd(i_pt-1);
  //          double min[3], max[3];
  //          for(int i_poly = 0; i_poly < 3; i_poly++)
  ...
  string outputname = Form("./figures/%s/%s/pstudy/Helicity2D_ExtractedSpinDensityElements_%s_Order%d.pdf",vmsa::mPID[pid].c_str(),vmsa::mBeamEnergy[energy].c_str(),vmsa::mBeamEnergy[energy].c_str(),order);
  ...
  string outputfile = Form("../output/AuAu%s/%s/Helicity2DP_RawPhiPSys_%s.root",vmsa::mBeamEnergy[energy].c_str(),vmsa::mPID[pid].c_str(),etamode.c_str());
  if(order == 1) outputfile = Form("../output/AuAu%s/%s/Helicity2DP_RawPhiPSys_%s_FirstOrder.root",vmsa::mBeamEnergy[energy].c_str(),vmsa::mPID[pid].c_str(),etamode.c_str());
  ...
  //                {
  //                  h_mRatio[KEY_counts]->SetName(Form("CosCosHDataRcRatio_p_%d",i_pt));
