...
void compareGlobalVsHelicity(Int_t energy = 4, Int_t pid = 0, Int_t i_cent = 9, string correction = "Raw", int order = 2, string etamode = "eta1_eta1", int yspectra = 23, std::string simmode = "Mc", std::string v2 = "nov2")//defaultF = 0 is BESII, defaultF = 1 is BESI
{
  ...
  std::string date = "20240808";
  if(v2 == "nov2") date = "20240809";

  std::string spectra[5][nvar];
  for(int i = start; i < stop; i++)
  {
    spectra[0][i] = Form("CEPT_O2_NoYSpec_%s_phieff_phiTPC_%s_2_ysig1000_rho0.3333_rerho0.0_imrho0.0_r0.0_i0.0_hrho%s_hrerho0.0_himrho0.0_hr0.0_hi0.0"   ,v2.c_str(),date.c_str(),sigmay_text[i].c_str());
    spectra[1][i] = Form("CEPT_O2_NoYSpec_%s_phieff_phiTPC_%s_2_ysig1000_rho0.3333_rerho0.0_imrho0.0_r0.0_i0.0_hrho0.3333_hrerho0.0_himrho0.0_hr%s_hi0.0",v2.c_str(),date.c_str(),sigmay_text_offdiag[i].c_str());
    spectra[2][i] = Form("CEPT_O2_NoYSpec_%s_phieff_phiTPC_%s_2_ysig1000_rho0.3333_rerho0.0_imrho0.0_r0.0_i0.0_hrho0.3333_hrerho0.0_himrho0.0_hr0.0_hi%s",v2.c_str(),date.c_str(),sigmay_text_offdiag[i].c_str());
    spectra[3][i] = Form("CEPT_O2_NoYSpec_%s_phieff_phiTPC_%s_2_ysig1000_rho0.3333_rerho0.0_imrho0.0_r0.0_i0.0_hrho0.3333_hrerho%s_himrho0.0_hr0.0_hi0.0",v2.c_str(),date.c_str(),sigmay_text_offdiag[i].c_str());
    spectra[4][i] = Form("CEPT_O2_NoYSpec_%s_phieff_phiTPC_%s_2_ysig1000_rho0.3333_rerho0.0_imrho0.0_r0.0_i0.0_hrho0.3333_hrerho0.0_himrho%s_hr0.0_hi0.0",v2.c_str(),date.c_str(),sigmay_text_offdiag[i].c_str());
  }
  ...
  for(int irho = 0; irho < 5; irho++)
  {
    for(int i_pt = 2; i_pt < 6; ++i_pt) // pt loop
    {
      ...
      for(int i = start; i < stop; i++)
      {
        string inputfileRC = Form("effaccfiles/%s/%s/%s/Eff_%s_SingleParticle_noToF_Mode0_EtaMode0_PtBins2_5.root",vmsa::mPID[pid].c_str(),vmsa::mBeamEnergy[energy].c_str(),spectra[irho][i].c_str(),vmsa::mBeamEnergy[energy].c_str());
        File_InPutRC[irho][i] = TFile::Open(inputfileRC.c_str());
        ...
        string KEY_2D = Form("h_m%sEffCosHPhiPrime_Cent_9_Pt_%d_%d",simmode.c_str(),i_pt,irho);
        string KEY_2D_Global = Form("h_m%sEffCosPhiPrime_Cent_9_Pt_%d_%d",simmode.c_str(),i_pt,irho);
        h_m2D_RC[irho][i_pt-2][i] = (TH2D*) ((TH2D*) File_InPutRC[irho][i]->Get(KEY_2D_RC.c_str()))->Clone(KEY_2D.c_str());
        h_m2D_RC_Global[irho][i_pt-2][i] = (TH2D*) ((TH2D*) File_InPutRC[irho][i]->Get(KEY_2D_RC_Global.c_str()))->Clone(KEY_2D_Global.c_str());
  ...
  TGraphAsymmErrors *g2D[5][5][4]; 
  TGraphAsymmErrors *g2D_Global[5][5][4]; 
  ...
  for(int irho = 0; irho < 5; irho++)
  {
    for(int ipt = 0; ipt < 4; ipt++)
    {
      ...
      {
        g2D[irho][par][ipt] = new TGraphAsymmErrors();
        g2D_Global[irho][par][ipt] = new TGraphAsymmErrors();
      ...
      for(int i = start; i < stop; i++)
      {
        ...
        fits2D[irho][ipt][i] = new TF2(Form("fit2D_%d_%d_%d",ipt,i,irho),SpinDensity2Dcos,-1.0,1.0,0.0,2.0*TMath::Pi(),6);
        fits2D[irho][ipt][i]->SetParameter(0,1./3.);
        fits2D[irho][ipt][i]->SetParLimits(0,0.0,1.0);
        fits2D[irho][ipt][i]->SetParameter(5,h_m2D_RC[irho][ipt][i]->GetMaximum());
        h_m2D_RC[irho][ipt][i]->Fit(fits2D[irho][ipt][i],"NMI");
        for(int par = 0; par < 5; par++)
        {
          if(par == 0 && irho == 0) g2D[irho][par][ipt]->SetPoint(i-start,sigmay[i]-1./3.,fits2D[irho][ipt][i]->GetParameter(0)-1./3.);
          if(par != 0 && irho == 0) g2D[irho][par][ipt]->SetPoint(i-start,sigmay[i]-1./3.,fits2D[irho][ipt][i]->GetParameter(par));
          if(par == 0 && irho != 0) g2D[irho][par][ipt]->SetPoint(i-start,sigmay_offdiag[i],fits2D[irho][ipt][i]->GetParameter(0)-1./3.);
          if(par != 0 && irho != 0) g2D[irho][par][ipt]->SetPoint(i-start,sigmay_offdiag[i],fits2D[irho][ipt][i]->GetParameter(par));
          g2D[irho][par][ipt]->SetPointError(i-start,0.0,0.0,fits2D[irho][ipt][i]->GetParError(par),fits2D[irho][ipt][i]->GetParError(par));
        }

        fits2D_Global[irho][ipt][i] = new TF2(Form("fit2D_Global_%d_%d_%d",ipt,i,irho),SpinDensity2Dcos,-1.0,1.0,0,2.0*TMath::Pi(),6);
        fits2D_Global[irho][ipt][i]->SetParameter(0,1./3.);
        fits2D_Global[irho][ipt][i]->SetParLimits(0,0.0,1.0);
        fits2D_Global[irho][ipt][i]->SetParameter(5,h_m2D_RC_Global[irho][ipt][i]->GetMaximum());
        h_m2D_RC_Global[irho][ipt][i]->Fit(fits2D_Global[irho][ipt][i],"NMI");
        for(int par = 0; par < 5; par++)
        {
          if(par == 0 && irho == 0) g2D_Global[irho][par][ipt]->SetPoint(i-start,sigmay[i]-1./3.,fits2D_Global[irho][ipt][i]->GetParameter(0)-1./3.);
          if(par != 0 && irho == 0) g2D_Global[irho][par][ipt]->SetPoint(i-start,sigmay[i]-1./3.,fits2D_Global[irho][ipt][i]->GetParameter(par));
          if(par == 0 && irho != 0) g2D_Global[irho][par][ipt]->SetPoint(i-start,sigmay_offdiag[i],fits2D_Global[irho][ipt][i]->GetParameter(0)-1./3.);
          if(par != 0 && irho != 0) g2D_Global[irho][par][ipt]->SetPoint(i-start,sigmay_offdiag[i],fits2D_Global[irho][ipt][i]->GetParameter(par));
          g2D_Global[irho][par][ipt]->SetPointError(i-start,0.0,0.0,fits2D_Global[irho][ipt][i]->GetParError(par),fits2D_Global[irho][ipt][i]->GetParError(par));
        }
  ...
  std::string outputname = Form("figures/%s/%s/pTstudy/GlobalvsHelicity_parametercomparison_%s_%s.pdf",vmsa::mPID[pid].c_str(),vmsa::mBeamEnergy[energy].c_str(),simmode.c_str(),v2.c_str());
  ...
  for(int i = 0; i < 4; i++)
  {
    for(int irho = 0; irho < 5; irho++)
    {
      for(int par = 0; par < 5; par++)
      {
        ...
        g2D_Global[irho][par][i]->SetMarkerStyle(20);
        g2D_Global[irho][par][i]->SetMarkerSize(1.2);
        g2D_Global[irho][par][i]->SetTitle(Form("%1.1f<p_{T}<%1.1f GeV/c",ptedges[i],ptedges[i+1]));
        g2D_Global[irho][par][i]->GetXaxis()->SetTitle(Form("%s^{Helicity} Input",param[irho].c_str()));
        g2D_Global[irho][par][i]->GetYaxis()->SetTitle(Form("%s^{Global} from 2D Fit",param[par].c_str()));
        g2D_Global[irho][par][i]->Draw("APE"); 
      }
    }
  ...
  outputname = Form("figures/%s/%s/pTstudy/HelicityvsHelicity_parametercomparison_%s_%s.pdf",vmsa::mPID[pid].c_str(),vmsa::mBeamEnergy[energy].c_str(),simmode.c_str(),v2.c_str());
  ...
  for(int i = 0; i < 4; i++)
  {
    for(int irho = 0; irho < 5; irho++)
    {
      for(int par = 0; par < 5; par++)
      {
        ...
        //cfit->cd(i+5)->SetLogx();
        //g2D[irho][par][i]->Print();
        g2D[irho][par][i]->SetMarkerStyle(20);
        g2D[irho][par][i]->SetMarkerSize(1.2);
        g2D[irho][par][i]->SetTitle(Form("%1.1f<p_{T}<%1.1f GeV/c",ptedges[i],ptedges[i+1]));
        g2D[irho][par][i]->GetXaxis()->SetTitle(Form("%s^{Helicity} Input",param[irho].c_str()));
        g2D[irho][par][i]->GetYaxis()->SetTitle(Form("%s^{Helicity} from 2D Fit",param[par].c_str()));
        g2D[irho][par][i]->Draw("APE"); 
      }
    }
    cfit2D->Print(outputname.c_str());
    cfit2D->Update();
