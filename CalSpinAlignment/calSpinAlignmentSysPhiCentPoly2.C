...
void calSpinAlignmentSysPhiCentPoly2(int energy = 4, int pid = 0, int year = 0, bool random3D = false, int centQA = 8)
{
  ...
  string inputfile = Form("../output/AuAu%s/%s/InvMassSubBgCent.root",vmsa::mBeamEnergy[energy].c_str(),vmsa::mPID[pid].c_str());
  ...
  for(int i_cent = vmsa::centStart; i_cent < vmsa::centStop; i_cent++) // Centrality loop
  {
    for(Int_t i_dca = vmsa::Dca_start; i_dca < vmsa::Dca_stop; i_dca++)
    {
      for(Int_t i_sig = vmsa::nSigKaon_start; i_sig < vmsa::nSigKaon_stop; i_sig++)
      {
        ...
        {
          string KEY_InteTheta = Form("Centrality_%d_2nd_Dca_%d_Sig_%d_%s_Norm_%d",i_cent,i_dca,i_sig,vmsa::mPID[pid].c_str(),i_norm);
          ...
          //h_mMass_InteTheta[KEY_InteTheta]->Rebin(5);
  ...
  for(int i_theta = 0; i_theta < 9; ++i_theta)
  {
    ...
    {
      string KEY_QA = Form("Centrality_%d_CosThetaStar_%d_2nd_Dca_%d_Sig_%d_%s_Norm_%d",centQA,i_theta,vmsa::Dca_start,vmsa::nSigKaon_start,vmsa::mPID[pid].c_str(),vmsa::Norm_QA);
    ...
    if(i_theta == vmsa::CTS_stop)
    {
      string KEY_InteTheta_QA = Form("Centrality_%d_2nd_Dca_%d_Sig_%d_%s_Norm_%d",centQA,vmsa::Dca_start,vmsa::nSigKaon_start,vmsa::mPID[pid].c_str(),vmsa::Norm_QA);
  ...
  */
          ...
          for(int i_method = vmsa::Method_start; i_method < vmsa::Method_stop; ++i_method)
          {
  	      ...
  	      string KEY_counts = Form("Centrality_%d_2nd_Dca_%d_Sig_%d_%s_Norm_%d_Sigma_%d_%s",i_cent,i_dca,i_sig,vmsa::mPID[pid].c_str(),i_norm,i_sigma,vmsa::mInteMethod[i_method].c_str());
              ...
              for(int i_theta = vmsa::CTS_start; i_theta < vmsa::CTS_stop; ++i_theta) // cos(theta*) loop
              {
                ...
                f_bw->SetParameter(5,Par_InteTheta[KEY_InteTheta][5]/7.0);
                f_bw->SetRange(vmsa::BW_Start[pid],vmsa::BW_Stop[pid]);
  ...
  for(int i_sigma = vmsa::Sig_start; i_sigma < vmsa::Sig_stop; ++i_sigma)
  {
    for(int i_method = vmsa::Method_start; i_method < vmsa::Method_stop; ++i_method)
    {
      string KEY_counts_QA = Form("Centrality_%d_2nd_Dca_%d_Sig_%d_%s_Norm_%d_Sigma_%d_%s",centQA,vmsa::Dca_start,vmsa::nSigKaon_start,vmsa::mPID[pid].c_str(),vmsa::Norm_QA,i_sigma,vmsa::mInteMethod[i_method].c_str());
  ...
  string outputfile = Form("../output/AuAu%s/%s/RawRhoCentSys.root",vmsa::mBeamEnergy[energy].c_str(),vmsa::mPID[pid].c_str());
