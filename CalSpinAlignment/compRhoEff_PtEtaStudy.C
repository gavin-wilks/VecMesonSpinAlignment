...
void compRhoEff_PtEtaStudy(Int_t energy = 3, Int_t pid = 0, std::string setting = "Raw", int order = 1)
{
  std::string ep = "";
  if(order == 1) ep = "_FirstOrder";
  string inputfileF0 = Form("../output/AuAu%s/%s/Rho_%sSysErrors_F_0_eta0p4%s.root",vmsa::mBeamEnergy[energy].c_str(),vmsa::mPID[pid].c_str(),setting.c_str(),ep.c_str());
  string inputfileF1 = Form("../output/AuAu%s/%s/Rho_%sSysErrors_F_0_eta0p6%s.root",vmsa::mBeamEnergy[energy].c_str(),vmsa::mPID[pid].c_str(),setting.c_str(),ep.c_str());
  string inputfileF2 = Form("../output/AuAu%s/%s/Rho_%sSysErrors_F_0_eta0p8%s.root",vmsa::mBeamEnergy[energy].c_str(),vmsa::mPID[pid].c_str(),setting.c_str(),ep.c_str());
  string inputfileF3 = Form("../output/AuAu%s/%s/Rho_%sSysErrors_F_0_eta1_eta1%s.root",vmsa::mBeamEnergy[energy].c_str(),vmsa::mPID[pid].c_str(),setting.c_str(),ep.c_str());
  ...
  string KEY_rho = Form("rhoRaw_2nd_Dca_%d_Sig_%d_%s_Norm_%d_Sigma_%d_%s",0,0,vmsa::mPID[pid].c_str(),0,0,vmsa::mInteMethod[1].c_str());
  string KEY_rhoStat = Form("rhoRawStat_2nd_Dca_%d_Sig_%d_%s_Norm_%d_Sigma_%d_%s",0,0,vmsa::mPID[pid].c_str(),0,0,vmsa::mInteMethod[1].c_str());
  string KEY_rhoSys  = Form("rhoRawSys_2nd_Dca_%d_Sig_%d_%s_Norm_%d_Sigma_%d_%s",0,0,vmsa::mPID[pid].c_str(),0,0,vmsa::mInteMethod[1].c_str());
...
//  PlotLine(0.0,5.0,1.0/3.0,1.0/3.0,1,2,2);
  ...
  Draw_TGAE_Point_new_Symbol(3.5,0.43,0.0,0.0,0.0,0.0,20,kBlue,1.3); 
  string leg_count0 = Form("#phi |#eta|<0.4");
  plotTopLegend((char*)leg_count0.c_str(),3.6,0.427,0.03,1,0.0,42,0);

  Draw_TGAE_Point_new_Symbol(3.5,0.41,0.0,0.0,0.0,0.0,20,kOrange+7,1.3); 
  string leg_count1 = Form("#phi |#eta|<0.6");
  plotTopLegend((char*)leg_count1.c_str(),3.6,0.407,0.03,1,0.0,42,0);

  Draw_TGAE_Point_new_Symbol(3.5,0.39,0.0,0.0,0.0,0.0,20,kGray+2,1.3); 
  string leg_count2 = Form("#phi |#eta|<0.8");
  plotTopLegend((char*)leg_count2.c_str(),3.6,0.387,0.03,1,0.0,42,0);

  Draw_TGAE_Point_new_Symbol(3.5,0.37,0.0,0.0,0.0,0.0,20,kBlack,1.3); 
  string leg_count3 = Form("#phi |#eta|<1.0");
  plotTopLegend((char*)leg_count3.c_str(),3.6,0.367,0.03,1,0.0,42,0);
  ...
  plotTopLegend((char*)leg_energy.c_str(),1.0,0.26,0.04,1,0.0,42,0);
  plotTopLegend((char*)"20%-60%",1.5,0.24,0.04,1,0.0,42,0);
