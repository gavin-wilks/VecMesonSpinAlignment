...
void calSpinAlignmentSysPhiPtIntegrated(int energy = 4, int pid = 0, int year = 0, bool random3D = false, int etaQA = 9, int ptQA = 1, int centQA = 2, int dcaQA = 0, int nsigQA = 0, int normQA = 0, int sigQA = 0, std::string etamode = "eta0p6")
{
  int i_cent = 9;
  ...
  string inputfile = Form("../output/AuAu%s/%s/InvMassSubBgPtEta_%s.root",vmsa::mBeamEnergy[energy].c_str(),vmsa::mPID[pid].c_str(),etamode.c_str());
  if(random3D) inputfile = Form("../output/AuAu%s/%s/3DRandom/InvMassSubBgEta.root",vmsa::mBeamEnergy[energy].c_str(),vmsa::mPID[pid].c_str());
  ...
  for(int i_eta = vmsa::y_rebin_first[energy]; i_eta <= vmsa::y_rebin_last[energy]; i_eta++) // pt loop
  {
...
//#endif
  ...
  */
        ...
        for(Int_t i_sig = vmsa::nSigKaon_start; i_sig < vmsa::nSigKaon_stop; i_sig++)
        {
		  ...
		  float pt_mean = (vmsa::pt_low[energy][i_pt]+vmsa::pt_up[energy][i_pt])/2.0;
                  ...
                  else
                  {
                    ...
                    Par_rhoFit[KEY_counts].push_back(static_cast<float>(f_rho->GetParameter(1)));
                    g_mRho[KEY_rho]->SetPoint(i_pt,pt_mean,-999.0);
                    g_mRho[KEY_rho]->SetPointError(i_pt,0.0,0.0,f_rho->GetParError(0),f_rho->GetParError(0));
                  }
  ...
  cout << "Created rho00 plots" << endl;
    ...
    for(int i_pt = vmsa::pt_rebin_first[energy]; i_pt <= vmsa::pt_rebin_last[energy]; ++i_pt) // pt loop
    {
      string outputname = Form("./figures/%s/%s/allThetaYields%s_cent%d_pt%d_%s_RapidityDependence.pdf",vmsa::mPID[pid].c_str(),vmsa::mBeamEnergy[energy].c_str(),vmsa::mPID[pid].c_str(),i_cent,i_pt,etamode.c_str());
      ...
      //for(int i_eta = vmsa::eta_start; i_eta < vmsa::eta_stop; i_eta++) // Rapidity bin loop
      {
        ...
        for(int i_theta = 0; i_theta < 9; ++i_theta)
        {
          ...
          if(i_theta < vmsa::CTS_stop)
          {
            string KEY_QA = Form("eta_%d_pt_%d_Centrality_%d_CosThetaStar_%d_2nd_Dca_%d_Sig_%d_%s_Norm_%d",i_eta,i_pt,i_cent,i_theta,vmsa::Dca_start,vmsa::nSigKaon_start,vmsa::mPID[pid].c_str(),vmsa::Norm_QA);
            h_mMass[KEY_QA]->SetTitle("");
            h_mMass[KEY_QA]->GetXaxis()->SetNdivisions(505,'N');
            h_mMass[KEY_QA]->GetXaxis()->SetLabelSize(0.03);
            h_mMass[KEY_QA]->GetXaxis()->SetTitle("M(K^{+},K^{-})");
            h_mMass[KEY_QA]->SetTitle(Form("%.2f < p_{T} < %.2f    %.2f < y < %.2f",vmsa::pt_low[energy][i_pt],vmsa::pt_up[energy][i_pt],vmsa::eta_bin[vmsa::y_rebin_start[energy][i_eta]],vmsa::eta_bin[vmsa::y_rebin_stop[energy][i_eta]]));
            h_mMass[KEY_QA]->GetXaxis()->SetTitleSize(0.05);
            h_mMass[KEY_QA]->GetXaxis()->SetTitleOffset(1.2);
            h_mMass[KEY_QA]->GetXaxis()->CenterTitle();
            ...
            string KEY_InteTheta = Form("eta_%d_pt_%d_Centrality_%d_2nd_Dca_%d_Sig_%d_%s_Norm_%d",i_eta,i_pt,i_cent,vmsa::Dca_start,vmsa::nSigKaon_start,vmsa::mPID[pid].c_str(),vmsa::Norm_QA);
          ...
          if(i_theta == vmsa::CTS_stop)
          {
            string KEY_InteTheta_QA = Form("eta_%d_pt_%d_Centrality_%d_2nd_Dca_%d_Sig_%d_%s_Norm_%d",i_eta,i_pt,i_cent,vmsa::Dca_start,vmsa::nSigKaon_start,vmsa::mPID[pid].c_str(),vmsa::Norm_QA);
            h_mMass_InteTheta[KEY_InteTheta_QA]->SetTitle("");
            h_mMass_InteTheta[KEY_InteTheta_QA]->GetXaxis()->SetNdivisions(505,'N');
            h_mMass_InteTheta[KEY_InteTheta_QA]->GetXaxis()->SetLabelSize(0.03);
            h_mMass_InteTheta[KEY_InteTheta_QA]->GetXaxis()->SetTitle("M(K^{+},K^{-})");
        ...
        for(int i_sigma = vmsa::Sig_start; i_sigma < /*vmsa::Sig_stop*/1; ++i_sigma)
        {
          ...
          {
            string KEY_counts_QA = Form("eta_%d_pt_%d_Centrality_%d_2nd_Dca_%d_Sig_%d_%s_Norm_%d_Sigma_%d_%s",i_eta,i_pt,i_cent,vmsa::Dca_start,vmsa::nSigKaon_start,vmsa::mPID[pid].c_str(),vmsa::Norm_QA,i_sigma,vmsa::mInteMethod[i_method].c_str());
...
//#endif
  ...
  string outputfile = Form("../output/AuAu%s/%s/RawRhoPtEtaSys_%s.root",vmsa::mBeamEnergy[energy].c_str(),vmsa::mPID[pid].c_str(),etamode.c_str());
  if(random3D) outputfile = Form("../output/AuAu%s/%s/3DRandom/RawRhoEtaSys.root",vmsa::mBeamEnergy[energy].c_str(),vmsa::mPID[pid].c_str());
