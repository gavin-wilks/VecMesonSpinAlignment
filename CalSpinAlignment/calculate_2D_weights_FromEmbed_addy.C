...
void calculate_2D_weights_FromEmbed_addy(int energy = 4, double inputv2 = 1000) 
{
    ...
    string spectra = "PhiEmbedding_noweights_nomccuts_rapidity";
    string filename = "PhiEmbedding_noweights_nomccuts_rapidity.root";
...
// new TH3F(flat.c_str(), Form("Flat pT-phi Distribution cent %d",icent), nBinsPt, ptMin, ptMax, nBinsY, yMin, yMax, nBinsPhi, phiMin, phiMax);
      ...
      //  {
      //    for(int k = 1; k <= nBinsPhi; k++)
      //    {
      //      double value = temp->GetBinContent(i+4, j, k);
      //      double error = temp->GetBinError(i+4, j, k); 
    ...
    TF1 *desired_pt[10];
    TF1 *v2_func[10];
    TF1 *ygaus = new TF1("ygaus","exp(-0.5*x*x/1.5/1.5)",-1.5,1.5);
    ...
    for (int i = 0; i < numberOfEvents; ++i) 
    {
        ...
        h_desired_pt_phi[cent]->Fill(pt_true, y_true, phi_true);
    }
