...
void calSysError_FixedSys_Poly200_AllOneBinFromStart(Int_t energy = 6, Int_t pid = 0, Int_t i_cent = 9, string correction = "Raw", bool random3D = false, int order = 2, int defaultF = 0, string etamode = "eta1_eta1", int yspectra = -1)//defaultF = 0 is BESII, defaultF = 1 is BESI
{
  ...
  string inputfileHframe = Form("../output/AuAu%s/%s/OneBigBinFromStart_RawPhiPtSys_eta1_eta1_PolySys.root",vmsa::mBeamEnergy[energy].c_str(),vmsa::mPID[pid].c_str());
  ...
  string inputfile = Form("../output/AuAu%s/%s/OneBigBinFromStart_%sPhiPtSys_%s_PolySys%s.root",vmsa::mBeamEnergy[energy].c_str(),vmsa::mPID[pid].c_str(),correction.c_str(),etamode.c_str(),spectra.c_str());
  ...
  for(Int_t i_point = 0; i_point < vmsa::pt_rebin_last[energy]; ++i_point)
  {
   ...
   // weight = PtCos->Integral(1,7);
    ...
    for(int i_poly = 0; i_poly < 1/*2*/; ++i_poly)
    {
      for(int i_sigma = vmsa::Sig_start; i_sigma < 1/*vmsa::Sig_stop*/; ++i_sigma)
      {
        for(int i_method = 1/*vmsa::Method_start*/; i_method < vmsa::Method_stop; ++i_method)
  ...
  TH1F *hint = new TH1F("hint","Au+Au 200 GeV p_{T} Integrated #rho_{00}, 1.2<p_{T}<5.4 GeV/c",nx,0,float(nx));
  hint->SetStats(0);
  ...
  hint->Draw("pEX0");
  c1->SaveAs(Form("figures/%s/%s/pTstudy/OneBigBinFromStart_AUAU200GeV_Integrated.pdf",vmsa::mPID[pid].c_str(),vmsa::mBeamEnergy[energy].c_str()));  
  ...
  TH1F *hint2 = new TH1F("hint2","Au+Au 200GeV Run18 1.2<p_{T}<5.4 GeV/c, Fixed Width",nx2,0,float(nx2));
  hint2->SetStats(0);
  //hint->SetFillColor(38);
  ...
  hint2->Draw("pEX0");
  c12->SaveAs(Form("figures/%s/%s/pTstudy/OneBigBinFromStart_AUAU200GeV_Integrated_Run18Comp.pdf",vmsa::mPID[pid].c_str(),vmsa::mBeamEnergy[energy].c_str()));  
...
/////////////////////////////////////////////////////////////////////////  
  ...
  TH1F *hintd = new TH1F("hintd","Au+Au 200 Run 18 GeV p_{T} Integrated #rho_{00} Difference, 1.2<p_{T}<5.4 GeV/c",nxr,0,float(nxr));
  hintd->SetStats(0);
  //hint->SetFillColor(38);
  ...
  c1->SaveAs(Form("figures/%s/%s/pTstudy/OneBigBinFromStart_AUAU200GeV_Integrated_Run18Difference.pdf",vmsa::mPID[pid].c_str(),vmsa::mBeamEnergy[energy].c_str()));  
}
