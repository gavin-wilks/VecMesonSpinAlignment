...
void calSpinAlignment_Raw(int energy = 4, int pid = 0, int year = 0)
{
...
/*  TCanvas *c_diff = new TCanvas("c_diff","c_diff",10,10,900,900);
  ...
  for(int i_theta = 0; i_theta < 9; ++i_theta)
  {
    ...
    {
      string KEY_QA = Form("pt_%d_Centrality_%d_CosThetaStar_%d_2nd_Dca_%d_Sig_%d_%s_Norm_%d",2,9,i_theta,0,1,vmsa::mPID[pid].c_str(),vmsa::Norm_QA);
    ...
    if(i_theta == vmsa::CTS_stop)
    {
      string KEY_InteTheta_QA = Form("pt_%d_Centrality_%d_2nd_Dca_%d_Sig_%d_%s_Norm_%d",2,9,0,1,vmsa::mPID[pid].c_str(),vmsa::Norm_QA);
  ...
  */
  ...
  for(int i_pt = vmsa::pt_rebin_first[energy]; i_pt < vmsa::pt_rebin_last[energy]; i_pt++) // pt loop
  {
	...
	for(Int_t i_sig = 1; i_sig < 2; i_sig++)
	{
	  ...
	  { 
	    ...
	    {
	      string KEY = Form("pt_%d_Centrality_%d_CosThetaStar_%d_2nd_Dca_%d_Sig_%d_%s_Norm_%d",i_pt,i_cent,i_theta,i_dca,i_sig,vmsa::mPID[pid].c_str(),i_norm);
	      if(i_theta == vmsa::CTS_start) h_mMass_total[KEY_InteTheta] = (TH1F*)h_mMass[KEY]->Clone();
	      else h_mMass_total[KEY_InteTheta]->Add(h_mMass[KEY],1.0);
	    }
	    TF1 *f_bw = new TF1("f_bw",BreitWigner,vmsa::BW_Start[pid],vmsa::BW_Stop[pid],3);
	    f_bw->SetParameter(0,vmsa::InvMass[pid]);
	    f_bw->SetParLimits(0,vmsa::InvMass[pid]-0.001,vmsa::InvMass[pid]+0.001);
	    f_bw->SetParameter(1,vmsa::Width[pid]);
	    f_bw->SetParameter(2,1.0);
	    float norm = h_mMass_total[KEY_InteTheta]->GetMaximum()/f_bw->GetMaximum();
	    f_bw->SetParameter(2,norm);
	    f_bw->SetRange(vmsa::BW_Start[pid],vmsa::BW_Stop[pid]);
	    h_mMass_total[KEY_InteTheta]->Fit(f_bw,"MQNR");
	    ParBW[KEY_InteTheta].clear();
	    ParBW[KEY_InteTheta].push_back(static_cast<float>(f_bw->GetParameter(0)));
	    ParBW[KEY_InteTheta].push_back(static_cast<float>(f_bw->GetParameter(1)));
            ParBW[KEY_InteTheta].push_back(static_cast<float>(f_bw->GetParameter(2)));
          }
	}
      }
    }
  ...
  for(int i_theta = 0; i_theta < 9; ++i_theta)
  {
    ...
    {
      string KEY_InteTheta_QA = Form("pt_%d_Centrality_%d_2nd_Dca_%d_Sig_%d_%s_Norm_%d",2,9,0,1,vmsa::mPID[pid].c_str(),vmsa::Norm_QA);
      h_mMass_total[KEY_InteTheta_QA]->SetTitle("");
      h_mMass_total[KEY_InteTheta_QA]->GetXaxis()->SetNdivisions(505,'N');
      h_mMass_total[KEY_InteTheta_QA]->GetXaxis()->SetLabelSize(0.03);
      h_mMass_total[KEY_InteTheta_QA]->GetXaxis()->SetTitle("M(K^{+},K^{-})");
      h_mMass_total[KEY_InteTheta_QA]->GetXaxis()->SetTitleSize(0.05);
      h_mMass_total[KEY_InteTheta_QA]->GetXaxis()->SetTitleOffset(1.2);
      h_mMass_total[KEY_InteTheta_QA]->GetXaxis()->CenterTitle();

      h_mMass_total[KEY_InteTheta_QA]->GetYaxis()->SetRangeUser(h_mMass_total[KEY_InteTheta_QA]->GetMinimum(),1.1*h_mMass_total[KEY_InteTheta_QA]->GetMaximum());
      h_mMass_total[KEY_InteTheta_QA]->GetYaxis()->SetNdivisions(505,'N');
      h_mMass_total[KEY_InteTheta_QA]->GetYaxis()->SetTitle("Yields");
      h_mMass_total[KEY_InteTheta_QA]->GetYaxis()->SetTitleSize(0.05);
      h_mMass_total[KEY_InteTheta_QA]->GetYaxis()->SetLabelSize(0.03);
      h_mMass_total[KEY_InteTheta_QA]->GetYaxis()->CenterTitle();

      h_mMass_total[KEY_InteTheta_QA]->SetMarkerStyle(24);
      h_mMass_total[KEY_InteTheta_QA]->SetMarkerColor(kGray+2);
      h_mMass_total[KEY_InteTheta_QA]->SetMarkerSize(1.2);
      h_mMass_total[KEY_InteTheta_QA]->Draw("pE");
      PlotLine(vmsa::InvMass_low[pid],vmsa::InvMass_high[pid],0.0,0.0,1,2,2);
      TF1 *f_bw = new TF1("f_bw",BreitWigner,vmsa::BW_Start[pid],vmsa::BW_Stop[pid],3);
      f_bw->SetParameter(0,ParBW[KEY_InteTheta_QA][0]);
      f_bw->SetParameter(1,ParBW[KEY_InteTheta_QA][1]);
      f_bw->SetParameter(2,ParBW[KEY_InteTheta_QA][2]);
      f_bw->SetLineColor(2);
      f_bw->SetLineStyle(2);
      f_bw->SetLineWidth(2);
      f_bw->Draw("l same");
	...
	for(int i_norm = vmsa::Norm_start; i_norm < vmsa::Norm_stop; ++i_norm)
	{
	  for(int i_sigma = vmsa::Sig_start; i_sigma < 1; ++i_sigma)
	  {
		...
		for(int i_theta = vmsa::CTS_start; i_theta < vmsa::CTS_stop; ++i_theta) // cos(theta*) loop
		{
		  string KEY = Form("pt_%d_Centrality_%d_CosThetaStar_%d_2nd_Dca_%d_Sig_%d_%s_Norm_%d",i_pt,i_cent,i_theta,i_dca,i_sig,vmsa::mPID[pid].c_str(),i_norm);
		  TF1 *f_bw = new TF1("f_bw",BreitWigner,vmsa::BW_Start[pid],vmsa::BW_Stop[pid],3);
		  f_bw->FixParameter(0,ParBW[KEY_InteTheta][0]);
		  f_bw->FixParameter(1,ParBW[KEY_InteTheta][1]);
		  f_bw->SetParameter(2,ParBW[KEY_InteTheta][2]/7.0);
		  f_bw->SetRange(vmsa::BW_Start[pid],vmsa::BW_Stop[pid]);
		  h_mMass[KEY]->Fit(f_bw,"MQNR");
		  ...
		  if(i_method == 0)
		  {
		    int bin_start = h_mMass[KEY]->FindBin(ParBW[KEY_InteTheta][0]-vmsa::nSigVecSys[i_sigma]*ParBW[KEY_InteTheta][1]);
		    int bin_stop  = h_mMass[KEY]->FindBin(ParBW[KEY_InteTheta][0]+vmsa::nSigVecSys[i_sigma]*ParBW[KEY_InteTheta][1]);
		    float counts = 0.0;
		    float errors = 0.0;
		  ...
		  if(i_method == 1)
		  {
		    ...
		    float Inte_start = ParBW[KEY_InteTheta][0]-vmsa::nSigVecSys[i_sigma]*ParBW[KEY_InteTheta][1]-0.5*bin_width;
		    float Inte_stop  = ParBW[KEY_InteTheta][0]+vmsa::nSigVecSys[i_sigma]*ParBW[KEY_InteTheta][1]+0.5*bin_width;
  ...
  for(int i_theta = vmsa::CTS_start; i_theta < vmsa::CTS_stop; ++i_theta)
  {
    ...
    string KEY_theta_QA = Form("pt_%d_Centrality_%d_2nd_Dca_%d_Sig_%d_%s_Norm_%d",2,9,0,1,vmsa::mPID[pid].c_str(),vmsa::Norm_QA);
  ...
  for(int i_sigma = vmsa::Sig_start; i_sigma < 1; ++i_sigma)
  {
    for(int i_method = vmsa::Method_start; i_method < 1; ++i_method)
    {
      string KEY_counts_QA = Form("pt_%d_Centrality_%d_2nd_Dca_%d_Sig_%d_%s_Norm_%d_Sigma_%d_%s",2,9,0,1,vmsa::mPID[pid].c_str(),vmsa::Norm_QA,i_sigma,vmsa::mInteMethod[i_method].c_str());
  ...
  c_diff->SaveAs("../figures/c_diff_2_LinearSub.pdf");
#endif
  ...
  for(int i_cent = vmsa::Cent_start; i_cent < vmsa::Cent_stop; ++i_cent) // Centrality loop
  {
	...
	for(int i_norm = vmsa::Norm_start; i_norm < vmsa::Norm_stop; ++i_norm)
	{
	  for(int i_sigma = vmsa::Sig_start; i_sigma < 1; ++i_sigma)
	  {
	    for(int i_method = vmsa::Method_start; i_method < 0; ++i_method)
	    {
              c_rho->cd()->Clear();
              c_rho->cd();
	      ...
	      Draw_TGAE_new_Symbol((TGraphAsymmErrors*)g_mRho[KEY_rho],24,i_sigma+10*i_method+1,1.1);
              ...
              c_rho->SaveAs(Form("../figures/c_rho_cent%d_norm%d_%s_LinearSub.pdf",i_cent,i_norm,vmsa::mInteMethod[i_method].c_str()));
	    }
	  }
	}
      }
