#include <string>
#include "TFile.h"
#include "TCanvas.h"
#include "TH1F.h"
#include "TH1D.h"
#include "TGraphAsymmErrors.h"
#include "TLegend.h"
#include "TF1.h"
#include "TStyle.h"
#include "../Utility/StSpinAlignmentCons.h"
#include "../Utility/type.h"
#include "../Utility/draw.h"
#include "../Utility/functions.h"

#ifndef _PlotQA_
#define _PlotQA_ 1
#endif

void plotSysErrorsBox(TGraphAsymmErrors *g_rho, int plot_color, int beamE);
void plotSysErrorsBox(TGraphAsymmErrors *g_rho, int plot_color);

void centEtaDependence(Int_t energy = 4, Int_t pid = 0, std::string setting = "AccRes")
{
  TH1F *h_frame = new TH1F("h_frame","h_frame",100,-0.05,10.05);
  for(int i_bin = 0; i_bin < 100; ++i_bin)
  {
    h_frame->SetBinContent(i_bin+1,-10.0);
    h_frame->SetBinError(i_bin+1,1.0);
  }
  h_frame->SetTitle("");
  h_frame->SetStats(0);
  h_frame->GetXaxis()->SetRangeUser(0.2,1.2);
  h_frame->GetXaxis()->SetNdivisions(505,'N');
  h_frame->GetXaxis()->SetLabelSize(0.03);
  h_frame->GetXaxis()->SetTitle("|#eta| cut");
  h_frame->GetXaxis()->SetTitleSize(0.05);
  h_frame->GetXaxis()->SetTitleOffset(1.2);
  h_frame->GetXaxis()->CenterTitle();

  h_frame->GetYaxis()->SetRangeUser(0.32,0.4);
  h_frame->GetYaxis()->SetNdivisions(505,'N');
  h_frame->GetYaxis()->SetTitle("#rho_{00}");
  h_frame->GetYaxis()->SetTitleSize(0.05);
  h_frame->GetYaxis()->SetLabelSize(0.03);
  h_frame->GetYaxis()->CenterTitle();


  TGraphAsymmErrors *g_before = new TGraphAsymmErrors();
  TGraphAsymmErrors *g_after = new TGraphAsymmErrors();
  TGraphAsymmErrors *g_before1 = new TGraphAsymmErrors();
  TGraphAsymmErrors *g_after1 = new TGraphAsymmErrors();
  TGraphAsymmErrors *g_beforePt = new TGraphAsymmErrors();
  TGraphAsymmErrors *g_afterPt = new TGraphAsymmErrors();
  TGraphAsymmErrors *g_beforePt27 = new TGraphAsymmErrors();
  TGraphAsymmErrors *g_afterPt27 = new TGraphAsymmErrors();

  float eta[4]       = {1.0,0.8,0.6,0.4};

  // 2nd Order 
  float before[4]    = {0.3399,0.3441,0.3545,0.3810}; // 0-80%
  float beforeErr[4] = {0.0008,0.0009,0.0011,0.0014}; // 0-80%
  //float before[4]    = {0.3417,0.3458,0.3560,0.3825}; // 20-60%
  //float beforeErr[4] = {0.0010,0.0011,0.0013,0.0018}; // 20-60%
  float after[4]     = {0.3494,0.3427,0.3370,0.3374}; // v2 on and pt on, Eff then Acc+Res correction 0-80%
  float afterErr[4]  = {0.0015,0.0016,0.0019,0.0025}; // v2 on and pt on, Eff then Acc+Res correction 0-80%
  //float after[4]     = {0.3512,0.3446,0.3392,0.3383}; // v2 on and pt on, Eff then Acc+Res correction 20-60%
  //float afterErr[4]  = {0.0020,0.0021,0.0025,0.0033}; // v2 on and pt on, Eff then Acc+Res correction 20-60%

  float beforePt[4]    = {0.3431,0.3460,0.3553,0.3755};//20-60%  
  float beforeErrPt[4] = {0.0012,0.0013,0.0015,0.0019};//20-60%
  float afterPt[4]     = {0.3525,0.3469,0.3422,0.3374};//20-60%
  float afterErrPt[4]  = {0.0024,0.0025,0.0030,0.0038};//20-60%
  float beforePt27[4]    = {0.435542, 0.392083, 0.37229, 0.36549}; //20-60%  
  float beforeErrPt27[4] = {0.00483134, 0.00374077, 0.00346523, 0.00346575}; //20-60%
  float afterPt27[4]     = {0.35206,0.35314,0.35566,0.36080}; //20-60%
  float afterErrPt27[4]  = {0.00492,0.00383,0.00356,0.00364}; //20-60%

  // 1st Order 0-80% 
  float before1[4]    = {0.3375,0.3411,0.3520,0.3778}; // 0-80%
  float beforeErr1[4] = {0.0007,0.0008,0.0009,0.0012}; // 0-80%
  float after1[4]     = {0.3470,0.3386,0.3322,0.3324}; // v2 on and pt on, Eff then Acc+Res correction 0-80%
  float afterErr1[4]  = {0.0019,0.0026,0.0024,0.0031}; // v2 on and pt on, Eff then Acc+Res correction 0-80%
  // 1st Order 20-60% 
  //float before1[4]    = {0.3389,0.3422,0.3536,0.3785}; // 20-60%
  //float beforeErr1[4] = {0.0010,0.0011,0.0012,0.0016}; // 20-60%
  //float after1[4]     = {0.3488,0.3409,0.3352,0.3330}; // v2 on and pt on, Eff then Acc+Res correction 20-60%
  //float afterErr1[4]  = {0.0024,0.0026,0.0030,0.0039}; // v2 on and pt on, Eff then Acc+Res correction 20-60%

  for( int i = 0; i < 4; i++ )
  {
    g_after->SetPoint(i, eta[i], after[i]);
    g_after->SetPointError(i, 0.0, 0.0, afterErr[i], afterErr[i]);
    g_before->SetPoint(i, eta[i], before[i]);
    g_before->SetPointError(i, 0.0, 0.0, beforeErr[i], beforeErr[i]);
    g_after1->SetPoint(i, eta[i], after1[i]);
    g_after1->SetPointError(i, 0.0, 0.0, afterErr1[i], afterErr1[i]);
    g_before1->SetPoint(i, eta[i], before1[i]);
    g_before1->SetPointError(i, 0.0, 0.0, beforeErr1[i], beforeErr1[i]);
    g_afterPt->SetPoint(i, eta[i], afterPt[i]);
    g_afterPt->SetPointError(i, 0.0, 0.0, afterErrPt[i], afterErrPt[i]);
    g_beforePt->SetPoint(i, eta[i], beforePt[i]);
    g_beforePt->SetPointError(i, 0.0, 0.0, beforeErrPt[i], beforeErrPt[i]);
    g_afterPt27->SetPoint(i, eta[i], afterPt27[3-i]);
    g_afterPt27->SetPointError(i, 0.0, 0.0, afterErrPt27[3-i], afterErrPt27[3-i]);
    g_beforePt27->SetPoint(i, eta[i], beforePt27[3-i]);
    g_beforePt27->SetPointError(i, 0.0, 0.0, beforeErrPt27[3-i], beforeErrPt27[3-i]);
  }


  TCanvas *c_rho_SysError = new TCanvas("c_rho_SysError","c_rho_SysError",600,10,800,800);
  c_rho_SysError->cd();
  c_rho_SysError->cd()->SetLeftMargin(0.15);
  c_rho_SysError->cd()->SetBottomMargin(0.15);
  c_rho_SysError->cd()->SetTicks(1,1);
  c_rho_SysError->cd()->SetGrid(0,0);
  h_frame->Draw("pE");

  PlotLine(0.2,1.2,1.0/3.0,1.0/3.0,1,2,2);

  g_before->SetMarkerStyle(20);
  g_before->SetMarkerColor(kOrange+7);
  g_before->SetLineColor(kOrange+7);
  g_before->SetMarkerSize(1.3);
  g_before->Draw("pE same");
  
  g_after->SetMarkerStyle(20);
  g_after->SetMarkerColor(kBlue);
  g_after->SetLineColor(kBlue);
  g_after->SetMarkerSize(1.3);
  g_after->Draw("pE same");

  TLegend *leg = new TLegend(0.5,0.6,0.8,0.8);
  leg->AddEntry(g_before,"Before Corrections","p");
  leg->AddEntry(g_after,"After Corrections","p");
  leg->Draw("same");
  
  c_rho_SysError->SaveAs("figures/Phi/19GeV/centEtaDependence.pdf");   

  TCanvas *c_vs = new TCanvas("c_vs","c_vs",600,10,800,800);
  c_vs->cd();
  c_vs->cd()->SetLeftMargin(0.15);
  c_vs->cd()->SetBottomMargin(0.15);
  c_vs->cd()->SetTicks(1,1);
  c_vs->cd()->SetGrid(0,0);
  h_frame->SetTitle("After Corrections");
  h_frame->DrawCopy("pE");

  PlotLine(0.2,1.2,1.0/3.0,1.0/3.0,1,2,2);

  g_after->SetMarkerStyle(20);
  g_after->SetMarkerColor(kOrange+7);
  g_after->SetLineColor(kOrange+7);
  g_after->SetMarkerSize(1.3);
  g_after->Draw("pE same");

  g_afterPt->SetMarkerStyle(20);
  g_afterPt->SetMarkerColor(kBlue);
  g_afterPt->SetLineColor(kBlue);
  g_afterPt->SetMarkerSize(1.3);
  g_afterPt->Draw("pE same");

  TLegend *leg2 = new TLegend(0.4,0.6,0.85,0.85);
  leg2->AddEntry(g_after,"Centrality Integrated Value, 1.0<p_{T}<5.0Gev/c","p");
  leg2->AddEntry(g_afterPt,"p_{T} Integrated Value, 1.2<p_{T}<4.2Gev/c","p");
  leg2->Draw("same");

  c_vs->SaveAs("figures/Phi/19GeV/centEtaDependence_and_ptEtaDependence_after.pdf");   


  TCanvas *c_vsb = new TCanvas("c_vsb","c_vsb",600,10,800,800);
  c_vsb->cd();
  c_vsb->cd()->SetLeftMargin(0.15);
  c_vsb->cd()->SetBottomMargin(0.15);
  c_vsb->cd()->SetTicks(1,1);
  c_vsb->cd()->SetGrid(0,0);
  h_frame->SetTitle("Before Corrections");
  h_frame->DrawCopy("pE");

  PlotLine(0.2,1.2,1.0/3.0,1.0/3.0,1,2,2);

  g_before->SetMarkerStyle(20);
  g_before->SetMarkerColor(kOrange+7);
  g_before->SetLineColor(kOrange+7);
  g_before->SetMarkerSize(1.3);
  g_before->Draw("pE same");

  g_beforePt->SetMarkerStyle(20);
  g_beforePt->SetMarkerColor(kBlue);
  g_beforePt->SetLineColor(kBlue);
  g_beforePt->SetMarkerSize(1.3);
  g_beforePt->Draw("pE same");

  TLegend *leg3 = new TLegend(0.4,0.6,0.85,0.85);
  leg3->AddEntry(g_before,"Centrality Integrated Value, 1.0<p_{T}<5.0Gev/c","p");
  leg3->AddEntry(g_beforePt,"p_{T} Integrated Value, 1.2<p_{T}<4.2Gev/c","p");
  leg3->Draw("same");

  c_vs->SaveAs("figures/Phi/19GeV/centEtaDependence_and_ptEtaDependence_before.pdf");   
  
  TCanvas *c_vs27 = new TCanvas("c_vs27","c_vs27",600,10,800,800);
  c_vs27->cd();
  c_vs27->cd()->SetLeftMargin(0.15);
  c_vs27->cd()->SetBottomMargin(0.15);
  c_vs27->cd()->SetTicks(1,1);
  c_vs27->cd()->SetGrid(0,0);
  h_frame->SetTitle("After Corrections");
  h_frame->DrawCopy("pE");

  PlotLine(0.2,1.2,1.0/3.0,1.0/3.0,1,2,2);

  g_afterPt27->SetMarkerStyle(20);
  g_afterPt27->SetMarkerColor(kOrange+7);
  g_afterPt27->SetLineColor(kOrange+7);
  g_afterPt27->SetMarkerSize(1.3);
  g_afterPt27->Draw("pE same");

  g_after->SetMarkerStyle(20);
  g_after->SetMarkerColor(kBlue);
  g_after->SetLineColor(kBlue);
  g_after->SetMarkerSize(1.3);
  g_after->Draw("pE same");

  TLegend *leg27 = new TLegend(0.4,0.6,0.85,0.85);
  leg27->AddEntry(g_afterPt27,  "27   GeV (Jinhui), 1.2<p_{T}<5.4Gev/c","p");
  leg27->AddEntry(g_after    ,  "19.6 GeV (Gavin) , 1.0<p_{T}<5.0Gev/c","p");
  leg27->Draw("same");

  c_vs27->SaveAs("figures/Phi/19GeV/jinhui27vsgavin19_after.pdf");   


  TCanvas *c_vsbo = new TCanvas("c_vsbo","c_vsbo",600,10,800,800);
  c_vsbo->cd();
  c_vsbo->cd()->SetLeftMargin(0.15);
  c_vsbo->cd()->SetBottomMargin(0.15);
  c_vsbo->cd()->SetTicks(1,1);
  c_vsbo->cd()->SetGrid(0,0);
  h_frame->SetTitle("Before Corrections");
  h_frame->DrawCopy("pE");

  PlotLine(0.2,1.2,1.0/3.0,1.0/3.0,1,2,2);

  g_before1->SetMarkerStyle(20);
  g_before1->SetMarkerColor(kOrange+7);
  g_before1->SetLineColor(kOrange+7);
  g_before1->SetMarkerSize(1.3);
  g_before1->Draw("pE same");

  g_before->SetMarkerStyle(20);
  g_before->SetMarkerColor(kBlue);
  g_before->SetLineColor(kBlue);
  g_before->SetMarkerSize(1.3);
  g_before->Draw("pE same");

  TLegend *leg12 = new TLegend(0.4,0.6,0.85,0.85);
  leg12->AddEntry(g_before1,"1^{st} Order Centrality Integrated","p");
  leg12->AddEntry(g_before, "2^{nd} Order Centrality Integrated","p");
  leg12->Draw("same");

  c_vsbo->SaveAs("figures/Phi/19GeV/centOrderComparison_before.pdf");   
  
  TCanvas *c_vsao = new TCanvas("c_vsao","c_vsao",600,10,800,800);
  c_vsao->cd();
  c_vsao->cd()->SetLeftMargin(0.15);
  c_vsao->cd()->SetBottomMargin(0.15);
  c_vsao->cd()->SetTicks(1,1);
  c_vsao->cd()->SetGrid(0,0);
  h_frame->SetTitle("After Corrections");
  h_frame->DrawCopy("pE");

  PlotLine(0.2,1.2,1.0/3.0,1.0/3.0,1,2,2);

  g_after1->SetMarkerStyle(20);
  g_after1->SetMarkerColor(kOrange+7);
  g_after1->SetLineColor(kOrange+7);
  g_after1->SetMarkerSize(1.3);
  g_after1->Draw("pE same");

  g_after->SetMarkerStyle(20);
  g_after->SetMarkerColor(kBlue);
  g_after->SetLineColor(kBlue);
  g_after->SetMarkerSize(1.3);
  g_after->Draw("pE same");

  TLegend *leg12a = new TLegend(0.4,0.6,0.85,0.85);
  leg12a->AddEntry(g_after1,"1^{st} Order Centrality Integrated","p");
  leg12a->AddEntry(g_after, "2^{nd} Order Centrality Integrated","p");
  leg12a->Draw("same");

  c_vsao->SaveAs("figures/Phi/19GeV/centOrderComparison_after.pdf");   


  /*for(int i_pt = vmsa::pt_rebin_first_y[energy]; i_pt <= vmsa::pt_rebin_last_y[energy]; ++i_pt) // pt loop
  {
    for(int i_cent = 0; i_cent < vmsa::cent_rebin_total; i_cent++) // Centrality loop
    {
      c_pt->cd(i_pt*3+i_cent+1);
      c_pt->cd(i_pt*3+i_cent+1)->SetLeftMargin(0.15);
      c_pt->cd(i_pt*3+i_cent+1)->SetBottomMargin(0.15);
      c_pt->cd(i_pt*3+i_cent+1)->SetTicks(1,1);
      c_pt->cd(i_pt*3+i_cent+1)->SetGrid(0,0);
      h_frame->DrawCopy("pE");
      PlotLine(-1.5,1.5,1.0/3.0,1.0/3.0,1,2,2);
      //Draw_TGAE_Point_new_Symbol(-0.1,0.46,0.0,0.0,0.0,0.0,style_phi_2nd,color_phi_2nd,size_marker+0.2); 

      //plotTopLegend((char*)leg_count.c_str(),0.0,0.457,0.03,1,0.0,42,0);

      //PlotLine(-0.25,-0.05,0.438,0.438,1,2,2);
      //plotTopLegend((char*)leg_line.c_str(),0.0,0.437,0.03,1,0.0,42,0);

      //plotTopLegend((char*)leg_energy.c_str(),-0.3,0.305,0.04,1,0.0,42,0);
      //plotTopLegend((char*)centStrings[i_cent].c_str(),-0.1,0.295,0.04,1,0.0,42,0);
      //plotTopLegend((char*)Form("%.2f<p_{T}<%.2f GeV/c",vmsa::pt_low_y[energy][i_pt],vmsa::pt_up_y[energy][i_pt]),-0.3,0.285,0.04,1,0.0,42,0);


      string KEY_Default = Form("rhoRaw_pt_%d_Centrality_%d_2nd_Dca_%d_Sig_%d_%s_Norm_%d_Sigma_%d_%s",i_pt,i_cent,0,0,vmsa::mPID[pid].c_str(),0,0,vmsa::mInteMethod[1].c_str());

      Draw_TGAE_new_Symbol((TGraphAsymmErrors*)g_mRhoF0_Stat[KEY_Default],20,kBlue,0.0,1.0);
      Draw_TGAE_new_Symbol((TGraphAsymmErrors*)g_mRhoF1_Stat[KEY_Default],20,kOrange+7,0.0,1.0);
      Draw_TGAE_new_Symbol((TGraphAsymmErrors*)g_mRhoF2_Stat[KEY_Default],20,kGray+2,0.0,1.0);
      Draw_TGAE_new_Symbol((TGraphAsymmErrors*)g_mRhoF3_Stat[KEY_Default],20,kBlack,0.0,1.0);

      plotSysErrorsBox(g_mRhoF0_Sys[KEY_Default],kBlue);     
      plotSysErrorsBox(g_mRhoF1_Sys[KEY_Default],kOrange+7);     
      plotSysErrorsBox(g_mRhoF2_Sys[KEY_Default],kGray+2);     
      plotSysErrorsBox(g_mRhoF3_Sys[KEY_Default],kBlack);     

    }
  }
  c_pt->Update();
  c_pt->Print(outputname.c_str());
  string output_stop = Form("%s]",outputname.c_str());
  c_pt->Print(output_stop.c_str()); // close pdf file
  */

}


void plotSysErrorsBox(TGraphAsymmErrors *g_rho, int plot_color, int beamE)
{
  const int nEnergy = g_rho->GetN();
  TBox *bSys[nEnergy];
  for(int i_energy = 0; i_energy < g_rho->GetN(); ++i_energy) // plot sys errors
  {
    double energy, rho;
    g_rho->GetPoint(i_energy,energy,rho);
    double err = g_rho->GetErrorYhigh(i_energy);
    
    //bSys[i_energy] = new TBox(energy/1.08,rho-err,energy*1.08,rho+err);
    bSys[i_energy] = new TBox(vmsa::pt_low[beamE][i_energy],rho-err,vmsa::pt_up[beamE][i_energy],rho+err);
    bSys[i_energy]->SetFillColor(0);
    bSys[i_energy]->SetFillStyle(0);
    bSys[i_energy]->SetLineStyle(1);
    bSys[i_energy]->SetLineWidth(1);
    bSys[i_energy]->SetLineColor(plot_color);
    bSys[i_energy]->Draw("l Same");
  }
}


void plotSysErrorsBox(TGraphAsymmErrors *g_rho, int plot_color)
{
  const int nPt = g_rho->GetN();
  TBox *bSys[nPt];
  for(int i_pt = 0; i_pt < g_rho->GetN(); ++i_pt) // plot sys errors
  {
    double pt, rho;
    g_rho->GetPoint(i_pt,pt,rho);
    double err = g_rho->GetErrorYhigh(i_pt);

    bSys[i_pt] = new TBox(pt-0.08,rho-err,pt+0.08,rho+err);
    bSys[i_pt]->SetFillColor(0);
    bSys[i_pt]->SetFillStyle(0);
    bSys[i_pt]->SetLineStyle(1);
    bSys[i_pt]->SetLineWidth(1);
    bSys[i_pt]->SetLineColor(plot_color);
    bSys[i_pt]->Draw("l Same");
  }
}
