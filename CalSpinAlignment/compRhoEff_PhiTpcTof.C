...
void compRhoEff_PhiTpcTof(Int_t energy = 4, Int_t pid = 0, Int_t i_cent = 9, string correction = "Raw")
{
  ...
  string inputfile1 = Form("../output/AuAu%s/%s/Poly/RawPhiPtSys_%s_Poly.root",vmsa::mBeamEnergy[energy].c_str(),vmsa::mPID[pid].c_str(),"eta1_eta1");
  ...
  string inputfile2 = Form("../output/AuAu%s/%s/Poly/RawPhiPtSys_%s_Poly_TPCOnly.root",vmsa::mBeamEnergy[energy].c_str(),vmsa::mPID[pid].c_str(),"eta1_eta1");

  TFile *File_InPut1 = TFile::Open(inputfile1.c_str());
  TFile *File_InPut2 = TFile::Open(inputfile2.c_str());
  
  TString StatErrorRho = Form("rhoRaw_Centrality_%d_2nd_Dca_%d_Sig_%d_%s_Norm_%d_Sigma_%d_%s_Poly%d",9,0,0,vmsa::mPID[pid].c_str(),0,1,vmsa::mInteMethod[1].c_str(),3);
  ...
  h_frame->GetYaxis()->SetRangeUser(0.27,0.42);
...
//  PlotLine(0.0,5.0,1.0/3.0,1.0/3.0,1,2,2);
  ...
  c_rho_SysError->SaveAs(Form("figures/tpctofcomp_phi_pt_%s.pdf",vmsa::mBeamEnergy[energy].c_str()));
