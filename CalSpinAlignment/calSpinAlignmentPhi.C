...
void calSpinAlignmentPhi(int energy = 4, int pid = 0, int year = 0, string setting = "3DRandom")
{
  TGaxis::SetMaxDigits(4);
  gStyle->SetOptDate(0);

  string InPutFile_SE = "../data/Yields_Phi_SE_19GeV_20220527.root";
...
/*  TCanvas *c_mMass_psi = new TCanvas("c_mMass_psi","c_mMass_psi",10,10,800,800);
  ...
  for(int i_theta = 2; i_theta < 5; i_theta++) // cos(theta*) loop
  {
    ...
    f_bw->SetParameter(3,ParBW[KEY_theta_QA][3]/7.0);
    f_bw->SetParameter(4,ParBW[KEY_theta_QA][4]/7.0);
    f_bw->SetRange(vmsa::BW_Start[pid],vmsa::BW_Stop[pid]);
...

{
    ...
    TH1F *h_frame_cos = new TH1F("h_frame_cos","h_frame_cos",100,0.95,1.1);
    ...
    h_frame_cos->GetXaxis()->SetRangeUser(0.985,1.05);
    h_frame_cos->GetXaxis()->SetNdivisions(505,'N');
    h_frame_cos->GetXaxis()->SetLabelSize(0.03);
    h_frame_cos->GetXaxis()->SetTitle("M(K^{+},K^{-}) (GeV/c^{2})");
  ...
  h_copy->Draw("pE same");
  TF1 *f_bw_QA = new TF1("f_bw_QA",PolyBreitWigner,vmsa::BW_Start[pid],vmsa::BW_Stop[pid],5);;
  f_bw_QA->FixParameter(0,ParBW[KEY_theta_QA][0]);
  f_bw_QA->FixParameter(1,ParBW[KEY_theta_QA][1]);
  f_bw_QA->FixParameter(2,ParBW[KEY_theta_QA][2]/459885291.);
  f_bw_QA->FixParameter(3,ParBW[KEY_theta_QA][3]/459885291.);
  f_bw_QA->FixParameter(4,ParBW[KEY_theta_QA][4]/459885291.);
  f_bw_QA->SetRange(vmsa::BW_Start[pid],vmsa::BW_Stop[pid]);
  f_bw_QA->SetLineColor(kGray+2);
  f_bw_QA->Draw("l same");

  TF1 *f_bg_QA = new TF1("f_bg_QA",Poly,vmsa::BW_Start[pid],vmsa::BW_Stop[pid],2);;
  f_bg_QA->FixParameter(0,ParBW[KEY_theta_QA][3]/459885291.);
  f_bg_QA->FixParameter(1,ParBW[KEY_theta_QA][4]/459885291.);
  f_bg_QA->SetLineColor(kGray+2);
  f_bg_QA->SetLineStyle(2);
  f_bg_QA->Draw("l same");

    string leg_pt = "1.2 < p_{T} < 1.8 GeV/c";
    plotTopLegend((char*)leg_pt.c_str(),0.988,0.00025,0.04,1,0.0,42,0);
    leg_pt = "|y| < 1.0";
  ...
  TF1 *f_bgQA = new TF1("f_bg",Poly,vmsa::BW_Start[pid],vmsa::BW_Stop[pid],2);
  f_bgQA->FixParameter(0,f_bw_QA->GetParameter(3));
  f_bgQA->FixParameter(1,f_bw_QA->GetParameter(4));
  f_bgQA->SetLineColor(kBlue);
  f_bgQA->SetLineStyle(2);
  f_bgQA->SetLineWidth(4);
  f_bgQA->DrawCopy("l same");
  ...
  TCanvas *c_mMass_psi_f = new TCanvas("c_mMass_psi_f","c_mMass_psi_f",10,10,1600,800);
  c_mMass_psi_f->Divide(4,2);
  for(int i_theta = vmsa::CTS_start; i_theta < vmsa::CTS_stop; i_theta++) // cos(theta*) loop
  {
    c_mMass_psi_f->cd(i_theta+1)->SetLeftMargin(0.1);
    c_mMass_psi_f->cd(i_theta+1)->SetBottomMargin(0.1);
    c_mMass_psi_f->cd(i_theta+1)->SetTicks(1,1);
    c_mMass_psi_f->cd(i_theta+1)->SetGrid(0,0);
    c_mMass_psi_f->cd(i_theta+1);
    string KEY_QA = Form("pt_%d_Centrality_%d_CosThetaStar_%d_2nd_%s_SM",2,9,i_theta,vmsa::mPID[pid].c_str());
    h_mMass[KEY_QA]->SetTitle(Form("%d/7 < cos(#theta*) < %d/7",i_theta,i_theta+1));
    h_mMass[KEY_QA]->GetXaxis()->SetTitle("m(K^{+},K^{-}) GeV/c^{2}");
    h_mMass[KEY_QA]->GetXaxis()->CenterTitle(true);
    h_mMass[KEY_QA]->GetYaxis()->SetTitle("Raw Yield");
    h_mMass[KEY_QA]->GetYaxis()->CenterTitle(true);
    h_mMass[KEY_QA]->SetMarkerColor(1);
    h_mMass[KEY_QA]->SetMarkerStyle(24);
    h_mMass[KEY_QA]->SetMarkerSize(0.8);
    h_mMass[KEY_QA]->DrawCopy("pE");
  ...
  //TF1 *f_bw_QA = new TF1("f_bw_QA",PolyBreitWigner,vmsa::BW_Start[pid],vmsa::BW_Stop[pid],5);;
  //f_bw_QA->FixParameter(0,ParBW[KEY_theta_QA][0]);
  //ef_bw_QA->FixParameter(1,ParBW[KEY_theta_QA][1]);
  //f_bw_QA->FixParameter(2,ParBW[KEY_theta_QA][2]);
  //f_bw_QA->FixParameter(3,ParBW[KEY_theta_QA][3]);
  //f_bw_QA->FixParameter(4,ParBW[KEY_theta_QA][4]);
  //f_bw_QA->SetRange(vmsa::BW_Start[pid],vmsa::BW_Stop[pid]);
  ...
  //leg_temp_2->AddEntry(h_mCounts[KEY_BW],"breit wigner","p");
  //leg_temp_2->Draw("same");
  c_mMass_psi_f->SaveAs("./figures/signalExtraction.eps");
#endif
  ...
  string figure_name = Form("./figures/rho00_pT_%s.pdf",vmsa::mBeamEnergy[energy].c_str());
  c_Rho00_pT->SaveAs(figure_name.c_str());

  
  string OutPutFile = "../output/RawPhiPt/RawPhiPtSys_3DRandom.root"; 
