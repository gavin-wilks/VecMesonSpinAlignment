...
void calSysErrorPhiY_FixedSys_Folded_RawRes(Int_t energy = 4, Int_t pid = 0, string correction = "AccRes", bool random3D = false, int ptQA = 0, int centQA = 2, int order = 2, std::string etamode = "eta1_eta1", std::string fileoption = "FixedResolutionFinal")//defaultF = 0 is BESII, defaultF = 1 is BESI
{
...
//  PlotLine(0.0,5.0,1.0/3.0,1.0/3.0,1,2,2);
  ...
  c_rho_SysError->SaveAs(Form("./figures/%s/%s/rapiditystudy/finaloutputrho_rapiditydependence_Res%s_%s_%s_order%d_%s.pdf",vmsa::mPID[pid].c_str(),vmsa::mBeamEnergy[energy].c_str(),correction.c_str(),vmsa::mPID[pid].c_str(),vmsa::mBeamEnergy[energy].c_str(),order,etamode.c_str()));  
  ...
  string outputname = Form("./figures/%s/%s/rapiditystudy/finaloutputrho_rapiditydependence_Res%s_ptandcentbins_%s_%s_order%d_%s.pdf",vmsa::mPID[pid].c_str(),vmsa::mBeamEnergy[energy].c_str(),correction.c_str(),vmsa::mPID[pid].c_str(),vmsa::mBeamEnergy[energy].c_str(),order,etamode.c_str());
  ...
  string OutPutFile = Form("../output/AuAu%s/%s/RhoEta_Res%sSysErrors_%s.root",vmsa::mBeamEnergy[energy].c_str(),vmsa::mPID[pid].c_str(),correction.c_str(),etamode.c_str());
  if(order == 1) OutPutFile = Form("../output/AuAu%s/%s/RhoEta_Res%sSysErrors_%s_FirstOrder.root",vmsa::mBeamEnergy[energy].c_str(),vmsa::mPID[pid].c_str(),correction.c_str(),etamode.c_str());
