...
void subBackGround_ptyspectra(int energy = 4, int pid = 0, int year = 0, string date = "20241218", bool random3D = false, int order = 2, string etamode = "eta1_eta1")
{
  ...
  //string InPutFile_SE = Form("../data/Yields_Phi_SE_%s_%s_ptyspectra_TPCTOF_eta1_eta1_y1_nsig2p5_dipangle.root",vmsa::mBeamEnergy[energy].c_str(),date.c_str());
  string InPutFile_SE = Form("../data/Yields_Phi_SE_%s_%s_ptyspectra_TPCTOF_eta1_eta1_y1_nsig2p5_dipangle.root",vmsa::mBeamEnergy[energy].c_str(),date.c_str());
  //string InPutFile_SE = Form("../data/Yields_Phi_SE_19GeV_20240828_cent2060_phispectra.root");
  ...
  //string InPutFile_ME = Form("../data/Yields_Phi_ME_%s_%s_cent2060_phispectra.root",vmsa::mBeamEnergy[energy].c_str(),date.c_str());
  //string InPutFile_ME = Form("../data/Yields_Phi_ME_19GeV_20240923_eta1_eta1_70vz_25phipsi.root");
  //string InPutFile_ME = Form("../data/Yields_Phi_ME_19GeV_20241009_eta1_eta1_reweight_fixed.root");
  //string InPutFile_ME = Form("../data/Yields_Phi_ME_19GeV_%s_ptyspectra_TPCTOF_eta1_eta1_y1_nsig2p5_dipangle.root",date.c_str());
  string InPutFile_ME = Form("../data/Yields_Phi_ME_19GeV_20241219_ptyspectra_TPCTOF_eta1_eta1_dipangle_LIKESIGN.root",date.c_str());
  ...
  float scalingratio[vmsa::tofrebinpttotal][vmsa::tofrebinytotal] = {0.0};
  ...
  for(Int_t i_pt = 0; i_pt < vmsa::pt_total; i_pt++) // pt bin 
  {
	...
	for(Int_t i_dca = vmsa::Dca_start; i_dca < 1/*vmsa::Dca_stop*/; i_dca++)
	{
	  for(Int_t i_sig = vmsa::nSigKaon_start; i_sig < 1/*vmsa::nSigKaon_stop*/; i_sig++)
	  {
	    ...
	    string KEY_InPutME = Form("pt_%d_eta_%d_Centrality_%d_%s_Dca_%d_Sig_%d_%s_SE",i_pt,i_eta,i_cent,EP[order-1].c_str(),i_dca,i_sig,vmsa::mPID[pid].c_str());
    ...
    for(Int_t i_dca = vmsa::Dca_start; i_dca < 1/*vmsa::Dca_stop*/; i_dca++)
    {
      for(Int_t i_sig = vmsa::nSigKaon_start; i_sig < 1/*vmsa::nSigKaon_stop*/; i_sig++)
      {
        for(int i_norm = vmsa::Norm_start; i_norm < 1; ++i_norm)
        {
          ...
          {
            for(Int_t i_pt = vmsa::tofrebinpt[ptbin]; i_pt < vmsa::tofrebinpt[ptbin+1]; i_pt++) // pt bin 
            {
              for(int etabin = 0; etabin < vmsa::tofrebinytotal; etabin++)
              {
                for(Int_t i_eta = vmsa::tofrebiny[etabin]; i_eta < vmsa::tofrebiny[etabin+1]; i_eta++) // phi-psi bin
                {
                  string KEY_SE = Form("pt_%d_eta_%d_Centrality_%d_%s_Dca_%d_Sig_%d_%s_Norm_%d_SE",i_pt,i_eta,i_cent,EP[order-1].c_str(),i_dca,i_sig,vmsa::mPID[pid].c_str(),i_norm);
                  //cout << KEY_SE << endl;
  ...
  cout << endl << "const float scalingME[vmsa::tofrebinpttotal][vmsa::tofrebinytotal] = {";
  for(int ptbin = 0; ptbin < vmsa::tofrebinpttotal; ptbin++)
  {
    cout << "{";
    for(int etabin = 0; etabin < vmsa::tofrebinytotal; etabin++)
    {
      if(etabin < vmsa::tofrebinytotal-1) cout << scalingratio[ptbin][etabin] << ",";
  ...
  string outputname = Form("figures/%s/%s/pTstudy/ptyspectra_InvMassDistributions_%s_Order%d_Cent9.pdf",vmsa::mPID[pid].c_str(),vmsa::mBeamEnergy[energy].c_str(),vmsa::mBeamEnergy[energy].c_str(),order);
  ...
  for(int ieta = 0; ieta < vmsa::tofrebinytotal; ieta++)
  {
    for(int i_pt = 0; i_pt < vmsa::tofrebinpttotal; i_pt++) // pt loop
    {
      ...
      string KEY_SE_QA = Form("pt_%d_eta_%d_Centrality_%d_%s_Dca_%d_Sig_%d_%s_Norm_%d_SE_Rebin",i_pt,ieta,9,EP[order-1].c_str(),vmsa::Dca_start,vmsa::nSigKaon_start,vmsa::mPID[pid].c_str(),vmsa::Norm_QA);
      h_mMass_SE[KEY_SE_QA]->GetYaxis()->SetRangeUser(-0.1*h_mMass_SE[KEY_SE_QA]->GetMaximum(),1.1*h_mMass_SE[KEY_SE_QA]->GetMaximum());
      h_mMass_SE[KEY_SE_QA]->SetTitle(Form("%.2f<y<%.2f",vmsa::tofrebinyval[ieta],vmsa::tofrebinyval[ieta+1]));
      h_mMass_SE[KEY_SE_QA]->DrawCopy();

      string KEY_ME_QA = Form("pt_%d_eta_%d_Centrality_%d_%s_Dca_%d_Sig_%d_%s_Norm_%d_ME_Rebin",i_pt,ieta,9,EP[order-1].c_str(),vmsa::Dca_start,vmsa::nSigKaon_start,vmsa::mPID[pid].c_str(),vmsa::Norm_QA);
      ...
      string KEY_SM_QA = Form("pt_%d_eta_%d_Centrality_%d_%s_Dca_%d_Sig_%d_%s_Norm_%d_SM_Rebin",i_pt,ieta,9,EP[order-1].c_str(),vmsa::Dca_start,vmsa::nSigKaon_start,vmsa::mPID[pid].c_str(),vmsa::Norm_QA);
      ...
      string pT_range = Form("[%.2f,%.2f]",vmsa::tofrebinptval[i_pt],vmsa::tofrebinptval[i_pt+1]);
  ...
  string outputfile = Form("../output/AuAu%s/%s/ptyspectra_Reweight_InvMassSubBg_%s.root",vmsa::mBeamEnergy[energy].c_str(),vmsa::mPID[pid].c_str(),etamode.c_str());
