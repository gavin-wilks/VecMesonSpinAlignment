...
void compareHelicityVsGlobal(Int_t energy = 4, Int_t pid = 0, Int_t i_cent = 9, string correction = "Raw", int order = 2, string etamode = "eta1_eta1", int yspectra = 23, std::string simmode = "Mc", std::string v2 = "nov2")//defaultF = 0 is BESII, defaultF = 1 is BESI
{
  ...
  {
    spectra[0][i] = Form("CEPT_O2_NoYSpec_%s_phieff_phiTPC_20240809g_ysig1000_rho%s_rerho0.0_imrho0.0_r0.0_i0.0_hrho0.3333_hrerho0.0_himrho0.0_hr0.0_hi0.0",v2.c_str(),sigmay_text[i].c_str());
    spectra[1][i] = Form("CEPT_O2_NoYSpec_%s_phieff_phiTPC_20240809g_ysig1000_rho0.3333_rerho0.0_imrho0.0_r%s_i0.0_hrho0.3333_hrerho0.0_himrho0.0_hr0.0_hi0.0",v2.c_str(),sigmay_text_offdiag[i].c_str());
    spectra[2][i] = Form("CEPT_O2_NoYSpec_%s_phieff_phiTPC_20240809g_ysig1000_rho0.3333_rerho0.0_imrho0.0_r0.0_i%s_hrho0.3333_hrerho0.0_himrho0.0_hr0.0_hi0.0",v2.c_str(),sigmay_text_offdiag[i].c_str());
    spectra[3][i] = Form("CEPT_O2_NoYSpec_%s_phieff_phiTPC_20240809g_ysig1000_rho0.3333_rerho%s_imrho0.0_r0.0_i0.0_hrho0.3333_hrerho0.0_himrho0.0_hr0.0_hi0.0",v2.c_str(),sigmay_text_offdiag[i].c_str());
    spectra[4][i] = Form("CEPT_O2_NoYSpec_%s_phieff_phiTPC_20240809g_ysig1000_rho0.3333_rerho0.0_imrho%s_r0.0_i0.0_hrho0.3333_hrerho0.0_himrho0.0_hr0.0_hi0.0",v2.c_str(),sigmay_text_offdiag[i].c_str());
  }
  ...
  std::string outputname = Form("figures/%s/%s/pTstudy/GlobalvsGlobal_parametercomparison_%s_%s.pdf",vmsa::mPID[pid].c_str(),vmsa::mBeamEnergy[energy].c_str(),simmode.c_str(),v2.c_str());
  ...
  for(int i = 0; i < 4; i++)
  {
    for(int irho = 0; irho < 5; irho++)
    {
      for(int par = 0; par < 5; par++)
      {
        ...
        g2D_Global[irho][par][i]->GetXaxis()->SetTitle(Form("%s^{Global} Input",param[irho].c_str()));
  ...
  outputname = Form("figures/%s/%s/pTstudy/HelicityvsGlobal_parametercomparison_%s_%s.pdf",vmsa::mPID[pid].c_str(),vmsa::mBeamEnergy[energy].c_str(),simmode.c_str(),v2.c_str());
  ...
  for(int i = 0; i < 4; i++)
  {
    for(int irho = 0; irho < 5; irho++)
    {
      for(int par = 0; par < 5; par++)
      {
        ...
        g2D[irho][par][i]->GetXaxis()->SetTitle(Form("%s^{Global} Input",param[irho].c_str()));
