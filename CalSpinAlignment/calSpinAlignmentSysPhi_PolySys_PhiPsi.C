...
void calSpinAlignmentSysPhi_PolySys_PhiPsi(int energy = 4, int pid = 0, int year = 0, bool random3D = false, int order = 2, string etamode = "eta1_eta1")
{
  ...
  string inputfile = Form("../output/AuAu%s/%s/PhiPsi_InvMassSubBg_%s.root",vmsa::mBeamEnergy[energy].c_str(),vmsa::mPID[pid].c_str(),etamode.c_str());
  if(order == 1) inputfile = Form("../output/AuAu%s/%s/PhiPsi_InvMassSubBg_%s_FirstOrder.root",vmsa::mBeamEnergy[energy].c_str(),vmsa::mPID[pid].c_str(),etamode.c_str());
...
//    {
//      string KEY_QA = Form("pt_%d_Centrality_%d_CosThetaStar_%d_PhiPsi_%d_2nd_Dca_%d_Sig_%d_%s_Norm_%d",vmsa::pt_QA[energy],9,i_theta,vmsa::Dca_start,vmsa::nSigKaon_start,vmsa::mPID[pid].c_str(),vmsa::Norm_QA);
...
//#endif
  ...
  for(int i_pt = vmsa::pt_rebin_first[energy]; i_pt < vmsa::pt_rebin_last[energy]; i_pt++) // pt loop
  {
	...
	for(int i_norm = vmsa::Norm_start; i_norm < vmsa::Norm_stop; ++i_norm)
	{
	  for(int i_func = vmsa::Func_start; i_func < vmsa::Func_stop; ++i_func)
	  {
	    for(int i_theta = vmsa::CTS_start; i_theta < vmsa::CTS_stop; i_theta++) // cos(theta*) loop
	    {
	      string KEY = Form("pt_%d_Centrality_%d_EtaGap_%d_CosThetaStar_%d_PhiPsi_%d_2nd_%s_Norm_%d_Func_%d",i_pt,i_cent,i_eta,i_theta,vmsa::mPID[pid].c_str(),i_norm,i_func);
	    }
	  }
	}
      }
  ...
  */
	    ...
	    for(int i_method = vmsa::Method_start; i_method < vmsa::Method_stop; ++i_method)
	    {
	        ...
	        for(int i_pt = vmsa::pt_rebin_first[energy]; i_pt < vmsa::pt_rebin_last[energy]; ++i_pt) // pt loop
	        {
	          ...
	          for(int i_theta = vmsa::CTS_start; i_theta < vmsa::CTS_stop; ++i_theta) // cos(theta*) loop
	          {
	            ...
	            {
	              string KEY = Form("pt_%d_Centrality_%d_CosThetaStar_%d_PhiPsi_%d_%s_Dca_%d_Sig_%d_%s_Norm_%d",i_pt,i_cent,i_theta,i_phipsi,EP[order-1].c_str(),i_dca,i_sig,vmsa::mPID[pid].c_str(),i_norm);
	              string KEY_Poly = Form("pt_%d_Centrality_%d_CosThetaStar_%d_PhiPsi_%d_%s_Dca_%d_Sig_%d_%s_Norm_%d_Poly%d",i_pt,i_cent,i_theta,i_phipsi,EP[order-1].c_str(),i_dca,i_sig,vmsa::mPID[pid].c_str(),i_norm,i_poly+1);
	              TF1 *f_bw;
                      if(i_poly == 0) f_bw = new TF1("f_bw",PolyBreitWigner, vmsa::BW_Start[pid],vmsa::BW_Stop[pid],5);
                      if(i_poly == 1) f_bw = new TF1("f_bw",Poly2BreitWigner,vmsa::BW_Start[pid],vmsa::BW_Stop[pid],6);
                      if(i_poly == 2) f_bw = new TF1("f_bw",Poly3BreitWigner,vmsa::BW_Start[pid],vmsa::BW_Stop[pid],7);
  ...
  for(int i_poly = 0; i_poly < 3; i_poly++)
  {
    for(int i_norm = 0; i_norm < 3; i_norm++)
    {
      ...
      {
        string outputname = Form("./figures/%s/%s/pTstudy/PhiPsi_allThetaYields%s_%s_Order%d_cent%d_%s_pTdependence_Norm%d_Poly%d.pdf",vmsa::mPID[pid].c_str(),vmsa::mBeamEnergy[energy].c_str(),vmsa::mPID[pid].c_str(),vmsa::mBeamEnergy[energy].c_str(),order,i_cent,etamode.c_str(),i_norm,i_poly+1);
        ...
        for(int i_pt = vmsa::pt_rebin_first[energy]; i_pt < vmsa::pt_rebin_last[energy]; ++i_pt) // pt loop
        {
          for(int i_phipsi = 0; i_phipsi < 7; i_phipsi++)
          {
            ...
            for(int i_theta = 0; i_theta < 9; ++i_theta)
            {
              ...
              if(i_theta < vmsa::CTS_stop)
              {
                string KEY_QA = Form("pt_%d_Centrality_%d_CosThetaStar_%d_PhiPsi_%d_%s_Dca_%d_Sig_%d_%s_Norm_%d",i_pt,i_cent,i_theta,i_phipsi,EP[order-1].c_str(),vmsa::Dca_start,1/*vmsa::nSigKaon_start*/,vmsa::mPID[pid].c_str(),i_norm);
                h_mMass[KEY_QA]->SetTitle("");
                h_mMass[KEY_QA]->GetXaxis()->SetNdivisions(505,'N');
                h_mMass[KEY_QA]->GetXaxis()->SetLabelSize(0.03);
                h_mMass[KEY_QA]->GetXaxis()->SetTitle("M(K^{+},K^{-})");
                h_mMass[KEY_QA]->SetTitle(Form("%.2f<p_{T}<%.2f, %d#pi/14<#phi-#Psi_{2}<%d#pi/14, %d/7<|cos(#theta*)|<%d/7",vmsa::pt_low[energy][i_pt],vmsa::pt_up[energy][i_pt],i_phipsi*2-7,(i_phipsi+1)*2-7,i_theta,i_theta+1));
                h_mMass[KEY_QA]->GetXaxis()->SetTitleSize(0.05);
                h_mMass[KEY_QA]->GetXaxis()->SetTitleOffset(1.2);
                h_mMass[KEY_QA]->GetXaxis()->CenterTitle();
              ...
              if(i_theta == vmsa::CTS_stop)
              {
                string KEY_InteTheta_QA = Form("pt_%d_Centrality_%d_%s_Dca_%d_Sig_%d_%s_Norm_%d",i_pt,i_cent,EP[order-1].c_str(),vmsa::Dca_start,1/*vmsa::nSigKaon_start*/,vmsa::mPID[pid].c_str(),i_norm);
                string KEY_InteTheta_QA_Poly = Form("pt_%d_Centrality_%d_%s_Dca_%d_Sig_%d_%s_Norm_%d_Poly%d",i_pt,i_cent,EP[order-1].c_str(),vmsa::Dca_start,1/*vmsa::nSigKaon_start*/,vmsa::mPID[pid].c_str(),i_norm,i_poly+1);
                h_mMass_InteTheta[KEY_InteTheta_QA]->SetTitle("");
                h_mMass_InteTheta[KEY_InteTheta_QA]->SetTitle(Form("%.2f<p_{T}<%.2f, 0/7<|cos(#theta*)|<7/7",vmsa::pt_low[energy][i_pt],vmsa::pt_up[energy][i_pt]));
                h_mMass_InteTheta[KEY_InteTheta_QA]->GetXaxis()->SetNdivisions(505,'N');
                h_mMass_InteTheta[KEY_InteTheta_QA]->GetXaxis()->SetLabelSize(0.03);
                h_mMass_InteTheta[KEY_InteTheta_QA]->GetXaxis()->SetTitle("M(K^{+},K^{-})");
                h_mMass_InteTheta[KEY_InteTheta_QA]->GetXaxis()->SetTitleSize(0.05);
            ...
            for(int i_theta = vmsa::CTS_start; i_theta < vmsa::CTS_stop; ++i_theta)
            {
              c_diff->cd(i_theta+1);
              string KEY_QA = Form("pt_%d_Centrality_%d_CosThetaStar_%d_PhiPsi_%d_%s_Dca_%d_Sig_%d_%s_Norm_%d",i_pt,i_cent,i_theta,i_phipsi,EP[order-1].c_str(),vmsa::Dca_start,1/*vmsa::nSigKaon_start*/,vmsa::mPID[pid].c_str(),i_norm);
              string KEY_QA_Poly = Form("pt_%d_Centrality_%d_CosThetaStar_%d_PhiPsi_%d_%s_Dca_%d_Sig_%d_%s_Norm_%d_Poly%d",i_pt,i_cent,i_theta,i_phipsi,EP[order-1].c_str(),vmsa::Dca_start,1/*vmsa::nSigKaon_start*/,vmsa::mPID[pid].c_str(),i_norm,i_poly+1);
              TF1 *f_bw;
              if(i_poly == 0) f_bw = new TF1("f_bw",PolyBreitWigner,vmsa::BW_Start[pid],vmsa::BW_Stop[pid],5);
              if(i_poly == 1) f_bw = new TF1("f_bw",Poly2BreitWigner,vmsa::BW_Start[pid],vmsa::BW_Stop[pid],6);
              if(i_poly == 2) f_bw = new TF1("f_bw",Poly3BreitWigner,vmsa::BW_Start[pid],vmsa::BW_Stop[pid],7);
            ...
            for(int i_sigma = 0/*vmsa::Sig_start*/; i_sigma < 1/*vmsa::Sig_stop*/; ++i_sigma)
            {
              ...
              {
                string KEY_counts_QA = Form("pt_%d_Centrality_%d_%s_Dca_%d_Sig_%d_%s_Norm_%d_Sigma_%d_%s_Poly%d",i_pt,i_cent,EP[order-1].c_str(),vmsa::Dca_start,1/*vmsa::nSigKaon_start*/,vmsa::mPID[pid].c_str(),i_norm,i_sigma,vmsa::mInteMethod[i_method].c_str(),i_poly+1);
                if(i_sigma == vmsa::Sig_start && i_method == vmsa::Method_start)
                {
                  h_mCounts1D[KEY_counts_QA];
                  h_mCounts1D[KEY_counts_QA]->SetTitle("");
                  h_mCounts1D[KEY_counts_QA]->GetXaxis()->SetNdivisions(505,'N');
                  h_mCounts1D[KEY_counts_QA]->GetXaxis()->SetLabelSize(0.03);
                  h_mCounts1D[KEY_counts_QA]->GetXaxis()->SetTitle("cos(#theta*)");
                  h_mCounts1D[KEY_counts_QA]->GetXaxis()->SetTitleSize(0.05);
                  h_mCounts1D[KEY_counts_QA]->GetXaxis()->SetTitleOffset(1.2);
                  h_mCounts1D[KEY_counts_QA]->GetXaxis()->CenterTitle();
  
                  h_mCounts1D[KEY_counts_QA]->GetYaxis()->SetRangeUser(0.8*h_mCounts1D[KEY_counts_QA]->GetMinimum(),1.2*h_mCounts1D[KEY_counts_QA]->GetMaximum());
                  h_mCounts1D[KEY_counts_QA]->GetYaxis()->SetNdivisions(505,'N');
                  h_mCounts1D[KEY_counts_QA]->GetYaxis()->SetTitle("Counts");
                  h_mCounts1D[KEY_counts_QA]->GetYaxis()->SetTitleSize(0.05);
                  h_mCounts1D[KEY_counts_QA]->GetYaxis()->SetLabelSize(0.03);
                  h_mCounts1D[KEY_counts_QA]->GetYaxis()->CenterTitle();
  
                  h_mCounts1D[KEY_counts_QA]->SetMarkerStyle(20);
                  h_mCounts1D[KEY_counts_QA]->SetMarkerColor(1);
                  h_mCounts1D[KEY_counts_QA]->SetMarkerSize(1.2);
                  h_mCounts1D[KEY_counts_QA]->Draw("pE");
                }
                else
                {
 ...
 // {
  ...
  //string outputname = Form("figures/%s/%s/pTstudy/PhiPsi_Chi2NDF_%s_Order%d_Cent9.pdf",vmsa::mPID[pid].c_str(),vmsa::mBeamEnergy[energy].c_str(),vmsa::mBeamEnergy[energy].c_str(),order);
  //string output_start = Form("%s[",outputname.c_str());
  //
  ...
  //outputname = Form("figures/%s/%s/pTstudy/PhiPsi_PValue_%s_Order%d_Cent9.pdf",vmsa::mPID[pid].c_str(),vmsa::mBeamEnergy[energy].c_str(),vmsa::mBeamEnergy[energy].c_str(),order);
  ...
  string outputfile = Form("../output/AuAu%s/%s/PhiPsi_RawPhiPtSys_%s_PolySys.root",vmsa::mBeamEnergy[energy].c_str(),vmsa::mPID[pid].c_str(),etamode.c_str());
  if(order == 1) outputfile = Form("../output/AuAu%s/%s/PhiPsi_RawPhiPtSys_%s_PolySys_FirstOrder.root",vmsa::mBeamEnergy[energy].c_str(),vmsa::mPID[pid].c_str(),etamode.c_str());
