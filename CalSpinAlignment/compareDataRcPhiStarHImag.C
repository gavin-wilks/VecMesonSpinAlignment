...
void compareDataRcPhiStarHImag(Int_t energy = 4, Int_t pid = 0, Int_t i_cent = 9, string correction = "Raw", int order = 2, string etamode = "eta1_eta1", int yspectra = 23, std::string simmode = "Rc")//defaultF = 0 is BESII, defaultF = 1 is BESI
{
  const int nvar = 7;
  ...
  {
    spectra[i] = Form("CEPT_O2_NoYSpec_prelimv2_phieff_phiTPC_20240808_2_ysig1000_rho0.3333_rerho0.0_imrho0.0_r0.0_i0.0_hrho0.3333_hrerho0.0_himrho0.0_hr0.0_hi%s",sigmay_text[i].c_str());
  }
  ...
  std::string outputname = Form("figures/%s/%s/pTstudy/%s_imagcomparison_%s.pdf",vmsa::mPID[pid].c_str(),vmsa::mBeamEnergy[energy].c_str(),spectra[defaultfile].c_str(),simmode.c_str());  
  ...
  for(int ipt = 0; ipt < 4; ipt++)
  {
    ...
    for(int i = 0; i < nvar; i++)
    {
      ...
      h_mCounts_Ratio[ipt][i]->SetTitle(Form("Im(#rho_{10})-Im(#rho_{0-1})=%s, %1.1f<p_{T}<%1.1f, 20-60 Centrality",sigmay_text[i].c_str(),vmsa::pt_low[energy][i+2],vmsa::pt_up[energy][i+2]));
  ...
  cfit2D->SaveAs(Form("figures/%s/%s/pTstudy/%s_parametercomparison_imagcomparison_%s.pdf",vmsa::mPID[pid].c_str(),vmsa::mBeamEnergy[energy].c_str(),spectra[defaultfile].c_str(),simmode.c_str()));  
  ...
  for(int i = 0; i < 4; i++)
  {
    ...
    gglobal[i]->GetXaxis()->SetTitle("Im(#rho_{10})-Im(#rho_{0-1})^{helicity} input");
    gglobal[i]->GetYaxis()->SetTitle("#Delta#rho_{00}^{global}");
    gglobal[i]->SetMarkerSize(1.2);
    gglobal[i]->Draw("APE"); 
  }
  cfit->SaveAs(Form("figures/%s/%s/pTstudy/FitParams_%s_imagcomparison_%s.pdf",vmsa::mPID[pid].c_str(),vmsa::mBeamEnergy[energy].c_str(),spectra[defaultfile].c_str(),simmode.c_str()));  


  outputname = Form("figures/%s/%s/pTstudy/Comparison_%s_imagcomparison_%s.pdf",vmsa::mPID[pid].c_str(),vmsa::mBeamEnergy[energy].c_str(),spectra[defaultfile].c_str(),simmode.c_str());  
  ...
  for(int ipt = 0; ipt < 4; ipt++)
  {
    ...
    { 
      ...
      h_mCounts_RC[ipt][i]->SetTitle(Form("Im(#rho_{10})-Im(#rho_{0-1})=%s, %1.1f<p_{T}<%1.1f, 20-60 Centrality",sigmay_text[i].c_str(),vmsa::pt_low[energy][ipt+2],vmsa::pt_up[energy][ipt+2]));
