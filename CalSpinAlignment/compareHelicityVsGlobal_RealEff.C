...
void compareHelicityVsGlobal_RealEff(Int_t energy = 4, Int_t pid = 0, Int_t i_cent = 9, string correction = "Raw", int order = 2, string etamode = "eta1_eta1", int yspectra = 23, std::string simmode = "Mc", int inputpar = 0)//defaultF = 0 is BESII, defaultF = 1 is BESI
{
  ...
  for(int ires = 0; ires < 1; ires++)
  {
    for(int ikin = 0; ikin < nkin; ikin++)
    {
      ...
      {
        string inputfileRC = Form("effaccfiles/%s/%s/PaperAllYRC_EtaFixed_GlobalInput_realeff/Eff_%s_SingleParticle_noToF_Mode0_EtaMode0_pt%1.1f_y%1.1f_nov2_alleff.root",vmsa::mPID[pid].c_str(),vmsa::mBeamEnergy[energy].c_str(),vmsa::mBeamEnergy[energy].c_str(),ptkin[ikin],vals[iopt]);
        //if(ires == 1) inputfileRC = Form("effaccfiles/%s/%s/PaperAllYRC_EtaFixed_GlobalInput_nov2_ptcuts_ptres/Eff_%s_SingleParticle_noToF_Mode0_EtaMode0_pt%1.1f_y%1.1f_ptres_fixed.root",vmsa::mPID[pid].c_str(),vmsa::mBeamEnergy[energy].c_str(),vmsa::mBeamEnergy[energy].c_str(),ptkin[ikin],vals[iopt]);
    ...
    for(int ikin = 0; ikin < nkin; ikin++)
    {
      for(int iopt = 0; iopt < nopts; iopt++)
      {
        for(int irho = 0; irho < 5; irho++)
        {
          ...
          //for(int i = 0; i < nvar; i++)
          {
            for(int ieta = 10; ieta < 11; ieta+=10)
            {
              for(int ipt = 4; ipt < 5; ipt+=4) 
              {
                for(int iphipt = 0; iphipt < npt; iphipt++)
                {
  ...
  for(int i = 0; i < 20; i++) cout << "FINISHED 1D FITTING" << endl;
  ...
  for(int i = 0; i < 20; i++) cout << "FINISHED 2D FITTING" << endl;
          ...
          for(int irho = 0; irho < 5; irho++)
          {
            for(int ieta = 10; ieta < 11; ieta+=10)
            {
              for(int ipt = 4; ipt < 5; ipt+=4)
              {
                ...
                cout << "About to loop over iphipt" << endl;
                for(int iphipt = 0; iphipt < npt; iphipt++)
                {
  ...
  for(int i = 0; i < 20; i++) cout << "FINISHED CORRECTED 2D FITTING" << endl;
