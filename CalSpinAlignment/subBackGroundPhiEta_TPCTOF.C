...
void subBackGroundPhiEta_TPCTOF(int energy = 4, int i_eta = 0, int pid = 0, int year = 0, string date = "20230531", bool random3D = false, int order = 2, std::string etamode = "eta1_eta1")
{
...
//  }
  ...
  string outputname = Form("figures/%s/%s/rapiditystudy/TPCTOF_InvMassDistributions_%s_Order%d_y%d.pdf",vmsa::mPID[pid].c_str(),vmsa::mBeamEnergy[energy].c_str(),vmsa::mBeamEnergy[energy].c_str(),order,i_eta);
...
//#endif
  ...
  outputname = Form("figures/%s/%s/rapiditystudy/TPCTOF_InvMassBackgroundSubtracted_Rebinned_%s_Order%d_y%d.pdf",vmsa::mPID[pid].c_str(),vmsa::mBeamEnergy[energy].c_str(),vmsa::mBeamEnergy[energy].c_str(),order,i_eta);
  ...
  string outputfile = Form("../output/AuAu%s/%s/TPCTOF_InvMassSubBgEta%d_%s.root",vmsa::mBeamEnergy[energy].c_str(),vmsa::mPID[pid].c_str(),i_eta,etamode.c_str());
