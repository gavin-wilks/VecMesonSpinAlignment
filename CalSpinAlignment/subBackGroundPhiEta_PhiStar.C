...
void subBackGroundPhiEta_PhiStar(int energy = 4, int i_eta = 0, int pid = 0, int year = 0, string date = "20240216", bool random3D = false, int order = 2, std::string etamode = "eta1_eta1")
{
  ...
  for(int i_cent = vmsa::centStart; i_cent < vmsa::centStop; i_cent++) // Centrality loop
  {
    for(Int_t i_pt = 0; i_pt < vmsa::pt_total; i_pt++) // pt bin 
    {
      ...
      //{
        for(Int_t i_theta = 0; i_theta < 10; i_theta++) // phi-psi bin
        {
          for(Int_t i_dca = vmsa::Dca_start; i_dca < vmsa::Dca_stop; i_dca++)
          {
            for(Int_t i_sig = vmsa::nSigKaon_start; i_sig < vmsa::nSigKaon_stop; i_sig++)
            {
              ...
              string KEY_InPutSE = Form("eta_%d_pt_%d_Centrality_%d_Cos2PhiStarPhi_%d_%s_Dca_%d_Sig_%d_%s_SE",i_eta,i_pt,i_cent,i_theta,EP[order-1].c_str(),i_dca,i_sig,vmsa::mPID[pid].c_str());
              h_mInPut_SE[KEY_InPutSE] = (TH1F*)File_SE->Get(KEY_InPutSE.c_str())->Clone(); 

              string KEY_InPutME = Form("eta_%d_pt_%d_Centrality_%d_Cos2PhiStarPhi_%d_%s_Dca_%d_Sig_%d_%s_ME",i_eta,i_pt,i_cent,i_theta,EP[order-1].c_str(),i_dca,i_sig,vmsa::mPID[pid].c_str());
              ...
              {
                string KEY_SE = Form("eta_%d_pt_%d_Centrality_%d_Cos2PhiStarPhi_%d_%s_Dca_%d_Sig_%d_%s_Norm_%d_SE",i_eta,i_pt,i_cent,i_theta,EP[order-1].c_str(),i_dca,i_sig,vmsa::mPID[pid].c_str(),i_norm);
                h_mMass_SE[KEY_SE] = (TH1F*)h_mInPut_SE[KEY_InPutSE]->Clone(KEY_SE.c_str());
                string KEY_ME = Form("eta_%d_pt_%d_Centrality_%d_Cos2PhiStarPhi_%d_%s_Dca_%d_Sig_%d_%s_Norm_%d_ME",i_eta,i_pt,i_cent,i_theta,EP[order-1].c_str(),i_dca,i_sig,vmsa::mPID[pid].c_str(),i_norm);
                h_mMass_ME[KEY_ME] = (TH1F*)h_mInPut_ME[KEY_InPutME]->Clone(KEY_ME.c_str());
                string KEY_SM = Form("eta_%d_pt_%d_Centrality_%d_Cos2PhiStarPhi_%d_%s_Dca_%d_Sig_%d_%s_Norm_%d_SM",i_eta,i_pt,i_cent,i_theta,EP[order-1].c_str(),i_dca,i_sig,vmsa::mPID[pid].c_str(),i_norm);
  ...
  string KEY_SE_QA = Form("eta_%d_pt_%d_Centrality_1_Cos2PhiStarPhi_%d_%s_Dca_%d_Sig_%d_%s_Norm_%d_SE",i_eta,7,0,EP[order-1].c_str(),vmsa::Dca_start,vmsa::nSigKaon_start,vmsa::mPID[pid].c_str(),vmsa::Norm_QA);
  ...
  string KEY_ME_QA = Form("eta_%d_pt_%d_Centrality_1_Cos2PhiStarPhi_%d_%s_Dca_%d_Sig_%d_%s_Norm_%d_ME",i_eta,7,0,EP[order-1].c_str(),vmsa::Dca_start,vmsa::nSigKaon_start,vmsa::mPID[pid].c_str(),vmsa::Norm_QA);
  ...
  string KEY_SM_QA = Form("eta_%d_pt_%d_Centrality_1_Cos2PhiStarPhi_%d_%s_Dca_%d_Sig_%d_%s_Norm_%d_SM",i_eta,7,0,EP[order-1].c_str(),vmsa::Dca_start,vmsa::nSigKaon_start,vmsa::mPID[pid].c_str(),vmsa::Norm_QA);
...
//    string KEY_SE_QA = Form("eta_%d_pt_%d_Centrality_1_Cos2PhiStarPhi_%d_%s_Dca_%d_Sig_%d_%s_Norm_%d_SE",i_eta,i_pt,0,EP[order-1].c_str(),vmsa::Dca_start,vmsa::nSigKaon_start,vmsa::mPID[pid].c_str(),vmsa::Norm_QA);
//    h_mMass_SE[KEY_SE_QA]->GetYaxis()->SetRangeUser(-0.1*h_mMass_SE[KEY_SE_QA]->GetMaximum(),1.1*h_mMass_SE[KEY_SE_QA]->GetMaximum());
//    h_mMass_SE[KEY_SE_QA]->DrawCopy();
//
//    string KEY_ME_QA = Form("eta_%d_pt_%d_Centrality_1_Cos2PhiStarPhi_%d_%s_Dca_%d_Sig_%d_%s_Norm_%d_ME",i_eta,i_pt,0,EP[order-1].c_str(),vmsa::Dca_start,vmsa::nSigKaon_start,vmsa::mPID[pid].c_str(),vmsa::Norm_QA);
//    h_mMass_ME[KEY_ME_QA]->SetLineColor(2);
//    h_mMass_ME[KEY_ME_QA]->SetFillColor(2);
//    h_mMass_ME[KEY_ME_QA]->SetFillStyle(3002);
//    h_mMass_ME[KEY_ME_QA]->DrawCopy("h same");
//
//    string KEY_SM_QA = Form("eta_%d_pt_%d_Centrality_1_Cos2PhiStarPhi_%d_%s_Dca_%d_Sig_%d_%s_Norm_%d_SM",i_eta,i_pt,0,EP[order-1].c_str(),vmsa::Dca_start,vmsa::nSigKaon_start,vmsa::mPID[pid].c_str(),vmsa::Norm_QA);
//    h_mMass_SM[KEY_SM_QA]->SetLineColor(4);
//    h_mMass_SM[KEY_SM_QA]->SetFillColor(4);
//    h_mMass_SM[KEY_SM_QA]->SetFillStyle(3004);
//    h_mMass_SM[KEY_SM_QA]->DrawCopy("h same");
...
//  }
  ...
  string outputname = Form("figures/%s/%s/rapiditystudy/Cos2PhiStarPhi_InvMassDistributions_%s_Order%d_y%d.pdf",vmsa::mPID[pid].c_str(),vmsa::mBeamEnergy[energy].c_str(),vmsa::mBeamEnergy[energy].c_str(),order,i_eta);
  ...
  for(int itheta = 0; itheta < 10; itheta++)
  {
    for(int icent = 0; icent < 9; icent++)
    {
      for(int i_pt = vmsa::pt_start; i_pt < vmsa::pt_stop; i_pt++) // pt loop
      {
        ...
        c_pT->cd(i_pt+1)->SetTicks(1,1);
        c_pT->cd(i_pt+1)->SetGrid(0,0);
        string KEY_SE_QA = Form("eta_%d_pt_%d_Centrality_%d_Cos2PhiStarPhi_%d_%s_Dca_%d_Sig_%d_%s_Norm_%d_SE",i_eta,i_pt,icent,itheta,EP[order-1].c_str(),vmsa::Dca_start,vmsa::nSigKaon_start,vmsa::mPID[pid].c_str(),vmsa::Norm_QA);
        //cout << KEY_SE_QA << endl;
        h_mMass_SE[KEY_SE_QA]->GetYaxis()->SetRangeUser(-0.1*h_mMass_SE[KEY_SE_QA]->GetMaximum(),1.1*h_mMass_SE[KEY_SE_QA]->GetMaximum());
        h_mMass_SE[KEY_SE_QA]->SetTitle(Form("%.1f<y<%.1f, Cent %d, %d/5<cos(2#phi*-2#phi)<%d/5",vmsa::eta_bin[i_eta],vmsa::eta_bin[i_eta+1],icent,itheta-5,itheta-4));
        h_mMass_SE[KEY_SE_QA]->DrawCopy();

        string KEY_ME_QA = Form("eta_%d_pt_%d_Centrality_%d_Cos2PhiStarPhi_%d_%s_Dca_%d_Sig_%d_%s_Norm_%d_ME",i_eta,i_pt,icent,itheta,EP[order-1].c_str(),vmsa::Dca_start,vmsa::nSigKaon_start,vmsa::mPID[pid].c_str(),vmsa::Norm_QA);
        //cout << KEY_ME_QA << endl;
        h_mMass_ME[KEY_ME_QA]->SetLineColor(2);
        h_mMass_ME[KEY_ME_QA]->SetFillColor(2);
        h_mMass_ME[KEY_ME_QA]->SetFillStyle(3002);
        ...
        string KEY_SM_QA = Form("eta_%d_pt_%d_Centrality_%d_Cos2PhiStarPhi_%d_%s_Dca_%d_Sig_%d_%s_Norm_%d_SM",i_eta,i_pt,icent,itheta,EP[order-1].c_str(),vmsa::Dca_start,vmsa::nSigKaon_start,vmsa::mPID[pid].c_str(),vmsa::Norm_QA);
        //cout << KEY_SM_QA << endl;
        h_mMass_SM[KEY_SM_QA]->SetLineColor(4);
        h_mMass_SM[KEY_SM_QA]->SetFillColor(4);
        h_mMass_SM[KEY_SM_QA]->SetFillStyle(3004);
...
//#endif
          ...
          for(Int_t i_sig = vmsa::nSigKaon_start; i_sig < vmsa::nSigKaon_stop; i_sig++)
          {
            ...
            for(int i_norm = vmsa::Norm_start; i_norm < vmsa::Norm_stop; ++i_norm)
            {
              for(int pt_bin = vmsa::pt_rebin_first_y[energy]; pt_bin <= vmsa::pt_rebin_last_y[energy]; pt_bin++) // pt loop
              {
                ...
                {
                  string KEY = Form("eta_%d_pt_%d_Centrality_%d_Cos2PhiStarPhi_%d_%s_Dca_%d_Sig_%d_%s_Norm_%d",i_eta,pt_bin,cent_bin,i_theta,EP[order-1].c_str(),i_dca,i_sig,vmsa::mPID[pid].c_str(),i_norm);
  ...
  for(int i_pt = vmsa::pt_rebin_first[energy]; i_pt < vmsa::pt_rebin_last[energy]; i_pt++) // pt loop
  {
    ...
    string KEY_QA = Form("pt_%d_Centrality_%d_Cos2PhiStarPhi_%d_2nd_Dca_%d_Sig_%d_%s_Norm_%d",i_pt,vmsa::centStart,0,vmsa::Dca_start,vmsa::nSigKaon_start,vmsa::mPID[pid].c_str(),vmsa::Norm_QA);
  ...
  outputname = Form("figures/%s/%s/rapiditystudy/Cos2PhiStarPhi_InvMassBackgroundSubtracted_Rebinned_%s_Order%d_y%d.pdf",vmsa::mPID[pid].c_str(),vmsa::mBeamEnergy[energy].c_str(),vmsa::mBeamEnergy[energy].c_str(),order,i_eta);
  ...
  for(int itheta = 0; itheta < 10; itheta++)
  {
    for(int icent = 0; icent < 3; icent++)
    {
      for(int i_pt = vmsa::pt_rebin_first_y[energy]; i_pt <= vmsa::pt_rebin_last_y[energy]; i_pt++) // pt loop
      {
        ...
        c_pT_rebin->cd(vmsa::pt_rebin_start[energy][i_pt]+1)->SetTicks(1,1);
        c_pT_rebin->cd(vmsa::pt_rebin_start[energy][i_pt]+1)->SetGrid(0,0);
        string KEY_QA = Form("eta_%d_pt_%d_Centrality_%d_Cos2PhiStarPhi_%d_%s_Dca_%d_Sig_%d_%s_Norm_%d",i_eta,i_pt,icent,itheta,EP[order-1].c_str(),vmsa::Dca_start,vmsa::nSigKaon_start,vmsa::mPID[pid].c_str(),vmsa::Norm_QA);
        //cout << KEY_QA << endl;
        h_mMass[KEY_QA]->SetMarkerStyle(24);
        h_mMass[KEY_QA]->SetMarkerSize(1.2);
        h_mMass[KEY_QA]->SetLineColor(kGray+2);
        h_mMass[KEY_QA]->SetTitle(Form("%.1f<y<%.1f, Cent (%d-%d), %d/5<cos(2#phi*-2#phi)|<%d/5",vmsa::eta_bin[i_eta],vmsa::eta_bin[i_eta+1],centlow[icent],centhigh[icent],itheta-5,itheta-4));
        h_mMass[KEY_QA]->DrawCopy("pE");
        // cout << "KEY_QA = " << KEY_QA.c_str() << endl;
    ...
    for(Int_t i_dca = vmsa::Dca_start; i_dca < vmsa::Dca_stop; i_dca++)
    {
      ...
      { 
        ...
        for(int i_norm = vmsa::Norm_start; i_norm < vmsa::Norm_stop; ++i_norm)
        {
          ...
          for(int i_theta = 0; i_theta < 10; i_theta++) // cos(theta*) loop
          {
            string KEY = Form("eta_%d_Cos2PhiStarPhi_%d_2nd_Dca_%d_Sig_%d_%s_Norm_%d",i_eta,i_theta,i_dca,i_sig,vmsa::mPID[pid].c_str(),i_norm);
    	    if(i_theta == 0) h_mMass_theta[KEY_theta] = (TH1F*)h_mMass[KEY]->Clone(KEY_theta.c_str());
  ...
  for(int i_pt = vmsa::pt_rebin_first[energy]; i_pt < vmsa::pt_rebin_last[energy]; i_pt++) // pt loop
  {
	...
	for(Int_t i_dca = vmsa::Dca_start; i_dca < vmsa::Dca_stop; i_dca++)
	{
	  for(Int_t i_sig = vmsa::nSigKaon_start; i_sig < vmsa::nSigKaon_stop; i_sig++)
	  {
		...
		string KEY = Form("pt_%d_Centrality_%d_Cos2PhiStarPhi_%d_2nd_Dca_%d_Sig_%d_%s_Norm_%d",i_pt,i_cent,i_theta,i_dca,i_sig,vmsa::mPID[pid].c_str(),i_norm);
  ...
  string outputfile = Form("../output/AuAu%s/%s/Cos2PhiStarPhi_InvMassSubBgEta%d_%s.root",vmsa::mBeamEnergy[energy].c_str(),vmsa::mPID[pid].c_str(),i_eta,etamode.c_str());
  if(order == 1) outputfile = Form("../output/AuAu%s/%s/Cos2PhiStarPhi_InvMassSubBgEta%d_%s_FirstOrder.root",vmsa::mBeamEnergy[energy].c_str(),vmsa::mPID[pid].c_str(),i_eta,etamode.c_str());
