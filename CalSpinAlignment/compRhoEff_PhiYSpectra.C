...
void compRhoEff_PhiYSpectra(Int_t energy = 4, Int_t pid = 0, Int_t i_cent = 9, string correction = "AccRes", int order = 2, std::string comp1file = "", std::string comp2file = "", std::string comp1 = "", std::string comp2 = "")
{
  ...
  string inputfile1 = Form("../output/AuAu%s/%s/Rho_%sSysErrors_F_0_eta1_eta1_PolySys%s%s.root",vmsa::mBeamEnergy[energy].c_str(),vmsa::mPID[pid].c_str(),correction.c_str(),ordertext.c_str(),comp1file.c_str());
  ...
  string inputfile2 = Form("../output/AuAu%s/%s/Rho_%sSysErrors_F_0_eta1_eta1_PolySys%s%s.root",vmsa::mBeamEnergy[energy].c_str(),vmsa::mPID[pid].c_str(),correction.c_str(),ordertext.c_str(),comp2file.c_str());
...
//  PlotLine(0.0,5.0,1.0/3.0,1.0/3.0,1,2,2);
  ...
  c_rho_SysError->SaveAs(Form("figures/phi_order%d_pt_%s%s_vs%s.pdf",order,vmsa::mBeamEnergy[energy].c_str(),comp1file.c_str(),comp2file.c_str()));
