...
void calSysErrorWrongF(Int_t energy = 4, Int_t pid = 0, Int_t i_cent = 9, string correction = "AccRes", bool random3D = false, int defaultF = 1)//defaultF = 0 is BESII, defaultF = 1 is BESI
{
  ...
  string inputfile = Form("../output/AuAu%s/%s/WrongF/%sPhiPtSys.root",vmsa::mBeamEnergy[energy].c_str(),vmsa::mPID[pid].c_str(),correction.c_str());
  ...
  std::ofstream fw("ptDependence_Integrated2060_2ndOrder_19GeV.txt", std::ofstream::out);
  //fw.open();

  fw << "sqrt{s_{nn}}      EP Order      dca(cm)        nσ_{kaon}        NormalizationRange         AcceptanceParameter(F)       IntegrationMethod          IntegrationRange             ρ_{00}" << "\n";
  ...
  { 
    for(int i_sigma = vmsa::Sig_start; i_sigma < vmsa::Sig_stop; ++i_sigma)
    {
      for(int i_method = vmsa::Method_start; i_method < vmsa::Method_stop; ++i_method)
      {
        ...
        fw << std::fixed << std::setprecision(1) << vmsa::mDcaSys[i_dca] << "     " << vmsa::mNSigmaKaonSys[0] << "      " << "[" << std::setprecision(2) << vmsa::Norm_Start[0][0] << "," << vmsa::Norm_Stop[0][0] << "]" << "      " << "BES-I" << "     " << vmsa::mInteMethod[i_method] << "     " << "[" << std::setprecision(1) << -vmsa::nSigVecSys[i_sigma] << "Γ," << vmsa::nSigVecSys[i_sigma] << "Γ]" << "       " << std::setprecision(4) << sysDca[idx] << "\n"; 
        idx++;
      }
    }
  }
  ...
  for(Int_t i_sig = vmsa::nSigKaon_start; i_sig < vmsa::nSigKaon_stop; i_sig++)
  {
    for(int i_sigma = vmsa::Sig_start; i_sigma < vmsa::Sig_stop; ++i_sigma)
    {
      for(int i_method = vmsa::Method_start; i_method < vmsa::Method_stop; ++i_method)
      {
        ...
        fw << std::fixed << std::setprecision(1) << vmsa::mDcaSys[0] << "     " << vmsa::mNSigmaKaonSys[i_sig] << "      " << "[" << std::setprecision(2) << vmsa::Norm_Start[0][0] << "," << vmsa::Norm_Stop[0][0] << "]" << "      "<< "BES-I" << "     " << vmsa::mInteMethod[i_method] << "     " << "[" << std::setprecision(1) << -vmsa::nSigVecSys[i_sigma] << "Γ," << vmsa::nSigVecSys[i_sigma] << "Γ]" << "       " << std::setprecision(4) << sysNSig[idx] << "\n"; 
        idx++;
      }
    }
  }
  ...
  for(int i_norm = vmsa::Norm_start; i_norm < vmsa::Norm_stop; ++i_norm)
  {
    for(int i_sigma = vmsa::Sig_start; i_sigma < vmsa::Sig_stop; ++i_sigma)
    {
      for(int i_method = vmsa::Method_start; i_method < vmsa::Method_stop; ++i_method)
      {
        ...
        if(i_sigma < 2) fw << std::fixed << std::setprecision(1) << vmsa::mDcaSys[0] << "     " << vmsa::mNSigmaKaonSys[0] << "      " << "[" << std::setprecision(2) << vmsa::Norm_Start[0][i_sigma] << "," << vmsa::Norm_Stop[0][i_sigma] << "]" << "      "<< "BES-I" << "     " << vmsa::mInteMethod[i_method] << "     " << "[" << std::setprecision(1) << -vmsa::nSigVecSys[i_sigma] << "Γ," << vmsa::nSigVecSys[i_sigma] << "Γ]" << "       " << std::setprecision(4) << sysNorm[idx] << "\n"; 
        if(i_sigma == 2) fw << std::fixed << std::setprecision(1) << vmsa::mDcaSys[0] << "     " << vmsa::mNSigmaKaonSys[0] << "      " << "[" << std::setprecision(2) << vmsa::Norm_Start[0][0] << "," << vmsa::Norm_Stop[0][0] << "]" << "," << "[" << std::setprecision(2) << vmsa::Norm_Start[0][1] << "," << vmsa::Norm_Stop[0][1] << "]" << "      "<< "BES-I" << "     " << vmsa::mInteMethod[i_method] << "     " << "[" << std::setprecision(1) << -vmsa::nSigVecSys[i_sigma] << "Γ," << vmsa::nSigVecSys[i_sigma] << "Γ]" << "       " << std::setprecision(4) << sysNorm[idx] << "\n"; 
        idx++;
      }
    }
  }	 
...
*/
  std::string Fval[2] = {"BES-I", "BES-II"};
  ...
  for(int i_F = 0; i_F < 2; ++i_F)
  {
    for(int i_sigma = vmsa::Sig_start; i_sigma < vmsa::Sig_stop; ++i_sigma)
    {
      for(int i_method = vmsa::Method_start; i_method < vmsa::Method_stop; ++i_method)
      {
        ...
        fw << std::fixed << std::setprecision(1) << vmsa::mDcaSys[0] << "     " << vmsa::mNSigmaKaonSys[0] << "      " << "[" << std::setprecision(2) << vmsa::Norm_Start[0][0] << "," << vmsa::Norm_Stop[0][0] << "]" << "      "<< Fval[i_F] << "     " << vmsa::mInteMethod[i_method] << "     " << "[" << std::setprecision(1) << -vmsa::nSigVecSys[i_sigma] << "Γ," << vmsa::nSigVecSys[i_sigma] << "Γ]" << "       " << std::setprecision(4) << sysF[idx] << "\n"; 
        idx++;
      }
    }
  }
  ...
  cout << "sigma from BESI = " << TMath::Abs((rho_def-0.358672)/TMath::Sqrt(0.007567*0.007567 + 0.007136*0.007136 + rho_defErr*rho_defErr + SysError_rho*SysError_rho)) << endl;
  }
  double val[6];
  double sysE[6];
  double statE[6];
  ...
  TGraph *besi_stat = (TGraph*)besi->Get("rho00_2ndEP_energy_stat;1");
  TGraph *besi_sys = (TGraph*)besi->Get("rho00_2ndEP_energy_sys;1");
