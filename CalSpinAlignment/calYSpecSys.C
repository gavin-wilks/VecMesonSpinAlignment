...
void calYSpecSys(int energy = 4, int pid = 0)
{
  string InPutFile_SE = Form("../data/Yields_%s_SE_%s_20230911_eta1_eta1.root",vmsa::mPID[pid].c_str(),vmsa::mBeamEnergy[energy].c_str());
  ...
  string InPutFile_ME = Form("../data/Yields_%s_ME_%s_20230911_eta1_eta1.root",vmsa::mPID[pid].c_str(),vmsa::mBeamEnergy[energy].c_str());
  ...
  {
    for(int i_cent = 0; i_cent < vmsa::cent_rebin_total; i_cent++) // centrality bin
    {
	...
	for(int i_sig = vmsa::nSigKaon_start; i_sig < vmsa::nSigKaon_stop; i_sig++)
	{
          if( i_dca != 0 && i_sig != 0 ) continue;
            for(int i_y = 0; i_y < 20; i_y++)
            {
	      string KEY_InPutSE = Form("Spec_eta_%d_pt_%d_Centrality_%d_Dca_%d_Sig_%d_%s_SE",i_y,i_pt,i_cent,i_dca,i_sig,vmsa::mPID[pid].c_str());
	      h_mInPut_SE[KEY_InPutSE] = (TH1F*)File_SE->Get(KEY_InPutSE.c_str())->Clone(); 

	      string KEY_InPutME = Form("Spec_eta_%d_pt_%d_Centrality_%d_Dca_%d_Sig_%d_%s_ME",i_y,i_pt,i_cent,i_dca,i_sig,vmsa::mPID[pid].c_str());
	      ...
	      {
	        string KEY_SE = Form("Spec_eta_%d_pt_%d_Centrality_%d_Dca_%d_Sig_%d_%s_Norm_%d_SE",i_y,i_pt,i_cent,i_dca,i_sig,vmsa::mPID[pid].c_str(),i_norm);
	        h_mMassSpec_SE[KEY_SE] = (TH1F*)h_mInPut_SE[KEY_InPutSE]->Clone(KEY_SE.c_str());

	        string KEY_ME = Form("Spec_eta_%d_pt_%d_Centrality_%d_Dca_%d_Sig_%d_%s_Norm_%d_ME",i_y,i_pt,i_cent,i_dca,i_sig,vmsa::mPID[pid].c_str(),i_norm);
	        h_mMassSpec_ME[KEY_ME] = (TH1F*)h_mInPut_ME[KEY_InPutME]->Clone(KEY_ME.c_str());

	        string KEY = Form("Spec_eta_%d_pt_%d_Centrality_%d_Dca_%d_Sig_%d_%s_Norm_%d",i_y,i_pt,i_cent,i_dca,i_sig,vmsa::mPID[pid].c_str(),i_norm);
  ...
  string KEY_SE_QA = Form("Spec_eta_%d_pt_%d_Centrality_%d_Dca_%d_Sig_%d_%s_Norm_%d_SE",0,vmsa::pt_RawQA[energy],2,vmsa::Dca_start,vmsa::nSigKaon_start,vmsa::mPID[pid].c_str(),vmsa::Norm_QA);
  ...
  string KEY_ME_QA = Form("Spec_eta_%d_pt_%d_Centrality_%d_Dca_%d_Sig_%d_%s_Norm_%d_ME",0,vmsa::pt_RawQA[energy],2,vmsa::Dca_start,vmsa::nSigKaon_start,vmsa::mPID[pid].c_str(),vmsa::Norm_QA);
  ...
  string KEY_QA = Form("Spec_eta_%d_pt_%d_Centrality_%d_Dca_%d_Sig_%d_%s_Norm_%d",0,vmsa::pt_RawQA[energy],2,vmsa::Dca_start,vmsa::nSigKaon_start,vmsa::mPID[pid].c_str(),vmsa::Norm_QA);
  ...
  for(int i_pt = vmsa::pt_start; i_pt < vmsa::pt_stop; i_pt++) // pt loop
  {
    ...
    string KEY_SE_QA = Form("Spec_eta_%d_pt_%d_Centrality_%d_Dca_%d_Sig_%d_%s_Norm_%d_SE",0,i_pt,2,vmsa::Dca_start,vmsa::nSigKaon_start,vmsa::mPID[pid].c_str(),vmsa::Norm_QA);
    h_mMassSpec_SE[KEY_SE_QA]->DrawCopy("PE");

    string KEY_ME_QA = Form("Spec_eta_%d_pt_%d_Centrality_%d_Dca_%d_Sig_%d_%s_Norm_%d_ME",0,i_pt,2,vmsa::Dca_start,vmsa::nSigKaon_start,vmsa::mPID[pid].c_str(),vmsa::Norm_QA);
    h_mMassSpec_ME[KEY_ME_QA]->SetLineColor(2);
    h_mMassSpec_ME[KEY_ME_QA]->SetFillColor(2);
    h_mMassSpec_ME[KEY_ME_QA]->SetFillStyle(3002);
    h_mMassSpec_ME[KEY_ME_QA]->DrawCopy("h same");

    string KEY_QA = Form("Spec_eta_%d_pt_%d_Centrality_%d_Dca_%d_Sig_%d_%s_Norm_%d",0,i_pt,2,vmsa::Dca_start,vmsa::nSigKaon_start,vmsa::mPID[pid].c_str(),vmsa::Norm_QA);
    h_mMassSpec[KEY_QA]->SetLineColor(4);
    h_mMassSpec[KEY_QA]->SetFillColor(4);
    h_mMassSpec[KEY_QA]->SetFillStyle(3004);
    h_mMassSpec[KEY_QA]->DrawCopy("h same");
  ...
  for(int i_y = 0; i_y < 20; i_y++) // pt bin
  {
    for(int i_pt = 0; i_pt < vmsa::pt_total; i_pt++) // pt bin
    {
      for(int i_cent = 0; i_cent < vmsa::cent_rebin_total; i_cent++) // centrality bin
          ...
          for(int i_sig = vmsa::nSigKaon_start; i_sig < vmsa::nSigKaon_stop; i_sig++)
          {
            ...
            for(int i_norm = vmsa::Norm_start; i_norm < vmsa::Norm_stop; ++i_norm)
            {
        	  ...
        	  for(int i_sigma = vmsa::Sig_start; i_sigma < vmsa::Sig_stop; ++i_sigma)
        	  {
        	    ...
        	    string KEY_yields = Form("Spec_eta_%d_pt_%d_Centrality_%d_Dca_%d_Sig_%d_%s_Norm_%d_Sigma_%d",i_y,i_pt,i_cent,i_dca,i_sig,vmsa::mPID[pid].c_str(),i_norm,i_sigma);
  ...
  float pt_low[26], pt_up[26], pt_width[26];
  for(int i_pt = vmsa::pt_start; i_pt < vmsa::pt_stop; i_pt++)
  {
    pt_low[i_pt] = vmsa::ptRawStart[i_pt];
    pt_up[i_pt]  = vmsa::ptRawStop[i_pt];
    pt_width[i_pt] = pt_up[i_pt]-pt_low[i_pt];
  }
  pt_low[25] = 8.0; // make sure pT = 8.0 in the histogram
  pt_up[25] = 10.0;
  pt_width[25] = pt_up[25]-pt_low[25];
  ...
  //              {
  //                string KEY_pT_counts = Form("Spec_pt_%d_Centrality_%d_Dca_%d_Sig_%d_%s_Norm_%d_Sigma_%d_Count",pt_bin,cent_bin,i_dca,i_sig,vmsa::mPID[pid].c_str(),i_norm,i_sigma);
  //                string KEY_pT_inte   = Form("Spec_pt_%d_Centrality_%d_Dca_%d_Sig_%d_%s_Norm_%d_Sigma_%d_Inte",pt_bin,cent_bin,i_dca,i_sig,vmsa::mPID[pid].c_str(),i_norm,i_sigma);
  //                if(i_pt == vmsa::pt_rebin_start_y[energy][pt_bin] && i_cent == vmsa::cent_rebin[cent_bin])
  //                {
  //                  h_mPt[KEY_pT_counts] = new TH2F(KEY_pT_counts.c_str(),KEY_pT_counts.c_str(),25,0.0,1.0,2*vmsa::pt_total,pt_low);
  //                  h_mPt[KEY_pT_inte]   = new TH2F(KEY_pT_inte.c_str(),KEY_pT_inte.c_str(),25,0.0,1.0,2*vmsa::pt_total,pt_low);
  //                }
  //                for(int i_pt = vmsa::pt_start; i_pt < vmsa::pt_stop; i_pt++) // pt loop
  //                {
    ...
    for(int i_sig = vmsa::nSigKaon_start; i_sig < vmsa::nSigKaon_stop; i_sig++)
    {
      ...
      for(int i_norm = vmsa::Norm_start; i_norm < vmsa::Norm_stop; ++i_norm)
      {
        for(int i_sigma = vmsa::Sig_start; i_sigma < vmsa::Sig_stop; ++i_sigma)
        {
          ...
          {
	    string KEY_pT_counts = Form("Spec_Centrality_%d_Dca_%d_Sig_%d_%s_Norm_%d_Sigma_%d_Count",i_pt,i_cent,i_dca,i_sig,vmsa::mPID[pid].c_str(),i_norm,i_sigma);
	    string KEY_pT_inte   = Form("Spec_Centrality_%d_Dca_%d_Sig_%d_%s_Norm_%d_Sigma_%d_Inte",i_pt,i_cent,i_dca,i_sig,vmsa::mPID[pid].c_str(),i_norm,i_sigma);
            h_mPt[KEY_pT_counts] = new TH2F(KEY_pT_counts.c_str(),KEY_pT_counts.c_str(),20,0.0,1.0,2*vmsa::pt_total,pt_low);
	    h_mPt[KEY_pT_inte]   = new TH2F(KEY_pT_inte.c_str(),KEY_pT_inte.c_str(),20,0.0,1.0,2*vmsa::pt_total,pt_low);
            for(int i_pt = vmsa::pt_start; i_pt <= vmsa::pt_stop; i_pt++)
            {
	      for(int i_y = 0; i_y < 20; i_y++) 
	      {
	        string KEY_yields = Form("Spec_eta_%d_pt_%d_Centrality_%d_Dca_%d_Sig_%d_%s_Norm_%d_Sigma_%d",i_y,i_pt,i_cent,i_dca,i_sig,vmsa::mPID[pid].c_str(),i_norm,i_sigma);
	        //h_mPt[KEY_pT_counts]->SetBinContent(i_y,i_pt+1,yields_count[KEY_yields][0]/pt_width[i_pt]);
	        //h_mPt[KEY_pT_counts]->SetBinError(i_y,i_pt+1,yields_count[KEY_yields][1]/TMath::Sqrt(pt_width[i_pt]));
	        //h_mPt[KEY_pT_inte]->SetBinContent(i_y,i_pt+1,yields_inte[KEY_yields][0]/pt_width[i_pt]);
	        //h_mPt[KEY_pT_inte]->SetBinError(i_y,i_pt+1,yields_inte[KEY_yields][1]/TMath::Sqrt(pt_width[i_pt])); 
	        h_mPt[KEY_pT_counts]->SetBinContent(i_y,i_pt+1,yields_count[KEY_yields][0]*20.);///pt_width[i_pt]);
	        h_mPt[KEY_pT_counts]->SetBinError(i_y,i_pt+1,yields_count[KEY_yields][1]/TMath::Sqrt(1./20.));
	        h_mPt[KEY_pT_inte]->SetBinContent(i_y,i_pt+1,yields_inte[KEY_yields][0]*20.);///pt_width[i_pt]);
	        h_mPt[KEY_pT_inte]->SetBinError(i_y,i_pt+1,yields_inte[KEY_yields][1]/TMath::Sqrt(1./20.)); 
	      } 
            }
          }
	}
  ...
  //c_Pt->cd()->SetLogy();
  //c_Pt->cd()->SetLogy();
  string KEY_pT_counts_QA = Form("Spec_Centrality_%d_Dca_%d_Sig_%d_%s_Norm_%d_Sigma_%d_Count",2,vmsa::Dca_start,vmsa::nSigKaon_start,vmsa::mPID[pid].c_str(),vmsa::Norm_QA,vmsa::Sig_QA);
  ...
  h_play->GetYaxis()->SetTitle("dN/dy");
  h_play->GetYaxis()->CenterTitle();
  h_play->GetYaxis()->SetTitleOffset(1.2);
  h_play->GetYaxis()->SetRangeUser(-5.0,1.2*h_mPt[KEY_pT_counts_QA]->GetMaximum());
  h_play->GetXaxis()->SetRangeUser(0.0,1.0);//-5.0,1.2*h_mPt[KEY_pT_counts_QA]->GetMaximum());
  h_play->DrawCopy("pE");
  TH1F *h_y_counts = h_mPt[KEY_pT_counts_QA]->ProjectionY("_py",5,9,"e");
  h_y_counts->SetMarkerStyle(24);
  h_y_counts->SetMarkerColor(4);
  h_y_counts->SetMarkerSize(1.0);
  h_y_counts->DrawCopy("pE same");

  string KEY_pT_inte_QA = Form("Spec_Centrality_%d_Dca_%d_Sig_%d_%s_Norm_%d_Sigma_%d_Inte",2,vmsa::Dca_start,vmsa::nSigKaon_start,vmsa::mPID[pid].c_str(),vmsa::Norm_QA,vmsa::Sig_QA);
  TH1F *h_y_inte = h_mPt[KEY_pT_inte_QA]->ProjectionY("_py_inte",5,9,"e");
  h_y_inte->SetMarkerColor(2);
  h_y_inte->SetMarkerStyle(24);
  h_y_inte->SetMarkerSize(1.0);
  h_y_inte->DrawCopy("pE same");
#endif

  string outputfile = Form("../output/AuAu%s/%s/YSpectraSys.root",vmsa::mBeamEnergy[energy].c_str(),vmsa::mPID[pid].c_str());
