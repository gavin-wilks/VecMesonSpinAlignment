...
void subBackGround_Helicity_NoAbs(int energy = 4, int pid = 0, int year = 0, string date = "20240521", bool random3D = false, int order = 2, string etamode = "eta1_eta1")
{
  ...
  string InPutFile_SE = Form("../data/Yields_Phi_SE_%s_%s_%s_Helicity_ExactSameEvents_NoAbs.root",vmsa::mBeamEnergy[energy].c_str(),date.c_str(),etamode.c_str());
  ...
  string InPutFile_ME = Form("../data/Yields_Phi_ME_%s_%s_%s_Helicity_ExactSameEvents_NoAbs.root",vmsa::mBeamEnergy[energy].c_str(),date.c_str(),etamode.c_str());
  ...
  for(Int_t i_pt = 0; i_pt < vmsa::pt_total; i_pt++) // pt bin 
  {
    ...
    {
      for(Int_t i_theta = 0; i_theta < 2*vmsa::CTS_total; i_theta++) // phi-psi bin
      {
  ...
  for(int itheta = 0; itheta < 2*vmsa::CTS_total; itheta++)
  {
...
//#endif
  ...
  string outputfile = Form("../output/AuAu%s/%s/InvMassSubBg_%s_Helicity_NoAbs.root",vmsa::mBeamEnergy[energy].c_str(),vmsa::mPID[pid].c_str(),etamode.c_str());
