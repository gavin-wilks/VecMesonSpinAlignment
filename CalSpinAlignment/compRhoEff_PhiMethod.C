...
void compRhoEff_PhiMethod(Int_t energy = 4, Int_t pid = 0, Int_t i_cent = 9, string correction = "AccRes")
{
  ...
  string inputfile1 = Form("../output/AuAu%s/%s/RawPhiPtSys_%s.root",vmsa::mBeamEnergy[energy].c_str(),vmsa::mPID[pid].c_str(),"eta1_eta1");
  ...
  string inputfile2 = Form("../output/AuAu%s/%s/RawPhiPtSys_%s_InvMass.root",vmsa::mBeamEnergy[energy].c_str(),vmsa::mPID[pid].c_str(),"eta1_eta1");

  TFile *File_InPut1 = TFile::Open(inputfile1.c_str());
  TFile *File_InPut2 = TFile::Open(inputfile2.c_str());
  
  TString StatErrorRho = Form("rhoRaw_Centrality_%d_2nd_Dca_%d_Sig_%d_%s_Norm_%d_Sigma_%d_%s",9,0,0,vmsa::mPID[pid].c_str(),0,0,vmsa::mInteMethod[1].c_str());
...
//  PlotLine(0.0,5.0,1.0/3.0,1.0/3.0,1,2,2);
  ...
  //}
    ...
    string leg_count = "<cos^{2}#theta*> vs Inv. Mass Method";
    plotTopLegend((char*)leg_count.c_str(),0.65,0.308,0.03,1,0.0,42,0);

    string leg_besi = "Yield vs. |cos#theta*| Method";
