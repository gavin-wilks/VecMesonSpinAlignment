...
void subBackGroundP_Global_2D_OffDiag(int energy = 4, int pid = 0, int year = 0, string date = "20241010", bool random3D = false, int order = 2, string etamode = "eta1_eta1", int frameopt = 1)
{
  ...
  string InPutFile_SE = Form("../data/Yields_Phi_SE_%s_%s_%s_%s_2D_Study2.root",vmsa::mBeamEnergy[energy].c_str(),date.c_str(),etamode.c_str(),frame.c_str());
  //if(energy == 3) InPutFile_SE = Form("../data/Yields_Phi_SE_14GeV_%s.root",etamode.c_str());
  if(order == 1) InPutFile_SE = Form("../data/Yields_Phi_SE_%s_%s_%s_%s_2D_Study2_FirstOrder.root",vmsa::mBeamEnergy[energy].c_str(),date.c_str(),etamode.c_str(),frame.c_str());
  ...
  string InPutFile_ME = Form("../data/Yields_Phi_ME_%s_%s_%s_%s_2D_Study2.root",vmsa::mBeamEnergy[energy].c_str(),date.c_str(),etamode.c_str(),frame.c_str());
  //if(energy == 3) InPutFile_ME = Form("../data/Yields_Phi_ME_14GeV_%s.root",etamode.c_str());
  if(order == 1) InPutFile_ME = Form("../data/Yields_Phi_ME_%s_%s_%s_%s_2D_Study2_FirstOrder.root",vmsa::mBeamEnergy[energy].c_str(),date.c_str(),etamode.c_str(),frame.c_str());
  ...
  for(Int_t i_pt = 0; i_pt < vmsa::p_total_2D; i_pt++) // pt bin 
  {
    for(Int_t i_cent = 9; i_cent < vmsa::Cent_stop; i_cent++) // centrality bin
    {
      for(Int_t i_theta = 0; i_theta < 9; i_theta++) // phi-psi bin
      {
        for(Int_t i_phipsi = 0; i_phipsi < 12; i_phipsi++) // phi-psi bin
        {
          for(Int_t i_dca = vmsa::Dca_start; i_dca < vmsa::Dca_stop; i_dca++)
          {
            for(Int_t i_sig = vmsa::nSigKaon_start; i_sig < vmsa::nSigKaon_stop; i_sig++)
            {
              ...
              string KEY_InPutSE = Form("p_%d_Centrality_%d_CosThetaStar_%d_PhiPrime_%d_%s_Dca_%d_Sig_%d_%s_SE",i_pt,i_cent,i_theta,i_phipsi,EP[order-1].c_str(),i_dca,i_sig,vmsa::mPID[pid].c_str());
              h_mInPut_SE[KEY_InPutSE] = (TH1F*)File_SE->Get(KEY_InPutSE.c_str())->Clone(); 

              string KEY_InPutME = Form("p_%d_Centrality_%d_CosThetaStar_%d_PhiPrime_%d_%s_Dca_%d_Sig_%d_%s_ME",i_pt,i_cent,i_theta,i_phipsi,EP[order-1].c_str(),i_dca,i_sig,vmsa::mPID[pid].c_str());
              ...
              {
                string KEY_SE = Form("p_%d_Centrality_%d_CosThetaStar_%d_PhiPrime_%d_%s_Dca_%d_Sig_%d_%s_Norm_%d_SE",i_pt,i_cent,i_theta,i_phipsi,EP[order-1].c_str(),i_dca,i_sig,vmsa::mPID[pid].c_str(),i_norm);
                h_mMass_SE[KEY_SE] = (TH1F*)h_mInPut_SE[KEY_InPutSE]->Clone(KEY_SE.c_str());
                string KEY_ME = Form("p_%d_Centrality_%d_CosThetaStar_%d_PhiPrime_%d_%s_Dca_%d_Sig_%d_%s_Norm_%d_ME",i_pt,i_cent,i_theta,i_phipsi,EP[order-1].c_str(),i_dca,i_sig,vmsa::mPID[pid].c_str(),i_norm);
                h_mMass_ME[KEY_ME] = (TH1F*)h_mInPut_ME[KEY_InPutME]->Clone(KEY_ME.c_str());
                string KEY_SM = Form("p_%d_Centrality_%d_CosThetaStar_%d_PhiPrime_%d_%s_Dca_%d_Sig_%d_%s_Norm_%d_SM",i_pt,i_cent,i_theta,i_phipsi,EP[order-1].c_str(),i_dca,i_sig,vmsa::mPID[pid].c_str(),i_norm);
  ...
  for(int i_pt = 0; i_pt < vmsa::p_total_2D; i_pt++)
  {
    ...
    string KEY_SE_QA = Form("p_%d_Centrality_%d_CosThetaStar_%d_PhiPrime_%d_%s_Dca_%d_Sig_%d_%s_Norm_%d_SE",i_pt,9,vmsa::CTS_start,0,EP[order-1].c_str(),vmsa::Dca_start,vmsa::nSigKaon_start,vmsa::mPID[pid].c_str(),vmsa::Norm_QA);
    ...
    string KEY_ME_QA = Form("p_%d_Centrality_%d_CosThetaStar_%d_PhiPrime_%d_%s_Dca_%d_Sig_%d_%s_Norm_%d_ME",i_pt,9,vmsa::CTS_start,0,EP[order-1].c_str(),vmsa::Dca_start,vmsa::nSigKaon_start,vmsa::mPID[pid].c_str(),vmsa::Norm_QA);
    ...
    string KEY_SM_QA = Form("p_%d_Centrality_%d_CosThetaStar_%d_PhiPrime_%d_%s_Dca_%d_Sig_%d_%s_Norm_%d_SM",i_pt,9,vmsa::CTS_start,0,EP[order-1].c_str(),vmsa::Dca_start,vmsa::nSigKaon_start,vmsa::mPID[pid].c_str(),vmsa::Norm_QA);
    ...
    c_peak->SaveAs(Form("figures/%s/%s/pstudy/%s2D_CloseUpMixedEvent_%s_Order%d_Cent0_pt%d.pdf",vmsa::mPID[pid].c_str(),vmsa::mBeamEnergy[energy].c_str(),frame.c_str(),vmsa::mBeamEnergy[energy].c_str(),order,i_pt));
  }
  ...
  string outputname = Form("figures/%s/%s/pstudy/%s2D_InvMassDistributions_%s_Order%d_Cent9.pdf",vmsa::mPID[pid].c_str(),vmsa::mBeamEnergy[energy].c_str(),frame.c_str(),vmsa::mBeamEnergy[energy].c_str(),order);
  string output_start = Form("%s[",outputname.c_str());
  
  c_pT->Print(output_start.c_str());
  for(int itheta = 0; itheta < 9; itheta++)
  {
    for(int i_phipsi = 0; i_phipsi < 12; i_phipsi++)
    {
      for(int i_pt = vmsa::p_start_2D; i_pt < vmsa::p_stop_2D; i_pt++) // pt loop
      {
        c_pT->cd(i_pt+1);
        ...
        string KEY_SE_QA = Form("p_%d_Centrality_%d_CosThetaStar_%d_PhiPrime_%d_%s_Dca_%d_Sig_%d_%s_Norm_%d_SE",i_pt,9,itheta,i_phipsi,EP[order-1].c_str(),vmsa::Dca_start,vmsa::nSigKaon_start,vmsa::mPID[pid].c_str(),vmsa::Norm_QA);
        h_mMass_SE[KEY_SE_QA]->GetYaxis()->SetRangeUser(-0.1*h_mMass_SE[KEY_SE_QA]->GetMaximum(),1.1*h_mMass_SE[KEY_SE_QA]->GetMaximum());
        h_mMass_SE[KEY_SE_QA]->SetTitle(Form("%1.1f/4.5<cos(#theta*)<%1.1f/4.5, %d#pi/6<#phi'<%d#pi/6",float(itheta)-4.5,float(itheta)-3.5,i_phipsi,i_phipsi+1));
        h_mMass_SE[KEY_SE_QA]->DrawCopy();

        string KEY_ME_QA = Form("p_%d_Centrality_%d_CosThetaStar_%d_PhiPrime_%d_%s_Dca_%d_Sig_%d_%s_Norm_%d_ME",i_pt,9,itheta,i_phipsi,EP[order-1].c_str(),vmsa::Dca_start,vmsa::nSigKaon_start,vmsa::mPID[pid].c_str(),vmsa::Norm_QA);
        h_mMass_ME[KEY_ME_QA]->SetLineColor(2);
        h_mMass_ME[KEY_ME_QA]->SetFillColor(2);
        h_mMass_ME[KEY_ME_QA]->SetFillStyle(3002);
        h_mMass_ME[KEY_ME_QA]->DrawCopy("h same");

        string KEY_SM_QA = Form("p_%d_Centrality_%d_CosThetaStar_%d_PhiPrime_%d_%s_Dca_%d_Sig_%d_%s_Norm_%d_SM",i_pt,9,itheta,i_phipsi,EP[order-1].c_str(),vmsa::Dca_start,vmsa::nSigKaon_start,vmsa::mPID[pid].c_str(),vmsa::Norm_QA);
        h_mMass_SM[KEY_SM_QA]->SetLineColor(4);
        h_mMass_SM[KEY_SM_QA]->SetFillColor(4);
        h_mMass_SM[KEY_SM_QA]->SetFillStyle(3004);
        h_mMass_SM[KEY_SM_QA]->DrawCopy("h same");
  ...
  {
    for(int i_theta = vmsa::CTS_start; i_theta < 9; i_theta++) // cos(theta*) loop
    {
      for(int i_phipsi = 0; i_phipsi < 12; i_phipsi++) // cos(theta*) loop
      {
        for(Int_t i_dca = vmsa::Dca_start; i_dca < vmsa::Dca_stop; i_dca++)
        {
          for(Int_t i_sig = vmsa::nSigKaon_start; i_sig < vmsa::nSigKaon_stop; i_sig++)
          {
            ...
            for(int i_norm = vmsa::Norm_start; i_norm < vmsa::Norm_stop; ++i_norm)
            {
              for(int p_bin = vmsa::p_rebin_first_2D[energy]; p_bin < vmsa::p_rebin_last_2D[energy]; p_bin++) // pt loop
              {
                string KEY = Form("p_%d_Centrality_%d_CosThetaStar_%d_PhiPrime_%d_%s_Dca_%d_Sig_%d_%s_Norm_%d",p_bin,i_cent,i_theta,i_phipsi,EP[order-1].c_str(),i_dca,i_sig,vmsa::mPID[pid].c_str(),i_norm);
                for(int i_pt = vmsa::p_rebin_start_2D[energy][p_bin]; i_pt <= vmsa::p_rebin_stop_2D[energy][p_bin]; i_pt++)
                {
          	string KEY_SM = Form("p_%d_Centrality_%d_CosThetaStar_%d_PhiPrime_%d_%s_Dca_%d_Sig_%d_%s_Norm_%d_SM",i_pt,i_cent,i_theta,i_phipsi,EP[order-1].c_str(),i_dca,i_sig,vmsa::mPID[pid].c_str(),i_norm);
          	// cout << "KEY= " << KEY.c_str() << ", KEY_SM = " << KEY_SM.c_str() << endl;
          	if(i_pt == vmsa::p_rebin_start_2D[energy][p_bin])
          	{
  ...
  outputname = Form("figures/%s/%s/pstudy/%s2D_InvMassBackgroundSubtracted_Rebinned_%s_Order%d_Cent9.pdf",vmsa::mPID[pid].c_str(),vmsa::mBeamEnergy[energy].c_str(),frame.c_str(),vmsa::mBeamEnergy[energy].c_str(),order);
  ...
  for(int itheta = 0; itheta < 9; itheta++)
  {
    for(int i_phipsi = 0; i_phipsi < 12; i_phipsi++)
    {
      for(int i_pt = vmsa::p_rebin_first_2D[energy]; i_pt < vmsa::p_rebin_last_2D[energy]; i_pt++) // pt loop
      {
        c_pT_rebin->cd(vmsa::p_rebin_start_2D[energy][i_pt]+1);
        c_pT_rebin->cd(vmsa::p_rebin_start_2D[energy][i_pt]+1)->SetLeftMargin(0.15);
        c_pT_rebin->cd(vmsa::p_rebin_start_2D[energy][i_pt]+1)->SetBottomMargin(0.15);
        c_pT_rebin->cd(vmsa::p_rebin_start_2D[energy][i_pt]+1)->SetTicks(1,1);
        c_pT_rebin->cd(vmsa::p_rebin_start_2D[energy][i_pt]+1)->SetGrid(0,0);
        string KEY_QA = Form("p_%d_Centrality_%d_CosThetaStar_%d_PhiPrime_%d_%s_Dca_%d_Sig_%d_%s_Norm_%d",i_pt,9,itheta,i_phipsi,EP[order-1].c_str(),vmsa::Dca_start,vmsa::nSigKaon_start,vmsa::mPID[pid].c_str(),vmsa::Norm_QA);
        h_mMass[KEY_QA]->SetMarkerStyle(24);
        h_mMass[KEY_QA]->SetMarkerSize(1.2);
        h_mMass[KEY_QA]->SetLineColor(kGray+2);
        h_mMass[KEY_QA]->SetTitle(Form("%1.1f/4.5<cos(#theta*)<%1.1f/4.5, %d#pi/6<#phi'<%d#pi/6",float(itheta)-4.5,float(itheta)-3.5,i_phipsi,i_phipsi+1));
        h_mMass[KEY_QA]->DrawCopy("pE");
        // cout << "KEY_QA = " << KEY_QA.c_str() << endl;

        string pT_range = Form("[%.2f,%.2f]",vmsa::p_low_2D[energy][i_pt],vmsa::p_up_2D[energy][i_pt]);
        plotTopLegend((char*)pT_range.c_str(),0.2,0.7,0.08,1,0.0,42,1);
...
//    for(int i_pt = vmsa::p_rebin_first[energy]; i_pt < vmsa::p_rebin_last[energy]; i_pt++) // pt loop
//    {
//      for(int i_cent = 9; i_cent < vmsa::Cent_stop; i_cent++) // Centrality loop
//      {
//	for(Int_t i_dca = vmsa::Dca_start; i_dca < vmsa::Dca_stop; i_dca++)
...
//            for(int i_norm = vmsa::Norm_start; i_norm < vmsa::Norm_stop; ++i_norm)
//	    {
//	      string KEY_theta = Form("p_%d_Centrality_%d_%s_Dca_%d_Sig_%d_%s_Norm_%d",i_pt,i_cent,EP[order-1].c_str(),i_dca,i_sig,vmsa::mPID[pid].c_str(),i_norm);
//	      for(int i_theta = vmsa::CTS_start; i_theta < 9; i_theta++) // cos(theta*) loop
//	      {
//	        for(int i_phipsi = 0; i_phipsi < 12; i_phipsi++) // cos(theta*) loop
//	        {
//		  string KEY = Form("p_%d_Centrality_%d_CosThetaStar_%d_PhiPrime_%d_%s_Dca_%d_Sig_%d_%s_Norm_%d",i_pt,i_cent,i_theta,i_phipsi,EP[order-1].c_str(),i_dca,i_sig,vmsa::mPID[pid].c_str(),i_norm);
//		  if(i_theta == vmsa::CTS_start && i_phipsi == 0) h_mMass_theta[KEY_theta] = (TH1F*)h_mMass[KEY]->Clone(KEY_theta.c_str());
...
//    for(int i_pt = vmsa::p_rebin_first[energy]; i_pt < vmsa::p_rebin_last[energy]; i_pt++) // pt loop
//    {
//      c_mMass_theta->cd(vmsa::p_rebin_start[energy][i_pt]+1);
//      c_mMass_theta->cd(vmsa::p_rebin_start[energy][i_pt]+1)->SetLeftMargin(0.15);
//      c_mMass_theta->cd(vmsa::p_rebin_start[energy][i_pt]+1)->SetBottomMargin(0.15);
//      c_mMass_theta->cd(vmsa::p_rebin_start[energy][i_pt]+1)->SetTicks(1,1);
//      c_mMass_theta->cd(vmsa::p_rebin_start[energy][i_pt]+1)->SetGrid(0,0);
//      string KEY_theta_QA = Form("p_%d_Centrality_%d_%s_Dca_%d_Sig_%d_%s_Norm_%d",i_pt,9,EP[order-1].c_str(),vmsa::Dca_start,vmsa::nSigKaon_start,vmsa::mPID[pid].c_str(),vmsa::Norm_QA);
...
//	      {
//		string KEY_theta = Form("p_%d_Centrality_%d_2nd_Dca_%d_Sig_%d_%s_Norm_%d",i_pt,i_cent,i_dca,i_sig,vmsa::mPID[pid].c_str(),i_norm);
...
//		string KEY = Form("p_%d_Centrality_%d_CosThetaStar_%d_PhiPrime_%d_2nd_Dca_%d_Sig_%d_%s_Norm_%d",i_pt,i_cent,i_theta,i_dca,i_sig,vmsa::mPID[pid].c_str(),i_norm);
...
////    for(int i_pt = vmsa::p_rebin_first[energy]; i_pt < vmsa::p_rebin_last[energy]; i_pt++) // pt loop
////    {
////      c_mMass_phi_diff->cd(vmsa::p_rebin_start[energy][i_pt]+1);
////      c_mMass_phi_diff->cd(vmsa::p_rebin_start[energy][i_pt]+1)->SetLeftMargin(0.15);
////      c_mMass_phi_diff->cd(vmsa::p_rebin_start[energy][i_pt]+1)->SetBottomMargin(0.15);
////      c_mMass_phi_diff->cd(vmsa::p_rebin_start[energy][i_pt]+1)->SetTicks(1,1);
////      c_mMass_phi_diff->cd(vmsa::p_rebin_start[energy][i_pt]+1)->SetGrid(0,0);
////      string KEY_QA = Form("p_%d_Centrality_%d_CosThetaStar_%d_PhiPrime_%d_%s_Dca_%d_Sig_%d_%s_Norm_%d",i_pt,9,vmsa::CTS_start,EP[order-1].c_str(),vmsa::Dca_start,vmsa::nSigKaon_start,vmsa::mPID[pid].c_str(),vmsa::Norm_QA);
////      h_mMass[KEY_QA]->SetMarkerColor(1);
////      h_mMass[KEY_QA]->SetMarkerStyle(24);
////      h_mMass[KEY_QA]->SetMarkerSize(0.8);
////      h_mMass[KEY_QA]->DrawCopy("PE");
////
////      string pT_range = Form("[%.2f,%.2f]",vmsa::p_low[energy][i_pt],vmsa::p_up[energy][i_pt]);
////      plotTopLegend((char*)pT_range.c_str(),0.2,0.7,0.08,1,0.0,42,1);
////      PlotLine(vmsa::InvMass_low[pid],vmsa::InvMass_high[pid],0.0,0.0,1,2,2);
////    }
////#endif
//  }
  ...
  string outputfile = Form("../output/AuAu%s/%s/%s2DP_InvMassSubBg_%s.root",vmsa::mBeamEnergy[energy].c_str(),vmsa::mPID[pid].c_str(),frame.c_str(),etamode.c_str());
  //string outputfile = Form("../output/AuAu%s/%s/InvMassSubBg.root",vmsa::mBeamEnergy[energy].c_str(),vmsa::mPID[pid].c_str());
  if(order == 1) outputfile = Form("../output/AuAu%s/%s/%s2DP_InvMassSubBg_%s_FirstOrder.root",vmsa::mBeamEnergy[energy].c_str(),vmsa::mPID[pid].c_str(),frame.c_str(),etamode.c_str());
