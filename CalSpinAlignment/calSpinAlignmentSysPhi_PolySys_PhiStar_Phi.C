...
void calSpinAlignmentSysPhi_PolySys_PhiStar_Phi(int energy = 4, int pid = 0, int year = 0, bool random3D = false, int order = 2, string etamode = "eta1_eta1")
{
  ...
  string inputfile = Form("../output/AuAu%s/%s/Cos2PhiStarPhi_InvMassSubBg_%s_Phi.root",vmsa::mBeamEnergy[energy].c_str(),vmsa::mPID[pid].c_str(),etamode.c_str());
...
//#endif
  ...
  */
	          ...
	          for(int i_theta = 0; i_theta < 10; ++i_theta) // cos(theta*) loop
	          {
	            ...
	            if(i_method == 1)
	            {
	              ...
	              h_mCounts[KEY_counts]->SetBinError(h_mCounts[KEY_counts]->FindBin(bin_center),TMath::Sqrt(errors_bw*errors_bw+errors_bg*errors_bg));
                      ...
                      cout << "counts_bw - counts_bg= " << counts_bw-counts_bg << endl;
	            }
  ...
  for(int i_poly = 0; i_poly < 1; i_poly++)
  {
    for(int i_norm = 0; i_norm < 1; i_norm++)
    {
      ...
      {
        string outputname = Form("./figures/%s/%s/pTstudy/Cos2PhiStarPhi_Phi_allThetaYields%s_%s_Order%d_cent%d_%s_pTdependence_Norm%d_Poly%d.pdf",vmsa::mPID[pid].c_str(),vmsa::mBeamEnergy[energy].c_str(),vmsa::mPID[pid].c_str(),vmsa::mBeamEnergy[energy].c_str(),order,i_cent,etamode.c_str(),i_norm,i_poly+1);
 ...
 // {
  ...
  string outputfile = Form("../output/AuAu%s/%s/Cos2PhiStarPhi_Phi_RawPhiPtSys_%s_PolySys.root",vmsa::mBeamEnergy[energy].c_str(),vmsa::mPID[pid].c_str(),etamode.c_str());
	    ...
	    for(int i_method = vmsa::Method_start; i_method < vmsa::Method_stop; ++i_method)
	    {
	        ...
	        for(int i_pt = 2/*vmsa::pt_rebin_first[energy]*/; i_pt < 6/*vmsa::pt_rebin_last[energy]*/; ++i_pt) // pt loop
	        {
	          ...
	          h_mCounts[KEY_counts]->Write();
                  cout << "Integral of ipt " << i_pt << " = " << h_mCounts[KEY_counts]->Integral(0,-1) << endl;; 
	        }
              }
	    }
	  }
