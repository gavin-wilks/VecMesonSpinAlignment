...
void plotHelicity2DOffDiag(int energy = 4, int pid = 0, int year = 0, bool random3D = false, int order = 2, string etamode = "eta1_eta1", string option = "")
{
  ...
  string inputfile = Form("../output/AuAu%s/%s/RawPhiPtSys_%s_PolySys_Helicity_2D_OffDiag%s.root",vmsa::mBeamEnergy[energy].c_str(),vmsa::mPID[pid].c_str(),etamode.c_str(),option.c_str());
  ...
  g_mReal[KEY_real]->GetYaxis()->SetTitle("Raw Re(#rho_{10})-Re(#rho_{0-1})");
  ...
  g_mImag[KEY_imag]->GetYaxis()->SetTitle("Raw Im(#rho_{10})-Im(#rho_{0-1})");
  ...
  g_mReRho1n1[KEY_rerho1n1]->GetYaxis()->SetTitle("Raw Re(#rho_{1-1})");
  ...
  g_mImRho1n1[KEY_imrho1n1]->GetYaxis()->SetTitle("Raw Im(#rho_{1-1})");
  ...
  string outputname = Form("./figures/%s/%s/pTstudy/Helicity2DCorrected_OffDiag_ExtractedSpinDensityElements_%s_Order%d%s.pdf",vmsa::mPID[pid].c_str(),vmsa::mBeamEnergy[energy].c_str(),vmsa::mBeamEnergy[energy].c_str(),order,option.c_str());
