...
void calSpinAlignmentSysPhiCentPoly3Integration(int energy = 4, int pid = 0, int year = 0, bool random3D = false, int centQA = 4, int ptQA = 0, int dcaQA = 0, int nsigQA = 0, int order = 2, std::string etamode = "eta1p5_eta1p5")
{
  std::string EP[2] = {"","2nd"};
  ...
  if(order == 1) inputfile = Form("../output/AuAu%s/%s/InvMassSubBgCent_%s_FirstOrder.root",vmsa::mBeamEnergy[energy].c_str(),vmsa::mPID[pid].c_str(),etamode.c_str());
...
#if _PlotQA_
  string outputname = Form("./figures/%s/%s/centralitystudy/allThetaYields%s_%s_Order%d_%s_CentDependence.pdf",vmsa::mPID[pid].c_str(),vmsa::mBeamEnergy[energy].c_str(),vmsa::mPID[pid].c_str(),vmsa::mBeamEnergy[energy].c_str(),order,etamode.c_str());
  ...
  {  
    ...
    for(int i_pt = vmsa::pt_rebin_first_cent[energy]; i_pt <= vmsa::pt_rebin_last_cent[energy]; ++i_pt) // pt loop
    {
      ...
      for(int i_sigma = vmsa::Sig_start; i_sigma < /*vmsa::Sig_stop*/1; ++i_sigma)
      {
        ...
        {
          string KEY_counts_QA = Form("pt_%d_Centrality_%d_%s_Dca_%d_Sig_%d_%s_Norm_%d_Sigma_%d_%s",i_pt,i_cent,EP[order-1].c_str(),vmsa::Dca_start,vmsa::nSigKaon_start,vmsa::mPID[pid].c_str(),vmsa::Norm_QA,i_sigma,vmsa::mInteMethod[i_method].c_str());
...
//#endif
    ...
    for(Int_t i_sig = vmsa::nSigKaon_start; i_sig < vmsa::nSigKaon_stop; i_sig++)
    {
      ...
      for(int i_norm = vmsa::Norm_start; i_norm < vmsa::Norm_stop; ++i_norm)
      {
        for(int i_sigma = vmsa::Sig_start; i_sigma < vmsa::Sig_stop; ++i_sigma)
        {
          for(int i_method = vmsa::Method_start; i_method < vmsa::Method_stop; ++i_method)
          {
            string KEY_rho = Form("rhoRaw_pt_%d_%s_Dca_%d_Sig_%d_%s_Norm_%d_Sigma_%d_%s",ptQA,EP[order-1].c_str(),i_dca,i_sig,vmsa::mPID[pid].c_str(),i_norm,i_sigma,vmsa::mInteMethod[i_method].c_str());
  //          cout << KEY_rho << endl;
  ...
  if(order == 1) outputfile = Form("../output/AuAu%s/%s/RawRhoCentSys_%s_FirstOrder.root",vmsa::mBeamEnergy[energy].c_str(),vmsa::mPID[pid].c_str(),etamode.c_str());
