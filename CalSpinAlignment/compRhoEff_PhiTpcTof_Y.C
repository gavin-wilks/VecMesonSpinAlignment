...
void compRhoEff_PhiTpcTof_Y(Int_t energy = 4, Int_t pid = 0, Int_t i_cent = 9, string correction = "Raw", bool folded = false)
{

  string foldedstring = "unfolded";
  if(folded) foldedstring = "folded";
 ...
 // string OutPutFile = Form("../output/AuAu%s/%s/TPCOnly_RhoEta_%sSysErrors_%s_PolySys.root",vmsa::mBeamEnergy[energy].c_str(),vmsa::mPID[pid].c_str(),correction.c_str(),"eta1_eta1");

  string inputfile1 = Form("../output/AuAu%s/%s/TPCTOF_RhoEta_%sSysErrors_%s_PolySys.root",vmsa::mBeamEnergy[energy].c_str(),vmsa::mPID[pid].c_str(),correction.c_str(),"eta1_eta1");
 ...
 // string inputfile2 = Form("../output/AuAu%s/%s/TPCOnly_RawPhiPtSys_%s_Poly_TPCOnly.root",vmsa::mBeamEnergy[energy].c_str(),vmsa::mPID[pid].c_str(),"eta1_eta1");
  string inputfile2 = Form("../output/AuAu%s/%s/TPCOnly_RhoEta_%sSysErrors_%s_PolySys.root",vmsa::mBeamEnergy[energy].c_str(),vmsa::mPID[pid].c_str(),correction.c_str(),"eta1_eta1");

  TFile *File_InPut1 = TFile::Open(inputfile1.c_str());
  TFile *File_InPut2 = TFile::Open(inputfile2.c_str());
  
  TString StatErrorRho;
  if(folded)StatErrorRho = Form("rhoRaw_2nd_Dca_%d_Sig_%d_%s_Norm_%d_Sigma_%d_%s_Poly%d",0,0,vmsa::mPID[pid].c_str(),0,1,vmsa::mInteMethod[1].c_str(),1);
  if(!folded)StatErrorRho = Form("rhoRaw_2nd_Dca_%d_Sig_%d_%s_Norm_%d_Sigma_%d_%s_Poly%d_Unfolded",0,0,vmsa::mPID[pid].c_str(),0,1,vmsa::mInteMethod[1].c_str(),1);
  //TString SysErrorRho = Form("g_rho00_%s_%s_SysError",vmsa::mBeamEnergy[energy].c_str(),vmsa::mPID[pid].c_str());

  TGraphAsymmErrors *g_Stat1   = (TGraphAsymmErrors*)((TGraphAsymmErrors*)   File_InPut1->Get(StatErrorRho.Data()))->Clone();
  //TGraphAsymmErrors *g_Sys1    = (TGraphAsymmErrors*)((TGraphAsymmErrors*)   File_InPut1->Get( SysErrorRho.Data()))->Clone();
  TGraphAsymmErrors *g_Stat2   = (TGraphAsymmErrors*)((TGraphAsymmErrors*)   File_InPut2->Get(StatErrorRho.Data()))->Clone();
  //TGraphAsymmErrors *g_Sys2    = (TGraphAsymmErrors*)((TGraphAsymmErrors*)   File_InPut2->Get( SysErrorRho.Data()))->Clone();
  ...
  h_frame->SetTitle("20-60% Centrality, 1.0 < p_{T} < 5.0 GeV/c");
  h_frame->GetYaxis()->SetRangeUser(0.27,0.5);
  if(folded)h_frame->GetXaxis()->SetRangeUser(0.0,1.0);
  if(!folded)h_frame->GetXaxis()->SetTitle("y");
  if(!folded)h_frame->GetXaxis()->SetRangeUser(-1.0,1.0);
...
//  PlotLine(0.0,5.0,1.0/3.0,1.0/3.0,1,2,2);
  ...
  if(folded) PlotLine(0.0,1.0,1.0/3.0,1.0/3.0,1,2,2);
  if(!folded)PlotLine(-1.0,1.0,1.0/3.0,1.0/3.0,1,2,2);
  ...
  //}
    Draw_TGAE_Point_new_Symbol(0.2,0.31,0.0,0.0,0.0,0.0,style_phi_2nd,color_phi_2nd,size_marker+0.2); 
    string leg_count = "TPC Only";
    plotTopLegend((char*)leg_count.c_str(),0.25,0.308,0.03,1,0.0,42,0);

    string leg_besi = "TOF+TPC";
    Draw_TGAE_Point_new_Symbol(0.2,0.295,0.0,0.0,0.0,0.0,style_phi_1st,color_phi_1st,size_marker-0.2);
    plotTopLegend((char*)leg_besi.c_str(),0.25,0.293,0.03,1,0.0,42,0);
 
    //string leg_sp = "STAR Preliminary";
    //plotTopLegend((char*)leg_sp.c_str(),0.3,0.42,0.03,2,0.0,42,0);

    PlotLine(0.2,0.23,0.28,0.28,1,2,2);
    string leg_line = "#rho_{00} = 1/3";
    plotTopLegend((char*)leg_line.c_str(),0.25,0.28,0.03,1,0.0,42,0);

    string leg_energy = Form("Au+Au %s", vmsa::mBeamEnergyText[energy].c_str());
    plotTopLegend((char*)leg_energy.c_str(),0.3,0.405,0.04,1,0.0,42,0);
    plotTopLegend((char*)"20%-60%",0.35,0.39,0.04,1,0.0,42,0);
  ...
  //g_Sys2->GetPoint(i,pt2sy,rho2sy);
    g_Stat2->SetPoint(i,pt2st+0.025,rho2st); 
  ...
  c_rho_SysError->SaveAs(Form("figures/tpctofcomp_phi_y_cent2060_%s_%s.pdf",vmsa::mBeamEnergy[energy].c_str(),foldedstring.c_str()));
