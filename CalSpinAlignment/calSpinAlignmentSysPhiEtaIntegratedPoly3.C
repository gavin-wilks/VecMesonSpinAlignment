...
void calSpinAlignmentSysPhiEtaIntegratedPoly3(int energy = 3, int pid = 0, int year = 0, bool random3D = false, int etaQA = 9, int ptQA = 1, int centQA = 2, int dcaQA = 0, int nsigQA = 0, int normQA = 0, int sigQA = 0, int order = 2, std::string etamode = "eta1_eta1")
{
  std::string EP[2] = {"","2nd"};
        ...
        for(Int_t i_sig = vmsa::nSigKaon_start; i_sig < vmsa::nSigKaon_stop; i_sig++)
        {
          ...
          for(int i_norm = vmsa::Norm_start; i_norm < vmsa::Norm_stop; ++i_norm)
          {
		    ...
		    TF1 *f_bg = new TF1("f_bg",Poly3,vmsa::BW_Start[pid],vmsa::BW_Stop[pid],4);
		      ...
		      f_bg->SetParameter(2,0.0);
		      f_bg->SetParameter(3,0.0);
		      f_bg->SetParError(0,1.0);
		      f_bg->SetParError(1,1.0);
		      f_bg->SetParError(2,1.0);
		      f_bg->SetParError(3,1.0);
		 

 
                      params[0] = 0.0;//result->GetParams()[3];
                      params[1] = 0.0;//result->GetParams()[4];
                      params[2] = 0.0;//result->GetParams()[5];
                      params[3] = 0.0;//result->GetParams()[5];
                    ...
                    float errors_bg;
                    if(result.Get() != nullptr) errors_bg = f_bg->IntegralError(Inte_start,Inte_stop,params,covArr.GetMatrixArray())/bin_width;
...
#if _PlotQA_
  string outputname = Form("./figures/%s/%s/rapiditystudy/allThetaYields%s_%s_Order%d_%s_RapidityDependence.pdf",vmsa::mPID[pid].c_str(),vmsa::mBeamEnergy[energy].c_str(),vmsa::mPID[pid].c_str(),vmsa::mBeamEnergy[energy].c_str(),order,etamode.c_str());
      ...
      for(int i_eta = vmsa::eta_start; i_eta < vmsa::eta_stop; i_eta++) // Rapidity bin loop
      {
        ...
        for(int i_theta = 0; i_theta < 9; ++i_theta)
        {
          ...
          if(i_theta < vmsa::CTS_stop)
          {
            ...
            string KEY_InteTheta = Form("eta_%d_pt_%d_Centrality_%d_%s_Dca_%d_Sig_%d_%s_Norm_%d",i_eta,i_pt,i_cent,EP[order-1].c_str(),vmsa::Dca_start,vmsa::nSigKaon_start,vmsa::mPID[pid].c_str(),vmsa::Norm_QA);
        ...
        for(int i_sigma = vmsa::Sig_start; i_sigma < /*vmsa::Sig_stop*/1; ++i_sigma)
        {
          ...
          {
            string KEY_counts_QA = Form("eta_%d_pt_%d_Centrality_%d_%s_Dca_%d_Sig_%d_%s_Norm_%d_Sigma_%d_%s",i_eta,i_pt,i_cent,EP[order-1].c_str(),vmsa::Dca_start,vmsa::nSigKaon_start,vmsa::mPID[pid].c_str(),vmsa::Norm_QA,i_sigma,vmsa::mInteMethod[i_method].c_str());
...
//#endif
  ...
  string outputfile = Form("../output/AuAu%s/%s/RawRhoEtaSys_%s.root",vmsa::mBeamEnergy[energy].c_str(),vmsa::mPID[pid].c_str(),etamode.c_str());
  if(order == 1) outputfile = Form("../output/AuAu%s/%s/RawRhoEtaSys_%s_FirstOrder.root",vmsa::mBeamEnergy[energy].c_str(),vmsa::mPID[pid].c_str(),etamode.c_str());
