...
void subBackGroundRapidity_Global_2D_OffDiag(int energy = 4, int pid = 0, int year = 0, string date = "20241010", bool random3D = false, int order = 2, string etamode = "eta1_eta1", int i_eta = 0, int folderopt = 0)
{
  ...
  string InPutFile_SE = Form("../data/Yields_Phi_SE_%s_%s_%s_%s_2D_Study1.root",vmsa::mBeamEnergy[energy].c_str(),date.c_str(),etamode.c_str(),frame.c_str());
  //if(energy == 3) InPutFile_SE = Form("../data/Yields_Phi_SE_14GeV_%s.root",etamode.c_str());
  if(order == 1) InPutFile_SE = Form("../data/Yields_Phi_SE_%s_%s_%s_%s_2D_Study1_FirstOrder.root",vmsa::mBeamEnergy[energy].c_str(),date.c_str(),etamode.c_str(),frame.c_str());
  ...
  string InPutFile_ME = Form("../data/Yields_Phi_ME_%s_%s_%s_%s_2D_Study1.root",vmsa::mBeamEnergy[energy].c_str(),date.c_str(),etamode.c_str(),frame.c_str());
  //if(energy == 3) InPutFile_ME = Form("../data/Yields_Phi_ME_14GeV_%s.root",etamode.c_str());
  if(order == 1) InPutFile_ME = Form("../data/Yields_Phi_ME_%s_%s_%s_%s_2D_Study1_FirstOrder.root",vmsa::mBeamEnergy[energy].c_str(),date.c_str(),etamode.c_str(),frame.c_str());
  ...
  for(Int_t i_phi = 0; i_phi < 9; i_phi++) // phi' bin
  {
    for(Int_t i_thetah = 0; i_thetah < 12; i_thetah++) // cos(theta*) bin
    {
      for(Int_t i_dca = vmsa::Dca_start; i_dca < vmsa::Dca_stop; i_dca++)
      {
        for(Int_t i_sig = vmsa::nSigKaon_start; i_sig < vmsa::nSigKaon_stop; i_sig++)
        {
          ...
          for(Int_t ptbin = 0; ptbin < vmsa::pt_rebin_y_2D; ptbin++)
          {
            ...
            {
              for(Int centbin = 0; centbin < vmsa::cent_rebin_total_2D; centbin++)
              {
                for(Int_t i_cent = vmsa::cent_rebin_2D[centbin]; i_cent < vmsa::cent_rebin_2D[centbin+1]; i_cent++)
                {
                  string KEY_InPutSE = Form("pt_%d_eta_%d_Centrality_%d_CosThetaStar_%d_PhiPrime_%d_%s_Dca_%d_Sig_%d_%s_SE",i_pt,i_etaraw,i_cent,i_thetah,i_phi,EP[order-1].c_str(),i_dca,i_sig,vmsa::mPID[pid].c_str());
                  string KEY_InPutSE_Rebin = Form("pt_%d_eta_%d_Centrality_%d_CosThetaStar_%d_PhiPrime_%d_%s_Dca_%d_Sig_%d_%s_SE",ptbin,i_eta,centbin,i_thetah,i_phi,EP[order-1].c_str(),i_dca,i_sig,vmsa::mPID[pid].c_str());
                  cout << KEY_InPutSE << endl;
                  cout << KEY_InPutSE_Rebin << endl;
                  if(i_pt == vmsa::pt_rebin_start_y_2D[energy][ptbin] && i_cent == vmsa::cent_rebin_2D[centbin]) h_mInPut_SE[KEY_InPutSE_Rebin] = (TH1F*)File_SE->Get(KEY_InPutSE.c_str())->Clone(); 
                  else                                                                                           h_mInPut_SE[KEY_InPutSE_Rebin]->Add((TH1F*)File_SE->Get(KEY_InPutSE.c_str()),1.0); 


                  string KEY_InPutME = Form("pt_%d_eta_%d_Centrality_%d_CosThetaStar_%d_PhiPrime_%d_%s_Dca_%d_Sig_%d_%s_ME",i_pt,i_etaraw,i_cent,i_thetah,i_phi,EP[order-1].c_str(),i_dca,i_sig,vmsa::mPID[pid].c_str());
                  string KEY_InPutME_Rebin = Form("pt_%d_eta_%d_Centrality_%d_CosThetaStar_%d_PhiPrime_%d_%s_Dca_%d_Sig_%d_%s_ME",ptbin,i_eta,centbin,i_thetah,i_phi,EP[order-1].c_str(),i_dca,i_sig,vmsa::mPID[pid].c_str());
                  cout << KEY_InPutME << endl;
                  cout << KEY_InPutME_Rebin << endl;
                  if(i_pt == vmsa::pt_rebin_start_y_2D[energy][ptbin] && i_cent == vmsa::cent_rebin_2D[centbin]) h_mInPut_ME[KEY_InPutME_Rebin] = (TH1F*)File_ME->Get(KEY_InPutME.c_str())->Clone(); 
          ...
          for(Int_t ptbin = 0; ptbin < vmsa::pt_rebin_y_2D; ptbin++)
          {
            for(Int centbin = 0; centbin < vmsa::cent_rebin_total_2D; centbin++)
            {
              ...
              {
                string KEY_InPutSE = Form("pt_%d_eta_%d_Centrality_%d_CosThetaStar_%d_PhiPrime_%d_%s_Dca_%d_Sig_%d_%s_SE",ptbin,i_eta,centbin,i_thetah,i_phi,EP[order-1].c_str(),i_dca,i_sig,vmsa::mPID[pid].c_str());
                string KEY_SE = Form("pt_%d_eta_%d_Centrality_%d_CosThetaStar_%d_Phi_%d_%s_Dca_%d_Sig_%d_%s_Norm_%d_SE",ptbin,i_eta,centbin,i_thetah,i_phi,EP[order-1].c_str(),i_dca,i_sig,vmsa::mPID[pid].c_str(),i_norm);
                h_mMass_SE[KEY_SE] = (TH1F*)h_mInPut_SE[KEY_InPutSE]->Clone(KEY_SE.c_str());
                string KEY_InPutME = Form("pt_%d_eta_%d_Centrality_%d_CosThetaStar_%d_PhiPrime_%d_%s_Dca_%d_Sig_%d_%s_ME",ptbin,i_eta,centbin,i_thetah,i_phi,EP[order-1].c_str(),i_dca,i_sig,vmsa::mPID[pid].c_str());
                string KEY_ME = Form("pt_%d_eta_%d_Centrality_%d_CosThetaStar_%d_Phi_%d_%s_Dca_%d_Sig_%d_%s_Norm_%d_ME",ptbin,i_eta,centbin,i_thetah,i_phi,EP[order-1].c_str(),i_dca,i_sig,vmsa::mPID[pid].c_str(),i_norm);
                h_mMass_ME[KEY_ME] = (TH1F*)h_mInPut_ME[KEY_InPutME]->Clone(KEY_ME.c_str());
                string KEY_SM = Form("pt_%d_eta_%d_Centrality_%d_CosThetaStar_%d_Phi_%d_%s_Dca_%d_Sig_%d_%s_Norm_%d_SM",ptbin,i_eta,centbin,i_thetah,i_phi,EP[order-1].c_str(),i_dca,i_sig,vmsa::mPID[pid].c_str(),i_norm);
  ...
  string KEY_SE_QA = Form("pt_%d_eta_%d_Centrality_%d_CosThetaStar_%d_Phi_%d_%s_Dca_%d_Sig_%d_%s_Norm_%d_SE",0,i_eta,0,vmsa::CTS_start,0,EP[order-1].c_str(),vmsa::Dca_start,vmsa::nSigKaon_start,vmsa::mPID[pid].c_str(),vmsa::Norm_QA);
  ...
  string KEY_ME_QA = Form("pt_%d_eta_%d_Centrality_%d_CosThetaStar_%d_Phi_%d_%s_Dca_%d_Sig_%d_%s_Norm_%d_ME",0,i_eta,0,vmsa::CTS_start,0,EP[order-1].c_str(),vmsa::Dca_start,vmsa::nSigKaon_start,vmsa::mPID[pid].c_str(),vmsa::Norm_QA);
  ...
  string KEY_SM_QA = Form("pt_%d_eta_%d_Centrality_%d_CosThetaStar_%d_Phi_%d_%s_Dca_%d_Sig_%d_%s_Norm_%d_SM",0,i_eta,0,vmsa::CTS_start,0,EP[order-1].c_str(),vmsa::Dca_start,vmsa::nSigKaon_start,vmsa::mPID[pid].c_str(),vmsa::Norm_QA);
  ...
  c_peak->SaveAs(Form("figures/%s/%s/rapiditystudy/%s2D_CloseUpMixedEvent_%s_Order%d_Cent0_y%d.pdf",vmsa::mPID[pid].c_str(),vmsa::mBeamEnergy[energy].c_str(),frame.c_str(),vmsa::mBeamEnergy[energy].c_str(),order,i_eta));
  ...
  string outputname = Form("figures/%s/%s/rapiditystudy/%s2D_InvMassDistributions_%s_Order%d_Cent0_y%d.pdf",vmsa::mPID[pid].c_str(),vmsa::mBeamEnergy[energy].c_str(),frame.c_str(),vmsa::mBeamEnergy[energy].c_str(),order,i_eta);
  ...
  for(int iphi = 0; iphi < 12; iphi++)
  {
    for(int ithetah = 0; ithetah < 9; ithetah++)
    {
      ...
      string KEY_SE_QA = Form("pt_%d_eta_%d_Centrality_%d_CosThetaStar_%d_Phi_%d_%s_Dca_%d_Sig_%d_%s_Norm_%d_SE",0,i_eta,0,ithetah,iphi,EP[order-1].c_str(),vmsa::Dca_start,vmsa::nSigKaon_start,vmsa::mPID[pid].c_str(),vmsa::Norm_QA);
      h_mMass_SE[KEY_SE_QA]->GetYaxis()->SetRangeUser(-0.1*h_mMass_SE[KEY_SE_QA]->GetMaximum(),1.1*h_mMass_SE[KEY_SE_QA]->GetMaximum());
      ...
      string KEY_ME_QA = Form("pt_%d_eta_%d_Centrality_%d_CosThetaStar_%d_Phi_%d_%s_Dca_%d_Sig_%d_%s_Norm_%d_ME",0,i_eta,0,ithetah,iphi,EP[order-1].c_str(),vmsa::Dca_start,vmsa::nSigKaon_start,vmsa::mPID[pid].c_str(),vmsa::Norm_QA);
      h_mMass_ME[KEY_ME_QA]->SetLineColor(2);
      h_mMass_ME[KEY_ME_QA]->SetFillColor(2);
      h_mMass_ME[KEY_ME_QA]->SetFillStyle(3002);
      h_mMass_ME[KEY_ME_QA]->DrawCopy("h same");

      string KEY_SM_QA = Form("pt_%d_eta_%d_Centrality_%d_CosThetaStar_%d_Phi_%d_%s_Dca_%d_Sig_%d_%s_Norm_%d_SM",0,i_eta,0,ithetah,iphi,EP[order-1].c_str(),vmsa::Dca_start,vmsa::nSigKaon_start,vmsa::mPID[pid].c_str(),vmsa::Norm_QA);
      h_mMass_SM[KEY_SM_QA]->SetLineColor(4);
      h_mMass_SM[KEY_SM_QA]->SetFillColor(4);
      h_mMass_SM[KEY_SM_QA]->SetFillStyle(3004);
      h_mMass_SM[KEY_SM_QA]->DrawCopy("h same");

      string pT_range = Form("[%.2f,%.2f)",vmsa::eta_bin[i_eta],vmsa::eta_bin[i_eta+1]);
          ...
          for(Int_t i_dca = vmsa::Dca_start; i_dca < vmsa::Dca_stop; i_dca++)
          {
            for(Int_t i_sig = vmsa::nSigKaon_start; i_sig < vmsa::nSigKaon_stop; i_sig++)
            {
              ...
              for(int i_norm = vmsa::Norm_start; i_norm < vmsa::Norm_stop; ++i_norm)
              {
                ...
                string KEY_SM = Form("pt_%d_eta_%d_Centrality_%d_CosThetaStar_%d_Phi_%d_%s_Dca_%d_Sig_%d_%s_Norm_%d_SM",i_pt,i_eta,i_cent,i_thetah,i_phi,EP[order-1].c_str(),i_dca,i_sig,vmsa::mPID[pid].c_str(),i_norm);
  ...
  outputname = Form("figures/%s/%s/rapiditystudy/%s2D_InvMassBackgroundSubtracted_Rebinned_%s_Order%d_Cent0_y%d.pdf",vmsa::mPID[pid].c_str(),vmsa::mBeamEnergy[energy].c_str(),frame.c_str(),vmsa::mBeamEnergy[energy].c_str(),order,i_eta);
  ...
  for(int iphi = 0; iphi < 12/*2*vmsa::CTS_total*/; iphi++)
  {
    for(int ithetah = 0; ithetah < 9/*2*vmsa::CTS_total*/; ithetah++)
    {
      ...
      string KEY_QA = Form("pt_%d_eta_%d_Centrality_%d_CosThetaStar_%d_Phi_%d_%s_Dca_%d_Sig_%d_%s_Norm_%d",0,i_eta,0,ithetah,iphi,EP[order-1].c_str(),vmsa::Dca_start,vmsa::nSigKaon_start,vmsa::mPID[pid].c_str(),vmsa::Norm_QA);
  ...
  string outputfile = Form("../output/AuAu%s/%s/%s2DRapidity_InvMassSubBg_%s.root",vmsa::mBeamEnergy[energy].c_str(),vmsa::mPID[pid].c_str(),frame.c_str(),etamode.c_str());
  //string outputfile = Form("../output/AuAu%s/%s/InvMassSubBg.root",vmsa::mBeamEnergy[energy].c_str(),vmsa::mPID[pid].c_str());
  if(order == 1) outputfile = Form("../output/AuAu%s/%s/%s2DRapidity_InvMassSubBg_%s_FirstOrder.root",vmsa::mBeamEnergy[energy].c_str(),vmsa::mPID[pid].c_str(),frame.c_str(),etamode.c_str());
