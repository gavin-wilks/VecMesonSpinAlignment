...
void calSysErrorPhiCent_FixedSys_PolySys(Int_t energy = 4, Int_t pid = 0, string correction = "Raw", bool random3D = false, int centQA = 0, int order = 2, std::string etamode = "eta1_eta1", std::string fileoption = "", int yspectra = 0)//defaultF = 0 is BESII, defaultF = 1 is BESI
{
  ...
  string inputfileHframe = Form("../output/AuAu%s/%s/RawRhoCentSys_%s_Poly.root",vmsa::mBeamEnergy[energy].c_str(),vmsa::mPID[pid].c_str(),etamode.c_str());
  ...
  string inputfile = Form("../output/AuAu%s/%s/%s/%sRhoCentSys_%s_Poly%s.root",vmsa::mBeamEnergy[energy].c_str(),vmsa::mPID[pid].c_str(),fileoption.c_str(),correction.c_str(),etamode.c_str(),spectra.c_str());
  if(order == 1) inputfile = Form("../output/AuAu%s/%s/%s/%sRhoCentSys_%s_Poly_FirstOrder%s.root",vmsa::mBeamEnergy[energy].c_str(),vmsa::mPID[pid].c_str(),fileoption.c_str(),correction.c_str(),etamode.c_str(),spectra.c_str());
  if(correction == "RawRes") inputfile = Form("../output/AuAu%s/%s/%s/RawRhoCentSys_%s_Poly%s.root",vmsa::mBeamEnergy[energy].c_str(),vmsa::mPID[pid].c_str(),fileoption.c_str(),etamode.c_str(),spectra.c_str());
  if(correction == "RawRes" && order == 1) inputfile = Form("../output/AuAu%s/%s/%s/RawRhoCentSys_%s_Poly_FirstOrder%s.root",vmsa::mBeamEnergy[energy].c_str(),vmsa::mPID[pid].c_str(),fileoption.c_str(),etamode.c_str(),spectra.c_str());
              ...
              for(int i_poly = 0; i_poly < 3; i_poly++)
              {
                ...
                if(correction == "RawRes")
                {
                  ...
                  {      
                    ...
                    double drhodobs = 4./(1.+3.*Res_12[i]);  // calculate d(rho)/d(rho_obs)
                    double drhodR = -12.*(rho_obs - 1./3.)/(1.+3.*Res_12[i])/(1.+3.*Res_12[i]); // calculate d(rho)/d(R)

                    double real_rho = 1./3. + 4./(1.+3.*Res_12[i])*(rho_obs - 1./3.);
                    double real_rho_error = TMath::Sqrt((rho_obs_err*rho_obs_err)*(drhodobs*drhodobs) + (Res_12_err[i]*Res_12_err[i])*(drhodR*drhodR));                 
                ...
                for(int i_cent = vmsa::centStart; i_cent < vmsa::centStop; i_cent++) // Centrality loop
                {
                  ...
                  if(yspectra == 7) KEY_counts = Form("pt_%d_Centrality_%d_%s_Dca_%d_Sig_%d_%s_Norm_%d_Sigma_%d_%s_Poly%d_px",i_pt,i_cent,EP[order-1].c_str(),i_dca,i_sig,vmsa::mPID[pid].c_str(),i_norm,i_sigma,vmsa::mInteMethod[i_method].c_str(),i_poly+1);
  ...
  h_frame->GetYaxis()->SetRangeUser(-.1,0.7);
  ...
  for(int i_pt = vmsa::pt_rebin_first_cent[energy]; i_pt <= vmsa::pt_rebin_last_cent[energy]; ++i_pt) // pt loop
  {           
    string KEY_Default = Form("rhoRaw_pt_%d_%s_Dca_%d_Sig_%d_%s_Norm_%d_Sigma_%d_%s_Poly1",i_pt,EP[order-1].c_str(),0,0,vmsa::mPID[pid].c_str(),0,0,vmsa::mInteMethod[1].c_str());
      ...
      { 
        for(int i_sigma = vmsa::Sig_start; i_sigma < vmsa::Sig_stop; ++i_sigma)
        {
          for(int i_method = vmsa::Method_start; i_method < vmsa::Method_stop; ++i_method)
          {
            ...
            string KEY = Form("rhoRaw_pt_%d_%s_Dca_%d_Sig_%d_%s_Norm_%d_Sigma_%d_%s_Poly1",i_pt,EP[order-1].c_str(),i_dca,0,vmsa::mPID[pid].c_str(),0,i_sigma,vmsa::mInteMethod[i_method].c_str());
      ...
      for(Int_t i_sig = vmsa::nSigKaon_start; i_sig < vmsa::nSigKaon_stop; i_sig++)
      {
        for(int i_sigma = vmsa::Sig_start; i_sigma < vmsa::Sig_stop; ++i_sigma)
        {
          for(int i_method = vmsa::Method_start; i_method < vmsa::Method_stop; ++i_method)
          {
            ...
            string KEY = Form("rhoRaw_pt_%d_%s_Dca_%d_Sig_%d_%s_Norm_%d_Sigma_%d_%s_Poly1",i_pt,EP[order-1].c_str(),0,i_sig,vmsa::mPID[pid].c_str(),0,i_sigma,vmsa::mInteMethod[i_method].c_str());
      ...
      for(int i_norm = vmsa::Norm_start; i_norm < vmsa::Norm_stop; ++i_norm)
      {
        for(int i_sigma = vmsa::Sig_start; i_sigma < vmsa::Sig_stop; ++i_sigma)
        {
          for(int i_method = vmsa::Method_start; i_method < vmsa::Method_stop; ++i_method)
          {
            ...
            string KEY = Form("rhoRaw_pt_%d_%s_Dca_%d_Sig_%d_%s_Norm_%d_Sigma_%d_%s_Poly1",i_pt,EP[order-1].c_str(),0,0,vmsa::mPID[pid].c_str(),i_norm,i_sigma,vmsa::mInteMethod[i_method].c_str());
      ...
      for(int i_poly = 0; i_poly < 2; ++i_poly)
      {
        for(int i_sigma = vmsa::Sig_start; i_sigma < vmsa::Sig_stop; ++i_sigma)
        {
          for(int i_method = vmsa::Method_start; i_method < vmsa::Method_stop; ++i_method)
          {
            ...
            string KEY = Form("rhoRaw_pt_%d_%s_Dca_%d_Sig_%d_%s_Norm_%d_Sigma_%d_%s_Poly%d",i_pt,EP[order-1].c_str(),0,0,vmsa::mPID[pid].c_str(),0,i_sigma,vmsa::mInteMethod[i_method].c_str(),i_poly+1);
            //if(correction == "AccRes") KEY = Form("rhoRaw_2nd_Dca_%d_Sig_%d_%s_Norm_%d_Sigma_%d_%s_F_%d_Eff_0",0,0,vmsa::mPID[pid].c_str(),i_norm,i_sigma,vmsa::mInteMethod[i_method].c_str(),defaultF);
            double pt_sys, rho_sys;
            g_mRho[KEY]->GetPoint(i_point,pt_sys,rho_sys);
            sysPoly[idx] = rho_sys;
            if(random3D) sysPoly[idx] = 4.*(sysNorm[idx]-1./3.)+1./3.;
            idx++;
          }
        }
      }	 
          ...
          for(int i_method = vmsa::Method_start; i_method < vmsa::Method_stop; ++i_method)
          {
            for(int i_poly = 0; i_poly < 3; i_poly++)
            {
              ...
              for(int i_cent = vmsa::centStart; i_cent < vmsa::centStop; i_cent++) // Centrality loop
              {
                ...
                for(int i_pt = vmsa::pt_rebin_first_cent[energy]; i_pt <= vmsa::pt_rebin_last_cent[energy]; ++i_pt) // pt loop
                {          
                  string KEY_Default = Form("rhoRaw_pt_%d_%s_Dca_%d_Sig_%d_%s_Norm_%d_Sigma_%d_%s_Poly%d",i_pt,EP[order-1].c_str(),i_dca,i_sig,vmsa::mPID[pid].c_str(),i_norm,i_sigma,vmsa::mInteMethod[i_method].c_str(),i_poly+1);
                  string KEY_counts = Form("pt_%d_Centrality_%d_%s_Dca_%d_Sig_%d_%s_Norm_%d_Sigma_%d_%s_Poly%d",i_pt,i_cent,EP[order-1].c_str(),i_dca,i_sig,vmsa::mPID[pid].c_str(),i_norm,i_sigma,vmsa::mInteMethod[i_method].c_str(),i_poly+1); 
                  if(yspectra == 7) KEY_counts = Form("pt_%d_Centrality_%d_%s_Dca_%d_Sig_%d_%s_Norm_%d_Sigma_%d_%s_Poly%d_px",i_pt,i_cent,EP[order-1].c_str(),i_dca,i_sig,vmsa::mPID[pid].c_str(),i_norm,i_sigma,vmsa::mInteMethod[i_method].c_str(),i_poly+1); 
                  ...
                  g_mRho[KEY_Default]->GetPoint(i_cent,ptSys,rhoSys);   
                  rhoErrSys = g_mRho[KEY_Default]->GetErrorYhigh(i_cent);  
                ...
                g_mStatErrorsInt[KEY_rho]->SetPoint(i_cent, cent_mean, weight_rho00_statweight);
                g_mStatErrorsInt[KEY_rho]->SetPointError(i_cent, 0.0, 0.0, weight_error_stat_rho00_statweight, weight_error_stat_rho00_statweight);
              }
              ...
              cout << "Integrated min-bias rho00 yield weight = " << weight_cent << " +/- " << weight_error_stat_cent << "stat. +/- " << endl;//weight_error_sys_cent << " sys." << endl;
              cout << "Integrated min-bias rho00 stat  weight = " << weight_cent_statweight << " +/- " << weight_error_stat_cent_statweight  << "stat. +/- " << endl;//weight_error_sys_cent_statweight  << " sys." << endl;
              cout << "Integrated 20-60% rho00 yield weight  = " << weight_cent9 << " +/- " << weight_error_stat_cent9 << "stat. +/- " << endl;//weight_error_sys_cent9 << " sys." << endl;
              cout << "Integrated 20-60% rho00 stat  weight  = " << weight_cent9_statweight << " +/- " << weight_error_stat_cent9_statweight << "stat. +/- " << endl;//weight_error_sys_cent9_statweight << " sys." << endl;
              vRho[KEY_rho].push_back(weight_cent_statweight);
              vRhoErr[KEY_rho].push_back(weight_error_stat_cent_statweight);
            }
          }
        }
      }
  ...
  cout << "Total Integrated Value" << endl;
  {
    ...
    cout << "FINAL ACTUAL RHO = " << vRho[KEY_Default][0] << " +/- " << vRhoErr[KEY_Default][0] << " stat. +/- " << SysError_rho << endl;
  ...
  for(Int_t i_point = 0; i_point < 9; ++i_point)
  {
    ...
    double pt_def, rho_def;
    g_mStatErrorsInt[KEY_Default]->GetPoint(i_point,pt_def,rho_def); 
  ...
  //        if(correction == "AccRes") KEY = Form("rhoRaw_2nd_Dca_%d_Sig_%d_%s_Norm_%d_Sigma_%d_%s_F_%d_Eff_0",i_dca,0,vmsa::mPID[pid].c_str(),0,0,vmsa::mInteMethod[i_method].c_str(),defaultF);
          double pt_sys, rho_sys;
          g_mStatErrorsInt[KEY]->GetPoint(i_point,pt_sys,rho_sys);
          sysDca[idx] = rho_sys;
          if(random3D) sysDca[idx] = 4.*(sysDca[idx]-1./3.)+1./3.;
          idx++;
        }
   ...
   /* if(correction == "AccRes")
    ...
    Double_t pt, rho;
    cout << "Before access " << KEY_Default << endl;
    g_mStatErrorsInt[KEY_Default]->GetPoint(i_point,pt,rho);
    cout << "After access" << endl;
    //float mean_rho = total_rho/(float)counter;
    cout << "Before Filling Sys errors" << endl;
    g_mSysErrorsFinal->SetPoint(i_point,pt,rho);
    g_mSysErrorsFinal->SetPointError(i_point,0.0,0.0,SysError_rho,SysError_rho);
    cout << "After Filling Sys errors" << endl;

    double StatError_rho = g_mStatErrorsInt[KEY_Default]->GetErrorYhigh(i_point);
    g_mStatErrorsFinal->SetPoint(i_point,pt,rho);
    g_mStatErrorsFinal->SetPointError(i_point,0.0,0.0,StatError_rho,StatError_rho);
    cout << "Finished cent = " << i_point << endl;
  }
  cout << "Finished integrated systematic error estimation " << endl;


  string outputname = Form("./figures/%s/%s/centralitystudy/finaloutputrho_%s_centralitydependence_pTbins_%s_Order%d_%s_PolySys.pdf",vmsa::mPID[pid].c_str(),vmsa::mBeamEnergy[energy].c_str(),correction.c_str(),vmsa::mBeamEnergy[energy].c_str(),order,etamode.c_str());
  string output_start = Form("%s[",outputname.c_str());

  TCanvas *c_pt = new TCanvas("c_pt","c_pt",10,10,900,300);
  c_pt->Divide(3,1);
  ...
  for(int i_pt = vmsa::pt_rebin_first_cent[energy]; i_pt <= vmsa::pt_rebin_last_cent[energy]; ++i_pt) // pt loop
  {
    ...
    string KEY_D = Form("rhoRaw_pt_%d_%s_Dca_%d_Sig_%d_%s_Norm_%d_Sigma_%d_%s_Poly1",i_pt,EP[order-1].c_str(),0,0,vmsa::mPID[pid].c_str(),0,0,vmsa::mInteMethod[1].c_str());

    Draw_TGAE_new_Symbol((TGraphAsymmErrors*)g_mStatErrors[KEY_D],style_phi_2nd,color_phi_2nd,colorDiff_phi,size_marker+0.2);

    plotSysErrorsBox(g_mSysErrors[KEY_D],color_phi_2nd);     
  ...
  */
  ...
  h_frame->GetYaxis()->SetRangeUser(0.15,0.50);
...
//  PlotLine(0.0,5.0,1.0/3.0,1.0/3.0,1,2,2);
  ...
  if(correction == "AccRes") leg_count = "#phi BES-II (|#eta_{K^{+/-}}| < 1.0)";
  ...
  c_rho_SysError->SaveAs(Form("figures/%s/%s/centralitystudy/finaloutputrho_%s_centralitydependence_%s_Order%d_%s_PolySys.pdf",vmsa::mPID[pid].c_str(),vmsa::mBeamEnergy[energy].c_str(),correction.c_str(),vmsa::mBeamEnergy[energy].c_str(),order,etamode.c_str()));  
  ...
  */
  ...
  string OutPutFile = Form("../output/AuAu%s/%s/RhoCent_%sSysErrors_%s_PolySys%s.root",vmsa::mBeamEnergy[energy].c_str(),vmsa::mPID[pid].c_str(),correction.c_str(),etamode.c_str(),spectra.c_str());
  if(order == 1) OutPutFile = Form("../output/AuAu%s/%s/RhoCent_%sSysErrors_%s_PolySys_FirstOrder%s.root",vmsa::mBeamEnergy[energy].c_str(),vmsa::mPID[pid].c_str(),correction.c_str(),etamode.c_str(),spectra.c_str());
      ...
      for(int i_norm = vmsa::Norm_start; i_norm < vmsa::Norm_stop; ++i_norm)
      {
        for(int i_sigma = vmsa::Sig_start; i_sigma < vmsa::Sig_stop; ++i_sigma)
        {
          for(int i_method = vmsa::Method_start; i_method < vmsa::Method_stop; ++i_method)
          {
            ...
            {
              string KEY_rho = Form("rhoRaw_%s_Dca_%d_Sig_%d_%s_Norm_%d_Sigma_%d_%s_Poly%d",EP[order-1].c_str(),i_dca,i_sig,vmsa::mPID[pid].c_str(),i_norm,i_sigma,vmsa::mInteMethod[i_method].c_str(),i_poly+1);
              g_mStatErrorsInt[KEY_rho]->Print();
              g_mStatErrorsInt[KEY_rho]->SetName(KEY_rho.c_str());
              g_mStatErrorsInt[KEY_rho]->Write();
              //cout << "Wrote: " << KEY_rho << endl;
            }
          }
        }
      }
  ...
  for(int i_pt = vmsa::pt_rebin_first_cent[energy]; i_pt <= vmsa::pt_rebin_last_cent[energy]; ++i_pt) // pt loop
  {           
    string KEY_rho = Form("rhoRaw_pt_%d_%s_Dca_%d_Sig_%d_%s_Norm_%d_Sigma_%d_%s_Poly1",i_pt,EP[order-1].c_str(),0,0,vmsa::mPID[pid].c_str(),0,0,vmsa::mInteMethod[1].c_str());
    string KEY_rhoStat = Form("rhoRawStat_pt_%d_%s_Dca_%d_Sig_%d_%s_Norm_%d_Sigma_%d_%s_Poly1",i_pt,EP[order-1].c_str(),0,0,vmsa::mPID[pid].c_str(),0,0,vmsa::mInteMethod[1].c_str());
    string KEY_rhoSys  = Form("rhoRawSys_pt_%d_%s_Dca_%d_Sig_%d_%s_Norm_%d_Sigma_%d_%s_Poly1",i_pt,EP[order-1].c_str(),0,0,vmsa::mPID[pid].c_str(),0,0,vmsa::mInteMethod[1].c_str());
