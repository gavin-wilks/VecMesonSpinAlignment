...
void calSpinAlignmentSysPhiY_PolySys_PhiPsi(int energy = 4, int pid = 0, int year = 0, bool random3D = false, int etaQA = 9, int ptQA = 1, int centQA = 2, int dcaQA = 0, int nsigQA = 0, int normQA = 0, int sigQA = 0, int order = 2, std::string etamode = "eta1_eta1")
{
  std::string EP[2] = {"","2nd"};
  ...
  string inputfile = Form("../output/AuAu%s/%s/PhiPsi_InvMassSubBgEta_%s.root",vmsa::mBeamEnergy[energy].c_str(),vmsa::mPID[pid].c_str(),etamode.c_str());
  if(order == 1) inputfile = Form("../output/AuAu%s/%s/PhiPsi_InvMassSubBgEta_%s_FirstOrder.root",vmsa::mBeamEnergy[energy].c_str(),vmsa::mPID[pid].c_str(),etamode.c_str());
  if(order == 1 && random3D) inputfile = Form("../output/AuAu%s/%s/3DRandom/InvMassSubBgEta_%s_FirstOrder_3DRandom.root",vmsa::mBeamEnergy[energy].c_str(),vmsa::mPID[pid].c_str(),etamode.c_str());
  ...
  for(int i_eta = vmsa::eta_start; i_eta < vmsa::eta_stop; i_eta++) // Rapidity bin loop
  {
      ...
      for(int i_cent = 0; i_cent < vmsa::cent_rebin_total; i_cent++) // Centrality loop
      {
        for(Int_t i_dca = vmsa::Dca_start; i_dca < vmsa::Dca_stop; i_dca++)
        {
          for(Int_t i_sig = vmsa::nSigKaon_start; i_sig < vmsa::nSigKaon_stop; i_sig++)
          {
            ...
            {
              string KEY_InteTheta = Form("eta_%d_pt_%d_Centrality_%d_%s_Dca_%d_Sig_%d_%s_Norm_%d",i_eta,i_pt,i_cent,EP[order-1].c_str(),i_dca,i_sig,vmsa::mPID[pid].c_str(),i_norm);
              for(int i_theta = vmsa::CTS_start; i_theta < vmsa::CTS_stop; i_theta++) // cos(theta*) loop
              {
                for(int i_phipsi = 0; i_phipsi < 7; i_phipsi++) // cos(theta*) loop
                {
                  string KEY = Form("eta_%d_pt_%d_Centrality_%d_CosThetaStar_%d_PhiPsi_%d_%s_Dca_%d_Sig_%d_%s_Norm_%d",i_eta,i_pt,i_cent,i_theta,i_phipsi,EP[order-1].c_str(),i_dca,i_sig,vmsa::mPID[pid].c_str(),i_norm);
              ...
              {
                string KEY_InteTheta_Poly = Form("eta_%d_pt_%d_Centrality_%d_%s_Dca_%d_Sig_%d_%s_Norm_%d_Poly%d",i_eta,i_pt,i_cent,EP[order-1].c_str(),i_dca,i_sig,vmsa::mPID[pid].c_str(),i_norm,i_poly+1);
                TF1 *f_bw;
...
//    {
//      string KEY_QA = Form("eta_%d_pt_%d_Centrality_%d_CosThetaStar_%d_PhiPsi_%d_2nd_Dca_%d_Sig_%d_%s_Norm_%d",etaQA,ptQA,centQA,i_theta,i_phipsi,dcaQA,nsigQA,vmsa::mPID[pid].c_str(),normQA);
...
//    if(i_theta == vmsa::CTS_stop)
//    {
//      string KEY_InteTheta_QA = Form("eta_%d_pt_%d_Centrality_%d_2nd_Dca_%d_Sig_%d_%s_Norm_%d",etaQA,ptQA,centQA,dcaQA,nsigQA,vmsa::mPID[pid].c_str(),normQA);
...
//#endif
  ...
  for(int i_pt = vmsa::pt_rebin_first[energy]; i_pt < vmsa::pt_rebin_last[energy]; i_pt++) // pt loop
  {
	...
	for(int i_norm = vmsa::Norm_start; i_norm < vmsa::Norm_stop; ++i_norm)
	{
	  for(int i_func = vmsa::Func_start; i_func < vmsa::Func_stop; ++i_func)
	  {
	    for(int i_theta = vmsa::CTS_start; i_theta < vmsa::CTS_stop; i_theta++) // cos(theta*) loop
	    {
	      string KEY = Form("pt_%d_Centrality_%d_EtaGap_%d_CosThetaStar_%d_PhiPsi_%d_2nd_%s_Norm_%d_Func_%d",i_pt,i_cent,i_eta,i_theta,i_phipsi,vmsa::mPID[pid].c_str(),i_norm,i_func);
	    }
	  }
	}
      }
  ...
  */
              ...
              for(int i_method = vmsa::Method_start; i_method < vmsa::Method_stop; ++i_method)
              {
                for(int i_poly = 0; i_poly < 3; i_poly++)
                {
          	    ...
          	    string KEY_counts = Form("eta_%d_pt_%d_Centrality_%d_%s_Dca_%d_Sig_%d_%s_Norm_%d_Sigma_%d_%s_Poly%d",i_eta,i_pt,i_cent,EP[order-1].c_str(),i_dca,i_sig,vmsa::mPID[pid].c_str(),i_norm,i_sigma,vmsa::mInteMethod[i_method].c_str(),i_poly+1);
                    h_mCounts[KEY_counts] = new TH2F(KEY_counts.c_str(),KEY_counts.c_str(),7,0.0,1.0,7,-TMath::Pi()/2.0,TMath::Pi()/2.0);
                    ...
                    for(int i_theta = vmsa::CTS_start; i_theta < vmsa::CTS_stop; ++i_theta) // cos(theta*) loop
                    {
                      ...
                      {
                        string KEY = Form("eta_%d_pt_%d_Centrality_%d_CosThetaStar_%d_PhiPsi_%d_%s_Dca_%d_Sig_%d_%s_Norm_%d",i_eta,i_pt,i_cent,i_theta,i_phipsi,EP[order-1].c_str(),i_dca,i_sig,vmsa::mPID[pid].c_str(),i_norm);
                        string KEY_Poly = Form("eta_%d_pt_%d_Centrality_%d_CosThetaStar_%d_PhiPsi_%d_%s_Dca_%d_Sig_%d_%s_Norm_%d_Poly%d",i_eta,i_pt,i_cent,i_theta,i_phipsi,EP[order-1].c_str(),i_dca,i_sig,vmsa::mPID[pid].c_str(),i_norm,i_poly+1);
                        TF1 *f_bw;
                        if(i_poly == 0) f_bw = new TF1("f_bw",PolyBreitWigner, vmsa::BW_Start[pid],vmsa::BW_Stop[pid],5);
                        if(i_poly == 1) f_bw = new TF1("f_bw",Poly2BreitWigner,vmsa::BW_Start[pid],vmsa::BW_Stop[pid],6);
                        if(i_poly == 2) f_bw = new TF1("f_bw",Poly3BreitWigner,vmsa::BW_Start[pid],vmsa::BW_Stop[pid],7);
                        ...
                        f_bw->SetParameter(2,Par_InteTheta[KEY_InteTheta_Poly][2]/49.0);
                        f_bw->SetParameter(3,Par_InteTheta[KEY_InteTheta_Poly][3]/49.0);
                        f_bw->SetParameter(4,Par_InteTheta[KEY_InteTheta_Poly][4]/49.0);
		        ...
		        TF1 *f_bg;
		          ...
		          if(i_poly >= 2) f_bg->SetParError(3,1.0);
	                ...
	                float bin_center_phipsi = ((float(i_phipsi)*2.)-7.)*TMath::Pi()/14.0+TMath::Pi()/14.0;
                        if(i_method == 0)
                        {
                      ...
                      h_mCounts1D[KEY_counts] = (TH1F*) h_mCounts[KEY_counts]->ProjectionX();

                      TF1 *f_rho = new TF1("f_rho",SpinDensity,0.0,1.0,2);
                      f_rho->SetParameter(0,0.33);
                      f_rho->SetParameter(1,h_mCounts1D[KEY_counts]->GetMaximum());
                      bool skip_eta = false;
                      //for(int ibin = 0; ibin < 7; ibin++) 
                      //{
                      //  if(h_mCounts[KEY_counts]->GetBinContent(ibin+1) == 0.0) 
                      //  {
                      //    skip_eta = true;
                      //    cout << "ONE OF THE BINS IS ZERO! SKIP THIS RAPIDITY" << endl;
                      //  }
                      //}
                      //if(skip_eta) continue;
                      h_mCounts1D[KEY_counts]->Fit(f_rho,"NMRI");
                      ...
                      if(!skip_y)
                      {
                        ...
                        //  g_mRho[KEY_rho]->SetPoint(i_eta,eta_mean,4.*(f_rho->GetParameter(0)-1./3.)+1./3.);
                        //  g_mRho[KEY_rho]->SetPointError(i_eta,0.0,0.0,f_rho->GetParError(0),4.*f_rho->GetParError(0));
                        //}
                        //else
                        //{
                          Par_rhoFit[KEY_counts].push_back(static_cast<float>(f_rho->GetParameter(0)));
                      ...
                      else
                      {
                        ...
                        Par_rhoFit[KEY_counts].push_back(static_cast<float>(f_rho->GetParameter(1)));
                        g_mRho[KEY_rho]->SetPoint(i_eta,eta_mean,-999.0);
                        g_mRho[KEY_rho]->SetPointError(i_eta,0.0,0.0,f_rho->GetParError(0),f_rho->GetParError(0));
                      }
                    }   
                  }
  ...
  for(int i_poly = 0; i_poly < 3; i_poly++)
  {
    string outputname = Form("./figures/%s/%s/rapiditystudy/PhiPsi_allThetaYields%s_%s_Order%d_%s_RapidityDependence_Poly%d.pdf",vmsa::mPID[pid].c_str(),vmsa::mBeamEnergy[energy].c_str(),vmsa::mPID[pid].c_str(),vmsa::mBeamEnergy[energy].c_str(),order,etamode.c_str(),i_poly+1);
    ...
    for(int i_cent = vmsa::Cent_start; i_cent < vmsa::cent_rebin_total; ++i_cent) // Centrality loop
    {
      for(int i_pt = vmsa::pt_rebin_first_y[energy]; i_pt <= vmsa::pt_rebin_last_y[energy]; ++i_pt) // pt loop
      {    
        for(int i_eta = vmsa::eta_start; i_eta < vmsa::eta_stop; i_eta++) // Rapidity bin loop
        {
          for(int i_phipsi = 0; i_phipsi < 7; i_phipsi++)
          {
            c_diff->Clear();
            c_diff->Divide(3,3);
            for(int i_theta = 0; i_theta < 9; ++i_theta)
            {
              ...
              if(i_theta < vmsa::CTS_stop)
              {
                string KEY_QA = Form("eta_%d_pt_%d_Centrality_%d_CosThetaStar_%d_PhiPsi_%d_%s_Dca_%d_Sig_%d_%s_Norm_%d",i_eta,i_pt,i_cent,i_theta,i_phipsi,EP[order-1].c_str(),vmsa::Dca_start,vmsa::nSigKaon_start,vmsa::mPID[pid].c_str(),vmsa::Norm_QA);
                h_mMass[KEY_QA]->SetTitle("");
                h_mMass[KEY_QA]->GetXaxis()->SetNdivisions(505,'N');
                h_mMass[KEY_QA]->GetXaxis()->SetLabelSize(0.03);
                h_mMass[KEY_QA]->GetXaxis()->SetTitle("M(K^{+},K^{-})");
                h_mMass[KEY_QA]->SetTitle(Form("%.2f<p_{T}<%.2f, %d#pi/14<#phi-#Psi_{2}<%d#pi/14, %.2f<y<%.2f, %d/7<|cos(#theta*)|<%d/7, cent(%d-%d)",vmsa::pt_low_y[energy][i_pt],vmsa::pt_up_y[energy][i_pt],i_phipsi*2-7,(i_phipsi+1)*2-7,vmsa::eta_bin[i_eta],vmsa::eta_bin[i_eta+1],i_theta,i_theta+1,centlow[i_cent],centhigh[i_cent]));
                h_mMass[KEY_QA]->GetXaxis()->SetTitleSize(0.05);
                h_mMass[KEY_QA]->GetXaxis()->SetTitleOffset(1.2);
                h_mMass[KEY_QA]->GetXaxis()->CenterTitle();
                ...
                //string KEY_InteTheta = Form("eta_%d_pt_%d_Centrality_%d_%s_Dca_%d_Sig_%d_%s_Norm_%d_Poly3",i_eta,i_pt,i_cent,EP[order-1].c_str(),vmsa::Dca_start,vmsa::nSigKaon_start,vmsa::mPID[pid].c_str(),vmsa::Norm_QA);
              ...
              if(i_theta == vmsa::CTS_stop)
              {
                string KEY_InteTheta_QA = Form("eta_%d_pt_%d_Centrality_%d_%s_Dca_%d_Sig_%d_%s_Norm_%d",i_eta,i_pt,i_cent,EP[order-1].c_str(),vmsa::Dca_start,vmsa::nSigKaon_start,vmsa::mPID[pid].c_str(),vmsa::Norm_QA);
                string KEY_InteTheta_QA_Poly = Form("eta_%d_pt_%d_Centrality_%d_%s_Dca_%d_Sig_%d_%s_Norm_%d_Poly%d",i_eta,i_pt,i_cent,EP[order-1].c_str(),vmsa::Dca_start,vmsa::nSigKaon_start,vmsa::mPID[pid].c_str(),vmsa::Norm_QA,i_poly+1);
                h_mMass_InteTheta[KEY_InteTheta_QA]->SetTitle("");
                h_mMass_InteTheta[KEY_InteTheta_QA]->GetXaxis()->SetNdivisions(505,'N');
                h_mMass_InteTheta[KEY_InteTheta_QA]->GetXaxis()->SetLabelSize(0.03);
                h_mMass_InteTheta[KEY_InteTheta_QA]->GetXaxis()->SetTitle("M(K^{+},K^{-})");
                h_mMass_InteTheta[KEY_InteTheta_QA]->SetTitle(Form("%.2f<p_{T}<%.2f, %.2f<y<%.2f, 0/7<|cos(#theta*)|<7/7, Cent(%d-%d)",vmsa::pt_low_y[energy][i_pt],vmsa::pt_up_y[energy][i_pt],vmsa::eta_bin[i_eta],vmsa::eta_bin[i_eta+1],centlow[i_cent],centhigh[i_cent]));
                h_mMass_InteTheta[KEY_InteTheta_QA]->GetXaxis()->SetTitleSize(0.05);
                h_mMass_InteTheta[KEY_InteTheta_QA]->GetXaxis()->SetTitleOffset(1.2);
                h_mMass_InteTheta[KEY_InteTheta_QA]->GetXaxis()->CenterTitle();
            ...
            for(int i_theta = vmsa::CTS_start; i_theta < vmsa::CTS_stop; ++i_theta)
            {
              c_diff->cd(i_theta+1);
              string KEY_QA = Form("eta_%d_pt_%d_Centrality_%d_CosThetaStar_%d_PhiPsi_%d_%s_Dca_%d_Sig_%d_%s_Norm_%d",i_eta,i_pt,i_cent,i_theta,i_phipsi,EP[order-1].c_str(),vmsa::Dca_start,vmsa::nSigKaon_start,vmsa::mPID[pid].c_str(),vmsa::Norm_QA);
              string KEY_QA_Poly = Form("eta_%d_pt_%d_Centrality_%d_CosThetaStar_%d_PhiPsi_%d_%s_Dca_%d_Sig_%d_%s_Norm_%d_Poly%d",i_eta,i_pt,i_cent,i_theta,i_phipsi,EP[order-1].c_str(),vmsa::Dca_start,vmsa::nSigKaon_start,vmsa::mPID[pid].c_str(),vmsa::Norm_QA,i_poly+1);
              TF1 *f_bw;
              if(i_poly == 0) f_bw = new TF1("f_bw",PolyBreitWigner,vmsa::BW_Start[pid],vmsa::BW_Stop[pid],5);
              if(i_poly == 1) f_bw = new TF1("f_bw",Poly2BreitWigner,vmsa::BW_Start[pid],vmsa::BW_Stop[pid],6);
              if(i_poly == 2) f_bw = new TF1("f_bw",Poly3BreitWigner,vmsa::BW_Start[pid],vmsa::BW_Stop[pid],7);
            ...
            for(int i_sigma = vmsa::Sig_start; i_sigma < /*vmsa::Sig_stop*/1; ++i_sigma)
            {
              ...
              {
                string KEY_counts_QA = Form("eta_%d_pt_%d_Centrality_%d_%s_Dca_%d_Sig_%d_%s_Norm_%d_Sigma_%d_%s_Poly%d",i_eta,i_pt,i_cent,EP[order-1].c_str(),vmsa::Dca_start,vmsa::nSigKaon_start,vmsa::mPID[pid].c_str(),vmsa::Norm_QA,i_sigma,vmsa::mInteMethod[i_method].c_str(),i_poly+1);
...
//#endif
    ...
    for(Int_t i_sig = vmsa::nSigKaon_start; i_sig < vmsa::nSigKaon_stop; i_sig++)
    {
      ...
      for(int i_norm = vmsa::Norm_start; i_norm < vmsa::Norm_stop; ++i_norm)
      {
        for(int i_sigma = vmsa::Sig_start; i_sigma < vmsa::Sig_stop; ++i_sigma)
        {
          for(int i_method = vmsa::Method_start; i_method < vmsa::Method_stop; ++i_method)
          {
            string KEY_rho = Form("rhoRaw_pt_%d_Centrality_%d_%s_Dca_%d_Sig_%d_%s_Norm_%d_Sigma_%d_%s_Poly3",ptQA,centQA,EP[order-1].c_str(),i_dca,i_sig,vmsa::mPID[pid].c_str(),i_norm,i_sigma,vmsa::mInteMethod[i_method].c_str());
  ...
  //TCanvas *c_chi = new TCanvas("c_chi","c_chi",10,10,1500,600);
  //c_chi->Divide(5,2);
  //for(int i = 0; i < 10; i++)
  //{
  //  c_chi->cd(i+1);
  ...
  //string outputname = Form("figures/%s/%s/rapiditystudy/Chi2NDF_%s_Order%d.pdf",vmsa::mPID[pid].c_str(),vmsa::mBeamEnergy[energy].c_str(),vmsa::mBeamEnergy[energy].c_str(),order);
  //string output_start = Form("%s[",outputname.c_str());
  //
  ...
  //          for(int i_eta = 2; i_eta < 12; i_eta++) // Centrality loop
  //          {
  //            c_chi->cd(i_eta-1);
  //            double min[3], max[3];
  //            for(int i_poly = 0; i_poly < 3; i_poly++)
  //            {
  //              string KEY_pT_Poly = Form("eta_%d_pt_%d_Centrality_%d_%s_Dca_%d_Sig_%d_%s_Norm_%d_Poly%d",i_eta,i_pt,i_cent,EP[order-1].c_str(),i_dca,i_sig,vmsa::mPID[pid].c_str(),i_norm,i_poly+1);
  ...
  //              if(i_eta == 2) leg1->AddEntry(g_mChiNDF[KEY_pT_Poly],Form("Poly %d",i_poly+1),"p");
  //            }
  //            for(int i_poly = 0; i_poly < 3; i_poly++)
  //            {
  //              string KEY_pT_Poly = Form("eta_%d_pt_%d_Centrality_%d_%s_Dca_%d_Sig_%d_%s_Norm_%d_Poly%d",i_eta,i_pt,i_cent,EP[order-1].c_str(),i_dca,i_sig,vmsa::mPID[pid].c_str(),i_norm,i_poly+1);
  ...
  //outputname = Form("figures/%s/%s/rapiditystudy/PValue_%s_Order%d.pdf",vmsa::mPID[pid].c_str(),vmsa::mBeamEnergy[energy].c_str(),vmsa::mBeamEnergy[energy].c_str(),order);
  ...
  //              }
  //              if(i_eta == 2) leg1->AddEntry(g_mPValue[KEY_pT_Poly],Form("Poly %d",i_poly+1),"p");
  //            }
  //            leg1->Draw("same");
  //          }
  //          
  ...
  string outputfile = Form("../output/AuAu%s/%s/PhiPsi_RawRhoEtaSys_%s_Poly.root",vmsa::mBeamEnergy[energy].c_str(),vmsa::mPID[pid].c_str(),etamode.c_str());
  if(order == 1) outputfile = Form("../output/AuAu%s/%s/PhiPsi_RawRhoEtaSys_%s_Poly_FirstOrder.root",vmsa::mBeamEnergy[energy].c_str(),vmsa::mPID[pid].c_str(),etamode.c_str());
  if(order == 1 && random3D) outputfile = Form("../output/AuAu%s/%s/3DRandom/RawRhoEtaSys_%s_FirstOrder.root",vmsa::mBeamEnergy[energy].c_str(),vmsa::mPID[pid].c_str(),etamode.c_str());
