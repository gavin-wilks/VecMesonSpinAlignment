...
void calSysErrorP_2D_OffDiag(Int_t energy = 4, Int_t pid = 0, Int_t i_cent = 9, string correction = "Raw", bool random3D = false, int order = 2, string etamode = "eta1_eta1", int yspectra = -1, int frameopt = 1)//defaultF = 0 is BESII, defaultF = 1 is BESI
{
  ...
  string inputfileHframe = Form("../output/AuAu%s/%s/%s2DP_RawPhiPSys_eta1_eta1.root",vmsa::mBeamEnergy[energy].c_str(),vmsa::mPID[pid].c_str(),frame.c_str());
  string inputfile = Form("../output/AuAu%s/%s/%s2DP_%sPhiPSys_%s%s.root",vmsa::mBeamEnergy[energy].c_str(),vmsa::mPID[pid].c_str(),frame.c_str(),correction.c_str(),etamode.c_str(),spectra.c_str());
  if(order == 1) inputfile = Form("../output/AuAu%s/%s/%s2DP_%sPhiPSys_%s_FirstOrder%s.root",vmsa::mBeamEnergy[energy].c_str(),vmsa::mPID[pid].c_str(),frame.c_str(),correction.c_str(),etamode.c_str(),spectra.c_str());
  if(correction == "RawRes") inputfile = Form("../output/AuAu%s/%s/%s2DP_RawPhiPSys_%s%s.root",vmsa::mBeamEnergy[energy].c_str(),vmsa::mPID[pid].c_str(),frame.c_str(),etamode.c_str(),spectra.c_str());
  if(correction == "RawRes" && order == 1) inputfile = Form("../output/AuAu%s/%s/%s2DP_RawPhiPSys_%s_FirstOrder%s.root",vmsa::mBeamEnergy[energy].c_str(),vmsa::mPID[pid].c_str(),frame.c_str(),etamode.c_str(),spectra.c_str());
  if(random3D) inputfile = Form("../output/AuAu%s/%s/3DRandom/%sPhiPSys.root",vmsa::mBeamEnergy[energy].c_str(),vmsa::mPID[pid].c_str(),correction.c_str());
	...
	for(int i_sigma = vmsa::Sig_start; i_sigma < vmsa::Sig_stop; ++i_sigma)
	{
	  for(int i_method = vmsa::Method_start; i_method < vmsa::Method_stop; ++i_method)
	  {
            ...
            { 
              ...
              {
	        string KEY = Form("%sRaw_Centrality_%d_%s_Dca_%d_Sig_%d_%s_Norm_%d_Sigma_%d_%s_Poly%d",parstring[i_par].c_str(),i_cent,EP[order-1].c_str(),i_dca,i_sig,vmsa::mPID[pid].c_str(),i_norm,i_sigma,vmsa::mInteMethod[i_method].c_str(),i_poly+1);
  ...
  h_frame->GetXaxis()->SetTitle("|p| (GeV/c)");
  ...
  TGraphAsymmErrors *g_SysErrors[5]; 
  TGraphAsymmErrors *g_StatErrors[5];
  for(int i_par = 0; i_par < 5; i_par++)
  {
    g_SysErrors[i_par]  = new TGraphAsymmErrors();
    g_StatErrors[i_par] = new TGraphAsymmErrors();
  } 
   
  double sysErr[vmsa::p_rebin_last_2D[energy]][6]; // N points with 5 sources of systematics for each
  ...
  for(int i_par = 0; i_par < 5; i_par++)
  {
    string KEY_Default = Form("%sRaw_Centrality_%d_%s_Dca_%d_Sig_%d_%s_Norm_%d_Sigma_%d_%s_Poly%d",parstring[i_par].c_str(),i_cent,EP[order-1].c_str(),0,0,vmsa::mPID[pid].c_str(),0,0,vmsa::mInteMethod[1].c_str(),1);
    for(Int_t i_point = 0/*vmsa::p_rebin_first[energy]*/; i_point < vmsa::p_rebin_last_2D[energy]-vmsa::p_rebin_first_2D[energy]; ++i_point)
    {
      double sysDca[9];
      double sysNSig[9];
      double sysNorm[9];
      ...
      double p_def, rho_def;
      g_mPar[KEY_Default]->GetPoint(i_point,p_def,rho_def); 
      ...
      cout << parstring[i_par] << ", pt = " << p_def << ", value = " << rho_def << " +/- " << rho_defErr << endl;
      ...
      { 
        for(int i_sigma = vmsa::Sig_start; i_sigma < vmsa::Sig_stop; ++i_sigma)
        {
          for(int i_method = vmsa::Method_start; i_method < vmsa::Method_stop; ++i_method)
          {
            ...
            string KEY = Form("%sRaw_Centrality_%d_%s_Dca_%d_Sig_%d_%s_Norm_%d_Sigma_%d_%s_Poly%d",parstring[i_par].c_str(),i_cent,EP[order-1].c_str(),i_dca,0,vmsa::mPID[pid].c_str(),0,i_sigma,vmsa::mInteMethod[i_method].c_str(),1);
            double p_sys, rho_sys;
            g_mPar[KEY]->GetPoint(i_point,p_sys,rho_sys);
            sysDca[idx] = rho_sys;
      ...
      for(Int_t i_sig = vmsa::nSigKaon_start; i_sig < vmsa::nSigKaon_stop; i_sig++)
      {
        for(int i_sigma = vmsa::Sig_start; i_sigma < vmsa::Sig_stop; ++i_sigma)
        {
          for(int i_method = vmsa::Method_start; i_method < vmsa::Method_stop; ++i_method)
          {
            ...
            string KEY = Form("%sRaw_Centrality_%d_%s_Dca_%d_Sig_%d_%s_Norm_%d_Sigma_%d_%s_Poly%d",parstring[i_par].c_str(),i_cent,EP[order-1].c_str(),0,i_sig,vmsa::mPID[pid].c_str(),0,i_sigma,vmsa::mInteMethod[i_method].c_str(),1);
            double p_sys, rho_sys;
            g_mPar[KEY]->GetPoint(i_point,p_sys,rho_sys);
            sysNSig[idx] = rho_sys;
      ...
      for(int i_norm = vmsa::Norm_start; i_norm < vmsa::Norm_stop; ++i_norm)
      {
        for(int i_sigma = vmsa::Sig_start; i_sigma < vmsa::Sig_stop; ++i_sigma)
        {
          for(int i_method = vmsa::Method_start; i_method < vmsa::Method_stop; ++i_method)
          {
            ...
            string KEY = Form("%sRaw_Centrality_%d_%s_Dca_%d_Sig_%d_%s_Norm_%d_Sigma_%d_%s_Poly%d",parstring[i_par].c_str(),i_cent,EP[order-1].c_str(),0,0,vmsa::mPID[pid].c_str(),i_norm,i_sigma,vmsa::mInteMethod[i_method].c_str(),1);
            double p_sys, rho_sys;
            g_mPar[KEY]->GetPoint(i_point,p_sys,rho_sys);
            sysNorm[idx] = rho_sys;
      ...
      for(int i_poly = 0; i_poly < 2; ++i_poly)
      {
        for(int i_sigma = vmsa::Sig_start; i_sigma < vmsa::Sig_stop; ++i_sigma)
        {
          for(int i_method = vmsa::Method_start; i_method < vmsa::Method_stop; ++i_method)
          {
            ...
            string KEY = Form("%sRaw_Centrality_%d_%s_Dca_%d_Sig_%d_%s_Norm_%d_Sigma_%d_%s_Poly%d",parstring[i_par].c_str(),i_cent,EP[order-1].c_str(),0,0,vmsa::mPID[pid].c_str(),0,i_sigma,vmsa::mInteMethod[i_method].c_str(),i_poly+1);
            double p_sys, rho_sys;
            g_mPar[KEY]->GetPoint(i_point,p_sys,rho_sys);
            sysPoly[idx] = rho_sys;
      ...
      g_SysErrors[i_par]->SetPoint(i_point,p_def,rho_def);
      g_SysErrors[i_par]->SetPointError(i_point,0.0,0.0,SysError_rho,SysError_rho);

      double StatError_rho = g_mPar[KEY_Default]->GetErrorYhigh(i_point);
      g_StatErrors[i_par]->SetPoint(i_point,p_def,rho_def);
      g_StatErrors[i_par]->SetPointError(i_point,0.0,0.0,StatError_rho,StatError_rho);
    }

    weight_par[i_par] /= weight_all[i_par];
    weight_error_stat_par[i_par] = TMath::Sqrt(1./weight_all[i_par]);
    cout << std::setprecision(4);
    cout << "Final rho00 = " << weight_par[i_par] << " +/- " << weight_error_stat_par[i_par] << " (stat.) " << endl;
          ...
          for(int i_sigma = vmsa::Sig_start; i_sigma < vmsa::Sig_stop; ++i_sigma)
          {
            for(int i_method = vmsa::Method_start; i_method < vmsa::Method_stop; ++i_method)
            {
              for(int i_poly = 0; i_poly < 3; i_poly++) 
              {
                ...
                {
                  Double_t pt, rho;
                  g_mPar[KEY]->GetPoint(i_point,pt,rho);
                ...
                vRho[KEY].push_back(weight_rho00f);
                vRhoErr[KEY].push_back(weight_error_stat_rho00f); 
              }
            }
          }
        }
    ...
    { 
      for(int i_sigma = vmsa::Sig_start; i_sigma < vmsa::Sig_stop; ++i_sigma)
      {
        for(int i_method = vmsa::Method_start; i_method < vmsa::Method_stop; ++i_method)
        {
          if(i_dca == 0 && (i_sigma != 0 || i_method == 0)) continue;
          if(i_dca != 0 && i_sigma != 0 && i_method == 1) continue;
          string KEY = Form("%sRaw_Centrality_%d_%s_Dca_%d_Sig_%d_%s_Norm_%d_Sigma_%d_%s_Poly1",parstring[i_par].c_str(),i_cent,EP[order-1].c_str(),i_dca,0,vmsa::mPID[pid].c_str(),0,i_sigma,vmsa::mInteMethod[i_method].c_str());
    ...
    for(Int_t i_sig = vmsa::nSigKaon_start; i_sig < vmsa::nSigKaon_stop; i_sig++)
    {
      for(int i_sigma = vmsa::Sig_start; i_sigma < vmsa::Sig_stop; ++i_sigma)
      {
        for(int i_method = vmsa::Method_start; i_method < vmsa::Method_stop; ++i_method)
        {
          if(i_sig == 0 && (i_sigma != 0 || i_method == 0)) continue;
          if(i_sig != 0 && i_sigma != 0 && i_method == 1) continue;
          string KEY = Form("%sRaw_Centrality_%d_%s_Dca_%d_Sig_%d_%s_Norm_%d_Sigma_%d_%s_Poly1",parstring[i_par].c_str(),i_cent,EP[order-1].c_str(),0,i_sig,vmsa::mPID[pid].c_str(),0,i_sigma,vmsa::mInteMethod[i_method].c_str());
    ...
    for(int i_norm = vmsa::Norm_start; i_norm < vmsa::Norm_stop; ++i_norm)
    {
      for(int i_sigma = vmsa::Sig_start; i_sigma < vmsa::Sig_stop; ++i_sigma)
      {
        for(int i_method = vmsa::Method_start; i_method < vmsa::Method_stop; ++i_method)
        {
          ...
          string KEY = Form("%sRaw_Centrality_%d_%s_Dca_%d_Sig_%d_%s_Norm_%d_Sigma_%d_%s_Poly1",parstring[i_par].c_str(),i_cent,EP[order-1].c_str(),0,0,vmsa::mPID[pid].c_str(),i_norm,i_sigma,vmsa::mInteMethod[i_method].c_str());
  ...
  //for(int ipt = 2; ipt < vmsa::p_rebin_last[energy]; ipt++)
  //{
  //  double pt,rho;
  ...
  //TGraphAsymmErrors *g27_18 = (TGraphAsymmErrors*) ((TGraphAsymmErrors*)bes27_18->Get("rho00_2ndEP_p_stat_27"))->Clone();
  ...
  //if(order == 2 && energy == 4) 
  //{
  //  besi19 = (TGraphAsymmErrors*)besi->Get("rho00_2ndEP_p_stat_27;1");
  //  besi19_sys = (TGraphAsymmErrors*)besi->Get("rho00_2ndEP_p_sys_27;1");
  //}
  //if(order == 1 && energy == 4) 
  //{
  //  besi19 = (TGraphAsymmErrors*)besi->Get("rho00_1stEP_p_stat_27;1");
  //  besi19_sys = (TGraphAsymmErrors*)besi->Get("rho00_1stEP_p_sys_27;1");
  //}
  ...
  for(int i_par = 0; i_par < 5; i_par++)
  {
    ...
    h_frame->GetYaxis()->SetRangeUser(-0.15,0.15);
    ...
    //}
    //}
     PlotLine(0.0,5.0,0.0,0.0,1,2,2);
  ...
  //plotTopLegend((char*)leg_sp.c_str(),0.3,0.48,0.03,2,0.0,42,0);
    ...
    g_StatErrors[i_par]->SetMarkerStyle(20);
    g_StatErrors[i_par]->SetMarkerColor(kRed);
    g_StatErrors[i_par]->SetLineColor(kRed);
    g_StatErrors[i_par]->SetMarkerSize(1.3);

    g_SysErrors[i_par]->SetMarkerStyle(20);
    g_SysErrors[i_par]->SetMarkerSize(1.3);
    g_SysErrors[i_par]->SetMarkerColor(kRed);
    g_SysErrors[i_par]->SetLineColor(kRed);
    ...
    Draw_TGAE_new_Symbol((TGraphAsymmErrors*)g_StatErrors[i_par],style_phi_2nd,color_phi_2nd,colorDiff_phi,size_marker+0.2);
    //if(energy == 4){
    ...
    plotSysErrorsBox(g_SysErrors[i_par],color_phi_2nd);
    //if(energy == 4){
  ...
  c_rho_SysError->SaveAs(Form("figures/%s/%s/pstudy/%s2D_finaloutputrho_%s_Order%d_%s.pdf",vmsa::mPID[pid].c_str(),vmsa::mBeamEnergy[energy].c_str(),frame.c_str(),correction.c_str(),order,etamode.c_str()));  
  c_rho_SysError->SaveAs(Form("figures/%s/%s/pstudy/%s2D_finaloutputrho_%s_Order%d_%s.eps",vmsa::mPID[pid].c_str(),vmsa::mBeamEnergy[energy].c_str(),frame.c_str(),correction.c_str(),order,etamode.c_str()));  
  c_rho_SysError->SaveAs(Form("figures/%s/%s/pstudy/%s2D_finaloutputrho_%s_Order%d_%s.png",vmsa::mPID[pid].c_str(),vmsa::mBeamEnergy[energy].c_str(),frame.c_str(),correction.c_str(),order,etamode.c_str()));  
  ...
  string OutPutFile = Form("../output/AuAu%s/%s/%s2DP_Rho_%sSysErrors_%s_%s.root",vmsa::mBeamEnergy[energy].c_str(),vmsa::mPID[pid].c_str(),frame.c_str(),correction.c_str(),etamode.c_str(),spectra.c_str());
  if(order == 1) OutPutFile = Form("../output/AuAu%s/%s/%s2DP_Rho_%sSysErrors_%s_FirstOrder%s.root",vmsa::mBeamEnergy[energy].c_str(),vmsa::mPID[pid].c_str(),frame.c_str(),correction.c_str(),etamode.c_str(),spectra.c_str());
  ...
  for(int i_par = 0; i_par < 5; i_par++)
  {
    ...
    g_StatErrors[i_par]->SetName(StatErrorRho.Data());
    g_StatErrors[i_par]->Write();
    ...
    g_SysErrors[i_par]->SetName(SysErrorRho.Data());
    g_SysErrors[i_par]->Write();
  }
  File_OutPut->Close();
}
...
/*void plotSysErrorsBox(TGraphAsymmErrors *g_rho, int plot_color, int beamE)
{
  ...
  for(int i_energy = 0; i_energy < 6; ++i_energy) // plot sys errors
  {
    ...
    bSys[i_energy] = new TBox(vmsa::p_low[beamE][i_energy],rho-err,vmsa::p_up[beamE][i_energy],rho+err);
