...
void calSpinAlignmentSysPhiCent_Global_2D_OffDiag(int energy = 4, int pid = 0, int year = 0, bool random3D = false, int centQA = 4, int ptQA = 0, int dcaQA = 0, int nsigQA = 0, int order = 2, std::string etamode = "eta1_eta1",int frameopt = 0)
{
  ...
  string inputfile = Form("../output/AuAu%s/%s/%s2DCent_InvMassSubBgCent_%s.root",vmsa::mBeamEnergy[energy].c_str(),vmsa::mPID[pid].c_str(),frame.c_str(),etamode.c_str());
  if(order == 1) inputfile = Form("../output/AuAu%s/%s/%s2DCent_InvMassSubBgCent_%s_FirstOrder.root",vmsa::mBeamEnergy[energy].c_str(),vmsa::mPID[pid].c_str(),frame.c_str(),etamode.c_str());
  ...
  for(int i_pt = vmsa::pt_rebin_first_cent_2D[energy]; i_pt < vmsa::pt_rebin_last_cent_2D[energy]; i_pt++) // pt loop
  {
      ...
      for(Int_t i_dca = vmsa::Dca_start; i_dca < 1/*vmsa::Dca_stop*/; i_dca++)
      {
        for(Int_t i_sig = vmsa::nSigKaon_start; i_sig < 1/*vmsa::nSigKaon_stop*/; i_sig++)
        {
          ...
          for(int i_norm = vmsa::Norm_start; i_norm < 1/*vmsa::Norm_stop*/; ++i_norm)
          {
            ...
            for(int i_theta = 0; i_theta < 9; i_theta++) // cos(theta*) loop
            {
              for(int i_phi = 0; i_phi < 12; i_phi++) // cos(theta*) loop
              {
                string KEY = Form("pt_%d_Centrality_%d_CosThetaStar_%d_PhiPrime_%d_%s_Dca_%d_Sig_%d_%s_Norm_%d",i_pt,i_cent,i_theta,i_phi,EP[order-1].c_str(),i_dca,i_sig,vmsa::mPID[pid].c_str(),i_norm);
                h_mMass[KEY] = (TH1F*)File_InPut->Get(KEY.c_str())->Clone();
                //h_mMass[KEY]->Rebin(2);

                if(i_theta == 0 && i_phi == 0) h_mMass_InteTheta[KEY_InteTheta] = (TH1F*)h_mMass[KEY]->Clone(KEY_InteTheta.c_str());
...
//#endif
  ...
  */
      ...
      for(Int_t i_sig = vmsa::nSigKaon_start; i_sig < 1/*vmsa::nSigKaon_stop*/; i_sig++)
      {
        ...
        for(int i_norm = vmsa::Norm_start; i_norm < 1/*vmsa::Norm_stop*/; ++i_norm)
        {
          for(int i_sigma = vmsa::Sig_start; i_sigma < 1/*vmsa::Sig_stop*/; ++i_sigma)
          {
	        ...
	        string KEY_real = Form("realRaw_pt_%d_%s_Dca_%d_Sig_%d_%s_Norm_%d_Sigma_%d_%s_Poly%d",i_pt,EP[order-1].c_str(),i_dca,i_sig,vmsa::mPID[pid].c_str(),i_norm,i_sigma,vmsa::mInteMethod[i_method].c_str(),i_poly+1);
	        string KEY_imag = Form("imagRaw_pt_%d_%s_Dca_%d_Sig_%d_%s_Norm_%d_Sigma_%d_%s_Poly%d",i_pt,EP[order-1].c_str(),i_dca,i_sig,vmsa::mPID[pid].c_str(),i_norm,i_sigma,vmsa::mInteMethod[i_method].c_str(),i_poly+1);
	        string KEY_rerho1n1 = Form("rerho1n1Raw_pt_%d_%s_Dca_%d_Sig_%d_%s_Norm_%d_Sigma_%d_%s_Poly%d",i_pt,EP[order-1].c_str(),i_dca,i_sig,vmsa::mPID[pid].c_str(),i_norm,i_sigma,vmsa::mInteMethod[i_method].c_str(),i_poly+1);
	        string KEY_imrho1n1 = Form("imrho1n1Raw_pt_%d_%s_Dca_%d_Sig_%d_%s_Norm_%d_Sigma_%d_%s_Poly%d",i_pt,EP[order-1].c_str(),i_dca,i_sig,vmsa::mPID[pid].c_str(),i_norm,i_sigma,vmsa::mInteMethod[i_method].c_str(),i_poly+1);
                //cout << KEY_rho << endl;
                ...
                for(int i_cent = vmsa::centStart; i_cent < vmsa::centStop; ++i_cent) // Centrality loop
                {
                  ...
                  for(int i_theta = 0; i_theta < 9; ++i_theta) // cos(theta*) loop
                  {
                    for(int i_phi = 0; i_phi < 12; ++i_phi) // cos(theta*) loop
                    {
                      ...
                      string KEY = Form("pt_%d_Centrality_%d_CosThetaStar_%d_PhiPrime_%d_%s_Dca_%d_Sig_%d_%s_Norm_%d",i_pt,i_cent,i_theta,i_phi,EP[order-1].c_str(),i_dca,i_sig,vmsa::mPID[pid].c_str(),i_norm);
                      string KEY_Poly = Form("pt_%d_Centrality_%d_CosThetaStar_%d_PhiPrime_%d_%s_Dca_%d_Sig_%d_%s_Norm_%d_Poly%d",i_pt,i_cent,i_theta,i_phi,EP[order-1].c_str(),i_dca,i_sig,vmsa::mPID[pid].c_str(),i_norm,i_poly+1);
                      TF1 *f_bw;
                      if(i_poly == 0) f_bw = new TF1("f_bw",PolyBreitWigner, vmsa::BW_Start[pid],vmsa::BW_Stop[pid],5);
                      if(i_poly == 1) f_bw = new TF1("f_bw",Poly2BreitWigner,vmsa::BW_Start[pid],vmsa::BW_Stop[pid],6);
                      if(i_poly == 2) f_bw = new TF1("f_bw",Poly3BreitWigner,vmsa::BW_Start[pid],vmsa::BW_Stop[pid],7);
	              ...
	              float cos_mean = float(i_theta+i_theta+1)/14.0;
                      ...
                      float bin_center     = float(i_theta-4)/4.5;
                      float bin_center_phi = float(i_phi+0.5)*TMath::Pi()/6.0;
                      if(i_method == 0)
                      {
                        ...
                        h_mCounts[KEY_counts]->SetBinContent(h_mCounts[KEY_counts]->FindBin(bin_center,bin_center_phi),counts-counts_bg);
                        h_mCounts[KEY_counts]->SetBinError(h_mCounts[KEY_counts]->FindBin(bin_center,bin_center_phi),TMath::Sqrt(errors+errors_bg*errors_bg));
                      }
                      if(i_method == 1)
                      {
                        float counts_bw;
                        ...
                        h_mCounts[KEY_counts]->SetBinContent(h_mCounts[KEY_counts]->FindBin(bin_center,bin_center_phi),counts_bw-counts_bg);
                        h_mCounts[KEY_counts]->SetBinError(h_mCounts[KEY_counts]->FindBin(bin_center,bin_center_phi),TMath::Sqrt(errors_bw*errors_bw+errors_bg*errors_bg));
                      }
                      Par[KEY_Poly].clear();
	              ...
	              g_mRho[KEY_rho]->SetPoint(i_cent,cent_mean,f_rho->GetParameter(0)-1./3.);
	              g_mRho[KEY_rho]->SetPointError(i_cent,0.0,0.0,f_rho->GetParError(0),f_rho->GetParError(0));
	              g_mReal[KEY_real]->SetPoint(i_cent,cent_mean,f_rho->GetParameter(1));
	              g_mReal[KEY_real]->SetPointError(i_cent,0.0,0.0,f_rho->GetParError(1),f_rho->GetParError(1));
	              g_mImag[KEY_imag]->SetPoint(i_cent,cent_mean,f_rho->GetParameter(2));
	              g_mImag[KEY_imag]->SetPointError(i_cent,0.0,0.0,f_rho->GetParError(2),f_rho->GetParError(2));
	              g_mReRho1n1[KEY_rerho1n1]->SetPoint(i_cent,cent_mean,f_rho->GetParameter(3));
	              g_mReRho1n1[KEY_rerho1n1]->SetPointError(i_cent,0.0,0.0,f_rho->GetParError(3),f_rho->GetParError(3));
	              g_mImRho1n1[KEY_imrho1n1]->SetPoint(i_cent,cent_mean,f_rho->GetParameter(4));
	              g_mImRho1n1[KEY_imrho1n1]->SetPointError(i_cent,0.0,0.0,f_rho->GetParError(4),f_rho->GetParError(4));
                    }
                    else
                    {
	              ...
	              g_mRho[KEY_rho]->SetPointError(i_cent,0.0,0.0,f_rho->GetParError(0),f_rho->GetParError(0));
	              g_mReal[KEY_real]->SetPoint(i_cent,cent_mean,-999.0);
	              g_mReal[KEY_real]->SetPointError(i_cent,0.0,0.0,f_rho->GetParError(1),f_rho->GetParError(1));
	              g_mImag[KEY_imag]->SetPoint(i_cent,cent_mean,-999.0);
	              g_mImag[KEY_imag]->SetPointError(i_cent,0.0,0.0,f_rho->GetParError(2),f_rho->GetParError(2));
	              g_mReRho1n1[KEY_rerho1n1]->SetPoint(i_cent,cent_mean,-999.0);
	              g_mReRho1n1[KEY_rerho1n1]->SetPointError(i_cent,0.0,0.0,f_rho->GetParError(3),f_rho->GetParError(3));
	              g_mImRho1n1[KEY_imrho1n1]->SetPoint(i_cent,cent_mean,-999.0);
	              g_mImRho1n1[KEY_imrho1n1]->SetPointError(i_cent,0.0,0.0,f_rho->GetParError(4),f_rho->GetParError(4));
                    }
                  }
                }
  ...
  for(int i_poly = 0; i_poly < 1/*3*/; i_poly++)
  {
    string outputname = Form("./figures/%s/%s/centralitystudy/%s2D_allThetaYields%s_%s_Order%d_%s_CentDependence_Poly%d.pdf",vmsa::mPID[pid].c_str(),vmsa::mBeamEnergy[energy].c_str(),frame.c_str(),vmsa::mPID[pid].c_str(),vmsa::mBeamEnergy[energy].c_str(),order,etamode.c_str(),i_poly+1);
      ...
      for(int i_pt = vmsa::pt_rebin_first_cent_2D[energy]; i_pt < vmsa::pt_rebin_last_cent_2D[energy]; ++i_pt) // pt loop
      {
        ...
        for(int i_phi = 0; i_phi < 12; i_phi++)
        {
          for(int i_theta = 0; i_theta < 9; ++i_theta)
          {
            ...
            if(i_theta < 9)
            {
              string KEY_QA = Form("pt_%d_Centrality_%d_CosThetaStar_%d_PhiPrime_%d_%s_Dca_%d_Sig_%d_%s_Norm_%d",i_pt,i_cent,i_theta,i_phi,EP[order-1].c_str(),vmsa::Dca_start,vmsa::nSigKaon_start,vmsa::mPID[pid].c_str(),vmsa::Norm_QA);
              h_mMass[KEY_QA]->SetTitle("");
              h_mMass[KEY_QA]->GetXaxis()->SetNdivisions(505,'N');
              h_mMass[KEY_QA]->GetXaxis()->SetLabelSize(0.03);
              h_mMass[KEY_QA]->GetXaxis()->SetTitle("M(K^{+},K^{-})");
              h_mMass[KEY_QA]->SetTitle(Form("%.2f<p_{T}<%.2f, phiH=%d, CTH=%d",vmsa::pt_low_cent_2D[energy][i_pt],vmsa::pt_up_cent_2D[energy][i_pt],i_phi,i_theta));
              h_mMass[KEY_QA]->GetXaxis()->SetTitleSize(0.05);
              h_mMass[KEY_QA]->GetXaxis()->SetTitleOffset(1.2);
              h_mMass[KEY_QA]->GetXaxis()->CenterTitle();
            ...
            //  h_mMass_InteTheta[KEY_InteTheta_QA]->SetTitle("");
            //  h_mMass_InteTheta[KEY_InteTheta_QA]->SetTitle(Form("%.2f<p_{T}<%.2f, Full phiH, Full CTG, cent%d",vmsa::pt_low_cent_2D[energy][i_pt],vmsa::pt_up_cent_2D[energy][i_pt],i_cent));
            //  h_mMass_InteTheta[KEY_InteTheta_QA]->GetXaxis()->SetNdivisions(505,'N');
            //  h_mMass_InteTheta[KEY_InteTheta_QA]->GetXaxis()->SetLabelSize(0.03);
            //  h_mMass_InteTheta[KEY_InteTheta_QA]->GetXaxis()->SetTitle("M(K^{+},K^{-})");
            //  h_mMass_InteTheta[KEY_InteTheta_QA]->GetXaxis()->SetTitleSize(0.05);
          ...
          {
            c_diff->cd(i_theta+1);
            string KEY_QA = Form("pt_%d_Centrality_%d_CosThetaStar_%d_%s_Dca_%d_Sig_%d_%s_Norm_%d",i_pt,i_cent,i_theta,EP[order-1].c_str(),vmsa::Dca_start,vmsa::nSigKaon_start,vmsa::mPID[pid].c_str(),vmsa::Norm_QA);
            string KEY_QA_Poly = Form("pt_%d_Centrality_%d_CosThetaStar_%d_PhiPrime_%d_%s_Dca_%d_Sig_%d_%s_Norm_%d_Poly%d",i_pt,i_cent,i_theta,i_phi,EP[order-1].c_str(),vmsa::Dca_start,vmsa::nSigKaon_start,vmsa::mPID[pid].c_str(),vmsa::Norm_QA,i_poly+1);
            TF1 *f_bw;
            if(i_poly == 0) f_bw = new TF1("f_bw",PolyBreitWigner,vmsa::BW_Start[pid],vmsa::BW_Stop[pid],5);
            if(i_poly == 1) f_bw = new TF1("f_bw",Poly2BreitWigner,vmsa::BW_Start[pid],vmsa::BW_Stop[pid],6);
            if(i_poly == 2) f_bw = new TF1("f_bw",Poly3BreitWigner,vmsa::BW_Start[pid],vmsa::BW_Stop[pid],7);
...
//#endif
  ...
  //  }
  //}
  //if(!random3D) c_rho->SaveAs("../figures/%s2DCent_OffDiag_c_rho.pdf");
  ...
  //string outputname = Form("figures/%s/%s/centralitystudy/%s2DCent_Chi2NDF_%s_Order%d.pdf",vmsa::mPID[pid].c_str(),vmsa::mBeamEnergy[energy].c_str(),frame.c_str(),vmsa::mBeamEnergy[energy].c_str(),order);
  //string output_start = Form("%s[",outputname.c_str());
  //
  ...
  //outputname = Form("figures/%s/%s/centralitystudy/%s2D_OffDiag_PValue_%s_Order%d.pdf",vmsa::mPID[pid].c_str(),vmsa::mBeamEnergy[energy].c_str(),frame.c_str(),vmsa::mBeamEnergy[energy].c_str(),order);
  ...
  for(int i_pt = vmsa::pt_rebin_first_cent_2D[energy]; i_pt < vmsa::pt_rebin_last_cent_2D[energy]; ++i_pt) // pt loop
  {
    string KEY_rho = Form("rhoRaw_pt_%d_%s_Dca_%d_Sig_%d_%s_Norm_%d_Sigma_%d_%s_Poly%d",i_pt,EP[order-1].c_str(),          0,0,vmsa::mPID[pid].c_str(),0,0,vmsa::mInteMethod[1].c_str(),1);
    string KEY_real = Form("realRaw_pt_%d_%s_Dca_%d_Sig_%d_%s_Norm_%d_Sigma_%d_%s_Poly%d",i_pt,EP[order-1].c_str(),        0,0,vmsa::mPID[pid].c_str(),0,0,vmsa::mInteMethod[1].c_str(),1);
    string KEY_imag = Form("imagRaw_pt_%d_%s_Dca_%d_Sig_%d_%s_Norm_%d_Sigma_%d_%s_Poly%d",i_pt,EP[order-1].c_str(),        0,0,vmsa::mPID[pid].c_str(),0,0,vmsa::mInteMethod[1].c_str(),1);
    string KEY_rerho1n1 = Form("rerho1n1Raw_pt_%d_%s_Dca_%d_Sig_%d_%s_Norm_%d_Sigma_%d_%s_Poly%d",i_pt,EP[order-1].c_str(),0,0,vmsa::mPID[pid].c_str(),0,0,vmsa::mInteMethod[1].c_str(),1);
    string KEY_imrho1n1 = Form("imrho1n1Raw_pt_%d_%s_Dca_%d_Sig_%d_%s_Norm_%d_Sigma_%d_%s_Poly%d",i_pt,EP[order-1].c_str(),0,0,vmsa::mPID[pid].c_str(),0,0,vmsa::mInteMethod[1].c_str(),1);

    c_spin->cd(1);
  g_mRho[KEY_rho]->GetXaxis()->SetNdivisions(505,'N');
  g_mRho[KEY_rho]->GetXaxis()->SetLabelSize(0.03);
  g_mRho[KEY_rho]->GetXaxis()->SetTitleSize(0.05);
  g_mRho[KEY_rho]->GetXaxis()->SetTitleOffset(1.2);
  g_mRho[KEY_rho]->GetXaxis()->CenterTitle();

  g_mRho[KEY_rho]->GetYaxis()->SetNdivisions(505,'N');
  g_mRho[KEY_rho]->GetYaxis()->SetLabelSize(0.03);
  g_mRho[KEY_rho]->GetYaxis()->SetTitleSize(0.05);
  g_mRho[KEY_rho]->GetYaxis()->CenterTitle();

    g_mRho[KEY_rho]->GetYaxis()->SetTitle("#Delta#rho_{00}");
    g_mRho[KEY_rho]->GetXaxis()->SetTitle("Centrality");
    g_mRho[KEY_rho]->SetMarkerStyle(20); 
    g_mRho[KEY_rho]->Draw("APE");
  PlotLine(0.0,80,0.0,0.0,1,1,2);

    c_spin->cd(2);
  g_mReal[KEY_real]->GetXaxis()->SetNdivisions(505,'N');
  g_mReal[KEY_real]->GetXaxis()->SetLabelSize(0.03);
  g_mReal[KEY_real]->GetXaxis()->SetTitleSize(0.05);
  g_mReal[KEY_real]->GetXaxis()->SetTitleOffset(1.2);
  g_mReal[KEY_real]->GetXaxis()->CenterTitle();

  g_mReal[KEY_real]->GetYaxis()->SetNdivisions(505,'N');
  g_mReal[KEY_real]->GetYaxis()->SetLabelSize(0.03);
  g_mReal[KEY_real]->GetYaxis()->SetTitleSize(0.05);
  g_mReal[KEY_real]->GetYaxis()->CenterTitle();

    g_mReal[KEY_real]->GetYaxis()->SetTitle("Re(#rho_{10})-Re(#rho_{0-1})");
    g_mReal[KEY_real]->GetXaxis()->SetTitle("Centrality");
    g_mReal[KEY_real]->SetMarkerStyle(20); 
    g_mReal[KEY_real]->Draw("APE");
  PlotLine(0.0,80,0.0,0.0,1,1,2);

    c_spin->cd(3);
  g_mImag[KEY_imag]->GetXaxis()->SetNdivisions(505,'N');
  g_mImag[KEY_imag]->GetXaxis()->SetLabelSize(0.03);
  g_mImag[KEY_imag]->GetXaxis()->SetTitleSize(0.05);
  g_mImag[KEY_imag]->GetXaxis()->SetTitleOffset(1.2);
  g_mImag[KEY_imag]->GetXaxis()->CenterTitle();

  g_mImag[KEY_imag]->GetYaxis()->SetNdivisions(505,'N');
  g_mImag[KEY_imag]->GetYaxis()->SetLabelSize(0.03);
  g_mImag[KEY_imag]->GetYaxis()->SetTitleSize(0.05);
  g_mImag[KEY_imag]->GetYaxis()->CenterTitle();

    g_mImag[KEY_imag]->GetYaxis()->SetTitle("Im(#rho_{10})-Im(#rho_{0-1})");
    g_mImag[KEY_imag]->GetXaxis()->SetTitle("Centrality");
    g_mImag[KEY_imag]->SetMarkerStyle(20); 
    g_mImag[KEY_imag]->Draw("APE");
  PlotLine(0.0,80,0.0,0.0,1,1,2);

    c_spin->cd(4);
  g_mReRho1n1[KEY_rerho1n1]->GetXaxis()->SetNdivisions(505,'N');
  g_mReRho1n1[KEY_rerho1n1]->GetXaxis()->SetLabelSize(0.03);
  g_mReRho1n1[KEY_rerho1n1]->GetXaxis()->SetTitleSize(0.05);
  g_mReRho1n1[KEY_rerho1n1]->GetXaxis()->SetTitleOffset(1.2);
  g_mReRho1n1[KEY_rerho1n1]->GetXaxis()->CenterTitle();

  g_mReRho1n1[KEY_rerho1n1]->GetYaxis()->SetNdivisions(505,'N');
  g_mReRho1n1[KEY_rerho1n1]->GetYaxis()->SetLabelSize(0.03);
  g_mReRho1n1[KEY_rerho1n1]->GetYaxis()->SetTitleSize(0.05);
  g_mReRho1n1[KEY_rerho1n1]->GetYaxis()->CenterTitle();

    g_mReRho1n1[KEY_rerho1n1]->GetYaxis()->SetTitle("Re(#rho_{1-1})");
    g_mReRho1n1[KEY_rerho1n1]->GetXaxis()->SetTitle("Centrality");
    g_mReRho1n1[KEY_rerho1n1]->SetMarkerStyle(20); 
    g_mReRho1n1[KEY_rerho1n1]->Draw("APE");
  PlotLine(0.0,80,0.0,0.0,1,1,2);

    c_spin->cd(5);
  g_mImRho1n1[KEY_imrho1n1]->GetXaxis()->SetNdivisions(505,'N');
  g_mImRho1n1[KEY_imrho1n1]->GetXaxis()->SetLabelSize(0.03);
  g_mImRho1n1[KEY_imrho1n1]->GetXaxis()->SetTitleSize(0.05);
  g_mImRho1n1[KEY_imrho1n1]->GetXaxis()->SetTitleOffset(1.2);
  g_mImRho1n1[KEY_imrho1n1]->GetXaxis()->CenterTitle();

  g_mImRho1n1[KEY_imrho1n1]->GetYaxis()->SetNdivisions(505,'N');
  g_mImRho1n1[KEY_imrho1n1]->GetYaxis()->SetLabelSize(0.03);
  g_mImRho1n1[KEY_imrho1n1]->GetYaxis()->SetTitleSize(0.05);
  g_mImRho1n1[KEY_imrho1n1]->GetYaxis()->CenterTitle();

    g_mImRho1n1[KEY_imrho1n1]->GetYaxis()->SetTitle("Im(#rho_{1-1})");
    g_mImRho1n1[KEY_imrho1n1]->GetXaxis()->SetTitle("Centrality");
    g_mImRho1n1[KEY_imrho1n1]->SetMarkerStyle(20); 
    g_mImRho1n1[KEY_imrho1n1]->Draw("APE");
  PlotLine(0.0,80,0.0,0.0,1,1,2);

    string outputname = Form("./figures/%s/%s/centralitystudy/%s2D_ExtractedSpinDensityElements_%s_Order%d_pt%d.pdf",vmsa::mPID[pid].c_str(),vmsa::mBeamEnergy[energy].c_str(),frame.c_str(),vmsa::mBeamEnergy[energy].c_str(),order,i_pt);
    c_spin->Print(outputname.c_str());
  }

  string outputfile = Form("../output/AuAu%s/%s/%s2DCent_RawRhoCentSys_%s_Poly.root",vmsa::mBeamEnergy[energy].c_str(),vmsa::mPID[pid].c_str(),frame.c_str(),etamode.c_str());
  if(order == 1) outputfile = Form("../output/AuAu%s/%s/%s2DCent_RawRhoCentSys_%s_Poly_FirstOrder.root",vmsa::mBeamEnergy[energy].c_str(),vmsa::mPID[pid].c_str(),frame.c_str(),etamode.c_str());
