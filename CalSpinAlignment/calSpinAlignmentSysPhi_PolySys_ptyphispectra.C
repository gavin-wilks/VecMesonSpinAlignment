...
void calSpinAlignmentSysPhi_PolySys_ptyphispectra(int energy = 4, int pid = 0, int year = 0, bool random3D = false, int order = 2, string etamode = "eta1_eta1")
{
  ...
  string inputfile = Form("../output/AuAu%s/%s/ptyphispectra_InvMassSubBg_%s.root",vmsa::mBeamEnergy[energy].c_str(),vmsa::mPID[pid].c_str(),etamode.c_str());
  ...
  bool skip[25][16][vmsa::phirebinphitotal] = {false};
  ...
  float InvMassLow[vmsa::phirebinpttotal][vmsa::phirebinytotal] = {0.0};
  float InvMassHigh[vmsa::phirebinpttotal][vmsa::phirebinytotal] = {0.0};
  float fpoly[vmsa::phirebinpttotal][vmsa::phirebinytotal] = {0.0};

  int phitotal = vmsa::phirebinphitotal; 
  float phivalues[phitotal+1];
  for(int iphi = 0; iphi < phitotal+1; iphi++)
  {
    phivalues[iphi] = (float(iphi)-12)*TMath::Pi()/12.;
  }
	...
	for(int i_norm = vmsa::Norm_start; i_norm < 1/*vmsa::Norm_stop*/; ++i_norm)
	{
	  for(int i_sigma = vmsa::Sig_start; i_sigma < 1/*vmsa::Sig_stop*/; ++i_sigma)
	  {
	    for(int i_method = 1/*vmsa::Method_start*/; i_method < vmsa::Method_stop; ++i_method)
	    {
              ...
              {
	        string KEY_spectra = Form("ptyphispectra_Centrality_%d_%s_Dca_%d_Sig_%d_%s_Norm_%d_Sigma_%d_%s_Poly%d",i_cent,EP[order-1].c_str(),i_dca,i_sig,vmsa::mPID[pid].c_str(),i_norm,i_sigma,vmsa::mInteMethod[i_method].c_str(),i_poly+1);
	        ...
	        h_mSpectra[KEY_spectra] = new TH2F(KEY_spectra.c_str(),KEY_spectra.c_str(),vmsa::phirebinytotal,vmsa::rebinyval,phitotal,phivalues);
	        h_mSpectra[KEY_spectra]->SetStats(0);
                cout << "Created 2D plot" << endl;
	        for(int i_pt = 0/*vmsa::pt_start*/; i_pt < vmsa::phirebinpttotal/*< vmsa::pt_stop*/; ++i_pt) // pt loop
	        {
                  cout << "i_pt = " << i_pt << endl;
	          for(int i_eta = 0; i_eta < vmsa::phirebinytotal; ++i_eta) // cos(theta*) loop
	          {
                    for(int i_phi = 0; i_phi < phitotal; i_phi++)
                    {
                      ...
                      double phi_mean = (phivalues[i_phi]+phivalues[i_phi])/2.0;                       

	              string KEY = Form("pt_%d_eta_%d_phi_%d_Centrality_%d_%s_Dca_%d_Sig_%d_%s_Norm_%d",i_pt,i_eta,i_phi,i_cent,EP[order-1].c_str(),i_dca,i_sig,vmsa::mPID[pid].c_str(),i_norm);
	              string KEY_Poly = Form("pt_%d_eta_%d_phi_%d_Centrality_%d_%s_Dca_%d_Sig_%d_%s_Norm_%d_Poly%d",i_pt,i_eta,i_phi,i_cent,EP[order-1].c_str(),i_dca,i_sig,vmsa::mPID[pid].c_str(),i_norm,i_poly+1);
                      cout << KEY << endl;
                      cout << KEY_Poly << endl;
	              TF1 *f_bw;
	              ...
	              if(i_method == 0)
	              {
	                ...
	                h_mSpectra[KEY_spectra]->SetBinContent(h_mSpectra[KEY_spectra]->FindBin(y_mean,phi_mean),final_counts);
	                h_mSpectra[KEY_spectra]->SetBinError(h_mSpectra[KEY_spectra]->FindBin(y_mean,phi_mean),TMath::Sqrt(errors+errors_bg*errors_bg));
	              }
	              if(i_method == 1)
	              {
	                //float bin_width = h_mMass[KEY]->GetBinWidth(1);
	                ...
	                h_mSpectra[KEY_spectra]->SetBinContent(h_mSpectra[KEY_spectra]->FindBin(y_mean,phi_mean),final_counts_bw);
	                h_mSpectra[KEY_spectra]->SetBinError(h_mSpectra[KEY_spectra]->FindBin(y_mean,phi_mean),TMath::Sqrt(errors_bw*errors_bw+errors_bg*errors_bg));
  ...
  //cout << endl << "const float InvMassLow[vmsa::phirebinpttotal][vmsa::phirebinytotal] = {";
  //for(int ptbin = 0; ptbin < vmsa::phirebinpttotal; ptbin++)
  //{
  //  cout << "{";
  //  for(int etabin = 0; etabin < vmsa::phirebinytotal; etabin++)
  //  {
  //    if(etabin < vmsa::phirebinytotal-1) cout << InvMassLow[ptbin][etabin] << ",";
  ...
  //cout << endl << "const float InvMassHigh[vmsa::phirebinpttotal][vmsa::phirebinytotal] = {";
  //for(int ptbin = 0; ptbin < vmsa::phirebinpttotal; ptbin++)
  //{
  //  cout << "{";
  //  for(int etabin = 0; etabin < vmsa::phirebinytotal; etabin++)
  //  {
  //    if(etabin < vmsa::phirebinytotal-1) cout << InvMassHigh[ptbin][etabin] << ",";
  ...
  //cout << endl << "const float fpoly[vmsa::phirebinpttotal][vmsa::phirebinytotal] = {";
  //for(int ptbin = 0; ptbin < vmsa::phirebinpttotal; ptbin++)
  //{
  //  cout << "{";
  //  for(int etabin = 0; etabin < vmsa::phirebinytotal; etabin++)
  //  {
  //    if(etabin < vmsa::phirebinytotal-1) cout << fpoly[ptbin][etabin] << ",";
  ...
  for(int i_poly = 0; i_poly < 1; i_poly++)
  {
    for(int i_norm = 0; i_norm < 1; i_norm++)
    {
      ...
      {
        string outputname = Form("./figures/%s/%s/pTstudy/ptyphispectra_allThetaYields%s_%s_Order%d_cent%d_%s_pTdependence_Norm%d_Poly%d.pdf",vmsa::mPID[pid].c_str(),vmsa::mBeamEnergy[energy].c_str(),vmsa::mPID[pid].c_str(),vmsa::mBeamEnergy[energy].c_str(),order,i_cent,etamode.c_str(),i_norm,i_poly+1);
	...
	for(int i_pt = 0/*vmsa::pt_start*/; i_pt < vmsa::phirebinpttotal/*< vmsa::pt_stop*/; ++i_pt) // pt loop
	    ...
	    for(int i_eta = 0; i_eta < vmsa::phirebinytotal; i_eta++) // cos(theta*) loop
	    {
              if(skip[i_pt][i_eta][i_phi]) continue;
              c_diff->cd(i_eta+1);
              ...
              string KEY_QA = Form("pt_%d_eta_%d_phi_%d_Centrality_%d_%s_Dca_%d_Sig_%d_%s_Norm_%d",i_pt,i_eta,i_phi,i_cent,EP[order-1].c_str(),vmsa::Dca_start,vmsa::nSigKaon_start,vmsa::mPID[pid].c_str(),i_norm);
              //cout << KEY_QA << endl;
              ...
              h_mMass[KEY_QA]->SetTitle(Form("%.2f<p_{T}<%.2f, %.2f<y<%.2f, %d#pi/12<#phi<%d#pi/12",vmsa::rebinptval[i_pt],vmsa::rebinptval[i_pt+1],vmsa::rebinyval[i_eta],vmsa::rebinyval[i_eta+1],i_phi-12,i_phi-11));
              ...
              string KEY_QA_Poly = Form("pt_%d_eta_%d_phi_%d_Centrality_%d_%s_Dca_%d_Sig_%d_%s_Norm_%d_Poly%d",i_pt,i_eta,i_phi,i_cent,EP[order-1].c_str(),vmsa::Dca_start,vmsa::nSigKaon_start,vmsa::mPID[pid].c_str(),i_norm,i_poly+1);
  ...
  for(int i_poly = 0; i_poly < 1/*3*/; i_poly++)
  {
    for(int i_norm = 0; i_norm < 1/*3*/; i_norm++)
    {
      for(int i_cent = 9; i_cent < 10; ++i_cent) // Centrality loop
      {
        string outputname = Form("./figures/%s/%s/pTstudy/ptyphispectra_2Dyield%s_%s_Order%d_cent%d_%s_pTdependence_Norm%d_Poly%d.pdf",vmsa::mPID[pid].c_str(),vmsa::mBeamEnergy[energy].c_str(),vmsa::mPID[pid].c_str(),vmsa::mBeamEnergy[energy].c_str(),order,i_cent,etamode.c_str(),i_norm,i_poly+1);
        ...
        for(int i_sigma = 0/*vmsa::Sig_start*/; i_sigma < 1/*vmsa::Sig_stop*/; ++i_sigma)
        {
          ...
          {
            string KEY_spectra_QA = Form("ptyphispectra_Centrality_%d_%s_Dca_%d_Sig_%d_%s_Norm_%d_Sigma_%d_%s_Poly%d",i_cent,EP[order-1].c_str(),vmsa::Dca_start,0/*vmsa::nSigKaon_start*/,vmsa::mPID[pid].c_str(),i_norm,i_sigma,vmsa::mInteMethod[i_method].c_str(),i_poly+1);
            ...
            h_mSpectra[KEY_spectra_QA]->GetXaxis()->SetTitle("y");
            ...
            h_mSpectra[KEY_spectra_QA]->GetYaxis()->SetTitle("#phi");
  ...
  string outputfile = Form("../output/AuAu%s/%s/ptyphispectra_RawPhiPtSys_%s_PolySys.root",vmsa::mBeamEnergy[energy].c_str(),vmsa::mPID[pid].c_str(),etamode.c_str());
