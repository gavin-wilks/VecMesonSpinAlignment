...
void appEffCorrSysPhiY(int energy = 4, int pid = 0, int ptQA = 2, bool isBesI = false, bool random3D = false, int etamode = 0, int y_padding = 2)
{
  ...
  if(etamode == 0) y_padding = 2; 
  if(etamode == 3) y_padding = 5; 
  if(etamode == 4) y_padding = 4; 
  if(etamode == 5) y_padding = 3; 

  string InPutRho = Form("../output/AuAu%s/%s/RawRhoEtaSys_%s.root",vmsa::mBeamEnergy[energy].c_str(),vmsa::mPID[pid].c_str(),etastring.c_str());
  if(random3D) InPutRho = Form("../output/AuAu%s/%s/3DRandom/RawRhoEtaSys.root",vmsa::mBeamEnergy[energy].c_str(),vmsa::mPID[pid].c_str());
  if(isBesI) InPutRho = Form("../data/BESI/RawRhoEtaSys.root");
  TFile *File_Rho = TFile::Open(InPutRho.c_str());
  cout << InPutRho << endl;

  string InPutEff = Form("./efficiencies/%s/%s/Eff_%s_SingleParticle_noToF_Mode2_EtaMode%d.root",vmsa::mPID[pid].c_str(),vmsa::mBeamEnergy[energy].c_str(),vmsa::mBeamEnergy[energy].c_str(),etamode);
  ...
  for(Int_t i_dca = vmsa::Dca_start; i_dca < vmsa::Dca_stop; i_dca++)
  {
    for(Int_t i_sig = vmsa::nSigKaon_start; i_sig < vmsa::nSigKaon_stop; i_sig++)
    {
      ...
      for(int i_norm = vmsa::Norm_start; i_norm < vmsa::Norm_stop; ++i_norm)
      {
        for(int i_sigma = vmsa::Sig_start; i_sigma < vmsa::Sig_stop; ++i_sigma)
        {
          ...
          {
            for(int i_pt = vmsa::pt_rebin_first_y[energy]; i_pt <= vmsa::pt_rebin_last_y[energy]; i_pt++)
            {
              for(int i_cent = 0; i_cent < vmsa::cent_rebin_total; ++i_cent) // Centrality loop
              {
                ...
                for(int i_y = y_padding; i_y < vmsa::eta_stop-y_padding; i_y++)
                { 
                  string KEY_counts = Form("eta_%d_pt_%d_Centrality_%d_2nd_Dca_%d_Sig_%d_%s_Norm_%d_Sigma_%d_%s",i_y,i_pt,i_cent,i_dca,i_sig,vmsa::mPID[pid].c_str(),i_norm,i_sigma,vmsa::mInteMethod[i_method].c_str());
                  cout << KEY_counts << endl;
                  h_mCounts[KEY_counts] = (TH1F*)File_Rho->Get(KEY_counts.c_str());
                  //h_mCounts[KEY_counts]->Print();
                  string KEY_Eff = Form("h_mEffCos_Cent_%d_Pt_%d_Y_%d",i_cent,i_pt,i_y); 
                  //cout << KEY_Eff << endl;
                  ...
                  float y_mean = (vmsa::eta_bin[i_y]+vmsa::eta_bin[i_y+1])/2.0;
                  ...
                  g_mRho[KEY_rho]->SetPoint(i_y-y_padding,y_mean,f_rho->GetParameter(0));
                  g_mRho[KEY_rho]->SetPointError(i_y-y_padding,0.0,0.0,f_rho->GetParError(0),f_rho->GetParError(0));
                }
              }
            }
          }
    ...
    //string KEY_counts_QA = Form("Centrality_%d_2nd_Dca_%d_Sig_%d_%s_Norm_%d_Sigma_%d_%s",ipt,4,0,0,vmsa::mPID[pid].c_str(),0,0,vmsa::mInteMethod[1].c_str());
  ...
  string OutPutFile = Form("../output/AuAu%s/%s/EffRhoEtaSys_%s.root",vmsa::mBeamEnergy[energy].c_str(),vmsa::mPID[pid].c_str(),etastring.c_str());
  if(random3D) OutPutFile = Form("../output/AuAu%s/%s/3DRandom/EffRhoCentSys.root",vmsa::mBeamEnergy[energy].c_str(),vmsa::mPID[pid].c_str());
  if(isBesI) OutPutFile = Form("../output/AuAu%s/%s/BESI/EffRhoCentSys.root",vmsa::mBeamEnergy[energy].c_str(),vmsa::mPID[pid].c_str());
          ...
          for(int i_method = vmsa::Method_start; i_method < vmsa::Method_stop; ++i_method)
          {
            for(int i_pt = vmsa::pt_rebin_first_y[energy]; i_pt <= vmsa::pt_rebin_last_y[energy]; i_pt++)
            {
              for(int i_cent = 0; i_cent < vmsa::cent_rebin_total; i_cent++)
              {
                ...
                {
                  string KEY_counts = Form("eta_%d_pt_%d_Centrality_%d_2nd_Dca_%d_Sig_%d_%s_Norm_%d_Sigma_%d_%s",i_y,i_pt,i_cent,i_dca,i_sig,vmsa::mPID[pid].c_str(),i_norm,i_sigma,vmsa::mInteMethod[i_method].c_str());
                  h_mCountsEff[KEY_counts]->SetName(KEY_counts.c_str());
                  h_mCountsEff[KEY_counts]->Write();
                }
              }
            }
          }
