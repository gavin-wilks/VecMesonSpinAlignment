...
void compRhoCorr_Cent(Int_t energy = 4, Int_t pid = 0, Int_t order = 2)
{
  std::string ep = "2nd";
  if(order == 1) ep = "";
  std::string EP = "";
  ...
  string inputfileF0 = Form("../output/AuAu%s/%s/RhoCent_RawSysErrors_eta1_eta1_PolySys%s.root",vmsa::mBeamEnergy[energy].c_str(),vmsa::mPID[pid].c_str(),EP.c_str());
  string inputfileF1 = Form("../output/AuAu%s/%s/RhoCent_RawResSysErrors_eta1_eta1_PolySys%s.root",vmsa::mBeamEnergy[energy].c_str(),vmsa::mPID[pid].c_str(),EP.c_str());
  string inputfileF2 = Form("../output/AuAu%s/%s/RhoCent_AccResSysErrors_eta1_eta1_PolySys%s_NoRapiditySpectra_FixedFirstEP.root",vmsa::mBeamEnergy[energy].c_str(),vmsa::mPID[pid].c_str(),EP.c_str());
  ...
  {           
    string KEY_rho = Form("rhoRaw_pt_%d_%s_Dca_%d_Sig_%d_%s_Norm_%d_Sigma_%d_%s_Poly1",i_pt,ep.c_str(),0,0,vmsa::mPID[pid].c_str(),0,0,vmsa::mInteMethod[1].c_str());
    string KEY_rhoStat = Form("rhoRawStat_pt_%d_%s_Dca_%d_Sig_%d_%s_Norm_%d_Sigma_%d_%s_Poly1",i_pt,ep.c_str(),0,0,vmsa::mPID[pid].c_str(),0,0,vmsa::mInteMethod[1].c_str());
    string KEY_rhoSys  = Form("rhoRawSys_pt_%d_%s_Dca_%d_Sig_%d_%s_Norm_%d_Sigma_%d_%s_Poly1",i_pt,ep.c_str(),0,0,vmsa::mPID[pid].c_str(),0,0,vmsa::mInteMethod[1].c_str());
...
//  PlotLine(0.0,5.0,1.0/3.0,1.0/3.0,1,2,2);
  ...
  Draw_TGAE_Point_new_Symbol(5,0.42,0.0,0.0,0.0,0.0,20,kBlue,1.3); 
  string leg_count0 = Form("Raw #phi");
  plotTopLegend((char*)leg_count0.c_str(),7,0.417,0.03,1,0.0,42,0);

  Draw_TGAE_Point_new_Symbol(5,0.40,0.0,0.0,0.0,0.0,20,kOrange+7,1.3); 
  string leg_count1 = Form("Raw Resolution Correctd #phi");
  plotTopLegend((char*)leg_count1.c_str(),7,0.397,0.03,1,0.0,42,0);

  Draw_TGAE_Point_new_Symbol(5,0.38,0.0,0.0,0.0,0.0,20,kGray+2,1.3); 
  string leg_count2 = Form("Fully Corrected #phi");
  plotTopLegend((char*)leg_count2.c_str(),7,0.377,0.03,1,0.0,42,0);
  ...
  PlotLine(1,5.5,0.442,0.442,1,2,2);
  string leg_line = "#rho_{00} = 1/3";
  plotTopLegend((char*)leg_line.c_str(),7,0.44,0.03,1,0.0,42,0);


  string leg_energy = Form("AuAu %s %d", vmsa::mBeamEnergy[energy].c_str(), vmsa::mBeamYear[energy]);
  plotTopLegend((char*)leg_energy.c_str(),30,0.28,0.04,1,0.0,42,0);
  ...
  {
    double cent, rho;
    g_StatF1->GetPoint(i,cent,rho);
    g_StatF1->SetPoint(i,cent+1,rho);
    g_SysF1->SetPoint(i,cent+1,rho);
  }
  ...
  for(int i = 0; i < g_StatF2->GetN(); i++)
  {
    double cent, rho;
    g_StatF2->GetPoint(i,cent,rho);
    g_StatF2->SetPoint(i,cent+2,rho);
    g_SysF2->SetPoint(i,cent+2,rho);
  }
  ...
  c_rho_SysError->SaveAs(Form("./figures/Phi/%s/rho00Cent_eta1_eta1_%s_Order%d.pdf",vmsa::mBeamEnergy[energy].c_str(),vmsa::mBeamEnergy[energy].c_str(),order));

  
  //string outputname = Form("./figures/Phi/19GeV/rho00centComparison_eta1_eta1.pdf");
  ...
  //  if(i_pt == 0)
  //  {
  //    Draw_TGAE_Point_new_Symbol(5,0.48,0.0,0.0,0.0,0.0,20,kBlue,1.3); 
  //    string leg_count0 = Form("#phi (Raw)");
  //    plotTopLegend((char*)leg_count0.c_str(),7,0.477,0.03,1,0.0,42,0);

  //    Draw_TGAE_Point_new_Symbol(5,0.46,0.0,0.0,0.0,0.0,20,kOrange+7,1.3); 
  //    string leg_count1 = Form("#phi (Eff)");
  //    plotTopLegend((char*)leg_count1.c_str(),7,0.457,0.03,1,0.0,42,0);

  //    Draw_TGAE_Point_new_Symbol(5,0.44,0.0,0.0,0.0,0.0,20,kGray+2,1.3); 
  //    string leg_count2 = Form("#phi (Eff+Acc+Res)");
  //    plotTopLegend((char*)leg_count2.c_str(),7,0.437,0.03,1,0.0,42,0);

  //    PlotLine(1,5.5,0.422,0.422,1,2,2);
  //    string leg_line = "#rho_{00} = 1/3";
  //    plotTopLegend((char*)leg_line.c_str(),7,0.42,0.03,1,0.0,42,0);
  ...
  //  Draw_TGAE_new_Symbol((TGraphAsymmErrors*)g_mRhoF0_Stat[KEY_Default],20,kBlue,0,1.3);
  //  plotSysErrorsBox(g_mRhoF0_Sys[KEY_Default],kBlue);     
  //  Draw_TGAE_new_Symbol((TGraphAsymmErrors*)g_mRhoF1_Stat[KEY_Default],20,kOrange+7,0,1.3);
  //  plotSysErrorsBox(g_mRhoF1_Sys[KEY_Default],kOrange+7);     
  //  Draw_TGAE_new_Symbol((TGraphAsymmErrors*)g_mRhoF2_Stat[KEY_Default],20,kGray+2,0,1.3);
