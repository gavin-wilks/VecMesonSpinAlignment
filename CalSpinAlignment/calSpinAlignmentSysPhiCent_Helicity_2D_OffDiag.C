...
void calSpinAlignmentSysPhiCent_Helicity_2D_OffDiag(int energy = 4, int pid = 0, int year = 0, bool random3D = false, int centQA = 4, int ptQA = 0, int dcaQA = 0, int nsigQA = 0, int order = 2, std::string etamode = "eta1_eta1")
{
  ...
  string inputfile = Form("../output/AuAu%s/%s/Helicity2DCent_InvMassSubBgCent_%s.root",vmsa::mBeamEnergy[energy].c_str(),vmsa::mPID[pid].c_str(),etamode.c_str());
...
//#endif
  ...
  */
          ...
          for(int i_sigma = vmsa::Sig_start; i_sigma < vmsa::Sig_stop; ++i_sigma)
          {
	        ...
	        g_mImRho1n1[KEY_imrho1n1] = new TGraphAsymmErrors();
                for(int i_cent = vmsa::centStart; i_cent < vmsa::centStop; ++i_cent) // Centrality loop
                {
                  ...
                  h_mCounts[KEY_counts] = new TH2F(KEY_counts.c_str(),KEY_counts.c_str(),10,-1.0,1.0,10,0.0,2.0*TMath::Pi());
	              ...
	              float cos_mean = float(i_theta+i_theta+1)/14.0;
                      ...
                      float bin_center     = float(i_theta-5)/5.0;
                      float bin_center_phi = float(i_phi)*TMath::Pi()/5.0;
                      if(i_method == 0)
                      {
  ...
  for(int i_poly = 0; i_poly < 3; i_poly++)
  {
    string outputname = Form("./figures/%s/%s/centralitystudy/Helicity2D_allThetaYields%s_%s_Order%d_%s_CentDependence_Poly%d.pdf",vmsa::mPID[pid].c_str(),vmsa::mBeamEnergy[energy].c_str(),vmsa::mPID[pid].c_str(),vmsa::mBeamEnergy[energy].c_str(),order,etamode.c_str(),i_poly+1);
        ...
        for(int i_phi = 0; i_phi < 12; i_phi++)
        {
          for(int i_theta = 0; i_theta < 10; ++i_theta)
          {
            ...
            if(i_theta < 9)
            {
              ...
              h_mMass[KEY_QA]->GetXaxis()->SetLabelSize(0.03);
              h_mMass[KEY_QA]->GetXaxis()->SetTitle("M(K^{+},K^{-})");
              h_mMass[KEY_QA]->SetTitle(Form("%.2f<p_{T}<%.2f, phiH=%d, CTH=%d",vmsa::pt_low_cent_2D[energy][i_pt],vmsa::pt_up_cent_2D[energy][i_pt],i_phi,i_theta,i_cent));
              h_mMass[KEY_QA]->GetXaxis()->SetTitleSize(0.05);
              h_mMass[KEY_QA]->GetXaxis()->SetTitleOffset(1.2);
              h_mMass[KEY_QA]->GetXaxis()->CenterTitle();
          ...
          c_diff->Update();
          c_diff->Print(outputname.c_str9());
        }
...
//#endif
  ...
  //  }
  //}
  //if(!random3D) c_rho->SaveAs("../figures/Global2DCent_OffDiag_c_rho.pdf");
  ...
  string outputname = Form("figures/%s/%s/centralitystudy/Global2D_Chi2NDF_%s_Order%d.pdf",vmsa::mPID[pid].c_str(),vmsa::mBeamEnergy[energy].c_str(),vmsa::mBeamEnergy[energy].c_str(),order);
  string output_start = Form("%s[",outputname.c_str());
  ...
  outputname = Form("figures/%s/%s/centralitystudy/Global2D_OffDiag_PValue_%s_Order%d.pdf",vmsa::mPID[pid].c_str(),vmsa::mBeamEnergy[energy].c_str(),vmsa::mBeamEnergy[energy].c_str(),order);
  ...
  string outputname = Form("./figures/%s/%s/centralitystudy/Global2D_ExtractedSpinDensityElements_%s_Order%d.pdf",vmsa::mPID[pid].c_str(),vmsa::mBeamEnergy[energy].c_str(),vmsa::mBeamEnergy[energy].c_str(),order);
  c_spin->Print(outputname.c_str());

  string outputfile = Form("../output/AuAu%s/%s/Global2DCent_RawRhoCentSys_%s.root",vmsa::mBeamEnergy[energy].c_str(),vmsa::mPID[pid].c_str(),etamode.c_str());
  if(order == 1) outputfile = Form("../output/AuAu%s/%s/Global2DCent_RawRhoCentSys_%s_FirstOrder.root",vmsa::mBeamEnergy[energy].c_str(),vmsa::mPID[pid].c_str(),etamode.c_str());
