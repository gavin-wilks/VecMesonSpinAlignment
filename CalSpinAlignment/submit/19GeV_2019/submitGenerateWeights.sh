...
codePath=/star/u/gwilks3/Workspace/VectorMesonSpinAlignment/VecMesonSpinAlignment/CalSpinAlignment
...
outDir=${pid}${SM}_EtaMode${etamode}_20240912_ygaus

mkdir -p JOBS/report
mkdir -p JOBS/csh
mkdir -p JOBS/list

mkdir -p ${outPath}/Log/EmbedWeights/${outDir}
mkdir -p ${outPath}/OutPut/EmbedWeights/${outDir}

##########Test Production##########
star-submit-template -template generateWeights.xml -entities etamode=$etamode,pid=$pid,mode=$mode,energy=$energy,flag_ME=$flag_ME,SM=$SM,library=$library,codePath=$codePath,outPath=$outPath,listPath=$listPath,outDir=$outDir
##########Test Production##########
