...
void appEffCorrSysPhiCent(int energy = 4, int pid = 0, int ptQA = 2, bool isBesI = false, bool random3D = false, int etamode = 5)
{
  ...
  string InPutRho = Form("../output/AuAu%s/%s/RawRhoCentSys_%s.root",vmsa::mBeamEnergy[energy].c_str(),vmsa::mPID[pid].c_str(),etastring.c_str());
  if(random3D) InPutRho = Form("../output/AuAu%s/%s/3DRandom/RawRhoCentSys.root",vmsa::mBeamEnergy[energy].c_str(),vmsa::mPID[pid].c_str());
  if(isBesI) InPutRho = Form("../data/BESI/RawRhoCentSys.root");
  TFile *File_Rho = TFile::Open(InPutRho.c_str());
  cout << InPutRho << endl;

  string InPutEff = Form("./efficiencies/%s/%s/Eff_%s_SingleParticle_noToF_Mode1_EtaMode%d.root",vmsa::mPID[pid].c_str(),vmsa::mBeamEnergy[energy].c_str(),vmsa::mBeamEnergy[energy].c_str(),etamode);
          ...
          for(int i_method = vmsa::Method_start; i_method < vmsa::Method_stop; ++i_method)
          {
            for(int i_pt = vmsa::pt_rebin_first_cent[energy]; i_pt <= vmsa::pt_rebin_last_cent[energy]; i_pt++)
            {
              ...
              for(int i_cent = 0; i_cent < 9; ++i_cent) // Centrality loop
              {
                ...
                string KEY_Eff = Form("h_mEffCos_Cent_%d_Pt_%d",i_cent,i_pt); 
                //cout << KEY_Eff << endl;
  ...
  string OutPutFile = Form("../output/AuAu%s/%s/EffRhoCentSys_%s.root",vmsa::mBeamEnergy[energy].c_str(),vmsa::mPID[pid].c_str(),etastring.c_str());
