...
void compareDataRcPhiStarRho00(Int_t energy = 4, Int_t pid = 0, Int_t i_cent = 9, string correction = "Raw", int order = 2, string etamode = "eta1_eta1", int yspectra = 23, std::string simmode = "Mc")//defaultF = 0 is BESII, defaultF = 1 is BESI
{
  const int nvar = 7;
  ...
  //std::string spectra[3] = Form("_NoRapiditySpectra_FixedFirstEP_RCBins_prelimv2_phieff_flatRP_Acc1Rc_pT0p1_TPC_TOF_everything_20240509_fixed2Dfunc_FinerPhiBins_Spectra_rerho1n10.0_ysigma1000");
 
  std::string spectra[nvar];
  for(int i = 0; i < nvar; i++)
  {
    spectra[i] = Form("CEPT_O2_NoYSpec_prelimv2_phieff_phiTPC_20240809g_ysig1000_rho%s_rerho0.0_imrho0.0_r0.0_i0.0_hrho0.3333_hrerho0.0_himrho0.0_hr0.0_hi0.0",sigmay_text[i].c_str());
  }
  ...
  std::string outputname = Form("figures/%s/%s/pTstudy/Cos2PhiStarPhi_Ratio_%s_Order%d_%s%s_rho00comparison_%s.pdf",vmsa::mPID[pid].c_str(),vmsa::mBeamEnergy[energy].c_str(),correction.c_str(),order,etamode.c_str(),spectra[3].c_str(),simmode.c_str());  
  ...
  for(int ipt = 0; ipt < 4; ipt++)
  {
    ...
    for(int i = 0; i < nvar; i++)
    {
      ...
      {
        if(par == 0) g2D[par][ipt]->SetPoint(i,sigmay[i]-1./3.,fits2D[ipt][i]->GetParameter(0)-1./3.);
        else g2D[par][ipt]->SetPoint(i,sigmay[i]-1./3.,fits2D[ipt][i]->GetParameter(par));
  ...
  cfit2D->SaveAs(Form("figures/%s/%s/pTstudy/Cos2PhiStarPhi_parametercomparison_%s_Order%d_%s%s_rho00comparison_%s.pdf",vmsa::mPID[pid].c_str(),vmsa::mBeamEnergy[energy].c_str(),correction.c_str(),order,etamode.c_str(),spectra[3].c_str(),simmode.c_str()));  
  ...
  cfit->SaveAs(Form("figures/%s/%s/pTstudy/Cos2PhiStarPhi_FitParams_%s_Order%d_%s%s_rho00comparison_%s.pdf",vmsa::mPID[pid].c_str(),vmsa::mBeamEnergy[energy].c_str(),correction.c_str(),order,etamode.c_str(),spectra[3].c_str(),simmode.c_str()));  


  outputname = Form("figures/%s/%s/pTstudy/Cos2PhiStarPhi_Comparison_%s_Order%d_%s%s_rho00comparison_%s.pdf",vmsa::mPID[pid].c_str(),vmsa::mBeamEnergy[energy].c_str(),correction.c_str(),order,etamode.c_str(),spectra[3].c_str(),simmode.c_str());  
