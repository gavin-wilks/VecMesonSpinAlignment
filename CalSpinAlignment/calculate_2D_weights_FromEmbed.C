...
void calculate_2D_weights_FromEmbed(int energy = 4, double inputv2 = 50) 
{
    ...
    string spectra = "PhiEmbedding_nov2nospec";
    string filename = "PhiEmbedding_nov2nospec.root";
    ...
    TH2F *h_flat_pt_phi[9];
    TH2F *h_desired_pt_phi[9];
    TH1F *h_flat_pt[9];
    TH1F *h_flat_phi[9];
    TH1F *h_desired_pt[9];
    TH1F *h_desired_phi[9];
    for(int icent = 2; icent <= 5; icent++)
    {
      string flat = Form("h_flat_pt_phi_cent%d",icent);
      h_flat_pt_phi[icent] = (TH2F*) ((TH2F*)File_Input->Get(Form("h_mRc0EffPtPhiPsi_Cent_%d",icent)))->Clone(flat.c_str());
      ...
      //h_flat_phi[icent] = (TH1F*) h_flat_pt_phi[icent]->ProjectionY(flat.c_str(),0,-1,"e");    

      string desired = Form("h_desired_pt_phi_cent%d",icent);
      h_desired_pt_phi[icent] = new TH2F(desired.c_str(), Form("Desired pT-phi Distribution cent %d",icent), vmsa::pt_total, vmsa::pt_bin, nBinsPhi, phiMin, phiMax);
      ...
      //h_desired_phi[icent] = (TH1F*) h_desired_pt_phi[icent]->ProjectionY(desired.c_str(),0,-1,"e");    
    }
    ...
    // Define the desired pT distribution as a TF1
    TF1 *desired_pt[9];
    TF1 *v2_func[9];
    for(int icent = 2; icent <= 5; icent++)
    {
    ...
    // Define the v2 function

    TF1 *azimuthal = new TF1("azimuthal",flowSample,phiMin,phiMax,1);
    // Fill the desired pT-phi histogram
    for (int i = 0; i < numberOfEvents; ++i) 
    {
        int cent = rand.Integer(4) + 2;
        if(i%10000 == 0){
          cout << "Processed " << i << " Events " << endl;
        }
        //cout << "CENTRALITY = " << cent << endl;

        //double pt_flat = rand.Uniform(ptMin, ptMax); // Flat pT distribution
        //double phi_flat = rand.Uniform(phiMin, phiMax); // Flat phi distribution
        //h_flat_pt_phi[cent]->Fill(pt_flat, phi_flat);

        //if(pt_flat > ptMax || pt_flat < ptMin) cout << "pT is out of range!" << endl;
        //if(phi_flat > phiMax || phi_flat < phiMin) cout << "phi is out of range!" << endl;

        //cout << "pt_flat = " << pt_flat << ", phi_flat = " << phi_flat << endl;
        ...
        double phi_true = azimuthal->GetRandom(); // Sample phi with v2 modulation
        h_desired_pt_phi[cent]->Fill(pt_true, phi_true);
    }
    ...
    for(int icent = 2; icent <= 5; icent++)
    {
      double integralfunc = h_desired_pt_phi[icent]->Integral(6,17,1,nBinsPhi) ;// from function
      double integraldata = h_flat_pt_phi[icent]->Integral(6,17,1,nBinsPhi) ;//from data
      h_desired_pt_phi[icent]->Scale(integraldata/integralfunc);
      h_ratio[icent] = (TH2F*)h_desired_pt_phi[icent]->Clone(Form("h_ratio_cent%d",icent));
