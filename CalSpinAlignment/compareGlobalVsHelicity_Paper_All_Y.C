...
void compareGlobalVsHelicity_Paper_All_Y(Int_t energy = 4, Int_t pid = 0, Int_t i_cent = 9, string correction = "Raw", int order = 2, string etamode = "eta1_eta1", int yspectra = 23, std::string simmode = "Mc", int inputpar = 0)//defaultF = 0 is BESII, defaultF = 1 is BESI
{
  ...
  const int nopts = 11;
  std::string option = "y_Variance_RC";
  ...
  for(int ikin = 0; ikin < nkin; ikin++)
  {
    ...
    {
      string inputfileRC = Form("effaccfiles/%s/%s/PaperAllYRC/Eff_%s_SingleParticle_noToF_Mode0_EtaMode0_pt%1.1f_y%1.1f.root",vmsa::mPID[pid].c_str(),vmsa::mBeamEnergy[energy].c_str(),vmsa::mBeamEnergy[energy].c_str(),ptkin[ikin],vals[iopt]);
    ...
    for(int irho = 0; irho < 5; irho++)
    {
      ...
      for(int par = 0; par < 5; par++)
      {
        ...
        for(int i = 0; i < nvar; i++)
        {
          ...
          {
            case 0: 
              rhoGv2[par][irho][i][ikin] = new TF1(Form("rhoGv2_%d_%d_%d_%d",par,irho,i,ikin),rhoGfromHy,-1.0,1.0,7);
              break;
            case 1: 
              rhoGv2[par][irho][i][ikin] = new TF1(Form("rhoGv2_%d_%d_%d_%d",par,irho,i,ikin),realGfromHy,-1.0,1.0,7);
              break;
            case 2: 
              rhoGv2[par][irho][i][ikin] = new TF1(Form("rhoGv2_%d_%d_%d_%d",par,irho,i,ikin),imagGfromHy,-1.0,1.0,7);
              break;
            case 3: 
              rhoGv2[par][irho][i][ikin] = new TF1(Form("rhoGv2_%d_%d_%d_%d",par,irho,i,ikin),rerho1n1GfromHy,-1.0,1.0,7);
              break;
            case 4: 
              rhoGv2[par][irho][i][ikin] = new TF1(Form("rhoGv2_%d_%d_%d_%d",par,irho,i,ikin),imrho1n1GfromHy,-1.0,1.0,7);
              break;
          }

          rhoGv2[par][irho][i][ikin]->SetParameter(0,0.2);      
          rhoGv2[par][irho][i][ikin]->SetParameter(1,double(ptkin[ikin]));      
          ...
          g2D_Global[irho][i][ikin][par]->SetTitle(Form("Input %s^{h}, v_{2}=0.2, p_{T}=%1.1f GeV/c",param[irho].c_str(),ptkin[ikin]));
          g2D_Global[irho][i][ikin][par]->GetXaxis()->SetTitle(Form("y"));
          ...
          g2D_Global[irho][i][ikin][par]->GetXaxis()->SetRangeUser(-1.0-0.05,1.0+0.05);
    ...
    for(int irho = 0; irho < 5; irho++)
    {
      for(int par = 0; par < 5; par++)
      {
        ...
        for(int i = 0; i < nvar; i++)
        {
          ...
          g2D[irho][i][ikin][par]->SetTitle(Form("Input %s^{Helicity}, v_{2}=0.2, p_{T}=%1.1f GeV/c",param[irho].c_str(),ptkin[ikin]));
  ...
  for(int ikin = 0; ikin < nkin; ikin++)
  {
    for(int irho = 0; irho < 5; irho++)
    {
      for(int i = 0; i < nvar; i++)
      {
        ...
        {
          c2D->cd(1+iopt);
          h_m2D_RC_Global[iopt][irho][i][ikin]->SetTitle(Form("p_{T}=%1.1f GeV/c, Input %s^{Helicity}=%1.1f, v_{2}=0.2, y=%1.1f",ptkin[ikin],param[irho].c_str(),(float(i)-2)*0.1,vals[iopt]));
  ...
  for(int ikin = 0; ikin < nkin; ikin++)
  {
    for(int irho = 0; irho < 5; irho++)
    {
      for(int i = 0; i < nvar; i++)
      {
        ...
        {
          c2D->cd(1+iopt);
          h_m2D_RC[iopt][irho][i][ikin]->SetTitle(Form("p_{T}=%1.1f GeV/c, Input %s^{Helicity}=%1.1f, v_{2}=0.2, y=%1.1f",ptkin[ikin],param[irho].c_str(),(float(i)-2)*0.1,vals[iopt]));
