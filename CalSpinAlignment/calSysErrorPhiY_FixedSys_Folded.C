...
void calSysErrorPhiY_FixedSys_Folded(Int_t energy = 4, Int_t pid = 0, string correction = "AccRes", bool random3D = false, int ptQA = 0, int centQA = 2, int order = 2, std::string etamode = "eta1_eta1", std::string fileoption = "FixedResolutionFinal")//defaultF = 0 is BESII, defaultF = 1 is BESI
{
...
//  PlotLine(0.0,5.0,1.0/3.0,1.0/3.0,1,2,2);
  ...
  c_rho_SysError->SaveAs(Form("./figures/%s/%s/rapiditystudy/finaloutputrho_rapiditydependence_%s_%s_%s_order%d_%s.pdf",vmsa::mPID[pid].c_str(),vmsa::mBeamEnergy[energy].c_str(),correction.c_str(),vmsa::mPID[pid].c_str(),vmsa::mBeamEnergy[energy].c_str(),order,etamode.c_str()));  
  ...
  string outputname = Form("./figures/%s/%s/rapiditystudy/finaloutputrho_rapiditydependence_%s_ptandcentbins_%s_%s_order%d_%s.pdf",vmsa::mPID[pid].c_str(),vmsa::mBeamEnergy[energy].c_str(),correction.c_str(),vmsa::mPID[pid].c_str(),vmsa::mBeamEnergy[energy].c_str(),order,etamode.c_str());
    ...
    for(Int_t i_sig = vmsa::nSigKaon_start; i_sig < vmsa::nSigKaon_stop; i_sig++)
    {
      ...
      for(int i_norm = vmsa::Norm_start; i_norm < vmsa::Norm_stop; ++i_norm)
      {
        for(int i_sigma = vmsa::Sig_start; i_sigma < vmsa::Sig_stop; ++i_sigma)
        {
          ...
          {
            string KEY_rho = Form("rhoRaw_%s_Dca_%d_Sig_%d_%s_Norm_%d_Sigma_%d_%s",EP[order-1].c_str(),i_dca,i_sig,vmsa::mPID[pid].c_str(),i_norm,i_sigma,vmsa::mInteMethod[i_method].c_str());
            string KEY_rho_unfolded = Form("rhoRaw_%s_Dca_%d_Sig_%d_%s_Norm_%d_Sigma_%d_%s_Unfolded",EP[order-1].c_str(),i_dca,i_sig,vmsa::mPID[pid].c_str(),i_norm,i_sigma,vmsa::mInteMethod[i_method].c_str());
