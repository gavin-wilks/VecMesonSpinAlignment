...
void compRhoEff_AcceptanceMethod(Int_t energy = 4, Int_t pid = 0, Int_t i_cent = 9, string correction = "AccRes")
{
  ...
  string inputfileF1 = Form("../output/AuAu%s/%s/Rho_%sSysErrors_F_0_2ndUpdated.root",vmsa::mBeamEnergy[energy].c_str(),vmsa::mPID[pid].c_str(),correction.c_str());
  string inputfileF2 = Form("../output/AuAu%s/%s/Rho_%sSysErrors_F_0_4th.root",vmsa::mBeamEnergy[energy].c_str(),vmsa::mPID[pid].c_str(),correction.c_str());
...
//  PlotLine(0.0,5.0,1.0/3.0,1.0/3.0,1,2,2);
  ...
  string leg_count0 = Form("#phi (2^{nd})");
  ...
  string leg_count1 = Form("#phi (2^{nd} Updated)");
  ...
  string leg_count2 = Form("#phi (4^{th})");
