...
void compareGlobalVsHelicity_Paper_All_PhiPsi(Int_t energy = 4, Int_t pid = 0, Int_t i_cent = 9, string correction = "Raw", int order = 2, string etamode = "eta1_eta1", int yspectra = 23, std::string simmode = "Mc", int inputpar = 0)//defaultF = 0 is BESII, defaultF = 1 is BESI
{
  ...
  const int nkin = 1;
  const int ptkin[nkin] = {2};
  const int ykin[nkin]  = {1};
  ...
  const int nopts = 20;
  std::string option = "phipsi_Variance_RC";
  ...
  for(int i = 0; i < nopts; i++) vals[i] = (float(i)+0.5)*TMath::Pi()/20.;
  ...
  {
    string inputfileRC = Form("effaccfiles/%s/%s/PaperAllPhiPsiRC/Eff_%s_SingleParticle_noToF_Mode0_EtaMode0_pt%d_y%d.root",vmsa::mPID[pid].c_str(),vmsa::mBeamEnergy[energy].c_str(),vmsa::mBeamEnergy[energy].c_str(),ptkin[ikin],ykin[ikin]);
    ...
    for(int iopt = 0; iopt < nopts; iopt++)
    {
      for(int irho = 0; irho < 5; irho++)
      {
        ...
        {
          string KEY_2D_RC = Form("h_m%sEffCosPhiPrimeH_v2_0_rhoinput_%d_rho_%d_phipsi_%d",simmode.c_str(),irho,i,iopt);
          string KEY_2D_RC_Global = Form("h_m%sEffCosPhiPrime_v2_0_rhoinput_%d_rho_%d_phipsi_%d",simmode.c_str(),irho,i,iopt);
          string KEY_2D = Form("h_m%sEffCosPhiPrimeH_v2_0_rhoinput_%d_rho_%d_phipsi_%d_kin_%d",simmode.c_str(),irho,i,iopt,ikin);
          string KEY_2D_Global = Form("h_m%sEffCosPhiPrime_v2_0_rhoinput_%d_rho_%d_phipsi_%d_kin_%d",simmode.c_str(),irho,i,iopt,ikin);
    ...
    for(int irho = 0; irho < 5; irho++)
    {
      ...
      for(int par = 0; par < 5; par++)
      {
        ...
        for(int i = 0; i < nvar; i++)
        {
          ...
          {
            case 0: 
              rhoGv2[par][irho][i][ikin] = new TF1(Form("rhoGv2_%d_%d_%d_%d",par,irho,i,ikin),rhoGfromHphipsi,0.0,TMath::Pi(),7);
              break;
            case 1: 
              rhoGv2[par][irho][i][ikin] = new TF1(Form("rhoGv2_%d_%d_%d_%d",par,irho,i,ikin),realGfromHphipsi,0.0,TMath::Pi(),7);
              break;
            case 2: 
              rhoGv2[par][irho][i][ikin] = new TF1(Form("rhoGv2_%d_%d_%d_%d",par,irho,i,ikin),imagGfromHphipsi,0.0,TMath::Pi(),7);
              break;
            case 3: 
              rhoGv2[par][irho][i][ikin] = new TF1(Form("rhoGv2_%d_%d_%d_%d",par,irho,i,ikin),rerho1n1GfromHphipsi,0.0,TMath::Pi(),7);
              break;
            case 4: 
              rhoGv2[par][irho][i][ikin] = new TF1(Form("rhoGv2_%d_%d_%d_%d",par,irho,i,ikin),imrho1n1GfromHphipsi,0.0,TMath::Pi(),7);
              break;
          }
          ...
          g2D_Global[irho][i][ikin][par]->SetTitle(Form("Input %s^{h}, p_{T}=%d GeV/c, y=%d",param[irho].c_str(),ptkin[ikin],ykin[ikin]));
          g2D_Global[irho][i][ikin][par]->GetXaxis()->SetTitle(Form("#phi-#Psi"));
          ...
          g2D_Global[irho][i][ikin][par]->GetXaxis()->SetRangeUser(0.0-0.05,TMath::Pi()+0.05);
    ...
    for(int irho = 0; irho < 5; irho++)
    {
      for(int par = 0; par < 5; par++)
      {
        ...
        for(int i = 0; i < nvar; i++)
        {
          ...
          g2D[irho][i][ikin][par]->GetXaxis()->SetTitle(Form("#phi-#Psi"));
  ...
  for(int ikin = 0; ikin < nkin; ikin++)
  {
    for(int irho = 0; irho < 5; irho++)
    {
      for(int i = 0; i < nvar; i++)
      {
        ...
        {
          c2D->cd(1+iopt);
          h_m2D_RC_Global[iopt][irho][i][ikin]->SetTitle(Form("#phi-#Psi=%1.2f, Input %s^{Helicity}=%1.1f, p_{T}=%d GeV/c, y=%d",vals[iopt],param[irho].c_str(),(float(i)-2)*0.1,ptkin[ikin],ykin[ikin]));
  ...
  for(int ikin = 0; ikin < nkin; ikin++)
  {
    for(int irho = 0; irho < 5; irho++)
    {
      for(int i = 0; i < nvar; i++)
      {
        ...
        {
          c2D->cd(1+iopt);
          h_m2D_RC[iopt][irho][i][ikin]->SetTitle(Form("#phi-#Psi=%1.2f, Input %s^{Helicity}=%1.1f, p_{T}=%d GeV/c, y=%d",vals[iopt],param[irho].c_str(),(float(i)-2)*0.1,ptkin[ikin],ykin[ikin]));
