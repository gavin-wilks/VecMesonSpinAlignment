...
void compareDataRcPhiStarHRho00_global(Int_t energy = 4, Int_t pid = 0, Int_t i_cent = 9, string correction = "Raw", int order = 2, string etamode = "eta1_eta1", int yspectra = 23, std::string simmode = "Mc")//defaultF = 0 is BESII, defaultF = 1 is BESI
{
  ...
  {
    spectra[i] = Form("CEPT_O2_NoYSpec_nov2_phieff_phiTPC_20240809_2_ysig1000_rho0.3333_rerho0.0_imrho0.0_r0.0_i0.0_hrho%s_hrerho0.0_himrho0.0_hr0.0_hi0.0",sigmay_text[i].c_str());
  }
  ...
  for(int i_pt = 2; i_pt < 6; ++i_pt) // pt loop
  {
    ...
    for(int i = start; i < stop; i++)
    {
      ...
      h_m2D_RC_Global[i_pt-2][i] = (TH2D*) ((TH2D*) File_InPutRC[i]->Get(KEY_2D_RC_Global.c_str()))->Clone();
  ...
  TGraphAsymmErrors *g2D_Global[5][4]; 
  ...
  for(int ipt = 0; ipt < 4; ipt++)
  {
    ...
    {
      g2D[par][ipt] = new TGraphAsymmErrors();
      g2D_Global[par][ipt] = new TGraphAsymmErrors();
    ...
    for(int i = start; i < stop; i++)
    {
      ...
      fits2D[ipt][i]->SetParameter(5,h_m2D_RC[ipt][i]->GetMaximum());
      h_m2D_RC[ipt][i]->Fit(fits2D[ipt][i],"NMI");
      for(int par = 0; par < 5; par++)
      {
        if(par == 0) g2D[par][ipt]->SetPoint(i-start,sigmay[i]-1./3.,fits2D[ipt][i]->GetParameter(0)-1./3.);
        else g2D[par][ipt]->SetPoint(i-start,sigmay[i]-1./3.,fits2D[ipt][i]->GetParameter(par));
        g2D[par][ipt]->SetPointError(i-start,0.0,0.0,fits2D[ipt][i]->GetParError(par),fits2D[ipt][i]->GetParError(par));
      }

      fits2D_Global[ipt][i] = new TF2(Form("fit2D_%d_%d",ipt,i),SpinDensity2Dcos,-1.0,1.0,-TMath::Pi(),TMath::Pi(),6);
      fits2D_Global[ipt][i]->SetParameter(0,1./3.);
      fits2D_Global[ipt][i]->SetParLimits(0,0.0,1.0);
      fits2D_Global[ipt][i]->SetParameter(5,h_m2D_RC_Global[ipt][i]->GetMaximum());
      h_m2D_RC_Global[ipt][i]->Fit(fits2D_Global[ipt][i],"NMI");
      for(int par = 0; par < 5; par++)
      {
        if(par == 0) g2D_Global[par][ipt]->SetPoint(i-start,sigmay[i]-1./3.,fits2D_Global[ipt][i]->GetParameter(0)-1./3.);
        else g2D_Global[par][ipt]->SetPoint(i-start,sigmay[i]-1./3.,fits2D_Global[ipt][i]->GetParameter(par));
        g2D_Global[par][ipt]->SetPointError(i-start,0.0,0.0,fits2D_Global[ipt][i]->GetParError(par),fits2D_Global[ipt][i]->GetParError(par));
      }
