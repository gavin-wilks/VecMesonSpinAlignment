...
void calSpinAlignmentSysPhiY_PolySys_Signed(int energy = 4, int pid = 0, int year = 0, bool random3D = false, int etaQA = 9, int ptQA = 1, int centQA = 2, int dcaQA = 0, int nsigQA = 0, int normQA = 0, int sigQA = 0, int order = 2, std::string etamode = "eta1_eta1")
{
  std::string EP[2] = {"","2nd"};
  ...
  string inputfile = Form("../output/AuAu%s/%s/InvMassSubBgEta_%s_Signed.root",vmsa::mBeamEnergy[energy].c_str(),vmsa::mPID[pid].c_str(),etamode.c_str());
  if(order == 1) inputfile = Form("../output/AuAu%s/%s/InvMassSubBgEta_%s_Signed_FirstOrder.root",vmsa::mBeamEnergy[energy].c_str(),vmsa::mPID[pid].c_str(),etamode.c_str());
            ...
            for(int i_sigma = vmsa::Sig_start; i_sigma < vmsa::Sig_stop; ++i_sigma)
            {
              for(int i_method = vmsa::Method_start; i_method < vmsa::Method_stop; ++i_method)
              {
                for(int i_poly = 0; i_poly < 3; i_poly++)
                {
          	    ...
          	    string KEY_counts = Form("eta_%d_pt_%d_Centrality_%d_%s_Dca_%d_Sig_%d_%s_Norm_%d_Sigma_%d_%s_Poly%d",i_eta,i_pt,i_cent,EP[order-1].c_str(),i_dca,i_sig,vmsa::mPID[pid].c_str(),i_norm,i_sigma,vmsa::mInteMethod[i_method].c_str(),i_poly+1);
                    h_mCounts[KEY_counts] = new TH1F(KEY_counts.c_str(),KEY_counts.c_str(),14,-1.0,1.0);
		      ...
		      TF1 *f_bg;
		        ...
		        if(i_poly >= 2) f_bg->SetParError(3,1.0);
                      ...
                      float bin_center = 1/14.0+(i_theta-7)/7.0;
                      if(i_method == 0)
                      {
...
#if _PlotQA_
  string outputname = Form("./figures/%s/%s/rapiditystudy/allThetaYields%s_%s_Order%d_%s_RapidityDependence_signedcosthetastar.pdf",vmsa::mPID[pid].c_str(),vmsa::mBeamEnergy[energy].c_str(),vmsa::mPID[pid].c_str(),vmsa::mBeamEnergy[energy].c_str(),order,etamode.c_str());
  ...
  for(int i_cent = vmsa::Cent_start; i_cent < vmsa::cent_rebin_total; ++i_cent) // Centrality loop
  {
    ...
    {    
      for(int i_eta = vmsa::eta_start; i_eta < vmsa::eta_stop; i_eta++) // Rapidity bin loop
      {
        c_diff->Clear();
        c_diff->Divide(3,3);
        for(int i_theta = 0; i_theta < 16; ++i_theta)
        {
          c_diff->cd(i_theta+1);
          c_diff->cd(i_theta+1)->SetLeftMargin(0.15);
          c_diff->cd(i_theta+1)->SetBottomMargin(0.15);
          ...
          if(i_theta < 14)
          {
            ...
            h_mMass[KEY_QA]->GetXaxis()->SetLabelSize(0.03);
            h_mMass[KEY_QA]->GetXaxis()->SetTitle("M(K^{+},K^{-})");
            h_mMass[KEY_QA]->SetTitle(Form("%.2f<p_{T}<%.2f, %.2f<y<%.2f, %d/7<|cos(#theta*)|<%d/7, cent(%d-%d)",vmsa::pt_low_y[energy][i_pt],vmsa::pt_up_y[energy][i_pt],vmsa::eta_bin[i_eta],vmsa::eta_bin[i_eta+1],i_theta-7,i_theta-6,centlow[i_cent],centhigh[i_cent]));
            h_mMass[KEY_QA]->GetXaxis()->SetTitleSize(0.05);
            h_mMass[KEY_QA]->GetXaxis()->SetTitleOffset(1.2);
            h_mMass[KEY_QA]->GetXaxis()->CenterTitle();
          ...
          if(i_theta == 14)
          {
            ...
            string KEY_InteTheta_QA_Poly = Form("eta_%d_pt_%d_Centrality_%d_%s_Dca_%d_Sig_%d_%s_Norm_%d_Poly3",i_eta,i_pt,i_cent,EP[order-1].c_str(),vmsa::Dca_start,vmsa::nSigKaon_start,vmsa::mPID[pid].c_str(),vmsa::Norm_QA);
            h_mMass_InteTheta[KEY_InteTheta_QA]->SetTitle("");
            h_mMass_InteTheta[KEY_InteTheta_QA]->GetXaxis()->SetNdivisions(505,'N');
            h_mMass_InteTheta[KEY_InteTheta_QA]->GetXaxis()->SetLabelSize(0.03);
            h_mMass_InteTheta[KEY_InteTheta_QA]->GetXaxis()->SetTitle("M(K^{+},K^{-})");
            h_mMass_InteTheta[KEY_InteTheta_QA]->SetTitle(Form("%.2f<p_{T}<%.2f, %.2f<y<%.2f, -1<|cos(#theta*)|<1, Cent(%d-%d)",vmsa::pt_low_y[energy][i_pt],vmsa::pt_up_y[energy][i_pt],vmsa::eta_bin[i_eta],vmsa::eta_bin[i_eta+1],centlow[i_cent],centhigh[i_cent]));
            h_mMass_InteTheta[KEY_InteTheta_QA]->GetXaxis()->SetTitleSize(0.05);
            h_mMass_InteTheta[KEY_InteTheta_QA]->GetXaxis()->SetTitleOffset(1.2);
            h_mMass_InteTheta[KEY_InteTheta_QA]->GetXaxis()->CenterTitle();
        ...
        for(int i_theta = vmsa::CTS_start; i_theta < 14; ++i_theta)
        {
          c_diff->cd(i_theta+1);
          string KEY_QA = Form("eta_%d_pt_%d_Centrality_%d_CosThetaStar_%d_%s_Dca_%d_Sig_%d_%s_Norm_%d",i_eta,i_pt,i_cent,i_theta,EP[order-1].c_str(),vmsa::Dca_start,vmsa::nSigKaon_start,vmsa::mPID[pid].c_str(),vmsa::Norm_QA);
          string KEY_QA_Poly = Form("eta_%d_pt_%d_Centrality_%d_CosThetaStar_%d_%s_Dca_%d_Sig_%d_%s_Norm_%d_Poly3",i_eta,i_pt,i_cent,i_theta,EP[order-1].c_str(),vmsa::Dca_start,vmsa::nSigKaon_start,vmsa::mPID[pid].c_str(),vmsa::Norm_QA);
          TF1 *f_bw = new TF1("f_bw",Poly3BreitWigner,vmsa::BW_Start[pid],vmsa::BW_Stop[pid],7);
          ...
          f_bw->SetParameter(5,Par[KEY_QA_Poly][5]);
          f_bw->SetParameter(6,Par[KEY_QA_Poly][6]);
          ...
          f_bg->SetParameter(2,Par[KEY_QA_Poly][5]);
          f_bg->SetParameter(3,Par[KEY_QA_Poly][6]);
        ...
        for(int i_sigma = vmsa::Sig_start; i_sigma < /*vmsa::Sig_stop*/1; ++i_sigma)
        {
          ...
          {
            string KEY_counts_QA = Form("eta_%d_pt_%d_Centrality_%d_%s_Dca_%d_Sig_%d_%s_Norm_%d_Sigma_%d_%s_Poly3",i_eta,i_pt,i_cent,EP[order-1].c_str(),vmsa::Dca_start,vmsa::nSigKaon_start,vmsa::mPID[pid].c_str(),vmsa::Norm_QA,i_sigma,vmsa::mInteMethod[i_method].c_str());
...
//#endif
  ...
  string outputfile = Form("../output/AuAu%s/%s/RawRhoEtaSys_%s_Poly_Signed.root",vmsa::mBeamEnergy[energy].c_str(),vmsa::mPID[pid].c_str(),etamode.c_str());
  if(order == 1) outputfile = Form("../output/AuAu%s/%s/RawRhoEtaSys_%s_Poly_Signed_FirstOrder.root",vmsa::mBeamEnergy[energy].c_str(),vmsa::mPID[pid].c_str(),etamode.c_str());
