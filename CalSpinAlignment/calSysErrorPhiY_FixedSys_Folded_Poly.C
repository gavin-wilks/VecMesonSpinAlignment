...
void calSysErrorPhiY_FixedSys_Folded_Poly(Int_t energy = 4, Int_t pid = 0, string correction = "Raw", bool random3D = false, int ptQA = 0, int centQA = 2, int order = 2, std::string etamode = "eta1_eta1", std::string fileoption = "", int yspectra = -1)//defaultF = 0 is BESII, defaultF = 1 is BESI
{
  ...
  string inputfileHframe = Form("../output/AuAu%s/%s/RawRhoEtaSys_%s_Poly.root",vmsa::mBeamEnergy[energy].c_str(),vmsa::mPID[pid].c_str(),etamode.c_str());
  ...
  string inputfile = Form("../output/AuAu%s/%s/%s/%sRhoEtaSys_%s_Poly%s.root",vmsa::mBeamEnergy[energy].c_str(),vmsa::mPID[pid].c_str(),fileoption.c_str(),correction.c_str(),etamode.c_str(),spectra.c_str());
...
//  PlotLine(0.0,5.0,1.0/3.0,1.0/3.0,1,2,2);
  ...
  c_rho_SysError->SaveAs(Form("./figures/%s/%s/rapiditystudy/finaloutputrho_rapiditydependence_%s_%s_%s_order%d_%s_PolySys.pdf",vmsa::mPID[pid].c_str(),vmsa::mBeamEnergy[energy].c_str(),correction.c_str(),vmsa::mPID[pid].c_str(),vmsa::mBeamEnergy[energy].c_str(),order,etamode.c_str()));  
  ...
  string outputname = Form("./figures/%s/%s/rapiditystudy/finaloutputrho_rapiditydependence_%s_ptandcentbins_%s_%s_order%d_%s_PolySys.pdf",vmsa::mPID[pid].c_str(),vmsa::mBeamEnergy[energy].c_str(),correction.c_str(),vmsa::mPID[pid].c_str(),vmsa::mBeamEnergy[energy].c_str(),order,etamode.c_str());
  ...
  string OutPutFile = Form("../output/AuAu%s/%s/RhoEta_%sSysErrors_%s_PolySys%s.root",vmsa::mBeamEnergy[energy].c_str(),vmsa::mPID[pid].c_str(),correction.c_str(),etamode.c_str(),spectra.c_str());
