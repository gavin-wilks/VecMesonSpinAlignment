...
void plotKaonTTrees_Compare_TPCOnly(int energy = 4, bool datarcweight = false) 
{
    ...
    TFile *inputweightsptyphi = TFile::Open(Form("../anapicodst_phiembed/StRoot/Utility/pt_y_phi_weights_v2_FunctionEmbed.root"));
    ...
    // INITIALIZE TTREES 
    TChain *mKaonTreeSE = new TChain("kaontree");
    mKaonTreeSE->Add("/gpfs01/star/pwg/gwilks3/VectorMesonSpinAlignment/AuAu19GeV_2019/OutPut/SpinAlignmentYields/PhiSE_EtaMode0_20240910_cent2060_kaonspectra_TTrees_TPCOnly_Retry/*");

    //TFile *mKaonFileSE = TFile::Open("../data/Yields_Phi_SE_19GeV_20240910_cent2060_kaonspectra_TPConly.root");
    ...
    TChain *mKaonTreeME = new TChain("kaontree");
    mKaonTreeME->Add("/gpfs01/star/pwg/gwilks3/VectorMesonSpinAlignment/AuAu19GeV_2019/OutPut/SpinAlignmentYields/PhiME_EtaMode0_20240910_cent2060_kaonspectra_TTrees_TPCOnly_Retry/*");
    //TFile *mKaonFileME = TFile::Open("../data/Yields_Phi_ME_19GeV_20240910_cent2060_kaonspectra_TPConly.root");
    ...
    TLorentzVector *mLMeson = NULL;
    TLorentzVector *mLKp = NULL;
    TLorentzVector *mLKm = NULL; 
    Bool_t mRcExists;
    TLorentzVector *mLRcKp = NULL;
    TLorentzVector *mLRcKm = NULL;
    ...
    TFile *mKaonFileEmbed = TFile::Open("/gpfs01/star/pwg/gwilks3/VectorMesonSpinAlignment/AuAu19GeV_2019/OutPut/Embedding/Phi/PhiEmbedding_kaontrees_notofmatchreq_EP.root");
    ...
    for (Long64_t i = 0; i < nentries; i++)
    {
      ...
      Double_t values[4] = {mPtSE,mRapiditySE,mEtaSE,mPhiSE};
      ptyetaphibasic[0][mChargeSE]->Fill(values,mWeightSE);
    ...
    for (Long64_t i = 0; i < nentriesME; i++)
    {
      ...
      Double_t values[4] = {mPtME,mRapidityME,mEtaME,mPhiME};
      ptyetaphibasic[1][mChargeME]->Fill(values,mWeightME);
    ...
    ct->SaveAs(Form("figures/KaonTTreesTPCOnly/PhiMeson_DataRCAndRatio.pdf"));
    ...
    for (Long64_t i = 0; i < nentriesEmbed; i++)
    {
      ...
      if(mRcExists && mPassTpc && mPassTof && mPassM2 && mPassNsig)    
      {
        ...
        Double_t valuesP[4] = {mRcPtP,mRcRapidityP,mRcEtaP,mRcPhiP};
        ptyetaphibasic[3][0]->Fill(valuesP,mWeight*PhiWeight*DataRcWeightVal);
        Double_t valuesM[4] = {mRcPtM,mRcRapidityM,mRcEtaM,mRcPhiM};
        ptyetaphibasic[3][1]->Fill(valuesM,mWeight*PhiWeight*DataRcWeightVal);
    ...
    ct->SaveAs(Form("figures/KaonTTreesTPCOnly/PhiMeson_DataRCAndRatio_After.pdf"));

    const int nset = 6;
    //                      y    y   y   pT
    Double_t axis[nset] = {   1,   1,  1,   0,   0,   0   };
    Double_t low[nset]  = {-1.5,-0.5,0.5, 1.0, 0.0, 2.0   };
    Double_t high[nset] = {-0.5, 0.5,1.5, 2.0, 0.7, ptmax };
    ...
    for(int ic = 0; ic < 3; ic++)
    {
      for(int im = 0; im < 5; im++)
      {
        ...
        c1->SaveAs(Form("figures/KaonTTreesTPCOnly/K%s_%s.pdf",charge[ic].c_str(),mixinglabel[im].c_str()));
      }
    }
    ...
    {
      cr->cd(i+1);
      cr->cd(i+1)->SetLeftMargin(0.15);
      cr->cd(i+1)->SetRightMargin(0.15);
      cr->cd(i+1)->SetBottomMargin(0.15);
    ...
    for(int im = 0; im < 2; im++)
    {
      for(int ic = 0; ic < 2; ic++)
      {
        ...
        cr->SaveAs(Form("figures/KaonTTreesTPCOnly/K%s_%s.pdf",charge[ic].c_str(),ratiofile[im].c_str()));
        
      }
    }

    
    string settitle[nset+1] = {"-1.5<y<-0.5","-0.5<y<0.5","0.5<y<1.5","0.0<p_{T}<0.7 GeV/c","1.0<p_{T}<2.0 GeV/c","p_{T}>2.0 GeV/c","Ignore Major #eta,#phi Deficiencies"};
    string setfile[nset+1]  = {"n1p5yn0p5"  ,"n0p5y0p5"  ,"0p5y1p5"  ,"0pt0p7",             "1p0pt2p0",           "pt2p0"          ,"IgnoreEtaPhiDef" };
    for(int is = 0; is < nset+1; is++)
    {
      for(int im = 0; im < 2; im++)
      {
    ...
    for(int ic = 0; ic < 2; ic++)
    {
      ...
      c43->SaveAs(Form("figures/KaonTTreesTPCOnly/K%s_1DRatios.pdf",charge[ic].c_str()));
    }
