...
void subBackGround_PhiStar_BigBin(int energy = 4, int pid = 0, int year = 0, string date = "20240216", bool random3D = false, int order = 2, string etamode = "eta1_eta1")
{
  std::string EP[2] = {"","2nd"};
  ...
  for(Int_t i_pt = 0; i_pt < vmsa::pt_total; i_pt++) // pt bin 
  {
	...
	for(Int_t i_dca = vmsa::Dca_start; i_dca < vmsa::Dca_stop; i_dca++)
	{
	  for(Int_t i_sig = vmsa::nSigKaon_start; i_sig < vmsa::nSigKaon_stop; i_sig++)
	  {
	    ...
	    string KEY_InPutSE = Form("pt_%d_Centrality_%d_Cos2PhiStarPhi_%d_%s_Dca_%d_Sig_%d_%s_SE",i_pt,i_cent,i_theta,EP[order-1].c_str(),i_dca,i_sig,vmsa::mPID[pid].c_str());
            h_mInPut_SE[KEY_InPutSE] = (TH1F*)File_SE->Get(KEY_InPutSE.c_str())->Clone(); 

	    string KEY_InPutME = Form("pt_%d_Centrality_%d_Cos2PhiStarPhi_%d_%s_Dca_%d_Sig_%d_%s_ME",i_pt,i_cent,i_theta,EP[order-1].c_str(),i_dca,i_sig,vmsa::mPID[pid].c_str());
	    ...
	    {
	      string KEY_SE = Form("pt_%d_Centrality_%d_Cos2PhiStarPhi_%d_%s_Dca_%d_Sig_%d_%s_Norm_%d_SE",i_pt,i_cent,i_theta,EP[order-1].c_str(),i_dca,i_sig,vmsa::mPID[pid].c_str(),i_norm);
	      h_mMass_SE[KEY_SE] = (TH1F*)h_mInPut_SE[KEY_InPutSE]->Clone(KEY_SE.c_str());
              h_mMass_SE[KEY_SE]->Rebin(2);
	      string KEY_ME = Form("pt_%d_Centrality_%d_Cos2PhiStarPhi_%d_%s_Dca_%d_Sig_%d_%s_Norm_%d_ME",i_pt,i_cent,i_theta,EP[order-1].c_str(),i_dca,i_sig,vmsa::mPID[pid].c_str(),i_norm);
	      h_mMass_ME[KEY_ME] = (TH1F*)h_mInPut_ME[KEY_InPutME]->Clone(KEY_ME.c_str());
              h_mMass_ME[KEY_ME]->Rebin(2);
	      string KEY_SM = Form("pt_%d_Centrality_%d_Cos2PhiStarPhi_%d_%s_Dca_%d_Sig_%d_%s_Norm_%d_SM",i_pt,i_cent,i_theta,EP[order-1].c_str(),i_dca,i_sig,vmsa::mPID[pid].c_str(),i_norm);
		...
		float Inte_ME = h_mMass_ME[KEY_ME]->Integral(Norm_bin_start,Norm_bin_stop);
                ...
                cout << "Inte_SE/Inte_ME = " << Inte_SE/Inte_ME  << endl;

		if(Inte_ME != 0.0) h_mMass_ME[KEY_ME]->Scale(Inte_SE/Inte_ME);
		h_mMass_SM[KEY_SM]->Add(h_mMass_ME[KEY_ME],-1.0);

                if(i_dca == 0 && i_sig == 0 && i_norm == 0) 
                {
                  if(i_pt >= 5 && i_pt <= 16)
                  {
                    cout << "pt = " << vmsa::ptRawStart[i_pt] << "," << vmsa::ptRawStop[i_pt] << endl;
                    mNorm[i_pt-5][i_theta] = Inte_SE/Inte_ME;
                  }
                }
	      }
	      else
  ...
  std::ofstream normFile(Form("phi_yieldextraction_%s.h",vmsa::mBeamEnergy[energy].c_str()));

  normFile << "const float phi_norm[12][10] = {";
  for(int ipt = 0; ipt < 12; ipt++)
  {
    normFile << "{";
    for(int iphi = 0; iphi < 10; iphi++)
    {
      if(iphi != 9) normFile << mNorm[ipt][iphi] << ",";
      if(iphi == 9) normFile << mNorm[ipt][iphi] << "}";
    }
    if(ipt != 11) normFile << "," << endl;
    if(ipt == 11) normFile << "}" << endl;
  }
  ...
  for(int i_pt = 0; i_pt < vmsa::pt_total; i_pt++)
  {
    ...
    string KEY_SE_QA = Form("pt_%d_Centrality_%d_Cos2PhiStarPhi_%d_%s_Dca_%d_Sig_%d_%s_Norm_%d_SE",i_pt,9,vmsa::CTS_start,EP[order-1].c_str(),vmsa::Dca_start,vmsa::nSigKaon_start,vmsa::mPID[pid].c_str(),vmsa::Norm_QA);
    ...
    string KEY_ME_QA = Form("pt_%d_Centrality_%d_Cos2PhiStarPhi_%d_%s_Dca_%d_Sig_%d_%s_Norm_%d_ME",i_pt,9,vmsa::CTS_start,EP[order-1].c_str(),vmsa::Dca_start,vmsa::nSigKaon_start,vmsa::mPID[pid].c_str(),vmsa::Norm_QA);
    ...
    string KEY_SM_QA = Form("pt_%d_Centrality_%d_Cos2PhiStarPhi_%d_%s_Dca_%d_Sig_%d_%s_Norm_%d_SM",i_pt,9,vmsa::CTS_start,EP[order-1].c_str(),vmsa::Dca_start,vmsa::nSigKaon_start,vmsa::mPID[pid].c_str(),vmsa::Norm_QA);
    ...
    c_peak->SaveAs(Form("figures/%s/%s/pTstudy/Cos2PhiStarPhi_CloseUpMixedEvent_%s_Order%d_Cent0_pt%d.pdf",vmsa::mPID[pid].c_str(),vmsa::mBeamEnergy[energy].c_str(),vmsa::mBeamEnergy[energy].c_str(),order,i_pt));
  }
  ...
  string outputname = Form("figures/%s/%s/pTstudy/Cos2PhiStarPhi_InvMassDistributions_%s_Order%d_Cent9.pdf",vmsa::mPID[pid].c_str(),vmsa::mBeamEnergy[energy].c_str(),vmsa::mBeamEnergy[energy].c_str(),order);
  ...
  for(int itheta = 0; itheta < 10; itheta++)
  {
    for(int i_pt = vmsa::pt_start; i_pt < vmsa::pt_stop; i_pt++) // pt loop
    {
      ...
      string KEY_SE_QA = Form("pt_%d_Centrality_%d_Cos2PhiStarPhi_%d_%s_Dca_%d_Sig_%d_%s_Norm_%d_SE",i_pt,9,itheta,EP[order-1].c_str(),vmsa::Dca_start,vmsa::nSigKaon_start,vmsa::mPID[pid].c_str(),vmsa::Norm_QA);
      h_mMass_SE[KEY_SE_QA]->GetYaxis()->SetRangeUser(-0.1*h_mMass_SE[KEY_SE_QA]->GetMaximum(),1.1*h_mMass_SE[KEY_SE_QA]->GetMaximum());
      h_mMass_SE[KEY_SE_QA]->SetTitle(Form("%d/5<cos(2#phi*-2#phi)<%d/5",itheta-5,itheta-4));
      h_mMass_SE[KEY_SE_QA]->DrawCopy();

      string KEY_ME_QA = Form("pt_%d_Centrality_%d_Cos2PhiStarPhi_%d_%s_Dca_%d_Sig_%d_%s_Norm_%d_ME",i_pt,9,itheta,EP[order-1].c_str(),vmsa::Dca_start,vmsa::nSigKaon_start,vmsa::mPID[pid].c_str(),vmsa::Norm_QA);
      h_mMass_ME[KEY_ME_QA]->SetLineColor(2);
      h_mMass_ME[KEY_ME_QA]->SetFillColor(2);
      h_mMass_ME[KEY_ME_QA]->SetFillStyle(3002);
      h_mMass_ME[KEY_ME_QA]->DrawCopy("h same");

      string KEY_SM_QA = Form("pt_%d_Centrality_%d_Cos2PhiStarPhi_%d_%s_Dca_%d_Sig_%d_%s_Norm_%d_SM",i_pt,9,itheta,EP[order-1].c_str(),vmsa::Dca_start,vmsa::nSigKaon_start,vmsa::mPID[pid].c_str(),vmsa::Norm_QA);
      h_mMass_SM[KEY_SM_QA]->SetLineColor(4);
      h_mMass_SM[KEY_SM_QA]->SetFillColor(4);
      h_mMass_SM[KEY_SM_QA]->SetFillStyle(3004);
      h_mMass_SM[KEY_SM_QA]->DrawCopy("h same");
  ...
  for(int i_cent = 9; i_cent < vmsa::Cent_stop; i_cent++) // Centrality loop
  {
	...
	for(Int_t i_sig = vmsa::nSigKaon_start; i_sig < vmsa::nSigKaon_stop; i_sig++)
	{
	  ...
	  {
	      int pt_bin = 0;
              string KEY = Form("pt_%d_Centrality_%d_Cos2PhiStarPhi_%d_%s_Dca_%d_Sig_%d_%s_Norm_%d",pt_bin,i_cent,i_theta,EP[order-1].c_str(),i_dca,i_sig,vmsa::mPID[pid].c_str(),i_norm);
              cout << "For pt_bin = " << pt_bin << endl;
	      for(int i_pt = 5/*vmsa::pt_rebin_start[energy][pt_bin]*/; i_pt <= 16/*vmsa::pt_rebin_stop[energy][pt_bin]*/; i_pt++)
	      {
                cout << "i_pt = " << i_pt << ", ptrange = " << vmsa::pt_bin[i_pt] << "," << vmsa::pt_bin[i_pt+1] << endl;
		string KEY_SM = Form("pt_%d_Centrality_%d_Cos2PhiStarPhi_%d_%s_Dca_%d_Sig_%d_%s_Norm_%d_SM",i_pt,i_cent,i_theta,EP[order-1].c_str(),i_dca,i_sig,vmsa::mPID[pid].c_str(),i_norm);
		// cout << "KEY= " << KEY.c_str() << ", KEY_SM = " << KEY_SM.c_str() << endl;
		if(i_pt == 5/*vmsa::pt_rebin_start[energy][pt_bin]*/)
		{
                  cout << "Initialize with " << i_pt << endl;
  ...
  //outputname = Form("figures/%s/%s/pTstudy/Cos2PhiStarPhi_InvMassBackgroundSubtracted_Rebinned_%s_Order%d_Cent9.pdf",vmsa::mPID[pid].c_str(),vmsa::mBeamEnergy[energy].c_str(),vmsa::mBeamEnergy[energy].c_str(),order);
  ...
  //    string KEY_QA = Form("pt_%d_Centrality_%d_Cos2PhiStarPhi_%d_%s_Dca_%d_Sig_%d_%s_Norm_%d",i_pt,9,itheta,EP[order-1].c_str(),vmsa::Dca_start,vmsa::nSigKaon_start,vmsa::mPID[pid].c_str(),vmsa::Norm_QA);
...
//      string KEY_QA = Form("pt_%d_Centrality_%d_Cos2PhiStarPhi_%d_%s_Dca_%d_Sig_%d_%s_Norm_%d",i_pt,9,vmsa::CTS_start,EP[order-1].c_str(),vmsa::Dca_start,vmsa::nSigKaon_start,vmsa::mPID[pid].c_str(),vmsa::Norm_QA);
...
//#endif
  ...
  string outputfile = Form("../output/AuAu%s/%s/BigBin_Cos2PhiStarPhi_InvMassSubBg_%s.root",vmsa::mBeamEnergy[energy].c_str(),vmsa::mPID[pid].c_str(),etamode.c_str());
  //string outputfile = Form("../output/AuAu%s/%s/InvMassSubBg.root",vmsa::mBeamEnergy[energy].c_str(),vmsa::mPID[pid].c_str());
  if(order == 1) outputfile = Form("../output/AuAu%s/%s/Cos2PhiStarPhi_InvMassSubBg_%s_FirstOrder.root",vmsa::mBeamEnergy[energy].c_str(),vmsa::mPID[pid].c_str(),etamode.c_str());
