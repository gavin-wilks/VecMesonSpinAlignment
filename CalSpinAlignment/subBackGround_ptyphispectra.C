...
void subBackGround_ptyphispectra(int energy = 4, int pid = 0, int year = 0, string date = "20240912", bool random3D = false, int order = 2, string etamode = "eta1_eta1")
{
  ...
  string InPutFile_SE = Form("../data/Yields_Phi_SE_%s_%s_eta1_eta1_cent2060_phispectra_ptyphi.root",vmsa::mBeamEnergy[energy].c_str(),date.c_str());
  ...
  string InPutFile_ME = Form("../data/Yields_Phi_ME_%s_%s_eta1_eta1_cent2060_phispectra_ptyphi.root",vmsa::mBeamEnergy[energy].c_str(),date.c_str());
  ...
  float scalingratio[vmsa::phirebinpttotal][vmsa::phirebinytotal][vmsa::phirebinphitotal] = {0.0};
  ...
  for(Int_t i_pt = 0; i_pt < vmsa::pt_total; i_pt++) // pt bin 
  {
    for(Int_t i_cent = 9/*vmsa::Cent_start*/; i_cent < vmsa::Cent_stop; i_cent++) // centrality bin
    {
      ...
      {
        for(Int_t i_phi = 0; i_phi < 24; i_phi++) // phi-psi bin
        {
	  for(Int_t i_dca = vmsa::Dca_start; i_dca < 1/*vmsa::Dca_stop*/; i_dca++)
	  {
	    for(Int_t i_sig = vmsa::nSigKaon_start; i_sig < 1/*vmsa::nSigKaon_stop*/; i_sig++)
	    {
	      ...
	      string KEY_InPutSE = Form("pt_%d_eta_%d_phi_%d_Centrality_%d_%s_Dca_%d_Sig_%d_%s_SE",i_pt,i_eta,i_phi,i_cent,EP[order-1].c_str(),i_dca,i_sig,vmsa::mPID[pid].c_str());
              h_mInPut_SE[KEY_InPutSE] = (TH1F*)File_SE->Get(KEY_InPutSE.c_str())->Clone(); 
              h_mInPut_SE[KEY_InPutSE]->Rebin(2);

	      string KEY_InPutME = Form("pt_%d_eta_%d_phi_%d_Centrality_%d_%s_Dca_%d_Sig_%d_%s_ME",i_pt,i_eta,i_phi,i_cent,EP[order-1].c_str(),i_dca,i_sig,vmsa::mPID[pid].c_str());
	      ...
	      {
	        string KEY_SE = Form("pt_%d_eta_%d_phi_%d_Centrality_%d_%s_Dca_%d_Sig_%d_%s_Norm_%d_SE",i_pt,i_eta,i_phi,i_cent,EP[order-1].c_str(),i_dca,i_sig,vmsa::mPID[pid].c_str(),i_norm);
	        h_mMass_SE[KEY_SE] = (TH1F*)h_mInPut_SE[KEY_InPutSE]->Clone(KEY_SE.c_str());
	        string KEY_ME = Form("pt_%d_eta_%d_phi_%d_Centrality_%d_%s_Dca_%d_Sig_%d_%s_Norm_%d_ME",i_pt,i_eta,i_phi,i_cent,EP[order-1].c_str(),i_dca,i_sig,vmsa::mPID[pid].c_str(),i_norm);
  ...
  for(Int_t i_cent = 9/*vmsa::Cent_start*/; i_cent < vmsa::Cent_stop; i_cent++) // centrality bin
  {
    for(Int_t i_dca = vmsa::Dca_start; i_dca < 1/*vmsa::Dca_stop*/; i_dca++)
    {
      for(Int_t i_sig = vmsa::nSigKaon_start; i_sig < 1/*vmsa::nSigKaon_stop*/; i_sig++)
      {
        for(int i_norm = vmsa::Norm_start; i_norm < 1/*vmsa::Norm_stop*/; ++i_norm)
        {
          for(int i_phi = 0; i_phi < 24; i_phi++)
          {
            for(int ptbin = 0; ptbin < vmsa::phirebinpttotal; ptbin++)
            {
              for(Int_t i_pt = vmsa::phirebinpt[ptbin]; i_pt < vmsa::phirebinpt[ptbin+1]; i_pt++) // pt bin 
              {
                for(int etabin = 0; etabin < vmsa::phirebinytotal; etabin++)
                {
                  for(Int_t i_eta = vmsa::phirebiny[etabin]; i_eta < vmsa::phirebiny[etabin+1]; i_eta++) // phi-psi bin
                  {
                    string KEY_SE = Form("pt_%d_eta_%d_phi_%d_Centrality_%d_%s_Dca_%d_Sig_%d_%s_Norm_%d_SE",i_pt,i_eta,i_phi,i_cent,EP[order-1].c_str(),i_dca,i_sig,vmsa::mPID[pid].c_str(),i_norm);
                    //cout << KEY_SE << endl;
                    string KEY_SE_rebin = Form("pt_%d_eta_%d_phi_%d_Centrality_%d_%s_Dca_%d_Sig_%d_%s_Norm_%d_SE_Rebin",ptbin,etabin,i_phi,i_cent,EP[order-1].c_str(),i_dca,i_sig,vmsa::mPID[pid].c_str(),i_norm);
                    //cout << KEY_SE_rebin << endl;
                    string KEY_ME = Form("pt_%d_eta_%d_phi_%d_Centrality_%d_%s_Dca_%d_Sig_%d_%s_Norm_%d_ME",i_pt,i_eta,i_phi,i_cent,EP[order-1].c_str(),i_dca,i_sig,vmsa::mPID[pid].c_str(),i_norm);
                    //cout << KEY_ME << endl;
                    string KEY_ME_rebin = Form("pt_%d_eta_%d_phi_%d_Centrality_%d_%s_Dca_%d_Sig_%d_%s_Norm_%d_ME_Rebin",ptbin,etabin,i_phi,i_cent,EP[order-1].c_str(),i_dca,i_sig,vmsa::mPID[pid].c_str(),i_norm);
                    //cout << KEY_ME_rebin << endl;
                    if(i_pt == vmsa::phirebinpt[ptbin] && i_eta == vmsa::phirebiny[etabin])
                    { 
        ...
        for(int i_norm = vmsa::Norm_start; i_norm < 1/*vmsa::Norm_stop*/; ++i_norm)
        {
          for(int i_phi = 0; i_phi < 24; i_phi++)
          {
            for(int ptbin = 0; ptbin < vmsa::phirebinpttotal; ptbin++)
            {
              for(int etabin = 0; etabin < vmsa::phirebinytotal; etabin++)
              {
                string KEY_SM = Form("pt_%d_eta_%d_phi_%d_Centrality_%d_%s_Dca_%d_Sig_%d_%s_Norm_%d_SM_Rebin",ptbin,etabin,i_phi,i_cent,EP[order-1].c_str(),i_dca,i_sig,vmsa::mPID[pid].c_str(),i_norm);
                //cout << KEY_SM << endl;
                string KEY_SE = Form("pt_%d_eta_%d_phi_%d_Centrality_%d_%s_Dca_%d_Sig_%d_%s_Norm_%d_SE_Rebin",ptbin,etabin,i_phi,i_cent,EP[order-1].c_str(),i_dca,i_sig,vmsa::mPID[pid].c_str(),i_norm);
                //cout << KEY_SE << endl;
                string KEY_ME = Form("pt_%d_eta_%d_phi_%d_Centrality_%d_%s_Dca_%d_Sig_%d_%s_Norm_%d_ME_Rebin",ptbin,etabin,i_phi,i_cent,EP[order-1].c_str(),i_dca,i_sig,vmsa::mPID[pid].c_str(),i_norm);
                //cout << KEY_ME << endl;
	        ...
	        if(i_norm < 2)
	        {
	          ...
	          h_mMass_SM[KEY_SM]->Add(h_mMass_ME[KEY_ME],-1.0);
                  ...
                  scalingratio[ptbin][etabin][i_phi] = Inte_SE/Inte_ME;
	        }
	        else
	        {
	          float Inte_SE = 0.0;
  ...
  cout << endl << "const float phiscalingME[phirebinpttotal][phirebinytotal][phirebinphitotal] = {";
  for(int ptbin = 0; ptbin < vmsa::phirebinpttotal; ptbin++)
  {
    cout << "{";
    for(int etabin = 0; etabin < vmsa::phirebinytotal; etabin++)
    {
      cout << "{";
      for(int i_phi = 0; i_phi < vmsa::phirebinphitotal; i_phi++)
      {
        if(i_phi < vmsa::phirebinphitotal-1) cout << scalingratio[ptbin][etabin][i_phi] << ",";
        else                             cout << scalingratio[ptbin][etabin][i_phi];
      }
      if(etabin < vmsa::phirebinytotal -1 ) cout << "}," << endl;
      else                                cout << "}";
    }
    if(ptbin < vmsa::phirebinpttotal -1 ) cout << "}," << endl;
    else                                cout << "}";
  }
  cout << "};" << endl << endl;
  ...
  string outputname = Form("figures/%s/%s/pTstudy/ptyphispectra_InvMassDistributions_%s_Order%d_Cent9.pdf",vmsa::mPID[pid].c_str(),vmsa::mBeamEnergy[energy].c_str(),vmsa::mBeamEnergy[energy].c_str(),order);
  string output_start = Form("%s[",outputname.c_str());
  
  c_pT->Print(output_start.c_str());
  for(int ieta = 0; ieta < vmsa::phirebinytotal; ieta++)
  {
    for(int i_pt = 0; i_pt < vmsa::phirebinpttotal; i_pt++) // pt loop
    {
      for(int i_phi = 0; i_phi < vmsa::phirebinphitotal; i_phi++)
      {
        c_pT->cd(i_phi+1);
        c_pT->cd(i_phi+1)->SetLeftMargin(0.15);
        c_pT->cd(i_phi+1)->SetBottomMargin(0.15);
        c_pT->cd(i_phi+1)->SetTicks(1,1);
        c_pT->cd(i_phi+1)->SetGrid(0,0);
        string KEY_SE_QA = Form("pt_%d_eta_%d_phi_%d_Centrality_%d_%s_Dca_%d_Sig_%d_%s_Norm_%d_SE_Rebin",i_pt,ieta,i_phi,9,EP[order-1].c_str(),vmsa::Dca_start,vmsa::nSigKaon_start,vmsa::mPID[pid].c_str(),vmsa::Norm_QA);
        h_mMass_SE[KEY_SE_QA]->GetYaxis()->SetRangeUser(-0.1*h_mMass_SE[KEY_SE_QA]->GetMaximum(),1.1*h_mMass_SE[KEY_SE_QA]->GetMaximum());
        h_mMass_SE[KEY_SE_QA]->SetTitle(Form("%.2f<y<%.2f, %d#pi/12<#phi<%d#pi/12",vmsa::phirebinyval[ieta],vmsa::phirebinyval[ieta+1],i_phi-12,i_phi-11));
        h_mMass_SE[KEY_SE_QA]->DrawCopy();

        string KEY_ME_QA = Form("pt_%d_eta_%d_phi_%d_Centrality_%d_%s_Dca_%d_Sig_%d_%s_Norm_%d_ME_Rebin",i_pt,ieta,i_phi,9,EP[order-1].c_str(),vmsa::Dca_start,vmsa::nSigKaon_start,vmsa::mPID[pid].c_str(),vmsa::Norm_QA);
        ...
        string KEY_SM_QA = Form("pt_%d_eta_%d_phi_%d_Centrality_%d_%s_Dca_%d_Sig_%d_%s_Norm_%d_SM_Rebin",i_pt,ieta,i_phi,9,EP[order-1].c_str(),vmsa::Dca_start,vmsa::nSigKaon_start,vmsa::mPID[pid].c_str(),vmsa::Norm_QA);
        ...
        string pT_range = Form("[%.2f,%.2f]",vmsa::phirebinptval[i_pt],vmsa::phirebinptval[i_pt+1]);
  ...
  string outputfile = Form("../output/AuAu%s/%s/ptyphispectra_InvMassSubBg_%s.root",vmsa::mBeamEnergy[energy].c_str(),vmsa::mPID[pid].c_str(),etamode.c_str());
  ...
  for(int i_pt = 0; i_pt < vmsa::phirebinpttotal; i_pt++) // pt loop
  {
	...
	{
          if( i_dca != 0 && i_sig != 0 ) continue;
	  for(int i_eta = 0; i_eta < vmsa::phirebinytotal; i_eta++) // cos(theta*) loop
	  {
            for(int i_phi = 0; i_phi < vmsa::phirebinphitotal; i_phi++)
            {
	      for(int i_norm = vmsa::Norm_start; i_norm < 1/*vmsa::Norm_stop*/; ++i_norm)
	      {
	        string KEY_SM = Form("pt_%d_eta_%d_phi_%d_Centrality_%d_%s_Dca_%d_Sig_%d_%s_Norm_%d_SM_Rebin",i_pt,i_eta,i_phi,i_cent,EP[order-1].c_str(),i_dca,i_sig,vmsa::mPID[pid].c_str(),i_norm);
	        string KEY    = Form("pt_%d_eta_%d_phi_%d_Centrality_%d_%s_Dca_%d_Sig_%d_%s_Norm_%d",i_pt,i_eta,i_phi,i_cent,EP[order-1].c_str(),i_dca,i_sig,vmsa::mPID[pid].c_str(),i_norm);
