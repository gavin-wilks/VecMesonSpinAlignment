...
void calSpinAlignmentSysPhiEtaIntegrated(int energy = 4, int pid = 0, int year = 0, bool random3D = false, int etaQA = 9, int ptQA = 1, int centQA = 2, int dcaQA = 0, int nsigQA = 0, int normQA = 0, int sigQA = 0, std::string etamode = "eta0p4")
{
      ...
      for(int i_eta = vmsa::eta_start; i_eta < vmsa::eta_stop; i_eta++) // Rapidity bin loop
      {
        ...
        for(int i_theta = 0; i_theta < 9; ++i_theta)
        {
          ...
          if(i_theta < vmsa::CTS_stop)
          {
            ...
            h_mMass[KEY_QA]->GetXaxis()->SetLabelSize(0.03);
            h_mMass[KEY_QA]->GetXaxis()->SetTitle("M(K^{+},K^{-})");
            h_mMass[KEY_QA]->SetTitle(Form("%.2f < p_{T} < %.2f    %.2f < y < %.2f",vmsa::pt_low_y[energy][i_pt],vmsa::pt_up_y[energy][i_pt],vmsa::eta_bin[i_eta],vmsa::eta_bin[i_eta+1]));
            h_mMass[KEY_QA]->GetXaxis()->SetTitleSize(0.05);
            h_mMass[KEY_QA]->GetXaxis()->SetTitleOffset(1.2);
            h_mMass[KEY_QA]->GetXaxis()->CenterTitle();
