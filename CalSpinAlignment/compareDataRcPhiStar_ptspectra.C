...
void compareDataRcPhiStar_ptspectra(Int_t energy = 4, Int_t pid = 0, Int_t i_cent = 9, string correction = "Raw", int order = 2, string etamode = "eta1_eta1", int yspectra = 76, string sim = "Embed")//defaultF = 0 is BESII, defaultF = 1 is BESI
{
  std::string filename = ""; 
  std::string spectra = "";
  ...
  h_mCountsRaw = (TH2D*) File_InPut->Get(KEY_counts.c_str());
  //h_mCounts->RebinY(2);
  ...
  TH2D *h_mCounts_RC = new TH2D("RC","RC",vmsa::rebinpttotal,vmsa::rebinptval,vmsa::rebinytotal,vmsa::rebinyval);

  double val_total[vmsa::rebinpttotal][vmsa::rebinytotal] = {0.0}; 
  double errsq_total[vmsa::rebinpttotal][vmsa::rebinytotal] = {0.0};
  for(ptbin = 0; ptbin < vmsa::rebinpttotal; ptbin++)
  {
    for(ipt = vmsa::rebinpt[ptbin]; ipt < vmsa::rebinpt[ptbin+1]; ipt++)
    {
      for(ybin = 0; ybin < vmsa::rebinytotal; ybin++)
      {
        for(iy = vmsa::rebiny[ybin]; iy < vmsa::rebiny[ybin+1]; iy++)
        {
