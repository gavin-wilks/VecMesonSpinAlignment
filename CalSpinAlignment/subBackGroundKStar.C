...
void subBackGroundKStar(int energy = 4, int pid = 2, int year = 0, string date = "20220708", bool random3D = false)
{
  TGaxis::SetMaxDigits(4);
  ROOT::Math::MinimizerOptions::SetDefaultMaxFunctionCalls(100000);

  string InPutFile_SE = Form("../data/Yields_KStar_SE_19GeV_%s.root",date.c_str());
  ...
  string InPutFile_ME = Form("../data/Yields_KStar_ME_19GeV_%s.root",date.c_str());
	...
	for(Int_t i_dca = vmsa::Dca_start; i_dca < vmsa::Dca_stop; i_dca++)
	{
	  for(Int_t i_sig = vmsa::nSigKaon_start; i_sig < vmsa::nSigKaon_stop; i_sig++)
	  {
            ...
            for(Int_t i_nhit = vmsa::mNHit_start; i_nhit < vmsa::mNHit_stop; i_nhit++) // systematic loop for nSigma
            {
              ...
              cout << "dca " << i_dca << "     i_sig = " << i_sig << "    i_nhit = " << i_nhit << endl;
	      string KEY_InPutSE = Form("pt_%d_Centrality_%d_CosThetaStar_%d_2nd_Dca_%d_Sig_%d_NHit_%d_%s_SE",i_pt,i_cent,i_theta,i_dca,i_sig,i_nhit,vmsa::mPID[pid].c_str());
              h_mInPut_SE[KEY_InPutSE] = (TH1F*)File_SE->Get(KEY_InPutSE.c_str())->Clone(); 

	      string KEY_InPutME = Form("pt_%d_Centrality_%d_CosThetaStar_%d_2nd_Dca_%d_Sig_%d_NHit_%d_%s_ME",i_pt,i_cent,i_theta,i_dca,i_sig,i_nhit,vmsa::mPID[pid].c_str());
	      h_mInPut_ME[KEY_InPutME] = (TH1F*)File_ME->Get(KEY_InPutME.c_str())->Clone(); 

	      for(int i_norm = vmsa::Norm_start; i_norm < 1; ++i_norm)
	      {
	        string KEY_SE = Form("pt_%d_Centrality_%d_CosThetaStar_%d_2nd_Dca_%d_Sig_%d_NHit_%d_%s_Norm_%d_SE",i_pt,i_cent,i_theta,i_dca,i_sig,i_nhit,vmsa::mPID[pid].c_str(),i_norm);
	        h_mMass_SE[KEY_SE] = (TH1F*)h_mInPut_SE[KEY_InPutSE]->Clone(KEY_SE.c_str());
	        string KEY_ME = Form("pt_%d_Centrality_%d_CosThetaStar_%d_2nd_Dca_%d_Sig_%d_NHit_%d_%s_Norm_%d_ME",i_pt,i_cent,i_theta,i_dca,i_sig,i_nhit,vmsa::mPID[pid].c_str(),i_norm);
	        h_mMass_ME[KEY_ME] = (TH1F*)h_mInPut_ME[KEY_InPutME]->Clone(KEY_ME.c_str());
	        string KEY_SM = Form("pt_%d_Centrality_%d_CosThetaStar_%d_2nd_Dca_%d_Sig_%d_NHit_%d_%s_Norm_%d_SM",i_pt,i_cent,i_theta,i_dca,i_sig,i_nhit,vmsa::mPID[pid].c_str(),i_norm);
  ...
  string KEY_SE_QA = Form("pt_%d_Centrality_%d_CosThetaStar_%d_2nd_Dca_%d_Sig_%d_NHit_0_%s_Norm_%d_SE",vmsa::pt_RawQAKS[energy],vmsa::Cent_start,vmsa::CTS_start,vmsa::Dca_start,vmsa::nSigKaon_start,vmsa::mPID[pid].c_str(),vmsa::Norm_QA);
  ...
  string KEY_ME_QA = Form("pt_%d_Centrality_%d_CosThetaStar_%d_2nd_Dca_%d_Sig_%d_NHit_0_%s_Norm_%d_ME",vmsa::pt_RawQAKS[energy],vmsa::Cent_start,vmsa::CTS_start,vmsa::Dca_start,vmsa::nSigKaon_start,vmsa::mPID[pid].c_str(),vmsa::Norm_QA);
  ...
  string KEY_SM_QA = Form("pt_%d_Centrality_%d_CosThetaStar_%d_2nd_Dca_%d_Sig_%d_NHit_0_%s_Norm_%d_SM",vmsa::pt_RawQAKS[energy],vmsa::Cent_start,vmsa::CTS_start,vmsa::Dca_start,vmsa::nSigKaon_start,vmsa::mPID[pid].c_str(),vmsa::Norm_QA);
  ...
  for(int i_pt = vmsa::pt_startKS; i_pt < vmsa::pt_stopKS; i_pt++) // pt loop
  {
    ...
    string KEY_SE_QA = Form("pt_%d_Centrality_%d_CosThetaStar_%d_2nd_Dca_%d_Sig_%d_NHit_0_%s_Norm_%d_SE",i_pt,vmsa::Cent_start,vmsa::CTS_start,vmsa::Dca_start,vmsa::nSigKaon_start,vmsa::mPID[pid].c_str(),vmsa::Norm_QA);
    h_mMass_SE[KEY_SE_QA]->GetYaxis()->SetRangeUser(-0.1*h_mMass_SE[KEY_SE_QA]->GetMaximum(),1.1*h_mMass_SE[KEY_SE_QA]->GetMaximum());
    h_mMass_SE[KEY_SE_QA]->DrawCopy();

    string KEY_ME_QA = Form("pt_%d_Centrality_%d_CosThetaStar_%d_2nd_Dca_%d_Sig_%d_NHit_0_%s_Norm_%d_ME",i_pt,vmsa::Cent_start,vmsa::CTS_start,vmsa::Dca_start,vmsa::nSigKaon_start,vmsa::mPID[pid].c_str(),vmsa::Norm_QA);
    h_mMass_ME[KEY_ME_QA]->SetLineColor(2);
    h_mMass_ME[KEY_ME_QA]->SetFillColor(2);
    h_mMass_ME[KEY_ME_QA]->SetFillStyle(3002);
    h_mMass_ME[KEY_ME_QA]->DrawCopy("h same");

    string KEY_SM_QA = Form("pt_%d_Centrality_%d_CosThetaStar_%d_2nd_Dca_%d_Sig_%d_NHit_0_%s_Norm_%d_SM",i_pt,vmsa::Cent_start,vmsa::CTS_start,vmsa::Dca_start,vmsa::nSigKaon_start,vmsa::mPID[pid].c_str(),vmsa::Norm_QA);
    h_mMass_SM[KEY_SM_QA]->SetLineColor(4);
    h_mMass_SM[KEY_SM_QA]->SetFillColor(4);
    h_mMass_SM[KEY_SM_QA]->SetFillStyle(3004);
    h_mMass_SM[KEY_SM_QA]->DrawCopy("h same");
  ...
  for(int i_cent = vmsa::Cent_start; i_cent < vmsa::Cent_stop; i_cent++) // Centrality loop
  {
	...
	for(Int_t i_sig = vmsa::nSigKaon_start; i_sig < vmsa::nSigKaon_stop; i_sig++)
	{
	    ...
	    for(int i_norm = vmsa::Norm_start; i_norm < 1; ++i_norm)
	    {
	      ...
	      {
	        string KEY = Form("pt_%d_Centrality_%d_CosThetaStar_%d_2nd_Dca_%d_Sig_%d_NHit_%d_%s_Norm_%d",pt_binKS,i_cent,i_theta,i_dca,i_sig,i_nhit,vmsa::mPID[pid].c_str(),i_norm);
  ...
  {
    c_pT_rebin->cd(vmsa::pt_rebin_startKS[energy][i_pt]+1);
    c_pT_rebin->cd(vmsa::pt_rebin_startKS[energy][i_pt]+1)->SetLeftMargin(0.15);
    c_pT_rebin->cd(vmsa::pt_rebin_startKS[energy][i_pt]+1)->SetBottomMargin(0.15);
    c_pT_rebin->cd(vmsa::pt_rebin_startKS[energy][i_pt]+1)->SetTicks(1,1);
    c_pT_rebin->cd(vmsa::pt_rebin_startKS[energy][i_pt]+1)->SetGrid(0,0);
    string KEY_QA = Form("pt_%d_Centrality_%d_CosThetaStar_%d_2nd_Dca_%d_Sig_%d_NHit_0_%s_Norm_%d",i_pt,vmsa::Cent_start,vmsa::CTS_start,vmsa::Dca_start,vmsa::nSigKaon_start,vmsa::mPID[pid].c_str(),vmsa::Norm_QA);
    ...
    string pT_range = Form("[%.2f,%.2f]",vmsa::pt_lowKS[energy][i_pt],vmsa::pt_upKS[energy][i_pt]);
    plotTopLegend((char*)pT_range.c_str(),0.2,0.7,0.08,1,0.0,42,1);
  ...
  /*{
	...
	for(Int_t i_dca = vmsa::Dca_start; i_dca < vmsa::Dca_stop; i_dca++)
	{
	  ...
	  { 
              ...
              for(int i_norm = vmsa::Norm_start; i_norm < 1; ++i_norm)
	      {
	        ...
	        f_sig->SetParLimits(0,vmsa::InvMass[pid]-0.01,vmsa::InvMass[pid]+0.01);
	        f_sig->SetParameter(1,vmsa::Width[pid]);
	        f_sig->SetParLimits(1,0.02,0.2);
    ...
    for(int i_pt = vmsa::pt_rebin_firstKS[energy]; i_pt < vmsa::pt_rebin_lastKS[energy]; i_pt++) // pt loop
    {
      c_mMass_theta->cd(vmsa::pt_rebin_startKS[energy][i_pt]+1);
      c_mMass_theta->cd(vmsa::pt_rebin_startKS[energy][i_pt]+1)->SetLeftMargin(0.15);
      c_mMass_theta->cd(vmsa::pt_rebin_startKS[energy][i_pt]+1)->SetBottomMargin(0.15);
      c_mMass_theta->cd(vmsa::pt_rebin_startKS[energy][i_pt]+1)->SetTicks(1,1);
      c_mMass_theta->cd(vmsa::pt_rebin_startKS[energy][i_pt]+1)->SetGrid(0,0);
      string KEY_theta_QA = Form("pt_%d_Centrality_%d_2nd_Dca_%d_Sig_%d_NHit_0_%s_Norm_%d",i_pt,vmsa::Cent_start,vmsa::Dca_start,vmsa::nSigKaon_start,vmsa::mPID[pid].c_str(),vmsa::Norm_QA);
...
*/
    ...
    for(int i_pt = vmsa::pt_rebin_firstKS[energy]; i_pt < vmsa::pt_rebin_lastKS[energy]; i_pt++) // pt loop
    {
      c_mMass_phi_diff->cd(vmsa::pt_rebin_startKS[energy][i_pt]+1);
      c_mMass_phi_diff->cd(vmsa::pt_rebin_startKS[energy][i_pt]+1)->SetLeftMargin(0.15);
      c_mMass_phi_diff->cd(vmsa::pt_rebin_startKS[energy][i_pt]+1)->SetBottomMargin(0.15);
      c_mMass_phi_diff->cd(vmsa::pt_rebin_startKS[energy][i_pt]+1)->SetTicks(1,1);
      c_mMass_phi_diff->cd(vmsa::pt_rebin_startKS[energy][i_pt]+1)->SetGrid(0,0);
      string KEY_QA = Form("pt_%d_Centrality_%d_CosThetaStar_%d_2nd_Dca_%d_Sig_%d_NHit_0_%s_Norm_%d",i_pt,vmsa::Cent_start,vmsa::CTS_start,vmsa::Dca_start,vmsa::nSigKaon_start,vmsa::mPID[pid].c_str(),vmsa::Norm_QA);
