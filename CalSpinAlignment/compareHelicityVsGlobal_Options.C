...
void compareHelicityVsGlobal_Options(Int_t energy = 4, Int_t pid = 0, Int_t i_cent = 9, string correction = "Raw", int order = 2, string etamode = "eta1_eta1", int yspectra = 23, std::string simmode = "Mc")//defaultF = 0 is BESII, defaultF = 1 is BESI
{
  ...
  std::string date[nopts] = {"20240809g","202408012h","202408012h","202408012h","202408012h","202408012h","202408012h","202408012h","202408012h","202408012h"}; // nov2, prelimv2
  std::string opts[nopts] = {"nov2","v20.0333","v20.0667","v20.1000","v20.1333","v20.1667","v20.2000","v20.2333","v20.2667","v20.3000"};
  float vals[nopts] = {0.0,1./33.,2./33,3./33.,4./33.,5./33,6./33.,7./33.,8./33,9./33.};
  ...
  for(int iopt = 0; iopt < nopts; iopt++)
  {
    ...
    {
      spectra[iopt][0][i] = Form("CEPT_O2_NoYSpec_%s_phieff_phiTPC_%s_ysig1000_rho%s_rerho0.0_imrho0.0_r0.0_i0.0_hrho0.3333_hrerho0.0_himrho0.0_hr0.0_hi0.0"   ,opts[iopt].c_str(),date[iopt].c_str(),sigmay_text[i].c_str());
      spectra[iopt][1][i] = Form("CEPT_O2_NoYSpec_%s_phieff_phiTPC_%s_ysig1000_rho0.3333_rerho0.0_imrho0.0_r%s_i0.0_hrho0.3333_hrerho0.0_himrho0.0_hr0.0_hi0.0",opts[iopt].c_str(),date[iopt].c_str(),sigmay_text_offdiag[i].c_str());
      spectra[iopt][2][i] = Form("CEPT_O2_NoYSpec_%s_phieff_phiTPC_%s_ysig1000_rho0.3333_rerho0.0_imrho0.0_r0.0_i%s_hrho0.3333_hrerho0.0_himrho0.0_hr0.0_hi0.0",opts[iopt].c_str(),date[iopt].c_str(),sigmay_text_offdiag[i].c_str());
      spectra[iopt][3][i] = Form("CEPT_O2_NoYSpec_%s_phieff_phiTPC_%s_ysig1000_rho0.3333_rerho%s_imrho0.0_r0.0_i0.0_hrho0.3333_hrerho0.0_himrho0.0_hr0.0_hi0.0",opts[iopt].c_str(),date[iopt].c_str(),sigmay_text_offdiag[i].c_str());
      spectra[iopt][4][i] = Form("CEPT_O2_NoYSpec_%s_phieff_phiTPC_%s_ysig1000_rho0.3333_rerho0.0_imrho%s_r0.0_i0.0_hrho0.3333_hrerho0.0_himrho0.0_hr0.0_hi0.0",opts[iopt].c_str(),date[iopt].c_str(),sigmay_text_offdiag[i].c_str());
    }
  }
  ...
  TGraphAsymmErrors *g2D[nvar][5][5][4]; 
  TGraphAsymmErrors *g2D_Global[nvar][5][5][4]; 
  ...
  for(int i = 0; i < nvar; i++)
  {
    for(int irho = 0; irho < 5; irho++)
    {
      ...
      //gglobal[ipt] = new TGraphAsymmErrors();
        ...
        {
          g2D[i][irho][par][ipt] = new TGraphAsymmErrors();
          g2D_Global[i][irho][par][ipt] = new TGraphAsymmErrors();
        ...
        for(int iopt = 0; iopt < nopts; iopt ++)
        {
          ...
          for(int par = 0; par < 5; par++)
          {
            if(par == 0 && irho == 0) g2D[i][irho][par][ipt]->SetPoint(iopt,vals[iopt]+0.002*(float(i)-float(nvar)/2.), fits2D[iopt][irho][ipt][i]->GetParameter(0)-1./3.);
            if(par != 0 && irho == 0) g2D[i][irho][par][ipt]->SetPoint(iopt,vals[iopt]+0.002*(float(i)-float(nvar)/2.), fits2D[iopt][irho][ipt][i]->GetParameter(par));
            if(par == 0 && irho != 0) g2D[i][irho][par][ipt]->SetPoint(iopt,vals[iopt]+0.002*(float(i)-float(nvar)/2.), fits2D[iopt][irho][ipt][i]->GetParameter(0)-1./3.);
            if(par != 0 && irho != 0) g2D[i][irho][par][ipt]->SetPoint(iopt,vals[iopt]+0.002*(float(i)-float(nvar)/2.), fits2D[iopt][irho][ipt][i]->GetParameter(par));
            g2D[i][irho][par][ipt]->SetPointError(iopt,0.0,0.0,fits2D[iopt][irho][ipt][i]->GetParError(par),fits2D[iopt][irho][ipt][i]->GetParError(par));
          }
          ...
          for(int par = 0; par < 5; par++)
          {
            if(par == 0 && irho == 0) g2D_Global[i][irho][par][ipt]->SetPoint(iopt,vals[iopt]+0.002*(float(i)-float(nvar)/2.), fits2D_Global[iopt][irho][ipt][i]->GetParameter(0)-1./3.);
            if(par != 0 && irho == 0) g2D_Global[i][irho][par][ipt]->SetPoint(iopt,vals[iopt]+0.002*(float(i)-float(nvar)/2.), fits2D_Global[iopt][irho][ipt][i]->GetParameter(par));
            if(par == 0 && irho != 0) g2D_Global[i][irho][par][ipt]->SetPoint(iopt,vals[iopt]+0.002*(float(i)-float(nvar)/2.), fits2D_Global[iopt][irho][ipt][i]->GetParameter(0)-1./3.);
            if(par != 0 && irho != 0) g2D_Global[i][irho][par][ipt]->SetPoint(iopt,vals[iopt]+0.002*(float(i)-float(nvar)/2.), fits2D_Global[iopt][irho][ipt][i]->GetParameter(par));
            g2D_Global[i][irho][par][ipt]->SetPointError(iopt,0.0,0.0,fits2D_Global[iopt][irho][ipt][i]->GetParError(par),fits2D_Global[iopt][irho][ipt][i]->GetParError(par));
          }
  ...
  std::string outputname = Form("figures/%s/%s/pTstudy/GlobalvsGlobal_parametercomparison_%s_%s.pdf",vmsa::mPID[pid].c_str(),vmsa::mBeamEnergy[energy].c_str(),simmode.c_str(),option.c_str());
  ...
  for(int i = 0; i < 4; i++)
  {
    for(int irho = 0; irho < 5; irho++)
    {
      if(i == 0) leg[irho] = new TLegend(0.4,0.75,0.7,0.9);
      for(int par = 0; par < 5; par++)
      {
        ...
        for(int iopt = 0; iopt < nvar; iopt++)
        {
          ...
          g2D_Global[iopt][irho][par][i]->SetTitle(Form("Input %s^{Global}, %1.1f<p_{T}<%1.1f GeV/c",param[irho].c_str(),ptedges[i],ptedges[i+1]));
          //if(irho != 0) g2D_Global[iopt][irho][par][i]->SetTitle(Form("Input %s^{Global}, %1.1f<p_{T}<%1.1f GeV/c",param[irho].c_str(),ptedges[i],ptedges[i+1]));
          g2D_Global[iopt][irho][par][i]->GetXaxis()->SetTitle(Form("v_{2} Input"));
          ...
          if(i == 0 && irho == 0 && par == 0) leg[irho]->AddEntry(g2D_Global[iopt][irho][par][i],Form("%s^{Global} = %1.4f",param[irho].c_str(),sigmay[iopt]),"p");
          if(i == 0 && irho != 0 && par == 0) leg[irho]->AddEntry(g2D_Global[iopt][irho][par][i],Form("%s^{Global} = %1.4f",param[irho].c_str(),sigmay_offdiag[iopt]),"p");
        }
        leg[irho]->Draw("same");
      }
    }
  ...
  outputname = Form("figures/%s/%s/pTstudy/HelicityvsGlobal_parametercomparison_%s_%s.pdf",vmsa::mPID[pid].c_str(),vmsa::mBeamEnergy[energy].c_str(),simmode.c_str(),option.c_str());
    ...
    for(int irho = 0; irho < 5; irho++)
    {
      ...
      for(int par = 0; par < 5; par++)
      {
        ...
        for(int iopt = 0; iopt < nvar; iopt++)
        {
          ...
          g2D[iopt][irho][par][i]->SetTitle(Form("Input %s^{Global}, %1.1f<p_{T}<%1.1f GeV/c",param[irho].c_str(),ptedges[i],ptedges[i+1]));
          ...
          if(i == 0 && irho == 0 && par == 0) leg2[irho]->AddEntry(g2D[iopt][irho][par][i],Form("%s^{Global} = %1.4f",param[irho].c_str(),sigmay[iopt]),"p");
          if(i == 0 && irho != 0 && par == 0) leg2[irho]->AddEntry(g2D[iopt][irho][par][i],Form("%s^{Global} = %1.4f",param[irho].c_str(),sigmay_offdiag[iopt]),"p");
