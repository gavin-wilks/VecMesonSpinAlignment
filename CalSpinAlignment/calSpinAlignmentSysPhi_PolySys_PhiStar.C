...
void calSpinAlignmentSysPhi_PolySys_PhiStar(int energy = 4, int pid = 0, int year = 0, bool random3D = false, int order = 2, string etamode = "eta1_eta1")
{
  ...
  string inputfile = Form("../output/AuAu%s/%s/Cos2PhiStarPhi_InvMassSubBg_%s.root",vmsa::mBeamEnergy[energy].c_str(),vmsa::mPID[pid].c_str(),etamode.c_str());
...
//#endif
  ...
  */
  ...
  for(int i_poly = 0; i_poly < 3; i_poly++)
  {
    for(int i_norm = 0; i_norm < 3; i_norm++)
    {
      ...
      {
        string outputname = Form("./figures/%s/%s/pTstudy/Cos2PhiStarPhi_allThetaYields%s_%s_Order%d_cent%d_%s_pTdependence_Norm%d_Poly%d.pdf",vmsa::mPID[pid].c_str(),vmsa::mBeamEnergy[energy].c_str(),vmsa::mPID[pid].c_str(),vmsa::mBeamEnergy[energy].c_str(),order,i_cent,etamode.c_str(),i_norm,i_poly+1);
 ...
 // {
  ...
  string outputfile = Form("../output/AuAu%s/%s/Cos2PhiStarPhi_RawPhiPtSys_%s_PolySys.root",vmsa::mBeamEnergy[energy].c_str(),vmsa::mPID[pid].c_str(),etamode.c_str());
