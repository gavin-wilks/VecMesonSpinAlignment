...
void calSpinAlignmentSysPhi_PolySys_AllOneBinFromStart_CWR(int energy = 5, int pid = 0, int year = 0, bool random3D = false, int order = 2, string etamode = "eta1_eta1")
{
  ...
  string inputfile = Form("../output/AuAu%s/%s/CWR_InvMassSubBg_%s.root",vmsa::mBeamEnergy[energy].c_str(),vmsa::mPID[pid].c_str(),etamode.c_str());
  ...
  string KEY_InteTheta = Form("Centrality_9");
  ...
  {
    string KEY_InteTheta_Poly = Form("Centrality_9_Poly%d",i_poly+1);
  ...
  cout << "Finished loading and fitting" << endl;
...
//      h_mMaiss[KEY_QA]->GetYaxis()->SetNdivisions(505,'N');
...
//#endif
  ...
  */
  ...
  for(int i_poly = 0; i_poly < 1; i_poly++)
  {
    string KEY_rho = Form("rhoRaw_Centrality_9_Poly%d",i_poly+1);
    g_mRho[KEY_rho] = new TGraphAsymmErrors();

    string KEY_counts = Form("Centrality_9_Poly%d",i_poly+1);
    ...
    for(int i_theta = vmsa::CTS_start; i_theta < vmsa::CTS_stop; ++i_theta) // cos(theta*) loop
    {
      string KEY = Form("Centrality_9_CosThetaStar_%d",i_theta);
      cout << KEY << endl;;
      string KEY_Poly = Form("Centrality_9_CosThetaStar_%d_Poly%d",i_theta,i_poly+1);
      TF1 *f_bw;
      if(i_poly == 0) f_bw = new TF1("f_bw",PolyBreitWigner, vmsa::BW_Start[pid],vmsa::BW_Stop[pid],5);
      if(i_poly == 1) f_bw = new TF1("f_bw",Poly2BreitWigner,vmsa::BW_Start[pid],vmsa::BW_Stop[pid],6);
      if(i_poly == 2) f_bw = new TF1("f_bw",Poly3BreitWigner,vmsa::BW_Start[pid],vmsa::BW_Stop[pid],7);
      cout << "BEFORE GRABBING FIT PARAMETERS" << endl;
      ...
      cout << "Pushed back fit parameter" << endl;
    }
    float pt_mean = (1.2+5.4)/2.0;
    ...
    cout << "Filled Rho distribution" << endl;
  }
  cout << "finished calculating rho" << endl;
  
#if _PlotQA_
  for(int i_poly = 0; i_poly < 1; i_poly++)
  {
    string outputname = Form("./figures/%s/%s/pTstudy/CWR_allThetaYields%s_%s_Order%d_cent9_pTdependence_Poly%d.pdf",vmsa::mPID[pid].c_str(),vmsa::mBeamEnergy[energy].c_str(),vmsa::mPID[pid].c_str(),vmsa::mBeamEnergy[energy].c_str(),order,i_poly+1);
    ...
    for(int i_theta = 0; i_theta < 9; ++i_theta)
    {
      ...
      if(i_theta < vmsa::CTS_stop)
      {
        string KEY_QA = Form("Centrality_9_CosThetaStar_%d",i_theta);
        h_mMass[KEY_QA]->SetTitle("");
        h_mMass[KEY_QA]->GetXaxis()->SetNdivisions(505,'N');
        h_mMass[KEY_QA]->GetXaxis()->SetLabelSize(0.03);
        h_mMass[KEY_QA]->GetXaxis()->SetTitle("M(K^{+},K^{-})");
        h_mMass[KEY_QA]->SetTitle(Form("1.2<p_{T}<5.4, %d/7<|cos(#theta*)|<%d/7",i_theta,i_theta+1));
        h_mMass[KEY_QA]->GetXaxis()->SetTitleSize(0.05);
        h_mMass[KEY_QA]->GetXaxis()->SetTitleOffset(1.2);
        h_mMass[KEY_QA]->GetXaxis()->CenterTitle();
      ...
      if(i_theta == vmsa::CTS_stop)
      {
        string KEY_InteTheta_QA = Form("Centrality_9");
        string KEY_InteTheta_QA_Poly = Form("Centrality_9_Poly%d",i_poly+1);
        h_mMass_InteTheta[KEY_InteTheta_QA]->SetTitle("");
        h_mMass_InteTheta[KEY_InteTheta_QA]->SetTitle(Form("1.2<p_{T}<5.4, 0/7<|cos(#theta*)|<7/7"));
        h_mMass_InteTheta[KEY_InteTheta_QA]->GetXaxis()->SetNdivisions(505,'N');
        h_mMass_InteTheta[KEY_InteTheta_QA]->GetXaxis()->SetLabelSize(0.03);
        h_mMass_InteTheta[KEY_InteTheta_QA]->GetXaxis()->SetTitle("M(K^{+},K^{-})");
        h_mMass_InteTheta[KEY_InteTheta_QA]->GetXaxis()->SetTitleSize(0.05);
    ...
    for(int i_theta = vmsa::CTS_start; i_theta < vmsa::CTS_stop; ++i_theta)
    {
      c_diff->cd(i_theta+1);
      string KEY_QA = Form("Centrality_9_CosThetaStar_%d",i_theta);
      string KEY_QA_Poly = Form("Centrality_9_CosThetaStar_%d_Poly%d",i_theta,i_poly+1);
      TF1 *f_bw;
      if(i_poly == 0) f_bw = new TF1("f_bw",PolyBreitWigner,vmsa::BW_Start[pid],vmsa::BW_Stop[pid],5);
      if(i_poly == 1) f_bw = new TF1("f_bw",Poly2BreitWigner,vmsa::BW_Start[pid],vmsa::BW_Stop[pid],6);
      if(i_poly == 2) f_bw = new TF1("f_bw",Poly3BreitWigner,vmsa::BW_Start[pid],vmsa::BW_Stop[pid],7);
    ...
    for(int i_sigma = 0/*vmsa::Sig_start*/; i_sigma < 1/*vmsa::Sig_stop*/; ++i_sigma)
    {
      ...
      {
        string KEY_counts_QA = Form("Centrality_9_Poly%d",i_poly+1);
 ...
 // if(random3D)  c_rho->SaveAs("../figures/3DRandom/c_rho.pdf");
  ...
  string outputfile = Form("../output/AuAu%s/%s/CWR_RawPhiPtSys_%s_PolySys.root",vmsa::mBeamEnergy[energy].c_str(),vmsa::mPID[pid].c_str(),etamode.c_str());
