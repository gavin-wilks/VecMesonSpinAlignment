...
void calSpinAlignmentSysPhiY_PolySys_TPCOnly(int energy = 4, int pid = 0, int year = 0, bool random3D = false, int etaQA = 9, int ptQA = 1, int centQA = 2, int dcaQA = 0, int nsigQA = 0, int normQA = 0, int sigQA = 0, int order = 2, std::string etamode = "eta1_eta1")
{
  ...
  string inputfile = Form("../output/AuAu%s/%s/TPCOnly_InvMassSubBgEta_%s.root",vmsa::mBeamEnergy[energy].c_str(),vmsa::mPID[pid].c_str(),etamode.c_str());
  if(order == 1) inputfile = Form("../output/AuAu%s/%s/InvMassSubBgEta_%s_FirstOrder.root",vmsa::mBeamEnergy[energy].c_str(),vmsa::mPID[pid].c_str(),etamode.c_str());
...
//#endif
  ...
  */
              ...
              for(int i_method = vmsa::Method_start; i_method < vmsa::Method_stop; ++i_method)
              {
                for(int i_poly = 0; i_poly < 3; i_poly++)
                {
          	    ...
          	    string KEY_counts = Form("eta_%d_pt_%d_Centrality_%d_%s_Dca_%d_Sig_%d_%s_Norm_%d_Sigma_%d_%s_Poly%d",i_eta,i_pt,i_cent,EP[order-1].c_str(),i_dca,i_sig,vmsa::mPID[pid].c_str(),i_norm,i_sigma,vmsa::mInteMethod[i_method].c_str(),i_poly+1);
                    ...
                    {
                      string KEY = Form("eta_%d_pt_%d_Centrality_%d_CosThetaStar_%d_%s_Dca_%d_Sig_%d_%s_Norm_%d",i_eta,i_pt,i_cent,i_theta,EP[order-1].c_str(),i_dca,i_sig,vmsa::mPID[pid].c_str(),i_norm);
                      string KEY_Poly = Form("eta_%d_pt_%d_Centrality_%d_CosThetaStar_%d_%s_Dca_%d_Sig_%d_%s_Norm_%d_Poly%d",i_eta,i_pt,i_cent,i_theta,EP[order-1].c_str(),i_dca,i_sig,vmsa::mPID[pid].c_str(),i_norm,i_poly+1);
                      TF1 *f_bw;
                      if(i_poly == 0) f_bw = new TF1("f_bw",PolyBreitWigner, vmsa::BW_Start[pid],vmsa::BW_Stop[pid],5);
                      if(i_poly == 1) f_bw = new TF1("f_bw",Poly2BreitWigner,vmsa::BW_Start[pid],vmsa::BW_Stop[pid],6);
                      if(i_poly == 2) f_bw = new TF1("f_bw",Poly3BreitWigner,vmsa::BW_Start[pid],vmsa::BW_Stop[pid],7);
  ...
  for(int i_poly = 0; i_poly < 3; i_poly++)
  {
    string outputname = Form("./figures/%s/%s/rapiditystudy/TPCOnly_allThetaYields%s_%s_Order%d_%s_RapidityDependence_Poly%d.pdf",vmsa::mPID[pid].c_str(),vmsa::mBeamEnergy[energy].c_str(),vmsa::mPID[pid].c_str(),vmsa::mBeamEnergy[energy].c_str(),order,etamode.c_str(),i_poly+1);
    ...
    for(int i_cent = vmsa::Cent_start; i_cent < vmsa::cent_rebin_total_2060; ++i_cent) // Centrality loop
    {
      for(int i_pt = vmsa::pt_rebin_first_y[energy]; i_pt <= vmsa::pt_rebin_last_y[energy]; ++i_pt) // pt loop
      {    
        for(int i_eta = vmsa::eta_start; i_eta < vmsa::eta_stop; i_eta++) // Rapidity bin loop
        {
          ...
          for(int i_theta = 0; i_theta < 9; ++i_theta)
          {
            ...
            if(i_theta < vmsa::CTS_stop)
            {
              string KEY_QA = Form("eta_%d_pt_%d_Centrality_%d_CosThetaStar_%d_%s_Dca_%d_Sig_%d_%s_Norm_%d",i_eta,i_pt,i_cent,i_theta,EP[order-1].c_str(),vmsa::Dca_start,vmsa::nSigKaon_start,vmsa::mPID[pid].c_str(),vmsa::Norm_QA);
              h_mMass[KEY_QA]->SetTitle("");
              h_mMass[KEY_QA]->GetXaxis()->SetNdivisions(505,'N');
              h_mMass[KEY_QA]->GetXaxis()->SetLabelSize(0.03);
              h_mMass[KEY_QA]->GetXaxis()->SetTitle("M(K^{+},K^{-})");
              h_mMass[KEY_QA]->SetTitle(Form("%.2f<p_{T}<%.2f, %.2f<y<%.2f, %d/7<|cos(#theta*)|<%d/7, cent(%d-%d)",vmsa::pt_low_y[energy][i_pt],vmsa::pt_up_y[energy][i_pt],vmsa::eta_bin[i_eta],vmsa::eta_bin[i_eta+1],i_theta,i_theta+1,centlow[i_cent],centhigh[i_cent]));
              h_mMass[KEY_QA]->GetXaxis()->SetTitleSize(0.05);
              h_mMass[KEY_QA]->GetXaxis()->SetTitleOffset(1.2);
              h_mMass[KEY_QA]->GetXaxis()->CenterTitle();
          ...
          for(int i_theta = vmsa::CTS_start; i_theta < vmsa::CTS_stop; ++i_theta)
          {
            c_diff->cd(i_theta+1);
            string KEY_QA = Form("eta_%d_pt_%d_Centrality_%d_CosThetaStar_%d_%s_Dca_%d_Sig_%d_%s_Norm_%d",i_eta,i_pt,i_cent,i_theta,EP[order-1].c_str(),vmsa::Dca_start,vmsa::nSigKaon_start,vmsa::mPID[pid].c_str(),vmsa::Norm_QA);
            string KEY_QA_Poly = Form("eta_%d_pt_%d_Centrality_%d_CosThetaStar_%d_%s_Dca_%d_Sig_%d_%s_Norm_%d_Poly%d",i_eta,i_pt,i_cent,i_theta,EP[order-1].c_str(),vmsa::Dca_start,vmsa::nSigKaon_start,vmsa::mPID[pid].c_str(),vmsa::Norm_QA,i_poly+1);
            TF1 *f_bw;
            if(i_poly == 0) f_bw = new TF1("f_bw",PolyBreitWigner,vmsa::BW_Start[pid],vmsa::BW_Stop[pid],5);
            if(i_poly == 1) f_bw = new TF1("f_bw",Poly2BreitWigner,vmsa::BW_Start[pid],vmsa::BW_Stop[pid],6);
            if(i_poly == 2) f_bw = new TF1("f_bw",Poly3BreitWigner,vmsa::BW_Start[pid],vmsa::BW_Stop[pid],7);
...
//#endif
  ...
  if(!random3D) c_rho->SaveAs("../figures/TPCOnly_c_rho.pdf");
  ...
  string outputname = Form("figures/%s/%s/rapiditystudy/TPCOnly_Chi2NDF_%s_Order%d.pdf",vmsa::mPID[pid].c_str(),vmsa::mBeamEnergy[energy].c_str(),vmsa::mBeamEnergy[energy].c_str(),order);
  string output_start = Form("%s[",outputname.c_str());
  ...
  outputname = Form("figures/%s/%s/rapiditystudy/TPCOnly_PValue_%s_Order%d.pdf",vmsa::mPID[pid].c_str(),vmsa::mBeamEnergy[energy].c_str(),vmsa::mBeamEnergy[energy].c_str(),order);
  ...
  string outputfile = Form("../output/AuAu%s/%s/TPCOnly_RawRhoEtaSys_%s_Poly.root",vmsa::mBeamEnergy[energy].c_str(),vmsa::mPID[pid].c_str(),etamode.c_str());
  if(order == 1) outputfile = Form("../output/AuAu%s/%s/RawRhoEtaSys_%s_Poly_FirstOrder.root",vmsa::mBeamEnergy[energy].c_str(),vmsa::mPID[pid].c_str(),etamode.c_str());
