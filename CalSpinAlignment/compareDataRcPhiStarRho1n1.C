...
void compareDataRcPhiStarRho1n1(Int_t energy = 4, Int_t pid = 0, Int_t i_cent = 9, string correction = "Raw", int order = 2, string etamode = "eta1_eta1", int yspectra = 23, std::string simmode = "Mc", std::string rho00 = "0.3333")//defaultF = 0 is BESII, defaultF = 1 is BESI
{

  const int nv = 7;
  std::string rho1n1_text[nv] = {"-0.3","-0.2","-0.1","0.0","0.1","0.2","0.3"};
  float rho1n1[nv] = {-0.3,-0.2,-0.1,0.0,0.1,0.2,0.3};

 
  std::string spectra[nv];
  for(int i = 0; i < nv; i++)
  {
    spectra[i] = Form("CEPT_O2_NoYSpec_prelimv2_phieff_phiTPC_20240809g_ysig1000_rho0.3333_rerho%s_imrho0.0_r0.0_i0.0_hrho0.3333_hrerho0.0_himrho0.0_hr0.0_hi0.0",rho1n1_text[i].c_str());
  }
  ...
  TFile *File_InPutRC[nv]; 
  TH1D *h_mCounts_RC[4][nv];  
  TH2D *h_m2D_RC[4][nv];  
  TH1D *h_mCounts_Ratio[4][nv];  
  TH1D *h_mCos_RC[4][nv];  
  ...
  TGraphAsymmErrors *gmeancos[4]; 
  TGraphAsymmErrors *grho00[4]; 
  ...
  std::string outputname = Form("figures/%s/%s/pTstudy/Cos2PhiStarPhi_Ratio_%s_Order%d_%s%s_rerho1n1comparison_%s_rho00%s.pdf",vmsa::mPID[pid].c_str(),vmsa::mBeamEnergy[energy].c_str(),correction.c_str(),order,etamode.c_str(),spectra[3].c_str(),simmode.c_str(),rho00.c_str());  
  ...
  for(int ipt = 0; ipt < 4; ipt++)
  {
    ...
    gmeancos[ipt] = new TGraphAsymmErrors();
    grho00[ipt] = new TGraphAsymmErrors();
    ...
    for(int i = 0; i < nv; i++)
    {
      ...
      double meancos = h_mCos_RC[ipt][i]->GetMean();
      double meancos_err = h_mCos_RC[ipt][i]->GetMeanError();
      
      h_mCounts_Ratio[ipt][i]->SetTitle(Form("Re(#rho_{1-1})=%1.1f, %1.1f<p_{T}<%1.1f, 20-60 Centrality",rho1n1[i],vmsa::pt_low[energy][ipt+2],vmsa::pt_up[energy][ipt+2]));
      ...
      fitscos[ipt][i] = new TF1(Form("fitcos_%d_%d",ipt,i),SpinDensity,-1.0,1.0,2);
      h_mCos_RC[ipt][i]->Fit(fitscos[ipt][i],"NMI");
      ...
      {
        if(par == 0) g2D[par][ipt]->SetPoint(i,rho1n1[i],fits2D[ipt][i]->GetParameter(0)-1./3.);
        else g2D[par][ipt]->SetPoint(i,rho1n1[i],fits2D[ipt][i]->GetParameter(par));
        g2D[par][ipt]->SetPointError(i,0.0,0.0,fits2D[ipt][i]->GetParError(par),fits2D[ipt][i]->GetParError(par));
      }

      float rho00 = fitscos[ipt][i]->GetParameter(0);
      float rho00err = fitscos[ipt][i]->GetParError(0);
      
      gslope[ipt]->SetPoint(i,rho1n1[i],fits[ipt][i]->GetParameter(1)); 
      gslope[ipt]->SetPointError(i,0.0,0.0,fits[ipt][i]->GetParError(1),fits[ipt][i]->GetParError(1));
      gintercept[ipt]->SetPoint(i,rho1n1[i],fits[ipt][i]->GetParameter(0)); 
      gintercept[ipt]->SetPointError(i,0.0,0.0,fits[ipt][i]->GetParError(0),fits[ipt][i]->GetParError(0));
      gmean[ipt]->SetPoint(i,rho1n1[i],mean); 
      gmean[ipt]->SetPointError(i,0.0,0.0,mean_err,mean_err);
      gmeancos[ipt]->SetPoint(i,rho1n1[i],meancos); 
      gmeancos[ipt]->SetPointError(i,0.0,0.0,meancos_err,meancos_err);
      grho00[ipt]->SetPoint(i,rho1n1[i],rho00-1./3.); 
      grho00[ipt]->SetPointError(i,0.0,0.0,rho00err,rho00err);
    }
    c->Update();
  ...
  for(int i = 0; i < 4; i++)
  {
    for(int par = 0; par < 5; par++)
    {
      ...
      g2D[par][i]->GetXaxis()->SetTitle("Re(#rho_{1-1})");
      g2D[par][i]->GetYaxis()->SetTitle(param[par].c_str());
      g2D[par][i]->Draw("APE"); 
    }
  }
  cfit2D->SaveAs(Form("figures/%s/%s/pTstudy/Cos2PhiStarPhi_parametercomparison_%s_Order%d_%s%s_rerho1n1comparison_%s_rho00%s.pdf",vmsa::mPID[pid].c_str(),vmsa::mBeamEnergy[energy].c_str(),correction.c_str(),order,etamode.c_str(),spectra[3].c_str(),simmode.c_str(),rho00.c_str()));  


  TCanvas *cfitcos = new TCanvas("cfitcos","cfitcos",10,10,1600,800);
  cfitcos->Divide(4,2);
  
  for(int i = 0; i < 8; i++)
  {
    cfitcos->cd(i+1);
    cfitcos->cd(i+1)->SetLeftMargin(0.15);  
    cfitcos->cd(i+1)->SetBottomMargin(0.15);
    cfitcos->cd(i+1)->SetTicks(1,1);
    cfitcos->cd(i+1)->SetGrid(0,0);
  }
  ...
  for(int i = 0; i < 4; i++)
  {
    cfitcos->cd(i+1);
    //cfit->cd(i+5)->SetLogx();
    grho00[i]->SetMarkerStyle(20);
    grho00[i]->SetMarkerSize(1.2);
    grho00[i]->SetTitle(Form("%1.1f<p_{T}<%1.1f GeV/c",ptedges[i],ptedges[i+1]));
    grho00[i]->GetXaxis()->SetTitle("Re(#rho_{1-1})");
    grho00[i]->GetYaxis()->SetTitle("#Delta#rho_{00}");
    grho00[i]->Draw("APE"); 
  }
  for(int i = 0; i < 4; i++)
  {
    cfitcos->cd(i+5);
    //cfit->cd(i+5)->SetLogx();
    gmeancos[i]->SetMarkerStyle(20);
    gmeancos[i]->SetMarkerSize(1.2);
    gmeancos[i]->SetTitle(Form("%1.1f<p_{T}<%1.1f GeV/c",ptedges[i],ptedges[i+1]));
    gmeancos[i]->GetXaxis()->SetTitle("Re(#rho_{1-1})");
    gmeancos[i]->GetYaxis()->SetTitle("<cos(#theta*)>");
    gmeancos[i]->Draw("APE"); 
  }
  cfitcos->SaveAs(Form("figures/%s/%s/pTstudy/Cos2PhiStarPhi_CosThetaStarRho00_%s_Order%d_%s%s_rerho1n1comparison_%s_rho00%s.pdf",vmsa::mPID[pid].c_str(),vmsa::mBeamEnergy[energy].c_str(),correction.c_str(),order,etamode.c_str(),spectra[3].c_str(),simmode.c_str(),rho00.c_str()));  



  TCanvas *cfit = new TCanvas("cfit","cfit",10,10,1600,1200);
  cfit->Divide(4,3);
  ...
  for(int i = 0; i < 4; i++)
  {
    ...
    gslope[i]->GetXaxis()->SetTitle("Re(#rho_{1-1})");
    gslope[i]->GetYaxis()->SetTitle(Form("d(Data/%s)/dcos(2#phi*-2#phi)",simmode.c_str()));
    gslope[i]->SetMarkerSize(1.2);
    gslope[i]->Draw("APE"); 
  }
  ...
  for(int i = 0; i < 4; i++)
  {
    ...
    gintercept[i]->GetXaxis()->SetTitle("Re(#rho_{1-1})");
  ...
  for(int i = 0; i < 4; i++)
  {
    ...
    gmean[i]->GetXaxis()->SetTitle("Re(#rho_{1-1})");
    gmean[i]->GetYaxis()->SetTitle("<cos(2#phi*-2#phi)>");
    gmean[i]->Draw("APE"); 
  }
  cfit->SaveAs(Form("figures/%s/%s/pTstudy/Cos2PhiStarPhi_FitParams_%s_Order%d_%s%s_rerho1n1comparison_%s_rho00%s.pdf",vmsa::mPID[pid].c_str(),vmsa::mBeamEnergy[energy].c_str(),correction.c_str(),order,etamode.c_str(),spectra[3].c_str(),simmode.c_str(),rho00.c_str()));  


  outputname = Form("figures/%s/%s/pTstudy/Cos2PhiStarPhi_Comparison_%s_Order%d_%s%s_rerho1n1comparison_%s_rho00%s.pdf",vmsa::mPID[pid].c_str(),vmsa::mBeamEnergy[energy].c_str(),correction.c_str(),order,etamode.c_str(),spectra[3].c_str(),simmode.c_str(),rho00.c_str());  
  ...
  for(int ipt = 0; ipt < 4; ipt++)
  {
    ...
    { 
      ...
      h_mCounts_RC[ipt][i]->SetTitle(Form("Re(#rho_{1-1})=%1.1f, %1.1f<p_{T}<%1.1f, 20-60 Centrality",rho1n1[i],vmsa::pt_low[energy][ipt+2],vmsa::pt_up[energy][ipt+2]));
