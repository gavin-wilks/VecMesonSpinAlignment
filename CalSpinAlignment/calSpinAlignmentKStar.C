...
void calSpinAlignmentKStar(int energy = 4, int pid = 2, int year = 0, double nSigVec = 2.0, int ptQA = 2, int method = 0, string date = "20220708")
{
  TGaxis::SetMaxDigits(4);
  gStyle->SetOptDate(0);

  string detector = "TPCorToF";
  if(method == 1) detector = "TPCandToF"; 
  ...
  for(int i_pt = vmsa::pt_rebin_firstKS[energy]; i_pt < vmsa::pt_rebin_lastKS[energy]; i_pt++) // pt loop
  {
    c_mMass_bw->cd(vmsa::pt_rebin_startKS[energy][i_pt]+1);
    c_mMass_bw->cd(vmsa::pt_rebin_startKS[energy][i_pt]+1)->SetLeftMargin(0.15);
    c_mMass_bw->cd(vmsa::pt_rebin_startKS[energy][i_pt]+1)->SetBottomMargin(0.15);
    c_mMass_bw->cd(vmsa::pt_rebin_startKS[energy][i_pt]+1)->SetTicks(1,1);
    c_mMass_bw->cd(vmsa::pt_rebin_startKS[energy][i_pt]+1)->SetGrid(0,0);
    string KEY_theta_QA = Form("pt_%d_Centrality_%d_2nd_%s_SM",i_pt,9,vmsa::mPID[pid].c_str());
...
//plotTopLegend((char*)"20%-60%",0.65,0.00258,0.04,1,0.0,42,0);
  ...
  for(int i_theta = vmsa::CTS_start; i_theta < vmsa::CTS_stop; i_theta++) // cos(theta*) loop
  {
    ...
    h_mMassBG[KEY_QA]->GetXaxis()->SetRangeUser(0.75,1.08);
    ...
    PlotLine(0.75,1.08,0.0,0.0,1,2,2);
  }
  ...
  h_mMass_BGtotal[KEY_theta_QA]->GetXaxis()->SetRangeUser(0.75,1.08);
  h_mMass_BGtotal[KEY_theta_QA]->DrawCopy("pE");
  TF1 *f_bw_QA2 = new TF1("f_bw_QA2",Poly2BreitWigner,vmsa::BW_Start[pid],vmsa::BW_Stop[pid],6);
  TF1 *f_bg_QA2 = new TF1("f_bg_QA2",Poly2,vmsa::BW_Start[pid],vmsa::BW_Stop[pid],3);
  ...
  string pT_range = Form("[%.2f,%.2f]",vmsa::pt_lowKS[energy][ptQA],vmsa::pt_upKS[energy][ptQA]);
  ...
  c_mMass_psi_4->SaveAs(Form("./figures/KStar/phi_SpinAlighment_noBGsub_pt%d_%s.pdf",ptQA,date.c_str()));
#endif
  ...
  f_bw_QA->Draw("l same");
  //string pT_range = Form("[%.2f,%.2f]",vmsa::pt_lowKS[energy][2],vmsa::pt_upKS[energy][2]);
  plotTopLegend((char*)pT_range.c_str(),0.15,0.7,0.08,1,0.0,42,1);
  PlotLine(0.6,1.2,0.0,0.0,1,2,2);
...
/*
  ...
  string pT_rangeR = Form("[%.2f,%.2f]",vmsa::pt_lowKS[energy][2],vmsa::pt_upKS[energy][2]);
...
*/
  ...
  h_play->GetYaxis()->SetRangeUser(0.1,0.6);
  ...
  PlotLine(2.0,2.5,0.5,0.5,1,2,2);
  string leg_line = "#rho_{00} = 1/3";
  plotTopLegend((char*)leg_line.c_str(),2.6,0.497,0.03,1,0.0,42,0);

  string leg_energy = Form("AuAu %s %d", vmsa::mBeamEnergy[energy].c_str(), vmsa::mBeamYear[energy]);
  plotTopLegend((char*)leg_energy.c_str(),0.4,0.15,0.04,1,0.0,42,0);
  string leg_centrality = "20%-60%";
  plotTopLegend((char*)leg_centrality.c_str(),1.0,0.12,0.04,1,0.0,42,0);

  string figure_name = Form("./figures/KStar/rho00_pT_%s.pdf",vmsa::mBeamEnergy[energy].c_str());
  c_Rho00_pT->SaveAs(figure_name.c_str());

  
  string OutPutFile = Form("../output/AuAu19GeV/KStar/RawRhoPt/RawRhoPtSys_%s_%s.root",detector.c_str(),date.c_str()); 
