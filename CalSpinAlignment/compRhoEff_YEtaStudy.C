...
void compRhoEff_YEtaStudy(Int_t energy = 3, Int_t pid = 0, std::string setting = "Raw", int order = 1)
{
  std::string ep = "";
  if(order == 1) ep = "_FirstOrder";
  std::string EP[2] = {"","2nd"};
  string inputfileF0 = Form("../output/AuAu%s/%s/RhoEta_%sSysErrors_eta0p4%s.root",vmsa::mBeamEnergy[energy].c_str(),vmsa::mPID[pid].c_str(),setting.c_str(),ep.c_str());
  string inputfileF1 = Form("../output/AuAu%s/%s/RhoEta_%sSysErrors_eta0p6%s.root",vmsa::mBeamEnergy[energy].c_str(),vmsa::mPID[pid].c_str(),setting.c_str(),ep.c_str());
  string inputfileF2 = Form("../output/AuAu%s/%s/RhoEta_%sSysErrors_eta0p8%s.root",vmsa::mBeamEnergy[energy].c_str(),vmsa::mPID[pid].c_str(),setting.c_str(),ep.c_str());
  string inputfileF3 = Form("../output/AuAu%s/%s/RhoEta_%sSysErrors_eta1_eta1%s.root",vmsa::mBeamEnergy[energy].c_str(),vmsa::mPID[pid].c_str(),setting.c_str(),ep.c_str());
...
//  PlotLine(0.0,5.0,1.0/3.0,1.0/3.0,1,2,2);
  ...
  Draw_TGAE_Point_new_Symbol(-0.1,0.55,0.0,0.0,0.0,0.0,20,kBlue,1.3); 
  string leg_count0 = Form("#phi |#eta|<0.4");
  plotTopLegend((char*)leg_count0.c_str(),0.0,0.547,0.03,1,0.0,42,0);

  Draw_TGAE_Point_new_Symbol(-0.1,0.53,0.0,0.0,0.0,0.0,20,kOrange+7,1.3); 
  string leg_count1 = Form("#phi |#eta|<0.6");
  plotTopLegend((char*)leg_count1.c_str(),0.0,0.527,0.03,1,0.0,42,0);
