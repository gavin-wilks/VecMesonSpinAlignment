...
void compRhoEff_PtComp(Int_t energy = 4, Int_t pid = 0, std::string setting = "AccRes")
{
  string inputfileF0 = Form("../output/AuAu%s/%s/Rho_%sSysErrors_F_0_eta0p6.root",vmsa::mBeamEnergy[energy].c_str(),vmsa::mPID[pid].c_str(),setting.c_str());
  string inputfileF1 = Form("../output/AuAu%s/%s/RhoPtEta_%sSysErrors_eta0p6.root",vmsa::mBeamEnergy[energy].c_str(),vmsa::mPID[pid].c_str(),setting.c_str());
  //string inputfileF2 = Form("../output/AuAu%s/%s/RhoEta_%sSysErrors_eta0p8.root",vmsa::mBeamEnergy[energy].c_str(),vmsa::mPID[pid].c_str(),setting.c_str());
  //string inputfileF3 = Form("../output/AuAu%s/%s/RhoEta_%sSysErrors_eta1_eta1.root",vmsa::mBeamEnergy[energy].c_str(),vmsa::mPID[pid].c_str(),setting.c_str());
  ...
  /*double i_cent = 9;
  for(int i_eta = vmsa::y_rebin_first[energy]+1; i_eta <= vmsa::y_rebin_last[energy]-1; ++i_eta) // pt loop
  {           
  ...
  h_frame->GetYaxis()->SetRangeUser(0.24,0.48);
  ...
  Draw_TGAE_Point_new_Symbol(0.5,0.46,0.0,0.0,0.0,0.0,20,kBlue,1.3); 
  string leg_count0 = Form("#phi |#eta|<0.6");
  plotTopLegend((char*)leg_count0.c_str(),0.6,0.457,0.03,1,0.0,42,0);

  Draw_TGAE_Point_new_Symbol(0.5,0.44,0.0,0.0,0.0,0.0,20,kOrange+7,1.3); 
  string leg_count1 = Form("#phi |#eta|<0.6 y binned");
  plotTopLegend((char*)leg_count1.c_str(),0.6,0.437,0.03,1,0.0,42,0);

  ///Draw_TGAE_Point_new_Symbol(-0.1,0.51,0.0,0.0,0.0,0.0,20,kGray+2,1.3); 
  //string leg_count2 = Form("#phi |#eta|<0.8");
  //plotTopLegend((char*)leg_count2.c_str(),0.0,0.507,0.03,1,0.0,42,0);

  //Draw_TGAE_Point_new_Symbol(-0.1,0.49,0.0,0.0,0.0,0.0,20,kBlack,1.3); 
  //string leg_count3 = Form("#phi |#eta|<1.0");
  //plotTopLegend((char*)leg_count3.c_str(),0.0,0.487,0.03,1,0.0,42,0);
  ...
  /*string outputname = Form("./figures/Phi/19GeV/rho00Y_eta1_eta1.pdf");
