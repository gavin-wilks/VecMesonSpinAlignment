...
void calSysErrorPhiPsi_2D_OffDiag(Int_t energy = 4, Int_t pid = 0, string correction = "Raw", bool random3D = false, int order = 2, string etamode = "eta1_eta1", int yspectra = -1, int frameopt = 0, int y_padding = 2)//defaultF = 0 is BESII, defaultF = 1 is BESI
{
  ...
  std::string spectra = "";
  if(yspectra == 0) spectra = "_NoPhiPsiSpectra";
  if(yspectra == 1) spectra = "_NoPhiPsiSpectra_EP";
  if(yspectra == 2) spectra = "_NoPhiPsiSpectra_EP_noV2";
  if(yspectra == 3) spectra = "_WithPhiPsiSpectra";
  if(yspectra == 4) spectra = "_WithPhiPsiSpectra_HalfSigma";
  if(yspectra == 5) spectra = "_NoPhiPsiSpectra_FixedFirstEP";
  if(yspectra == 6) spectra = "_NoPhiPsiSpectra_FixedFirstEP_PhiPsi";
  if(yspectra == 7) spectra = "_NoPhiPsiSpectra_PhiPsi_EP";
  ...
  string inputfileHframe = Form("../output/AuAu%s/%s/%s2DPhiPsi_RawPhiPhiPsiSys_eta1_eta1.root",frame.c_str(),vmsa::mBeamEnergy[energy].c_str(),vmsa::mPID[pid].c_str());
  string inputfile = Form("../output/AuAu%s/%s/%s2DPhiPsi_%sPhiPhiPsiSys_%s%s.root",frame.c_str(),vmsa::mBeamEnergy[energy].c_str(),vmsa::mPID[pid].c_str(),correction.c_str(),etamode.c_str(),spectra.c_str());
  if(order == 1) inputfile = Form("../output/AuAu%s/%s/%s2DPhiPsi_%sPhiPhiPsiSys_%s_FirstOrder%s.root",frame.c_str(),vmsa::mBeamEnergy[energy].c_str(),vmsa::mPID[pid].c_str(),correction.c_str(),etamode.c_str(),spectra.c_str());
  if(correction == "RawRes") inputfile = Form("../output/AuAu%s/%s/%s2DPhiPsi_RawPhiPhiPsiSys_%s%s.root",frame.c_str(),vmsa::mBeamEnergy[energy].c_str(),vmsa::mPID[pid].c_str(),etamode.c_str(),spectra.c_str());
  if(correction == "RawRes" && order == 1) inputfile = Form("../output/AuAu%s/%s/%s2DPhiPsi_RawPhiPhiPsiSys_%s_FirstOrder%s.root",frame.c_str(),vmsa::mBeamEnergy[energy].c_str(),vmsa::mPID[pid].c_str(),etamode.c_str(),spectra.c_str());
  ...
  for(int i_par = 0; i_par < 5; i_par++)
  {
    ...
    {           
      for(int i_cent = 0; i_cent < vmsa::cent_rebin_total_2D; i_cent++)
      {
        ...
        for(Int_t i_point = 0; i_point < 12; ++i_point)
        {
          double sysDca[9];
          double sysNSig[9];
          double sysNorm[9];
  ...
  c_rho_SysError->SaveAs(Form("figures/%s/%s/phipsistudy/%s2D_finaloutputrho_%s_Order%d_%s.pdf",vmsa::mPID[pid].c_str(),vmsa::mBeamEnergy[energy].c_str(),frame.c_str(),correction.c_str(),order,etamode.c_str()));  
  c_rho_SysError->SaveAs(Form("figures/%s/%s/phipsistudy/%s2D_finaloutputrho_%s_Order%d_%s.eps",vmsa::mPID[pid].c_str(),vmsa::mBeamEnergy[energy].c_str(),frame.c_str(),correction.c_str(),order,etamode.c_str()));  
  c_rho_SysError->SaveAs(Form("figures/%s/%s/phipsistudy/%s2D_finaloutputrho_%s_Order%d_%s.png",vmsa::mPID[pid].c_str(),vmsa::mBeamEnergy[energy].c_str(),frame.c_str(),correction.c_str(),order,etamode.c_str()));  
  ...
  string OutPutFile = Form("../output/AuAu%s/%s/%s2DPhiPsi_Rho_%sSysErrors_%s_%s.root",frame.c_str(),vmsa::mBeamEnergy[energy].c_str(),vmsa::mPID[pid].c_str(),correction.c_str(),etamode.c_str(),spectra.c_str());
  if(order == 1) OutPutFile = Form("../output/AuAu%s/%s/%s2DPhiPsi_Rho_%sSysErrors_%s_FirstOrder%s.root",frame.c_str(),vmsa::mBeamEnergy[energy].c_str(),vmsa::mPID[pid].c_str(),correction.c_str(),etamode.c_str(),spectra.c_str());
