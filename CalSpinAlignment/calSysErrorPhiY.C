...
void calSysErrorPhiY(Int_t energy = 3, Int_t pid = 0, string correction = "Raw", bool random3D = false, int ptQA = 0, int centQA = 2, int order = 2, std::string etamode = "eta1_eta1", std::string fileoption = "")//defaultF = 0 is BESII, defaultF = 1 is BESI
{
  ...
  if(random3D) inputfile = Form("../output/AuAu%s/%s/3DRandom/%sRhoEtaSys.root",vmsa::mBeamEnergy[energy].c_str(),vmsa::mPID[pid].c_str(),correction.c_str());
  ...
  if(etamode == "eta1_eta1") h_frame->GetYaxis()->SetRangeUser(0.26,0.50);
  ...
  double weight_error_sys_y_statweight = 0;
  ...
  for(int i_eta = vmsa::eta_start+ypadding; i_eta < vmsa::eta_stop-ypadding; i_eta++) // Rapidity bin loop
  {
    ...
    double weight_error_sys_rho00_sw = 0;
    ...
    {           
      ...
      { 
        string KEY_Default = Form("rhoRaw_pt_%d_Centrality_%d_%s_Dca_%d_Sig_%d_%s_Norm_%d_Sigma_%d_%s",i_pt,i_cent,EP[order-1].c_str(),0,0,vmsa::mPID[pid].c_str(),0,0,vmsa::mInteMethod[1].c_str());
        string KEY_counts = Form("eta_%d_pt_%d_Centrality_%d_%s_Dca_%d_Sig_%d_%s_Norm_%d_Sigma_%d_%s",i_eta,i_pt,i_cent,EP[order-1].c_str(),0,0,vmsa::mPID[pid].c_str(),0,0,vmsa::mInteMethod[1].c_str());
        ...
        g_mStatErrors[KEY_Default]->GetPoint(i_eta-ypadding,etaStat,rhoStat);   
        if(rhoStat == -999.0) { cout << "skip this rapidity" << endl; continue;}
        rhoErrStat = g_mStatErrors[KEY_Default]->GetErrorYhigh(i_eta-ypadding);   
        double etaSys, rhoSys, rhoErrSys;
        g_mSysErrors[KEY_Default]->GetPoint(i_eta-ypadding,etaSys,rhoSys);   
        rhoErrSys = g_mSysErrors[KEY_Default]->GetErrorYhigh(i_eta-ypadding);  
        ...
        cout << "eta = " << i_eta << ", Cent = " << i_cent << ", pT = " << i_pt << ",    rho00 sys error  = " << rhoErrSys  << ", weight = " << weight << endl;
        weight_all += weight;

        weight_rho00_sw += rhoStat/rhoErrStat/rhoErrStat;
        weight_error_sys_rho00_sw += rhoErrSys/rhoErrStat/rhoErrStat;
        ...
        weight_error_sys_y_statweight += rhoErrSys/rhoErrStat/rhoErrStat;
    ...
    weight_error_sys_rho00_sw /= weight_all_sw;
    ...
    double eta_mean = (vmsa::eta_bin[i_eta] + vmsa::eta_bin[i_eta+1])/2.0;
    g_mStatErrorsInt->SetPoint(i_eta-ypadding, eta_mean, weight_rho00_sw);
    g_mStatErrorsInt->SetPointError(i_eta-ypadding, 0.0, 0.0, weight_error_stat_rho00_sw, weight_error_stat_rho00_sw);
    g_mSysErrorsInt->SetPoint(i_eta-ypadding, eta_mean, weight_rho00_sw);
    g_mSysErrorsInt->SetPointError(i_eta-ypadding, 0.0, 0.0, weight_error_sys_rho00_sw, weight_error_sys_rho00_sw);  
  }
  ...
  weight_error_sys_y_statweight /= weight_all_y_statweight;
  cout << std::setprecision(4);
  cout << "Total Integrated rho00 yield weight = " << weight_y << " +/- " << weight_error_stat_y << " stat. " << " +/- " << weight_error_sys_y << "sys." << endl; 
  cout << "Total Integrated rho00 stat  weight = " << weight_y_statweight << " +/- " << weight_error_stat_y_statweight << " stat. " << " +/- " << weight_error_sys_y_statweight << "sys." << endl; 
