...
void calSysErrorPtY(Int_t energy = 4, Int_t pid = 0, string correction = "AccRes", bool random3D = false, int ptQA = 0, int centQA = 2, std::string etamode = "eta0p6")//defaultF = 0 is BESII, defaultF = 1 is BESI
{
  double ptfirst = 2;
  double ptlast = 5;
  ...
  int ypadding = 0;
  if(etamode == "eta1_eta1" && (correction == "AccRes" || correction == "Eff" || correction == "Acc")) ypadding = 1;
  if(etamode == "eta0p6" && (correction == "AccRes" || correction == "Eff" || correction == "Acc")) ypadding = 2;


  string inputfileHframe = Form("../output/AuAu%s/%s/RawRhoPtEtaSys_%s.root",vmsa::mBeamEnergy[energy].c_str(),vmsa::mPID[pid].c_str(),etamode.c_str());
  //string inputfile = Form("../output/AuAu%s/%s/%sRhoPtEtaSys_%s_sub_noDelta_2ndSubEPRes.root",vmsa::mBeamEnergy[energy].c_str(),vmsa::mPID[pid].c_str(),correction.c_str(),etamode.c_str());
  string inputfile = Form("../output/AuAu%s/%s/%sRhoPtEtaSys_%s.root",vmsa::mBeamEnergy[energy].c_str(),vmsa::mPID[pid].c_str(),correction.c_str(),etamode.c_str());
  ...
  //TH1D *h_frame = (TH1D*)File_InPut->Get("h_frame");
  TGraMap g_mRho;
  TH1DMap h_mRho; 
  TH1DMap h_mCounts;
  ...
  for(int i_eta = vmsa::y_rebin_first[energy]+ypadding; i_eta <= vmsa::y_rebin_last[energy]-ypadding; ++i_eta) // pt loop
  {           
      ...
      for(Int_t i_sig = vmsa::nSigKaon_start; i_sig < vmsa::nSigKaon_stop; i_sig++)
      {
        ...
        for(int i_norm = vmsa::Norm_start; i_norm < vmsa::Norm_stop; ++i_norm)
        {
          for(int i_sigma = vmsa::Sig_start; i_sigma < vmsa::Sig_stop; ++i_sigma)
          {
            ...
            {
              string KEY_rho = Form("rhoRaw_eta_%d_Centrality_%d_2nd_Dca_%d_Sig_%d_%s_Norm_%d_Sigma_%d_%s",i_eta,i_cent,i_dca,i_sig,vmsa::mPID[pid].c_str(),i_norm,i_sigma,vmsa::mInteMethod[i_method].c_str());
              cout << KEY_rho << endl;
              g_mRho[KEY_rho] = (TGraphAsymmErrors*)File_InPut->Get(KEY_rho.c_str())->Clone();
              for(int i_pt = ptfirst; i_pt <= ptlast; ++i_pt) // pt loop
              {
  ...
  //if(etamode == "eta1_eta1") h_frame->GetYaxis()->SetRangeUser(0.23,0.51);
        ...
        for(int i_norm = vmsa::Norm_start; i_norm < vmsa::Norm_stop; ++i_norm)
        {
          for(int i_sigma = vmsa::Sig_start; i_sigma < vmsa::Sig_stop; ++i_sigma)
          {
            for(int i_method = vmsa::Method_start; i_method < vmsa::Method_stop; ++i_method)
            {
             ...
             //   {
             //     if(i_F > 0) continue;
                  string KEY_rho = Form("rhoRaw_eta_%d_Centrality_%d_2nd_Dca_%d_Sig_%d_%s_Norm_%d_Sigma_%d_%s",3,9,i_dca,i_sig,vmsa::mPID[pid].c_str(),i_norm,i_sigma,vmsa::mInteMethod[i_method].c_str());
  ...
  cout << "Before calculating systematics" << endl;

  for(int i_eta = vmsa::y_rebin_first[energy]+ypadding; i_eta <= vmsa::y_rebin_last[energy]-ypadding; ++i_eta) // pt loop
  {           
    //for(int i_cent = vmsa::centStart; i_cent < vmsa::centStop; i_cent++) // Centrality loop
      string KEY_Default = Form("rhoRaw_eta_%d_Centrality_%d_2nd_Dca_%d_Sig_%d_%s_Norm_%d_Sigma_%d_%s",i_eta,i_cent,0,0,vmsa::mPID[pid].c_str(),0,0,vmsa::mInteMethod[1].c_str());
      ...
      for(Int_t i_point = ptfirst; i_point <= ptlast; ++i_point)
      {
        double sysDca[9];
        double sysNSig[9];
        double sysNorm[9];
        ...
        { 
          for(int i_sigma = vmsa::Sig_start; i_sigma < vmsa::Sig_stop; ++i_sigma)
          {
            for(int i_method = vmsa::Method_start; i_method < vmsa::Method_stop; ++i_method)
            {
              ...
              string KEY = Form("rhoRaw_eta_%d_Centrality_%d_2nd_Dca_%d_Sig_%d_%s_Norm_%d_Sigma_%d_%s",i_eta,i_cent,i_dca,0,vmsa::mPID[pid].c_str(),0,i_sigma,vmsa::mInteMethod[i_method].c_str());
        ...
        for(Int_t i_sig = vmsa::nSigKaon_start; i_sig < vmsa::nSigKaon_stop; i_sig++)
        {
          for(int i_sigma = vmsa::Sig_start; i_sigma < vmsa::Sig_stop; ++i_sigma)
          {
            for(int i_method = vmsa::Method_start; i_method < vmsa::Method_stop; ++i_method)
            {
              ...
              string KEY = Form("rhoRaw_eta_%d_Centrality_%d_2nd_Dca_%d_Sig_%d_%s_Norm_%d_Sigma_%d_%s",i_eta,i_cent,0,i_sig,vmsa::mPID[pid].c_str(),0,i_sigma,vmsa::mInteMethod[i_method].c_str());
        ...
        for(int i_norm = vmsa::Norm_start; i_norm < vmsa::Norm_stop; ++i_norm)
        {
          for(int i_sigma = vmsa::Sig_start; i_sigma < vmsa::Sig_stop; ++i_sigma)
          {
            for(int i_method = vmsa::Method_start; i_method < vmsa::Method_stop; ++i_method)
            {
              ...
              string KEY = Form("rhoRaw_eta_%d_Centrality_%d_2nd_Dca_%d_Sig_%d_%s_Norm_%d_Sigma_%d_%s",i_eta,i_cent,0,0,vmsa::mPID[pid].c_str(),i_norm,i_sigma,vmsa::mInteMethod[i_method].c_str());
  ...
  double weight_rho00t = 0;
  double weight_error_stat_rho00t = 0;
  double weight_allt = 0;
  ...
  for(int i_pt = ptfirst; i_pt <= ptlast; ++i_pt) // pt loop
  {
    ...
    {           
        string KEY_Default = Form("rhoRaw_eta_%d_Centrality_%d_2nd_Dca_%d_Sig_%d_%s_Norm_%d_Sigma_%d_%s",i_eta,i_cent,0,0,vmsa::mPID[pid].c_str(),0,0,vmsa::mInteMethod[1].c_str());
        string KEY_counts = Form("eta_%d_pt_%d_Centrality_%d_2nd_Dca_%d_Sig_%d_%s_Norm_%d_Sigma_%d_%s",i_eta,i_pt,i_cent,0,0,vmsa::mPID[pid].c_str(),0,0,vmsa::mInteMethod[1].c_str());
        ...
        g_mStatErrors[KEY_Default]->GetPoint(i_pt,etaStat,rhoStat);   
        if(rhoStat == -999.0) { cout << "skip this rapidity" << endl; continue;}
        rhoErrStat = g_mStatErrors[KEY_Default]->GetErrorYhigh(i_pt);   
        double etaSys, rhoSys, rhoErrSys;
        g_mSysErrors[KEY_Default]->GetPoint(i_pt,etaSys,rhoSys);   
        rhoErrSys = g_mSysErrors[KEY_Default]->GetErrorYhigh(i_pt);  

        weight_rho00 += rhoStat*weight;
        weight_rho00t += rhoStat*weight;
        ...
        weight_error_stat_rho00t += rhoErrStat*rhoErrStat*weight*weight;
    ...
    cout << "pt = " << i_pt << ",    weight_rho00 = " << weight_rho00 << endl;
    ...
    g_mStatErrorsInt->SetPoint(i_pt, pt_mean, weight_rho00);
    g_mStatErrorsInt->SetPointError(i_pt, 0.0, 0.0, weight_error_stat_rho00, weight_error_stat_rho00);
    g_mSysErrorsInt->SetPoint(i_pt, pt_mean, weight_rho00);
    g_mSysErrorsInt->SetPointError(i_pt, 0.0, 0.0, weight_error_sys_rho00, weight_error_sys_rho00);  
  }
  weight_rho00t /= weight_allt;
  weight_error_stat_rho00t = TMath::Sqrt(weight_error_stat_rho00t)/weight_allt;

  cout << "Final Integrated rho00 = " << weight_rho00t << " +/- " << weight_error_stat_rho00t << endl;
...
//  PlotLine(0.0,5.0,1.0/3.0,1.0/3.0,1,2,2);
  ...
  c_rho_SysError->SaveAs(Form("figures/finaloutputrhoPtEta_%s.pdf",etamode.c_str()));  
  ...
  string outputname = Form("./figures/Phi/19GeV/rho00pteta_%s.pdf",etamode.c_str());
...
//  std::string centStrings[4] = {"50-80","30-50","10-30","0-10"};
  double y_bin[8] = {-1.5,-1.0,-0.6,-0.2,0.2,0.6,1.0,1.5};
  ...
  for(int i_eta = vmsa::y_rebin_first[energy]+ypadding; i_eta <= vmsa::y_rebin_last[energy]-ypadding; ++i_eta) // pt loop
  {
      ...
      leg_count = Form("%1.1f < y < %1.1f",y_bin[i_eta],y_bin[i_eta+1]);
      ...
      //plotTopLegend((char*)Form("%.2f<p_{T}<%.2f GeV/c",vmsa::pt_low[energy][i_pt],vmsa::pt_up[energy][i_pt]),-0.3,0.285,0.04,1,0.0,42,0);
  ...
  string OutPutFile = Form("../output/AuAu%s/%s/RhoPtEta_%sSysErrors_%s.root",vmsa::mBeamEnergy[energy].c_str(),vmsa::mPID[pid].c_str(),correction.c_str(),etamode.c_str());
  ...
  for(int i_eta = vmsa::y_rebin_first[energy]+ypadding; i_eta <= vmsa::y_rebin_last[energy]-ypadding; ++i_eta) // pt loop
  {           
      string KEY_rho = Form("rhoRaw_eta_%d_Centrality_%d_2nd_Dca_%d_Sig_%d_%s_Norm_%d_Sigma_%d_%s",i_eta,i_cent,0,0,vmsa::mPID[pid].c_str(),0,0,vmsa::mInteMethod[1].c_str());
      string KEY_rhoStat = Form("rhoRawStat_eta_%d_Centrality_%d_2nd_Dca_%d_Sig_%d_%s_Norm_%d_Sigma_%d_%s",i_eta,i_cent,0,0,vmsa::mPID[pid].c_str(),0,0,vmsa::mInteMethod[1].c_str());
      string KEY_rhoSys  = Form("rhoRawSys_eta_%d_Centrality_%d_2nd_Dca_%d_Sig_%d_%s_Norm_%d_Sigma_%d_%s",i_eta,i_cent,0,0,vmsa::mPID[pid].c_str(),0,0,vmsa::mInteMethod[1].c_str());
