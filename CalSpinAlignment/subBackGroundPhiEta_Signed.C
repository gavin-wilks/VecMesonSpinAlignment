...
void subBackGroundPhiEta_Signed(int energy = 4, int i_eta = 0, int pid = 0, int year = 0, string date = "20230531", bool random3D = false, int order = 2, std::string etamode = "eta1_eta1")
{
...
//  }
  ...
  string outputname = Form("figures/%s/%s/rapiditystudy/InvMassDistributions_%s_Order%d_y%d_signedcosthetastar.pdf",vmsa::mPID[pid].c_str(),vmsa::mBeamEnergy[energy].c_str(),vmsa::mBeamEnergy[energy].c_str(),order,i_eta);
  ...
  for(int itheta = 0; itheta < 14; itheta++)
  {
    for(int icent = 0; icent < 9; icent++)
    {
      for(int i_pt = vmsa::pt_start; i_pt < vmsa::pt_stop; i_pt++) // pt loop
      {
        ...
        //cout << KEY_SE_QA << endl;
        h_mMass_SE[KEY_SE_QA]->GetYaxis()->SetRangeUser(-0.1*h_mMass_SE[KEY_SE_QA]->GetMaximum(),1.1*h_mMass_SE[KEY_SE_QA]->GetMaximum());
        h_mMass_SE[KEY_SE_QA]->SetTitle(Form("%.1f<y<%.1f, Cent %d, %d/7<|cos(#theta*)|<%d/7",vmsa::eta_bin[i_eta],vmsa::eta_bin[i_eta+1],icent,itheta-7,itheta-6));
        h_mMass_SE[KEY_SE_QA]->DrawCopy();
...
//#endif
  ...
  outputname = Form("figures/%s/%s/rapiditystudy/InvMassBackgroundSubtracted_Rebinned_%s_Order%d_y%d_signedcosthetastar.pdf",vmsa::mPID[pid].c_str(),vmsa::mBeamEnergy[energy].c_str(),vmsa::mBeamEnergy[energy].c_str(),order,i_eta);
  ...
  for(int itheta = 0; itheta < 14; itheta++)
  {
    for(int icent = 0; icent < 3; icent++)
    {
      for(int i_pt = vmsa::pt_rebin_first_y[energy]; i_pt <= vmsa::pt_rebin_last_y[energy]; i_pt++) // pt loop
      {
        ...
        h_mMass[KEY_QA]->SetMarkerSize(1.2);
        h_mMass[KEY_QA]->SetLineColor(kGray+2);
        h_mMass[KEY_QA]->SetTitle(Form("%.1f<y<%.1f, Cent (%d-%d), %d/7<|cos(#theta*)|<%d/7",vmsa::eta_bin[i_eta],vmsa::eta_bin[i_eta+1],centlow[icent],centhigh[icent],itheta-7,itheta-6));
        h_mMass[KEY_QA]->DrawCopy("pE");
        // cout << "KEY_QA = " << KEY_QA.c_str() << endl;
  ...
  string outputfile = Form("../output/AuAu%s/%s/InvMassSubBgEta%d_%s_Signed.root",vmsa::mBeamEnergy[energy].c_str(),vmsa::mPID[pid].c_str(),i_eta,etamode.c_str());
  if(order == 1) outputfile = Form("../output/AuAu%s/%s/InvMassSubBgEta%d_%s_Signed_FirstOrder.root",vmsa::mBeamEnergy[energy].c_str(),vmsa::mPID[pid].c_str(),i_eta,etamode.c_str());
