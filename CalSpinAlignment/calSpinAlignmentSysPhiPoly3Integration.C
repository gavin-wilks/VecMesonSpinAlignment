...
void calSpinAlignmentSysPhiCentPoly3Integration(int energy = 4, int pid = 0, int year = 0, bool random3D = false, int centQA = 4, int ptQA = 0, int dcaQA = 0, int nsigQA = 0, std::string etamode = "eta1p5_eta1p5")
{
      ...
      for(Int_t i_dca = vmsa::Dca_start; i_dca < vmsa::Dca_stop; i_dca++)
      {
        for(Int_t i_sig = vmsa::nSigKaon_start; i_sig < vmsa::nSigKaon_stop; i_sig++)
        {
          ...
          for(int i_norm = vmsa::Norm_start; i_norm < vmsa::Norm_stop; ++i_norm)
          {
            ...
            TF1 *f_bw = new TF1("f_bw",Poly3MBreitWigner,vmsa::BW_Start[pid],vmsa::BW_Stop[pid],7);
...
//#endif
  ...
  */
          ...
          for(int i_sigma = vmsa::Sig_start; i_sigma < vmsa::Sig_stop; ++i_sigma)
          {
            for(int i_method = vmsa::Method_start; i_method < vmsa::Method_stop; ++i_method)
            {
              ...
              for(int i_cent = vmsa::centStart; i_cent < vmsa::centStop; ++i_cent) // Centrality loop
              {
                ...
                for(int i_theta = vmsa::CTS_start; i_theta < vmsa::CTS_stop; ++i_theta) // cos(theta*) loop
                {
                  cout << "cent = " << i_cent << " i_dca = " << i_dca << " i_sig = " << i_sig << " i_norm = " << i_norm << " i_sigma = " << i_sigma << " i_method = " << i_method << " i_pt = " << i_pt << " i_theta = " << i_theta << endl;
                  string KEY = Form("pt_%d_Centrality_%d_CosThetaStar_%d_2nd_Dca_%d_Sig_%d_%s_Norm_%d",i_pt,i_cent,i_theta,i_dca,i_sig,vmsa::mPID[pid].c_str(),i_norm);
                  TF1 *f_bw = new TF1("f_bw",Poly3MBreitWigner,vmsa::BW_Start[pid],vmsa::BW_Stop[pid],7);
                  ...
                  f_bw->SetParameter(5,Par_InteTheta[KEY_InteTheta][6]);
                  f_bw->SetRange(vmsa::BW_Start[pid],vmsa::BW_Stop[pid]);
                  TFitResultPtr result = h_mMass[KEY]->Fit(f_bw,"MQNRS");
                
		  TF1 *f_bg = new TF1("f_bg",Poly3M,vmsa::BW_Start[pid],vmsa::BW_Stop[pid],5);
                  TMatrixDSym covArr(5);
                  double params[5];//
                  ...
                  { 
                    ...
                    f_bg->FixParameter(4,f_bw->GetParameter(0));
                    ...
                    covArr(0,4) = result->GetCovarianceMatrix()(3,0);
                    ...
                    covArr(1,4) = result->GetCovarianceMatrix()(4,0);
                    ...
                    covArr(2,4) = result->GetCovarianceMatrix()(5,0);
                    ...
                    covArr(3,4) = result->GetCovarianceMatrix()(6,0);
                    covArr(4,0) = result->GetCovarianceMatrix()(0,3);
                    covArr(4,1) = result->GetCovarianceMatrix()(0,4);
                    covArr(4,2) = result->GetCovarianceMatrix()(0,5);
                    covArr(4,3) = result->GetCovarianceMatrix()(0,6);
                    covArr(4,4) = result->GetCovarianceMatrix()(0,0);
                  }
                  else
  		  { 
                    skip_y = true;  
		    ...
		    //f_bg->SetParError(3,1.0);
                    ...
                    params[4] = 0.0;//result->GetParams()[5];
                    covArr(0,0) = 1.0;//result->GetCovarianceMatrix()(3,3);
                    covArr(0,1) = 0.0;//result->GetCovarianceMatrix()(3,4);
                    covArr(0,2) = 0.0;//result->GetCovarianceMatrix()(3,5);
                    covArr(0,3) = 0.0;//result->GetCovarianceMatrix()(3,6);
                    covArr(0,4) = 0.0;//result->GetCovarianceMatrix()(3,0);
                    covArr(1,0) = 0.0;//result->GetCovarianceMatrix()(4,3);
                    covArr(1,1) = 1.0;//result->GetCovarianceMatrix()(4,4);
                    covArr(1,2) = 0.0;//result->GetCovarianceMatrix()(4,5);
                    covArr(1,3) = 0.0;//result->GetCovarianceMatrix()(4,6);
                    covArr(1,4) = 0.0;//result->GetCovarianceMatrix()(4,0);
                    covArr(2,0) = 0.0;//result->GetCovarianceMatrix()(5,3);
                    covArr(2,1) = 0.0;//result->GetCovarianceMatrix()(5,4);
                    covArr(2,2) = 1.0;//result->GetCovarianceMatrix()(5,5);
                    covArr(2,3) = 0.0;//result->GetCovarianceMatrix()(5,6);
                    covArr(2,4) = 0.0;//result->GetCovarianceMatrix()(5,0);
                    covArr(3,0) = 0.0;//result->GetCovarianceMatrix()(6,3);
                    covArr(3,1) = 0.0;//result->GetCovarianceMatrix()(6,4);
                    covArr(3,2) = 0.0;//result->GetCovarianceMatrix()(6,5);
                    covArr(3,3) = 1.0;//result->GetCovarianceMatrix()(6,6);
                    covArr(3,4) = 0.0;//result->GetCovarianceMatrix()(6,0);
                    covArr(4,0) = 0.0;//result->GetCovarianceMatrix()(0,3);
                    covArr(4,1) = 0.0;//result->GetCovarianceMatrix()(0,4);
                    covArr(4,2) = 0.0;//result->GetCovarianceMatrix()(0,5);
                    covArr(4,3) = 0.0;//result->GetCovarianceMatrix()(0,6);
                    covArr(4,4) = 1.0;//result->GetCovarianceMatrix()(0,0);
  ...
  {
    string outputname = Form("./figures/%s/%s/allThetaYields%s_cent%d_%s_CentDependence.pdf",vmsa::mPID[pid].c_str(),vmsa::mBeamEnergy[energy].c_str(),vmsa::mPID[pid].c_str(),i_cent,etamode.c_str());
    ...
    for(int i_pt = vmsa::pt_rebin_first_cent[energy]; i_pt <= vmsa::pt_rebin_last_cent[energy]; ++i_pt) // pt loop
    {
      ...
      for(int i_theta = 0; i_theta < 9; ++i_theta)
      {
        ...
        if(i_theta < vmsa::CTS_stop)
        {
          string KEY_QA = Form("pt_%d_Centrality_%d_CosThetaStar_%d_2nd_Dca_%d_Sig_%d_%s_Norm_%d",i_pt,i_cent,i_theta,vmsa::Dca_start,vmsa::nSigKaon_start,vmsa::mPID[pid].c_str(),vmsa::Norm_QA);
          h_mMass[KEY_QA]->SetTitle("");
          h_mMass[KEY_QA]->GetXaxis()->SetNdivisions(505,'N');
          h_mMass[KEY_QA]->GetXaxis()->SetLabelSize(0.03);
          h_mMass[KEY_QA]->GetXaxis()->SetTitle("M(K^{+},K^{-})");
          h_mMass[KEY_QA]->SetTitle(Form("%.2f < p_{T} < %.2f",vmsa::pt_low_cent[energy][i_pt],vmsa::pt_up_cent[energy][i_pt]));
          h_mMass[KEY_QA]->GetXaxis()->SetTitleSize(0.05);
          h_mMass[KEY_QA]->GetXaxis()->SetTitleOffset(1.2);
          h_mMass[KEY_QA]->GetXaxis()->CenterTitle();
          ...
          string KEY_InteTheta = Form("pt_%d_Centrality_%d_2nd_Dca_%d_Sig_%d_%s_Norm_%d",i_pt,i_cent,vmsa::Dca_start,vmsa::nSigKaon_start,vmsa::mPID[pid].c_str(),vmsa::Norm_QA);
        ...
        if(i_theta == vmsa::CTS_stop)
        {
          string KEY_InteTheta_QA = Form("pt_%d_Centrality_%d_2nd_Dca_%d_Sig_%d_%s_Norm_%d",i_pt,i_cent,vmsa::Dca_start,vmsa::nSigKaon_start,vmsa::mPID[pid].c_str(),vmsa::Norm_QA);
          h_mMass_InteTheta[KEY_InteTheta_QA]->SetTitle("");
          h_mMass_InteTheta[KEY_InteTheta_QA]->GetXaxis()->SetNdivisions(505,'N');
          h_mMass_InteTheta[KEY_InteTheta_QA]->GetXaxis()->SetLabelSize(0.03);
          h_mMass_InteTheta[KEY_InteTheta_QA]->GetXaxis()->SetTitle("M(K^{+},K^{-})");
      ...
      for(int i_theta = vmsa::CTS_start; i_theta < vmsa::CTS_stop; ++i_theta)
      {
        ...
        f_bg->SetParameter(3,Par[KEY_QA][1]);
        f_bg->SetLineColor(4);
        f_bg->SetLineStyle(2);
        f_bg->SetLineWidth(2);
        f_bg->Draw("l same");
      ...
      for(int i_sigma = vmsa::Sig_start; i_sigma < /*vmsa::Sig_stop*/1; ++i_sigma)
      {
        ...
        {
          string KEY_counts_QA = Form("pt_%d_Centrality_%d_2nd_Dca_%d_Sig_%d_%s_Norm_%d_Sigma_%d_%s",i_pt,i_cent,vmsa::Dca_start,vmsa::nSigKaon_start,vmsa::mPID[pid].c_str(),vmsa::Norm_QA,i_sigma,vmsa::mInteMethod[i_method].c_str());
    ...
    for(Int_t i_sig = vmsa::nSigKaon_start; i_sig < vmsa::nSigKaon_stop; i_sig++)
    {
      ...
      for(int i_norm = vmsa::Norm_start; i_norm < vmsa::Norm_stop; ++i_norm)
      {
        for(int i_sigma = vmsa::Sig_start; i_sigma < vmsa::Sig_stop; ++i_sigma)
        {
          for(int i_method = vmsa::Method_start; i_method < vmsa::Method_stop; ++i_method)
          {
            string KEY_rho = Form("rhoRaw_pt_%d_2nd_Dca_%d_Sig_%d_%s_Norm_%d_Sigma_%d_%s",ptQA,i_dca,i_sig,vmsa::mPID[pid].c_str(),i_norm,i_sigma,vmsa::mInteMethod[i_method].c_str());
  //          cout << KEY_rho << endl;
