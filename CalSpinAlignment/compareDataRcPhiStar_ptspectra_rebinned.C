...
void compareDataRcPhiStar_ptspectra_rebinned(Int_t energy = 4, Int_t pid = 0, Int_t i_cent = 9, string correction = "Raw", int order = 2, string etamode = "eta1_eta1", int yspectra = 76, string sim = "Embed")//defaultF = 0 is BESII, defaultF = 1 is BESI
{
  std::string filename = ""; 
  std::string spectra = "";
  ...
  if(yspectra == 47)
  {
    spectra = "Pythia_PIDEff_GlobalRhoY_20240731";
    filename = "Eff_19GeV_SingleParticle_noToF_Mode0_EtaMode0_PtBins2_5.root"; 
  }
  if(yspectra == 48)
  {
    spectra = "Pythia_PhiMesonWeights_20240731";
    filename = "Eff_19GeV_SingleParticle_noToF_Mode0_EtaMode0_PtBins2_5.root"; 
  }
  if(yspectra == 49)
  {
    spectra = "Pythia_NoWeights_20240731";
    filename = "Eff_19GeV_SingleParticle_noToF_Mode0_EtaMode0_PtBins2_5.root"; 
  }
  if(yspectra == 50)
  {
    spectra = "Pythia_PhiMesonWeights_20240731_Retry";
    filename = "Eff_19GeV_SingleParticle_noToF_Mode0_EtaMode0_PtBins2_5.root"; 
  }
  if(yspectra == 51)
  {
    spectra = "Pythia_PhiMesonWeights_KaonWeights_20240731";
    filename = "Eff_19GeV_SingleParticle_noToF_Mode0_EtaMode0_PtBins2_5.root"; 
  }
  if(yspectra == 52)
  {
    spectra = "Pythia_PhiMesonWeights_20240801_Helicity2D";
    filename = "Eff_19GeV_SingleParticle_noToF_Mode0_EtaMode0_PtBins2_5.root"; 
  }
  if(yspectra == 60)
  {
    spectra = "PhiEmbedding_20240731_Reweight";
    filename = "PhiEmbed_WithWeights_20240731_Reweight.root"; 
  }
  if(yspectra == 61)
  {
    spectra = "Pythia_20240808_Helicity2DRapidity";
    filename = "Eff_19GeV_SingleParticle_noToF_Mode4_EtaMode0_PtBins2_5.root"; 
  }
  if(yspectra == 62)
  {
    spectra = "KaonOnlyReweight";
    filename = "Eff_19GeV_SingleParticle_noToF_Mode0_EtaMode0_PtBins2_5.root"; 
  }
  if(yspectra == 63)
  {
    spectra = "Pythia_20240808_EtaCutFirst";
    filename = "Eff_19GeV_SingleParticle_noToF_Mode0_EtaMode0_PtBins2_5.root"; 
  }
  if(yspectra == 64)
  {
    spectra = "PhiEmbeddingWithWeights_PIDEff";
    filename = "PhiEmbed_Weights_PIDEff_1.root"; 
  }
  if(yspectra == 65)
  {
    spectra = "Pythia_Helicity2DPt_20240813";
    filename = "Eff_19GeV_SingleParticle_noToF_Mode0_EtaMode0_PtBins2_5.root"; 
  }
  if(yspectra == 66)
  {
    spectra = "Pythia_Helicity2DY_20240813";
    filename = "Eff_19GeV_SingleParticle_noToF_Mode0_EtaMode0_PtBins2_5.root"; 
  }
  if(yspectra == 67)
  {
  ...
  if(yspectra == 72)
  {
    spectra = "PhiEmbedding_20240822EP_phiswitch0";
    filename = "PhiEmbedding_20240822_FinerToFEta_phiswitch0.root"; 
  }
  if(yspectra == 73)
  {
    spectra = "PhiEmbedding_FuncEmbedWeight";
    //filename = "PhiEmbedding_FuncEmbedWeight.root"; 
    filename = "PhiEmbedding_FuncEmbedWeight_20240826.root"; 
  }
  if(yspectra == 74)
  {
    spectra = "PhiEmbedding_FuncEmbedWeight_20240827_FinerToFEta_phiswitch0";
    filename = "PhiEmbedding_FuncEmbedWeight_20240827_FinerToFEta_phiswitch0.root"; 
  }
  if(yspectra == 75)
  {
    spectra = "PhiEmbedding_FuncEmbedWeight_20240827_FinerToFEta_phiswitch1";
    filename = "PhiEmbedding_FuncEmbedWeight_20240827_FinerToFEta_phiswitch1.root"; 
  }
  if(yspectra == 76)
  {
    spectra = "PhiEmbedding_weights_nomccuts_rapidity";
    filename = "PhiEmbedding_weights_nomccuts_rapidity.root"; 
  }
  ...
  //h_mCounts->RebinY(2);
  string KEY_counts_RC = "h_mRc9EffPtY_Cent_9";
  //string KEY_counts_RC = "h_mRcEffPtY_Cent_9";
  h_mCounts_RC_Before = (TH2D*) File_InPutRC->Get(KEY_counts_RC.c_str());
  cout << "NBinsX Before = " << h_mCounts_RC_Before->GetNbinsX() << endl;
  cout << "NBinsY Before = " << h_mCounts_RC_Before->GetNbinsY() << endl;
  h_mCounts_RC = new TH2D("RC","RC",vmsa::rebinpttotal,vmsa::rebinptval,vmsa::rebinytotal,vmsa::rebinyval);

  double val_total[vmsa::rebinpttotal][vmsa::rebinytotal] = {0.0}; 
  double errsq_total[vmsa::rebinpttotal][vmsa::rebinytotal] = {0.0};
  for(int ptbin = 0; ptbin < vmsa::rebinpttotal; ptbin++)
  {
    for(int ipt = vmsa::rebinpt[ptbin]; ipt < vmsa::rebinpt[ptbin+1]; ipt++)
    {
      for(int ybin = 0; ybin < vmsa::rebinytotal; ybin++)
      {
        for(int iy = vmsa::rebiny[ybin]; iy < vmsa::rebiny[ybin+1]; iy++)
        {
          double val = h_mCounts_RC_Before->GetBinContent(ipt+1,iy+1);
          double err = h_mCounts_RC_Before->GetBinError(ipt+1,iy+1);

          val_total[ptbin][ybin] += val;
          errsq_total[ptbin][ybin] += err*err;

          cout << "value = " << val_total[ptbin][ybin] << endl;
          cout << "errsq = " << errsq_total[ptbin][ybin] << endl;
    
          if(ipt == vmsa::rebinpt[ptbin+1]-1 && iy == vmsa::rebiny[ybin+1]-1)
          {
            cout << "Fill ipt = " << ipt << ", iy = " << iy << endl;
            h_mCounts_RC->SetBinContent(ptbin+1,ybin+1,val_total[ptbin][ybin]);   
            h_mCounts_RC->SetBinError(ptbin+1,ybin+1,TMath::Sqrt(errsq_total[ptbin][ybin]));   
          }
        }
      }
    }
  ...
  leg->AddEntry(h_mCounts_pt,"Data","p");
  leg->AddEntry(h_mCounts_RC_pt,sim.c_str(),"p");

  h_mCounts_pt->Draw("pE");
  h_mCounts_RC_pt->Draw("pE same");
  leg->Draw("same");  

  c->SaveAs(Form("figures/%s/%s/pTstudy/%s/Cos2PhiStarPhi_ptspectra_%s_Order%d_%s_%s.pdf",vmsa::mPID[pid].c_str(),vmsa::mBeamEnergy[energy].c_str(),spectra.c_str(),correction.c_str(),order,etamode.c_str(),sim.c_str()));  
  ...
  legy->AddEntry(h_mCounts_y,"Data","p");
  legy->AddEntry(h_mCounts_RC_y,sim.c_str(),"p");

  h_mCounts_y->Draw("pE");
  h_mCounts_RC_y->Draw("pE same");
  legy->Draw("same");  

  c->SaveAs(Form("figures/%s/%s/pTstudy/%s/Cos2PhiStarPhi_yspectra_%s_Order%d_%s_%s.pdf",vmsa::mPID[pid].c_str(),vmsa::mBeamEnergy[energy].c_str(),spectra.c_str(),correction.c_str(),order,etamode.c_str(),sim.c_str()));  
  ...
  c->SaveAs(Form("figures/%s/%s/pTstudy/%s/Cos2PhiStarPhi_ptyspectra2Dratio_%s_Order%d_%s_%s.pdf",vmsa::mPID[pid].c_str(),vmsa::mBeamEnergy[energy].c_str(),spectra.c_str(),correction.c_str(),order,etamode.c_str(),sim.c_str()));  
  ...
  h_mCounts_Ratio_pt->GetYaxis()->SetRangeUser(0.0,1.5);
  h_mCounts_Ratio_pt->Draw("pE");
  
  c->SaveAs(Form("figures/%s/%s/pTstudy/%s/Cos2PhiStarPhi_ptspectra1Dratio_%s_Order%d_%s_%s.pdf",vmsa::mPID[pid].c_str(),vmsa::mBeamEnergy[energy].c_str(),spectra.c_str(),correction.c_str(),order,etamode.c_str(),sim.c_str()));  
  ...
  c->SaveAs(Form("figures/%s/%s/pTstudy/%s/Cos2PhiStarPhi_yspectra1Dratio_%s_Order%d_%s_%s.pdf",vmsa::mPID[pid].c_str(),vmsa::mBeamEnergy[energy].c_str(),spectra.c_str(),correction.c_str(),order,etamode.c_str(),sim.c_str()));  

  string outputfile = "ptyspectra_datarcratio_19GeV_Helicity2D.root";
  ...
  for(int ipt = 0; ipt < vmsa::rebinpttotal; ipt++)
  {
    ...
    h_mCounts_ybins[ipt]->SetTitle(Form("%1.3f<p_{T}<%1.3f, 20-60 Centrality",vmsa::rebinptval[ipt],vmsa::rebinptval[ipt+1]));
    ...
    TLegend *leg = new TLegend(0.6,0.5,0.8,0.7);
    leg->AddEntry(h_mCounts_ybins[ipt],"Data","p");
    leg->AddEntry(h_mCounts_RC_ybins[ipt],sim.c_str(),"p");

    h_mCounts_ybins[ipt]->Draw("pE");
    h_mCounts_RC_ybins[ipt]->Draw("pE same");
    leg->Draw("same");  
  }
  cy->SaveAs(Form("figures/%s/%s/pTstudy/%s/Cos2PhiStarPhi_ptspectra_ybincomp_%s_Order%d_%s_%s.pdf",vmsa::mPID[pid].c_str(),vmsa::mBeamEnergy[energy].c_str(),spectra.c_str(),correction.c_str(),order,etamode.c_str(),sim.c_str()));  

  for(int ipt = 0; ipt < vmsa::rebinpttotal; ipt++)
  {
    cy->cd(ipt+1);
    h_mCounts_Ratio_ybins[ipt]->SetTitle(Form("%1.3f<p_{T}<%1.3f, 20-60 Centrality",vmsa::rebinptval[ipt],vmsa::rebinptval[ipt+1]));
  ...
  cy->SaveAs(Form("figures/%s/%s/pTstudy/%s/Cos2PhiStarPhi_ptspectraratio_ybincomp_%s_Order%d_%s_%s.pdf",vmsa::mPID[pid].c_str(),vmsa::mBeamEnergy[energy].c_str(),spectra.c_str(),correction.c_str(),order,etamode.c_str(),sim.c_str()));  
