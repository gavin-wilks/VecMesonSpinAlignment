...
void calSpinAlignmentKStar_BinByBin(int energy = 4, int pid = 2, int year = 0, double nSigVec = 2.0, int ptQA = 0)
{
  ...
  for(int i_cent = vmsa::Cent_start; i_cent < vmsa::Cent_stop; i_cent++) // Centrality loop
  {
	...
	for(int i_theta = vmsa::CTS_start; i_theta < vmsa::CTS_stop; i_theta++) // cos(theta*) loop
	{
	  ...
	  h_mCounts[KEY_BW]->SetBinError(h_mCounts[KEY_BW]->FindBin(bin_center),errors_bw);
          if (i_cent == 9)
          {
            //aveSBRbw+=counts_bw/TMath::Sqrt(SBRcounts);
            //if (counts_bw/TMath::Sqrt(SBRcounts) < lowSigBW) lowSigBW = counts_bw/TMath::Sqrt(SBRcounts);
            //aveSBRcount+=counts/TMath::Sqrt(SBRcounts);
            //if (counts/TMath::Sqrt(SBRcounts) < lowSigCount) lowSigCount = counts/TMath::Sqrt(SBRcounts);
