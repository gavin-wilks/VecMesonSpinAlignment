...
void subBackGroundCent_Global_2D_OffDiag(int energy = 4, int pid = 0, int year = 0, string date = "20241017", bool random3D = false, int order = 2, std::string etamode = "eta1_eta1", int frameopt = 0)
{
  ...
  {
    for(Int_t i_cent = vmsa::centStart; i_cent < vmsa::centStop; i_cent++) // centrality bin
    {
        ...
        for(Int_t i_phi = 0; i_phi < 12; i_phi++) // phi-psi bin
        {
          for(Int_t i_dca = vmsa::Dca_start; i_dca < 1/*vmsa::Dca_stop*/; i_dca++)
          {
            for(Int_t i_sig = vmsa::nSigKaon_start; i_sig < 1/*vmsa::nSigKaon_stop*/; i_sig++)
            {
              ...
              string KEY_InPutSE = Form("pt_%d_Centrality_%d_CosThetaStar_%d_PhiPrime_%d_%s_Dca_%d_Sig_%d_%s_SE",i_pt,i_cent,i_theta,i_phi,EP[order-1].c_str(),i_dca,i_sig,vmsa::mPID[pid].c_str());
              h_mInPut_SE[KEY_InPutSE] = (TH1F*)File_SE->Get(KEY_InPutSE.c_str())->Clone(); 
              h_mInPut_SE[KEY_InPutSE]->Rebin(2);               

              string KEY_InPutME = Form("pt_%d_Centrality_%d_CosThetaStar_%d_PhiPrime_%d_%s_Dca_%d_Sig_%d_%s_ME",i_pt,i_cent,i_theta,i_phi,EP[order-1].c_str(),i_dca,i_sig,vmsa::mPID[pid].c_str());
              ...
              {
                string KEY_SE = Form("pt_%d_Centrality_%d_CosThetaStar_%d_PhiPrime_%d_%s_Dca_%d_Sig_%d_%s_Norm_%d_SE",i_pt,i_cent,i_theta,i_phi,EP[order-1].c_str(),i_dca,i_sig,vmsa::mPID[pid].c_str(),i_norm);
                h_mMass_SE[KEY_SE] = (TH1F*)h_mInPut_SE[KEY_InPutSE]->Clone(KEY_SE.c_str());
                string KEY_ME = Form("pt_%d_Centrality_%d_CosThetaStar_%d_PhiPrime_%d_%s_Dca_%d_Sig_%d_%s_Norm_%d_ME",i_pt,i_cent,i_theta,i_phi,EP[order-1].c_str(),i_dca,i_sig,vmsa::mPID[pid].c_str(),i_norm);
                h_mMass_ME[KEY_ME] = (TH1F*)h_mInPut_ME[KEY_InPutME]->Clone(KEY_ME.c_str());
                string KEY_SM = Form("pt_%d_Centrality_%d_CosThetaStar_%d_PhiPrime_%d_%s_Dca_%d_Sig_%d_%s_Norm_%d_SM",i_pt,i_cent,i_theta,i_phi,EP[order-1].c_str(),i_dca,i_sig,vmsa::mPID[pid].c_str(),i_norm);
  ...
  //string KEY_SE_QA = Form("pt_%d_Centrality_%d_CosThetaStar_%d_PhiPrime_%d_%s_Dca_%d_Sig_%d_%s_Norm_%d_SE",vmsa::pt_RawQA[energy],vmsa::centStart,vmsa::CTS_start,5,EP[order-1].c_str(),vmsa::Dca_start,vmsa::nSigKaon_start,vmsa::mPID[pid].c_str(),vmsa::Norm_QA);
  ...
  //string KEY_ME_QA = Form("pt_%d_Centrality_%d_CosThetaStar_%d_PhiPrime_%d_%s_Dca_%d_Sig_%d_%s_Norm_%d_ME",vmsa::pt_RawQA[energy],vmsa::centStart,vmsa::CTS_start,5,EP[order-1].c_str(),vmsa::Dca_start,vmsa::nSigKaon_start,vmsa::mPID[pid].c_str(),vmsa::Norm_QA);
  ...
  //string KEY_SM_QA = Form("pt_%d_Centrality_%d_CosThetaStar_%d_PhiPrime_%d_%s_Dca_%d_Sig_%d_%s_Norm_%d_SM",vmsa::pt_RawQA[energy],vmsa::centStart,vmsa::CTS_start,5,EP[order-1].c_str(),vmsa::Dca_start,vmsa::nSigKaon_start,vmsa::mPID[pid].c_str(),vmsa::Norm_QA);
  ...
  //string outputname = Form("figures/%s/%s/centralitystudy/%s2D_InvMassDistributions_%s_Order%d.pdf",vmsa::mPID[pid].c_str(),vmsa::mBeamEnergy[energy].c_str(),frame.c_str(),vmsa::mBeamEnergy[energy].c_str(),order);
  ...
  //        string KEY_SE_QA = Form("pt_%d_Centrality_%d_CosThetaStar_%d_PhiPrime_%d_%s_Dca_%d_Sig_%d_%s_Norm_%d_SE",i_pt,icent,itheta,iphi,EP[order-1].c_str(),vmsa::Dca_start,vmsa::nSigKaon_start,vmsa::mPID[pid].c_str(),vmsa::Norm_QA);
  //        //cout << KEY_SE_QA << endl;
  //        h_mMass_SE[KEY_SE_QA]->GetYaxis()->SetRangeUser(-0.1*h_mMass_SE[KEY_SE_QA]->GetMaximum(),1.1*h_mMass_SE[KEY_SE_QA]->GetMaximum());
  //        h_mMass_SE[KEY_SE_QA]->SetTitle(Form("Cent %d, %1.1f/4.5<cos(#theta*)<%1.1f/4.5, %d#pi/6<#beta<%d#pi/6",icent,float(itheta)-4.5,float(itheta)-3.5,iphi,iphi+1));
  //        h_mMass_SE[KEY_SE_QA]->DrawCopy();

  //        string KEY_ME_QA = Form("pt_%d_Centrality_%d_CosThetaStar_%d_PhiPrime_%d_%s_Dca_%d_Sig_%d_%s_Norm_%d_ME",i_pt,icent,itheta,iphi,EP[order-1].c_str(),vmsa::Dca_start,vmsa::nSigKaon_start,vmsa::mPID[pid].c_str(),vmsa::Norm_QA);
  //        //cout << KEY_ME_QA << endl;
  //        h_mMass_ME[KEY_ME_QA]->SetLineColor(2);
  //        h_mMass_ME[KEY_ME_QA]->SetFillColor(2);
  //        h_mMass_ME[KEY_ME_QA]->SetFillStyle(3002);
  ...
  //        string KEY_SM_QA = Form("pt_%d_Centrality_%d_CosThetaStar_%d_PhiPrime_%d_%s_Dca_%d_Sig_%d_%s_Norm_%d_SM",i_pt,icent,itheta,iphi,EP[order-1].c_str(),vmsa::Dca_start,vmsa::nSigKaon_start,vmsa::mPID[pid].c_str(),vmsa::Norm_QA);
  //        //cout << KEY_SM_QA << endl;
  //        h_mMass_SM[KEY_SM_QA]->SetLineColor(4);
  //        h_mMass_SM[KEY_SM_QA]->SetFillColor(4);
  //        h_mMass_SM[KEY_SM_QA]->SetFillStyle(3004);
      ...
      for(int i_phi = 0; i_phi < 12; i_phi++) // cos(theta*) loop
      {
        for(Int_t i_dca = vmsa::Dca_start; i_dca < 1/*vmsa::Dca_stop*/; i_dca++)
        {
          for(Int_t i_sig = vmsa::nSigKaon_start; i_sig < 1/*vmsa::nSigKaon_stop*/; i_sig++)
          {
            ...
            for(int i_norm = vmsa::Norm_start; i_norm < 1/*vmsa::Norm_stop*/; ++i_norm)
            {
              for(int pt_bin = vmsa::pt_rebin_first_cent_2D[energy]; pt_bin < vmsa::pt_rebin_last_cent_2D[energy]; pt_bin++) // pt loop
              {
                //cout << "pt bin " << pt_bin << " = [" << vmsa::ptRawStart[vmsa::pt_rebin_start_cent_2D[energy][pt_bin]] << ", " << vmsa::ptRawStop[vmsa::pt_rebin_stop_cent_2D[energy][pt_bin]] << "]" << endl;
                string KEY = Form("pt_%d_Centrality_%d_CosThetaStar_%d_PhiPrime_%d_%s_Dca_%d_Sig_%d_%s_Norm_%d",pt_bin,i_cent,i_theta,i_phi,EP[order-1].c_str(),i_dca,i_sig,vmsa::mPID[pid].c_str(),i_norm);
                for(int i_pt = vmsa::pt_rebin_start_cent_2D[energy][pt_bin]; i_pt < vmsa::pt_rebin_stop_cent_2D[energy][pt_bin]; i_pt++)
                {
                  //cout << "ipt = [" << vmsa::ptRawStart[i_pt] << ", " << vmsa::ptRawStop[i_pt] << "]" << endl;
          	  string KEY_SM = Form("pt_%d_Centrality_%d_CosThetaStar_%d_PhiPrime_%d_%s_Dca_%d_Sig_%d_%s_Norm_%d_SM",i_pt,i_cent,i_theta,i_phi,EP[order-1].c_str(),i_dca,i_sig,vmsa::mPID[pid].c_str(),i_norm);
          	  if(i_pt == vmsa::pt_rebin_start_cent_2D[energy][pt_bin])
          	  {
  ...
  //outputname = Form("figures/%s/%s/centralitystudy/%s2D_InvMassBackgroundSubtracted_Rebinned_%s_Order%d.pdf",vmsa::mPID[pid].c_str(),vmsa::mBeamEnergy[energy].c_str(),frame.c_str(),vmsa::mBeamEnergy[energy].c_str(),order);
  ...
  //        string KEY_QA = Form("pt_%d_Centrality_%d_CosThetaStar_%d_PhiPrime_%d_%s_Dca_%d_Sig_%d_%s_Norm_%d",i_pt,icent,itheta,iphi,EP[order-1].c_str(),vmsa::Dca_start,vmsa::nSigKaon_start,vmsa::mPID[pid].c_str(),vmsa::Norm_QA);
  //        cout << KEY_QA << endl;
  //        h_mMass[KEY_QA]->SetMarkerStyle(24);
  //        h_mMass[KEY_QA]->SetMarkerSize(1.2);
  //        h_mMass[KEY_QA]->SetLineColor(kGray+2);
  //        h_mMass[KEY_QA]->SetTitle(Form("Cent %d, %1.1f/4.5<cos(#theta*)<%1.1f/4.5, %d#pi/6<#beta<%d#pi/6",icent,float(itheta)-4.5,float(itheta)-3.5,iphi,iphi+1));
  //        h_mMass[KEY_QA]->DrawCopy("pE");
  //        // cout << "KEY_QA = " << KEY_QA.c_str() << endl;

  //        string pT_range = Form("[%.2f,%.2f]",vmsa::pt_low_cent_2D[energy][i_pt],vmsa::pt_up_cent_2D[energy][i_pt]);
  //        plotTopLegend((char*)pT_range.c_str(),0.2,0.7,0.08,1,0.0,42,1);
  ...
  for(int i_cent = vmsa::centStart; i_cent < vmsa::centStop; i_cent++) // Centrality loop
  {
    for(Int_t i_dca = vmsa::Dca_start; i_dca < vmsa::Dca_stop; i_dca++)
    {
      ...
      { 
        ...
        {
          string KEY_theta = Form("Centrality_%d_2nd_Dca_%d_Sig_%d_%s_Norm_%d",i_cent,i_dca,i_sig,vmsa::mPID[pid].c_str(),i_norm);
          for(int i_theta = vmsa::CTS_start; i_theta < vmsa::CTS_stop; i_theta++) // cos(theta*) loop
          {
            string KEY = Form("Centrality_%d_CosThetaStar_%d_PhiPrime_%d_2nd_Dca_%d_Sig_%d_%s_Norm_%d",i_cent,i_theta,i_dca,i_sig,vmsa::mPID[pid].c_str(),i_norm);
...
*/
  ...
  for(int i_pt = vmsa::pt_rebin_first[energy]; i_pt < vmsa::pt_rebin_last[energy]; i_pt++) // pt loop
  {
   ...
   /* TCanvas *c_mMass_phi_diff = new TCanvas("c_mMass_phi_diff","c_mMass_phi_diff",10,10,1400,1400);
    ...
    for(int i_pt = vmsa::pt_rebin_first[energy]; i_pt < vmsa::pt_rebin_last[energy]; i_pt++) // pt loop
    {
      ...
      string KEY_QA = Form("pt_%d_Centrality_%d_CosThetaStar_%d_PhiPrime_%d_2nd_Dca_%d_Sig_%d_%s_Norm_%d",i_pt,vmsa::centStart,vmsa::CTS_start,vmsa::Dca_start,vmsa::nSigKaon_start,vmsa::mPID[pid].c_str(),vmsa::Norm_QA);
  ...
  string outputfile = Form("../output/AuAu%s/%s/%s2DCent_InvMassSubBgCent_%s.root",vmsa::mBeamEnergy[energy].c_str(),vmsa::mPID[pid].c_str(),frame.c_str(),etamode.c_str());
  if(order == 1) outputfile = Form("../output/AuAu%s/%s/%s2DCent_InvMassSubBgCent_%s_FirstOrder.root",vmsa::mBeamEnergy[energy].c_str(),vmsa::mPID[pid].c_str(),frame.c_str(),etamode.c_str());
