...
void calSpinAlignmentSysPhiP_Global_2D_OffDiag(int energy = 4, int pid = 0, int year = 0, bool random3D = false, int order = 2, string etamode = "eta1_eta1", int frameopt = 1)
{
  ...
  string inputfile = Form("../output/AuAu%s/%s/%s2DP_InvMassSubBg_%s.root",vmsa::mBeamEnergy[energy].c_str(),vmsa::mPID[pid].c_str(),frame.c_str(),etamode.c_str());
  if(order == 1) inputfile = Form("../output/AuAu%s/%s/%s2DP_InvMassSubBg_%s_FirstOrder.root",vmsa::mBeamEnergy[energy].c_str(),vmsa::mPID[pid].c_str(),frame.c_str(),etamode.c_str());
  ...
  for(int i_pt = vmsa::p_rebin_first_2D[energy]; i_pt < vmsa::p_rebin_last_2D[energy]; i_pt++) // pt loop
  {
	...
	for(Int_t i_sig = vmsa::nSigKaon_start; i_sig < vmsa::nSigKaon_stop; i_sig++)
	{
	  ...
	  {
	    string KEY_InteTheta = Form("p_%d_Centrality_%d_%s_Dca_%d_Sig_%d_%s_Norm_%d",i_pt,i_cent,EP[order-1].c_str(),i_dca,i_sig,vmsa::mPID[pid].c_str(),i_norm);
	    for(int i_theta = vmsa::CTS_start; i_theta < 9; i_theta++) // cos(theta*) loop
	    {
	      for(int i_phipsi = 0; i_phipsi < 12; i_phipsi++) // cos(theta*) loop
	      {
            ...
            {
	      string KEY_InteTheta_Poly = Form("p_%d_Centrality_%d_%s_Dca_%d_Sig_%d_%s_Norm_%d_Poly%d",i_pt,i_cent,EP[order-1].c_str(),i_dca,i_sig,vmsa::mPID[pid].c_str(),i_norm,i_poly+1);
	      string KEY_Poly = Form("Centrality_%d_%s_Dca_%d_Sig_%d_%s_Norm_%d_Poly%d",i_cent,EP[order-1].c_str(),i_dca,i_sig,vmsa::mPID[pid].c_str(),i_norm,i_poly+1);
              if(i_pt == vmsa::p_rebin_first_2D[energy]) g_mChiNDF[KEY_Poly] = new TGraphAsymmErrors();             
              if(i_pt == vmsa::p_rebin_first_2D[energy]) g_mPValue[KEY_Poly] = new TGraphAsymmErrors();             
	      ...
	      float norm = h_mMass_InteTheta[KEY_InteTheta]->GetMaximum();
	      ...
	      float p_mean = (vmsa::p_low_2D[energy][i_pt]+vmsa::p_up_2D[energy][i_pt])/2.0;
              float chi2NDF = float(f_bw->GetChisquare())/float(f_bw->GetNDF()); 
              float pvalue = TMath::Prob(f_bw->GetChisquare(),f_bw->GetNDF());
              //cout << "Poly " << i_poly + 1 << ", pT = " << p_mean << ", chi2 = " << f_bw->GetChisquare() << ", NDF = " << f_bw->GetNDF() << ", pvalue = " << pvalue << endl;
              if(i_pt >= 2) g_mChiNDF[KEY_Poly]->SetPoint(i_pt,p_mean,chi2NDF);
              if(i_pt >= 2) g_mChiNDF[KEY_Poly]->SetPoint(i_pt,p_mean,pvalue);
...
//    {
//      string KEY_QA = Form("p_%d_Centrality_%d_CosThetaStar_%d_PhiPrime_%d_2nd_Dca_%d_Sig_%d_%s_Norm_%d",vmsa::p_QA[energy],9,i_theta,vmsa::Dca_start,vmsa::nSigKaon_start,vmsa::mPID[pid].c_str(),vmsa::Norm_QA);
...
//    if(i_theta == vmsa::CTS_stop)
//    {
//      string KEY_InteTheta_QA = Form("p_%d_Centrality_%d_2nd_Dca_%d_Sig_%d_%s_Norm_%d",vmsa::p_QA[energy],9,vmsa::Dca_start,vmsa::nSigKaon_start,vmsa::mPID[pid].c_str(),vmsa::Norm_QA);
  ...
  for(int i_pt = vmsa::p_rebin_first[energy]; i_pt < vmsa::p_rebin_last[energy]; i_pt++) // pt loop
  {
	...
	for(int i_norm = vmsa::Norm_start; i_norm < vmsa::Norm_stop; ++i_norm)
	{
	  for(int i_func = vmsa::Func_start; i_func < vmsa::Func_stop; ++i_func)
	  {
	    ...
	    {
	      string KEY = Form("p_%d_Centrality_%d_EtaGap_%d_CosThetaStar_%d_PhiPrime_%d_2nd_%s_Norm_%d_Func_%d",i_pt,i_cent,i_eta,i_theta,vmsa::mPID[pid].c_str(),i_norm,i_func);
	    }
	  }
	}
      }
  ...
  */
	...
	for(int i_norm = vmsa::Norm_start; i_norm < vmsa::Norm_stop; ++i_norm)
	{
	  for(int i_sigma = vmsa::Sig_start; i_sigma < vmsa::Sig_stop; ++i_sigma)
	  {
	    for(int i_method = vmsa::Method_start; i_method < vmsa::Method_stop; ++i_method)
	    {
	        ...
	        for(int i_pt = vmsa::p_rebin_first_2D[energy]; i_pt < vmsa::p_rebin_last_2D[energy]; ++i_pt) // pt loop
	        {
	          string KEY_counts = Form("p_%d_Centrality_%d_%s_Dca_%d_Sig_%d_%s_Norm_%d_Sigma_%d_%s_Poly%d",i_pt,i_cent,EP[order-1].c_str(),i_dca,i_sig,vmsa::mPID[pid].c_str(),i_norm,i_sigma,vmsa::mInteMethod[i_method].c_str(),i_poly+1);
	          ...
	          for(int i_theta = vmsa::CTS_start; i_theta < 9; ++i_theta) // cos(theta*) loop
	          {
	            ...
	            {
	              string KEY = Form("p_%d_Centrality_%d_CosThetaStar_%d_PhiPrime_%d_%s_Dca_%d_Sig_%d_%s_Norm_%d",i_pt,i_cent,i_theta,i_phipsi,EP[order-1].c_str(),i_dca,i_sig,vmsa::mPID[pid].c_str(),i_norm);
	              string KEY_Poly = Form("p_%d_Centrality_%d_CosThetaStar_%d_PhiPrime_%d_%s_Dca_%d_Sig_%d_%s_Norm_%d_Poly%d",i_pt,i_cent,i_theta,i_phipsi,EP[order-1].c_str(),i_dca,i_sig,vmsa::mPID[pid].c_str(),i_norm,i_poly+1);
	              TF1 *f_bw;
                      if(i_poly == 0) f_bw = new TF1("f_bw",PolyBreitWigner, vmsa::BW_Start[pid],vmsa::BW_Stop[pid],5);
                      if(i_poly == 1) f_bw = new TF1("f_bw",Poly2BreitWigner,vmsa::BW_Start[pid],vmsa::BW_Stop[pid],6);
                      if(i_poly == 2) f_bw = new TF1("f_bw",Poly3BreitWigner,vmsa::BW_Start[pid],vmsa::BW_Stop[pid],7);
	              ...
	              TFitResultPtr result = h_mMass[KEY]->Fit(f_bw,"MQNIRS");
	              ...
	              float bin_center_phipsi = float(i_phipsi)*TMath::Pi()/6.+TMath::Pi()/12.;
                      //cout << "cos(theta*) = " << bin_center << ", phi' = " << bin_center_phipsi << endl;
	              if(i_method == 0)
	              {
	          ...
	          h_mCounts[KEY_counts]->Fit(f_rho,"NMRIE");
	          ...
	          g_mRho[KEY_rho]->SetPoint(i_pt,p_mean,f_rho->GetParameter(0)-1./3.);
	          g_mRho[KEY_rho]->SetPointError(i_pt,0.0,0.0,f_rho->GetParError(0),f_rho->GetParError(0));
	          g_mReal[KEY_real]->SetPoint(i_pt,p_mean,f_rho->GetParameter(1));
	          g_mReal[KEY_real]->SetPointError(i_pt,0.0,0.0,f_rho->GetParError(1),f_rho->GetParError(1));
	          g_mImag[KEY_imag]->SetPoint(i_pt,p_mean,f_rho->GetParameter(2));
	          g_mImag[KEY_imag]->SetPointError(i_pt,0.0,0.0,f_rho->GetParError(2),f_rho->GetParError(2));
	          g_mReRho1n1[KEY_rerho1n1]->SetPoint(i_pt,p_mean,f_rho->GetParameter(3));
	          g_mReRho1n1[KEY_rerho1n1]->SetPointError(i_pt,0.0,0.0,f_rho->GetParError(3),f_rho->GetParError(3));
	          g_mImRho1n1[KEY_imrho1n1]->SetPoint(i_pt,p_mean,f_rho->GetParameter(4));
	          g_mImRho1n1[KEY_imrho1n1]->SetPointError(i_pt,0.0,0.0,f_rho->GetParError(4),f_rho->GetParError(4));
	        }
              }
	    }
	  }
  ...
  for(int i_poly = 0; i_poly < 1; i_poly++)
  {
    for(int i_norm = 0; i_norm < 1; i_norm++)
    {
      ...
      {
        string outputname = Form("./figures/%s/%s/pstudy/%s2D_allThetaYields%s_%s_Order%d_cent%d_%s_pTdependence_Norm%d_Poly%d.pdf",vmsa::mPID[pid].c_str(),vmsa::mBeamEnergy[energy].c_str(),frame.c_str(),vmsa::mPID[pid].c_str(),vmsa::mBeamEnergy[energy].c_str(),order,i_cent,etamode.c_str(),i_norm,i_poly+1);
        string output_start = Form("%s[",outputname.c_str());
        
        //TCanvas *c_diff = new TCanvas("c_diff","c_diff",10,10,1200,900);
        TCanvas *c_diff = new TCanvas("c_diff","c_diff",10,10,1500,600);
        ...
        for(int i_pt = vmsa::p_rebin_first_2D[energy]; i_pt < vmsa::p_rebin_last_2D[energy]; ++i_pt) // pt loop
        {
          for(int i_phipsi = 0; i_phipsi < 12; i_phipsi++)
          {
            ...
            for(int i_theta = 0; i_theta < 9; ++i_theta)
            {
              ...
              if(i_theta < 9)
              {
                string KEY_QA = Form("p_%d_Centrality_%d_CosThetaStar_%d_PhiPrime_%d_%s_Dca_%d_Sig_%d_%s_Norm_%d",i_pt,i_cent,i_theta,i_phipsi,EP[order-1].c_str(),vmsa::Dca_start,0/*vmsa::nSigKaon_start*/,vmsa::mPID[pid].c_str(),i_norm);
                h_mMass[KEY_QA]->SetTitle("");
                h_mMass[KEY_QA]->GetXaxis()->SetNdivisions(505,'N');
                h_mMass[KEY_QA]->GetXaxis()->SetLabelSize(0.03);
                h_mMass[KEY_QA]->GetXaxis()->SetTitle("M(K^{+},K^{-})");
                h_mMass[KEY_QA]->SetTitle(Form("%.2f<p_{T}<%.2f, %d#pi/6<#beta<%d#pi/6, %1.1f/4.5<cos(#theta*)<%1.1f/4.5",vmsa::p_low_2D[energy][i_pt],vmsa::p_up_2D[energy][i_pt],i_phipsi,(i_phipsi+1),float(i_theta)-4.5,float(i_theta)-3.5));
                h_mMass[KEY_QA]->GetXaxis()->SetTitleSize(0.05);
                h_mMass[KEY_QA]->GetXaxis()->SetTitleOffset(1.2);
                h_mMass[KEY_QA]->GetXaxis()->CenterTitle();
                ...
                //string KEY_InteTheta = Form("p_%d_Centrality_%d_%s_Dca_%d_Sig_%d_%s_Norm_%d",i_pt,i_cent,EP[order-1].c_str(),vmsa::Dca_start,1/*vmsa::nSigKaon_start*/,vmsa::mPID[pid].c_str(),vmsa::Norm_QA);
              ...
              //if(i_theta == vmsa::CTS_stop)
              //{
              //  string KEY_InteTheta_QA = Form("p_%d_Centrality_%d_%s_Dca_%d_Sig_%d_%s_Norm_%d",i_pt,i_cent,EP[order-1].c_str(),vmsa::Dca_start,0/*vmsa::nSigKaon_start*/,vmsa::mPID[pid].c_str(),i_norm);
              //  string KEY_InteTheta_QA_Poly = Form("p_%d_Centrality_%d_%s_Dca_%d_Sig_%d_%s_Norm_%d_Poly%d",i_pt,i_cent,EP[order-1].c_str(),vmsa::Dca_start,0/*vmsa::nSigKaon_start*/,vmsa::mPID[pid].c_str(),i_norm,i_poly+1);
              //  h_mMass_InteTheta[KEY_InteTheta_QA]->SetTitle("");
              //  h_mMass_InteTheta[KEY_InteTheta_QA]->SetTitle(Form("%.2f<p_{T}<%.2f, -1<cos(#theta*)<1",vmsa::p_low[energy][i_pt],vmsa::p_up[energy][i_pt]));
              //  h_mMass_InteTheta[KEY_InteTheta_QA]->GetXaxis()->SetNdivisions(505,'N');
              //  h_mMass_InteTheta[KEY_InteTheta_QA]->GetXaxis()->SetLabelSize(0.03);
              //  h_mMass_InteTheta[KEY_InteTheta_QA]->GetXaxis()->SetTitle("M(K^{+},K^{-})");
              //  h_mMass_InteTheta[KEY_InteTheta_QA]->GetXaxis()->SetTitleSize(0.05);
            ...
            for(int i_theta = 0; i_theta < 9; ++i_theta)
            {
              c_diff->cd(i_theta+1);
              string KEY_QA = Form("p_%d_Centrality_%d_CosThetaStar_%d_PhiPrime_%d_%s_Dca_%d_Sig_%d_%s_Norm_%d",i_pt,i_cent,i_theta,i_phipsi,EP[order-1].c_str(),vmsa::Dca_start,0/*vmsa::nSigKaon_start*/,vmsa::mPID[pid].c_str(),i_norm);
              string KEY_QA_Poly = Form("p_%d_Centrality_%d_CosThetaStar_%d_PhiPrime_%d_%s_Dca_%d_Sig_%d_%s_Norm_%d_Poly%d",i_pt,i_cent,i_theta,i_phipsi,EP[order-1].c_str(),vmsa::Dca_start,0/*vmsa::nSigKaon_start*/,vmsa::mPID[pid].c_str(),i_norm,i_poly+1);
              TF1 *f_bw;
              if(i_poly == 0) f_bw = new TF1("f_bw",PolyBreitWigner,vmsa::BW_Start[pid],vmsa::BW_Stop[pid],5);
              if(i_poly == 1) f_bw = new TF1("f_bw",Poly2BreitWigner,vmsa::BW_Start[pid],vmsa::BW_Stop[pid],6);
              if(i_poly == 2) f_bw = new TF1("f_bw",Poly3BreitWigner,vmsa::BW_Start[pid],vmsa::BW_Stop[pid],7);
          ...
          for(int i_sigma = 0/*vmsa::Sig_start*/; i_sigma < 1/*vmsa::Sig_stop*/; ++i_sigma)
          {
            ...
            {
              string KEY_counts_QA = Form("p_%d_Centrality_%d_%s_Dca_%d_Sig_%d_%s_Norm_%d_Sigma_%d_%s_Poly%d",i_pt,i_cent,EP[order-1].c_str(),vmsa::Dca_start,0/*vmsa::nSigKaon_start*/,vmsa::mPID[pid].c_str(),i_norm,i_sigma,vmsa::mInteMethod[i_method].c_str(),i_poly+1);
              //if(i_sigma == vmsa::Sig_start && i_method == vmsa::Method_start)
              //{
                h_mCounts[KEY_counts_QA]->SetStats(0);
                h_mCounts[KEY_counts_QA]->SetTitle(Form("Raw Yields, %.2f<p_{T}<%.2f GeV/c",vmsa::p_low_2D[energy][i_pt],vmsa::p_up_2D[energy][i_pt]));
...
//  if(random3D)  c_rho->SaveAs("../figures/3DRandom/c_rho.pdf");
  ...
  //string outputname = Form("figures/%s/%s/pTstudy/PhiPrime_Chi2NDF_%s_Order%d_Cent9.pdf",vmsa::mPID[pid].c_str(),vmsa::mBeamEnergy[energy].c_str(),vmsa::mBeamEnergy[energy].c_str(),order);
  //string output_start = Form("%s[",outputname.c_str());
  //
  ...
  //          for(int i_poly = 0; i_poly < 3; i_poly++)
  //          {
  //            string KEY_pT_Poly = Form("p_%d_Centrality_%d_%s_Dca_%d_Sig_%d_%s_Norm_%d_Poly%d",i_pt,i_cent,EP[order-1].c_str(),i_dca,i_sig,vmsa::mPID[pid].c_str(),i_norm,i_poly+1);
  //            
  ...
  //outputname = Form("figures/%s/%s/pTstudy/PhiPrime_PValue_%s_Order%d_Cent9.pdf",vmsa::mPID[pid].c_str(),vmsa::mBeamEnergy[energy].c_str(),vmsa::mBeamEnergy[energy].c_str(),order);
  ...
  string outputfile = Form("../output/AuAu%s/%s/%s2DP_RawPhiPSys_%s.root",vmsa::mBeamEnergy[energy].c_str(),vmsa::mPID[pid].c_str(),frame.c_str(),etamode.c_str());
  if(order == 1) outputfile = Form("../output/AuAu%s/%s/%s2DP_RawPhiPSys_%s_FirstOrder.root",vmsa::mBeamEnergy[energy].c_str(),vmsa::mPID[pid].c_str(),frame.c_str(),etamode.c_str());
  if(random3D) outputfile = Form("../output/AuAu%s/%s/3DRandom/RawPhiPSys.root",vmsa::mBeamEnergy[energy].c_str(),vmsa::mPID[pid].c_str());
