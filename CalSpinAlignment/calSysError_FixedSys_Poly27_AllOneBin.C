...
void calSysError_FixedSys_Poly27_AllOneBin(Int_t energy = 6, Int_t pid = 0, Int_t i_cent = 9, string correction = "Raw", bool random3D = false, int order = 2, int defaultF = 0, string etamode = "eta1_eta1", int yspectra = -1)//defaultF = 0 is BESII, defaultF = 1 is BESI
{
  ...
  string inputfileHframe = Form("../output/AuAu%s/%s/NoPhiPsi_RawPhiPtSys_eta1_eta1_PolySys.root",vmsa::mBeamEnergy[energy].c_str(),vmsa::mPID[pid].c_str());
  ...
  string inputfile = Form("../output/AuAu%s/%s/NoPhiPsi_%sPhiPtSys_%s_PolySys%s.root",vmsa::mBeamEnergy[energy].c_str(),vmsa::mPID[pid].c_str(),correction.c_str(),etamode.c_str(),spectra.c_str());
  ...
  for(Int_t i_point = vmsa::pt_rebin_first[energy]; i_point < vmsa::pt_rebin_last[energy]; ++i_point)
  {
    double sysDca[9];
    double sysNSig[9];
    double sysNorm[9];
  ...
  cout << "Final rho00 = " << weight_rho00 << " +/- " << std::setprecision(6) << weight_error_stat_rho00 << " (stat.) " << endl;
  ...
  hint->Draw("pEX0");
  c1->SaveAs(Form("figures/%s/%s/pTstudy/OneBigBin_AUAU27GeV_Integrated.pdf",vmsa::mPID[pid].c_str(),vmsa::mBeamEnergy[energy].c_str()));  
