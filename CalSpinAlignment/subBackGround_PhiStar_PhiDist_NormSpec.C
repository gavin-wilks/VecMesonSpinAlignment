...
void subBackGround_PhiStar_PhiDist_NormSpec(int energy = 4, int pid = 0, int year = 0, string date = "20240416", bool random3D = false, int order = 2, string etamode = "eta1_eta1", int deltaonly = 0)
{
  ...
  string InPutSpec = "pTspectra/HEPData-ins1378002-v1-root.root";
  TFile *File_Spec = TFile::Open(InPutSpec.c_str());
  cout << "Input spectra" << InPutSpec << endl;
 
  TGraphAsymmErrors *g_spec[9];
  TF1 *f_Levy[9];
  TF1 *f_spec[9];
  double integral[9][4]; //centrality, then pT range, 1.2-1.8, 1.8-2.4, 2.4-3.0, 3.0-4.2
  double integralsum9[4] = {0.0};

  for(int centrality = 0; centrality < 9; centrality++)
  {
    TDirectory *dir = (TDirectory*) File_Spec->Get(Form("Table %d",tableNum[energy][centrality]));
    dir->cd(); 
    g_spec[centrality] = (TGraphAsymmErrors*)dir->Get("Graph1D_y1");
    for(int i = 0; i < g_spec[centrality]->GetN(); i++)
    {
      double x,y;
      g_spec[centrality]->GetPoint(i,x,y);
      g_spec[centrality]->SetPoint(i,ptvalues[energy][centrality][i],y);
      g_spec[centrality]->SetPointError(i,0.0,0.0,g_spec[centrality]->GetErrorYlow(i),g_spec[centrality]->GetErrorYhigh(i));
    }
  

    f_Levy[centrality] = new TF1(Form("f_Levy%d",centrality),Levy,0.1,5.0,3);
    f_Levy[centrality] ->SetParameter(0,1);
    if(centrality >= 0 && centrality <= 1) f_Levy[centrality] ->SetParameter(0,0.01);
    f_Levy[centrality] ->SetParameter(1,10);
    f_Levy[centrality] ->SetParameter(2,0.1);
    cout << "Fitting full pT distribution centrality = " << centrality << endl;
    g_spec[centrality] ->Fit(f_Levy[centrality] ,"N");

    f_spec[centrality] = new TF1(Form("f_spec%d",centrality),pTLevy,0.1,5.0,3);
    f_spec[centrality] ->SetParameter(0,f_Levy[centrality] ->GetParameter(0));
    f_spec[centrality] ->SetParameter(1,f_Levy[centrality] ->GetParameter(1));
    f_spec[centrality] ->SetParameter(2,f_Levy[centrality] ->GetParameter(2));

    integral[centrality][0] = f_spec[centrality]->Integral(1.2,1.8);
    integral[centrality][1] = f_spec[centrality]->Integral(1.8,2.4);
    integral[centrality][2] = f_spec[centrality]->Integral(2.4,3.0);
    integral[centrality][3] = f_spec[centrality]->Integral(3.0,4.2);

    if(centrality >= 2 && centrality <= 5) 
    {  
      integralsum9[0] += integral[centrality][0];
      integralsum9[1] += integral[centrality][1];
      integralsum9[2] += integral[centrality][2];
      integralsum9[3] += integral[centrality][3];
    }
       
  }
  
  //double weightedpt[4] = {0.0};
  double weightedcounts[4] = {0.0};
  for(int ipt = 0; ipt < 4; ipt++)
  {
    for(int centrality = 2; centrality <= 5; centrality++)
    {
      weightedcounts[ipt] += integral[centrality][ipt]*integral[centrality][ipt]/integralsum9[ipt];
    }   
  }
  ...
  string InPutFile_SE = Form("../data/Yields_Phi_SE_%s_%s_%s_phis_evenmorebins.root",vmsa::mBeamEnergy[energy].c_str(),date.c_str(),etamode.c_str());
  ...
  string InPutFile_ME = Form("../data/Yields_Phi_ME_%s_%s_%s_phis_evenmorebins.root",vmsa::mBeamEnergy[energy].c_str(),date.c_str(),etamode.c_str());
  ...
  string folder = "CosEfficiencyProccessTuple_EPRes_EffAcc_Ycut_Order2_NoRapiditySpectra_FixedFirstEP_RCBins_prelimv2_EPeff_flatRP_Acc1Rc_pT0p1_TPC_TOF_phis_20240416";
  ...
  string InPutFile_RC = "effaccfiles/Phi/19GeV/CosEfficiencyProccessTuple_EPRes_EffAcc_Ycut_Order2_NoRapiditySpectra_FixedFirstEP_RCBins_prelimv2_EPeff_flatRP_Acc1Rc_pT0p1_TPC_TOF_phis_20240416/Eff_19GeV_SingleParticle_noToF_Mode0_EtaMode0.root";  
  ...
  double total_scaling_yields = 0.0;
  double scaling_yields[4] = {0.0};
  double scaling_rcyields[4] = {0.0};
  double scaling_mcyields[4] = {0.0};
  
  int scalingidx[12] = {0,0,0,1,1,1,2,2,2,3,3,3};
  ...
  double mcspectrascaled = 0.0;
  double rcspectrascaled = 0.0;
  double mcspectrascalederr = 0.0;
  double rcspectrascalederr = 0.0;
  ...
  for(Int_t i_pt_phi = 0; i_pt_phi < 12; i_pt_phi++) // pt bin 
  {
    ...
    double rcscale = weightedcounts[scalingidx[i_pt_phi]]/scaling_mcyields[scalingidx[i_pt_phi]];
    double mcscale = weightedcounts[scalingidx[i_pt_phi]]/scaling_mcyields[scalingidx[i_pt_phi]];
    ...
    rcspectrascaled += rcyield;
    mcspectrascaled += mcyield;
    rcspectrascalederr += rcyielderror;
    mcspectrascalederr += mcyielderror;
  ...
  g_mRcPhi_pT[KEY_RC_pT]->Scale(total_scaling_yields/rcspectrascaled,"y");
  g_mMcPhi_pT[KEY_MC_pT]->Scale(total_scaling_yields/mcspectrascaled,"y");
