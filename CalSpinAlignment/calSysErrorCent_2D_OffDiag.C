...
void calSysErrorCent_2D_OffDiag(Int_t energy = 4, Int_t pid = 0, Int_t i_cent = 9, string correction = "Raw", bool random3D = false, int order = 2, string etamode = "eta1_eta1", int yspectra = -1, int frameopt = 0)//defaultF = 0 is BESII, defaultF = 1 is BESI
{
  ...
  string inputfileHframe = Form("../output/AuAu%s/%s/%s2DCent_RawPhiCentSys_eta1_eta1.root",frame.c_str(),vmsa::mBeamEnergy[energy].c_str(),vmsa::mPID[pid].c_str());
  string inputfile = Form("../output/AuAu%s/%s/%s2DCent_%sPhiCentSys_%s%s.root",frame.c_str(),vmsa::mBeamEnergy[energy].c_str(),vmsa::mPID[pid].c_str(),correction.c_str(),etamode.c_str(),spectra.c_str());
  if(order == 1) inputfile = Form("../output/AuAu%s/%s/%s2DCent_%sPhiCentSys_%s_FirstOrder%s.root",frame.c_str(),vmsa::mBeamEnergy[energy].c_str(),vmsa::mPID[pid].c_str(),correction.c_str(),etamode.c_str(),spectra.c_str());
  if(correction == "RawRes") inputfile = Form("../output/AuAu%s/%s/%s2DCent_RawPhiCentSys_%s%s.root",frame.c_str(),vmsa::mBeamEnergy[energy].c_str(),vmsa::mPID[pid].c_str(),etamode.c_str(),spectra.c_str());
  if(correction == "RawRes" && order == 1) inputfile = Form("../output/AuAu%s/%s/%s2DCent_RawPhiCentSys_%s_FirstOrder%s.root",frame.c_str(),vmsa::mBeamEnergy[energy].c_str(),vmsa::mPID[pid].c_str(),etamode.c_str(),spectra.c_str());
          ...
          for(int i_sigma = vmsa::Sig_start; i_sigma < vmsa::Sig_stop; ++i_sigma)
          {
            for(int i_method = vmsa::Method_start; i_method < vmsa::Method_stop; ++i_method)
            {
              ...
              { 
                ...
                {
                  string KEY = Form("%sRaw_pt_%d_%s_Dca_%d_Sig_%d_%s_Norm_%d_Sigma_%d_%s_Poly%d",parstring[i_par].c_str(),i_pt,EP[order-1].c_str(),i_dca,i_sig,vmsa::mPID[pid].c_str(),i_norm,i_sigma,vmsa::mInteMethod[i_method].c_str(),i_poly+1);
  ...
  for(int i_par = 0; i_par < 5; i_par++)
  {
    ...
    {           
      string KEY_Default = Form("%sRaw_pt_%d_%s_Dca_%d_Sig_%d_%s_Norm_%d_Sigma_%d_%s_Poly1",parstring[i_par].c_str(),i_pt,EP[order-1].c_str(),0,0,vmsa::mPID[pid].c_str(),0,0,vmsa::mInteMethod[1].c_str());
      ...
      for(Int_t i_point = 0; i_point < 9; ++i_point)
      {
        ...
        double cent_def, rho_def;
        g_mPar[KEY_Default]->GetPoint(i_point,cent_def,rho_def); 
        ...
        cout << parstring[i_par] << ", cent = " << cent_def << ", value = " << rho_def << " +/- " << rho_defErr << endl;
        ...
        { 
          for(int i_sigma = vmsa::Sig_start; i_sigma < vmsa::Sig_stop; ++i_sigma)
          {
            for(int i_method = vmsa::Method_start; i_method < vmsa::Method_stop; ++i_method)
            {
              ...
              string KEY = Form("%sRaw_pt_%d_%s_Dca_%d_Sig_%d_%s_Norm_%d_Sigma_%d_%s_Poly%d",parstring[i_par].c_str(),i_pt,EP[order-1].c_str(),i_dca,0,vmsa::mPID[pid].c_str(),0,i_sigma,vmsa::mInteMethod[i_method].c_str(),1);
        ...
        for(Int_t i_sig = vmsa::nSigKaon_start; i_sig < vmsa::nSigKaon_stop; i_sig++)
        {
          for(int i_sigma = vmsa::Sig_start; i_sigma < vmsa::Sig_stop; ++i_sigma)
          {
            for(int i_method = vmsa::Method_start; i_method < vmsa::Method_stop; ++i_method)
            {
              ...
              string KEY = Form("%sRaw_pt_%d_%s_Dca_%d_Sig_%d_%s_Norm_%d_Sigma_%d_%s_Poly%d",parstring[i_par].c_str(),i_pt,EP[order-1].c_str(),0,i_sig,vmsa::mPID[pid].c_str(),0,i_sigma,vmsa::mInteMethod[i_method].c_str(),1);
              double pt_sys, rho_sys;
              g_mPar[KEY]->GetPoint(i_point,pt_sys,rho_sys);
              sysNSig[idx] = rho_sys;
        ...
        for(int i_norm = vmsa::Norm_start; i_norm < vmsa::Norm_stop; ++i_norm)
        {
          for(int i_sigma = vmsa::Sig_start; i_sigma < vmsa::Sig_stop; ++i_sigma)
          {
            for(int i_method = vmsa::Method_start; i_method < vmsa::Method_stop; ++i_method)
            {
              ...
              string KEY = Form("%sRaw_pt_%d_%s_Dca_%d_Sig_%d_%s_Norm_%d_Sigma_%d_%s_Poly%d",parstring[i_par].c_str(),i_pt,EP[order-1].c_str(),0,0,vmsa::mPID[pid].c_str(),i_norm,i_sigma,vmsa::mInteMethod[i_method].c_str(),1);
              double pt_sys, rho_sys;
              g_mPar[KEY]->GetPoint(i_point,pt_sys,rho_sys);
              sysNorm[idx] = rho_sys;
        ...
        for(int i_poly = 0; i_poly < 2; ++i_poly)
        {
          for(int i_sigma = vmsa::Sig_start; i_sigma < vmsa::Sig_stop; ++i_sigma)
          {
            for(int i_method = vmsa::Method_start; i_method < vmsa::Method_stop; ++i_method)
            {
              ...
              string KEY = Form("%sRaw_pt_%d_%s_Dca_%d_Sig_%d_%s_Norm_%d_Sigma_%d_%s_Poly%d",parstring[i_par].c_str(),i_pt,EP[order-1].c_str(),0,0,vmsa::mPID[pid].c_str(),0,i_sigma,vmsa::mInteMethod[i_method].c_str(),i_poly+1);
              double pt_sys, rho_sys;
              g_mPar[KEY]->GetPoint(i_point,pt_sys,rho_sys);
              sysPoly[idx] = rho_sys;
        ...
        g_SysErrors[KEY_Default]->SetPoint(i_point,cent_def,rho_def);
        ...
        g_StatErrors[KEY_Default]->SetPoint(i_point,cent_def,rho_def);
            ...
            for(int i_method = vmsa::Method_start; i_method < vmsa::Method_stop; ++i_method)
            {
              for(int i_poly = 0; i_poly < 3; i_poly++) 
              {
                ...
                for(Int_t i_point = 0; i_point < 9; ++i_point)
                {
                  ...
                  {           
                    string KEY = Form("%sRaw_pt_%d_%s_Dca_%d_Sig_%d_%s_Norm_%d_Sigma_%d_%s_Poly%d",parstring[i_par].c_str(),i_pt,EP[order-1].c_str(),i_dca,i_sig,vmsa::mPID[pid].c_str(),i_norm,i_sigma,vmsa::mInteMethod[i_method].c_str(),i_poly+1);
                    g_mPar[KEY]->GetPoint(i_point,cent,rho);
                  ...
                  g_mStatErrors[KEY_Int]->SetPoint(i_point,cent,weight_rho00f);
  ...
  c_rho_SysError->SaveAs(Form("figures/%s/%s/centralitystudy/%s2D_finaloutputrho_%s_Order%d_%s.pdf",vmsa::mPID[pid].c_str(),vmsa::mBeamEnergy[energy].c_str(),frame.c_str(),correction.c_str(),order,etamode.c_str()));  
  c_rho_SysError->SaveAs(Form("figures/%s/%s/centralitystudy/%s2D_finaloutputrho_%s_Order%d_%s.eps",vmsa::mPID[pid].c_str(),vmsa::mBeamEnergy[energy].c_str(),frame.c_str(),correction.c_str(),order,etamode.c_str()));  
  c_rho_SysError->SaveAs(Form("figures/%s/%s/centralitystudy/%s2D_finaloutputrho_%s_Order%d_%s.png",vmsa::mPID[pid].c_str(),vmsa::mBeamEnergy[energy].c_str(),frame.c_str(),correction.c_str(),order,etamode.c_str()));  
  ...
  string OutPutFile = Form("../output/AuAu%s/%s/%s2DCent_Rho_%sSysErrors_%s_%s.root",frame.c_str(),vmsa::mBeamEnergy[energy].c_str(),vmsa::mPID[pid].c_str(),correction.c_str(),etamode.c_str(),spectra.c_str());
  if(order == 1) OutPutFile = Form("../output/AuAu%s/%s/%s2DCent_Rho_%sSysErrors_%s_FirstOrder%s.root",frame.c_str(),vmsa::mBeamEnergy[energy].c_str(),vmsa::mPID[pid].c_str(),correction.c_str(),etamode.c_str(),spectra.c_str());
