...
void calSysErrorPhiCent_FixedSys_RawRes(Int_t energy = 3, Int_t pid = 0, string correction = "Raw", bool random3D = false, int centQA = 0, int order = 2, std::string etamode = "eta1_eta1", std::string fileoption = "")//defaultF = 0 is BESII, defaultF = 1 is BESI
{
  std::string EP[2] = {"","2nd"};
        ...
        for(int i_sigma = vmsa::Sig_start; i_sigma < vmsa::Sig_stop; ++i_sigma)
        {
          for(int i_method = vmsa::Method_start; i_method < vmsa::Method_stop; ++i_method)
          {
            ...
            for(int i_cent = vmsa::centStart; i_cent < vmsa::centStop; i_cent++) // Centrality loop
            {
              ...
              for(int i_pt = vmsa::pt_rebin_first_cent[energy]; i_pt <= vmsa::pt_rebin_last_cent[energy]; ++i_pt) // pt loop
              {          
                string KEY_Default = Form("rhoRaw_pt_%d_%s_Dca_%d_Sig_%d_%s_Norm_%d_Sigma_%d_%s",i_pt,EP[order-1].c_str(),i_dca,i_sig,vmsa::mPID[pid].c_str(),i_norm,i_sigma,vmsa::mInteMethod[i_method].c_str());
                string KEY_counts = Form("pt_%d_Centrality_%d_%s_Dca_%d_Sig_%d_%s_Norm_%d_Sigma_%d_%s",i_pt,i_cent,EP[order-1].c_str(),i_dca,i_sig,vmsa::mPID[pid].c_str(),i_norm,i_sigma,vmsa::mInteMethod[i_method].c_str()); 
  ...
  string outputname = Form("./figures/%s/%s/centralitystudy/finaloutputrho_%s_centralitydependence_pTbins_Res%s_Order%d_%s.pdf",vmsa::mPID[pid].c_str(),vmsa::mBeamEnergy[energy].c_str(),correction.c_str(),vmsa::mBeamEnergy[energy].c_str(),order,etamode.c_str());
  ...
  for(int i_pt = vmsa::pt_rebin_first_cent[energy]; i_pt <= vmsa::pt_rebin_last_cent[energy]; ++i_pt) // pt loop
  {
    ...
    string KEY_D = Form("rhoRaw_pt_%d_%s_Dca_%d_Sig_%d_%s_Norm_%d_Sigma_%d_%s",i_pt,EP[order-1].c_str(),0,0,vmsa::mPID[pid].c_str(),0,0,vmsa::mInteMethod[1].c_str());
...
//  PlotLine(0.0,5.0,1.0/3.0,1.0/3.0,1,2,2);
  ...
  c_rho_SysError->SaveAs(Form("figures/%s/%s/centralitystudy/finaloutputrho_%s_centralitydependence_Res%s_Order%d_%s.pdf",vmsa::mPID[pid].c_str(),vmsa::mBeamEnergy[energy].c_str(),correction.c_str(),vmsa::mBeamEnergy[energy].c_str(),order,etamode.c_str()));  
  ...
  */
  ...
  string OutPutFile = Form("../output/AuAu%s/%s/RhoCent_Res%sSysErrors_%s.root",vmsa::mBeamEnergy[energy].c_str(),vmsa::mPID[pid].c_str(),correction.c_str(),etamode.c_str());
  if(order == 1) OutPutFile = Form("../output/AuAu%s/%s/RhoCent_Res%sSysErrors_%s_FirstOrder.root",vmsa::mBeamEnergy[energy].c_str(),vmsa::mPID[pid].c_str(),correction.c_str(),etamode.c_str());
