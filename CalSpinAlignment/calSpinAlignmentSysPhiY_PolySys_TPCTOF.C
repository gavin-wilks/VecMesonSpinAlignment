...
void calSpinAlignmentSysPhiY_PolySys_TPCTOF(int energy = 4, int pid = 0, int year = 0, bool random3D = false, int etaQA = 9, int ptQA = 1, int centQA = 2, int dcaQA = 0, int nsigQA = 0, int normQA = 0, int sigQA = 0, int order = 2, std::string etamode = "eta1_eta1")
{
  ...
  string inputfile = Form("../output/AuAu%s/%s/TPCTOF_InvMassSubBgEta_%s.root",vmsa::mBeamEnergy[energy].c_str(),vmsa::mPID[pid].c_str(),etamode.c_str());
...
//#endif
  ...
  */
  ...
  for(int i_poly = 0; i_poly < 3; i_poly++)
  {
    string outputname = Form("./figures/%s/%s/rapiditystudy/TPCTOF_allThetaYields%s_%s_Order%d_%s_RapidityDependence_Poly%d.pdf",vmsa::mPID[pid].c_str(),vmsa::mBeamEnergy[energy].c_str(),vmsa::mPID[pid].c_str(),vmsa::mBeamEnergy[energy].c_str(),order,etamode.c_str(),i_poly+1);
...
//#endif
  ...
  if(!random3D) c_rho->SaveAs("../figures/TPCTOF_c_rho.pdf");
  ...
  string outputname = Form("figures/%s/%s/rapiditystudy/TPCTOF_Chi2NDF_%s_Order%d.pdf",vmsa::mPID[pid].c_str(),vmsa::mBeamEnergy[energy].c_str(),vmsa::mBeamEnergy[energy].c_str(),order);
  string output_start = Form("%s[",outputname.c_str());
  ...
  outputname = Form("figures/%s/%s/rapiditystudy/TPCTOF_PValue_%s_Order%d.pdf",vmsa::mPID[pid].c_str(),vmsa::mBeamEnergy[energy].c_str(),vmsa::mBeamEnergy[energy].c_str(),order);
  ...
  string outputfile = Form("../output/AuAu%s/%s/TPCTOF_RawRhoEtaSys_%s_Poly.root",vmsa::mBeamEnergy[energy].c_str(),vmsa::mPID[pid].c_str(),etamode.c_str());
