...
void compRhoEffY_PhiYSpectra(Int_t energy = 4, Int_t pid = 0, Int_t i_cent = 9, string correction = "AccRes", int order = 2, std::string comp1file = "", std::string comp2file = "", std::string comp1 = "", std::string comp2 = "")
{

  std::string ordertext = "";
  if(order == 1) ordertext = "_FirstOrder";

  //std::string comp1file = "_NoRapiditySpectra_EP"; 
  //std::string comp2file = "_NoRapiditySpectra_EP_noV2";
  //std::string comp1     = "flat y, w/ v_{2}";
  //std::string comp2     = "flat y, w/o v_{2}";
  ...
  string inputfile1 = Form("../output/AuAu%s/%s/RhoEta_%sSysErrors_eta1_eta1_PolySys%s%s.root",vmsa::mBeamEnergy[energy].c_str(),vmsa::mPID[pid].c_str(),correction.c_str(),ordertext.c_str(),comp1file.c_str());
  string inputfile2 = Form("../output/AuAu%s/%s/RhoEta_%sSysErrors_eta1_eta1_PolySys%s%s.root",vmsa::mBeamEnergy[energy].c_str(),vmsa::mPID[pid].c_str(),correction.c_str(),ordertext.c_str(),comp2file.c_str());
  ...
  TString StatErrorRho1 = Form("g_rho00_order%d_%s_%s_StatError",order,vmsa::mBeamEnergy[energy].c_str(),vmsa::mPID[pid].c_str());
  TString SysErrorRho1 = Form("g_rho00_order%d_%s_%s_SysError",order,vmsa::mBeamEnergy[energy].c_str(),vmsa::mPID[pid].c_str());
  TString StatErrorRho2 = Form("g_rho00_order%d_%s_%s_StatError",order,vmsa::mBeamEnergy[energy].c_str(),vmsa::mPID[pid].c_str());
  TString SysErrorRho2 = Form("g_rho00_order%d_%s_%s_SysError",order,vmsa::mBeamEnergy[energy].c_str(),vmsa::mPID[pid].c_str());
...
//  PlotLine(0.0,5.0,1.0/3.0,1.0/3.0,1,2,2);
  ...
  //if(energy == 4)
  {
    ...
    string leg_count = Form("%s (1.0 < p_{T} < 5.0 GeV/c)",comp1.c_str());
    plotTopLegend((char*)leg_count.c_str(),0.15,0.497,0.03,1,0.0,42,0);

    string leg_besi = Form("%s (1.0 < p_{T} < 5.0 GeV/c)",comp2.c_str());
  ...
  Draw_TGAE_new_Symbol((TGraphAsymmErrors*)g_Stat1,style_phi_2nd,color_phi_2nd,colorDiff_phi,size_marker+0.2);
  Draw_TGAE_new_Symbol((TGraphAsymmErrors*)g_Stat2,style_phi_1st,color_phi_1st,colorDiff_phi,size_marker-0.2);

  plotSysErrorsBox(g_Sys1,color_phi_2nd);//,energy);
  plotSysErrorsBox(g_Sys2,color_phi_1st);//,energy);
  ...
  c_rho_SysError->SaveAs(Form("figures/phi_order%d_rapidity_%s%s_vs%s.pdf",order,vmsa::mBeamEnergy[energy].c_str(),comp1file.c_str(),comp2file.c_str()));
  c_rho_SysError->SaveAs(Form("figures/phi_order%d_rapidity_%s%s_vs%s.png",order,vmsa::mBeamEnergy[energy].c_str(),comp1file.c_str(),comp2file.c_str()));
