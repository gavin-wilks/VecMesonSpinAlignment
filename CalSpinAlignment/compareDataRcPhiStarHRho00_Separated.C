...
void compareDataRcPhiStarHRho00_Separated(Int_t energy = 4, Int_t pid = 0, Int_t i_cent = 9, string correction = "Raw", int order = 2, string etamode = "eta1_eta1", int yspectra = 23, std::string simmode = "Mc")//defaultF = 0 is BESII, defaultF = 1 is BESI
{
  ...
  {
    spectra[i] = Form("CEPT_O2_NoYSpec_prev2_phieff_phiTPC_20240703_ysig1000_rho0.3333_rerho0.0_imrho0.0_r0.0_i0.0_hrho%s_hrerho0.0_himrho0.0_hr0.0_hi0.0",sigmay_text[i].c_str());
  }
  ...
  std::string outputname = Form("figures/%s/%s/pTstudy/%s_rho00comparison_%s.pdf",vmsa::mPID[pid].c_str(),vmsa::mBeamEnergy[energy].c_str(),spectra[defaultfile].c_str(),simmode.c_str());  
  ...
  for(int ipt = 0; ipt < 4; ipt++)
  {
    ...
    for(int i = start; i < stop; i++)
    {
      ...
      fits2D[ipt][i] = new TF2(Form("fit2D_%d_%d",ipt,i),SpinDensity2DcosSeparated,-1.0,1.0,0.0,2.0*TMath::Pi(),8);
      fits2D[ipt][i]->SetParameter(0,1./3.);
      fits2D[ipt][i]->SetParameter(7,h_m2D_RC[ipt][i]->GetMaximum());
      //fits2D[ipt][i]->FixParameter(0,1./3.);
      fits2D[ipt][i]->SetParLimits(0,0.0,1.0);
      fits2D[ipt][i]->SetParameter(1,0.0);
      TFitResultPtr fitResult = h_m2D_RC[ipt][i]->Fit(fits2D[ipt][i],"NMIRS");
      for(int par = 0; par < 7; par++)
      {
        if(par == 0) 
        {
          g2D[par][ipt]->SetPoint(i-start,sigmay[i]-1./3.,fits2D[ipt][i]->GetParameter(0)-1./3.);
          g2D[par][ipt]->SetPointError(i-start,0.0,0.0,fits2D[ipt][i]->GetParError(par),fits2D[ipt][i]->GetParError(par));
        }
        else if(par == 1) 
        {
          g2D[1][ipt]->SetPoint(i-start,sigmay[i]-1./3.,fits2D[ipt][i]->GetParameter(1)-fits2D[ipt][i]->GetParameter(2));
          double cov = fitResult->GetCovarianceMatrix()(1,2);
          cout << "Covariance = " << cov << endl;
          double error = TMath::Sqrt(fits2D[ipt][i]->GetParError(1)*fits2D[ipt][i]->GetParError(1) + fits2D[ipt][i]->GetParError(2)*fits2D[ipt][i]->GetParError(2) - 2.*cov);
          g2D[1][ipt]->SetPointError(i-start,0.0,0.0,error,error);
          par++;
        }
        else if(par == 3) 
        {
          g2D[2][ipt]->SetPoint(i-start,sigmay[i]-1./3.,fits2D[ipt][i]->GetParameter(3)-fits2D[ipt][i]->GetParameter(4));
          double cov = fitResult->GetCovarianceMatrix()(3,4);
          cout << "Covariance = " << cov << endl;
          double error = TMath::Sqrt(fits2D[ipt][i]->GetParError(3)*fits2D[ipt][i]->GetParError(3) + fits2D[ipt][i]->GetParError(4)*fits2D[ipt][i]->GetParError(4) - 2.*cov);
          g2D[2][ipt]->SetPointError(i-start,0.0,0.0,error,error);
          par++;
        }
        else if(par == 5 || par == 6)
        {
          g2D[par-2][ipt]->SetPoint(i-start,sigmay[i]-1./3.,fits2D[ipt][i]->GetParameter(par));
          g2D[par-2][ipt]->SetPointError(i-start,0.0,0.0,fits2D[ipt][i]->GetParError(par),fits2D[ipt][i]->GetParError(par));
        }
      }
  ...
  for(int i = 0; i < 4; i++)
  {
    for(int par = 0; par < 5; par++)
    {
      ...
      g2D[par][i]->GetXaxis()->SetTitle("#Delta#rho_{00}");
      g2D[par][i]->GetYaxis()->SetTitle(param[par].c_str());
      g2D[par][i]->Draw("APE"); 
    }
  }
  cfit2D->SaveAs(Form("figures/%s/%s/pTstudy/%s_parametercomparison_rho00comparison_%s.pdf",vmsa::mPID[pid].c_str(),vmsa::mBeamEnergy[energy].c_str(),spectra[defaultfile].c_str(),simmode.c_str()));  
