...
void appEffCorrSysKStar(int energy = 4, int pid = 2, int ptQA = 2, bool isBesI = false, bool random3D = false)
{
  string InPutRho = Form("../output/AuAu%s/%s/RawKStarPtSys.root",vmsa::mBeamEnergy[energy].c_str(),vmsa::mPID[pid].c_str());
  if(random3D) InPutRho = Form("../output/AuAu%s/%s/3DRandom/RawKStarPtSys.root",vmsa::mBeamEnergy[energy].c_str(),vmsa::mPID[pid].c_str());
  if(isBesI) InPutRho = Form("../data/BESI/RawRhoPtSys.root");
  TFile *File_Rho = TFile::Open(InPutRho.c_str());
  cout << InPutRho << endl;

  string InPutEff = Form("efficiencies/KStar/Eff_%s_SingleParticle_2060_noToF.root",vmsa::mBeamEnergy[energy].c_str());
  ...
  string OutPutFile = Form("../output/AuAu%s/%s/EffKStarPtSys.root",vmsa::mBeamEnergy[energy].c_str(),vmsa::mPID[pid].c_str());
  if(random3D) OutPutFile = Form("../output/AuAu%s/%s/3DRandom/EffKStarPtSys.root",vmsa::mBeamEnergy[energy].c_str(),vmsa::mPID[pid].c_str());
  if(isBesI) OutPutFile = Form("../output/AuAu%s/%s/BESI/EffKStarPtSys.root",vmsa::mBeamEnergy[energy].c_str(),vmsa::mPID[pid].c_str());
