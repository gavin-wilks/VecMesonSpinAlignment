...
void compareDataRcPhiStar_ptspectra_y0p5(Int_t energy = 4, Int_t pid = 0, Int_t i_cent = 9, string correction = "Raw", int order = 2, string etamode = "eta1_eta1", int yspectra = 27)//defaultF = 0 is BESII, defaultF = 1 is BESI
{
  ...
  string inputfileRC = Form("effaccfiles/%s/%s/CosEfficiencyProccessTuple_EPRes_EffAcc_Ycut_Order%d%s/Eff_%s_SingleParticle_noToF_Mode0_EtaMode0_PtBins2_5.root",vmsa::mPID[pid].c_str(),vmsa::mBeamEnergy[energy].c_str(),order,spectra.c_str(),vmsa::mBeamEnergy[energy].c_str());
  ...
  ///////////// 2D y vs pT /////////////
  float data = h_mCounts->Integral(0,-1,5,12);
  float rc   = h_mCounts_RC->Integral(0,-1,5,12);

  // Print bin edges and corresponding bin numbers
  std::cout << "Bin Edges for X-axis:" << std::endl;
  for (int i = 1; i <= h_mCounts->GetNbinsX(); i++) {
      double xlow = h_mCounts->GetXaxis()->GetBinLowEdge(i);
      double xup = h_mCounts->GetXaxis()->GetBinUpEdge(i);
      std::cout << "Bin " << i << ": [" << xlow << ", " << xup << "]" << std::endl;
  }

  std::cout << "Bin Edges for Y-axis:" << std::endl;
  for (int j = 1; j <= h_mCounts->GetNbinsY(); j++) {
      double ylow = h_mCounts->GetYaxis()->GetBinLowEdge(j);
      double yup = h_mCounts->GetYaxis()->GetBinUpEdge(j);
      std::cout << "Bin " << j << ": [" << ylow << ", " << yup << "]" << std::endl;
  }
  ...
  ///////////// 1D pT /////////////
  h_mCounts_pt = (TH1D*) h_mCounts->ProjectionX("pt",5,12,"e"); 
  h_mCounts_RC_pt = (TH1D*) h_mCounts_RC->ProjectionX("ptrc",5,12,"e"); 
  ...
  //string outputfile = "ptyspectra_datarcratio.root";
