...
void calSysErrorPt_2D_OffDiag(Int_t energy = 4, Int_t pid = 0, Int_t i_cent = 9, string correction = "Eff", bool random3D = false, int order = 2, string etamode = "eta1_eta1", int yspectra = -1, int frameopt = 0)//defaultF = 0 is BESII, defaultF = 1 is BESI
{
  ...
  string inputfileHframe = Form("../output/AuAu%s/%s/%s2DPt_RawPhiPtSys_eta1_eta1.root",vmsa::mBeamEnergy[energy].c_str(),vmsa::mPID[pid].c_str(),frame.c_str());
  string inputfile = Form("../output/AuAu%s/%s/%s2DPt_%sPhiPtSys_%s%s.root",vmsa::mBeamEnergy[energy].c_str(),vmsa::mPID[pid].c_str(),frame.c_str(),correction.c_str(),etamode.c_str(),spectra.c_str());
  if(order == 1) inputfile = Form("../output/AuAu%s/%s/%s2DPt_%sPhiPtSys_%s_FirstOrder%s.root",vmsa::mBeamEnergy[energy].c_str(),vmsa::mPID[pid].c_str(),frame.c_str(),correction.c_str(),etamode.c_str(),spectra.c_str());
  if(correction == "RawRes") inputfile = Form("../output/AuAu%s/%s/%s2DPt_RawPhiPtSys_%s%s.root",vmsa::mBeamEnergy[energy].c_str(),vmsa::mPID[pid].c_str(),frame.c_str(),etamode.c_str(),spectra.c_str());
  if(correction == "RawRes" && order == 1) inputfile = Form("../output/AuAu%s/%s/%s2DPt_RawPhiPtSys_%s_FirstOrder%s.root",vmsa::mBeamEnergy[energy].c_str(),vmsa::mPID[pid].c_str(),frame.c_str(),etamode.c_str(),spectra.c_str());
  ...
  for(int i_par = 0; i_par < 5; i_par++)
  {
    ...
    for(Int_t i_point = 0/*vmsa::pt_rebin_first[energy]*/; i_point < vmsa::pt_rebin_last[energy]-vmsa::pt_rebin_first[energy]; ++i_point)
    {
      double sysDca[9];
      double sysNSig[9];
      double sysNorm[9];
      ...
      g_mPar[KEY_Default]->GetPoint(i_point,pt_def,rho_def); 
      ...
      cout << parstring[i_par] << ", pt = " << pt_def << ", value = " << rho_def << " +/- " << rho_defErr << endl;
      ...
      g_SysErrors[i_par]->SetPoint(i_point,pt_def,rho_def);
      ...
      g_StatErrors[i_par]->SetPoint(i_point,pt_def,rho_def);
  ...
  for(int i_par = 0; i_par < 5; i_par++)
  {
    ...
    cout << "Final corrected " << parstring[i_par] << " = " << std::fixed << std::setprecision(4) << rho_def << " +/- " << rho_defErr << " (stat) +/- " << SysError_rho << " (sys)" << endl;
  ...
  for(int i_par = 0; i_par < 5; i_par++)
  {
    ...
    if(frameopt == 0) h_frame->GetYaxis()->SetRangeUser(-0.09,0.03);
    if(frameopt == 1) h_frame->GetYaxis()->SetRangeUser(-0.15,0.15);
  ...
  c_rho_SysError->SaveAs(Form("figures/%s/%s/pTstudy/%s2D_finaloutputrho_%s_Order%d_%s.pdf",vmsa::mPID[pid].c_str(),vmsa::mBeamEnergy[energy].c_str(),frame.c_str(),correction.c_str(),order,etamode.c_str()));  
  c_rho_SysError->SaveAs(Form("figures/%s/%s/pTstudy/%s2D_finaloutputrho_%s_Order%d_%s.eps",vmsa::mPID[pid].c_str(),vmsa::mBeamEnergy[energy].c_str(),frame.c_str(),correction.c_str(),order,etamode.c_str()));  
  c_rho_SysError->SaveAs(Form("figures/%s/%s/pTstudy/%s2D_finaloutputrho_%s_Order%d_%s.png",vmsa::mPID[pid].c_str(),vmsa::mBeamEnergy[energy].c_str(),frame.c_str(),correction.c_str(),order,etamode.c_str()));  
  ...
  string OutPutFile = Form("../output/AuAu%s/%s/%s2DPt_Rho_%sSysErrors_%s_%s.root",vmsa::mBeamEnergy[energy].c_str(),vmsa::mPID[pid].c_str(),frame.c_str(),correction.c_str(),etamode.c_str(),spectra.c_str());
  if(order == 1) OutPutFile = Form("../output/AuAu%s/%s/%s2DPt_Rho_%sSysErrors_%s_FirstOrder%s.root",vmsa::mBeamEnergy[energy].c_str(),vmsa::mPID[pid].c_str(),frame.c_str(),correction.c_str(),etamode.c_str(),spectra.c_str());
