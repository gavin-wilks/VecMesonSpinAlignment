...
void compRhoEffPhiCent(Int_t energy = 4, Int_t pid = 0, Int_t i_cent = 9)
{
  string inputfileRaw = Form("../output/AuAu%s/%s/RhoCent_RawSysErrors.root",vmsa::mBeamEnergy[energy].c_str(),vmsa::mPID[pid].c_str());
  string inputfileEff = Form("../output/AuAu%s/%s/RhoCent_EffSysErrors.root",vmsa::mBeamEnergy[energy].c_str(),vmsa::mPID[pid].c_str());
  string inputfileAcc = Form("../output/AuAu%s/%s/RhoCent_AccResSysErrors.root",vmsa::mBeamEnergy[energy].c_str(),vmsa::mPID[pid].c_str());
...
//  PlotLine(0.0,5.0,1.0/3.0,1.0/3.0,1,2,2);
  ...
  Draw_TGAE_Point_new_Symbol(5.,0.31,0.0,0.0,0.0,0.0,20,kGray+2,1.3); 
  string leg_count = "Raw #phi BES-II";
  plotTopLegend((char*)leg_count.c_str(),7.,0.308,0.03,1,0.0,42,0);

  Draw_TGAE_Point_new_Symbol(5.,0.30,0.0,0.0,0.0,0.0,20,kBlue,1.3); 
  string leg_countE = "Eff Corrected #phi BES-II";
  plotTopLegend((char*)leg_countE.c_str(),7.,0.298,0.03,1,0.0,42,0);

  Draw_TGAE_Point_new_Symbol(5.,0.29,0.0,0.0,0.0,0.0,20,kOrange+7,1.3); 
  string leg_countA = "Eff+Acc+Res Corrected #phi BES-II";
  plotTopLegend((char*)leg_countA.c_str(),7.,0.288,0.03,1,0.0,42,0);
  ...
  PlotLine(2,6,0.278,0.278,1,2,2);
  string leg_line = "#rho_{00} = 1/3";
  plotTopLegend((char*)leg_line.c_str(),7.,0.278,0.03,1,0.0,42,0);


  string leg_energy = Form("AuAu %s %d", vmsa::mBeamEnergy[energy].c_str(), vmsa::mBeamYear[energy]);
  plotTopLegend((char*)leg_energy.c_str(),20,0.38,0.04,1,0.0,42,0);
  //plotTopLegend((char*)"20%-60%",30,0.37,0.04,1,0.0,42,0);
  ...
  plotSysErrorsBox(g_RawSys,kGray+2);
  ...
  plotSysErrorsBox(g_EffSys,kBlue);
  ...
  plotSysErrorsBox(g_AccSys,kOrange+7);
}
