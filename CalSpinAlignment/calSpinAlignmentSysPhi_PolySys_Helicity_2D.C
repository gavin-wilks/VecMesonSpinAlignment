...
void calSpinAlignmentSysPhi_PolySys_Helicity_2D(int energy = 4, int pid = 0, int year = 0, bool random3D = false, int order = 2, string etamode = "eta1_eta1")
{
  ...
  string inputfile = Form("../output/AuAu%s/%s/InvMassSubBg_%s_Helicity_2D.root",vmsa::mBeamEnergy[energy].c_str(),vmsa::mPID[pid].c_str(),etamode.c_str());
	  ...
	  for(int i_norm = vmsa::Norm_start; i_norm < vmsa::Norm_stop; ++i_norm)
	  {
	    ...
	    for(int i_theta = vmsa::CTS_start; i_theta < 9/*2*vmsa::CTS_stop*/; i_theta++) // cos(theta*) loop
	    {
	      for(int i_thetah = vmsa::CTS_start; i_thetah < 9/*2*vmsa::CTS_stop*/; i_thetah++) // cos(theta*) loop
	      {
	        ...
	        if(i_theta == vmsa::CTS_start && i_thetah == vmsa::CTS_start) h_mMass_InteTheta[KEY_InteTheta] = (TH1F*)h_mMass[KEY]->Clone(KEY_InteTheta.c_str());
...
//#endif
  ...
  */
	  ...
	  for(int i_sigma = vmsa::Sig_start; i_sigma < 1/*vmsa::Sig_stop*/; ++i_sigma)
	  {
	    for(int i_method = vmsa::Method_start; i_method < vmsa::Method_stop; ++i_method)
	    {
	        ...
	        for(int i_pt = vmsa::pt_rebin_first[energy]; i_pt < vmsa::pt_rebin_last[energy]; ++i_pt) // pt loop
	        {
	          ...
	          h_mCounts[KEY_counts] = new TH2F(KEY_counts.c_str(),KEY_counts.c_str(),9,-1.0,1.0,9,-1.0,1.0);
	          ...
	          for(int i_theta = vmsa::CTS_start; i_theta < 9/*2*vmsa::CTS_stop*/; ++i_theta) // cos(theta*) loop
	          {
	            ...
	            {
	              string KEY = Form("pt_%d_Centrality_%d_CosThetaStar_%d_CosThetaStarH_%d_%s_Dca_%d_Sig_%d_%s_Norm_%d",i_pt,i_cent,i_theta,i_thetah,EP[order-1].c_str(),i_dca,i_sig,vmsa::mPID[pid].c_str(),i_norm);
	              string KEY_Poly = Form("pt_%d_Centrality_%d_CosThetaStar_%d_CosThetaStarH_%d_%s_Dca_%d_Sig_%d_%s_Norm_%d_Poly%d",i_pt,i_cent,i_theta,i_thetah,EP[order-1].c_str(),i_dca,i_sig,vmsa::mPID[pid].c_str(),i_norm,i_poly+1);
	              TF1 *f_bw;
                      if(i_poly == 0) f_bw = new TF1("f_bw",PolyBreitWigner, vmsa::BW_Start[pid],vmsa::BW_Stop[pid],5);
                      if(i_poly == 1) f_bw = new TF1("f_bw",Poly2BreitWigner,vmsa::BW_Start[pid],vmsa::BW_Stop[pid],6);
                      if(i_poly == 2) f_bw = new TF1("f_bw",Poly3BreitWigner,vmsa::BW_Start[pid],vmsa::BW_Stop[pid],7);
	              ...
	              float bin_center = 1./9.+float(i_theta-4.5)/4.5;
	              ...
	              if(i_method == 0)
	              {
	                ...
	                h_mCounts[KEY_counts]->SetBinContent(h_mCounts[KEY_counts]->FindBin(bin_center,bin_centerh),counts-counts_bg);
	                h_mCounts[KEY_counts]->SetBinError(h_mCounts[KEY_counts]->FindBin(bin_center,bin_centerh),TMath::Sqrt(errors+errors_bg*errors_bg));
	              }
	              if(i_method == 1)
	              {
	                //float bin_width = h_mMass[KEY]->GetBinWidth(1);
	                ...
	                h_mCounts[KEY_counts]->SetBinContent(h_mCounts[KEY_counts]->FindBin(bin_center,bin_centerh),counts_bw-counts_bg);
	                h_mCounts[KEY_counts]->SetBinError(h_mCounts[KEY_counts]->FindBin(bin_center,bin_centerh),TMath::Sqrt(errors_bw*errors_bw+errors_bg*errors_bg));
	              }
	              Par[KEY_Poly].clear();
  ...
  for(int i_poly = 0; i_poly < 1/*3*/; i_poly++)
  {
    for(int i_norm = 0; i_norm < 1/*3*/; i_norm++)
    {
      ...
      {
        string outputname = Form("./figures/%s/%s/pTstudy/allThetaYields%s_%s_Order%d_cent%d_%s_pTdependence_Norm%d_Poly%d.pdf",vmsa::mPID[pid].c_str(),vmsa::mBeamEnergy[energy].c_str(),vmsa::mPID[pid].c_str(),vmsa::mBeamEnergy[energy].c_str(),order,i_cent,etamode.c_str(),i_norm,i_poly+1);
        ...
        for(int i_pt = vmsa::pt_rebin_first[energy]; i_pt < vmsa::pt_rebin_last[energy]; ++i_pt) // pt loop
        {
          ...
          { 
            ...
            for(int i_theta = 0; i_theta < 12; ++i_theta)
            {
              ...
              if(i_theta < 9)
              {
                string KEY_QA = Form("pt_%d_Centrality_%d_CosThetaStar_%d_CosThetaStarH_%d_%s_Dca_%d_Sig_%d_%s_Norm_%d",i_pt,i_cent,i_theta,i_thetah,EP[order-1].c_str(),vmsa::Dca_start,0/*vmsa::nSigKaon_start*/,vmsa::mPID[pid].c_str(),i_norm);
                h_mMass[KEY_QA]->SetTitle("");
                h_mMass[KEY_QA]->GetXaxis()->SetNdivisions(505,'N');
                h_mMass[KEY_QA]->GetXaxis()->SetLabelSize(0.03);
                h_mMass[KEY_QA]->GetXaxis()->SetTitle("M(K^{+},K^{-})");
                h_mMass[KEY_QA]->SetTitle(Form("%.2f<p_{T}<%.2f CTG=%d, CTH=%d",vmsa::pt_low[energy][i_pt],vmsa::pt_up[energy][i_pt],i_theta,i_thetah));
                h_mMass[KEY_QA]->GetXaxis()->SetTitleSize(0.05);
                h_mMass[KEY_QA]->GetXaxis()->SetTitleOffset(1.2);
                h_mMass[KEY_QA]->GetXaxis()->CenterTitle();
              ...
              if(i_theta == 9)
              {
                ...
                h_mMass_InteTheta[KEY_InteTheta_QA]->SetTitle("");
                h_mMass_InteTheta[KEY_InteTheta_QA]->SetTitle(Form("%.2f<p_{T}<%.2f, Full CTG",vmsa::pt_low[energy][i_pt],vmsa::pt_up[energy][i_pt]));
                h_mMass_InteTheta[KEY_InteTheta_QA]->GetXaxis()->SetNdivisions(505,'N');
                h_mMass_InteTheta[KEY_InteTheta_QA]->GetXaxis()->SetLabelSize(0.03);
                h_mMass_InteTheta[KEY_InteTheta_QA]->GetXaxis()->SetTitle("M(K^{+},K^{-})");
                h_mMass_InteTheta[KEY_InteTheta_QA]->GetXaxis()->SetTitleSize(0.05);
            ...
            for(int i_theta = vmsa::CTS_start; i_theta < 9/*&2*vmsa::CTS_stop*/; ++i_theta)
            {
              c_diff->cd(i_theta+1);
              string KEY_QA = Form("pt_%d_Centrality_%d_CosThetaStar_%d_CosThetaStarH_%d_%s_Dca_%d_Sig_%d_%s_Norm_%d",i_pt,i_cent,i_theta,i_thetah,EP[order-1].c_str(),vmsa::Dca_start,0/*vmsa::nSigKaon_start*/,vmsa::mPID[pid].c_str(),i_norm);
              string KEY_QA_Poly = Form("pt_%d_Centrality_%d_CosThetaStar_%d_CosThetaStarH_%d_%s_Dca_%d_Sig_%d_%s_Norm_%d_Poly%d",i_pt,i_cent,i_theta,i_thetah,EP[order-1].c_str(),vmsa::Dca_start,0/*vmsa::nSigKaon_start*/,vmsa::mPID[pid].c_str(),i_norm,i_poly+1);
              TF1 *f_bw;
              if(i_poly == 0) f_bw = new TF1("f_bw",PolyBreitWigner,vmsa::BW_Start[pid],vmsa::BW_Stop[pid],5);
              if(i_poly == 1) f_bw = new TF1("f_bw",Poly2BreitWigner,vmsa::BW_Start[pid],vmsa::BW_Stop[pid],6);
              if(i_poly == 2) f_bw = new TF1("f_bw",Poly3BreitWigner,vmsa::BW_Start[pid],vmsa::BW_Stop[pid],7);
          ...
          for(int i_sigma = 0/*vmsa::Sig_start*/; i_sigma < 1/*vmsa::Sig_stop*/; ++i_sigma)
          {
            for(int i_method = 1/*vmsa::Method_start*/; i_method < 2/*vmsa::Method_stop*/; ++i_method)
            {
              ...
              h_mCounts[KEY_counts_QA]->GetXaxis()->SetTitle("cos(#theta*)_{global}");
              ...
              h_mCounts[KEY_counts_QA]->GetYaxis()->SetTitle("cos(#theta*)_{helicity}");
 ...
 // {
  ...
  string outputfile = Form("../output/AuAu%s/%s/RawPhiPtSys_%s_PolySys_Helicity_2D.root",vmsa::mBeamEnergy[energy].c_str(),vmsa::mPID[pid].c_str(),etamode.c_str());
