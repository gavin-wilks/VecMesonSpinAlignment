...
void compRhoEffKStar(Int_t energy = 4, Int_t pid = 2, Int_t i_cent = 9)
{
  string inputfileRaw = Form("../output/AuAu%s/%s/Rho_RawSysErrors_F_0.root",vmsa::mBeamEnergy[energy].c_str(),vmsa::mPID[pid].c_str());
  string inputfileEff = Form("../output/AuAu%s/%s/Rho_EffSysErrors_F_0.root",vmsa::mBeamEnergy[energy].c_str(),vmsa::mPID[pid].c_str());
  string inputfileAcc = Form("../output/AuAu%s/%s/Rho_AccResSysErrors_F_0.root",vmsa::mBeamEnergy[energy].c_str(),vmsa::mPID[pid].c_str());
...
//  PlotLine(0.0,5.0,1.0/3.0,1.0/3.0,1,2,2);
  ...
  string leg_count = "Raw K*^{0} BES-II";
  ...
  string leg_countE = "Eff Corrected K*^{0} BES-II";
  ...
  string leg_countA = "Eff+Acc+Res Corrected K*^{0} BES-II";
  ...
  {
    double pt, val;
    g_RawStat->GetPoint(i,pt,val);
    g_RawStat->SetPoint(i,pt+0.06,val);
    g_RawSys->SetPoint(i,pt+0.06,val);
    g_EffStat->GetPoint(i,pt,val);
    g_EffStat->SetPoint(i,pt+0.12,val);
    g_EffSys->SetPoint(i,pt+0.12,val);
    g_AccStat->GetPoint(i,pt,val);
    g_AccStat->SetPoint(i,pt+0.18,val);
    g_AccSys->SetPoint(i,pt+0.18,val);
  }
