...
void compareGlobalVsHelicity_Options(Int_t energy = 4, Int_t pid = 0, Int_t i_cent = 9, string correction = "Raw", int order = 2, string etamode = "eta1_eta1", int yspectra = 23, std::string simmode = "Mc")//defaultF = 0 is BESII, defaultF = 1 is BESI
{
  ...
  for(int iopt = 0; iopt < nopts; iopt++)
  {
    ...
    {
      spectra[iopt][0][i] = Form("CEPT_O2_NoYSpec_%s_phieff_phiTPC_%s_ysig1000_rho0.3333_rerho0.0_imrho0.0_r0.0_i0.0_hrho%s_hrerho0.0_himrho0.0_hr0.0_hi0.0"   ,opts[iopt].c_str(),date[iopt].c_str(),sigmay_text[i].c_str());
      spectra[iopt][1][i] = Form("CEPT_O2_NoYSpec_%s_phieff_phiTPC_%s_ysig1000_rho0.3333_rerho0.0_imrho0.0_r0.0_i0.0_hrho0.3333_hrerho0.0_himrho0.0_hr%s_hi0.0",opts[iopt].c_str(),date[iopt].c_str(),sigmay_text_offdiag[i].c_str());
      spectra[iopt][2][i] = Form("CEPT_O2_NoYSpec_%s_phieff_phiTPC_%s_ysig1000_rho0.3333_rerho0.0_imrho0.0_r0.0_i0.0_hrho0.3333_hrerho0.0_himrho0.0_hr0.0_hi%s",opts[iopt].c_str(),date[iopt].c_str(),sigmay_text_offdiag[i].c_str());
      spectra[iopt][3][i] = Form("CEPT_O2_NoYSpec_%s_phieff_phiTPC_%s_ysig1000_rho0.3333_rerho0.0_imrho0.0_r0.0_i0.0_hrho0.3333_hrerho%s_himrho0.0_hr0.0_hi0.0",opts[iopt].c_str(),date[iopt].c_str(),sigmay_text_offdiag[i].c_str());
      spectra[iopt][4][i] = Form("CEPT_O2_NoYSpec_%s_phieff_phiTPC_%s_ysig1000_rho0.3333_rerho0.0_imrho0.0_r0.0_i0.0_hrho0.3333_hrerho0.0_himrho%s_hr0.0_hi0.0",opts[iopt].c_str(),date[iopt].c_str(),sigmay_text_offdiag[i].c_str());
    }
  }
  ...
  std::string outputname = Form("figures/%s/%s/pTstudy/GlobalvsHelicity_parametercomparison_%s_%s.pdf",vmsa::mPID[pid].c_str(),vmsa::mBeamEnergy[energy].c_str(),simmode.c_str(),option.c_str());
    ...
    for(int irho = 0; irho < 5; irho++)
    {
      ...
      for(int par = 0; par < 5; par++)
      {
        ...
        for(int iopt = 0; iopt < nvar; iopt++)
        {
          ...
          g2D_Global[iopt][irho][par][i]->SetTitle(Form("Input %s^{Helicity}, %1.1f<p_{T}<%1.1f GeV/c",param[irho].c_str(),ptedges[i],ptedges[i+1]));
          ...
          if(i == 0 && irho == 0 && par == 0) leg[irho]->AddEntry(g2D_Global[iopt][irho][par][i],Form("%s^{Helicity} = %1.4f",param[irho].c_str(),sigmay[iopt]),"p");
          if(i == 0 && irho != 0 && par == 0) leg[irho]->AddEntry(g2D_Global[iopt][irho][par][i],Form("%s^{Helicity} = %1.4f",param[irho].c_str(),sigmay_offdiag[iopt]),"p");
        }
        leg[irho]->Draw("same");
      }
    }
  ...
  outputname = Form("figures/%s/%s/pTstudy/HelicityvsHelicity_parametercomparison_%s_%s.pdf",vmsa::mPID[pid].c_str(),vmsa::mBeamEnergy[energy].c_str(),simmode.c_str(),option.c_str());
