...
void calSpinAlignmentSysPhiEta(int energy = 4, int pid = 0, int year = 0, bool random3D = false)
{
  ...
  string inputfile = Form("../output/AuAu%s/%s/InvMassSubBgEta.root",vmsa::mBeamEnergy[energy].c_str(),vmsa::mPID[pid].c_str());
  ...
  */
  ...
  string outputfile = Form("../output/AuAu%s/%s/RawRhoEtaSys.root",vmsa::mBeamEnergy[energy].c_str(),vmsa::mPID[pid].c_str());
