...
void calSysError_FixedSys_Poly_Helicity(Int_t energy = 4, Int_t pid = 0, Int_t i_cent = 9, string correction = "Raw", bool random3D = false, int order = 2, int defaultF = 0, string etamode = "eta1_eta1", int yspectra = -1)//defaultF = 0 is BESII, defaultF = 1 is BESI
{
  ...
  string inputfileHframe = Form("../output/AuAu%s/%s/RawPhiPtSys_eta1_eta1_PolySys_Helicity.root",vmsa::mBeamEnergy[energy].c_str(),vmsa::mPID[pid].c_str());
  ...
  string inputfile = Form("../output/AuAu%s/%s/Poly/%sPhiPtSys_%s_Poly%s_Helicity.root",vmsa::mBeamEnergy[energy].c_str(),vmsa::mPID[pid].c_str(),correction.c_str(),etamode.c_str(),spectra.c_str());
...
*/
  ...
  h_frame->GetYaxis()->SetRangeUser(0.22,0.34);
  h_frame->GetYaxis()->SetTitle("Helicity Frame #rho_{00}");
...
//  PlotLine(0.0,5.0,1.0/3.0,1.0/3.0,1,2,2);
  ...
  Draw_TGAE_Point_new_Symbol(0.5,0.27,0.0,0.0,0.0,0.0,style_phi_2nd,color_phi_2nd,size_marker+0.2); 
  Draw_TGAE_Point_new_Symbol(0.5,0.26,0.0,0.0,0.0,0.0,style_phi_1st,color_phi_1st,size_marker-0.2); 
  string leg_count;
  if(correction == "EffAcc") leg_count = "#phi EffAcc Corrected";
  if(correction == "Raw") leg_count = "#phi Raw";
  if(correction == "AccRes") leg_count = "#phi BES-II (|y| < 1.0)";
  plotTopLegend((char*)leg_count.c_str(),0.65,0.269,0.03,1,0.0,42,0);
  plotTopLegend("#Delta<cos(2#phi*-2#phi)> = 0",0.65,0.259,0.03,1,0.0,42,0);
  ...
  //plotTopLegend((char*)leg_besi.c_str(),0.65,0.357,0.03,1,0.0,42,0);
  }
  if(energy == 5){
  ...
  plotTopLegend((char*)leg_energy.c_str(),0.65,0.245,0.04,1,0.0,42,0);
  plotTopLegend((char*)"20%-60%",1.0,0.235,0.04,1,0.0,42,0);
  ...
  TGraphAsymmErrors *gzero = new TGraphAsymmErrors();
  gzero->SetPoint(0,1.5,0.292105);
  gzero->SetPoint(1,2.1,0.280381);
  gzero->SetPoint(2,2.7,0.262049);
  gzero->SetPoint(3,3.6,0.232921);

  //g_StatErrors->SetLineWidth(2);
  //besi19->SetLineWidth(2);
  Draw_TGAE_new_Symbol((TGraphAsymmErrors*)gzero,style_phi_1st,color_phi_1st,colorDiff_phi,size_marker-0.2);
  ...
  string OutPutFile = Form("../output/AuAu%s/%s/Rho_%sSysErrors_F_%d_%s_PolySys%s_Helicity.root",vmsa::mBeamEnergy[energy].c_str(),vmsa::mPID[pid].c_str(),correction.c_str(),defaultF,etamode.c_str(),spectra.c_str());
