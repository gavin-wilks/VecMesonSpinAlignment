...
void calSpinAlignmentSysPhi_PolySys_ptyspectra(int energy = 4, int pid = 0, int year = 0, bool random3D = false, int order = 2, string etamode = "eta1_eta1")
{
  ...
  string inputfile = Form("../output/AuAu%s/%s/ptyspectra_Reweight_InvMassSubBg_%s.root",vmsa::mBeamEnergy[energy].c_str(),vmsa::mPID[pid].c_str(),etamode.c_str());
  ...
  for(int i_pt = 0; i_pt < vmsa::tofrebinpttotal; i_pt++) // pt loop
  {
	...
	for(Int_t i_sig = vmsa::nSigKaon_start; i_sig < 1/*vmsa::nSigKaon_stop*/; i_sig++)
	{
	  ...
	  {
	    for(int i_eta = 0; i_eta < vmsa::tofrebinytotal; i_eta++) // cos(theta*) loop
	    {
  ...
  float InvMassLow[vmsa::tofrebinpttotal][vmsa::tofrebinytotal] = {0.0};
  float InvMassHigh[vmsa::tofrebinpttotal][vmsa::tofrebinytotal] = {0.0};
  float fpoly[vmsa::tofrebinpttotal][vmsa::tofrebinytotal] = {0.0};
	...
	for(int i_norm = vmsa::Norm_start; i_norm < 1; ++i_norm)
	{
	  for(int i_sigma = vmsa::Sig_start; i_sigma < 1/*vmsa::Sig_stop*/; ++i_sigma)
	  {
	    for(int i_method = 1/*vmsa::Method_start*/; i_method < vmsa::Method_stop; ++i_method)
	    {
	        ...
	        h_mSpectra[KEY_spectra] = new TH2F(KEY_spectra.c_str(),KEY_spectra.c_str(),vmsa::tofrebinpttotal,vmsa::tofrebinptval,vmsa::tofrebinytotal,vmsa::tofrebinyval);
	        h_mSpectra[KEY_spectra]->SetStats(0);
                cout << "Created 2D plot" << endl;
	        for(int i_pt = 0/*vmsa::pt_start*/; i_pt < vmsa::tofrebinpttotal/*< vmsa::pt_stop*/; ++i_pt) // pt loop
	        {
                  cout << "i_pt = " << i_pt << endl;
	          ...
	          for(int i_eta = 0; i_eta < vmsa::tofrebinytotal; ++i_eta) // cos(theta*) loop
	          {
                    ...
                    double pt_mean = (vmsa::tofrebinptval[i_pt]+vmsa::tofrebinptval[i_pt+1])/2.0;
                    double y_mean =  (vmsa::tofrebinyval[i_eta]+vmsa::tofrebinyval[i_eta+1])/2.0;
	            ...
	            float norm = h_mMass[KEY]->GetMaximum();
	  ...
	  for(int i_sigma = vmsa::Sig_start; i_sigma < 1/*vmsa::Sig_stop*/; ++i_sigma)
	  {
	    for(int i_method = 1/*vmsa::Method_start*/; i_method < vmsa::Method_stop; ++i_method)
	    {
	        ...
	        for(int i_pt = 0/*vmsa::pt_start*/; i_pt < vmsa::tofrebinpttotal/*< vmsa::pt_stop*/; ++i_pt) // pt loop
	        {
	          ...
	          string KEY_int = Form("intercept_pt_%d_Centrality_%d_%s_Dca_%d_Sig_%d_%s_Norm_%d",i_pt,i_cent,EP[order-1].c_str(),i_dca,i_sig,vmsa::mPID[pid].c_str(),i_norm);
                  g_mIntercept[KEY_int]->SetTitle(Form("%.1f<p_{T}<%.1f GeV/c",vmsa::tofrebinptval[i_pt],vmsa::tofrebinptval[i_pt+1]));
	          ...
	          string KEY_slope = Form("slope_pt_%d_Centrality_%d_%s_Dca_%d_Sig_%d_%s_Norm_%d",i_pt,i_cent,EP[order-1].c_str(),i_dca,i_sig,vmsa::mPID[pid].c_str(),i_norm);
                  g_mSlope[KEY_slope]->SetTitle(Form("%.1f<p_{T}<%.1f GeV/c",vmsa::tofrebinptval[i_pt],vmsa::tofrebinptval[i_pt+1]));
	  ...
	  for(int i_sigma = vmsa::Sig_start; i_sigma < 1/*vmsa::Sig_stop*/; ++i_sigma)
	  {
	    for(int i_method = 1/*vmsa::Method_start*/; i_method < vmsa::Method_stop; ++i_method)
	    {
	        ...
	        for(int i_pt = 0/*vmsa::pt_start*/; i_pt < vmsa::tofrebinpttotal/*< vmsa::pt_stop*/; ++i_pt) // pt loop
	        {
                  ...
                  c_diff->cd(i_pt+1)->SetGrid(0,0);
	          string KEY_total = Form("total_pt_%d_Centrality_%d_%s_Dca_%d_Sig_%d_%s_Norm_%d",i_pt,i_cent,EP[order-1].c_str(),i_dca,i_sig,vmsa::mPID[pid].c_str(),i_norm);
                  g_mTotal[KEY_total]->SetTitle(Form("%.1f<p_{T}<%.1f GeV/c",vmsa::tofrebinptval[i_pt],vmsa::tofrebinptval[i_pt+1]));
	  ...
	  for(int i_sigma = vmsa::Sig_start; i_sigma < 1/*vmsa::Sig_stop*/; ++i_sigma)
	  {
	    for(int i_method = 1/*vmsa::Method_start*/; i_method < vmsa::Method_stop; ++i_method)
	    {
	        ...
	        for(int i_pt = 0/*vmsa::pt_start*/; i_pt < vmsa::tofrebinpttotal/*< vmsa::pt_stop*/; ++i_pt) // pt loop
	        {
                  ...
                  c_diff->cd(i_pt+1)->SetGrid(0,0);
	          string KEY_poly = Form("poly_pt_%d_Centrality_%d_%s_Dca_%d_Sig_%d_%s_Norm_%d",i_pt,i_cent,EP[order-1].c_str(),i_dca,i_sig,vmsa::mPID[pid].c_str(),i_norm);
                  g_mPoly[KEY_poly]->SetTitle(Form("%.1f<p_{T}<%.1f GeV/c",vmsa::tofrebinptval[i_pt],vmsa::tofrebinptval[i_pt+1]));
	  ...
	  for(int i_sigma = vmsa::Sig_start; i_sigma < 1/*vmsa::Sig_stop*/; ++i_sigma)
	  {
	    for(int i_method = 1/*vmsa::Method_start*/; i_method < vmsa::Method_stop; ++i_method)
	    {
	        ...
	        for(int i_pt = 0/*vmsa::pt_start*/; i_pt < vmsa::tofrebinpttotal/*< vmsa::pt_stop*/; ++i_pt) // pt loop
	        {
                  ...
                  c_diff->cd(i_pt+1)->SetGrid(0,0);
	          string KEY_totalpoly = Form("totalpoly_pt_%d_Centrality_%d_%s_Dca_%d_Sig_%d_%s_Norm_%d",i_pt,i_cent,EP[order-1].c_str(),i_dca,i_sig,vmsa::mPID[pid].c_str(),i_norm);
                  g_mTotalPoly[KEY_totalpoly]->SetTitle(Form("%.1f<p_{T}<%.1f GeV/c",vmsa::tofrebinptval[i_pt],vmsa::tofrebinptval[i_pt+1]));
  ...
  cout << endl << "const float InvMassLow[vmsa::tofrebinpttotal][vmsa::tofrebinytotal] = {";
  for(int ptbin = 0; ptbin < vmsa::tofrebinpttotal; ptbin++)
  {
    cout << "{";
    for(int etabin = 0; etabin < vmsa::tofrebinytotal; etabin++)
    {
      if(etabin < vmsa::tofrebinytotal-1) cout << InvMassLow[ptbin][etabin] << ",";
      else                             cout << InvMassLow[ptbin][etabin];
    }
    if(ptbin < vmsa::tofrebinpttotal -1 ) cout << "}," << endl;
    else                                cout << "}";
  }
  cout << "};" << endl << endl;

  cout << endl << "const float InvMassHigh[vmsa::tofrebinpttotal][vmsa::tofrebinytotal] = {";
  for(int ptbin = 0; ptbin < vmsa::tofrebinpttotal; ptbin++)
  {
    cout << "{";
    for(int etabin = 0; etabin < vmsa::tofrebinytotal; etabin++)
    {
      if(etabin < vmsa::tofrebinytotal-1) cout << InvMassHigh[ptbin][etabin] << ",";
  ...
  cout << endl << "const float fpoly[vmsa::tofrebinpttotal][vmsa::tofrebinytotal] = {";
  for(int ptbin = 0; ptbin < vmsa::tofrebinpttotal; ptbin++)
  {
    cout << "{";
    for(int etabin = 0; etabin < vmsa::tofrebinytotal; etabin++)
    {
      if(etabin < vmsa::tofrebinytotal-1) cout << fpoly[ptbin][etabin] << ",";
  ...
  for(int i_poly = 0; i_poly < 1; i_poly++)
  {
    for(int i_norm = 0; i_norm < 1; i_norm++)
    {
      ...
      {
        string outputname = Form("./figures/%s/%s/pTstudy/ptyspectra_allThetaYields%s_%s_Order%d_cent%d_%s_pTdependence_Norm%d_Poly%d.pdf",vmsa::mPID[pid].c_str(),vmsa::mBeamEnergy[energy].c_str(),vmsa::mPID[pid].c_str(),vmsa::mBeamEnergy[energy].c_str(),order,i_cent,etamode.c_str(),i_norm,i_poly+1);
        string output_start = Form("%s[",outputname.c_str());
	...
	for(int i_pt = 0/*vmsa::pt_start*/; i_pt < vmsa::tofrebinpttotal/*< vmsa::pt_stop*/; ++i_pt) // pt loop
	  ...
	  for(int i_eta = 0; i_eta < vmsa::tofrebinytotal; i_eta++) // cos(theta*) loop
	  {
            ...
            h_mMass[KEY_QA]->SetTitle(Form("%.2f<p_{T}<%.2f, %.2f<y<%.2f",vmsa::tofrebinptval[i_pt],vmsa::tofrebinptval[i_pt+1],vmsa::tofrebinyval[i_eta],vmsa::tofrebinyval[i_eta+1]));
  ...
  for(int i_poly = 0; i_poly < 1/*3*/; i_poly++)
  {
    for(int i_norm = 0; i_norm < 1; i_norm++)
    {
      for(int i_cent = 9; i_cent < 10; ++i_cent) // Centrality loop
      {
        string outputname = Form("./figures/%s/%s/pTstudy/ptyspectra_2Dyield%s_%s_Order%d_cent%d_%s_pTdependence_Norm%d_Poly%d.pdf",vmsa::mPID[pid].c_str(),vmsa::mBeamEnergy[energy].c_str(),vmsa::mPID[pid].c_str(),vmsa::mBeamEnergy[energy].c_str(),order,i_cent,etamode.c_str(),i_norm,i_poly+1);
  ...
  string outputfile = Form("../output/AuAu%s/%s/ptyspectra_Reweight_RawPhiPtSys_%s_PolySys.root",vmsa::mBeamEnergy[energy].c_str(),vmsa::mPID[pid].c_str(),etamode.c_str());
