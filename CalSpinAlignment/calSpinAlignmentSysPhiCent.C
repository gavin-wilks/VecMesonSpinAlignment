...
void calSpinAlignmentSysPhiCent(int energy = 4, int pid = 0, int year = 0, bool random3D = false)
{
  ...
  for(int i_theta = 0; i_theta < 9; ++i_theta)
  {
    ...
    {
      string KEY_QA = Form("Centrality_%d_CosThetaStar_%d_2nd_Dca_%d_Sig_%d_%s_Norm_%d",4,i_theta,vmsa::Dca_start,vmsa::nSigKaon_start,vmsa::mPID[pid].c_str(),vmsa::Norm_QA);
    ...
    if(i_theta == vmsa::CTS_stop)
    {
      string KEY_InteTheta_QA = Form("Centrality_%d_2nd_Dca_%d_Sig_%d_%s_Norm_%d",4,vmsa::Dca_start,vmsa::nSigKaon_start,vmsa::mPID[pid].c_str(),vmsa::Norm_QA);
  ...
  */
  ...
  for(int i_sigma = vmsa::Sig_start; i_sigma < vmsa::Sig_stop; ++i_sigma)
  {
    for(int i_method = vmsa::Method_start; i_method < vmsa::Method_stop; ++i_method)
    {
      string KEY_counts_QA = Form("Centrality_%d_2nd_Dca_%d_Sig_%d_%s_Norm_%d_Sigma_%d_%s",4,vmsa::Dca_start,vmsa::nSigKaon_start,vmsa::mPID[pid].c_str(),vmsa::Norm_QA,i_sigma,vmsa::mInteMethod[i_method].c_str());
