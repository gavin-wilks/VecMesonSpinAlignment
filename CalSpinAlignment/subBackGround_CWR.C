...
void subBackGround_CWR(int energy = 5, int pid = 0, int year = 0, string date = "20240424", bool random3D = false, int order = 2, string etamode = "eta1_eta1")
{
  std::string EP[2] = {"1st","2nd"};
  TGaxis::SetMaxDigits(4);
  ROOT::Math::MinimizerOptions::SetDefaultMaxFunctionCalls(50000);

  string InPutFile = Form("../output/AuAu%s/%s/CWRPrepped.root",vmsa::mBeamEnergy[energy].c_str(),vmsa::mPID[pid].c_str());
  TFile *File_SE = TFile::Open(InPutFile.c_str());
  ...
  {
    string KEY_InPutSE = Form("Centrality_9_CosThetaStar_%d_SE_Rebinned_CWR",i_theta);
    cout << KEY_InPutSE << endl;
    h_mInPut_SE[KEY_InPutSE] = (TH1F*)File_SE->Get(KEY_InPutSE.c_str())->Clone(); 

    string KEY_InPutME = Form("Centrality_9_CosThetaStar_%d_ME_Rebinned_CWR",i_theta);
    ...
    {
      string KEY_SE = Form("Centrality_9_CosThetaStar_%d_%s_Dca_%d_Sig_%d_%s_Norm_%d_SE",i_theta,EP[order-1].c_str(),0,0,vmsa::mPID[pid].c_str(),0);
      h_mMass_SE[KEY_SE] = (TH1F*)h_mInPut_SE[KEY_InPutSE]->Clone(KEY_SE.c_str());
      string KEY_ME = Form("Centrality_9_CosThetaStar_%d_%s_Dca_%d_Sig_%d_%s_Norm_%d_ME",i_theta,EP[order-1].c_str(),0,0,vmsa::mPID[pid].c_str(),0);
      h_mMass_ME[KEY_ME] = (TH1F*)h_mInPut_ME[KEY_InPutME]->Clone(KEY_ME.c_str());
      string KEY_SM = Form("Centrality_9_CosThetaStar_%d_%s_Dca_%d_Sig_%d_%s_Norm_%d_SM",i_theta,EP[order-1].c_str(),0,0,vmsa::mPID[pid].c_str(),0);
  ...
  //  string KEY_SE_QA = Form("pt_%d_Centrality_%d_CosThetaStar_%d_%s_Dca_%d_Sig_%d_%s_Norm_%d_SE",i_pt,9,vmsa::CTS_start,EP[order-1].c_str(),vmsa::Dca_start,vmsa::nSigKaon_start,vmsa::mPID[pid].c_str(),vmsa::Norm_QA);
  ...
  //  string KEY_ME_QA = Form("pt_%d_Centrality_%d_CosThetaStar_%d_%s_Dca_%d_Sig_%d_%s_Norm_%d_ME",i_pt,9,vmsa::CTS_start,EP[order-1].c_str(),vmsa::Dca_start,vmsa::nSigKaon_start,vmsa::mPID[pid].c_str(),vmsa::Norm_QA);
  ...
  //  string KEY_SM_QA = Form("pt_%d_Centrality_%d_CosThetaStar_%d_%s_Dca_%d_Sig_%d_%s_Norm_%d_SM",i_pt,9,vmsa::CTS_start,EP[order-1].c_str(),vmsa::Dca_start,vmsa::nSigKaon_start,vmsa::mPID[pid].c_str(),vmsa::Norm_QA);
  ...
  //  c_peak->SaveAs(Form("figures/%s/%s/pTstudy/CloseUpMixedEvent_%s_Order%d_Cent0_pt%d.pdf",vmsa::mPID[pid].c_str(),vmsa::mBeamEnergy[energy].c_str(),vmsa::mBeamEnergy[energy].c_str(),order,i_pt));
  //}

  // QA Plots for pT bins
  TCanvas *c_pT = new TCanvas("c_pT","c_pT",10,10,1200,600);
  c_pT->Divide(4,2);

  string outputname = Form("figures/%s/%s/pTstudy/CWR_InvMassDistributions_%s_Order%d_Cent9.pdf",vmsa::mPID[pid].c_str(),vmsa::mBeamEnergy[energy].c_str(),vmsa::mBeamEnergy[energy].c_str(),order);
  ...
  {
    c_pT->cd(itheta+1);
    c_pT->cd(itheta+1)->SetLeftMargin(0.15);
    c_pT->cd(itheta+1)->SetBottomMargin(0.15);
    c_pT->cd(itheta+1)->SetTicks(1,1);
    c_pT->cd(itheta+1)->SetGrid(0,0);
    string KEY_SE_QA = Form("Centrality_9_CosThetaStar_%d_%s_Dca_%d_Sig_%d_%s_Norm_%d_SE",itheta,EP[order-1].c_str(),vmsa::Dca_start,vmsa::nSigKaon_start,vmsa::mPID[pid].c_str(),vmsa::Norm_QA);
    h_mMass_SE[KEY_SE_QA]->GetYaxis()->SetRangeUser(-0.1*h_mMass_SE[KEY_SE_QA]->GetMaximum(),1.1*h_mMass_SE[KEY_SE_QA]->GetMaximum());
    h_mMass_SE[KEY_SE_QA]->SetTitle(Form("20-60, %d/7<|cos(#theta*)|<%d/7",itheta,itheta+1));
    h_mMass_SE[KEY_SE_QA]->SetLineColor(kBlack);
    h_mMass_SE[KEY_SE_QA]->SetMarkerColor(kBlack);
    h_mMass_SE[KEY_SE_QA]->SetMarkerStyle(1);
    h_mMass_SE[KEY_SE_QA]->DrawCopy();

    string KEY_ME_QA = Form("Centrality_9_CosThetaStar_%d_%s_Dca_%d_Sig_%d_%s_Norm_%d_ME",itheta,EP[order-1].c_str(),vmsa::Dca_start,vmsa::nSigKaon_start,vmsa::mPID[pid].c_str(),vmsa::Norm_QA);
    h_mMass_ME[KEY_ME_QA]->SetLineColor(2);
    h_mMass_ME[KEY_ME_QA]->SetFillColor(2);
    h_mMass_ME[KEY_ME_QA]->SetFillStyle(3002);
    h_mMass_ME[KEY_ME_QA]->DrawCopy("h same");

    string KEY_SM_QA = Form("Centrality_9_CosThetaStar_%d_%s_Dca_%d_Sig_%d_%s_Norm_%d_SM",itheta,EP[order-1].c_str(),vmsa::Dca_start,vmsa::nSigKaon_start,vmsa::mPID[pid].c_str(),vmsa::Norm_QA);
    h_mMass_SM[KEY_SM_QA]->SetLineColor(4);
    h_mMass_SM[KEY_SM_QA]->SetFillColor(4);
    h_mMass_SM[KEY_SM_QA]->SetMarkerColor(4);
    h_mMass_SM[KEY_SM_QA]->SetMarkerStyle(1);
    h_mMass_SM[KEY_SM_QA]->SetFillStyle(3004);
    h_mMass_SM[KEY_SM_QA]->DrawCopy("h same");

    //string pT_range = Form("[%.2f,%.2f]",ptbinedge[i_pt],ptbinedge[i_pt+1]);
...
//
//  outputname = Form("figures/%s/%s/pTstudy/InvMassBackgroundSubtracted_Rebinned_%s_Order%d_Cent9.pdf",vmsa::mPID[pid].c_str(),vmsa::mBeamEnergy[energy].c_str(),vmsa::mBeamEnergy[energy].c_str(),order);
...
//#endif
  ...
  string outputfile = Form("../output/AuAu%s/%s/CWR_InvMassSubBg_%s.root",vmsa::mBeamEnergy[energy].c_str(),vmsa::mPID[pid].c_str(),etamode.c_str());
  TFile *File_OutPut = new TFile(outputfile.c_str(),"RECREATE");
  File_OutPut->cd();
  for(int i_theta = vmsa::CTS_start; i_theta < vmsa::CTS_stop; i_theta++) // cos(theta*) loop
  {
    string KEY_SM = Form("Centrality_9_CosThetaStar_%d_%s_Dca_%d_Sig_%d_%s_Norm_%d_SM",i_theta,EP[order-1].c_str(),0,0,vmsa::mPID[pid].c_str(),0);
    string KEY    = Form("Centrality_9_CosThetaStar_%d",i_theta);
