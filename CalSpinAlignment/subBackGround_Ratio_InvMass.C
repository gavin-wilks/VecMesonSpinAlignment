...
void subBackGround_Ratio_InvMass(int energy = 3, int pid = 0, int year = 0, string date = "20230625", bool random3D = false, int order = 2, string etamode = "eta1_eta1")
{
  std::string EP[2] = {"","2nd"};
  ...
  //string outputname = Form("figures/%s/%s/pTstudy/InvMassDistributions_%s_Order%d_Cent9.pdf",vmsa::mPID[pid].c_str(),vmsa::mBeamEnergy[energy].c_str(),vmsa::mBeamEnergy[energy].c_str(),order);
    ...
    for(Int_t i_dca = vmsa::Dca_start; i_dca < vmsa::Dca_stop; i_dca++)
    {
      for(Int_t i_sig = vmsa::nSigKaon_start; i_sig < vmsa::nSigKaon_stop; i_sig++)
      {
        ...
        for(int i_norm = 0; i_norm < 1; ++i_norm)
        {
          ...
          {
            string KEY = Form("pt_%d_Centrality_%d_%s_Dca_%d_Sig_%d_%s_Norm_%d",pt_bin,i_cent,EP[order-1].c_str(),i_dca,i_sig,vmsa::mPID[pid].c_str(),i_norm);
            string KEY_Add_SE = Form("pt_%d_Centrality_%d_%s_Dca_%d_Sig_%d_%s_Norm_%d_SE",pt_bin,i_cent,EP[order-1].c_str(),i_dca,i_sig,vmsa::mPID[pid].c_str(),i_norm);
            string KEY_Add_ME = Form("pt_%d_Centrality_%d_%s_Dca_%d_Sig_%d_%s_Norm_%d_ME",pt_bin,i_cent,EP[order-1].c_str(),i_dca,i_sig,vmsa::mPID[pid].c_str(),i_norm);
            for(int i_theta = vmsa::CTS_start; i_theta < vmsa::CTS_stop; i_theta++) // cos(theta*) loop
            {
              ...
              //string KEY_SE = Form("pt_%d_Centrality_%d_CosThetaStar_%d_%s_Dca_%d_Sig_%d_%s_Norm_%d_SE",pt_bin,i_cent,i_theta,EP[order-1].c_str(),i_dca,i_sig,vmsa::mPID[pid].c_str(),i_norm);
              //string KEY_ME = Form("pt_%d_Centrality_%d_CosThetaStar_%d_%s_Dca_%d_Sig_%d_%s_Norm_%d_ME",pt_bin,i_cent,i_theta,EP[order-1].c_str(),i_dca,i_sig,vmsa::mPID[pid].c_str(),i_norm);
              for(int i_pt = vmsa::pt_rebin_start[energy][pt_bin]; i_pt <= vmsa::pt_rebin_stop[energy][pt_bin]; i_pt++)
              {
      	        ...
      	        if(i_pt == vmsa::pt_rebin_start[energy][pt_bin] && i_theta == 0)
      	        {
      	          h_mMass_Add_SE[KEY_Add_SE] = (TH1F*)h_mMass_SE[KEY_SE]->Clone(KEY_Add_SE.c_str());
      	          h_mMass_Add_ME[KEY_Add_ME] = (TH1F*)h_mMass_ME[KEY_ME]->Clone(KEY_Add_ME.c_str());
      	        }
                if(i_pt > vmsa::pt_rebin_start[energy][pt_bin] && i_pt < vmsa::pt_rebin_stop[energy][pt_bin])
                {
      	          h_mMass_Add_SE[KEY_Add_SE]->Add(h_mMass_SE[KEY_SE]);
      	          h_mMass_Add_ME[KEY_Add_ME]->Add(h_mMass_ME[KEY_ME]);
                }
      	        if(i_pt == vmsa::pt_rebin_stop[energy][pt_bin] && i_theta == 6)
      	        {
      	          h_mMass[KEY] = (TH1F*)h_mMass_Add_SE[KEY_Add_SE]->Clone(KEY.c_str());
                  h_mMass[KEY]->Divide(h_mMass_Add_ME[KEY_Add_ME]);
      	        }
              }
            }
          }
...
//#endif
  ...
  string outputfile = Form("../output/AuAu%s/%s/InvMassSubBg_%s_InvMass_Ratios.root",vmsa::mBeamEnergy[energy].c_str(),vmsa::mPID[pid].c_str(),etamode.c_str());
  ...
  for(int i_pt = vmsa::pt_rebin_first[energy]; i_pt < vmsa::pt_rebin_last[energy]; i_pt++) // pt loop
  {
	...
	for(Int_t i_sig = vmsa::nSigKaon_start; i_sig < vmsa::nSigKaon_stop; i_sig++)
	{
	  ...
	  //{
	    ...
	    for(int i_norm = 0; i_norm < 1; ++i_norm)
	    {
	      string KEY = Form("pt_%d_Centrality_%d_%s_Dca_%d_Sig_%d_%s_Norm_%d",i_pt,i_cent,EP[order-1].c_str(),i_dca,i_sig,vmsa::mPID[pid].c_str(),i_norm);
	      h_mMass[KEY]->Write();
	    }
	  //}
	}
