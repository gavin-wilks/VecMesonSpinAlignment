...
void subBackGroundCent_Helicity_2D_OffDiag(int energy = 4, int pid = 0, int year = 0, string date = "20240722", bool random3D = false, int order = 2, std::string etamode = "eta1_eta1")
{
        ...
        for(Int_t i_phi = 0; i_phi < 12; i_phi++) // phi-psi bin
        {
          for(Int_t i_dca = vmsa::Dca_start; i_dca < vmsa::Dca_stop; i_dca++)
          {
            for(Int_t i_sig = vmsa::nSigKaon_start; i_sig < vmsa::nSigKaon_stop; i_sig++)
            {
              ...
              string KEY_InPutSE = Form("pt_%d_Centrality_%d_CosThetaStar_%d_Phi_%d_%s_Dca_%d_Sig_%d_%s_SE",i_pt,i_cent,i_theta,i_phi,EP[order-1].c_str(),i_dca,i_sig,vmsa::mPID[pid].c_str());
              cout << KEY_InPutSE << endl;
              h_mInPut_SE[KEY_InPutSE] = (TH1F*)File_SE->Get(KEY_InPutSE.c_str())->Clone(); 

              string KEY_InPutME = Form("pt_%d_Centrality_%d_CosThetaStar_%d_Phi_%d_%s_Dca_%d_Sig_%d_%s_ME",i_pt,i_cent,i_theta,i_phi,EP[order-1].c_str(),i_dca,i_sig,vmsa::mPID[pid].c_str());
              ...
              {
                string KEY_SE = Form("pt_%d_Centrality_%d_CosThetaStar_%d_Phi_%d_%s_Dca_%d_Sig_%d_%s_Norm_%d_SE",i_pt,i_cent,i_theta,i_phi,EP[order-1].c_str(),i_dca,i_sig,vmsa::mPID[pid].c_str(),i_norm);
                h_mMass_SE[KEY_SE] = (TH1F*)h_mInPut_SE[KEY_InPutSE]->Clone(KEY_SE.c_str());
                string KEY_ME = Form("pt_%d_Centrality_%d_CosThetaStar_%d_Phi_%d_%s_Dca_%d_Sig_%d_%s_Norm_%d_ME",i_pt,i_cent,i_theta,i_phi,EP[order-1].c_str(),i_dca,i_sig,vmsa::mPID[pid].c_str(),i_norm);
                h_mMass_ME[KEY_ME] = (TH1F*)h_mInPut_ME[KEY_InPutME]->Clone(KEY_ME.c_str());
                string KEY_SM = Form("pt_%d_Centrality_%d_CosThetaStar_%d_Phi_%d_%s_Dca_%d_Sig_%d_%s_Norm_%d_SM",i_pt,i_cent,i_theta,i_phi,EP[order-1].c_str(),i_dca,i_sig,vmsa::mPID[pid].c_str(),i_norm);
  ...
  string KEY_SE_QA = Form("pt_%d_Centrality_%d_CosThetaStar_%d_Phi_%d_%s_Dca_%d_Sig_%d_%s_Norm_%d_SE",vmsa::pt_RawQA[energy],vmsa::centStart,vmsa::CTS_start,5,EP[order-1].c_str(),vmsa::Dca_start,vmsa::nSigKaon_start,vmsa::mPID[pid].c_str(),vmsa::Norm_QA);
  ...
  string KEY_ME_QA = Form("pt_%d_Centrality_%d_CosThetaStar_%d_Phi_%d_%s_Dca_%d_Sig_%d_%s_Norm_%d_ME",vmsa::pt_RawQA[energy],vmsa::centStart,vmsa::CTS_start,5,EP[order-1].c_str(),vmsa::Dca_start,vmsa::nSigKaon_start,vmsa::mPID[pid].c_str(),vmsa::Norm_QA);
  ...
  string KEY_SM_QA = Form("pt_%d_Centrality_%d_CosThetaStar_%d_Phi_%d_%s_Dca_%d_Sig_%d_%s_Norm_%d_SM",vmsa::pt_RawQA[energy],vmsa::centStart,vmsa::CTS_start,5,EP[order-1].c_str(),vmsa::Dca_start,vmsa::nSigKaon_start,vmsa::mPID[pid].c_str(),vmsa::Norm_QA);
  ...
  string outputname = Form("figures/%s/%s/centralitystudy/Helicity2D_InvMassDistributions_%s_Order%d.pdf",vmsa::mPID[pid].c_str(),vmsa::mBeamEnergy[energy].c_str(),vmsa::mBeamEnergy[energy].c_str(),order);
    ...
    for(int iphi = 0; iphi < 12; iphi++)
    {
      for(int icent = 0; icent < 9; icent++)
      {
        for(int i_pt = vmsa::pt_start; i_pt < vmsa::pt_stop; i_pt++) // pt loop
        {
          ...
          string KEY_SE_QA = Form("pt_%d_Centrality_%d_CosThetaStar_%d_Phi_%d_%s_Dca_%d_Sig_%d_%s_Norm_%d_SE",i_pt,icent,itheta,iphi,EP[order-1].c_str(),vmsa::Dca_start,vmsa::nSigKaon_start,vmsa::mPID[pid].c_str(),vmsa::Norm_QA);
          //cout << KEY_SE_QA << endl;
          h_mMass_SE[KEY_SE_QA]->GetYaxis()->SetRangeUser(-0.1*h_mMass_SE[KEY_SE_QA]->GetMaximum(),1.1*h_mMass_SE[KEY_SE_QA]->GetMaximum());
          h_mMass_SE[KEY_SE_QA]->SetTitle(Form("Cent %d, %d/5<cos(#theta*)<%d/5, %d#pi/5<#beta<%d#pi/5",icent,itheta-5,itheta-4,iphi,iphi+1));
          h_mMass_SE[KEY_SE_QA]->DrawCopy();

          string KEY_ME_QA = Form("pt_%d_Centrality_%d_CosThetaStar_%d_Phi_%d_%s_Dca_%d_Sig_%d_%s_Norm_%d_ME",i_pt,icent,itheta,iphi,EP[order-1].c_str(),vmsa::Dca_start,vmsa::nSigKaon_start,vmsa::mPID[pid].c_str(),vmsa::Norm_QA);
          //cout << KEY_ME_QA << endl;
          h_mMass_ME[KEY_ME_QA]->SetLineColor(2);
          h_mMass_ME[KEY_ME_QA]->SetFillColor(2);
          h_mMass_ME[KEY_ME_QA]->SetFillStyle(3002);
          ...
          string KEY_SM_QA = Form("pt_%d_Centrality_%d_CosThetaStar_%d_Phi_%d_%s_Dca_%d_Sig_%d_%s_Norm_%d_SM",i_pt,icent,itheta,iphi,EP[order-1].c_str(),vmsa::Dca_start,vmsa::nSigKaon_start,vmsa::mPID[pid].c_str(),vmsa::Norm_QA);
          //cout << KEY_SM_QA << endl;
          h_mMass_SM[KEY_SM_QA]->SetLineColor(4);
          h_mMass_SM[KEY_SM_QA]->SetFillColor(4);
          h_mMass_SM[KEY_SM_QA]->SetFillStyle(3004);
          ...
          for(Int_t i_sig = vmsa::nSigKaon_start; i_sig < vmsa::nSigKaon_stop; i_sig++)
          {
            ...
            for(int i_norm = vmsa::Norm_start; i_norm < vmsa::Norm_stop; ++i_norm)
            {
              for(int pt_bin = vmsa::pt_rebin_first_cent_2D[energy]; pt_bin <= vmsa::pt_rebin_last_cent_2D[energy]; pt_bin++) // pt loop
              {
                ...
                string KEY = Form("pt_%d_Centrality_%d_CosThetaStar_%d_Phi_%d_%s_Dca_%d_Sig_%d_%s_Norm_%d",pt_bin,i_cent,i_theta,i_phi,EP[order-1].c_str(),i_dca,i_sig,vmsa::mPID[pid].c_str(),i_norm);
  ...
  outputname = Form("figures/%s/%s/centralitystudy/Helicity2D_InvMassBackgroundSubtracted_Rebinned_%s_Order%d.pdf",vmsa::mPID[pid].c_str(),vmsa::mBeamEnergy[energy].c_str(),vmsa::mBeamEnergy[energy].c_str(),order);
    ...
    for(int iphi = 0; iphi < 12; iphi++)
    {
      for(int icent = 0; icent < 9; icent++)
      {
        for(int i_pt = vmsa::pt_rebin_first_cent_2D[energy]; i_pt <= vmsa::pt_rebin_last_cent_2D[energy]; i_pt++) // pt loop
        {
          ...
          string KEY_QA = Form("pt_%d_Centrality_%d_CosThetaStar_%d_Phi_%d_%s_Dca_%d_Sig_%d_%s_Norm_%d",i_pt,icent,itheta,iphi,EP[order-1].c_str(),vmsa::Dca_start,vmsa::nSigKaon_start,vmsa::mPID[pid].c_str(),vmsa::Norm_QA);
          cout << KEY_QA << endl;
          h_mMass[KEY_QA]->SetMarkerStyle(24);
          h_mMass[KEY_QA]->SetMarkerSize(1.2);
          h_mMass[KEY_QA]->SetLineColor(kGray+2);
          h_mMass[KEY_QA]->SetTitle(Form("Cent %d, %d/5<cos(#theta*)<%d/5, %d#pi/5<#beta<%d#pi/5",icent,itheta-5,itheta-4,iphi,iphi+1));
    ...
    for(Int_t i_dca = vmsa::Dca_start; i_dca < vmsa::Dca_stop; i_dca++)
    {
      ...
      { 
        ...
        for(int i_norm = vmsa::Norm_start; i_norm < vmsa::Norm_stop; ++i_norm)
        {
          ...
          {
            string KEY = Form("Centrality_%d_CosThetaStar_%d_Phi_%d_2nd_Dca_%d_Sig_%d_%s_Norm_%d",i_cent,i_theta,i_dca,i_sig,vmsa::mPID[pid].c_str(),i_norm);
...
*/
  ...
  for(int i_pt = vmsa::pt_rebin_first[energy]; i_pt < vmsa::pt_rebin_last[energy]; i_pt++) // pt loop
  {
	...
	for(Int_t i_dca = vmsa::Dca_start; i_dca < vmsa::Dca_stop; i_dca++)
	{
	  for(Int_t i_sig = vmsa::nSigKaon_start; i_sig < vmsa::nSigKaon_stop; i_sig++)
	  {
		...
		string KEY = Form("pt_%d_Centrality_%d_CosThetaStar_%d_Phi_%d_2nd_Dca_%d_Sig_%d_%s_Norm_%d",i_pt,i_cent,i_theta,i_dca,i_sig,vmsa::mPID[pid].c_str(),i_norm);
   ...
   /* TCanvas *c_mMass_phi_diff = new TCanvas("c_mMass_phi_diff","c_mMass_phi_diff",10,10,1400,1400);
    ...
    for(int i_pt = vmsa::pt_rebin_first[energy]; i_pt < vmsa::pt_rebin_last[energy]; i_pt++) // pt loop
    {
      ...
      string KEY_QA = Form("pt_%d_Centrality_%d_CosThetaStar_%d_Phi_%d_2nd_Dca_%d_Sig_%d_%s_Norm_%d",i_pt,vmsa::centStart,vmsa::CTS_start,vmsa::Dca_start,vmsa::nSigKaon_start,vmsa::mPID[pid].c_str(),vmsa::Norm_QA);
  ...
  string outputfile = Form("../output/AuAu%s/%s/Helicity2DCent_InvMassSubBgCent_%s.root",vmsa::mBeamEnergy[energy].c_str(),vmsa::mPID[pid].c_str(),etamode.c_str());
  if(order == 1) outputfile = Form("../output/AuAu%s/%s/Helicity2DCent_InvMassSubBgCent_%s_FirstOrder.root",vmsa::mBeamEnergy[energy].c_str(),vmsa::mPID[pid].c_str(),etamode.c_str());
        ...
        for(Int_t i_sig = vmsa::nSigKaon_start; i_sig < vmsa::nSigKaon_stop; i_sig++)
        {
          ...
          for(int i_theta = 0; i_theta < 9; i_theta++) // cos(theta*) loop
          {
            for(int i_phi = 0; i_phi < 12; i_phi++) // cos(theta*) loop
            {
              for(int i_norm = vmsa::Norm_start; i_norm < vmsa::Norm_stop; ++i_norm)
              {
                string KEY = Form("pt_%d_Centrality_%d_CosThetaStar_%d_Phi_%d_%s_Dca_%d_Sig_%d_%s_Norm_%d",i_pt,i_cent,i_theta,i_phi,EP[order-1].c_str(),i_dca,i_sig,vmsa::mPID[pid].c_str(),i_norm);
                h_mMass[KEY]->Write();
              }
            }
          }
