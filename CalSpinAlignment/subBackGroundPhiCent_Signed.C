...
void subBackGroundPhiCent_Signed(int energy = 3, int pid = 0, int year = 0, string date = "20231108", bool random3D = false, int order = 2, std::string etamode = "eta1_eta1")
{
  ...
  string outputname = Form("figures/%s/%s/centralitystudy/InvMassDistributions_%s_Order%d_signedcosthetastar.pdf",vmsa::mPID[pid].c_str(),vmsa::mBeamEnergy[energy].c_str(),vmsa::mBeamEnergy[energy].c_str(),order);
  ...
  for(int itheta = 0; itheta < 14; itheta++)
  {
    for(int icent = 0; icent < 9; icent++)
    {
      for(int i_pt = vmsa::pt_start; i_pt < vmsa::pt_stop; i_pt++) // pt loop
      {
        ...
        //cout << KEY_SE_QA << endl;
        h_mMass_SE[KEY_SE_QA]->GetYaxis()->SetRangeUser(-0.1*h_mMass_SE[KEY_SE_QA]->GetMaximum(),1.1*h_mMass_SE[KEY_SE_QA]->GetMaximum());
        h_mMass_SE[KEY_SE_QA]->SetTitle(Form("Cent %d, %d/7<|cos(#theta*)|<%d/7",icent,itheta-7,itheta-6));
  ...
  outputname = Form("figures/%s/%s/centralitystudy/InvMassBackgroundSubtracted_Rebinned_%s_Order%d_signedcosthetastar.pdf",vmsa::mPID[pid].c_str(),vmsa::mBeamEnergy[energy].c_str(),vmsa::mBeamEnergy[energy].c_str(),order);
  ...
  for(int itheta = 0; itheta < 14; itheta++)
  {
    for(int icent = 0; icent < 9; icent++)
    {
      for(int i_pt = vmsa::pt_rebin_first_cent[energy]; i_pt <= vmsa::pt_rebin_last_cent[energy]; i_pt++) // pt loop
      {
        ...
        h_mMass[KEY_QA]->SetTitle(Form("Cent %d, %d/7<|cos(#theta*)|<%d/7",icent,itheta-7,itheta-6));
...
*/
  ...
  string outputfile = Form("../output/AuAu%s/%s/InvMassSubBgCent_%s_Signed.root",vmsa::mBeamEnergy[energy].c_str(),vmsa::mPID[pid].c_str(),etamode.c_str());
  if(order == 1) outputfile = Form("../output/AuAu%s/%s/InvMassSubBgCent_%s_Signed_FirstOrder.root",vmsa::mBeamEnergy[energy].c_str(),vmsa::mPID[pid].c_str(),etamode.c_str());
