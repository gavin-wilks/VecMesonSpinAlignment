...
void compareHelicityVsGlobal_v2(Int_t energy = 4, Int_t pid = 0, Int_t i_cent = 9, string correction = "Raw", int order = 2, string etamode = "eta1_eta1", int yspectra = 23, std::string simmode = "Mc")//defaultF = 0 is BESII, defaultF = 1 is BESI
{
  ...
  //std::string date[nopts] = {"20240809g"}; // nov2, prelimv2
  ...
  //int color[nopts] = {kBlue, kOrange+7, kBlue, kGray, kMagenta, kGreen, kViolet, kRed, }; 
  //std::string label[nopts] = {"v_{2} OFF","v_{2} ON (Prelim)"};
    ...
    for(int irho = 0; irho < 5; irho++)
    {
      for(int par = 0; par < 5; par++)
      {
        ...
        for(int iopt = 0; iopt < nopts; iopt++)
        {
          ...
          g2D_Global[iopt][irho][par][i]->GetXaxis()->SetTitle(Form("%s^{Global} Input",param[irho].c_str()));
    ...
    for(int irho = 0; irho < 5; irho++)
    {
      for(int par = 0; par < 5; par++)
      {
        ...
        for(int iopt = 0; iopt < nopts; iopt++)
        {
          ...
          g2D[iopt][irho][par][i]->GetXaxis()->SetTitle(Form("%s^{Global} Input",param[irho].c_str()));
