...
void plot2DOffDiag_Comparison(string correction = "Eff", int energy = 4, int pid = 0, int year = 0, bool random3D = false, int order = 2, string etamode = "eta1_eta1", int frameopt = 0)
{
  ...
  for(int i = 0; i < 5; i++)
  {
    ...
    { 
      ...
      g_mRho[KEY_Eff]->SetPoint(i, pt+0.05, rho);
    }
