...
void subBackGroundTpcOnly(int energy = 4, int pid = 0, int year = 0, string date = "20240906", bool random3D = false, int order = 2, string etamode = "eta1_eta1")
{
  ...
  for(int i_pt = 0; i_pt < vmsa::pt_total; i_pt++)
  {
    ...
    c_peak->SaveAs(Form("figures/%s/%s/pTstudy/TPCOnly_CloseUpMixedEvent_%s_Order%d_Cent0_pt%d.pdf",vmsa::mPID[pid].c_str(),vmsa::mBeamEnergy[energy].c_str(),vmsa::mBeamEnergy[energy].c_str(),order,i_pt));
  }
  ...
  string outputname = Form("figures/%s/%s/pTstudy/TPCOnly_InvMassDistributions_%s_Order%d_Cent9.pdf",vmsa::mPID[pid].c_str(),vmsa::mBeamEnergy[energy].c_str(),vmsa::mBeamEnergy[energy].c_str(),order);
  ...
  outputname = Form("figures/%s/%s/pTstudy/TPCOnly_InvMassBackgroundSubtracted_Rebinned_%s_Order%d_Cent9.pdf",vmsa::mPID[pid].c_str(),vmsa::mBeamEnergy[energy].c_str(),vmsa::mBeamEnergy[energy].c_str(),order);
...
//#endif
  ...
  string outputfile = Form("../output/AuAu%s/%s/InvMassSubBg_%s_TPCOnly.root",vmsa::mBeamEnergy[energy].c_str(),vmsa::mPID[pid].c_str(),etamode.c_str());
