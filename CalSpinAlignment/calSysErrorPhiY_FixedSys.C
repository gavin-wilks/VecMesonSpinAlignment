...
void calSysErrorPhiY_FixedSys(Int_t energy = 4, Int_t pid = 0, string correction = "AccRes", bool random3D = false, int ptQA = 0, int centQA = 2, int order = 2, std::string etamode = "eta1_eta1", std::string fileoption = "FixedResolutionFinal")//defaultF = 0 is BESII, defaultF = 1 is BESI
{
  ...
  string inputfileHframe = Form("../output/AuAu%s/%s/RawRhoEtaSys_%s.root",vmsa::mBeamEnergy[energy].c_str(),vmsa::mPID[pid].c_str(),etamode.c_str());
  ...
  string inputfile = Form("../output/AuAu%s/%s/%s/%sRhoEtaSys_%s.root",vmsa::mBeamEnergy[energy].c_str(),vmsa::mPID[pid].c_str(),fileoption.c_str(),correction.c_str(),etamode.c_str());
  if(order == 1) inputfile = Form("../output/AuAu%s/%s/%s/%sRhoEtaSys_%s_FirstOrder.root",vmsa::mBeamEnergy[energy].c_str(),vmsa::mPID[pid].c_str(),fileoption.c_str(),correction.c_str(),etamode.c_str());
  ...
  if(order == 1 && random3D) inputfile = Form("../output/AuAu%s/%s/3DRandom/%sRhoEtaSys_%s_FirstOrder.root",vmsa::mBeamEnergy[energy].c_str(),vmsa::mPID[pid].c_str(),correction.c_str(),etamode.c_str());
        ...
        for(Int_t i_sig = vmsa::nSigKaon_start; i_sig < vmsa::nSigKaon_stop; i_sig++)
        {
          ...
          for(int i_norm = vmsa::Norm_start; i_norm < vmsa::Norm_stop; ++i_norm)
          {
            for(int i_sigma = vmsa::Sig_start; i_sigma < vmsa::Sig_stop; ++i_sigma)
            {
              ...
              {
                string KEY_rho = Form("rhoRaw_pt_%d_Centrality_%d_%s_Dca_%d_Sig_%d_%s_Norm_%d_Sigma_%d_%s",i_pt,i_cent,EP[order-1].c_str(),i_dca,i_sig,vmsa::mPID[pid].c_str(),i_norm,i_sigma,vmsa::mInteMethod[i_method].c_str());
                ...
                {
                  string KEY_counts = Form("eta_%d_pt_%d_Centrality_%d_%s_Dca_%d_Sig_%d_%s_Norm_%d_Sigma_%d_%s",i_eta,i_pt,i_cent,EP[order-1].c_str(),i_dca,i_sig,vmsa::mPID[pid].c_str(),i_norm,i_sigma,vmsa::mInteMethod[i_method].c_str());
        ...
        for(int i_norm = vmsa::Norm_start; i_norm < vmsa::Norm_stop; ++i_norm)
        {
          for(int i_sigma = vmsa::Sig_start; i_sigma < vmsa::Sig_stop; ++i_sigma)
          {
            for(int i_method = vmsa::Method_start; i_method < vmsa::Method_stop; ++i_method)
            {
             ...
             //   {
             //     if(i_F > 0) continue;
                  string KEY_rho = Form("rhoRaw_pt_%d_Centrality_%d_%s_Dca_%d_Sig_%d_%s_Norm_%d_Sigma_%d_%s",ptQA,centQA,EP[order-1].c_str(),i_dca,i_sig,vmsa::mPID[pid].c_str(),i_norm,i_sigma,vmsa::mInteMethod[i_method].c_str());
        ...
        { 
          for(int i_sigma = vmsa::Sig_start; i_sigma < vmsa::Sig_stop; ++i_sigma)
          {
            for(int i_method = vmsa::Method_start; i_method < vmsa::Method_stop; ++i_method)
            {
              ...
              string KEY = Form("rhoRaw_pt_%d_Centrality_%d_%s_Dca_%d_Sig_%d_%s_Norm_%d_Sigma_%d_%s",i_pt,i_cent,EP[order-1].c_str(),i_dca,0,vmsa::mPID[pid].c_str(),0,i_sigma,vmsa::mInteMethod[i_method].c_str());
        ...
        for(Int_t i_sig = vmsa::nSigKaon_start; i_sig < vmsa::nSigKaon_stop; i_sig++)
        {
          for(int i_sigma = vmsa::Sig_start; i_sigma < vmsa::Sig_stop; ++i_sigma)
          {
            for(int i_method = vmsa::Method_start; i_method < vmsa::Method_stop; ++i_method)
            {
              ...
              string KEY = Form("rhoRaw_pt_%d_Centrality_%d_%s_Dca_%d_Sig_%d_%s_Norm_%d_Sigma_%d_%s",i_pt,i_cent,EP[order-1].c_str(),0,i_sig,vmsa::mPID[pid].c_str(),0,i_sigma,vmsa::mInteMethod[i_method].c_str());
        ...
        for(int i_norm = vmsa::Norm_start; i_norm < vmsa::Norm_stop; ++i_norm)
        {
          for(int i_sigma = vmsa::Sig_start; i_sigma < vmsa::Sig_stop; ++i_sigma)
          {
            for(int i_method = vmsa::Method_start; i_method < vmsa::Method_stop; ++i_method)
            {
              ...
              string KEY = Form("rhoRaw_pt_%d_Centrality_%d_%s_Dca_%d_Sig_%d_%s_Norm_%d_Sigma_%d_%s",i_pt,i_cent,EP[order-1].c_str(),0,0,vmsa::mPID[pid].c_str(),i_norm,i_sigma,vmsa::mInteMethod[i_method].c_str());
    ...
    for(Int_t i_sig = vmsa::nSigKaon_start; i_sig < vmsa::nSigKaon_stop; i_sig++)
    {
      ...
      for(int i_norm = vmsa::Norm_start; i_norm < vmsa::Norm_stop; ++i_norm)
      {
        for(int i_sigma = vmsa::Sig_start; i_sigma < vmsa::Sig_stop; ++i_sigma)
        {
          for(int i_method = vmsa::Method_start; i_method < vmsa::Method_stop; ++i_method)
          {
            string KEY_rho = Form("rhoRaw_%s_Dca_%d_Sig_%d_%s_Norm_%d_Sigma_%d_%s",EP[order-1].c_str(),i_dca,i_sig,vmsa::mPID[pid].c_str(),i_norm,i_sigma,vmsa::mInteMethod[i_method].c_str());
            ...
            for(int i_eta = vmsa::eta_start+ypadding; i_eta < vmsa::eta_stop-ypadding; i_eta++) // Rapidity bin loop
            {
              ...
              {           
                for(int i_cent = 0; i_cent < vmsa::cent_rebin_total; i_cent++) // Centrality loop
                { 
                  string KEY_Default = Form("rhoRaw_pt_%d_Centrality_%d_%s_Dca_%d_Sig_%d_%s_Norm_%d_Sigma_%d_%s",i_pt,i_cent,EP[order-1].c_str(),i_dca,i_sig,vmsa::mPID[pid].c_str(),i_norm,i_sigma,vmsa::mInteMethod[i_method].c_str());
                  string KEY_counts = Form("eta_%d_pt_%d_Centrality_%d_%s_Dca_%d_Sig_%d_%s_Norm_%d_Sigma_%d_%s",i_eta,i_pt,i_cent,EP[order-1].c_str(),i_dca,i_sig,vmsa::mPID[pid].c_str(),i_norm,i_sigma,vmsa::mInteMethod[i_method].c_str());
  ...
  {
    string KEY_Default = Form("rhoRaw_%s_Dca_%d_Sig_%d_%s_Norm_%d_Sigma_%d_%s",EP[order-1].c_str(),0,0,vmsa::mPID[pid].c_str(),0,0,vmsa::mInteMethod[1].c_str());
    double sysDca[9];
    double sysNSig[9];
    double sysNorm[9];
    //double sysSig[vmsa::Sig_stop];
    ...
    { 
      for(int i_sigma = vmsa::Sig_start; i_sigma < vmsa::Sig_stop; ++i_sigma)
      {
        for(int i_method = vmsa::Method_start; i_method < vmsa::Method_stop; ++i_method)
        {
          if(i_dca == 0 && (i_sigma != 0 || i_method == 0)) continue;
          if(i_dca != 0 && i_sigma != 0 && i_method == 1) continue;
          string KEY = Form("rhoRaw_%s_Dca_%d_Sig_%d_%s_Norm_%d_Sigma_%d_%s",EP[order-1].c_str(),i_dca,0,vmsa::mPID[pid].c_str(),0,i_sigma,vmsa::mInteMethod[i_method].c_str());
    ...
    for(Int_t i_sig = vmsa::nSigKaon_start; i_sig < vmsa::nSigKaon_stop; i_sig++)
    {
      for(int i_sigma = vmsa::Sig_start; i_sigma < vmsa::Sig_stop; ++i_sigma)
      {
        for(int i_method = vmsa::Method_start; i_method < vmsa::Method_stop; ++i_method)
        {
          if(i_sig == 0 && (i_sigma != 0 || i_method == 0)) continue;
          if(i_sig != 0 && i_sigma != 0 && i_method == 1) continue;
          string KEY = Form("rhoRaw_%s_Dca_%d_Sig_%d_%s_Norm_%d_Sigma_%d_%s",EP[order-1].c_str(),0,i_sig,vmsa::mPID[pid].c_str(),0,i_sigma,vmsa::mInteMethod[i_method].c_str());
    ...
    for(int i_norm = vmsa::Norm_start; i_norm < vmsa::Norm_stop; ++i_norm)
    {
      for(int i_sigma = vmsa::Sig_start; i_sigma < vmsa::Sig_stop; ++i_sigma)
      {
        for(int i_method = vmsa::Method_start; i_method < vmsa::Method_stop; ++i_method)
        {
          if(i_norm == 0 && (i_sigma != 0 || i_method == 0)) continue;
          if(i_norm != 0 && i_sigma != 0 && i_method == 1) continue;
          string KEY = Form("rhoRaw_%s_Dca_%d_Sig_%d_%s_Norm_%d_Sigma_%d_%s",EP[order-1].c_str(),0,0,vmsa::mPID[pid].c_str(),i_norm,i_sigma,vmsa::mInteMethod[i_method].c_str());
  ...
  for(Int_t i_point = vmsa::eta_start+ypadding; i_point < vmsa::eta_stop-ypadding; ++i_point)
  {
    ...
    double pt_def, rho_def;
    g_mStatErrorsInt[KEY_Default]->GetPoint(i_point-ypadding,pt_def,rho_def); 
  ...
  //        if(correction == "AccRes") KEY = Form("rhoRaw_2nd_Dca_%d_Sig_%d_%s_Norm_%d_Sigma_%d_%s_F_%d_Eff_0",i_dca,0,vmsa::mPID[pid].c_str(),0,0,vmsa::mInteMethod[i_method].c_str(),defaultF);
          double pt_sys, rho_sys;
          g_mStatErrorsInt[KEY]->GetPoint(i_point-ypadding,pt_sys,rho_sys);
          sysDca[idx] = rho_sys;
          if(random3D) sysDca[idx] = 4.*(sysDca[idx]-1./3.)+1./3.;
          idx++;
        }
   ...
   /* if(correction == "AccRes")
    ...
    Double_t pt, rho;
    g_mStatErrorsInt[KEY_Default]->GetPoint(i_point-ypadding,pt,rho);
    ...
    double StatError_rho = g_mStatErrorsInt[KEY_Default]->GetErrorYhigh(i_point-ypadding);
...
//  PlotLine(0.0,5.0,1.0/3.0,1.0/3.0,1,2,2);
  ...
  plotTopLegend((char*)leg_energy.c_str(),-0.3,0.295,0.04,1,0.0,42,0);
  plotTopLegend((char*)"0%-80%",-0.1,0.285,0.04,1,0.0,42,0);
  ...
  c_rho_SysError->SaveAs(Form("figures/finaloutputrhoEta_%s.pdf",etamode.c_str()));  
  ...
  string outputname = Form("./figures/Phi/19GeV/rho00eta_cent_%s.pdf",etamode.c_str());
  string output_start = Form("%s[",outputname.c_str());

  TCanvas *c_pt = new TCanvas("c_pt","c_pt",10,10,1200,900);
  c_pt->Divide(4,3);

  c_pt->Print(output_start.c_str());

  std::string centStrings[4] = {"50-80","30-50","10-30","0-10"};
  ...
  for(int i_pt = vmsa::pt_rebin_first_y[energy]; i_pt <= vmsa::pt_rebin_last_y[energy]; ++i_pt) // pt loop
  {
    for(int i_cent = 0; i_cent < vmsa::cent_rebin_total; i_cent++) // Centrality loop
    {
      c_pt->cd(i_pt*4+i_cent+1);
      c_pt->cd(i_pt*4+i_cent+1)->SetLeftMargin(0.15);
      c_pt->cd(i_pt*4+i_cent+1)->SetBottomMargin(0.15);
      c_pt->cd(i_pt*4+i_cent+1)->SetTicks(1,1);
      c_pt->cd(i_pt*4+i_cent+1)->SetGrid(0,0);
      h_frame->DrawCopy("pE");
      PlotLine(-1.5,1.5,1.0/3.0,1.0/3.0,1,2,2);
      Draw_TGAE_Point_new_Symbol(-0.1,0.46,0.0,0.0,0.0,0.0,style_phi_2nd,color_phi_2nd,size_marker+0.2); 
  ...
  string OutPutFile = Form("../output/AuAu%s/%s/RhoEta_%sSysErrors_%s.root",vmsa::mBeamEnergy[energy].c_str(),vmsa::mPID[pid].c_str(),correction.c_str(),etamode.c_str());
  if(order == 1) OutPutFile = Form("../output/AuAu%s/%s/RhoEta_%sSysErrors_%s_FirstOrder.root",vmsa::mBeamEnergy[energy].c_str(),vmsa::mPID[pid].c_str(),correction.c_str(),etamode.c_str());
  if(order == 1 && random3D) OutPutFile = Form("../output/AuAu%s/%s/3DRandom/RhoEta_%sSysErrors_%s_FirstOrder.root",vmsa::mBeamEnergy[energy].c_str(),vmsa::mPID[pid].c_str(),correction.c_str(),etamode.c_str());
  ...
  {           
    for(int i_cent = 0; i_cent < vmsa::cent_rebin_total; i_cent++) // Centrality loop
    {
      string KEY_rho = Form("rhoRaw_pt_%d_Centrality_%d_%s_Dca_%d_Sig_%d_%s_Norm_%d_Sigma_%d_%s",i_pt,i_cent,EP[order-1].c_str(),0,0,vmsa::mPID[pid].c_str(),0,0,vmsa::mInteMethod[1].c_str());
      string KEY_rhoStat = Form("rhoRawStat_pt_%d_Centrality_%d_%s_Dca_%d_Sig_%d_%s_Norm_%d_Sigma_%d_%s",i_pt,i_cent,EP[order-1].c_str(),0,0,vmsa::mPID[pid].c_str(),0,0,vmsa::mInteMethod[1].c_str());
      string KEY_rhoSys  = Form("rhoRawSys_pt_%d_Centrality_%d_%s_Dca_%d_Sig_%d_%s_Norm_%d_Sigma_%d_%s",i_pt,i_cent,EP[order-1].c_str(),0,0,vmsa::mPID[pid].c_str(),0,0,vmsa::mInteMethod[1].c_str());
...
void plotSysErrorsBox(TGraphAsymmErrors *g_rho, int plot_color)
{
  ...
  for(int i_pt = 0; i_pt < g_rho->GetN(); ++i_pt) // plot sys errors
  {
    ...
    bSys[i_pt] = new TBox(pt-0.04,rho-err,pt+0.04,rho+err);
