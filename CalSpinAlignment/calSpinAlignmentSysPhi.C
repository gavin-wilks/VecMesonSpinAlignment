...
void calSpinAlignmentSysPhi(int energy = 3, int pid = 0, int year = 0, bool random3D = false, int order = 2, string etamode = "eta1_eta1")
{
...
//#endif
  ...
  */
  ...
  {
    string outputname = Form("./figures/%s/%s/pTstudy/allThetaYields%s_%s_Order%d_cent%d_%s_pTdependence.pdf",vmsa::mPID[pid].c_str(),vmsa::mBeamEnergy[energy].c_str(),vmsa::mPID[pid].c_str(),vmsa::mBeamEnergy[energy].c_str(),order,i_cent,etamode.c_str());
    ...
    for(int i_pt = vmsa::pt_rebin_first[energy]; i_pt < vmsa::pt_rebin_last[energy]; ++i_pt) // pt loop
    {
      ...
      {
        for(int i_method = 0/*vmsa::Method_start*/; i_method < 1/*vmsa::Method_stop*/; ++i_method)
        {
