...
void calSpinAlignmentSysPhiCent_PolySys_PhiPsi(int energy = 4, int pid = 0, int year = 0, bool random3D = false, int centQA = 4, int ptQA = 0, int dcaQA = 0, int nsigQA = 0, int order = 2, std::string etamode = "eta1_eta1")
{
  std::string EP[2] = {"","2nd"};
  ...
  string inputfile = Form("../output/AuAu%s/%s/PhiPsi_InvMassSubBgCent_%s.root",vmsa::mBeamEnergy[energy].c_str(),vmsa::mPID[pid].c_str(),etamode.c_str());
  if(order == 1) inputfile = Form("../output/AuAu%s/%s/PhiPsi_InvMassSubBgCent_%s_FirstOrder.root",vmsa::mBeamEnergy[energy].c_str(),vmsa::mPID[pid].c_str(),etamode.c_str());
...
//    {
//      string KEY_QA = Form("pt_%d_Centrality_%d_CosThetaStar_%d_PhiPsi_%d_2nd_Dca_%d_Sig_%d_%s_Norm_%d",ptQA,centQA,i_theta,dcaQA,nsigQA,vmsa::mPID[pid].c_str(),vmsa::Norm_QA);
...
//#endif
  ...
  */
  ...
  for(int i_poly = 0; i_poly < 3; i_poly++)
  {
    string outputname = Form("./figures/%s/%s/centralitystudy/PhiPsi_allThetaYields%s_%s_Order%d_%s_CentDependence_Poly%d.pdf",vmsa::mPID[pid].c_str(),vmsa::mBeamEnergy[energy].c_str(),vmsa::mPID[pid].c_str(),vmsa::mBeamEnergy[energy].c_str(),order,etamode.c_str(),i_poly+1);
      ...
      for(int i_pt = vmsa::pt_rebin_first_cent[energy]; i_pt <= vmsa::pt_rebin_last_cent[energy]; ++i_pt) // pt loop
      {
        for(int i_phipsi = 0; i_phipsi < 7; i_phipsi++)
        {
          ...
          for(int i_theta = 0; i_theta < 9; ++i_theta)
          {
            ...
            if(i_theta < vmsa::CTS_stop)
            {
              string KEY_QA = Form("pt_%d_Centrality_%d_CosThetaStar_%d_PhiPsi_%d_%s_Dca_%d_Sig_%d_%s_Norm_%d",i_pt,i_cent,i_theta,i_phipsi,EP[order-1].c_str(),vmsa::Dca_start,vmsa::nSigKaon_start,vmsa::mPID[pid].c_str(),vmsa::Norm_QA);
              h_mMass[KEY_QA]->SetTitle("");
              h_mMass[KEY_QA]->GetXaxis()->SetNdivisions(505,'N');
              h_mMass[KEY_QA]->GetXaxis()->SetLabelSize(0.03);
              h_mMass[KEY_QA]->GetXaxis()->SetTitle("M(K^{+},K^{-})");
              h_mMass[KEY_QA]->SetTitle(Form("%.2f<p_{T}<%.2f, %d#pi/14<#phi-#Psi_{2}<%d#pi/14, %d/7<|cos(#theta*)|<%d/7, cent%d",vmsa::pt_low_cent[energy][i_pt],vmsa::pt_up_cent[energy][i_pt],i_phipsi*2-7,(i_phipsi+1)*2-7,i_theta,i_theta+1,i_cent));
              h_mMass[KEY_QA]->GetXaxis()->SetTitleSize(0.05);
              h_mMass[KEY_QA]->GetXaxis()->SetTitleOffset(1.2);
              h_mMass[KEY_QA]->GetXaxis()->CenterTitle();
          ...
          for(int i_theta = vmsa::CTS_start; i_theta < vmsa::CTS_stop; ++i_theta)
          {
            c_diff->cd(i_theta+1);
            string KEY_QA = Form("pt_%d_Centrality_%d_CosThetaStar_%d_PhiPsi_%d_%s_Dca_%d_Sig_%d_%s_Norm_%d",i_pt,i_cent,i_theta,i_phipsi,EP[order-1].c_str(),vmsa::Dca_start,vmsa::nSigKaon_start,vmsa::mPID[pid].c_str(),vmsa::Norm_QA);
            string KEY_QA_Poly = Form("pt_%d_Centrality_%d_CosThetaStar_%d_PhiPsi_%d_%s_Dca_%d_Sig_%d_%s_Norm_%d_Poly%d",i_pt,i_cent,i_theta,i_phipsi,EP[order-1].c_str(),vmsa::Dca_start,vmsa::nSigKaon_start,vmsa::mPID[pid].c_str(),vmsa::Norm_QA,i_poly+1);
            TF1 *f_bw;
            if(i_poly == 0) f_bw = new TF1("f_bw",PolyBreitWigner,vmsa::BW_Start[pid],vmsa::BW_Stop[pid],5);
            if(i_poly == 1) f_bw = new TF1("f_bw",Poly2BreitWigner,vmsa::BW_Start[pid],vmsa::BW_Stop[pid],6);
            if(i_poly == 2) f_bw = new TF1("f_bw",Poly3BreitWigner,vmsa::BW_Start[pid],vmsa::BW_Stop[pid],7);
...
//#endif
  ...
  //string outputname = Form("figures/%s/%s/centralitystudy/Chi2NDF_%s_Order%d.pdf",vmsa::mPID[pid].c_str(),vmsa::mBeamEnergy[energy].c_str(),vmsa::mBeamEnergy[energy].c_str(),order);
  //string output_start = Form("%s[",outputname.c_str());
  //
  ...
  //outputname = Form("figures/%s/%s/centralitystudy/PValue_%s_Order%d.pdf",vmsa::mPID[pid].c_str(),vmsa::mBeamEnergy[energy].c_str(),vmsa::mBeamEnergy[energy].c_str(),order);
  ...
  string outputfile = Form("../output/AuAu%s/%s/PhiPsi_RawRhoCentSys_%s_Poly.root",vmsa::mBeamEnergy[energy].c_str(),vmsa::mPID[pid].c_str(),etamode.c_str());
  if(order == 1) outputfile = Form("../output/AuAu%s/%s/PhiPsi_RawRhoCentSys_%s_Poly_FirstOrder.root",vmsa::mBeamEnergy[energy].c_str(),vmsa::mPID[pid].c_str(),etamode.c_str());
