...
void compRhoEff_Y(Int_t energy = 4, Int_t pid = 0)
{
  string inputfileF0 = Form("../output/AuAu%s/%s/RhoEta_RawSysErrors.root",vmsa::mBeamEnergy[energy].c_str(),vmsa::mPID[pid].c_str());
  string inputfileF1 = Form("../output/AuAu%s/%s/RhoEta_EffSysErrors.root",vmsa::mBeamEnergy[energy].c_str(),vmsa::mPID[pid].c_str());
  string inputfileF2 = Form("../output/AuAu%s/%s/RhoEta_AccResSysErrors.root",vmsa::mBeamEnergy[energy].c_str(),vmsa::mPID[pid].c_str());
...
//  PlotLine(0.0,5.0,1.0/3.0,1.0/3.0,1,2,2);
  ...
  Draw_TGAE_Point_new_Symbol(-0.1,0.47,0.0,0.0,0.0,0.0,20,kBlue,1.3); 
  string leg_count0 = Form("#phi (Raw)");
  plotTopLegend((char*)leg_count0.c_str(),0.0,0.467,0.03,1,0.0,42,0);

  Draw_TGAE_Point_new_Symbol(-0.1,0.45,0.0,0.0,0.0,0.0,20,kOrange+7,1.3); 
  string leg_count1 = Form("#phi (Eff)");
  plotTopLegend((char*)leg_count1.c_str(),0.0,0.447,0.03,1,0.0,42,0);

  Draw_TGAE_Point_new_Symbol(-0.1,0.43,0.0,0.0,0.0,0.0,20,kGray+2,1.3); 
  string leg_count2 = Form("#phi (Eff+Acc+Res)");
  plotTopLegend((char*)leg_count2.c_str(),0.0,0.427,0.03,1,0.0,42,0);
