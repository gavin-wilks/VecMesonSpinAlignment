...
void compareHelicityVsGlobal_Paper_All_Y_Eta_PtCut_PtRes(Int_t energy = 4, Int_t pid = 0, Int_t i_cent = 9, string correction = "Raw", int order = 2, string etamode = "eta1_eta1", int yspectra = 23, std::string simmode = "Mc", int inputpar = 0)//defaultF = 0 is BESII, defaultF = 1 is BESI
{
  gStyle->SetOptStat(0);
  ...
  std::string option = "y_Variance_RC_EtaCuts_GlobalInput_pt1_individualy_ptcuts_ptres";
  ...
  TH2D *h_m2D_RC[2][nopts][5][nvar][nkin][11][9];  
  TH2D *h_m2D_RC_Global[2][nopts][5][nvar][nkin][11][9]; 
  //TH2D *h_m2D_RC[2][nopts][5][nvar][nkin][11];  
  TH1D *h_m1D_RC_Global_Cos[2][nopts][5][nvar][nkin][11][9][20]; 
  TH1D *h_m1D_RC_Global_Cos_All[2][nopts][5][nvar][nkin][11][9]; 
  TH1D *h_m1D_RC_Global_Ratio_Cos_All[2][nopts][5][nvar][nkin][11][9]; 
  TH1D *h_m1D_RC_Global_Corrected_Cos_All[2][nopts][5][nvar][nkin][11][9]; 
  ...
  TH2D *h_m2D_RC_Global_Ratio[2][nopts][5][nvar][nkin][11][9];
  TH2D *h_m2D_RC_Ratio[2][nopts][5][nvar][nkin][11][9];
  TH2D *h_m2D_RC_Global_Corrected[2][nopts][5][nvar][nkin][11][9];
  TH2D *h_m2D_RC_Global_Corrected_BackToMC[2][nopts][5][nvar][nkin][11][9];
  TH2D *h_m2D_RC_Global_Corrected_BackToMC_E[2][nopts][5][nvar][nkin][11][9];
  TH2D *h_m2D_RC_Corrected[2][nopts][5][nvar][nkin][11][9];
  ...
  for(int ires = 0; ires < 2; ires++)
  {
    for(int ikin = 0; ikin < nkin; ikin++)
    {
      for(int iopt = 0; iopt < nopts; iopt++)
      {
        ...
        if(ires == 1) inputfileRC = Form("effaccfiles/%s/%s/PaperAllYRC_EtaFixed_GlobalInput_nov2_ptcuts_ptres/Eff_%s_SingleParticle_noToF_Mode0_EtaMode0_pt%1.1f_y%1.1f.root",vmsa::mPID[pid].c_str(),vmsa::mBeamEnergy[energy].c_str(),vmsa::mBeamEnergy[energy].c_str(),ptkin[ikin],vals[iopt]);
    ...
    for(int ikin = 0; ikin < nkin; ikin++)
    {
      for(int iopt = 0; iopt < nopts; iopt++)
      {
        ...
        for(int irho = 0; irho < 5; irho++)
        {
          ...
          //for(int i = 0; i < nvar; i++)
          {
            ...
            for(int ieta = 0; ieta < 11; ieta+=10)
            {
              for(int ipt = 0; ipt < 9; ipt++)
              {
                ...
                string KEY_2D = Form("h_m%sEffCosPhiPrimeH_v2_0_rhoinput_%d_rho_%d_eta_%d_pt_%d_y_%d_kin_%d_res_%d",simmode.c_str(),irho,i,ieta,ipt,iopt,ikin,ires);
                string KEY_2D_Global = Form("h_m%sEffCosPhiPrime_v2_0_rhoinput_%d_rho_%d_eta_%d_pt_%d_y_%d_kin_%d_res_%d",simmode.c_str(),irho,i,ieta,ipt,iopt,ikin,ires);

                h_m2D_RC[ires][iopt][irho][i][ikin][ieta][ipt] = (TH2D*) ((TH2D*) File_InPutRC[ires][iopt][ikin]->Get(KEY_2D_RC.c_str()))->Clone(KEY_2D.c_str());
                h_m2D_RC_Global[ires][iopt][irho][i][ikin][ieta][ipt] = (TH2D*) ((TH2D*) File_InPutRC[ires][iopt][ikin]->Get(KEY_2D_RC_Global.c_str()))->Clone(KEY_2D_Global.c_str());

                string KEY_1D_RC_Global = Form("h_m%sEffCosPhiPrimeCos_v2_0_rhoinput_%d_rho_%d_eta_%d_pt_%d_y_%d_kin_%d_res_%d",simmode.c_str(),irho,i,ieta,ipt,iopt,ikin,ires);
                h_m1D_RC_Global_Cos_All[ires][iopt][irho][i][ikin][ieta][ipt] = (TH1D*) h_m2D_RC_Global[ires][iopt][irho][i][ikin][ieta][ipt]->ProjectionX(KEY_1D_RC_Global.c_str());

                //if(irho == 0 && i == 2 && ieta == 0)
                //{
                //  string KEY_Weight = Form("Weight_v2_0_%d_pt_%d",iopt,ptkin[ikin]);
                //  h_m2D_Weight[ires][iopt][ikin] = (TH2D*) h_m2D_RC[ires][iopt][irho][i][ikin][ieta][ipt]->Clone(KEY_Weight.c_str());

                //  double integral = h_m2D_Weight[ires][iopt][ikin]->Integral();           
                //  int totalbins = h_m2D_Weight[ires][iopt][ikin]->GetNbinsX() * h_m2D_Weight[ires][iopt][ikin]->GetNbinsY();
 
                //  cout << "integral = " << integral << ", totalbins = " << totalbins << endl;
           
                //  for(int ix = 1; ix <= h_m2D_Weight[ires][iopt][ikin]->GetNbinsX(); ix++)
                //  {
                //    for(int iy = 1; iy <= h_m2D_Weight[ires][iopt][ikin]->GetNbinsY(); iy++)
                //    {
                //      double val = h_m2D_RC[ires][iopt][irho][i][ikin][ieta][ipt]->GetBinContent(ix,iy);
                //      double weight = integral/double(totalbins)/val;
         
                //      h_m2D_Weight[ires][iopt][ikin]->SetBinContent(ix,iy,weight);
                //      //cout << "weight = " << weight << endl;
                //    }
                //  }           
                //  //h_m2D_Weight[ires][iopt][ikin]->SetDirectory(0);
                //}
                //h_m2D_RC[ires][iopt][irho][i][ikin][ieta][ipt]->Multiply(h_m2D_Weight[ires][iopt][ikin]); 
                //h_m2D_RC_Global[ires][iopt][irho][i][ikin][ieta][ipt]->Multiply(h_m2D_Weight[ires][iopt][ikin]); 
                ...
                //  h_m1D_RC_Global_Cos[ires][iopt][irho][i][ikin][ieta][ipt][ibin] = (TH1D*) h_m2D_RC_Global[ires][iopt][irho][i][ikin][ieta][ipt]->ProjectionX(KEY.c_str(),ibin+1,ibin+1);
                //  //h_m1D_RC_Global_Cos[ires][iopt][irho][i][ikin][ieta][ipt][ibin]->SetDirectory(0);
                ...
                //  h_m1D_RC_Cos[ires][iopt][irho][i][ikin][ieta][ipt][ibin] = (TH1D*) h_m2D_RC[ires][iopt][irho][i][ikin][ieta][ipt]->ProjectionX(KEY.c_str(),ibin+1,ibin+1);
                //  //h_m1D_RC_Cos[ires][iopt][irho][i][ikin][ieta][ipt][ibin]->SetDirectory(0);
                //}
                //for(int ibin = 0; ibin < 20; ibin++)
                //{
                ...
                //  h_m1D_RC_Global_Beta[ires][iopt][irho][i][ikin][ieta][ipt][ibin] = (TH1D*) h_m2D_RC_Global[ires][iopt][irho][i][ikin][ieta][ipt]->ProjectionY(KEY.c_str(),ibin+1,ibin+1);
                //  //h_m1D_RC_Global_Beta[ires][iopt][irho][i][ikin][ieta][ipt][ibin]->SetDirectory(0);
                ...
                //  h_m1D_RC_Beta[ires][iopt][irho][i][ikin][ieta][ipt][ibin] = (TH1D*) h_m2D_RC[ires][iopt][irho][i][ikin][ieta][ipt]->ProjectionY(KEY.c_str(),ibin+1,ibin+1);
                //  //h_m1D_RC_Beta[ires][iopt][irho][i][ikin][ieta][ipt][ibin]->SetDirectory(0);
                //}
                ...
                //if(ieta >= 1)
                //{
                  string KEY = Form("h_m%s2DGLOBALCOSRATIO_v2_6_rhoinput_%d_rho_%d_eta_%d_pt_%d_y_%d_kin_%d_res_%d",simmode.c_str(),irho,i,ieta,ipt,iopt,ikin,ires);
                  h_m1D_RC_Global_Ratio_Cos_All[ires][iopt][irho][i][ikin][ieta][ipt] = (TH1D*) h_m1D_RC_Global_Cos_All[ires][iopt][irho][i][ikin][ieta][ipt]->Clone(KEY.c_str());
                  h_m1D_RC_Global_Ratio_Cos_All[ires][iopt][irho][i][ikin][ieta][ipt]->Divide(h_m1D_RC_Global_Cos_All[ires][iopt][irho][i][ikin][ieta][ipt],h_m1D_RC_Global_Cos_All[ires][iopt][irho][i][ikin][0][0],1,1,"B");
         ...
         //         h_m2D_RC_Corrected[ires][iopt][irho][i][ikin][ieta-1]->SetDirectory(0);
         //         h_m2D_RC_Global_Corrected[ires][iopt][irho][i][ikin][ieta-1]->SetDirectory(0);
         //         h_m2D_RC_Ratio[ires][iopt][irho][i][ikin][ieta-1]->SetDirectory(0);
         //         h_m2D_RC_Global_Ratio[ires][iopt][irho][i][ikin][ieta-1]->SetDirectory(0);
                //}
         //       h_m2D_RC[ires][iopt][irho][i][ikin][ieta][ipt]->SetDirectory(0);
         //       h_m2D_RC_Global[ires][iopt][irho][i][ikin][ieta][ipt]->SetDirectory(0);
              }
            }
          }
        }
        //File_InPutRC[ires][iopt][ikin]->Close();
        ...
        for(int irho = 0; irho < 5; irho++)
        {
          ...
          //for(int i = 0; i < nvar; i++)
          {
            for(int ieta = 0; ieta < 11; ieta+=10)
            {
              for(int ipt = 0; ipt < 9; ipt++) 
              {
                string KEY = Form("h_m%s2DGLOBALCOSCORRECTED_v2_6_rhoinput_%d_rho_%d_eta_%d_pt_%d_y_%d_kin_%d_res_%d",simmode.c_str(),irho,i,ieta,ipt,iopt,ikin,ires);
                h_m1D_RC_Global_Corrected_Cos_All[ires][iopt][irho][i][ikin][ieta][ipt] = (TH1D*) h_m1D_RC_Global_Cos_All[ires][iopt][irho][i][ikin][ieta][ipt]->Clone(KEY.c_str());
                h_m1D_RC_Global_Corrected_Cos_All[ires][iopt][irho][i][ikin][ieta][ipt]->Divide(h_m1D_RC_Global_Ratio_Cos_All[0][iopt][irho][2][ikin][ieta][ipt]);
  ...
  TGraphAsymmErrors *g2D[5][nvar][nkin][5][2][nopts][11]; 
  TGraphAsymmErrors *g2D_Global[5][nvar][nkin][5][2][nopts][11]; 
  TGraphAsymmErrors *g2D_Global_Corrected[5][nvar][nkin][5][2][nopts][11]; 
  TGraphAsymmErrors *g2D_Corrected[5][nvar][nkin][5][2][nopts][11]; 
  TF2 *fits2D[2][nopts][5][nvar][nkin][11][9];
  TF2 *fits2D_Global[2][nopts][5][nvar][nkin][11][9];
  TF2 *fits2D_Global_Corrected[2][nopts][5][nvar][nkin][11][9];
  TF2 *fits2D_Corrected[2][nopts][5][nvar][nkin][11][9];


  TGraphAsymmErrors *g1D_Global[5][nvar][nkin][2][nopts][11]; 
  TGraphAsymmErrors *g1D_Global_Corrected[5][nvar][nkin][2][nopts][11]; 
  TF1 *fits1D_Global[2][nopts][5][nvar][nkin][11][9];
  TF1 *fits1D_Global_Corrected[2][nopts][5][nvar][nkin][11][9];
      ...
      for(int i = 0; i < nvar; i++)
      {
        for(int ikin = 0; ikin < nkin; ikin++)
        {
          for(int irho = 0; irho < 5; irho++)
          {
            for(int ieta = 0; ieta < 11; ieta+=10)
            {
              g1D_Global[irho][i][ikin][ires][iopt][ieta] = new TGraphAsymmErrors();
              g1D_Global_Corrected[irho][i][ikin][ires][iopt][ieta] = new TGraphAsymmErrors();
              for(int ipt = 0; ipt < 9; ipt++) 
              {
                cout << "ipt = " << ipt << ", ieta = " << ieta << endl;
               
                fits1D_Global[ires][iopt][irho][i][ikin][ieta][ipt] = new TF1(Form("fit1D_GlobalCos_%d_%d_%d_%d_%d",iopt,irho,i,ikin,ieta),SpinDensity,-1.0,1.0,2);
                fits1D_Global[ires][iopt][irho][i][ikin][ieta][ipt]->SetParameter(0,1./3.);
                fits1D_Global[ires][iopt][irho][i][ikin][ieta][ipt]->SetParLimits(0,0.0,1.0);
                fits1D_Global[ires][iopt][irho][i][ikin][ieta][ipt]->SetParameter(1,h_m1D_RC_Global_Cos_All[ires][iopt][irho][i][ikin][ieta][ipt]->GetMaximum());
                h_m1D_RC_Global_Cos_All[ires][iopt][irho][i][ikin][ieta][ipt]->Fit(fits1D_Global[ires][iopt][irho][i][ikin][ieta][ipt],"NMRI");
                if(irho == 0) g1D_Global[irho][i][ikin][ires][iopt][ieta]->SetPoint(ipt,float(ipt)*0.025,fits1D_Global[ires][iopt][irho][i][ikin][ieta][ipt]->GetParameter(0)-1./3.-(float(i)-2)*0.1);
                if(irho != 0) g1D_Global[irho][i][ikin][ires][iopt][ieta]->SetPoint(ipt,float(ipt)*0.025,fits1D_Global[ires][iopt][irho][i][ikin][ieta][ipt]->GetParameter(0)-1./3.);
                g1D_Global[irho][i][ikin][ires][iopt][ieta]->SetPointError(ipt,0.0,0.0,fits1D_Global[ires][iopt][irho][i][ikin][ieta][ipt]->GetParError(0),fits1D_Global[ires][iopt][irho][i][ikin][ieta][ipt]->GetParError(0));

                //if(ieta >= 1) 
                //{
                  fits1D_Global_Corrected[ires][iopt][irho][i][ikin][ieta][ipt] = new TF1(Form("fit1D_Global_CorrectedCos_%d_%d_%d_%d_%d",iopt,irho,i,ikin,ieta),SpinDensity,-1.0,1.0,2);
                  fits1D_Global_Corrected[ires][iopt][irho][i][ikin][ieta][ipt]->SetParameter(0,1./3.);
                  fits1D_Global_Corrected[ires][iopt][irho][i][ikin][ieta][ipt]->SetParLimits(0,0.0,1.0);
                  fits1D_Global_Corrected[ires][iopt][irho][i][ikin][ieta][ipt]->SetParameter(1,h_m1D_RC_Global_Corrected_Cos_All[ires][iopt][irho][i][ikin][ieta][ipt]->GetMaximum());
                  h_m1D_RC_Global_Corrected_Cos_All[ires][iopt][irho][i][ikin][ieta][ipt]->Fit(fits1D_Global_Corrected[ires][iopt][irho][i][ikin][ieta][ipt],"NMRI");
                  if(irho == 0) g1D_Global_Corrected[irho][i][ikin][ires][iopt][ieta]->SetPoint(ipt,float(ipt)*0.025,fits1D_Global_Corrected[ires][iopt][irho][i][ikin][ieta][ipt]->GetParameter(0)-1./3.-(float(i)-2)*0.1);
                  if(irho != 0) g1D_Global_Corrected[irho][i][ikin][ires][iopt][ieta]->SetPoint(ipt,float(ipt)*0.025,fits1D_Global_Corrected[ires][iopt][irho][i][ikin][ieta][ipt]->GetParameter(0)-1./3.);
                  g1D_Global_Corrected[irho][i][ikin][ires][iopt][ieta]->SetPointError(ipt,0.0,0.0,fits1D_Global_Corrected[ires][iopt][irho][i][ikin][ieta][ipt]->GetParError(0),fits1D_Global_Corrected[ires][iopt][irho][i][ikin][ieta][ipt]->GetParError(0));
                //}
        ...
        for(int ikin = 0; ikin < nkin; ikin++)
        {
          for(int irho = 0; irho < 5; irho++)
          {
            for(int ieta = 0; ieta < 11; ieta+=10)
            {
              ...
              {
                g2D[irho][i][ikin][par][ires][iopt][ieta] = new TGraphAsymmErrors();
                g2D_Global[irho][i][ikin][par][ires][iopt][ieta] = new TGraphAsymmErrors();
              }   
              for(int ipt = 0; ipt < 9; ipt++)
              {
                fits2D[ires][iopt][irho][i][ikin][ieta][ipt] = new TF2(Form("fit2D_%d_%d_%d_%d_%d",iopt,irho,i,ikin,ieta),SpinDensity2Dcos,-1.0,1.0,0.0,2.0*TMath::Pi(),6);
                fits2D[ires][iopt][irho][i][ikin][ieta][ipt]->SetParameter(0,1./3.);
                fits2D[ires][iopt][irho][i][ikin][ieta][ipt]->SetParLimits(0,0.0,1.0);
                fits2D[ires][iopt][irho][i][ikin][ieta][ipt]->SetParameter(1,0.0);
                fits2D[ires][iopt][irho][i][ikin][ieta][ipt]->SetParameter(2,0.0);
                fits2D[ires][iopt][irho][i][ikin][ieta][ipt]->SetParameter(3,0.0);
                fits2D[ires][iopt][irho][i][ikin][ieta][ipt]->SetParameter(4,0.0);
                fits2D[ires][iopt][irho][i][ikin][ieta][ipt]->SetParameter(5,h_m2D_RC[ires][iopt][irho][i][ikin][ieta][ipt]->GetMaximum());
                //h_m2D_RC[ires][iopt][irho][i][ikin][ieta][ipt]->Fit(fits2D[ires][iopt][irho][i][ikin][ieta][ipt],"NMR");
                for(int par = 0; par < 5; par++)
                {
                  if(par == 0 && par == irho) g2D[irho][i][ikin][par][ires][iopt][ieta]->SetPoint(ipt,float(ipt)*0.025, fits2D[ires][iopt][irho][i][ikin][ieta][ipt]->GetParameter(0)-1./3.-(float(i)-2)*0.1);
                  if(par != 0 && par == irho) g2D[irho][i][ikin][par][ires][iopt][ieta]->SetPoint(ipt,float(ipt)*0.025, fits2D[ires][iopt][irho][i][ikin][ieta][ipt]->GetParameter(par)-(float(i)-2)*0.1);
                  if(par == 0 && par != irho) g2D[irho][i][ikin][par][ires][iopt][ieta]->SetPoint(ipt,float(ipt)*0.025, fits2D[ires][iopt][irho][i][ikin][ieta][ipt]->GetParameter(0)-1./3.);
                  if(par != 0 && par != irho) g2D[irho][i][ikin][par][ires][iopt][ieta]->SetPoint(ipt,float(ipt)*0.025, fits2D[ires][iopt][irho][i][ikin][ieta][ipt]->GetParameter(par));
                  g2D[irho][i][ikin][par][ires][iopt][ieta]->SetPointError(ipt,0.0,0.0,fits2D[ires][iopt][irho][i][ikin][ieta][ipt]->GetParError(par),fits2D[ires][iopt][irho][i][ikin][ieta][ipt]->GetParError(par));
                }
                cout << "Fit helicity " << endl;

                fits2D_Global[ires][iopt][irho][i][ikin][ieta][ipt] = new TF2(Form("fit2D_Global_%d_%d_%d_%d_%d",iopt,irho,i,ikin,ieta),SpinDensity2Dcos,-1.0,1.0,0,2.0*TMath::Pi(),6);
                fits2D_Global[ires][iopt][irho][i][ikin][ieta][ipt]->SetParameter(0,1./3.);
                fits2D_Global[ires][iopt][irho][i][ikin][ieta][ipt]->SetParLimits(0,0.0,1.0);
                fits2D_Global[ires][iopt][irho][i][ikin][ieta][ipt]->SetParameter(1,0.0);
                fits2D_Global[ires][iopt][irho][i][ikin][ieta][ipt]->SetParameter(2,0.0);
                fits2D_Global[ires][iopt][irho][i][ikin][ieta][ipt]->SetParameter(3,0.0);
                fits2D_Global[ires][iopt][irho][i][ikin][ieta][ipt]->SetParameter(4,0.0);
                fits2D_Global[ires][iopt][irho][i][ikin][ieta][ipt]->SetParameter(5,h_m2D_RC_Global[ires][iopt][irho][i][ikin][ieta][ipt]->GetMaximum());
                //h_m2D_RC_Global[ires][iopt][irho][i][ikin][ieta][ipt]->Fit(fits2D_Global[ires][iopt][irho][i][ikin][ieta][ipt],"NMRI");
                for(int par = 0; par < 5; par++)
                {
                  if(par == 0 && par == irho) g2D_Global[irho][i][ikin][par][ires][iopt][ieta]->SetPoint(ipt,float(ipt)*0.025, fits2D_Global[ires][iopt][irho][i][ikin][ieta][ipt]->GetParameter(0)-1./3.-(float(i)-2)*0.1);
                  if(par != 0 && par == irho) g2D_Global[irho][i][ikin][par][ires][iopt][ieta]->SetPoint(ipt,float(ipt)*0.025, fits2D_Global[ires][iopt][irho][i][ikin][ieta][ipt]->GetParameter(par)-(float(i)-2)*0.1);
                  if(par == 0 && par != irho) g2D_Global[irho][i][ikin][par][ires][iopt][ieta]->SetPoint(ipt,float(ipt)*0.025, fits2D_Global[ires][iopt][irho][i][ikin][ieta][ipt]->GetParameter(0)-1./3.);
                  if(par != 0 && par != irho) g2D_Global[irho][i][ikin][par][ires][iopt][ieta]->SetPoint(ipt,float(ipt)*0.025, fits2D_Global[ires][iopt][irho][i][ikin][ieta][ipt]->GetParameter(par));
                  g2D_Global[irho][i][ikin][par][ires][iopt][ieta]->SetPointError(ipt,0.0,0.0,fits2D_Global[ires][iopt][irho][i][ikin][ieta][ipt]->GetParError(par),fits2D_Global[ires][iopt][irho][i][ikin][ieta][ipt]->GetParError(par));
                }
              } 
            }
          }
        ...
        for(int irho = 0; irho < 5; irho++)
        {
          for(int par = 0; par < 5; par++)
          {
            ...
            for(int i = 0; i < nvar; i++)
            {
              ...
              for(int ires = 0; ires < 2; ires++)
              {
                double tmax = g2D_Global[irho][i][ikin][par][ires][iopt][ieta]->GetHistogram()->GetMaximum();   
                double tmin = g2D_Global[irho][i][ikin][par][ires][iopt][ieta]->GetHistogram()->GetMinimum(); 
              ...
              for(int ires = 0; ires < 2; ires++)
              {
                g2D_Global[irho][i][ikin][par][ires][iopt][ieta]->SetMarkerStyle(20);
                g2D_Global[irho][i][ikin][par][ires][iopt][ieta]->SetMarkerSize(1.0);
                g2D_Global[irho][i][ikin][par][ires][iopt][ieta]->SetMarkerColor(color[ires]);
                g2D_Global[irho][i][ikin][par][ires][iopt][ieta]->SetLineColor(color[ires]);
                g2D_Global[irho][i][ikin][par][ires][iopt][ieta]->SetTitle(Form("Input %s^{g}=%1.1f, v_{2}=0.0, p_{T}=%1.1f GeV/c, |#eta|<%1.1f",param[irho].c_str(),(float(i)-2.)*0.1,ptkin[ikin],etacuts[ieta]));
                g2D_Global[irho][i][ikin][par][ires][iopt][ieta]->GetXaxis()->SetTitle(Form("(GeV/c) p_{T,K^{+/-}}<"));
                g2D_Global[irho][i][ikin][par][ires][iopt][ieta]->GetYaxis()->SetTitle(Form("%s^{g} from 2D Fit - Truth",param[par].c_str()));
                g2D_Global[irho][i][ikin][par][ires][iopt][ieta]->GetXaxis()->SetRangeUser(-0.025,0.225);
                if(min < 0.0 && max < 0.0) g2D_Global[irho][i][ikin][par][ires][iopt][ieta]->GetYaxis()->SetRangeUser(1.6*min,0.4*max);
                if(min < 0.0 && max > 0.0) g2D_Global[irho][i][ikin][par][ires][iopt][ieta]->GetYaxis()->SetRangeUser(1.6*min,1.6*max);
                if(min > 0.0 && max > 0.0) g2D_Global[irho][i][ikin][par][ires][iopt][ieta]->GetYaxis()->SetRangeUser(0.4*min,1.6*max);

                if(ires == 0) g2D_Global[irho][i][ikin][par][ires][iopt][ieta]->Draw("APE"); 
                else       g2D_Global[irho][i][ikin][par][ires][iopt][ieta]->Draw("PE same"); 

                if(ikin == 0 && iopt == 0 && ieta == 0 && par == 0 && i == 0 && irho == 0 && ires == 0) leg->AddEntry(g2D_Global[irho][i][ikin][par][ires][iopt][ieta],"MC p_{T}","p");
                if(ikin == 0 && iopt == 0 && ieta == 0 && par == 0 && i == 0 && irho == 0 && ires == 1) leg->AddEntry(g2D_Global[irho][i][ikin][par][ires][iopt][ieta],"p_{T} res. = 5%","p");
              }
              leg->Draw("same");
            }
          }
      ...
      for(int ieta = 0; ieta < 11; ieta+=10)
      {
        for(int irho = 0; irho < 5; irho++)
        {
          ...
          for(int i = 0; i < nvar; i++)
          {
            ...
            for(int ires = 0; ires < 2; ires++)
            {
              double tmax = g1D_Global[irho][i][ikin][ires][iopt][ieta]->GetHistogram()->GetMaximum();   
              double tmin = g1D_Global[irho][i][ikin][ires][iopt][ieta]->GetHistogram()->GetMinimum(); 
            ...
            for(int ires = 0; ires < 2; ires++)
            {
              g1D_Global[irho][i][ikin][ires][iopt][ieta]->SetMarkerStyle(20);
              g1D_Global[irho][i][ikin][ires][iopt][ieta]->SetMarkerSize(1.0);
              g1D_Global[irho][i][ikin][ires][iopt][ieta]->SetMarkerColor(color[ires]);
              g1D_Global[irho][i][ikin][ires][iopt][ieta]->SetLineColor(color[ires]);
              g1D_Global[irho][i][ikin][ires][iopt][ieta]->SetTitle(Form("Input %s^{g}=%1.1f, v_{2}=0.0, p_{T}=%1.1f GeV/c, |#eta|<%1.1f",param[irho].c_str(),(float(i)-2.)*0.1,ptkin[ikin],etacuts[ieta]));
              g1D_Global[irho][i][ikin][ires][iopt][ieta]->GetXaxis()->SetTitle(Form("(GeV/c) p_{T,K^{+/-}}<"));
              g1D_Global[irho][i][ikin][ires][iopt][ieta]->GetYaxis()->SetTitle(Form("%s^{g} from 2D Fit - Truth",param[0].c_str()));
              g1D_Global[irho][i][ikin][ires][iopt][ieta]->GetXaxis()->SetRangeUser(-0.025,0.225);
              if(min < 0.0 && max < 0.0) g1D_Global[irho][i][ikin][ires][iopt][ieta]->GetYaxis()->SetRangeUser(1.6*min,0.4*max);
              if(min < 0.0 && max > 0.0) g1D_Global[irho][i][ikin][ires][iopt][ieta]->GetYaxis()->SetRangeUser(1.6*min,1.6*max);
              if(min > 0.0 && max > 0.0) g1D_Global[irho][i][ikin][ires][iopt][ieta]->GetYaxis()->SetRangeUser(0.4*min,1.6*max);

              if(ires == 0) g1D_Global[irho][i][ikin][ires][iopt][ieta]->Draw("APE"); 
              else       g1D_Global[irho][i][ikin][ires][iopt][ieta]->Draw("PE same"); 
