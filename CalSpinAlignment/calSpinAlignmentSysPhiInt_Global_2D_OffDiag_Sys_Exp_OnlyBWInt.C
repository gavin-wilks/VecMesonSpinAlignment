...
void calSpinAlignmentSysPhiInt_Global_2D_OffDiag_Sys_Exp_OnlyBWInt(int energy = 4, int pid = 0, int year = 0, bool random3D = false, int order = 2, string etamode = "eta1_eta1", int frameopt = 0, int sysidx = 0, string flag = "")
{
  ...
  if(sysidx == 0) 
  {
    //stop_norm = vmsa::Norm_stop;
    //sig_stop = vmsa::Sig_stop;
    start_method = vmsa::Method_start;
    stop_poly = 2;
  }
          ...
          for(int i_poly = 0; i_poly < stop_poly; i_poly++)
          {
            ...
            for(int i_theta = vmsa::CTS_start;  i_theta < vmsa::Cos_Theta_Star_Bins; ++i_theta) // cos(theta*) loop
            {
              ...
              for(int i_phipsi = 0;  i_phipsi < vmsa::Beta_Bins; ++i_phipsi) // cos(theta*) loop
              {
                ...
                if(i_method == 1)
                {
                  //string KEY_Pull = Form("CosThetaStar_%d_Beta_%d_%s_Dca_%d_Sig_%d_HF_%d_HR_%d_M2_%d_%s_Norm_%d_Sigma_%d_Poly%d",i_theta,i_phipsi,EP[order-1].c_str(),vmsa::mSys[sysidx][0],vmsa::mSys[sysidx][1],vmsa::mSys[sysidx][2],vmsa::mSys[sysidx][3],vmsa::mSys[sysidx][4],vmsa::mPID[pid].c_str(),i_norm,i_sigma,i_poly+1);
                  g_pull[KEY_Poly] = new TGraphAsymmErrors();
                  g_pull[KEY_Poly_diff] = new TGraphAsymmErrors();
                  g_pull[KEY_Poly_diffcum] = new TGraphAsymmErrors();
                }
