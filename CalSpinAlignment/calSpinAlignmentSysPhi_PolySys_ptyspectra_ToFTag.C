...
void calSpinAlignmentSysPhi_PolySys_ptyspectra_ToFTag(int energy = 4, int pid = 0, int year = 0, bool random3D = false, int order = 2, string etamode = "eta1_eta1")
{
  ...
  string inputfile = Form("../output/AuAu%s/%s/Tagging_ptyspectra_InvMassSubBg_%s.root",vmsa::mBeamEnergy[energy].c_str(),vmsa::mPID[pid].c_str(),etamode.c_str());
  ...
  string Tag[2] = {"TagKplus","TagKminus"};
	...
	for(Int_t i_sig = vmsa::nSigKaon_start; i_sig < 1/*vmsa::nSigKaon_stop*/; i_sig++)
	{
	  ...
	  for(int i_norm = vmsa::Norm_start; i_norm < 1; ++i_norm)
	  {
	    for(int i_eta = 0; i_eta < vmsa::rebinytotal; i_eta++) // cos(theta*) loop
	    {
              ...
              {            
	        string KEY = Form("pt_%d_eta_%d_Centrality_%d_%s_Dca_%d_Sig_%d_%s_Norm_%d_%s",i_pt,i_eta,i_cent,EP[order-1].c_str(),i_dca,i_sig,vmsa::mPID[pid].c_str(),i_norm,Tag[i_tag].c_str());
                cout << KEY << endl;
  ...
  float InvMassLow[2][vmsa::rebinpttotal][vmsa::rebinytotal] = {0.0};
  float InvMassHigh[2][vmsa::rebinpttotal][vmsa::rebinytotal] = {0.0};
  float fpoly[2][vmsa::rebinpttotal][vmsa::rebinytotal] = {0.0};
  ...
  vecFMap Par_rhoFit;
  TGraMap g_mRho;
  TGraMap g_mIntercept;
  TGraMap g_mSlope;
  TGraMap g_mTotal;
  TGraMap g_mPoly;
  TGraMap g_mTotalPoly;
	  ...
	  for(int i_sigma = vmsa::Sig_start; i_sigma < 1/*vmsa::Sig_stop*/; ++i_sigma)
	  {
	    for(int i_method = 1/*vmsa::Method_start*/; i_method < vmsa::Method_stop; ++i_method)
	    {
              for(int i_poly = 0; i_poly < 1/*3*/; i_poly++)
              {
                for(int i_tag = 0; i_tag < 2; i_tag++)
                {
	          string KEY_spectra = Form("ptyspectra_Centrality_%d_%s_Dca_%d_Sig_%d_%s_Norm_%d_Sigma_%d_%s_Poly%d_%s",i_cent,EP[order-1].c_str(),i_dca,i_sig,vmsa::mPID[pid].c_str(),i_norm,i_sigma,vmsa::mInteMethod[i_method].c_str(),i_poly+1,Tag[i_tag].c_str());
	          ...
	          {
                    cout << "i_pt = " << i_pt << endl;
	            string KEY_int = Form(      "intercept_pt_%d_Centrality_%d_%s_Dca_%d_Sig_%d_%s_Norm_%d_%s",i_pt,i_cent,EP[order-1].c_str(),i_dca,i_sig,vmsa::mPID[pid].c_str(),i_norm,Tag[i_tag].c_str());
	            string KEY_slope = Form(        "slope_pt_%d_Centrality_%d_%s_Dca_%d_Sig_%d_%s_Norm_%d_%s",i_pt,i_cent,EP[order-1].c_str(),i_dca,i_sig,vmsa::mPID[pid].c_str(),i_norm,Tag[i_tag].c_str());
	            string KEY_total = Form(        "total_pt_%d_Centrality_%d_%s_Dca_%d_Sig_%d_%s_Norm_%d_%s",i_pt,i_cent,EP[order-1].c_str(),i_dca,i_sig,vmsa::mPID[pid].c_str(),i_norm,Tag[i_tag].c_str());
	            string KEY_poly = Form(          "poly_pt_%d_Centrality_%d_%s_Dca_%d_Sig_%d_%s_Norm_%d_%s",i_pt,i_cent,EP[order-1].c_str(),i_dca,i_sig,vmsa::mPID[pid].c_str(),i_norm,Tag[i_tag].c_str());
	            string KEY_totalpoly = Form("totalpoly_pt_%d_Centrality_%d_%s_Dca_%d_Sig_%d_%s_Norm_%d_%s",i_pt,i_cent,EP[order-1].c_str(),i_dca,i_sig,vmsa::mPID[pid].c_str(),i_norm,Tag[i_tag].c_str());
                    g_mIntercept[KEY_int]       = new TGraphAsymmErrors();
                    g_mSlope[KEY_slope]         = new TGraphAsymmErrors();
                    g_mTotal[KEY_total]         = new TGraphAsymmErrors();
                    g_mPoly[KEY_poly]           = new TGraphAsymmErrors();
                    g_mTotalPoly[KEY_totalpoly] = new TGraphAsymmErrors();
	            ...
	            for(int i_eta = 0; i_eta < vmsa::rebinytotal; ++i_eta) // cos(theta*) loop
	            {
	              ...
	              string KEY = Form("pt_%d_eta_%d_Centrality_%d_%s_Dca_%d_Sig_%d_%s_Norm_%d_%s",i_pt,i_eta,i_cent,EP[order-1].c_str(),i_dca,i_sig,vmsa::mPID[pid].c_str(),i_norm,Tag[i_tag].c_str());
	              string KEY_Poly = Form("pt_%d_eta_%d_Centrality_%d_%s_Dca_%d_Sig_%d_%s_Norm_%d_Poly%d_%s",i_pt,i_eta,i_cent,EP[order-1].c_str(),i_dca,i_sig,vmsa::mPID[pid].c_str(),i_norm,i_poly+1,Tag[i_tag].c_str());
                      cout << KEY << endl;
                      cout << KEY_Poly << endl;
	              TF1 *f_bw;
	              ...
	              TFitResultPtr result = h_mMass[KEY]->Fit(f_bw,"MQNRIS");
                      if(result.Get() == nullptr) 
                      {
                        skip[i_tag][i_pt][i_eta] = true;
	              ...
	              float Inte_start = f_bw->GetParameter(0)-vmsa::nSigVecSys[i_sigma]*f_bw->GetParameter(1)-0.5*bin_width;
                      ...
                      InvMassLow[i_tag][i_pt][i_eta]  = Inte_start;
                      InvMassHigh[i_tag][i_pt][i_eta] = Inte_stop;
	              ...
	              if(i_method == 1)
	              {
	                ...
	                //h_mSpectra[KEY_spectra]->SetBinContent(h_mSpectra[KEY_spectra]->FindBin(pt_mean,y_mean),counts_bw);
	                //h_mSpectra[KEY_spectra]->SetBinError(h_mSpectra[KEY_spectra]->FindBin(pt_mean,y_mean),errors_bw);

                        g_mTotal[KEY_total]         -> SetPoint(i_eta,y_mean,counts_bw);
                        g_mPoly[KEY_poly]           -> SetPoint(i_eta,y_mean,counts_bg);
                        g_mTotalPoly[KEY_totalpoly] -> SetPoint(i_eta,y_mean,final_counts_bw);
                        g_mTotal[KEY_total]         -> SetPointError(i_eta,0.0,0.0,errors_bw,errors_bw);
                        g_mPoly[KEY_poly]           -> SetPointError(i_eta,0.0,0.0,errors_bg,errors_bg);
                        g_mTotalPoly[KEY_totalpoly] -> SetPointError(i_eta,0.0,0.0,TMath::Sqrt(errors_bw*errors_bw+errors_bg*errors_bg),TMath::Sqrt(errors_bw*errors_bw+errors_bg*errors_bg));

                        fpoly[i_tag][i_pt][i_eta] = counts_bg/counts_bw;
	              }
	              Par[KEY_Poly].clear();
	              ...
	              if(i_poly >= 2) Par[KEY_Poly].push_back(static_cast<float>(f_bw->GetParameter(6)));
                      ...
                      g_mIntercept[KEY_int]->SetPoint(i_eta,y_mean,f_bw->GetParameter(3));
                      g_mIntercept[KEY_int]->SetPointError(i_eta,0.0,0.0,f_bw->GetParError(3),f_bw->GetParError(3));
                      g_mSlope[KEY_slope]->SetPoint(i_eta,y_mean,f_bw->GetParameter(4));
                      g_mSlope[KEY_slope]->SetPointError(i_eta,0.0,0.0,f_bw->GetParError(4),f_bw->GetParError(4));
  ...
  //            for(int i_poly = 0; i_poly < 1/*3*/; i_poly++)
  //            {
  //              string outputname = Form("./figures/%s/%s/pTstudy/polynomialbackground_ptyspectra_allThetaYields%s_%s_Order%d_cent%d_%s_pTdependence_Norm%d_Poly%d.pdf",vmsa::mPID[pid].c_str(),vmsa::mBeamEnergy[energy].c_str(),vmsa::mPID[pid].c_str(),vmsa::mBeamEnergy[energy].c_str(),order,i_cent,etamode.c_str(),i_norm,i_poly+1);
  //              string output_start = Form("%s[",outputname.c_str());
  //              
  //              TCanvas *c_diff = new TCanvas("c_diff","c_diff",10,10,2000,1600);
  ...
  //                string KEY_int = Form("intercept_pt_%d_Centrality_%d_%s_Dca_%d_Sig_%d_%s_Norm_%d",i_pt,i_cent,EP[order-1].c_str(),i_dca,i_sig,vmsa::mPID[pid].c_str(),i_norm);
  //                g_mIntercept[KEY_int]->SetTitle(Form("%.1f<p_{T}<%.1f GeV/c",vmsa::rebinptval[i_pt],vmsa::rebinptval[i_pt+1]));
  //                g_mIntercept[KEY_int]->GetXaxis()->SetTitle("y");
  //                g_mIntercept[KEY_int]->GetYaxis()->SetTitle("Poly 1 Intercept");
  //                g_mIntercept[KEY_int]->Draw("APE");
  //               
  //                c_diff->cd(i_pt+11);
  //                c_diff->cd(i_pt+11)->SetLeftMargin(0.15);
  //                c_diff->cd(i_pt+11)->SetBottomMargin(0.15);
  //                c_diff->cd(i_pt+11)->SetTicks(1,1);
  //                c_diff->cd(i_pt+11)->SetGrid(0,0);
  //                string KEY_slope = Form("slope_pt_%d_Centrality_%d_%s_Dca_%d_Sig_%d_%s_Norm_%d",i_pt,i_cent,EP[order-1].c_str(),i_dca,i_sig,vmsa::mPID[pid].c_str(),i_norm);
  //                g_mSlope[KEY_slope]->SetTitle(Form("%.1f<p_{T}<%.1f GeV/c",vmsa::rebinptval[i_pt],vmsa::rebinptval[i_pt+1]));
  //                g_mSlope[KEY_slope]->GetXaxis()->SetTitle("y");
  //                g_mSlope[KEY_slope]->GetYaxis()->SetTitle("Poly 1 Slope");
  //                g_mSlope[KEY_slope]->Draw("APE");
  //              }
  //              c_diff->Update();
  ...
  //            for(int i_poly = 0; i_poly < 1/*3*/; i_poly++)
  //            {
  //              string outputname = Form("./figures/%s/%s/pTstudy/total_ptyspectra_allThetaYields%s_%s_Order%d_cent%d_%s_pTdependence_Norm%d_Poly%d.pdf",vmsa::mPID[pid].c_str(),vmsa::mBeamEnergy[energy].c_str(),vmsa::mPID[pid].c_str(),vmsa::mBeamEnergy[energy].c_str(),order,i_cent,etamode.c_str(),i_norm,i_poly+1);
  //              string output_start = Form("%s[",outputname.c_str());
  //              
  //              TCanvas *c_diff = new TCanvas("c_diff","c_diff",10,10,2000,800);
  //              c_diff->Print(output_start.c_str());
  //              c_diff->Divide(5,2);
  //              for(int i_pt = 0/*vmsa::pt_start*/; i_pt < vmsa::rebinpttotal/*< vmsa::pt_stop*/; ++i_pt) // pt loop
  //              {
  ...
  //                c_diff->cd(i_pt+1)->SetTicks(1,1);
  //                c_diff->cd(i_pt+1)->SetGrid(0,0);
  //                string KEY_total = Form("total_pt_%d_Centrality_%d_%s_Dca_%d_Sig_%d_%s_Norm_%d",i_pt,i_cent,EP[order-1].c_str(),i_dca,i_sig,vmsa::mPID[pid].c_str(),i_norm);
  //                g_mTotal[KEY_total]->SetTitle(Form("%.1f<p_{T}<%.1f GeV/c",vmsa::rebinptval[i_pt],vmsa::rebinptval[i_pt+1]));
  //                g_mTotal[KEY_total]->GetXaxis()->SetTitle("y");
  //                g_mTotal[KEY_total]->GetYaxis()->SetTitle("Integral of BW+Poly1");
  //                g_mTotal[KEY_total]->Draw("APE");
  //              }
  //              c_diff->Update();
  //              c_diff->Print(outputname.c_str());
  //              string output_stop = Form("%s]",outputname.c_str());
  ...
  //            for(int i_poly = 0; i_poly < 1/*3*/; i_poly++)
  //            {
  //              string outputname = Form("./figures/%s/%s/pTstudy/poly_ptyspectra_allThetaYields%s_%s_Order%d_cent%d_%s_pTdependence_Norm%d_Poly%d.pdf",vmsa::mPID[pid].c_str(),vmsa::mBeamEnergy[energy].c_str(),vmsa::mPID[pid].c_str(),vmsa::mBeamEnergy[energy].c_str(),order,i_cent,etamode.c_str(),i_norm,i_poly+1);
  //              string output_start = Form("%s[",outputname.c_str());
  //              
  //              TCanvas *c_diff = new TCanvas("c_diff","c_diff",10,10,2000,800);
  //              c_diff->Print(output_start.c_str());
  ...
  //                c_diff->cd(i_pt+1)->SetTicks(1,1);
  //                c_diff->cd(i_pt+1)->SetGrid(0,0);
  //                string KEY_poly = Form("poly_pt_%d_Centrality_%d_%s_Dca_%d_Sig_%d_%s_Norm_%d",i_pt,i_cent,EP[order-1].c_str(),i_dca,i_sig,vmsa::mPID[pid].c_str(),i_norm);
  //                g_mPoly[KEY_poly]->SetTitle(Form("%.1f<p_{T}<%.1f GeV/c",vmsa::rebinptval[i_pt],vmsa::rebinptval[i_pt+1]));
  //                g_mPoly[KEY_poly]->GetXaxis()->SetTitle("y");
  //                g_mPoly[KEY_poly]->GetYaxis()->SetTitle("Integral of Poly1");
  //                g_mPoly[KEY_poly]->Draw("APE");
  //              }
  //              c_diff->Update();
  //              c_diff->Print(outputname.c_str());
  //              string output_stop = Form("%s]",outputname.c_str());
  ...
  //            for(int i_poly = 0; i_poly < 1/*3*/; i_poly++)
  //            {
  //              string outputname = Form("./figures/%s/%s/pTstudy/totalpoly_ptyspectra_allThetaYields%s_%s_Order%d_cent%d_%s_pTdependence_Norm%d_Poly%d.pdf",vmsa::mPID[pid].c_str(),vmsa::mBeamEnergy[energy].c_str(),vmsa::mPID[pid].c_str(),vmsa::mBeamEnergy[energy].c_str(),order,i_cent,etamode.c_str(),i_norm,i_poly+1);
  //              string output_start = Form("%s[",outputname.c_str());
  //              
  //              TCanvas *c_diff = new TCanvas("c_diff","c_diff",10,10,2000,800);
  //              c_diff->Print(output_start.c_str());
  ...
  //                c_diff->cd(i_pt+1)->SetTicks(1,1);
  //                c_diff->cd(i_pt+1)->SetGrid(0,0);
  //                string KEY_totalpoly = Form("totalpoly_pt_%d_Centrality_%d_%s_Dca_%d_Sig_%d_%s_Norm_%d",i_pt,i_cent,EP[order-1].c_str(),i_dca,i_sig,vmsa::mPID[pid].c_str(),i_norm);
  //                g_mTotalPoly[KEY_totalpoly]->SetTitle(Form("%.1f<p_{T}<%.1f GeV/c",vmsa::rebinptval[i_pt],vmsa::rebinptval[i_pt+1]));
  //                g_mTotalPoly[KEY_totalpoly]->GetXaxis()->SetTitle("y");
  //                g_mTotalPoly[KEY_totalpoly]->GetYaxis()->SetTitle("Int. of BW+Poly1 - Int. of Poly1");
  //                g_mTotalPoly[KEY_totalpoly]->Draw("APE");
  //              }
  //              c_diff->Update();
  //              c_diff->Print(outputname.c_str());
  //              string output_stop = Form("%s]",outputname.c_str());
  ...
  cout << endl << "const float InvMassLow[2][rebinpttotal][rebinytotal] = {";
  for(int i_tag = 0; i_tag < 2; i_tag++)
  {
    cout << "{"; 
    for(int ptbin = 0; ptbin < vmsa::rebinpttotal; ptbin++)
    {
      ...
      {
        if(etabin < vmsa::rebinytotal-1) cout << InvMassLow[i_tag][ptbin][etabin] << ",";
        else                             cout << InvMassLow[i_tag][ptbin][etabin];
      }
      if(ptbin < vmsa::rebinpttotal -1 ) cout << "}," << endl;
      else                                cout << "}" << endl;
    }
    if(i_tag == 0) cout << "}," << endl;
    if(i_tag == 1) cout << "}" << endl;
  }
  cout << "};" << endl << endl;

  cout << endl << "const float InvMassHigh[2][rebinpttotal][rebinytotal] = {";
  for(int i_tag = 0; i_tag < 2; i_tag++)
  {
    cout << "{"; 
    for(int ptbin = 0; ptbin < vmsa::rebinpttotal; ptbin++)
    {
      ...
      for(int etabin = 0; etabin < vmsa::rebinytotal; etabin++)
      {
        if(etabin < vmsa::rebinytotal-1) cout << InvMassHigh[i_tag][ptbin][etabin] << ",";
        else                             cout << InvMassHigh[i_tag][ptbin][etabin];
      }
      if(ptbin < vmsa::rebinpttotal -1 ) cout << "}," << endl;
      else                                cout << "}" << endl;
    }
  ...
  cout << endl << "const float fpoly[2][rebinpttotal][rebinytotal] = {";
  for(int i_tag = 0; i_tag < 2; i_tag++)
  {
    cout << "{"; 
    for(int ptbin = 0; ptbin < vmsa::rebinpttotal; ptbin++)
    {
      ...
      for(int etabin = 0; etabin < vmsa::rebinytotal; etabin++)
      {
        if(etabin < vmsa::rebinytotal-1) cout << fpoly[i_tag][ptbin][etabin] << ",";
        else                             cout << fpoly[i_tag][ptbin][etabin];
      }
      if(ptbin < vmsa::rebinpttotal -1 ) cout << "}," << endl;
      else                                cout << "}" << endl;
    }
  ...
  for(int i_tag = 0; i_tag < 2; i_tag++)
  {
    for(int i_poly = 0; i_poly < 1; i_poly++)
    {
      for(int i_norm = 0; i_norm < 1; i_norm++)
      {
        for(int i_cent = 9; i_cent < 10; ++i_cent) // Centrality loop
        {
          string outputname = Form("./figures/%s/%s/pTstudy/ptyspectra_allThetaYields%s_%s_Order%d_cent%d_%s_pTdependence_Norm%d_Poly%d_%s.pdf",vmsa::mPID[pid].c_str(),vmsa::mBeamEnergy[energy].c_str(),vmsa::mPID[pid].c_str(),vmsa::mBeamEnergy[energy].c_str(),order,i_cent,etamode.c_str(),i_norm,i_poly+1,Tag[i_tag].c_str());
          string output_start = Form("%s[",outputname.c_str());
          
          //TCanvas *c_diff = new TCanvas("c_diff","c_diff",10,10,1200,900);
          TCanvas *c_diff = new TCanvas("c_diff","c_diff",10,10,1200,1200);
          c_diff->Print(output_start.c_str());
          ...
          for(int i_pt = 0/*vmsa::pt_start*/; i_pt < vmsa::rebinpttotal/*< vmsa::pt_stop*/; ++i_pt) // pt loop
          {
            ...
            for(int i_eta = 0; i_eta < vmsa::rebinytotal; i_eta++) // cos(theta*) loop
            {
              if(skip[i_tag][i_pt][i_eta]) continue;
              c_diff->cd(i_eta+1);
              ...
              string KEY_QA = Form("pt_%d_eta_%d_Centrality_%d_%s_Dca_%d_Sig_%d_%s_Norm_%d_%s",i_pt,i_eta,i_cent,EP[order-1].c_str(),vmsa::Dca_start,vmsa::nSigKaon_start,vmsa::mPID[pid].c_str(),i_norm,Tag[i_tag].c_str());
              //cout << KEY_QA << endl;
              ...
              string KEY_QA_Poly = Form("pt_%d_eta_%d_Centrality_%d_%s_Dca_%d_Sig_%d_%s_Norm_%d_Poly%d_%s",i_pt,i_eta,i_cent,EP[order-1].c_str(),vmsa::Dca_start,vmsa::nSigKaon_start,vmsa::mPID[pid].c_str(),i_norm,i_poly+1,Tag[i_tag].c_str());
  ...
  for(int i_tag = 0; i_tag < 2; i_tag++)
  {
    for(int i_poly = 0; i_poly < 1/*3*/; i_poly++)
    {
      for(int i_norm = 0; i_norm < 1; i_norm++)
      {
        for(int i_cent = 9; i_cent < 10; ++i_cent) // Centrality loop
        {
          string outputname = Form("./figures/%s/%s/pTstudy/ptyspectra_2Dyield%s_%s_Order%d_cent%d_%s_pTdependence_Norm%d_Poly%d_%s.pdf",vmsa::mPID[pid].c_str(),vmsa::mBeamEnergy[energy].c_str(),vmsa::mPID[pid].c_str(),vmsa::mBeamEnergy[energy].c_str(),order,i_cent,etamode.c_str(),i_norm,i_poly+1,Tag[i_tag].c_str());
          ...
          for(int i_sigma = 0/*vmsa::Sig_start*/; i_sigma < 1/*vmsa::Sig_stop*/; ++i_sigma)
          {
            ...
            {
              string KEY_spectra_QA = Form("ptyspectra_Centrality_%d_%s_Dca_%d_Sig_%d_%s_Norm_%d_Sigma_%d_%s_Poly%d_%s",i_cent,EP[order-1].c_str(),vmsa::Dca_start,0/*vmsa::nSigKaon_start*/,vmsa::mPID[pid].c_str(),i_norm,i_sigma,vmsa::mInteMethod[i_method].c_str(),i_poly+1,Tag[i_tag].c_str());
  ...
  string outputfile = Form("../output/AuAu%s/%s/Tagging_ptyspectra_RawPhiPtSys_%s_PolySys.root",vmsa::mBeamEnergy[energy].c_str(),vmsa::mPID[pid].c_str(),etamode.c_str());
