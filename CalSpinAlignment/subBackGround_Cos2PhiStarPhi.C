...
void subBackGround_Cos2PhiStarPhi(int energy = 4, int pid = 0, int year = 0, string date = "20240318", bool random3D = false, int order = 2, string etamode = "eta1_eta1")
{
  std::string EP[2] = {"","2nd"};
  ...
  string InPutFile_SE = Form("../data/Yields_Phi_SE_%s_%s_%s_Cos2PhiStarPhi.root",vmsa::mBeamEnergy[energy].c_str(),date.c_str(),etamode.c_str());
  ...
  string InPutFile_ME = Form("../data/Yields_Phi_ME_%s_%s_%s_Cos2PhiStarPhi.root",vmsa::mBeamEnergy[energy].c_str(),date.c_str(),etamode.c_str());
  ...
  // calculate SE - ME
  TProMap h_mInPut_SE, h_mInPut_ME;
  TProMap h_mMass_SE, h_mMass_ME, h_mMass_SM;
  for(Int_t i_pt = 0; i_pt < vmsa::pt_total; i_pt++) // pt bin 
  {
	...
	for(Int_t i_dca = vmsa::Dca_start; i_dca < vmsa::Dca_stop; i_dca++)
	{
	  for(Int_t i_sig = vmsa::nSigKaon_start; i_sig < vmsa::nSigKaon_stop; i_sig++)
	  {
	    ...
	    string KEY_InPutSE = Form("profile_pt_%d_Centrality_%d_Cos2PhiStarPhi_%s_Dca_%d_Sig_%d_%s_SE",i_pt,i_cent,EP[order-1].c_str(),i_dca,i_sig,vmsa::mPID[pid].c_str());
            h_mInPut_SE[KEY_InPutSE] = (TProfile*)File_SE->Get(KEY_InPutSE.c_str())->Clone(); 

	    string KEY_InPutME = Form("profile_pt_%d_Centrality_%d_Cos2PhiStarPhi_%s_Dca_%d_Sig_%d_%s_ME",i_pt,i_cent,EP[order-1].c_str(),i_dca,i_sig,vmsa::mPID[pid].c_str());
	    h_mInPut_ME[KEY_InPutME] = (TProfile*)File_ME->Get(KEY_InPutME.c_str())->Clone(); 

	    //for(int i_norm = vmsa::Norm_start; i_norm < vmsa::Norm_stop; ++i_norm)
	    for(int i_norm = 0; i_norm < 1; ++i_norm)
	    {
	      string KEY_SE = Form("profile_pt_%d_Centrality_%d_Cos2PhiStarPhi_%s_Dca_%d_Sig_%d_%s_Norm_%d_SE",i_pt,i_cent,EP[order-1].c_str(),i_dca,i_sig,vmsa::mPID[pid].c_str(),i_norm);
	      h_mMass_SE[KEY_SE] = (TProfile*)h_mInPut_SE[KEY_InPutSE]->Clone(KEY_SE.c_str());
              //h_mMass_SE[KEY_SE]->Rebin(4);
	      string KEY_ME = Form("profile_pt_%d_Centrality_%d_Cos2PhiStarPhi_%s_Dca_%d_Sig_%d_%s_Norm_%d_ME",i_pt,i_cent,EP[order-1].c_str(),i_dca,i_sig,vmsa::mPID[pid].c_str(),i_norm);
	      h_mMass_ME[KEY_ME] = (TProfile*)h_mInPut_ME[KEY_InPutME]->Clone(KEY_ME.c_str());
              //h_mMass_ME[KEY_ME]->Rebin(4);
	      string KEY_SM = Form("profile_pt_%d_Centrality_%d_Cos2PhiStarPhi_%s_Dca_%d_Sig_%d_%s_Norm_%d_SM",i_pt,i_cent,EP[order-1].c_str(),i_dca,i_sig,vmsa::mPID[pid].c_str(),i_norm);
	      h_mMass_SM[KEY_SM] = (TProfile*)h_mMass_SE[KEY_SE]->Clone();
	      if(i_norm < 2)
	      {
		...
		//h_mMass_SM[KEY_SM]->Divide(h_mMass_ME[KEY_ME]);
	      }
	      //else
	      //{
	      //  float Inte_SE = 0.0;
...
//  }
  ...
  string outputname = Form("figures/%s/%s/pTstudy/AvgCos2PhiStarPhiDistributions_%s_Order%d_Cent9.pdf",vmsa::mPID[pid].c_str(),vmsa::mBeamEnergy[energy].c_str(),vmsa::mBeamEnergy[energy].c_str(),order);
  ...
  for(int i_pt = vmsa::pt_start; i_pt < vmsa::pt_stop; i_pt++) // pt loop
  {
    ...
    string KEY_SE_QA = Form("profile_pt_%d_Centrality_%d_Cos2PhiStarPhi_%s_Dca_%d_Sig_%d_%s_Norm_%d_SE",i_pt,9,EP[order-1].c_str(),vmsa::Dca_start,vmsa::nSigKaon_start,vmsa::mPID[pid].c_str(),vmsa::Norm_QA);
    h_mMass_SE[KEY_SE_QA]->GetYaxis()->SetRangeUser(-0.1*h_mMass_SE[KEY_SE_QA]->GetMaximum(),1.1*h_mMass_SE[KEY_SE_QA]->GetMaximum());
    //h_mMass_SE[KEY_SE_QA]->SetTitle(Form("%d/7<|cos(#theta*)|<%d/7",itheta,itheta+1));
    h_mMass_SE[KEY_SE_QA]->DrawCopy();

    string KEY_ME_QA = Form("profile_pt_%d_Centrality_%d_Cos2PhiStarPhi_%s_Dca_%d_Sig_%d_%s_Norm_%d_ME",i_pt,9,EP[order-1].c_str(),vmsa::Dca_start,vmsa::nSigKaon_start,vmsa::mPID[pid].c_str(),vmsa::Norm_QA);
    h_mMass_ME[KEY_ME_QA]->SetLineColor(2);
    h_mMass_ME[KEY_ME_QA]->SetFillColor(2);
    h_mMass_ME[KEY_ME_QA]->SetFillStyle(3002);
    h_mMass_ME[KEY_ME_QA]->DrawCopy("h same");

    string KEY_SM_QA = Form("profile_pt_%d_Centrality_%d_Cos2PhiStarPhi_%s_Dca_%d_Sig_%d_%s_Norm_%d_SM",i_pt,9,EP[order-1].c_str(),vmsa::Dca_start,vmsa::nSigKaon_start,vmsa::mPID[pid].c_str(),vmsa::Norm_QA);
    h_mMass_SM[KEY_SM_QA]->SetLineColor(4);
    h_mMass_SM[KEY_SM_QA]->SetFillColor(4);
    h_mMass_SM[KEY_SM_QA]->SetFillStyle(3004);
    h_mMass_SM[KEY_SM_QA]->DrawCopy("h same");
  ...
  TH1FMap h_mMass_Add_SE; // rebinned InvMass distribution, SE-ME
  TH1FMap h_mMass_Add_ME; // rebinned InvMass distribution, SE-ME
  ...
  for(int i_cent = vmsa::Cent_start; i_cent < vmsa::Cent_stop; i_cent++) // Centrality loop
  {
	...
	for(Int_t i_sig = vmsa::nSigKaon_start; i_sig < vmsa::nSigKaon_stop; i_sig++)
	{
	  ...
	  for(int i_norm = 0; i_norm < 1; ++i_norm)
	  {
	    ...
	    {
	      string KEY = Form("profile_pt_%d_Centrality_%d_Cos2PhiStarPhi_%s_Dca_%d_Sig_%d_%s_Norm_%d",pt_bin,i_cent,EP[order-1].c_str(),i_dca,i_sig,vmsa::mPID[pid].c_str(),i_norm);
	      string KEY_Add_SE = Form("profile_pt_%d_Centrality_%d_Cos2PhiStarPhi_%s_Dca_%d_Sig_%d_%s_Norm_%d_Add_SE",pt_bin,i_cent,EP[order-1].c_str(),i_dca,i_sig,vmsa::mPID[pid].c_str(),i_norm);
	      string KEY_Add_ME = Form("profile_pt_%d_Centrality_%d_Cos2PhiStarPhi_%s_Dca_%d_Sig_%d_%s_Norm_%d_Add_ME",pt_bin,i_cent,EP[order-1].c_str(),i_dca,i_sig,vmsa::mPID[pid].c_str(),i_norm);
		...
		{
		  //h_mMass[KEY] = (TProfile*)h_mMass_SM[KEY_SM]->Clone(KEY.c_str());
		  h_mMass_Add_SE[KEY_Add_SE] = (TH1F*)(h_mMass_SE[KEY_SE]->ProjectionX()->Clone(KEY.c_str()));
		  h_mMass_Add_ME[KEY_Add_ME] = (TH1F*)(h_mMass_ME[KEY_ME]->ProjectionX()->Clone(KEY.c_str()));
		}
		if(i_pt > vmsa::pt_rebin_start[energy][pt_bin] && i_pt < vmsa::pt_rebin_stop[energy][pt_bin])
		{
		  h_mMass_Add_SE[KEY_Add_SE]->Add(h_mMass_SE[KEY_SE]->ProjectionX());
		  h_mMass_Add_ME[KEY_Add_ME]->Add(h_mMass_ME[KEY_ME]->ProjectionX());
		}
                if(i_pt == vmsa::pt_rebin_start[energy][pt_bin])
                {
		  h_mMass_Add_SE[KEY_Add_SE]->Print();
		  h_mMass_Add_ME[KEY_Add_ME]->Print();
                  h_mMass[KEY] = (TH1F*)h_mMass_Add_SE[KEY_Add_SE]->Clone(KEY.c_str());
                  h_mMass[KEY]->Divide(h_mMass_Add_SE[KEY_Add_SE],h_mMass_Add_ME[KEY_Add_ME]);
                }
	      }
	    }
	  }
  ...
  TCanvas *c_pT_rebin = new TCanvas("c_pT_rebin","c_pT_rebin",10,10,900,600);
  c_pT_rebin->Divide(3,2);

  outputname = Form("figures/%s/%s/pTstudy/AvgCos2PhiStarPhiBackgroundSubtracted_Rebinned_%s_Order%d_Cent9.pdf",vmsa::mPID[pid].c_str(),vmsa::mBeamEnergy[energy].c_str(),vmsa::mBeamEnergy[energy].c_str(),order);
  ...
  c_pT_rebin->Print(output_start.c_str());
    ...
    {
      //cout << "i_pt-1 = " << i_pt-1 << endl;
      c_pT_rebin->cd(i_pt+1);
      c_pT_rebin->cd(i_pt+1)->SetLeftMargin(0.15);
      c_pT_rebin->cd(i_pt+1)->SetBottomMargin(0.15);
      c_pT_rebin->cd(i_pt+1)->SetTicks(1,1);
      c_pT_rebin->cd(i_pt+1)->SetGrid(0,0);
      string KEY_QA = Form("profile_pt_%d_Centrality_%d_Cos2PhiStarPhi_%s_Dca_%d_Sig_%d_%s_Norm_%d",i_pt,9,EP[order-1].c_str(),vmsa::Dca_start,vmsa::nSigKaon_start,vmsa::mPID[pid].c_str(),vmsa::Norm_QA);
      h_mMass[KEY_QA]->SetMarkerStyle(24);
      h_mMass[KEY_QA]->SetMarkerSize(1.2);
      h_mMass[KEY_QA]->SetLineColor(kGray+2);
      //h_mMass[KEY_QA]->SetTitle(Form("%d/7<|cos(#theta*)|<%d/7",itheta,itheta+1));
      h_mMass[KEY_QA]->GetYaxis()->SetRangeUser(0.9*h_mMass[KEY_QA]->GetMaximum(),h_mMass[KEY_QA]->GetMaximum()*1.05);
      h_mMass[KEY_QA]->DrawCopy("pE");
      // cout << "KEY_QA = " << KEY_QA.c_str() << endl;

      string pT_range = Form("p_{T}=[%.2f,%.2f]GeV/c",vmsa::pt_low[energy][i_pt],vmsa::pt_up[energy][i_pt]);
      plotTopLegend((char*)pT_range.c_str(),0.5,0.7,0.05,1,0.0,42,1);
  ...
  outputname = Form("figures/%s/%s/pTstudy/AvgCos2PhiStarPhi_SE_Rebinned_%s_Order%d_Cent9.pdf",vmsa::mPID[pid].c_str(),vmsa::mBeamEnergy[energy].c_str(),vmsa::mBeamEnergy[energy].c_str(),order);
  output_start = Form("%s[",outputname.c_str());
  ...
  c_pT_rebin->Print(output_start.c_str());
    ...
    for(int i_pt = vmsa::pt_rebin_first[energy]; i_pt < vmsa::pt_rebin_last[energy]; i_pt++) // pt loop
    {
      ...
      c_pT_rebin->cd(i_pt+1)->SetTicks(1,1);
      c_pT_rebin->cd(i_pt+1)->SetGrid(0,0);
      string KEY_QA = Form("profile_pt_%d_Centrality_%d_Cos2PhiStarPhi_%s_Dca_%d_Sig_%d_%s_Norm_%d_Add_SE",i_pt,9,EP[order-1].c_str(),vmsa::Dca_start,vmsa::nSigKaon_start,vmsa::mPID[pid].c_str(),vmsa::Norm_QA);
      h_mMass_Add_SE[KEY_QA]->SetMarkerStyle(24);
      h_mMass_Add_SE[KEY_QA]->SetMarkerSize(1.2);
      h_mMass_Add_SE[KEY_QA]->SetLineColor(kGray+2);
      h_mMass_Add_SE[KEY_QA]->GetXaxis()->SetTitle("M(K^{+},K^{-}) (GeV)");
      h_mMass_Add_SE[KEY_QA]->GetYaxis()->SetTitle("<cos(2#phi*-2phi)>");
      //h_mMass[KEY_QA]->SetTitle(Form("%d/7<|cos(#theta*)|<%d/7",itheta,itheta+1));
      //h_mMass_Add_SE[KEY_QA]->GetYaxis()->SetRangeUser(0.9*h_mMass_Add_SE[KEY_QA]->GetMaximum(),h_mMass_Add_SE[KEY_QA]->GetMaximum()*1.05);
      h_mMass_Add_SE[KEY_QA]->DrawCopy("pE");
      // cout << "KEY_QA = " << KEY_QA.c_str() << endl;
  ...
  outputname = Form("figures/%s/%s/pTstudy/AvgCos2PhiStarPhi_ME_Rebinned_%s_Order%d_Cent9.pdf",vmsa::mPID[pid].c_str(),vmsa::mBeamEnergy[energy].c_str(),vmsa::mBeamEnergy[energy].c_str(),order);
  output_start = Form("%s[",outputname.c_str());
  ...
  c_pT_rebin->Print(output_start.c_str());
    ...
    for(int i_pt = vmsa::pt_rebin_first[energy]; i_pt < vmsa::pt_rebin_last[energy]; i_pt++) // pt loop
    {
      ...
      c_pT_rebin->cd(i_pt+1)->SetTicks(1,1);
      c_pT_rebin->cd(i_pt+1)->SetGrid(0,0);
      string KEY_QA = Form("profile_pt_%d_Centrality_%d_Cos2PhiStarPhi_%s_Dca_%d_Sig_%d_%s_Norm_%d_Add_ME",i_pt,9,EP[order-1].c_str(),vmsa::Dca_start,vmsa::nSigKaon_start,vmsa::mPID[pid].c_str(),vmsa::Norm_QA);
      h_mMass_Add_ME[KEY_QA]->SetMarkerStyle(24);
      h_mMass_Add_ME[KEY_QA]->SetMarkerSize(1.2);
      h_mMass_Add_ME[KEY_QA]->SetLineColor(kGray+2);
      h_mMass_Add_ME[KEY_QA]->GetXaxis()->SetTitle("M(K^{+},K^{-}) (GeV)");
      h_mMass_Add_ME[KEY_QA]->GetYaxis()->SetTitle("<cos(2#phi*-2phi)>");
      //h_mMass[KEY_QA]->SetTitle(Form("%d/7<|cos(#theta*)|<%d/7",itheta,itheta+1));
      //h_mMass_Add_ME[KEY_QA]->GetYaxis()->SetRangeUser(0.9*h_mMass_Add_ME[KEY_QA]->GetMaximum(),h_mMass_Add_ME[KEY_QA]->GetMaximum()*1.05);
      h_mMass_Add_ME[KEY_QA]->DrawCopy("pE");
      // cout << "KEY_QA = " << KEY_QA.c_str() << endl;
  ...
  //      	if(i_theta == vmsa::CTS_start) h_mMass_theta[KEY_theta] = (TProfile*)h_mMass[KEY]->Clone(KEY_theta.c_str());
...
//  }
  ...
  string outputfile = Form("../output/AuAu%s/%s/InvMassSubBg_%s_Cos2PhiStarPhi.root",vmsa::mBeamEnergy[energy].c_str(),vmsa::mPID[pid].c_str(),etamode.c_str());
  ...
  for(int i_pt = vmsa::pt_rebin_first[energy]; i_pt < vmsa::pt_rebin_last[energy]; i_pt++) // pt loop
  {
	...
	for(Int_t i_sig = vmsa::nSigKaon_start; i_sig < vmsa::nSigKaon_stop; i_sig++)
	{
	    ...
	    for(int i_norm = 0; i_norm < 1; ++i_norm)
	    {
	      string KEY = Form("profile_pt_%d_Centrality_%d_Cos2PhiStarPhi_%s_Dca_%d_Sig_%d_%s_Norm_%d",i_pt,i_cent,EP[order-1].c_str(),i_dca,i_sig,vmsa::mPID[pid].c_str(),i_norm);
	      h_mMass[KEY]->Write();
	      string KEY_Add_SE = Form("profile_pt_%d_Centrality_%d_Cos2PhiStarPhi_%s_Dca_%d_Sig_%d_%s_Norm_%d_Add_SE",i_pt,i_cent,EP[order-1].c_str(),i_dca,i_sig,vmsa::mPID[pid].c_str(),i_norm);
	      string KEY_Add_ME = Form("profile_pt_%d_Centrality_%d_Cos2PhiStarPhi_%s_Dca_%d_Sig_%d_%s_Norm_%d_Add_ME",i_pt,i_cent,EP[order-1].c_str(),i_dca,i_sig,vmsa::mPID[pid].c_str(),i_norm);
	      h_mMass_Add_SE[KEY_Add_SE]->SetName(KEY_Add_SE.c_str());
	      h_mMass_Add_SE[KEY_Add_SE]->Write();
	      h_mMass_Add_ME[KEY_Add_ME]->SetName(KEY_Add_ME.c_str());
	      h_mMass_Add_ME[KEY_Add_ME]->Write();
	    }
	}
      }
    }
