...
void calSysError_FixedSys_RawRes(Int_t energy = 4, Int_t pid = 0, Int_t i_cent = 9, string correction = "Raw", bool random3D = false, int order = 1, int defaultF = 0, string etamode = "eta1_eta1")//defaultF = 0 is BESII, defaultF = 1 is BESI
{
...
//  PlotLine(0.0,5.0,1.0/3.0,1.0/3.0,1,2,2);
  ...
  c_rho_SysError->SaveAs(Form("figures/%s/%s/pTstudy/finaloutputrho_Res%s_Order%d_%s.pdf",vmsa::mPID[pid].c_str(),vmsa::mBeamEnergy[energy].c_str(),correction.c_str(),order,etamode.c_str()));  
  c_rho_SysError->SaveAs(Form("figures/%s/%s/pTstudy/finaloutputrho_Res%s_Order%d_%s.eps",vmsa::mPID[pid].c_str(),vmsa::mBeamEnergy[energy].c_str(),correction.c_str(),order,etamode.c_str()));  
  c_rho_SysError->SaveAs(Form("figures/%s/%s/pTstudy/finaloutputrho_Res%s_Order%d_%s.png",vmsa::mPID[pid].c_str(),vmsa::mBeamEnergy[energy].c_str(),correction.c_str(),order,etamode.c_str()));  
  ...
  string OutPutFile = Form("../output/AuAu%s/%s/Rho_Res%sSysErrors_F_%d_%s.root",vmsa::mBeamEnergy[energy].c_str(),vmsa::mPID[pid].c_str(),correction.c_str(),defaultF,etamode.c_str());
  if(order == 1) OutPutFile = Form("../output/AuAu%s/%s/Rho_Res%sSysErrors_F_%d_%s_FirstOrder.root",vmsa::mBeamEnergy[energy].c_str(),vmsa::mPID[pid].c_str(),correction.c_str(),defaultF,etamode.c_str());
