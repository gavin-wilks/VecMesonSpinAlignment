...
void subBackGround_200GeV(int energy = 6, int pid = 0, int year = 0, string date = "20240827", bool random3D = false, int order = 2, string etamode = "eta1_eta1")
{
  ...
  //string InPutFile_ME = Form("../data/Yields_Phi_ME_%s_20241001_%s_nophipsi.root",vmsa::mBeamEnergy[energy].c_str(),etamode.c_str());
  ...
  for(Int_t i_pt = 0; i_pt < vmsa::pt_total; i_pt++) // pt bin 
  {
    for(Int_t i_cent = vmsa::Cent_start; i_cent < vmsa::Cent_stop; i_cent++) // centrality bin
    {
      ...
      {
	for(Int_t i_dca = vmsa::Dca_start; i_dca < /*vmsa::Dca_stop*/1; i_dca++)
	{
	  for(Int_t i_sig = vmsa::nSigKaon_start; i_sig < /*vmsa::nSigKaon_stop*/1; i_sig++)
	  {
  ...
  for(int i_cent = vmsa::Cent_start; i_cent < vmsa::Cent_stop; i_cent++) // Centrality loop
  {
	...
	{
          if( i_dca != 0 && i_sig != 0 ) continue;
	  for(int i_norm = vmsa::Norm_start; i_norm < /*vmsa::Norm_stop*/1; ++i_norm)
	  {
...
//#endif
  ...
  //string outputfile = Form("../output/AuAu%s/%s/InvMassSubBg_%s.root",vmsa::mBeamEnergy[energy].c_str(),vmsa::mPID[pid].c_str(),etamode.c_str());
  string outputfile = Form("../output/AuAu%s/%s/NoPhiPsi_InvMassSubBg_%s.root",vmsa::mBeamEnergy[energy].c_str(),vmsa::mPID[pid].c_str(),etamode.c_str());
