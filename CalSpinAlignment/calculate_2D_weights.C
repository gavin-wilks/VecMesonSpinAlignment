...
void calculate_2D_weights(int energy = 4, double inputv2 = 1) 
{
    ...
    const int nBinsPhi = 75;
    double ptMin = 1.2, ptMax = 4.2;
    double phiMin = 0, phiMax = 2 * TMath::Pi();

    double resolution[9] = {0.0};

    TH2F *h_flat_pt_phi[9];
    TH2F *h_desired_pt_phi[9];
    for(int icent = 0; icent < 9; icent++)
    {
      h_desired_pt_phi[icent] = new TH2F(Form("h_desired_pt_phi_cent%d",icent), Form("Desired pT-phi Distribution cent %d",icent), nBinsPt, ptMin, ptMax, nBinsPhi, phiMin, phiMax);
      h_flat_pt_phi[icent] = new TH2F(Form("h_flat_pt_phi_cent%d",icent), Form("Flat pT-phi Distribution cent %d",icent), nBinsPt, ptMin, ptMax, nBinsPhi, phiMin, phiMax);
    }
    ...
    for (int i = 0; i < numberOfEvents; ++i) 
    {
        ...
        double pt_true = desired_pt[cent]->GetRandom(); // Sample from the desired pT distribution
        double v2 = v2_func[cent]->Eval(pt_true); // Get v2 for the sampled pt
        azimuthal->ReleaseParameter(0);
        azimuthal->SetParameter(0,v2);
    ...
    c1->SaveAs("pt_phi_flat_v2_function.pdf");
