...
void calSpinAlignmentSysPhiPoly3MultipleBW(int energy = 4, int pid = 0, int year = 0, bool random3D = false, int order = 2, string etamode = "eta1p5_eta1p5")
{
  ...
  string inputfile = Form("../output/AuAu%s/%s/InvMassSubBg_%s.root",vmsa::mBeamEnergy[energy].c_str(),vmsa::mPID[pid].c_str(),etamode.c_str());
  if(order == 1) inputfile = Form("../output/AuAu%s/%s/InvMassSubBg_Psi1.root",vmsa::mBeamEnergy[energy].c_str(),vmsa::mPID[pid].c_str());
  ...
  for(int i_pt = vmsa::pt_rebin_first[energy]; i_pt < vmsa::pt_rebin_last[energy]; i_pt++) // pt loop
  {
	...
	for(Int_t i_sig = vmsa::nSigKaon_start; i_sig < vmsa::nSigKaon_stop; i_sig++)
	{
	  ...
	  for(int i_norm = vmsa::Norm_start; i_norm < vmsa::Norm_stop; ++i_norm)
	  {
	    ...
	    TF1 *f_bw = new TF1("f_bw",Poly3BreitWignerMultiple,vmsa::BW_Start[pid],vmsa::BW_Stop[pid],13);
	    f_bw->SetParameter(0,vmsa::InvMass[pid]);
	    f_bw->SetParLimits(0,vmsa::InvMass[pid]-0.002,vmsa::InvMass[pid]+0.002);
	    f_bw->SetParameter(1,vmsa::Width[pid]);
	    f_bw->SetParameter(2,1.0);
	    f_bw->SetParameter(7,0.9875);
	    f_bw->SetParLimits(7,0.9875-0.003,0.9875+0.003);
	    f_bw->SetParameter(8,0.004);
	    f_bw->SetParameter(9,1.0);
	    f_bw->SetParameter(10,1.005);
	    f_bw->SetParLimits(10,1.005-0.003,1005+0.003);
	    f_bw->SetParameter(11,0.004);
	    f_bw->SetParameter(12,1.0);
	    ...
	    Par_InteTheta[KEY_InteTheta].push_back(static_cast<float>(f_bw->GetParameter(7)));
	    Par_InteTheta[KEY_InteTheta].push_back(static_cast<float>(f_bw->GetParameter(8)));
	    Par_InteTheta[KEY_InteTheta].push_back(static_cast<float>(f_bw->GetParameter(9)));
	    Par_InteTheta[KEY_InteTheta].push_back(static_cast<float>(f_bw->GetParameter(10)));
	    Par_InteTheta[KEY_InteTheta].push_back(static_cast<float>(f_bw->GetParameter(11)));
	    Par_InteTheta[KEY_InteTheta].push_back(static_cast<float>(f_bw->GetParameter(12)));
	  }
	}
      }
    }
	...
	for(int i_norm = vmsa::Norm_start; i_norm < vmsa::Norm_stop; ++i_norm)
	{
	  for(int i_sigma = vmsa::Sig_start; i_sigma < vmsa::Sig_stop; ++i_sigma)
	  {
		...
		for(int i_theta = vmsa::CTS_start; i_theta < vmsa::CTS_stop; ++i_theta) // cos(theta*) loop
		{
		  ...
		  f_bw->FixParameter(7,Par_InteTheta[KEY_InteTheta][7]);
		  f_bw->FixParameter(8,Par_InteTheta[KEY_InteTheta][8]);
		  f_bw->SetParameter(9,Par_InteTheta[KEY_InteTheta][9]/7.0);
		  f_bw->FixParameter(10,Par_InteTheta[KEY_InteTheta][10]);
		  f_bw->FixParameter(11,Par_InteTheta[KEY_InteTheta][11]);
		  f_bw->SetParameter(12,Par_InteTheta[KEY_InteTheta][12]/7.0);
		  //f_bw->SetParameter(5,Par_InteTheta[KEY_InteTheta][5]);
		  f_bw->SetRange(vmsa::BW_Start[pid],vmsa::BW_Stop[pid]);
	          f_bw->SetParLimits(7,0.9875-0.003,0.9875+0.003);
	          f_bw->SetParLimits(10,1.005-0.003,1.005+0.003);
		  TFitResultPtr result = h_mMass[KEY]->Fit(f_bw,"MQNRS");

                  TF1 *f_bg = new TF1("f_bg",Poly3MultipleBW,vmsa::BW_Start[pid],vmsa::BW_Stop[pid],10);
                  ...
                  f_bg->FixParameter(4,f_bw->GetParameter(7));
                  f_bg->FixParameter(5,f_bw->GetParameter(8));
                  f_bg->SetParameter(6,f_bw->GetParameter(9));
                  f_bg->FixParameter(7,f_bw->GetParameter(10));
                  f_bg->FixParameter(8,f_bw->GetParameter(11));
                  f_bg->SetParameter(9,f_bw->GetParameter(12));
                  ...
                  f_bg->SetParError(6,f_bw->GetParError(9));
                  f_bg->SetParError(9,f_bw->GetParError(12));

                  //double params[2] = {result->GetParams()[3],result->GetParams()[4]};//,result->GetParams()[5]};
                  //double params[3] = {result->GetParams()[3],result->GetParams()[4],result->GetParams()[5]};
                  double params[10] = {result->GetParams()[3],
                                       result->GetParams()[4],
                                       result->GetParams()[5],
                                       result->GetParams()[6],
                                       result->GetParams()[7],
                                       ...
                                       result->GetParams()[11],
                                       result->GetParams()[12]};
                  TMatrixDSym covArr(10); 
                  result->GetCovarianceMatrix().GetSub(3,12,3,12,covArr);
                  ...
                  //covArr(0,4) = result->GetCovarianceMatrix()(3,7);
                  //covArr(0,5) = result->GetCovarianceMatrix()(3,8);
                  //covArr(0,6) = result->GetCovarianceMatrix()(3,9);
                  //covArr(0,7) = result->GetCovarianceMatrix()(3,10);
                  //covArr(0,8) = result->GetCovarianceMatrix()(3,11);
                  //covArr(0,9) = result->GetCovarianceMatrix()(3,12);
		  ...
		  Par[KEY].push_back(static_cast<float>(f_bw->GetParameter(7)));
		  Par[KEY].push_back(static_cast<float>(f_bw->GetParameter(8)));
		  Par[KEY].push_back(static_cast<float>(f_bw->GetParameter(9)));
		  Par[KEY].push_back(static_cast<float>(f_bw->GetParameter(10)));
		  Par[KEY].push_back(static_cast<float>(f_bw->GetParameter(11)));
		  Par[KEY].push_back(static_cast<float>(f_bw->GetParameter(12)));
		}
...
#if _PlotQA_
  for(int i_cent = vmsa::Cent_start; i_cent < 10; ++i_cent) // Centrality loop
  {
    string outputname = Form("./figures/%s/%s/allThetaYields%s_cent%d_%s.pdf",vmsa::mPID[pid].c_str(),vmsa::mBeamEnergy[energy].c_str(),vmsa::mPID[pid].c_str(),i_cent,etamode.c_str());
    ...
    for(int i_pt = vmsa::pt_rebin_first[energy]; i_pt < vmsa::pt_rebin_last[energy]; ++i_pt) // pt loop
    {
      ...
      for(int i_theta = 0; i_theta < 9; ++i_theta)
      {
        ...
        if(i_theta < vmsa::CTS_stop)
        {
          string KEY_QA = Form("pt_%d_Centrality_%d_CosThetaStar_%d_2nd_Dca_%d_Sig_%d_%s_Norm_%d",i_pt,i_cent,i_theta,vmsa::Dca_start,1/*vmsa::nSigKaon_start*/,vmsa::mPID[pid].c_str(),vmsa::Norm_QA);
          h_mMass[KEY_QA]->SetTitle("");
          h_mMass[KEY_QA]->GetXaxis()->SetNdivisions(505,'N');
          h_mMass[KEY_QA]->GetXaxis()->SetLabelSize(0.03);
          h_mMass[KEY_QA]->GetXaxis()->SetTitle("M(K^{+},K^{-})");
          h_mMass[KEY_QA]->SetTitle(Form("%.2f < p_{T} < %.2f",vmsa::pt_low[energy][i_pt],vmsa::pt_up[energy][i_pt]));
          h_mMass[KEY_QA]->GetXaxis()->SetTitleSize(0.05);
          h_mMass[KEY_QA]->GetXaxis()->SetTitleOffset(1.2);
          h_mMass[KEY_QA]->GetXaxis()->CenterTitle();
          ...
          string KEY_InteTheta = Form("pt_%d_Centrality_%d_2nd_Dca_%d_Sig_%d_%s_Norm_%d",i_pt,i_cent,vmsa::Dca_start,1/*vmsa::nSigKaon_start*/,vmsa::mPID[pid].c_str(),vmsa::Norm_QA);
        ...
        if(i_theta == vmsa::CTS_stop)
        {
          string KEY_InteTheta_QA = Form("pt_%d_Centrality_%d_2nd_Dca_%d_Sig_%d_%s_Norm_%d",i_pt,i_cent,vmsa::Dca_start,1/*vmsa::nSigKaon_start*/,vmsa::mPID[pid].c_str(),vmsa::Norm_QA);
          h_mMass_InteTheta[KEY_InteTheta_QA]->SetTitle("");
          h_mMass_InteTheta[KEY_InteTheta_QA]->GetXaxis()->SetNdivisions(505,'N');
          h_mMass_InteTheta[KEY_InteTheta_QA]->GetXaxis()->SetLabelSize(0.03);
          h_mMass_InteTheta[KEY_InteTheta_QA]->GetXaxis()->SetTitle("M(K^{+},K^{-})");
          ...
          f_bw->SetParameter(7,Par_InteTheta[KEY_InteTheta_QA][7]);
          f_bw->SetParameter(8,Par_InteTheta[KEY_InteTheta_QA][8]);
          f_bw->SetParameter(9,Par_InteTheta[KEY_InteTheta_QA][9]);
          f_bw->SetParameter(10,Par_InteTheta[KEY_InteTheta_QA][10]);
          f_bw->SetParameter(11,Par_InteTheta[KEY_InteTheta_QA][11]);
          f_bw->SetParameter(12,Par_InteTheta[KEY_InteTheta_QA][12]);
          f_bw->SetLineColor(2);
          f_bw->SetLineStyle(2);
          f_bw->SetLineWidth(2);
          f_bw->Draw("l same");
      ...
      for(int i_theta = vmsa::CTS_start; i_theta < vmsa::CTS_stop; ++i_theta)
      {
        ...
        f_bw->SetParameter(7,Par[KEY_QA][7]);
        f_bw->SetParameter(8,Par[KEY_QA][8]);
        f_bw->SetParameter(9,Par[KEY_QA][9]);
        f_bw->SetParameter(10,Par[KEY_QA][10]);
        f_bw->SetParameter(11,Par[KEY_QA][11]);
        f_bw->SetParameter(12,Par[KEY_QA][12]);
        f_bw->SetLineColor(4);
        f_bw->SetLineStyle(2);
        f_bw->SetLineWidth(2);
        f_bw->Draw("l same");
        ...
        f_bg->SetParameter(4,Par[KEY_QA][7]);
        f_bg->SetParameter(5,Par[KEY_QA][8]);
        f_bg->SetParameter(6,Par[KEY_QA][9]);
        f_bg->SetParameter(7,Par[KEY_QA][10]);
        f_bg->SetParameter(8,Par[KEY_QA][11]);
        f_bg->SetParameter(9,Par[KEY_QA][12]);
        f_bg->SetLineColor(4);
        f_bg->SetLineStyle(2);
        f_bg->SetLineWidth(2);
        f_bg->Draw("l same");
      ...
      for(int i_sigma = vmsa::Sig_start; i_sigma < vmsa::Sig_stop; ++i_sigma)
      {
        for(int i_method = vmsa::Method_start; i_method < vmsa::Method_stop; ++i_method)
        {
          string KEY_counts_QA = Form("pt_%d_Centrality_%d_2nd_Dca_%d_Sig_%d_%s_Norm_%d_Sigma_%d_%s",i_pt,i_cent,vmsa::Dca_start,1/*vmsa::nSigKaon_start*/,vmsa::mPID[pid].c_str(),vmsa::Norm_QA,i_sigma,vmsa::mInteMethod[i_method].c_str());
 ...
 // {
	...
	for(int i_norm = vmsa::Norm_start; i_norm < vmsa::Norm_stop; ++i_norm)
	{
	  for(int i_sigma = vmsa::Sig_start; i_sigma < vmsa::Sig_stop; ++i_sigma)
	  {
	    for(int i_method = vmsa::Method_start; i_method < vmsa::Method_stop; ++i_method)
	    {
	      string KEY_rho = Form("rhoRaw_Centrality_%d_2nd_Dca_%d_Sig_%d_%s_Norm_%d_Sigma_%d_%s",9,i_dca,i_sig,vmsa::mPID[pid].c_str(),i_norm,i_sigma,vmsa::mInteMethod[i_method].c_str());
  ...
  string outputfile = Form("../output/AuAu%s/%s/RawPhiPtSys_%s.root",vmsa::mBeamEnergy[energy].c_str(),vmsa::mPID[pid].c_str(),etamode.c_str());
  if(order == 1) outputfile = Form("../output/AuAu%s/%s/RawPhiPtSys_Psi1.root",vmsa::mBeamEnergy[energy].c_str(),vmsa::mPID[pid].c_str());
