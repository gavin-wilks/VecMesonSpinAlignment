...
void subBackGroundPhiCent_PhiStar(int energy = 4, int pid = 0, int year = 0, string date = "20240216", bool random3D = false, int order = 2, std::string etamode = "eta1_eta1")
{
  ...
  string KEY_SE_QA = Form("pt_%d_Centrality_%d_Cos2PhiStarPhi_%d_%s_Dca_%d_Sig_%d_%s_Norm_%d_SE",vmsa::pt_RawQA[energy],vmsa::centStart,vmsa::CTS_start,EP[order-1].c_str(),vmsa::Dca_start,vmsa::nSigKaon_start,vmsa::mPID[pid].c_str(),vmsa::Norm_QA);
  ...
  string KEY_ME_QA = Form("pt_%d_Centrality_%d_Cos2PhiStarPhi_%d_%s_Dca_%d_Sig_%d_%s_Norm_%d_ME",vmsa::pt_RawQA[energy],vmsa::centStart,vmsa::CTS_start,EP[order-1].c_str(),vmsa::Dca_start,vmsa::nSigKaon_start,vmsa::mPID[pid].c_str(),vmsa::Norm_QA);
  ...
  string KEY_SM_QA = Form("pt_%d_Centrality_%d_Cos2PhiStarPhi_%d_%s_Dca_%d_Sig_%d_%s_Norm_%d_SM",vmsa::pt_RawQA[energy],vmsa::centStart,vmsa::CTS_start,EP[order-1].c_str(),vmsa::Dca_start,vmsa::nSigKaon_start,vmsa::mPID[pid].c_str(),vmsa::Norm_QA);
  ...
  string outputname = Form("figures/%s/%s/centralitystudy/Cos2PhiStarPhi_InvMassDistributions_%s_Order%d.pdf",vmsa::mPID[pid].c_str(),vmsa::mBeamEnergy[energy].c_str(),vmsa::mBeamEnergy[energy].c_str(),order);
  ...
  for(int itheta = 0; itheta < 10; itheta++)
  {
    for(int icent = 0; icent < 9; icent++)
    {
      for(int i_pt = vmsa::pt_start; i_pt < vmsa::pt_stop; i_pt++) // pt loop
      {
        ...
        string KEY_SE_QA = Form("pt_%d_Centrality_%d_Cos2PhiStarPhi_%d_%s_Dca_%d_Sig_%d_%s_Norm_%d_SE",i_pt,icent,itheta,EP[order-1].c_str(),vmsa::Dca_start,vmsa::nSigKaon_start,vmsa::mPID[pid].c_str(),vmsa::Norm_QA);
        //cout << KEY_SE_QA << endl;
        h_mMass_SE[KEY_SE_QA]->GetYaxis()->SetRangeUser(-0.1*h_mMass_SE[KEY_SE_QA]->GetMaximum(),1.1*h_mMass_SE[KEY_SE_QA]->GetMaximum());
        h_mMass_SE[KEY_SE_QA]->SetTitle(Form("Cent %d, %d/5<|cos(2#phi*-2#phi)|<%d/5",icent,itheta-5,itheta-4));
        h_mMass_SE[KEY_SE_QA]->DrawCopy();

        string KEY_ME_QA = Form("pt_%d_Centrality_%d_Cos2PhiStarPhi_%d_%s_Dca_%d_Sig_%d_%s_Norm_%d_ME",i_pt,icent,itheta,EP[order-1].c_str(),vmsa::Dca_start,vmsa::nSigKaon_start,vmsa::mPID[pid].c_str(),vmsa::Norm_QA);
        //cout << KEY_ME_QA << endl;
        h_mMass_ME[KEY_ME_QA]->SetLineColor(2);
        h_mMass_ME[KEY_ME_QA]->SetFillColor(2);
        h_mMass_ME[KEY_ME_QA]->SetFillStyle(3002);
        ...
        string KEY_SM_QA = Form("pt_%d_Centrality_%d_Cos2PhiStarPhi_%d_%s_Dca_%d_Sig_%d_%s_Norm_%d_SM",i_pt,icent,itheta,EP[order-1].c_str(),vmsa::Dca_start,vmsa::nSigKaon_start,vmsa::mPID[pid].c_str(),vmsa::Norm_QA);
        //cout << KEY_SM_QA << endl;
        h_mMass_SM[KEY_SM_QA]->SetLineColor(4);
        h_mMass_SM[KEY_SM_QA]->SetFillColor(4);
        h_mMass_SM[KEY_SM_QA]->SetFillStyle(3004);
  ...
  outputname = Form("figures/%s/%s/centralitystudy/Cos2PhiStarPhi_InvMassBackgroundSubtracted_Rebinned_%s_Order%d.pdf",vmsa::mPID[pid].c_str(),vmsa::mBeamEnergy[energy].c_str(),vmsa::mBeamEnergy[energy].c_str(),order);
  ...
  for(int itheta = 0; itheta < 10; itheta++)
  {
    for(int icent = 0; icent < 9; icent++)
    {
      for(int i_pt = vmsa::pt_rebin_first_cent[energy]; i_pt <= vmsa::pt_rebin_last_cent[energy]; i_pt++) // pt loop
      {
        ...
        string KEY_QA = Form("pt_%d_Centrality_%d_Cos2PhiStarPhi_%d_%s_Dca_%d_Sig_%d_%s_Norm_%d",i_pt,icent,itheta,EP[order-1].c_str(),vmsa::Dca_start,vmsa::nSigKaon_start,vmsa::mPID[pid].c_str(),vmsa::Norm_QA);
        //cout << KEY_QA << endl;
    ...
    for(Int_t i_dca = vmsa::Dca_start; i_dca < vmsa::Dca_stop; i_dca++)
    {
      ...
      { 
        ...
        for(int i_norm = vmsa::Norm_start; i_norm < vmsa::Norm_stop; ++i_norm)
        {
          ...
          {
            string KEY = Form("Centrality_%d_Cos2PhiStarPhi_%d_2nd_Dca_%d_Sig_%d_%s_Norm_%d",i_cent,i_theta,i_dca,i_sig,vmsa::mPID[pid].c_str(),i_norm);
...
*/
  ...
  for(int i_pt = vmsa::pt_rebin_first[energy]; i_pt < vmsa::pt_rebin_last[energy]; i_pt++) // pt loop
  {
   ...
   /* TCanvas *c_mMass_phi_diff = new TCanvas("c_mMass_phi_diff","c_mMass_phi_diff",10,10,1400,1400);
    ...
    for(int i_pt = vmsa::pt_rebin_first[energy]; i_pt < vmsa::pt_rebin_last[energy]; i_pt++) // pt loop
    {
      ...
      string KEY_QA = Form("pt_%d_Centrality_%d_Cos2PhiStarPhi_%d_2nd_Dca_%d_Sig_%d_%s_Norm_%d",i_pt,vmsa::centStart,vmsa::CTS_start,vmsa::Dca_start,vmsa::nSigKaon_start,vmsa::mPID[pid].c_str(),vmsa::Norm_QA);
  ...
  string outputfile = Form("../output/AuAu%s/%s/Cos2PhiStarPhi_InvMassSubBgCent_%s.root",vmsa::mBeamEnergy[energy].c_str(),vmsa::mPID[pid].c_str(),etamode.c_str());
  if(order == 1) outputfile = Form("../output/AuAu%s/%s/Cos2PhiStarPhi_InvMassSubBgCent_%s_FirstOrder.root",vmsa::mBeamEnergy[energy].c_str(),vmsa::mPID[pid].c_str(),etamode.c_str());
  ...
  File_OutPut->Close();
  cout << "Finished and closed file" << endl;
}
