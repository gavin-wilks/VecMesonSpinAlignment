...
void subBackGroundP_Helicity_2D_OffDiag(int energy = 4, int pid = 0, int year = 0, string date = "20240722", bool random3D = false, int order = 2, string etamode = "eta1_eta1")
{
  ...
  string InPutFile_SE = Form("../data/Yields_Phi_SE_%s_%s_%s_Helicity_2D.root",vmsa::mBeamEnergy[energy].c_str(),date.c_str(),etamode.c_str());
  ...
  string InPutFile_ME = Form("../data/Yields_Phi_ME_%s_%s_%s_Helicity_2D.root",vmsa::mBeamEnergy[energy].c_str(),date.c_str(),etamode.c_str());
  ...
  for(Int_t i_pt = 0; i_pt < vmsa::p_total; i_pt++) // pt bin 
  {
    for(Int_t i_cent = 9/*vmsa::Cent_start*/; i_cent < vmsa::Cent_stop; i_cent++) // centrality bin
    {
	  ...
	  for(Int_t i_dca = vmsa::Dca_start; i_dca < vmsa::Dca_stop; i_dca++)
	  {
	    for(Int_t i_sig = vmsa::nSigKaon_start; i_sig < vmsa::nSigKaon_stop; i_sig++)
	    {
	      ...
	      string KEY_InPutSE = Form("p_%d_Centrality_%d_CosThetaStar_%d_Phi_%d_%s_Dca_%d_Sig_%d_%s_SE",i_pt,i_cent,i_thetah,i_phi,EP[order-1].c_str(),i_dca,i_sig,vmsa::mPID[pid].c_str());
              cout << KEY_InPutSE << endl;
              h_mInPut_SE[KEY_InPutSE] = (TH1F*)File_SE->Get(KEY_InPutSE.c_str())->Clone(); 
              h_mInPut_SE[KEY_InPutSE]->Rebin(4);

	      string KEY_InPutME = Form("p_%d_Centrality_%d_CosThetaStar_%d_Phi_%d_%s_Dca_%d_Sig_%d_%s_ME",i_pt,i_cent,i_thetah,i_phi,EP[order-1].c_str(),i_dca,i_sig,vmsa::mPID[pid].c_str());
	      ...
	      {
	        string KEY_SE = Form("p_%d_Centrality_%d_CosThetaStar_%d_Phi_%d_%s_Dca_%d_Sig_%d_%s_Norm_%d_SE",i_pt,i_cent,i_thetah,i_phi,EP[order-1].c_str(),i_dca,i_sig,vmsa::mPID[pid].c_str(),i_norm);
	        h_mMass_SE[KEY_SE] = (TH1F*)h_mInPut_SE[KEY_InPutSE]->Clone(KEY_SE.c_str());
	        string KEY_ME = Form("p_%d_Centrality_%d_CosThetaStar_%d_Phi_%d_%s_Dca_%d_Sig_%d_%s_Norm_%d_ME",i_pt,i_cent,i_thetah,i_phi,EP[order-1].c_str(),i_dca,i_sig,vmsa::mPID[pid].c_str(),i_norm);
	        h_mMass_ME[KEY_ME] = (TH1F*)h_mInPut_ME[KEY_InPutME]->Clone(KEY_ME.c_str());
	        string KEY_SM = Form("p_%d_Centrality_%d_CosThetaStar_%d_Phi_%d_%s_Dca_%d_Sig_%d_%s_Norm_%d_SM",i_pt,i_cent,i_thetah,i_phi,EP[order-1].c_str(),i_dca,i_sig,vmsa::mPID[pid].c_str(),i_norm);
  ...
  for(int i_pt = 0; i_pt < vmsa::p_total; i_pt++)
  {
    ...
    string KEY_SE_QA = Form("p_%d_Centrality_%d_CosThetaStar_%d_Phi_%d_%s_Dca_%d_Sig_%d_%s_Norm_%d_SE",i_pt,9,vmsa::CTS_start,0,EP[order-1].c_str(),vmsa::Dca_start,vmsa::nSigKaon_start,vmsa::mPID[pid].c_str(),vmsa::Norm_QA);
    ...
    string KEY_ME_QA = Form("p_%d_Centrality_%d_CosThetaStar_%d_Phi_%d_%s_Dca_%d_Sig_%d_%s_Norm_%d_ME",i_pt,9,vmsa::CTS_start,0,EP[order-1].c_str(),vmsa::Dca_start,vmsa::nSigKaon_start,vmsa::mPID[pid].c_str(),vmsa::Norm_QA);
    ...
    string KEY_SM_QA = Form("p_%d_Centrality_%d_CosThetaStar_%d_Phi_%d_%s_Dca_%d_Sig_%d_%s_Norm_%d_SM",i_pt,9,vmsa::CTS_start,0,EP[order-1].c_str(),vmsa::Dca_start,vmsa::nSigKaon_start,vmsa::mPID[pid].c_str(),vmsa::Norm_QA);
    ...
    c_peak->SaveAs(Form("figures/%s/%s/pTstudy/2DHelicityOffDiag_CloseUpMixedEvent_%s_Order%d_Cent0_pt%d.pdf",vmsa::mPID[pid].c_str(),vmsa::mBeamEnergy[energy].c_str(),vmsa::mBeamEnergy[energy].c_str(),order,i_pt));
  }
  ...
  string outputname = Form("figures/%s/%s/pTstudy/2DHelicityOffDiag_InvMassDistributions_%s_Order%d_Cent9.pdf",vmsa::mPID[pid].c_str(),vmsa::mBeamEnergy[energy].c_str(),vmsa::mBeamEnergy[energy].c_str(),order);
  ...
  for(int iphi = 0; iphi < 12; iphi++)
  {
    ...
    {
      for(int i_pt = vmsa::p_start; i_pt < vmsa::p_stop; i_pt++) // pt loop
      {
        c_pT->cd(i_pt+1);
        ...
        string KEY_SE_QA = Form("p_%d_Centrality_%d_CosThetaStar_%d_Phi_%d_%s_Dca_%d_Sig_%d_%s_Norm_%d_SE",i_pt,9,ithetah,iphi,EP[order-1].c_str(),vmsa::Dca_start,vmsa::nSigKaon_start,vmsa::mPID[pid].c_str(),vmsa::Norm_QA);
        h_mMass_SE[KEY_SE_QA]->GetYaxis()->SetRangeUser(-0.1*h_mMass_SE[KEY_SE_QA]->GetMaximum(),1.1*h_mMass_SE[KEY_SE_QA]->GetMaximum());
        ...
        string KEY_ME_QA = Form("p_%d_Centrality_%d_CosThetaStar_%d_Phi_%d_%s_Dca_%d_Sig_%d_%s_Norm_%d_ME",i_pt,9,ithetah,iphi,EP[order-1].c_str(),vmsa::Dca_start,vmsa::nSigKaon_start,vmsa::mPID[pid].c_str(),vmsa::Norm_QA);
        h_mMass_ME[KEY_ME_QA]->SetLineColor(2);
        h_mMass_ME[KEY_ME_QA]->SetFillColor(2);
        h_mMass_ME[KEY_ME_QA]->SetFillStyle(3002);
        h_mMass_ME[KEY_ME_QA]->DrawCopy("h same");

        string KEY_SM_QA = Form("p_%d_Centrality_%d_CosThetaStar_%d_Phi_%d_%s_Dca_%d_Sig_%d_%s_Norm_%d_SM",i_pt,9,ithetah,iphi,EP[order-1].c_str(),vmsa::Dca_start,vmsa::nSigKaon_start,vmsa::mPID[pid].c_str(),vmsa::Norm_QA);
        h_mMass_SM[KEY_SM_QA]->SetLineColor(4);
        h_mMass_SM[KEY_SM_QA]->SetFillColor(4);
        h_mMass_SM[KEY_SM_QA]->SetFillStyle(3004);
        h_mMass_SM[KEY_SM_QA]->DrawCopy("h same");
      ...
      for(int i_thetah = vmsa::CTS_start; i_thetah < 9/*2*vmsa::CTS_stop*/; i_thetah++) // cos(theta*) loop
      {
        for(Int_t i_dca = vmsa::Dca_start; i_dca < vmsa::Dca_stop; i_dca++)
        {
          for(Int_t i_sig = vmsa::nSigKaon_start; i_sig < vmsa::nSigKaon_stop; i_sig++)
          {
            ...
            for(int i_norm = vmsa::Norm_start; i_norm < vmsa::Norm_stop; ++i_norm)
            {
              for(int p_bin = vmsa::p_rebin_first[energy]; p_bin < vmsa::p_rebin_last[energy]; p_bin++) // pt loop
              {
                string KEY = Form("p_%d_Centrality_%d_CosThetaStar_%d_Phi_%d_%s_Dca_%d_Sig_%d_%s_Norm_%d",p_bin,i_cent,i_thetah,i_phi,EP[order-1].c_str(),i_dca,i_sig,vmsa::mPID[pid].c_str(),i_norm);
                for(int i_pt = vmsa::p_rebin_start[energy][p_bin]; i_pt <= vmsa::p_rebin_stop[energy][p_bin]; i_pt++)
                {
          	string KEY_SM = Form("p_%d_Centrality_%d_CosThetaStar_%d_Phi_%d_%s_Dca_%d_Sig_%d_%s_Norm_%d_SM",i_pt,i_cent,i_thetah,i_phi,EP[order-1].c_str(),i_dca,i_sig,vmsa::mPID[pid].c_str(),i_norm);
          	// cout << "KEY= " << KEY.c_str() << ", KEY_SM = " << KEY_SM.c_str() << endl;
          	if(i_pt == vmsa::p_rebin_start[energy][p_bin])
          	{
  ...
  outputname = Form("figures/%s/%s/pTstudy/2DHelicityOffDiag_InvMassBackgroundSubtracted_Rebinned_%s_Order%d_Cent9.pdf",vmsa::mPID[pid].c_str(),vmsa::mBeamEnergy[energy].c_str(),vmsa::mBeamEnergy[energy].c_str(),order);
  ...
  for(int iphi = 0; iphi < 12/*2*vmsa::CTS_total*/; iphi++)
  {
    for(int ithetah = 0; ithetah < 9/*2*vmsa::CTS_total*/; ithetah++)
    {
      ...
      {
        c_pT_rebin->cd(vmsa::p_rebin_start[energy][i_pt]+1);
        c_pT_rebin->cd(vmsa::p_rebin_start[energy][i_pt]+1)->SetLeftMargin(0.15);
        c_pT_rebin->cd(vmsa::p_rebin_start[energy][i_pt]+1)->SetBottomMargin(0.15);
        c_pT_rebin->cd(vmsa::p_rebin_start[energy][i_pt]+1)->SetTicks(1,1);
        c_pT_rebin->cd(vmsa::p_rebin_start[energy][i_pt]+1)->SetGrid(0,0);
        string KEY_QA = Form("p_%d_Centrality_%d_CosThetaStar_%d_Phi_%d_%s_Dca_%d_Sig_%d_%s_Norm_%d",i_pt,9,ithetah,iphi,EP[order-1].c_str(),vmsa::Dca_start,vmsa::nSigKaon_start,vmsa::mPID[pid].c_str(),vmsa::Norm_QA);
...
//	      for(int i_theta = vmsa::CTS_start; i_theta < 2*vmsa::CTS_stop; i_theta++) // cos(theta*) loop
//	      {
//		string KEY = Form("p_%d_Centrality_%d_CosThetaStar_%d_%s_Dca_%d_Sig_%d_%s_Norm_%d",i_pt,i_cent,i_theta,EP[order-1].c_str(),i_dca,i_sig,vmsa::mPID[pid].c_str(),i_norm);
...
//      string KEY_theta_QA = Form("p_%d_Centrality_%d_%s_Dca_%d_Sig_%d_%s_Norm_%d",i_pt,vmsa::Cent_start,EP[order-1].c_str(),vmsa::Dca_start,vmsa::nSigKaon_start,vmsa::mPID[pid].c_str(),vmsa::Norm_QA);
...
//#endif
	...
	for(Int_t i_dca = vmsa::Dca_start; i_dca < vmsa::Dca_stop; i_dca++)
	{
	  for(Int_t i_sig = vmsa::nSigKaon_start; i_sig < vmsa::nSigKaon_stop; i_sig++)
	  {
		...
		string KEY = Form("p_%d_Centrality_%d_CosThetaStar_%d_2nd_Dca_%d_Sig_%d_%s_Norm_%d",i_pt,i_cent,i_theta,i_dca,i_sig,vmsa::mPID[pid].c_str(),i_norm);
...
//      string KEY_QA = Form("p_%d_Centrality_%d_CosThetaStar_%d_%s_Dca_%d_Sig_%d_%s_Norm_%d",i_pt,vmsa::Cent_start,vmsa::CTS_start,EP[order-1].c_str(),vmsa::Dca_start,vmsa::nSigKaon_start,vmsa::mPID[pid].c_str(),vmsa::Norm_QA);
...
//#endif
 // }
  ...
  string outputfile = Form("../output/AuAu%s/%s/Helicity2DP_InvMassSubBg_%s.root",vmsa::mBeamEnergy[energy].c_str(),vmsa::mPID[pid].c_str(),etamode.c_str());
  //string outputfile = Form("../output/AuAu%s/%s/InvMassSubBg.root",vmsa::mBeamEnergy[energy].c_str(),vmsa::mPID[pid].c_str());
  if(order == 1) outputfile = Form("../output/AuAu%s/%s/Helicity2DP_InvMassSubBg_%s_FirstOrder.root",vmsa::mBeamEnergy[energy].c_str(),vmsa::mPID[pid].c_str(),etamode.c_str());
	...
	for(Int_t i_sig = vmsa::nSigKaon_start; i_sig < vmsa::nSigKaon_stop; i_sig++)
	{
	  ...
	  for(int i_phi = vmsa::CTS_start; i_phi < 12/*2*vmsa::CTS_stop*/; i_phi++) // cos(theta*) loop
	  {
	    for(int i_thetah = vmsa::CTS_start; i_thetah < 9/*2*vmsa::CTS_stop*/; i_thetah++) // cos(theta*) loop
	    {
	      for(int i_norm = vmsa::Norm_start; i_norm < vmsa::Norm_stop; ++i_norm)
	      {
	        string KEY = Form("p_%d_Centrality_%d_CosThetaStar_%d_Phi_%d_%s_Dca_%d_Sig_%d_%s_Norm_%d",i_pt,i_cent,i_thetah,i_phi,EP[order-1].c_str(),i_dca,i_sig,vmsa::mPID[pid].c_str(),i_norm);
	        h_mMass[KEY]->Write();
	      }
	    }
          }
