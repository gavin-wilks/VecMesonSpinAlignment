...
void plotKaonTTrees_Compare_TPCOnly(int energy = 4, bool datarcweight = false)
{
    gStyle->SetOptStat(0);
    string folderopt = "Eta1p5_Retry_70Vz_25PhiPsi_FinerPID/";
...
//    mKaonTreeEmbed->SetBranchAddress("passns", &mPassNsig);
    ...
    string KEY = Form("ptyspectra_Centrality_%d_%s_Dca_%d_Sig_%d_%s_Norm_%d_Sigma_%d_%s_Poly%d",9,"2nd",0,0,vmsa::mPID[0].c_str(),0,0,vmsa::mInteMethod[0].c_str(),1);
    ...
    for (Long64_t i = 0; i < nentriesEmbed; i++)
    {
      ...
      if(mRcExists && mPassTpc && mPassNsig)    
      {
    ...
    for (Long64_t i = 0; i < nentriesEmbed; i++)
    {
      ...
      if(mRcExists && mPassTpc && mPassNsig)    
      {
        ...
        if(mRcMass > 1.024461 || mRcMass < 1.014461) continue;
        ...
        ptyphibasic[3][0]->Fill(mRcPtP,mRcRapidityP,mRcPhiP,mWeight*PhiWeight*DataRcWeightVal*pythiaweight);
        ptyphibasic[3][1]->Fill(mRcPtM,mRcRapidityM,mRcPhiM,mWeight*PhiWeight*DataRcWeightVal*pythiaweight);
