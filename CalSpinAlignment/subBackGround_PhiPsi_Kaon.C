...
void subBackGround_PhiPsi_Kaon(int energy = 4, int pid = 0, int year = 0, string date = "20240326", bool random3D = false, int order = 2, string etamode = "eta1_eta1")
{
  std::string EP[2] = {"","2nd"};
  ...
  string InPutFile_SE = Form("../data/Yields_Phi_SE_%s_%s_%s_kaon.root",vmsa::mBeamEnergy[energy].c_str(),date.c_str(),etamode.c_str());
  ...
  string InPutFile_ME = Form("../data/Yields_Phi_ME_%s_%s_%s_kaon.root",vmsa::mBeamEnergy[energy].c_str(),date.c_str(),etamode.c_str());
  ...
  for(Int_t i_pt = 0; i_pt < vmsa::pt_total; i_pt++) // pt bin 
  {
    for(Int_t i_cent = 9; i_cent < vmsa::Cent_stop; i_cent++) // centrality bin
    {
      ...
      {
        for(Int_t i_phipsi = 0; i_phipsi < 7; i_phipsi++) // phi-psi bin
        {
          for(Int_t i_dca = vmsa::Dca_start; i_dca < vmsa::Dca_stop; i_dca++)
          {
            for(Int_t i_sig = vmsa::nSigKaon_start; i_sig < vmsa::nSigKaon_stop; i_sig++)
            {
              ...
              string KEY_InPutSE = Form("pt_%d_Centrality_%d_CosThetaStar_%d_PhiPsi_%d_%s_Dca_%d_Sig_%d_%s_SE",i_pt,i_cent,i_theta,i_phipsi,EP[order-1].c_str(),i_dca,i_sig,vmsa::mPID[pid].c_str());
              h_mInPut_SE[KEY_InPutSE] = (TH1F*)File_SE->Get(KEY_InPutSE.c_str())->Clone(); 

              string KEY_InPutME = Form("pt_%d_Centrality_%d_CosThetaStar_%d_PhiPsi_%d_%s_Dca_%d_Sig_%d_%s_ME",i_pt,i_cent,i_theta,i_phipsi,EP[order-1].c_str(),i_dca,i_sig,vmsa::mPID[pid].c_str());
              ...
              {
                string KEY_SE = Form("pt_%d_Centrality_%d_CosThetaStar_%d_PhiPsi_%d_%s_Dca_%d_Sig_%d_%s_Norm_%d_SE",i_pt,i_cent,i_theta,i_phipsi,EP[order-1].c_str(),i_dca,i_sig,vmsa::mPID[pid].c_str(),i_norm);
                h_mMass_SE[KEY_SE] = (TH1F*)h_mInPut_SE[KEY_InPutSE]->Clone(KEY_SE.c_str());
                string KEY_ME = Form("pt_%d_Centrality_%d_CosThetaStar_%d_PhiPsi_%d_%s_Dca_%d_Sig_%d_%s_Norm_%d_ME",i_pt,i_cent,i_theta,i_phipsi,EP[order-1].c_str(),i_dca,i_sig,vmsa::mPID[pid].c_str(),i_norm);
                h_mMass_ME[KEY_ME] = (TH1F*)h_mInPut_ME[KEY_InPutME]->Clone(KEY_ME.c_str());
                string KEY_SM = Form("pt_%d_Centrality_%d_CosThetaStar_%d_PhiPsi_%d_%s_Dca_%d_Sig_%d_%s_Norm_%d_SM",i_pt,i_cent,i_theta,i_phipsi,EP[order-1].c_str(),i_dca,i_sig,vmsa::mPID[pid].c_str(),i_norm);
  ...
  for(int i_pt = 0; i_pt < vmsa::pt_total; i_pt++)
  {
    ...
    string KEY_SE_QA = Form("pt_%d_Centrality_%d_CosThetaStar_%d_PhiPsi_%d_%s_Dca_%d_Sig_%d_%s_Norm_%d_SE",i_pt,9,vmsa::CTS_start,0,EP[order-1].c_str(),vmsa::Dca_start,vmsa::nSigKaon_start,vmsa::mPID[pid].c_str(),vmsa::Norm_QA);
    ...
    string KEY_ME_QA = Form("pt_%d_Centrality_%d_CosThetaStar_%d_PhiPsi_%d_%s_Dca_%d_Sig_%d_%s_Norm_%d_ME",i_pt,9,vmsa::CTS_start,0,EP[order-1].c_str(),vmsa::Dca_start,vmsa::nSigKaon_start,vmsa::mPID[pid].c_str(),vmsa::Norm_QA);
    ...
    string KEY_SM_QA = Form("pt_%d_Centrality_%d_CosThetaStar_%d_PhiPsi_%d_%s_Dca_%d_Sig_%d_%s_Norm_%d_SM",i_pt,9,vmsa::CTS_start,0,EP[order-1].c_str(),vmsa::Dca_start,vmsa::nSigKaon_start,vmsa::mPID[pid].c_str(),vmsa::Norm_QA);
    ...
    c_peak->SaveAs(Form("figures/%s/%s/pTstudy/PhiPsi_CloseUpMixedEvent_%s_Order%d_Cent0_pt%d.pdf",vmsa::mPID[pid].c_str(),vmsa::mBeamEnergy[energy].c_str(),vmsa::mBeamEnergy[energy].c_str(),order,i_pt));
  }
  ...
  string outputname = Form("figures/%s/%s/pTstudy/PhiPsi_InvMassDistributions_%s_Order%d_Cent9.pdf",vmsa::mPID[pid].c_str(),vmsa::mBeamEnergy[energy].c_str(),vmsa::mBeamEnergy[energy].c_str(),order);
  ...
  for(int itheta = 0; itheta < 7; itheta++)
  {
    for(int i_phipsi = 0; i_phipsi < 7; i_phipsi++)
    {
      for(int i_pt = vmsa::pt_start; i_pt < vmsa::pt_stop; i_pt++) // pt loop
      {
        ...
        string KEY_SE_QA = Form("pt_%d_Centrality_%d_CosThetaStar_%d_PhiPsi_%d_%s_Dca_%d_Sig_%d_%s_Norm_%d_SE",i_pt,9,itheta,i_phipsi,EP[order-1].c_str(),vmsa::Dca_start,vmsa::nSigKaon_start,vmsa::mPID[pid].c_str(),vmsa::Norm_QA);
        h_mMass_SE[KEY_SE_QA]->GetYaxis()->SetRangeUser(-0.1*h_mMass_SE[KEY_SE_QA]->GetMaximum(),1.1*h_mMass_SE[KEY_SE_QA]->GetMaximum());
        h_mMass_SE[KEY_SE_QA]->SetTitle(Form("%d/7<|cos(#theta*)|<%d/7",itheta,itheta+1));
        h_mMass_SE[KEY_SE_QA]->DrawCopy();

        string KEY_ME_QA = Form("pt_%d_Centrality_%d_CosThetaStar_%d_PhiPsi_%d_%s_Dca_%d_Sig_%d_%s_Norm_%d_ME",i_pt,9,itheta,i_phipsi,EP[order-1].c_str(),vmsa::Dca_start,vmsa::nSigKaon_start,vmsa::mPID[pid].c_str(),vmsa::Norm_QA);
        h_mMass_ME[KEY_ME_QA]->SetLineColor(2);
        h_mMass_ME[KEY_ME_QA]->SetFillColor(2);
        h_mMass_ME[KEY_ME_QA]->SetFillStyle(3002);
        h_mMass_ME[KEY_ME_QA]->DrawCopy("h same");

        string KEY_SM_QA = Form("pt_%d_Centrality_%d_CosThetaStar_%d_PhiPsi_%d_%s_Dca_%d_Sig_%d_%s_Norm_%d_SM",i_pt,9,itheta,i_phipsi,EP[order-1].c_str(),vmsa::Dca_start,vmsa::nSigKaon_start,vmsa::mPID[pid].c_str(),vmsa::Norm_QA);
        h_mMass_SM[KEY_SM_QA]->SetLineColor(4);
        h_mMass_SM[KEY_SM_QA]->SetFillColor(4);
        h_mMass_SM[KEY_SM_QA]->SetFillStyle(3004);
        h_mMass_SM[KEY_SM_QA]->DrawCopy("h same");
        ...
        for(Int_t i_dca = vmsa::Dca_start; i_dca < vmsa::Dca_stop; i_dca++)
        {
          for(Int_t i_sig = vmsa::nSigKaon_start; i_sig < vmsa::nSigKaon_stop; i_sig++)
          {
            ...
            for(int i_norm = vmsa::Norm_start; i_norm < vmsa::Norm_stop; ++i_norm)
            {
              ...
              {
                string KEY = Form("pt_%d_Centrality_%d_CosThetaStar_%d_PhiPsi_%d_%s_Dca_%d_Sig_%d_%s_Norm_%d",pt_bin,i_cent,i_theta,i_phipsi,EP[order-1].c_str(),i_dca,i_sig,vmsa::mPID[pid].c_str(),i_norm);
  ...
  outputname = Form("figures/%s/%s/pTstudy/PhiPsi_InvMassBackgroundSubtracted_Rebinned_%s_Order%d_Cent9.pdf",vmsa::mPID[pid].c_str(),vmsa::mBeamEnergy[energy].c_str(),vmsa::mBeamEnergy[energy].c_str(),order);
  ...
  for(int itheta = 0; itheta < 7; itheta++)
  {
    for(int i_phipsi = 0; i_phipsi < 7; i_phipsi++)
    {
      for(int i_pt = vmsa::pt_rebin_first[energy]; i_pt < vmsa::pt_rebin_last[energy]; i_pt++) // pt loop
      {
        ...
        string KEY_QA = Form("pt_%d_Centrality_%d_CosThetaStar_%d_PhiPsi_%d_%s_Dca_%d_Sig_%d_%s_Norm_%d",i_pt,9,itheta,i_phipsi,EP[order-1].c_str(),vmsa::Dca_start,vmsa::nSigKaon_start,vmsa::mPID[pid].c_str(),vmsa::Norm_QA);
        h_mMass[KEY_QA]->SetMarkerStyle(24);
        h_mMass[KEY_QA]->SetMarkerSize(1.2);
        h_mMass[KEY_QA]->SetLineColor(kGray+2);
        h_mMass[KEY_QA]->SetTitle(Form("%d/7<|cos(#theta*)|<%d/7, %d#pi/14<#phi-#Psi_{2}<%d#pi/14",itheta,itheta+1,(i_phipsi)*2-7,(i_phipsi+1)*2-7));
  ...
  if(pid == 0 || pid == 1) // Polynomial fit subtraction is only needed for phi meson
  {
	  ...
	  { 
            ...
            for(int i_norm = vmsa::Norm_start; i_norm < vmsa::Norm_stop; ++i_norm)
	    {
	      ...
	      for(int i_theta = vmsa::CTS_start; i_theta < vmsa::CTS_stop; i_theta++) // cos(theta*) loop
	      {
	        for(int i_phipsi = 0; i_phipsi < 7; i_phipsi++) // cos(theta*) loop
	        {
		  string KEY = Form("pt_%d_Centrality_%d_CosThetaStar_%d_PhiPsi_%d_%s_Dca_%d_Sig_%d_%s_Norm_%d",i_pt,i_cent,i_theta,i_phipsi,EP[order-1].c_str(),i_dca,i_sig,vmsa::mPID[pid].c_str(),i_norm);
	...
	for(Int_t i_dca = vmsa::Dca_start; i_dca < vmsa::Dca_stop; i_dca++)
	{
	  for(Int_t i_sig = vmsa::nSigKaon_start; i_sig < vmsa::nSigKaon_stop; i_sig++)
	  {
		...
		string KEY = Form("pt_%d_Centrality_%d_CosThetaStar_%d_PhiPsi_%d_2nd_Dca_%d_Sig_%d_%s_Norm_%d",i_pt,i_cent,i_theta,i_dca,i_sig,vmsa::mPID[pid].c_str(),i_norm);
...
//      string KEY_QA = Form("pt_%d_Centrality_%d_CosThetaStar_%d_PhiPsi_%d_%s_Dca_%d_Sig_%d_%s_Norm_%d",i_pt,9,vmsa::CTS_start,EP[order-1].c_str(),vmsa::Dca_start,vmsa::nSigKaon_start,vmsa::mPID[pid].c_str(),vmsa::Norm_QA);
...
//#endif
  ...
  string outputfile = Form("../output/AuAu%s/%s/PhiPsi_InvMassSubBg_%s.root",vmsa::mBeamEnergy[energy].c_str(),vmsa::mPID[pid].c_str(),etamode.c_str());
  //string outputfile = Form("../output/AuAu%s/%s/InvMassSubBg.root",vmsa::mBeamEnergy[energy].c_str(),vmsa::mPID[pid].c_str());
  if(order == 1) outputfile = Form("../output/AuAu%s/%s/PhiPsi_InvMassSubBg_%s_FirstOrder.root",vmsa::mBeamEnergy[energy].c_str(),vmsa::mPID[pid].c_str(),etamode.c_str());
