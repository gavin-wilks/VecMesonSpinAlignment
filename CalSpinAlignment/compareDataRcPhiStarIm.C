...
void compareDataRcPhiStarIm(Int_t energy = 4, Int_t pid = 0, Int_t i_cent = 9, string correction = "Raw", int order = 2, string etamode = "eta1_eta1", int yspectra = 23, std::string simmode = "Mc")//defaultF = 0 is BESII, defaultF = 1 is BESI
{
  ...
  for(int i = 0; i < nv; i++)
  {
    spectra[i] = Form("CEPT_O2_NoYSpec_prelimv2_phieff_phiTPC_20240809g_ysig1000_rho0.3333_rerho0.0_imrho0.0_r0.0_i%s_hrho0.3333_hrerho0.0_himrho0.0_hr0.0_hi0.0",rho1n1_text[i].c_str());
  }
  ...
  std::string outputname = Form("figures/%s/%s/pTstudy/Cos2PhiStarPhi_Ratio_%s_Order%d_%s%s_imcomparison_%s.pdf",vmsa::mPID[pid].c_str(),vmsa::mBeamEnergy[energy].c_str(),correction.c_str(),order,etamode.c_str(),spectra[3].c_str(),simmode.c_str());  
  ...
  for(int ipt = 0; ipt < 4; ipt++)
  {
    ...
    for(int i = 0; i < nv; i++)
    {
      ...
      h_mCounts_Ratio[ipt][i]->SetTitle(Form("Im(#rho_{10})-Im(#rho_{0-1})=%1.1f, %1.1f<p_{T}<%1.1f, 20-60 Centrality",rho1n1[i],vmsa::pt_low[energy][ipt+2],vmsa::pt_up[energy][ipt+2]));
  ...
  for(int i = 0; i < 4; i++)
  {
    for(int par = 0; par < 5; par++)
    {
      ...
      g2D[par][i]->GetXaxis()->SetTitle("Im(#rho_{10})-Im(#rho_{0-1})");
      g2D[par][i]->GetYaxis()->SetTitle(param[par].c_str());
      g2D[par][i]->Draw("APE"); 
    }
  }
  cfit2D->SaveAs(Form("figures/%s/%s/pTstudy/Cos2PhiStarPhi_parametercomparison_%s_Order%d_%s%s_imcomparison_%s.pdf",vmsa::mPID[pid].c_str(),vmsa::mBeamEnergy[energy].c_str(),correction.c_str(),order,etamode.c_str(),spectra[3].c_str(),simmode.c_str()));  
  ...
  for(int i = 0; i < 4; i++)
  {
    ...
    gslope[i]->GetXaxis()->SetTitle("Im(#rho_{10})-Im(#rho_{0-1})");
  ...
  for(int i = 0; i < 4; i++)
  {
    ...
    gintercept[i]->GetXaxis()->SetTitle("Im(#rho_{10})-Im(#rho_{0-1})");
  ...
  for(int i = 0; i < 4; i++)
  {
    ...
    gmean[i]->GetXaxis()->SetTitle("Im(#rho_{10})-Im(#rho_{0-1})");
    gmean[i]->GetYaxis()->SetTitle("<cos(2#phi*-2#phi)>");
    gmean[i]->Draw("APE"); 
  }
  cfit->SaveAs(Form("figures/%s/%s/pTstudy/Cos2PhiStarPhi_FitParams_%s_Order%d_%s%s_imcomparison_%s.pdf",vmsa::mPID[pid].c_str(),vmsa::mBeamEnergy[energy].c_str(),correction.c_str(),order,etamode.c_str(),spectra[3].c_str(),simmode.c_str()));  


  outputname = Form("figures/%s/%s/pTstudy/Cos2PhiStarPhi_Comparison_%s_Order%d_%s%s_imcomparison_%s.pdf",vmsa::mPID[pid].c_str(),vmsa::mBeamEnergy[energy].c_str(),correction.c_str(),order,etamode.c_str(),spectra[3].c_str(),simmode.c_str());  
  ...
  for(int ipt = 0; ipt < 4; ipt++)
  {
    ...
    { 
      ...
      h_mCounts_RC[ipt][i]->SetTitle(Form("Im(#rho_{10})-Im(#rho_{0-1})=%1.1f, %1.1f<p_{T}<%1.1f, 20-60 Centrality",rho1n1[i],vmsa::pt_low[energy][ipt+2],vmsa::pt_up[energy][ipt+2]));
