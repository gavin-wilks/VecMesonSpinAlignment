...
void calSysErrorPhiCent_FixedSys(Int_t energy = 3, Int_t pid = 0, string correction = "Raw", bool random3D = false, int centQA = 0, int order = 2, std::string etamode = "eta1_eta1", std::string fileoption = "")//defaultF = 0 is BESII, defaultF = 1 is BESI
{
  std::string EP[2] = {"","2nd"};
  ...
  string outputname = Form("./figures/%s/%s/centralitystudy/finaloutputrho_%s_centralitydependence_pTbins_%s_Order%d_%s.pdf",vmsa::mPID[pid].c_str(),vmsa::mBeamEnergy[energy].c_str(),correction.c_str(),vmsa::mBeamEnergy[energy].c_str(),order,etamode.c_str());
...
//  PlotLine(0.0,5.0,1.0/3.0,1.0/3.0,1,2,2);
  ...
  c_rho_SysError->SaveAs(Form("figures/%s/%s/centralitystudy/finaloutputrho_%s_centralitydependence_%s_Order%d_%s.pdf",vmsa::mPID[pid].c_str(),vmsa::mBeamEnergy[energy].c_str(),correction.c_str(),vmsa::mBeamEnergy[energy].c_str(),order,etamode.c_str()));  
