...
void plotKaonTTrees_Compare_phi(int energy = 4, bool datarcweight = false) 
{
    ...
    string inputfile = Form("../output/AuAu%s/Phi/ptyphispectra_RawPhiPtSys_eta1_eta1_PolySys.root",vmsa::mBeamEnergy[energy].c_str());
    TFile *File_InPut = TFile::Open(inputfile.c_str());
    string KEY = Form("ptyphispectra_Centrality_%d_%s_Dca_%d_Sig_%d_%s_Norm_%d_Sigma_%d_%s_Poly%d",9,"2nd",0,0,vmsa::mPID[0].c_str(),0,0,vmsa::mInteMethod[1].c_str(),1);
    TH2F *ptrapidityPhiData = (TH2F*) File_InPut->Get(KEY.c_str());
    TH2F *ptrapidityPhiRc = new TH2F("ptrapidityPhiRC","ptrapidityPhiRC",vmsa::phirebinytotal,vmsa::phirebinyval,vmsa::phirebinphitotal,vmsa::phirebinphival); 
    TH2F *ptrapidityPhiRcAfter = new TH2F("ptrapidityPhiRCAfter","ptrapidityPhiRCAfter",vmsa::phirebinytotal,vmsa::phirebinyval,vmsa::phirebinphitotal,vmsa::phirebinphival); 
    ...
    TH3F *CosThetaStarPhiRc = new TH3F("CosThetaStarPhiRC","CosThetaStarPhiRC",vmsa::phirebinytotal,vmsa::phirebinyval,vmsa::phirebinphitotal,vmsa::phirebinphival,ncostheta,costheta); 
    TH3F *CosThetaStarPhiRcAfter = new TH3F("CosThetaStarPhiRCAfter","CosThetaStarPhiRCAfter",vmsa::phirebinytotal,vmsa::phirebinyval,vmsa::phirebinphitotal,vmsa::phirebinphival,ncostheta,costheta); 
    ...
    for (Long64_t i = 0; i < nentriesEmbed; i++)
    {
      ...
      if(mRcExists && mPassTpc && mPassTof && mPassM2 && mPassNsig)    
      {
        ...
        ptrapidityPhiRc->Fill(mRcRapidity,mRcPhi,mWeight*PhiWeight);
        //ptrapidityPhiRc->Fill(mMcPt,mMcRapidity,mWeight*PhiWeight);
        CosThetaStarPhiRc->Fill(mRcPt,mRcRapidity,mRcCosTheta,mWeight*PhiWeight);
      }
    }

    TH2F *ptrapidityPhiDataRc;

    double integralPhiData = ptrapidityPhiData->Integral(1,vmsa::phirebinytotal,1,vmsa::phirebinphitotal); 
    double integralPhiRc = ptrapidityPhiRc->Integral(1,vmsa::phirebinytotal,1,vmsa::phirebinphitotal); 
    ...
    TH1F *ptPhiData = (TH1F*) ptrapidityPhiData->ProjectionX("ptData",1,vmsa::phirebinytotal);
    TH1F *yPhiData  = (TH1F*) ptrapidityPhiData->ProjectionY("yData",1,vmsa::phirebinphitotal);  
    TH1F *ptPhiRc   = (TH1F*) ptrapidityPhiRc->ProjectionX("ptRc",1,vmsa::phirebinytotal);  
    TH1F *yPhiRc    = (TH1F*) ptrapidityPhiRc->ProjectionY("yRc",1,vmsa::phirebinphitotal);  
    ...
    ptrapidityPhiData->GetXaxis()->SetTitle("y"); 
    ptrapidityPhiData->GetYaxis()->SetTitle("#phi"); 
    ptrapidityPhiData->Draw("Colz"); 
    ct->cd(2);
    //ct->cd(2)->SetLogz();
    ptrapidityPhiRc->SetTitle("Embedding"); 
    ptrapidityPhiRc->GetXaxis()->SetTitle("y"); 
    ptrapidityPhiRc->GetYaxis()->SetTitle("#phi"); 
    ptrapidityPhiRc->Draw("Colz"); 
    ct->cd(3);
    ct->cd(3)->SetLogy(0);
    ptrapidityPhiDataRc->SetTitle("Data/Embedding"); 
    ptrapidityPhiDataRc->GetXaxis()->SetTitle("y"); 
    ptrapidityPhiDataRc->GetYaxis()->SetTitle("#phi"); 
    ptrapidityPhiDataRc->Draw("Colz"); 

    ct->cd(4);
    //ct->cd(4)->SetLogy();
    ptPhiData->SetTitle("Before Data/RC Reweight #phi-meson"); 
    ptPhiData->GetXaxis()->SetTitle("y"); 
    ...
    yPhiData->GetXaxis()->SetTitle("#phi"); 
    ...
    yPhiRc->Draw("pE same");
    //TLegend *legy = new TLegend(0.4,0.15,0.6,0.35);
    ...
    ptPhiDataRc->GetXaxis()->SetTitle("y"); 
    ptPhiDataRc->GetYaxis()->SetTitle("Data/RC"); 
    ptPhiDataRc->Draw("pE");

    ct->cd(8);
    yPhiDataRc->SetTitle("Before Data/RC Reweight #phi-meson"); 
    yPhiDataRc->GetXaxis()->SetTitle("#phi"); 
    yPhiDataRc->GetYaxis()->SetTitle("Data/RC"); 
    yPhiDataRc->Draw("pE");

    ct->SaveAs(Form("figures/AddPhiKaonTTrees/PhiMeson_DataRCAndRatio.pdf"));
    ...
    for (Long64_t i = 0; i < nentriesEmbed; i++)
    {
      ...
      if(mRcExists && mPassTpc && mPassTof && mPassM2 && mPassNsig)    
      {
        ...
        ptrapidityPhiRcAfter->Fill(mRcRapidity,mRcPhi,mWeight*PhiWeight*DataRcWeightVal);
        CosThetaStarPhiRcAfter->Fill(mRcPt,mRcRapidity,mRcCosTheta,mWeight*PhiWeight*DataRcWeightVal);
    ...
    TH1F *CosThetaStarPhiRcDiff[vmsa::phirebinytotal][vmsa::phirebinphitotal];
    TH1F *CosThetaStarPhiRcDiffAfter[vmsa::phirebinytotal][vmsa::phirebinphitotal];
    TH1F *CosThetaStarPhiRcDiffRatio[vmsa::phirebinytotal][vmsa::phirebinphitotal];
    TH1F *CosThetaStarPhiRcPt[vmsa::phirebinphitotal];
    TH1F *CosThetaStarPhiRcPtAfter[vmsa::phirebinphitotal];
    TGraphAsymmErrors *gRcPt = new TGraphAsymmErrors();
    TGraphAsymmErrors *gRcPtAfter = new TGraphAsymmErrors();
    double rhoPhiRcPt[vmsa::phirebinphitotal];
    double rhoPhiRcPtAfter[vmsa::phirebinphitotal];
    double rhoErrPhiRcPt[vmsa::phirebinphitotal];
    double rhoErrPhiRcPtAfter[vmsa::phirebinphitotal];
    TH1F *CosThetaStarPhiRcPtRatio[vmsa::phirebinphitotal];
    TH1F *CosThetaStarPhiRcPtOpt[vmsa::phirebinytotal][3];
    TH1F *CosThetaStarPhiRcPtOptAfter[vmsa::phirebinytotal][3];
    ...
    double rhoPhiRcPtOpt[vmsa::phirebinytotal][3];
    double rhoPhiRcPtOptAfter[vmsa::phirebinytotal][3];
    double rhoErrPhiRcPtOpt[vmsa::phirebinytotal][3];
    double rhoErrPhiRcPtOptAfter[vmsa::phirebinytotal][3];
    TH1F *CosThetaStarPhiRcPtOptRatio[vmsa::phirebinytotal][3];
    TH1F *CosThetaStarPhiRcY[vmsa::phirebinytotal];
    TH1F *CosThetaStarPhiRcYAfter[vmsa::phirebinytotal];
    TGraphAsymmErrors *gRcY      = new TGraphAsymmErrors();
    TGraphAsymmErrors *gRcYAfter = new TGraphAsymmErrors();
    double rhoPhiRcY[vmsa::phirebinytotal];
    double rhoPhiRcYAfter[vmsa::phirebinytotal];
    double rhoErrPhiRcY[vmsa::phirebinytotal];
    double rhoErrPhiRcYAfter[vmsa::phirebinytotal];
    TH1F *CosThetaStarPhiRcYRatio[vmsa::phirebinytotal];

    int ylow[3] = {1,4,12};
    int yhigh[3] = {3,11,14};
    for(int ipt = 0; ipt < vmsa::phirebinytotal; ipt++)
    {
      for(int iy = 0; iy < 3; iy++)
      {
        ...
        double ptmean = (vmsa::phirebinyval[ipt]+vmsa::phirebinyval[ipt+1])/2.0;
    ...
    for(int ipt = 0; ipt < vmsa::phirebinytotal; ipt++)
    {
      for(int iy = 0; iy < vmsa::phirebinphitotal; iy++)
      {
    ...
    for(int ipt = 0; ipt < vmsa::phirebinytotal; ipt++)
    {
      ...
      CosThetaStarPhiRcY[ipt] = (TH1F*) CosThetaStarPhiRc->ProjectionZ(Hist.c_str(),ipt+1,ipt+1,1,vmsa::phirebinphitotal,"e");
      string HistAfter = Form("costhetaafter_pt_%d",ipt);
      CosThetaStarPhiRcYAfter[ipt] = (TH1F*) CosThetaStarPhiRcAfter->ProjectionZ(HistAfter.c_str(),ipt+1,ipt+1,1,vmsa::phirebinphitotal,"e");
    ...
    { 
      ...
      CosThetaStarPhiRcPt[iy] = (TH1F*) CosThetaStarPhiRc->ProjectionZ(Hist.c_str(),1,vmsa::phirebinytotal,iy+1,iy+1,"e");
      string HistAfter = Form("costhetaafter_y_%d",iy);
      CosThetaStarPhiRcPtAfter[iy] = (TH1F*) CosThetaStarPhiRcAfter->ProjectionZ(HistAfter.c_str(),1,vmsa::phirebinytotal,iy+1,iy+1,"e");
      ...
      double ymean = (vmsa::phirebinphival[iy]+vmsa::phirebinphival[iy+1])/2.0;
    ...
    string outputname = Form("figures/AddPhiKaonTTrees/%sCosThetaDistributions.pdf",folderopt.c_str());
    ...
    for(int ipt = 0; ipt < vmsa::phirebinytotal; ipt++)
    {
      for(int iy = 0; iy < vmsa::phirebinphitotal; iy++)
      {
        ...
        CosThetaStarPhiRcDiff[ipt][iy]->SetTitle(Form("%.2f<p_{T}<%.2f GeV/c, %.2f<y<%.2f",vmsa::phirebinyval[ipt],vmsa::phirebinyval[ipt+1],vmsa::phirebinphival[iy],vmsa::phirebinphival[iy+1]));   
    ...
    outputname = Form("figures/AddPhiKaonTTrees/%sCosThetaDistributions_PtYselections.pdf",folderopt.c_str());
    ...
    for(int ipt = 0; ipt < vmsa::phirebinytotal; ipt++)
    {
      for(int iy = 0; iy < 3; iy++)
      {
        ...
        CosThetaStarPhiRcPtOpt[ipt][iy]->SetTitle(Form("%.2f<p_{T}<%.2f GeV/c, %.2f<y<%.2f",vmsa::phirebinyval[ipt],vmsa::phirebinyval[ipt+1],ylowval[iy],yhighval[iy]));   
        ...
        ccosopt->cd(iy+4);
        CosThetaStarPhiRcPtOptRatio[ipt][iy]->SetTitle(Form("(Data/RC Reweight)/(No Reweight) %.2f<p_{T}<%.2f GeV/c, %.2f<y<%.2f",vmsa::phirebinyval[ipt],vmsa::phirebinyval[ipt+1],ylowval[iy],yhighval[iy]));   
    ...
    outputname = Form("figures/AddPhiKaonTTrees/%sCosThetaDistributions_rho00.pdf",folderopt.c_str());
    outputstart = Form("%s[",outputname.c_str());
    outputstop = Form("%s]",outputname.c_str());
...
//    for(int i = 0; i < 16; i++)  ccos->cd(i+1)->Clear();
    ...
    outputname = Form("figures/AddPhiKaonTTrees/%sCosThetaDistributions_Pt.pdf",folderopt.c_str());
    outputstart = Form("%s[",outputname.c_str());
    outputstop = Form("%s]",outputname.c_str());
    ...
    for(int iy = 0; iy < vmsa::phirebinphitotal; iy++)
    {
      ...
      CosThetaStarPhiRcPt[iy]->SetTitle(Form("%.2f<p_{T}<%.2f GeV/c, %.2f<y<%.2f",vmsa::phirebinyval[0],vmsa::phirebinyval[vmsa::phirebinytotal-1],vmsa::phirebinphival[iy],vmsa::phirebinphival[iy+1]));   
    ...
    outputname = Form("figures/AddPhiKaonTTrees/%sCosThetaDistributions_Y.pdf",folderopt.c_str());
    outputstart = Form("%s[",outputname.c_str());
    outputstop = Form("%s]",outputname.c_str());
    ...
    for(int ipt = 0; ipt < vmsa::phirebinytotal; ipt++)
    {
      ...
      CosThetaStarPhiRcY[ipt]->SetTitle(Form("%.2f<p_{T}<%.2f GeV/c, 1.0<y<1.0",vmsa::phirebinyval[ipt],vmsa::phirebinyval[ipt+1]));   
    ...
    for(int i = 0; i < 16; i++)  ccos->cd(i+1)->Clear();
    outputname = Form("figures/AddPhiKaonTTrees/%sCosThetaRatios.pdf",folderopt.c_str());
    outputstart = Form("%s[",outputname.c_str());
    outputstop = Form("%s]",outputname.c_str());
    ...
    for(int ipt = 0; ipt < vmsa::phirebinytotal; ipt++)
    {
      for(int iy = 0; iy < vmsa::phirebinphitotal; iy++)
      {
        ...
        CosThetaStarPhiRcDiffRatio[ipt][iy]->SetTitle(Form("(Data/RC Reweight)/(No Reweight) %.2f<p_{T}<%.2f GeV/c, %.2f<y<%.2f",vmsa::phirebinyval[ipt],vmsa::phirebinyval[ipt+1],vmsa::phirebinphival[iy],vmsa::phirebinphival[iy+1]));   
    ...
    for(int i = 0; i < 16; i++)  ccos->cd(i+1)->Clear();
    outputname = Form("figures/AddPhiKaonTTrees/%sCosThetaRatios_Y.pdf",folderopt.c_str());
    outputstart = Form("%s[",outputname.c_str());
    outputstop = Form("%s]",outputname.c_str());
    ...
    for(int ipt = 0; ipt < vmsa::phirebinytotal; ipt++)
    {
      ...
      CosThetaStarPhiRcYRatio[ipt]->SetTitle(Form("(Data/RC Reweight)/(No Reweight) %.2f<p_{T}<%.2f GeV/c, -1.0<y<1.0",vmsa::phirebinyval[ipt],vmsa::phirebinyval[ipt+1]));   
    ...
    for(int i = 0; i < 16; i++)  ccos->cd(i+1)->Clear();
    outputname = Form("figures/AddPhiKaonTTrees/%sCosThetaRatios_Pt.pdf",folderopt.c_str());
    outputstart = Form("%s[",outputname.c_str());
    outputstop = Form("%s]",outputname.c_str());
    ...
    for(int iy = 0; iy < vmsa::phirebinphitotal; iy++)
    {
      ...
      CosThetaStarPhiRcPtRatio[iy]->SetTitle(Form("(Data/RC Reweight)/(No Reweight) %.2f<p_{T}<%.2f GeV/c, %.2f<y<%.2f",vmsa::phirebinyval[0],vmsa::phirebinyval[vmsa::phirebinytotal-1],vmsa::phirebinphival[iy],vmsa::phirebinphival[iy+1]));   
    ...
    //double integralPhiData = ptrapidityPhiData->Integral(1,vmsa::phirebinytotal,1,vmsa::phirebinphitotal); 
    double integralPhiRcAfter = ptrapidityPhiRcAfter->Integral(1,vmsa::phirebinytotal,1,vmsa::phirebinphitotal); 
    ...
    ct->SaveAs(Form("figures/AddPhiKaonTTrees/PhiMeson_DataRCAndRatio_After.pdf"));
    ...
    for(int ic = 0; ic < 3; ic++)
    {
      for(int im = 0; im < 5; im++)
      {
        ...
        c1->SaveAs(Form("figures/AddPhiKaonTTrees/%sK%s_%s.pdf",folderopt.c_str(),charge[ic].c_str(),mixinglabel[im].c_str()));
      }
    }
    ...
    for(int im = 0; im < 2; im++)
    {
      for(int ic = 0; ic < 2; ic++)
      {
        ...
        cr->SaveAs(Form("figures/AddPhiKaonTTrees/%sK%s_%s.pdf",folderopt.c_str(),charge[ic].c_str(),ratiofile[im].c_str()));
    ...
    for(int is = 0; is < nset+1; is++)
    {
      for(int im = 0; im < 2; im++)
      {
        for(int ic = 0; ic < 2; ic++)
        {
          ...
          cr->SaveAs(Form("figures/AddPhiKaonTTrees/%sK%s_%s_%s.pdf",folderopt.c_str(),charge[ic].c_str(),ratiofile[im].c_str(),setfile[is].c_str()));
    ...
    for(int ic = 0; ic < 2; ic++)
    {
      ...
      c43->SaveAs(Form("figures/AddPhiKaonTTrees/%sK%s_1DRatios.pdf",folderopt.c_str(),charge[ic].c_str()));
    }
    ...
    for(int is = 0; is < nset+1; is++)
    {
      for(int ic = 0; ic < 2; ic++)
      {
        ...
        c43->SaveAs(Form("figures/AddPhiKaonTTrees/%sK%s_1DRatios_%s.pdf",folderopt.c_str(),charge[ic].c_str(),setfile[is].c_str()));
      }
    }
}
