...
void calSysErrorPhiY_AfterIntegration(Int_t energy = 4, Int_t pid = 0, string correction = "AccRes", bool random3D = false, int ptQA = 0, int centQA = 2, std::string etamode = "eta0p4")//defaultF = 0 is BESII, defaultF = 1 is BESI
{
  ...
  int ypadding = 0;
  if(etamode == "eta1_eta1" && (correction == "AccRes" || correction == "Eff")) ypadding = 2;
  if(etamode == "eta0p4" && (correction == "AccRes" || correction == "Eff")) ypadding = 5;
  if(etamode == "eta0p6" && (correction == "AccRes" || correction == "Eff")) ypadding = 4;
  if(etamode == "eta0p8" && (correction == "AccRes" || correction == "Eff")) ypadding = 3;
  ...
  //string inputfile = Form("../output/AuAu%s/%s/%sRhoEtaSys_%s_2ndOrder.root",vmsa::mBeamEnergy[energy].c_str(),vmsa::mPID[pid].c_str(),correction.c_str(),etamode.c_str());
  string inputfile = Form("../output/AuAu%s/%s/%sRhoEtaSys_%s.root",vmsa::mBeamEnergy[energy].c_str(),vmsa::mPID[pid].c_str(),correction.c_str(),etamode.c_str());
        ...
        { 
          for(int i_sigma = vmsa::Sig_start; i_sigma < vmsa::Sig_stop; ++i_sigma)
          {
            for(int i_method = vmsa::Method_start; i_method < vmsa::Method_stop; ++i_method)
            {
              ...
              string KEY = Form("rhoRaw_pt_%d_Centrality_%d_2nd_Dca_%d_Sig_%d_%s_Norm_%d_Sigma_%d_%s",i_pt,i_cent,i_dca,0,vmsa::mPID[pid].c_str(),0,i_sigma,vmsa::mInteMethod[i_method].c_str());
        ...
        for(Int_t i_sig = vmsa::nSigKaon_start; i_sig < vmsa::nSigKaon_stop; i_sig++)
        {
          for(int i_sigma = vmsa::Sig_start; i_sigma < vmsa::Sig_stop; ++i_sigma)
          {
            for(int i_method = vmsa::Method_start; i_method < vmsa::Method_stop; ++i_method)
            {
              ...
              string KEY = Form("rhoRaw_pt_%d_Centrality_%d_2nd_Dca_%d_Sig_%d_%s_Norm_%d_Sigma_%d_%s",i_pt,i_cent,0,i_sig,vmsa::mPID[pid].c_str(),0,i_sigma,vmsa::mInteMethod[i_method].c_str());
        ...
        for(int i_norm = vmsa::Norm_start; i_norm < vmsa::Norm_stop; ++i_norm)
        {
          for(int i_sigma = vmsa::Sig_start; i_sigma < vmsa::Sig_stop; ++i_sigma)
          {
            for(int i_method = vmsa::Method_start; i_method < vmsa::Method_stop; ++i_method)
            {
              ...
              string KEY = Form("rhoRaw_pt_%d_Centrality_%d_2nd_Dca_%d_Sig_%d_%s_Norm_%d_Sigma_%d_%s",i_pt,i_cent,0,0,vmsa::mPID[pid].c_str(),i_norm,i_sigma,vmsa::mInteMethod[i_method].c_str());
          ...
          for(int i_method = vmsa::Method_start; i_method < vmsa::Method_stop; ++i_method)
          {
            ...
            for(int i_eta = vmsa::eta_start+ypadding; i_eta < vmsa::eta_stop-ypadding; i_eta++) // Rapidity bin loop
            {
              ...
              {           
                ...
                for(int i_cent = 0; i_cent < vmsa::cent_rebin_total; i_cent++) // Centrality loop
                { 
                  string KEY_Default = Form("rhoRaw_pt_%d_Centrality_%d_2nd_Dca_%d_Sig_%d_%s_Norm_%d_Sigma_%d_%s",i_pt,i_cent,i_dca,i_sig,vmsa::mPID[pid].c_str(),i_norm,i_sigma,vmsa::mInteMethod[i_method].c_str());
                  string KEY_counts = Form("eta_%d_pt_%d_Centrality_%d_2nd_Dca_%d_Sig_%d_%s_Norm_%d_Sigma_%d_%s",i_eta,i_pt,i_cent,i_dca,i_sig,vmsa::mPID[pid].c_str(),i_norm,i_sigma,vmsa::mInteMethod[i_method].c_str());
              ...
              g_mRhoYieldWeight[KEY]->SetPoint(i_eta-ypadding, eta_mean, weight_rho00);
              g_mRhoYieldWeight[KEY]->SetPointError(i_eta-ypadding, 0.0, 0.0, weight_error_stat_rho00, weight_error_stat_rho00);
              g_mRhoStatWeight[KEY]->SetPoint(i_eta-ypadding, eta_mean, statweight_rho00);
              g_mRhoStatWeight[KEY]->SetPointError(i_eta-ypadding, 0.0, 0.0, statweight_error_stat_rho00, statweight_error_stat_rho00);
            }
          }
        }
      }
  ...
  //Yield Weighted
  {
    ...
    for(Int_t i_point = vmsa::eta_start+ypadding; i_point < vmsa::eta_stop-ypadding; ++i_point)
    {
      ...
      double pt_def, rho_def;
      g_mRhoYieldWeight[KEY_Default]->GetPoint(i_point-ypadding,pt_def,rho_def); 
    ...
    //        if(correction == "AccRes") KEY = Form("rhoRaw_2nd_Dca_%d_Sig_%d_%s_Norm_%d_Sigma_%d_%s_F_%d_Eff_0",i_dca,0,vmsa::mPID[pid].c_str(),0,0,vmsa::mInteMethod[i_method].c_str(),defaultF);
            double pt_sys, rho_sys;
            g_mRhoYieldWeight[KEY]->GetPoint(i_point-ypadding,pt_sys,rho_sys);
            sysDca[idx] = rho_sys;
            if(random3D) sysDca[idx] = 4.*(sysDca[idx]-1./3.)+1./3.;
            idx++;
          }
      ...
      Double_t pt, rho;
      g_mRhoYieldWeight[KEY_Default]->GetPoint(i_point-ypadding,pt,rho);

      //float mean_rho = total_rho/(float)counter;
      g_mSysErrorsYieldWeight->SetPoint(i_point-ypadding,pt,rho);
      g_mSysErrorsYieldWeight->SetPointError(i_point-ypadding,0.0,0.0,SysError_rho,SysError_rho);
    }
  }
  ...
  //Stat Weighted
  {
    ...
    for(Int_t i_point = vmsa::eta_start+ypadding; i_point < vmsa::eta_stop-ypadding; ++i_point)
    {
      ...
      double pt_def, rho_def;
      g_mRhoStatWeight[KEY_Default]->GetPoint(i_point-ypadding,pt_def,rho_def); 
    ...
    //        if(correction == "AccRes") KEY = Form("rhoRaw_2nd_Dca_%d_Sig_%d_%s_Norm_%d_Sigma_%d_%s_F_%d_Eff_0",i_dca,0,vmsa::mPID[pid].c_str(),0,0,vmsa::mInteMethod[i_method].c_str(),defaultF);
            double pt_sys, rho_sys;
            g_mRhoStatWeight[KEY]->GetPoint(i_point-ypadding,pt_sys,rho_sys);
            sysDca[idx] = rho_sys;
            if(random3D) sysDca[idx] = 4.*(sysDca[idx]-1./3.)+1./3.;
            idx++;
          }
      ...
      Double_t pt, rho;
      g_mRhoStatWeight[KEY_Default]->GetPoint(i_point-ypadding,pt,rho);

      //float mean_rho = total_rho/(float)counter;
      g_mSysErrorsStatWeight->SetPoint(i_point-ypadding,pt,rho);
      g_mSysErrorsStatWeight->SetPointError(i_point-ypadding,0.0,0.0,SysError_rho,SysError_rho);
    }
  }
...
//  PlotLine(0.0,5.0,1.0/3.0,1.0/3.0,1,2,2);
  ...
  Draw_TGAE_Point_new_Symbol(-0.1,0.48,0.0,0.0,0.0,0.0,style_phi_2nd,color_phi_2nd,size_marker+0.2); 
  string leg_count;
  ...
  plotTopLegend((char*)leg_count.c_str(),0.0,0.477,0.03,1,0.0,42,0);

  string leg_besi = "#phi (|y| < 1.5) Stat";
  Draw_TGAE_Point_new_Symbol(-0.1,0.46,0.0,0.0,0.0,0.0,style_phi_1st,color_phi_1st,size_marker-0.2);
  plotTopLegend((char*)leg_besi.c_str(),0.0,0.457,0.03,1,0.0,42,0);
  ...
  {           
    for(int i_cent = 0; i_cent < vmsa::cent_rebin_total; i_cent++) // Centrality loop
    {
      string KEY_rho = Form("rhoRaw_pt_%d_Centrality_%d_2nd_Dca_%d_Sig_%d_%s_Norm_%d_Sigma_%d_%s",i_pt,i_cent,0,0,vmsa::mPID[pid].c_str(),0,0,vmsa::mInteMethod[1].c_str());
      string KEY_rhoStat = Form("rhoRawStat_pt_%d_Centrality_%d_2nd_Dca_%d_Sig_%d_%s_Norm_%d_Sigma_%d_%s",i_pt,i_cent,0,0,vmsa::mPID[pid].c_str(),0,0,vmsa::mInteMethod[1].c_str());
      string KEY_rhoSys  = Form("rhoRawSys_pt_%d_Centrality_%d_2nd_Dca_%d_Sig_%d_%s_Norm_%d_Sigma_%d_%s",i_pt,i_cent,0,0,vmsa::mPID[pid].c_str(),0,0,vmsa::mInteMethod[1].c_str());
