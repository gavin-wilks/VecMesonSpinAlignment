...
void subBackGround_HistPrep(int energy = 5, int pid = 0, int year = 0, string date = "20240424", bool random3D = false, int order = 2, string etamode = "eta1_eta1")
{
  std::string EP[2] = {"","2nd"};
  ...
  float const pt_low_local[pt_rebin] = {0.0,1.2,1.8,2.4,3.0,4.2};
  float const pt_up_local[pt_rebin]  = {1.2,1.8,2.4,3.0,4.2,5.4};
  int const pt_rebin_start_local[pt_rebin] = {0,5,8,11,14,17};
  int const pt_rebin_stop_local[pt_rebin]  = {4,7,10,13,16,19};
  ...
  string outputfile_comp = Form("../output/AuAu%s/%s/AllRawHists_MediumPt_%s_%s.root",vmsa::mBeamEnergy[energy].c_str(),vmsa::mPID[pid].c_str(),vmsa::mBeamEnergy[energy].c_str(),etamode.c_str());
