...
void subBackGround_PhiStar_Phi(int energy = 4, int pid = 0, int year = 0, string date = "20240415", bool random3D = false, int order = 2, string etamode = "eta1_eta1")
{
  std::string EP[2] = {"","2nd"};
  ...
  string InPutFile_SE = Form("../data/Yields_Phi_SE_%s_%s_%s_Phi.root",vmsa::mBeamEnergy[energy].c_str(),date.c_str(),etamode.c_str());
  ...
  string InPutFile_ME = Form("../data/Yields_Phi_ME_%s_%s_%s_Phi.root",vmsa::mBeamEnergy[energy].c_str(),date.c_str(),etamode.c_str());
  ...
  for(Int_t i_pt = 5; i_pt <= 16; i_pt++) // pt bin 
  {
  ...
  TCanvas *c_pT = new TCanvas("c_pT","c_pT",10,10,1600,1200);
  c_pT->Divide(4,3);
  ...
  {
    for(int i_pt = 5; i_pt <= 16; i_pt++) // pt loop
    {
      c_pT->cd(i_pt-4);
      c_pT->cd(i_pt-4)->SetLeftMargin(0.15);
      c_pT->cd(i_pt-4)->SetBottomMargin(0.15);
      c_pT->cd(i_pt-4)->SetTicks(1,1);
      c_pT->cd(i_pt-4)->SetGrid(0,0);
      ...
      if(itheta == 0 && i_pt == 5)
      {
        legend->AddEntry(h_mMass_SE[KEY_SE_QA],"SE","lep");
        legend->AddEntry(h_mMass_ME[KEY_ME_QA],"ME","f");
        legend->AddEntry(h_mMass_SM[KEY_SM_QA],"SE-ME","f");
      }
      legend->Draw("same");
  ...
  for(int i_cent = 9; i_cent < vmsa::Cent_stop; i_cent++) // Centrality loop
  {
	...
	for(Int_t i_sig = vmsa::nSigKaon_start; i_sig < 1/*vmsa::nSigKaon_stop*/; i_sig++)
	{
	  ...
	  for(int i_norm = vmsa::Norm_start; i_norm < 1/*vmsa::Norm_stop*/; ++i_norm)
	  {
	    for(int pt_bin = 2; pt_bin < 6; pt_bin++) // pt loop
	    {
  ...
  TCanvas *c_pT_rebin = new TCanvas("c_pT_rebin","c_pT_rebin",10,10,800,800);
  c_pT_rebin->Divide(2,2);
  ...
  for(int itheta = 0; itheta < 10; itheta++)
  {
    for(int i_pt = 2; i_pt < 6; i_pt++) // pt loop
    {
      ...
      h_mMass[KEY_QA]->SetTitle(Form("%1.1f<|cos(2#phi*-2#phi)|<%1.1f",float(itheta-5.)/5.,float(itheta-4.)/5.));
...
//#endif
  ...
  string outputfile = Form("../output/AuAu%s/%s/Cos2PhiStarPhi_InvMassSubBg_%s_Phi.root",vmsa::mBeamEnergy[energy].c_str(),vmsa::mPID[pid].c_str(),etamode.c_str());
  ...
  for(int i_pt = 2; i_pt < 6; i_pt++) // pt loop
  {
	...
	for(Int_t i_sig = vmsa::nSigKaon_start; i_sig < 1/*vmsa::nSigKaon_stop*/; i_sig++)
	{
	  ...
	  for(int i_norm = vmsa::Norm_start; i_norm < 1/*vmsa::Norm_stop*/; ++i_norm)
	  {
	    ...
	    {
	      string KEY = Form("pt_%d_Centrality_%d_Cos2PhiStarPhi_%d_%s_Dca_%d_Sig_%d_%s_Norm_%d",i_pt,i_cent,i_theta,EP[order-1].c_str(),i_dca,i_sig,vmsa::mPID[pid].c_str(),i_norm);
              //if(i_theta == 0) sum = (TH1F*) h_mMass[KEY]->Clone();//pttotal += h_mMass[KEY]->Integral(0,-1);
              //else sum->Add(h_mMass[KEY]);//pttotal += h_mMass[KEY]->Integral(0,-1);
	      h_mMass[KEY]->Write();
	    }
            cout << "i_pt = " << i_pt << ", yield = " << h_mMass_theta[KEY_theta]->Integral(0,-1) << endl;;
	  }
	}
      }
    }
  
    //cout << "i_pt = " << i_pt << ", pttotal = " << pttotal << endl;
    //cout << "i_pt = " << i_pt << ", pttotal = " << sum->Integral(0,-1) << endl;
  }
  File_OutPut->Close();
}
