...
void calSpinAlignmentSysPhiPhiPsi_Global_2D_OffDiag(int energy = 4, int pid = 0, int year = 0, bool random3D = false, int order = 2, string etamode = "eta1_eta1", int frameopt = 0)
{
  ...
  string inputfile = Form("../output/AuAu%s/%s/%s2DPhiPsi_InvMassSubBg_%s.root",vmsa::mBeamEnergy[energy].c_str(),vmsa::mPID[pid].c_str(),frame.c_str(),etamode.c_str());
  if(order == 1) inputfile = Form("../output/AuAu%s/%s/%s2DPhiPsi_InvMassSubBg_%s_FirstOrder.root",vmsa::mBeamEnergy[energy].c_str(),vmsa::mPID[pid].c_str(),frame.c_str(),etamode.c_str());
  ...
  for(int i_phipsi = 0; i_phipsi < 6; i_phipsi++) // pt loop
  {
      ...
      for(int i_cent = 0; i_cent < vmsa::cent_rebin_total_2D; i_cent++) // Centrality loop
      {
        for(Int_t i_dca = vmsa::Dca_start; i_dca < 1/*vmsa::Dca_stop*/; i_dca++)
        {
          for(Int_t i_sig = vmsa::nSigKaon_start; i_sig < 1/*vmsa::nSigKaon_stop*/; i_sig++)
          {
            ...
            {
              string KEY_InteTheta = Form("pt_%d_phipsi_%d_Centrality_%d_%s_Dca_%d_Sig_%d_%s_Norm_%d",i_pt,i_phipsi,i_cent,EP[order-1].c_str(),i_dca,i_sig,vmsa::mPID[pid].c_str(),i_norm);
              ...
              {
                string KEY_InteTheta_Poly = Form("pt_%d_phipsi_%d_Centrality_%d_%s_Dca_%d_Sig_%d_%s_Norm_%d_Poly%d",i_pt,i_phipsi,i_cent,EP[order-1].c_str(),i_dca,i_sig,vmsa::mPID[pid].c_str(),i_norm,i_poly+1);
                ...
                //if(i_phipsi == vmsa::pt_rebin_first[energy]) g_mChiNDF[KEY_Poly] = new TGraphAsymmErrors();             
                //if(i_phipsi == vmsa::pt_rebin_first[energy]) g_mPValue[KEY_Poly] = new TGraphAsymmErrors();             
            ...
            for(int i_sigma = vmsa::Sig_start; i_sigma < 1/*vmsa::Sig_stop*/; ++i_sigma)
            {
              for(int i_method = 1/*vmsa::Method_start*/; i_method < vmsa::Method_stop; ++i_method)
              {
                for(int i_poly = 0; i_poly < 1/*3*/; i_poly++)
                {
                  ...
                  for(int i_phipsi = 0; i_phipsi < 6; i_phipsi++) // pt loop
                  {
                    string KEY_counts = Form("pt_%d_phipsi_%d_Centrality_%d_%s_Dca_%d_Sig_%d_%s_Norm_%d_Sigma_%d_%s_Poly%d",i_pt,i_phipsi,i_cent,EP[order-1].c_str(),i_dca,i_sig,vmsa::mPID[pid].c_str(),i_norm,i_sigma,vmsa::mInteMethod[i_method].c_str(),i_poly+1);
                    ...
                    for(int i_phi = vmsa::CTS_start; i_phi < 12/*2*vmsa::CTS_stop*/; ++i_phi) // cos(theta*) loop
                    {
                      ...
                      {
                        string KEY = Form("pt_%d_phipsi_%d_Centrality_%d_CosThetaStar_%d_Phi_%d_%s_Dca_%d_Sig_%d_%s_Norm_%d",i_pt,i_phipsi,i_cent,i_thetah,i_phi,EP[order-1].c_str(),i_dca,i_sig,vmsa::mPID[pid].c_str(),i_norm);
                        string KEY_Poly = Form("pt_%d_phipsi_%d_Centrality_%d_CosThetaStar_%d_Phi_%d_%s_Dca_%d_Sig_%d_%s_Norm_%d_Poly%d",i_pt,i_phipsi,i_cent,i_thetah,i_phi,EP[order-1].c_str(),i_dca,i_sig,vmsa::mPID[pid].c_str(),i_norm,i_poly+1);
                        TF1 *f_bw;
                        if(i_poly == 0) f_bw = new TF1("f_bw",PolyBreitWigner, vmsa::BW_Start[pid],vmsa::BW_Stop[pid],5);
                        if(i_poly == 1) f_bw = new TF1("f_bw",Poly2BreitWigner,vmsa::BW_Start[pid],vmsa::BW_Stop[pid],6);
                        if(i_poly == 2) f_bw = new TF1("f_bw",Poly3BreitWigner,vmsa::BW_Start[pid],vmsa::BW_Stop[pid],7);
                        ...
                        if(!result.Get()) 
                        {
                          skip[i_pt][i_cent][i_phipsi][i_thetah][i_phi] = true;
                    ...
                    //float pt_mean = float(float(i_phipsi)-4.5)/5.0;
                    //float pt_mean = (vmsa::eta_bin[i_phipsi]+vmsa::eta_bin[i_phipsi+1])/2.0;
                    float pt_mean = (i_phipsi+0.5)*TMath::Pi()/6.0;
                    ...
                    g_mRho[KEY_rho]->SetPoint(i_phipsi,pt_mean,f_rho->GetParameter(0)-1./3.);
                    g_mRho[KEY_rho]->SetPointError(i_phipsi,0.0,0.0,f_rho->GetParError(0),f_rho->GetParError(0));
                    g_mReal[KEY_real]->SetPoint(i_phipsi,pt_mean,f_rho->GetParameter(1));
                    g_mReal[KEY_real]->SetPointError(i_phipsi,0.0,0.0,f_rho->GetParError(1),f_rho->GetParError(1));
                    g_mImag[KEY_imag]->SetPoint(i_phipsi,pt_mean,f_rho->GetParameter(2));
                    g_mImag[KEY_imag]->SetPointError(i_phipsi,0.0,0.0,f_rho->GetParError(2),f_rho->GetParError(2));
                    g_mReRho1n1[KEY_rerho1n1]->SetPoint(i_phipsi,pt_mean,f_rho->GetParameter(3));
                    g_mReRho1n1[KEY_rerho1n1]->SetPointError(i_phipsi,0.0,0.0,f_rho->GetParError(3),f_rho->GetParError(3));
                    g_mImRho1n1[KEY_imrho1n1]->SetPoint(i_phipsi,pt_mean,f_rho->GetParameter(4));
                    g_mImRho1n1[KEY_imrho1n1]->SetPointError(i_phipsi,0.0,0.0,f_rho->GetParError(4),f_rho->GetParError(4));
    ...
    for(int i_norm = 0; i_norm < 1/*3*/; i_norm++)
    {
      for(int i_pt = 0; i_pt < vmsa::pt_rebin_y_2D; i_pt++)
      {
        for(int i_cent = 0; i_cent < vmsa::cent_rebin_total_2D; ++i_cent) // Centrality loop
        {
          for(int i_pt = 0; i_pt < vmsa::pt_rebin_y_2D; i_pt++)
          {
            string outputname = Form("./figures/%s/%s/phipsistudy/Global2D_allThetaYields%s_%s_Order%d_cent%d_pt%d_%s_pTdependence_Norm%d_Poly%d.pdf",vmsa::mPID[pid].c_str(),vmsa::mBeamEnergy[energy].c_str(),vmsa::mPID[pid].c_str(),vmsa::mBeamEnergy[energy].c_str(),order,i_cent,i_pt,etamode.c_str(),i_norm,i_poly+1);
            ...
            for(int i_phipsi = 0; i_phipsi < 6; i_phipsi++) // pt loop
            {
              ...
              { 
                ...
                {
                  cout << "i_cent = " << i_cent << ", i_phipsi = " << i_phipsi << ", i_thetah = " << i_thetah << ", i_phi = " << i_phi << ", skip? " << skip[i_pt][i_cent][i_phipsi][i_thetah][i_phi] << endl;
                  if(skip[i_pt][i_cent][i_phipsi][i_thetah][i_phi]) continue;
                  ...
                  if(i_thetah < 9)
                  {
                    string KEY_QA = Form("pt_%d_phipsi_%d_Centrality_%d_CosThetaStar_%d_Phi_%d_%s_Dca_%d_Sig_%d_%s_Norm_%d",i_pt,i_phipsi,i_cent,i_thetah,i_phi,EP[order-1].c_str(),vmsa::Dca_start,0/*vmsa::nSigKaon_start*/,vmsa::mPID[pid].c_str(),i_norm);
                    //cout << KEY_QA << endl;
                    ...
                    h_mMass[KEY_QA]->SetTitle(Form("%d#pi/6<#phi-#Psi_{2}<%d#pi/6 phiH=%d, CTH=%d",i_phipsi,i_phipsi+1,i_phi,i_thetah));
                    ...
                    //string KEY_InteTheta = Form("pt_%d_phipsi_%d_Centrality_%d_%s_Dca_%d_Sig_%d_%s_Norm_%d",i_phipsi,i_cent,EP[order-1].c_str(),vmsa::Dca_start,1/*vmsa::nSigKaon_start*/,vmsa::mPID[pid].c_str(),vmsa::Norm_QA);
                  ...
                  //if(i_thetah == 10)
                  //{
                  //  string KEY_InteTheta_QA = Form("pt_%d_phipsi_%d_Centrality_%d_%s_Dca_%d_Sig_%d_%s_Norm_%d",i_pt,i_phipsi,i_cent,EP[order-1].c_str(),vmsa::Dca_start,0/*vmsa::nSigKaon_start*/,vmsa::mPID[pid].c_str(),i_norm);
                  //  string KEY_InteTheta_QA_Poly = Form("pt_%d_phipsi_%d_Centrality_%d_%s_Dca_%d_Sig_%d_%s_Norm_%d_Poly%d",i_pt,i_phipsi,i_cent,EP[order-1].c_str(),vmsa::Dca_start,0/*vmsa::nSigKaon_start*/,vmsa::mPID[pid].c_str(),i_norm,i_poly+1);
                  //  h_mMass_InteTheta[KEY_InteTheta_QA]->Print();
                  //  h_mMass_InteTheta[KEY_InteTheta_QA]->SetTitle("");
                  //  h_mMass_InteTheta[KEY_InteTheta_QA]->SetTitle(Form("%d#pi/6<#phi-#Psi_{2}<%d#pi/6, Full phiH, Full CTG",i_phipsi,i_phipsi+1));
                ...
                for(int i_thetah = 0; i_thetah < 9/*&2*vmsa::CTS_stop*/; ++i_thetah)
                {
                  if(skip[i_cent][i_phipsi][i_thetah][i_phi]) continue;
                  c_diff->cd(i_thetah+1);
                  string KEY_QA = Form("pt_%d_phipsi_%d_Centrality_%d_CosThetaStar_%d_Phi_%d_%s_Dca_%d_Sig_%d_%s_Norm_%d",i_pt,i_phipsi,i_cent,i_thetah,i_phi,EP[order-1].c_str(),vmsa::Dca_start,0/*vmsa::nSigKaon_start*/,vmsa::mPID[pid].c_str(),i_norm);
                  string KEY_QA_Poly = Form("pt_%d_phipsi_%d_Centrality_%d_CosThetaStar_%d_Phi_%d_%s_Dca_%d_Sig_%d_%s_Norm_%d_Poly%d",i_pt,i_phipsi,i_cent,i_thetah,i_phi,EP[order-1].c_str(),vmsa::Dca_start,0/*vmsa::nSigKaon_start*/,vmsa::mPID[pid].c_str(),i_norm,i_poly+1);
                  cout << KEY_QA << endl;
              ...
              for(int i_sigma = 0/*vmsa::Sig_start*/; i_sigma < 1/*vmsa::Sig_stop*/; ++i_sigma)
              {
                ...
                {
                  string KEY_counts_QA = Form("pt_%d_phipsi_%d_Centrality_%d_%s_Dca_%d_Sig_%d_%s_Norm_%d_Sigma_%d_%s_Poly%d",i_pt,i_phipsi,i_cent,EP[order-1].c_str(),vmsa::Dca_start,0/*vmsa::nSigKaon_start*/,vmsa::mPID[pid].c_str(),i_norm,i_sigma,vmsa::mInteMethod[i_method].c_str(),i_poly+1);
                  cout << KEY_counts_QA << endl;
                  h_mCounts[KEY_counts_QA]->SetStats(0);
                  h_mCounts[KEY_counts_QA]->Print();
                  h_mCounts[KEY_counts_QA]->SetTitle(Form("Raw Yields, %d#pi/6<#phi-#Psi_{2}<%d#pi/6",i_phipsi,i_phipsi+1));
  ...
  h_frame->GetYaxis()->SetTitle("#Delta#rho_{00}");
  ...
  string KEY_rho = Form("rhoRaw_pt_%d_Centrality_%d_%s_Dca_%d_Sig_%d_%s_Norm_%d_Sigma_%d_%s_Poly%d",0,0,EP[order-1].c_str(),          0,0,vmsa::mPID[pid].c_str(),0,0,vmsa::mInteMethod[1].c_str(),1);
  string KEY_real = Form("realRaw_pt_%d_Centrality_%d_%s_Dca_%d_Sig_%d_%s_Norm_%d_Sigma_%d_%s_Poly%d",0,0,EP[order-1].c_str(),        0,0,vmsa::mPID[pid].c_str(),0,0,vmsa::mInteMethod[1].c_str(),1);
  string KEY_imag = Form("imagRaw_pt_%d_Centrality_%d_%s_Dca_%d_Sig_%d_%s_Norm_%d_Sigma_%d_%s_Poly%d",0,0,EP[order-1].c_str(),        0,0,vmsa::mPID[pid].c_str(),0,0,vmsa::mInteMethod[1].c_str(),1);
  string KEY_rerho1n1 = Form("rerho1n1Raw_pt_%d_Centrality_%d_%s_Dca_%d_Sig_%d_%s_Norm_%d_Sigma_%d_%s_Poly%d",0,0,EP[order-1].c_str(),0,0,vmsa::mPID[pid].c_str(),0,0,vmsa::mInteMethod[1].c_str(),1);
  string KEY_imrho1n1 = Form("imrho1n1Raw_pt_%d_Centrality_%d_%s_Dca_%d_Sig_%d_%s_Norm_%d_Sigma_%d_%s_Poly%d",0,0,EP[order-1].c_str(),0,0,vmsa::mPID[pid].c_str(),0,0,vmsa::mInteMethod[1].c_str(),1);
  ...
  g_mRho[KEY_rho]->GetXaxis()->SetTitle("#phi-#Psi_{2}");
  g_mRho[KEY_rho]->SetMarkerStyle(20); 
  g_mRho[KEY_rho]->Draw("APE");
  PlotLine(0.0,TMath::Pi(),0.0,0.0,1,1,2);
  ...
  g_mReal[KEY_real]->GetXaxis()->SetTitle("#phi-#Psi_{2}");
  ...
  g_mImag[KEY_imag]->GetXaxis()->SetTitle("#phi-#Psi_{2}");
  ...
  g_mReRho1n1[KEY_rerho1n1]->GetXaxis()->SetTitle("#phi-#Psi_{2}");
  ...
  g_mImRho1n1[KEY_imrho1n1]->GetXaxis()->SetTitle("#phi-#Psi_{2}");
  g_mImRho1n1[KEY_imrho1n1]->SetMarkerStyle(20); 
  g_mImRho1n1[KEY_imrho1n1]->Draw("APE");
  PlotLine(0.0,TMath::Pi(),0.0,0.0,1,1,2);

  string outputname = Form("./figures/%s/%s/phipsistudy/Global2D_ExtractedSpinDensityElements_%s_Order%d.pdf",vmsa::mPID[pid].c_str(),vmsa::mBeamEnergy[energy].c_str(),vmsa::mBeamEnergy[energy].c_str(),order);
  ...
  string outputfile = Form("../output/AuAu%s/%s/Global2DPhiPsi_RawPhiPhiPsiSys_%s.root",vmsa::mBeamEnergy[energy].c_str(),vmsa::mPID[pid].c_str(),etamode.c_str());
  if(order == 1) outputfile = Form("../output/AuAu%s/%s/Global2DPhiPsi_RawPhiPhiPsiSys_%s_PolySys_FirstOrder.root",vmsa::mBeamEnergy[energy].c_str(),vmsa::mPID[pid].c_str(),etamode.c_str());
