...
void calSpinAlignmentSysPhiRapidity_Helicity_2D_OffDiag(int energy = 4, int pid = 0, int year = 0, bool random3D = false, int order = 2, string etamode = "eta1_eta1")
{
  ...
  string inputfile = Form("../output/AuAu%s/%s/Helicity2DRapidity_InvMassSubBg_%s.root",vmsa::mBeamEnergy[energy].c_str(),vmsa::mPID[pid].c_str(),etamode.c_str());
  ...
  for(int  i_eta = 0; i_eta < 10; i_eta++) // pt loop
  {
	...
	for(Int_t i_sig = vmsa::nSigKaon_start; i_sig < vmsa::nSigKaon_stop; i_sig++)
	{
	  ...
	  {
	    string KEY_InteTheta = Form("eta_%d_Centrality_%d_%s_Dca_%d_Sig_%d_%s_Norm_%d",i_eta,i_cent,EP[order-1].c_str(),i_dca,i_sig,vmsa::mPID[pid].c_str(),i_norm);
	    for(int i_phi = vmsa::CTS_start; i_phi < 12/*2*vmsa::CTS_stop*/; i_phi++) // cos(theta*) loop
	    {
	      for(int i_thetah = vmsa::CTS_start; i_thetah < 9/*2*vmsa::CTS_stop*/; i_thetah++) // cos(theta*) loop
	      {
            ...
            {
	      string KEY_InteTheta_Poly = Form("eta_%d_Centrality_%d_%s_Dca_%d_Sig_%d_%s_Norm_%d_Poly%d",i_eta,i_cent,EP[order-1].c_str(),i_dca,i_sig,vmsa::mPID[pid].c_str(),i_norm,i_poly+1);
	    ...
	    for(int i_method = 1/*vmsa::Method_start*/; i_method < vmsa::Method_stop; ++i_method)
	    {
	        ...
	        for(int  i_eta = 0; i_eta < 10; ++i_eta) // pt loop
	        {
	          ...
	          for(int i_phi = vmsa::CTS_start; i_phi < 12/*2*vmsa::CTS_stop*/; ++i_phi) // cos(theta*) loop
	          {
	            ...
	            {
	              string KEY = Form("eta_%d_Centrality_%d_CosThetaStar_%d_Phi_%d_%s_Dca_%d_Sig_%d_%s_Norm_%d",i_eta,i_cent,i_thetah,i_phi,EP[order-1].c_str(),i_dca,i_sig,vmsa::mPID[pid].c_str(),i_norm);
	              string KEY_Poly = Form("eta_%d_Centrality_%d_CosThetaStar_%d_Phi_%d_%s_Dca_%d_Sig_%d_%s_Norm_%d_Poly%d",i_eta,i_cent,i_thetah,i_phi,EP[order-1].c_str(),i_dca,i_sig,vmsa::mPID[pid].c_str(),i_norm,i_poly+1);
	              TF1 *f_bw;
                      if(i_poly == 0) f_bw = new TF1("f_bw",PolyBreitWigner, vmsa::BW_Start[pid],vmsa::BW_Stop[pid],5);
                      if(i_poly == 1) f_bw = new TF1("f_bw",Poly2BreitWigner,vmsa::BW_Start[pid],vmsa::BW_Stop[pid],6);
                      if(i_poly == 2) f_bw = new TF1("f_bw",Poly3BreitWigner,vmsa::BW_Start[pid],vmsa::BW_Stop[pid],7);
	          ...
	          g_mRho[KEY_rho]->SetPoint(i_eta,pt_mean,f_rho->GetParameter(0));
  ...
  for(int i_poly = 0; i_poly < 1/*3*/; i_poly++)
  {
    for(int i_norm = 0; i_norm < 1/*3*/; i_norm++)
    {
      ...
      {
        string outputname = Form("./figures/%s/%s/rapiditystudy/Helicity2D_allThetaYields%s_%s_Order%d_cent%d_%s_pTdependence_Norm%d_Poly%d.pdf",vmsa::mPID[pid].c_str(),vmsa::mBeamEnergy[energy].c_str(),vmsa::mPID[pid].c_str(),vmsa::mBeamEnergy[energy].c_str(),order,i_cent,etamode.c_str(),i_norm,i_poly+1);
          ...
          { 
            ...
            for(int i_thetah = 0; i_thetah < 12; ++i_thetah)
            {
              ...
              if(i_thetah < 9)
              {
                ...
                h_mMass[KEY_QA]->SetTitle(Form("%.2f<y<%.2f phiH=%d, CTH=%d",(float(i_eta)-5.)/5.,(float(i_eta)-4.)/5.,i_phi,i_thetah));
              ...
              if(i_thetah == 9)
              {
                ...
                h_mMass_InteTheta[KEY_InteTheta_QA]->SetTitle(Form("%.2f<y<%.2f, Full phiH, Full CTG",(float(i_eta)-5.)/5.,(float(i_eta)-4.)/5.));
            ...
            //    h_mCounts[KEY_counts_QA]->GetYaxis()->SetTitle("#phi_{helicity}");
  ...
  string outputname = Form("./figures/%s/%s/rapiditystudy/Helicity2D_ExtractedSpinDensityElements_%s_Order%d.pdf",vmsa::mPID[pid].c_str(),vmsa::mBeamEnergy[energy].c_str(),vmsa::mBeamEnergy[energy].c_str(),order);
  ...
  string outputfile = Form("../output/AuAu%s/%s/Helicity2DRapidity_RawPhiYSys_%s.root",vmsa::mBeamEnergy[energy].c_str(),vmsa::mPID[pid].c_str(),etamode.c_str());
