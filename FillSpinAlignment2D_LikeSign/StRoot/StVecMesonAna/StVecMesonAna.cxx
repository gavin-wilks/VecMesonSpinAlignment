...
void StVecMesonAna::MakePhi()
{
  ...
  for(int i_event = 0; i_event < numOfEvents; ++i_event)
  {
    ...
    if(mVecMesonCorr->passTrackEtaNumCut(NumTrackEast,NumTrackWest))
    {
      ...
      std::vector<StMesonTrack*> LikeSignPairs; 

      std::vector<TLorentzVector> posKaons; 
      std::vector<float> posKaonMass;
      std::vector<float> posKaonNsig;
      std::vector<float> posKaonDca;
      std::vector<TLorentzVector> negKaons; 
      std::vector<float> negKaonMass;
      std::vector<float> negKaonNsig;
      std::vector<float> negKaonDca;
	...
	flagB = mMeson_track->getFlagB();
        ...
        if(fabs(lTrackA.Eta()) < 1.0 && fabs(nsA) < 1.0 && fabs(dcaA) < 2.0)
        { 
          posKaons.push_back(lTrackA);       
          posKaonMass.push_back(m2A);       
          posKaonNsig.push_back(nsA);       
          posKaonDca.push_back(dcaA);       
        }
      
        if(fabs(lTrackB.Eta()) < 1.0 && fabs(nsB) < 1.0 && fabs(dcaB) < 2.0)
        { 
          negKaons.push_back(lTrackB);
          negKaonMass.push_back(m2B);
          negKaonNsig.push_back(nsB);
          negKaonDca.push_back(dcaB);
        }
      }


      for(UShort_t nTracks1 = 0; nTracks1 < posKaons.size(); nTracks1++) // loop over all tracks of the actual event
      {
        for(UShort_t nTracks2 = 0; nTracks2 < posKaons.size(); nTracks2++) // loop over all tracks of the actual event
        {
          TLorentzVector kaonp1 = posKaons[nTracks1];
          TLorentzVector kaonp2 = posKaons[nTracks2];

          float m2p1 = posKaonMass[nTracks1];
          float m2p2 = posKaonMass[nTracks2];
          
          float nsp1 = posKaonNsig[nTracks1];
          float nsp2 = posKaonNsig[nTracks2];

          float dcap1 = posKaonDca[nTracks1];
          float dcap2 = posKaonDca[nTracks2];
 
          //if(kaonp1 != kaonp2 || m2p1 != m2p2 || dcap1 != dcap2 || kaonp1 != kaonp2)  
          if(fabs(kaonp1.Px()-kaonp2.Px()) < tolerance && fabs(kaonp1.Py()-kaonp2.Py()) < tolerance &&
             fabs(kaonp1.Pz()-kaonp2.Pz()) < tolerance && fabs(kaonp1.E()-kaonp2.E())   < tolerance)  
          {
            StMesonTrack *posMeson = new StMesonTrack();
            
            posMeson->setMass2A(m2p1);
            posMeson->setMass2B(m2p2);
            posMeson->setNSigA(nsp1);
            posMeson->setNSigB(nsp2);
            posMeson->setDcaA(dcap1);
            posMeson->setDcaB(dcap2);
            posMeson->setTrackA(kaonp1);
            posMeson->setTrackB(kaonp2);

            LikeSignPairs.push_back(posMeson);
          }   
        }
      }
 
      //for(TLorentzVector* ptr : posKaons) delete ptr; // free memory
      ...
      for(UShort_t nTracks1 = 0; nTracks1 < negKaons.size(); nTracks1++) // loop over all tracks of the actual event
      {
        for(UShort_t nTracks2 = 0; nTracks2 < negKaons.size(); nTracks2++) // loop over all tracks of the actual event
        {
          TLorentzVector kaonn1 = negKaons[nTracks1];
          TLorentzVector kaonn2 = negKaons[nTracks2];
          
          float m2n1 = negKaonMass[nTracks1];
          float m2n2 = negKaonMass[nTracks2];

          float nsn1 = negKaonNsig[nTracks1];
          float nsn2 = negKaonNsig[nTracks2];

          float dcan1 = negKaonDca[nTracks1];
          float dcan2 = negKaonDca[nTracks2];

          //if(kaonn1 != kaonn2 || m2n1 != m2n2 || dcan1 != dcan2 || kaonn1 != kaonn2)  
          if(fabs(kaonn1.Px()-kaonn2.Px()) < tolerance && fabs(kaonn1.Py()-kaonn2.Py()) < tolerance && 
             fabs(kaonn1.Pz()-kaonn2.Pz()) < tolerance && fabs(kaonn1.E()-kaonn2.E())   < tolerance)  
          {
            StMesonTrack *negMeson = new StMesonTrack();
            
            negMeson->setMass2A(m2n1);
            negMeson->setMass2B(m2n2);
            negMeson->setNSigA(nsn1);
            negMeson->setNSigB(nsn2);
            negMeson->setDcaA(dcan1);
            negMeson->setDcaB(dcan2);
            negMeson->setTrackA(kaonn1);
            negMeson->setTrackB(kaonn2);

            LikeSignPairs.push_back(negMeson);
          }   
        } 
      }

      //for(TLorentzVector* ptr : negKaons) delete ptr; // free memory
      ...
      for(UShort_t nTracks = 0; nTracks < LikeSignPairs.size(); nTracks++) // loop over all tracks of the actual event
      {
	mMeson_track = LikeSignPairs[nTracks];
	m2A = mMeson_track->getMass2A();
	m2B = mMeson_track->getMass2B();
	nsA = mMeson_track->getNSigA();
	nsB = mMeson_track->getNSigB();
        ...
        //  )
	{
	  ...
	  //Float_t rapidity_lTrackOriginal = lTrackOriginal.Rapidity();
          ...
          //cout << "K+ eta = " << eta_TrackA << endl;        
          //cout << "K- eta = " << eta_TrackB << endl;        
	  ...
	  //lKmRest.Boost(vBetaPhi); // boost K- back to phi rest frame
          //if(randomval<0.5)lKmRest.RotateZ(TMath::Pi());
          //lKmRest.Boost(-vBetaPhi);

	  //TLorentzVector lKpRest = lTrackA;
	  //lKpRest.Boost(vBetaPhi); // boost K- back to phi rest frame
          //if(randomval>=0.5)lKpRest.RotateZ(TMath::Pi());
          //lKpRest.Boost(-vBetaPhi);

          //TLorentzVector lTrackRotated = lKmRest + lKpRest;

          //double InvMass_lTrackRotated = lTrackRotated.M();

          //cout << "InvMass = " << InvMass_lTrack << ", InvMassRotated = " << InvMass_lTrackRotated << endl;
...
//	        mVecMesonHistoManger->FillSys(pt_lTrack,rapidity_lTrack,phi_lTrack,cent9,0.0,0.0,0.0,0.0,i_dca,i_sig,1.0,InvMass_lTrackRotated,reweight,mX_flag,mMode);

		//if(mX_flag == 0) mVecMesonHistoManger->FillSys(pt_lTrack,rapidity_lTrack,phi_lTrack,cent9,0.0,0.0,0.0,0.0,i_dca,i_sig,1.0,InvMass_lTrack,reweight,mX_flag,mMode);
		//if(mX_flag == 1) mVecMesonHistoManger->FillSys(pt_lTrack,rapidity_lTrack,phi_lTrack,cent9,0.0,0.0,0.0,0.0,i_dca,i_sig,1.0,InvMass_lTrack,weightA*weightB,mX_flag,mMode);
      ...
      for(StMesonTrack* ptr : LikeSignPairs) delete ptr; // free memory
      LikeSignPairs.clear(); 
