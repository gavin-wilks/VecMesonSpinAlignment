...
void plotEventPlane(int beamEnergy = 0)
{
  ...
  HistName = "h_mEastRaw";
  h_mTpcRawEpEast = (TH1F*)File_InPutRaw->Get(HistName.c_str());
  HistName = "h_mWestRaw";
  h_mTpcRawEpWest = (TH1F*)File_InPutRaw->Get(HistName.c_str());
  HistName = "h_mFullRaw";
  h_mTpcRawEpFull = (TH1F*)File_InPutRaw->Get(HistName.c_str());
 ...
 cout << "Are these loaded in" << endl;
  ...
  legE = new TLegend(0.2,0.6,0.5,0.8);
  ...
  legW = new TLegend(0.2,0.6,0.5,0.8);
