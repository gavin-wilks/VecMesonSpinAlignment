...
void plotEpdResolution(int beamEnergy = 0)
{
  ...
  TGraphAsymmErrors *g_mTpcSubRes2[5];
  TFile *File_InPut[5];
  
  int energies[3] = {0,3,4};
  for(int idx = 0; idx < 3; idx++)
  {
    int i = energies[idx];
    std::string inputfile;
    if(i == 0)inputfile = Form("../StRoot/Utility/EpdResolution/Resolution_file_%s_EpdCorrections_6.root",vmsa::mBeamEnergy[i].c_str());
    if(i == 3)inputfile = Form("../StRoot/Utility/EpdResolution/Resolution_file_%s_EpdCorrections_6.root",vmsa::mBeamEnergy[i].c_str());
    if(i == 4)inputfile = Form("../StRoot/Utility/EpdResolution/Resolution_file_%s_EpdCorrections_4.root",vmsa::mBeamEnergy[i].c_str());
  ...
  TF1 *f_res = new TF1("f_res",ResolutionFull,0,80,0);
  ...
  */
  ...
  double mTpcSubRes2Val[5][9];
  double mTpcSubRes2Err[5][9];
 ...
 // TGraphAsymmErrors *g_mTpcFullRes2 = new TGraphAsymmErrors();
  ...
  cout << "TPC Event Plane Resolution:" << endl;
  cout << "Sub Event Plane:" << endl;
  TProfile *p_mTpcSubRes2[5];
  for(int idx = 0; idx < 3; idx++)
  {
    int i = energies[idx];
    p_mTpcSubRes2[i] = (TProfile*)File_InPut[i]->Get("AveCosDeltaPsi1");
  }
  ...
  for(int idx = 0; idx < 3; idx++)
  {
    ...
    for(int i_cent = 0; i_cent < 9; ++i_cent)
    {
      ...
      if(resRaw > 0)
      {
        Double_t CosMean = resRaw; // load from resolution file
        Double_t CosError = errRaw;
      ...
      cout << "i_cent = " << i_cent << ", resRaw = " << resRaw << ", resSub = " << ResZ << " +/- " << ResErrZ << endl;
      g_mTpcSubRes2[i]->SetPoint(i_cent,50.0*(Centrality_start[i_cent]+Centrality_stop[i_cent]),ResZ);
      g_mTpcSubRes2[i]->SetPointError(i_cent,0.0,0.0,ResErrZ,ResErrZ);
...
*/
  ...
  g_mTpcSubRes2[0]->SetMarkerStyle(20);
  g_mTpcSubRes2[0]->SetMarkerColor(kBlack);
  g_mTpcSubRes2[0]->SetMarkerSize(2);
  g_mTpcSubRes2[0]->Draw("pE Same");
  ...
  leg->AddEntry(g_mTpcSubRes2[0],"7.7 GeV EPD","p");
  leg->AddEntry(g_mTpcSubRes2[3],"14.6 GeV EPD","p");
  leg->AddEntry(g_mTpcSubRes2[4],"19.6 GeV EPD","p");
